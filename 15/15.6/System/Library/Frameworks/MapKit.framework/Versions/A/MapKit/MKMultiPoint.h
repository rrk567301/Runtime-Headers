...
@interface MKMultiPoint : MKShape <MKGeoJSONObject> {
    ...
    struct GEOOnce_s { struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } lock; char didRun; } _calculatedMapPointsLength;
    double _mapPointsLength;
    struct GEOOnce_s { struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } lock; char didRun; } _calculatedSelfIntersecting;
