@class NWEndpoint, NSString, NEIKEv2Packet, NEIKEv2IKESA, NSMutableDictionary, NEIKEv2RTT, NEIPSecSASession, NSObject, NSMutableArray, NEIKEv2IKESAConfiguration, NEIKEv2ChildSAConfiguration, NEIKEv2ConfigurationMessage;
...
@interface NEIKEv2Session : NSObject <NEIKEv2TransportDelegate, NEIPSecSASessionDelegate> {
    char _isListening;
    char _ownsDatabaseSession;
    char _ownsIPSecInterface;
    char _isAborted;
    char _isInvalidated;
    char _isExpectingConfigBlock;
    char _hasContactedConfigurationDelegate;
    char _hasOutboundRequestInFlight;
...
@property (readonly) char peerAuthenticated;
@property (readonly) char ppkAuthenticated;
...
- (char)checkSelectedChildSAProposal:(id)a0;
- (char)checkSelectedIKEProposal:(id)a0;
- (void)deleteSA:(id)a0;
- (void)expireSA:(id)a0;
...
- (void)invalidateWithCompletionHandler:(char)a0 completionHandler:(id /* block */)a1;
...
- (void)sendMOBIKEWithRetries:(unsigned int)a0 retryInterval:(unsigned long long)a1 interfaceName:(id)a2 invalidateTransport:(char)a3 resetEndpoint:(id)a4 callbackQueue:(id)a5 callback:(id /* block */)a6;
