...
@property char canStartRecording;
@property (getter=isCaptureSessionRunning) char captureSessionRunning;
...
@property (getter=isVideoPreviewEnabled) char videoPreviewEnabled;
@property (getter=isAudioPreviewEnabled) char audioPreviewEnabled;
@property (readonly) AVCaptureSession *session;
@property (readonly) char sessionHasBeenSet;
@property char wantsDefaultConfiguration;
...
@property (nonatomic) char audioPreviewMuted;
@property (readonly) char canAssociateAudioAndVideoDevices;
...
- (void)pauseRecording;
- (char)_associateItemsWithVideoInputSourceItems:(id)a0 audioInputSourceItems:(id)a1 outIndexOfBuiltInVideoItem:(unsigned long long *)a2 outIndexOfBuiltInAudioItem:(unsigned long long *)a3 outIndexOfExternalVideoItem:(unsigned long long *)a4 outIndexOfExternalAudioItem:(unsigned long long *)a5;
- (void)_createDefaultSessionAndFileOutputAsynchronously:(char)a0;
...
- (void)_setWirelessDeviceDiscoveryEnabled:(char)a0;
...
- (char)canHandleSelector:(SEL)a0 withEvent:(id)a1 shouldIgnoreSpaceKey:(char)a2;
...
- (void)setSession:(id)a0 videoPreviewEnabled:(char)a1 audioPreviewEnabled:(char)a2;
