...
@property struct { unsigned int internalWidth; unsigned int internalHeight; unsigned int radius; unsigned int subsampling; float epsilon; unsigned int iterations; char useDepthFilter; float fgThresholdValue; float bgThresholdValue; unsigned int erosionKernelSize; unsigned int resolutionMode; } config;

- (void).cxx_destruct;
- (int)_compileShadersWithLibrary:(id)a0;
- (int)allocateResources:(struct { unsigned int x0; unsigned int x1; unsigned int x2; unsigned int x3; float x4; unsigned int x5; char x6; float x7; float x8; unsigned int x9; unsigned int x10; })a0;
