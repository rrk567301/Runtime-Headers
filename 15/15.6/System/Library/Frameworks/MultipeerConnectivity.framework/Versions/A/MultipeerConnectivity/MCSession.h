...
@property (nonatomic, getter=isAWDLDisabled) char AWDLDisabled;
@property (nonatomic) char preferNCMOverEthernet;
...
@property (weak) id<MCSessionDelegate> delegate;
...
- (char)sendData:(id)a0 toPeers:(id)a1 withMode:(long long)a2 error:(id *)a3;
...
- (char)isEncryptionPreferenceCompatible:(long long)a0;
- (void)logSessionInfo;
...
- (void)setICETimeoutForced:(char)a0;
...
- (void)syncCloseIncomingStream:(id)a0 forPeer:(id)a1 state:(id)a2 error:(id)a3 reason:(int)a4 shouldTriggerCancelProgress:(char)a5;
- (void)syncCloseOutgoingStream:(id)a0 forPeer:(id)a1 state:(id)a2 error:(id)a3 shouldTriggerCancelProgress:(char)a4;
...
- (void)syncHandleNetworkEvent:(struct { int x0; char *x1; int x2; unsigned int x3; } *)a0 pid:(unsigned int)a1 freeEventWhenDone:(char *)a2;
...
- (void)syncPeer:(id)a0 changeStateTo:(long long)a1 shouldForceCallback:(char)a2;
...
- (void)syncStartStreamWithName:(id)a0 toPeer:(id)a1 mcFD:(int)a2 isResource:(char)a3;
