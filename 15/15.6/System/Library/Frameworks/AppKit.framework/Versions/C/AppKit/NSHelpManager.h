...
@property (class, getter=isContextHelpModeActive) char contextHelpModeActive;

+ (char)_activateHelpModeBasedOnEvent:(id)a0;
+ (char)_isCtrlAltForHelpDesired;
+ (char)_keyboardIsOldNeXT;
+ (void)_setCtrlAltForHelpDesired:(char)a0;

- (void)dealloc;
- (id)_auxiliaryHelpBookBundlesCreatingIfNeeded:(char)a0;
...
- (char)_orderOutHelpWindowAfterEventMask:(unsigned long long)a0;
