...
@interface NSPersistentUIManager : NSObject <NSPersistentUIFlushHandler, NSPersistentUIManagerCarbonMethods> {
    ...
    char _didInitialEmptying;
    char treatEverythingAsDirty;
    char _hadPersistentStateToRestoreAtLaunch;
    char _isWaiting;
}
...
- (void)changePersistentKeyPathObservationForPaths:(id)a0 inObject:(id)a1 to:(char)a2;
...
- (void)flushPersistentStateAndClose:(char)a0 waitingUntilDone:(char)a1;
...
- (char)restoreAllPersistentStateWithFullFidelity:(char)a0 completionHandler:(id /* block */)a1;
...
- (char)shouldRestoreWithFullFidelity;
- (char)shouldUseOneWindowHeuristic;
- (id)stateDirectoryAtLaunch;
- (id)windowInfoForWindowID:(unsigned int)a0 createIfNecessary:(char)a1;
