@class _NSDiffableDataSourceViewUpdater, _NSDiffableDataSourceState, NSArray, NSString, NSObject, NSCollectionView;
@protocol OS_dispatch_queue;

@interface __NSDiffableDataSource : NSObject <NSCollectionViewDataSource, __NSDiffableDataSourceItemAndSectionOperations> {
    NSCollectionView *_nsCollectionView;
}

@property (copy, nonatomic) id /* block */ nsCollectionViewItemReuseIdentifierProvider;
@property (copy, nonatomic) id /* block */ nsCollectionViewItemConfigurationHandler;
@property (retain, nonatomic) _NSDiffableDataSourceViewUpdater *viewUpdater;
@property (readonly, nonatomic) NSObject<OS_dispatch_queue> *applyQueue;
@property (readonly, nonatomic) _Atomic long long outstandingApplyCount;
@property (readonly, nonatomic) NSObject<OS_dispatch_queue> *snapshotQueue;
@property unsigned long long mutationQueueSource;
@property (retain, nonatomic) _NSDiffableDataSourceState *state;
@property (readonly, weak, nonatomic) NSCollectionView *nsCollectionView;
@property (readonly, weak, nonatomic) NSCollectionView *_nsCollectionView;
@property (copy, nonatomic) id /* block */ nsSupplementaryViewProvider;
@property (readonly, nonatomic) long long numberOfItems;
@property (readonly, nonatomic) long long numberOfSections;
@property (readonly, nonatomic) NSArray *sectionIdentifiers;
@property (readonly, nonatomic) NSArray *itemIdentifiers;
@property (readonly, nonatomic) NSArray *reloadedSectionIdentifiers;
@property (readonly, nonatomic) NSArray *reloadedItemIdentifiers;
@property (readonly, nonatomic) NSArray *reconfiguredItemIdentifiers;
@property (copy, nonatomic) id /* block */ collectionViewItemProvider;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (void).cxx_destruct;
- (id)snapshot;
- (void)_applyDifferencesFromSnapshot:(id)a0 animated:(char)a1 completion:(id /* block */)a2;
- (void)_applyDifferencesFromSnapshot:(id)a0 completion:(id /* block */)a1;
- (char)_canApplySnapshotUpdateWithoutDiffing:(id)a0;
- (void)_commitNewDataSource:(id)a0 withViewUpdates:(id)a1 animated:(char)a2 completion:(id /* block */)a3;
- (void)_commitStateAtomicallyWithIdentifiers:(id)a0 sections:(id)a1 dataSourceSnapshot:(id)a2 shouldAdvanceGenerationalUUID:(char)a3 shouldCopy:(char)a4;
- (void)_commitUpdate:(id)a0 animated:(char)a1 completion:(id /* block */)a2;
- (char)_isDiffableDataSource;
- (id)_itemAtIndexPath:(id)a0 nsCollectionView:(id)a1;
- (long long)_numberOfItemsInSection:(long long)a0 nsCollectionView:(id)a1;
- (long long)_numberOfSectionsForNSCollectionView:(id)a0;
- (id)_pendingReloadUpdatesForSnapshot:(id)a0;
- (id)_reloadViewUpdatesForDiffUpdate:(id)a0;
- (id)_reloadViewUpdatesForDiffUpdate:(id)a0 dataSource:(id)a1 ignoreInvalidItems:(char)a2;
- (id)_snapshotWithHandlerAtomic:(id /* block */)a0;
- (id)_viewForSupplementaryElementOfKind:(id)a0 atIndexPath:(id)a1 nsCollectionView:(id)a2;
- (void)appendItemsWithIdentifiers:(id)a0;
- (void)appendItemsWithIdentifiers:(id)a0 intoSectionWithIdentifier:(id)a1;
- (void)appendSectionWithIdentifier:(id)a0;
- (void)appendSectionsWithIdentifiers:(id)a0;
- (void)applyDifferencesFromSnapshot:(id)a0;
- (void)applyDifferencesFromSnapshot:(id)a0 animatingDifferences:(char)a1;
- (void)applyDifferencesFromSnapshot:(id)a0 animatingDifferences:(char)a1 completion:(id /* block */)a2;
- (void)applyDifferencesFromSnapshot:(id)a0 completion:(id /* block */)a1;
- (void)applySnapshot:(id)a0;
- (id)collectionView:(id)a0 itemForRepresentedObjectAtIndexPath:(id)a1;
- (long long)collectionView:(id)a0 numberOfItemsInSection:(long long)a1;
- (id)collectionView:(id)a0 viewForSupplementaryElementOfKind:(id)a1 atIndexPath:(id)a2;
- (void)deleteAllItems;
- (void)deleteItemsWithIdentifiers:(id)a0;
- (void)deleteSectionsWithIdentifiers:(id)a0;
- (id)emptySnapshot;
- (long long)indexForSectionIdentifier:(id)a0;
- (long long)indexOfItemIdentifier:(id)a0;
- (long long)indexOfSectionIdentifier:(id)a0;
- (id)indexPathForItemIdentifier:(id)a0;
- (id)initWithNSCollectionView:(id)a0 itemProvider:(id /* block */)a1;
- (id)initWithNSCollectionView:(id)a0 itemProvider:(id /* block */)a1 dataSource:(id)a2;
- (id)initWithNSCollectionView:(id)a0 itemProvider:(id /* block */)a1 reuseIdentifierProvider:(id /* block */)a2 itemConfigurationHandler:(id /* block */)a3 state:(id)a4 dataSource:(id)a5;
- (id)initWithNSCollectionView:(id)a0 reuseIdentifierProvider:(id /* block */)a1 itemConfigurationHandler:(id /* block */)a2;
- (id)initWithState:(id)a0;
- (id)initWithViewUpdater:(id)a0;
- (id)initWithViewUpdatesSink:(id)a0;
- (void)insertItemsWithIdentifiers:(id)a0 afterItemWithIdentifier:(id)a1;
- (void)insertItemsWithIdentifiers:(id)a0 beforeItemWithIdentifier:(id)a1;
- (void)insertSectionWithIdentifier:(id)a0 afterSectionWithIdentifier:(id)a1;
- (void)insertSectionWithIdentifier:(id)a0 beforeSectionWithIdentifier:(id)a1;
- (void)insertSectionsWithIdentifiers:(id)a0 afterSectionWithIdentifier:(id)a1;
- (void)insertSectionsWithIdentifiers:(id)a0 beforeSectionWithIdentifier:(id)a1;
- (id)itemIdentifierForIndexPath:(id)a0;
- (id)itemIdentifiersInSectionWithIdentifier:(id)a0;
- (void)moveItemWithIdentifier:(id)a0 afterItemWithIdentifier:(id)a1;
- (void)moveItemWithIdentifier:(id)a0 beforeItemWithIdentifier:(id)a1;
- (void)moveSectionWithIdentifier:(id)a0 afterSectionWithIdentifier:(id)a1;
- (void)moveSectionWithIdentifier:(id)a0 beforeSectionWithIdentifier:(id)a1;
- (long long)numberOfItemsInSection:(id)a0;
- (long long)numberOfSectionsInCollectionView:(id)a0;
- (void)reconfigureItemsWithIdentifiers:(id)a0;
- (void)reloadFromSnapshot:(id)a0;
- (void)reloadFromSnapshot:(id)a0 completion:(id /* block */)a1;
- (void)reloadItemsWithIdentifiers:(id)a0;
- (void)reloadSectionsWithIdentifiers:(id)a0;
- (id)sectionIdentifierForIndex:(long long)a0;
- (id)sectionIdentifierForSectionContainingItemIdentifier:(id)a0;

@end
