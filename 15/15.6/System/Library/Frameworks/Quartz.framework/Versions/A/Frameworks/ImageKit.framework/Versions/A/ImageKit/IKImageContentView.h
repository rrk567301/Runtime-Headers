...
@property char documentViewLayerSetupComplete;
@property char backgroundThumbnailLoadingComplete;
@property char scrollingLoadsTiles;
...
@property char ignoreWindowUpdates;
@property char isReloading;
@property (getter=isAnimatingImageContent) char animatingImageContent;
...
+ (char)_colorSpaceNeedsExtendedRangeSupport:(struct CGColorSpace { } *)a0;
+ (char)_dropsToSDRWhenBackgrounded;
...
- (void)setAutomaticallyShowVisualSearchResults:(char)a0;
...
- (void)_clearBackgroundLayerDelayed:(char)a0;
...
- (double)_imageScaleWithImageProperties:(id)a0 andDisplayProperties:(id)a1 usingLowResScaleFactorIfAvailable:(char)a2;
...
- (void)_resetLayersAndReloadLayersWithImageProperties:(id)a0 preserveAnnotations:(char)a1 displayProperties:(id)a2;
...
- (char)_setCIImage:(id)a0 imageProperties:(id)a1 displayProperties:(id)a2 preserveAnnotations:(char)a3;
- (void)_setEnclosingScrollViewScrollersCanShow:(char)a0;
- (void)_setEnclosingScrollViewScrollersCanShowIfAllowed:(char)a0;
- (char)_setImage:(struct CGImage { } *)a0 imageProperties:(id)a1 displayProperties:(id)a2 preserveAnnotations:(char)a3;
- (char)_setImageFromImageSource:(struct CGImageSource { } *)a0 imageAtIndex:(long long)a1 preserveAnnotations:(char)a2 displayProperties:(id)a3 completion:(id /* block */)a4;
- (char)_setImageWithSize:(struct CGSize { double x0; double x1; })a0 imageProperties:(id)a1 preserveAnnotations:(char)a2 displayProperties:(id)a3;
...
- (void)_updateVisionKitOverlayWithReanalysis:(char)a0;
- (void)_updatedHighlightedString;
- (char)_viewSupportsHDR;
...
- (char)automaticallyShowVisualSearchResults;
- (void)beginImageSubjectAnalysisIfNecessary;
- (id)borrowContent;
- (void)clearBlocksFromBlockCollection:(id)a0 didCancelDisplay:(char *)a1;
...
- (char)isSubjectHighlightAvailable;
