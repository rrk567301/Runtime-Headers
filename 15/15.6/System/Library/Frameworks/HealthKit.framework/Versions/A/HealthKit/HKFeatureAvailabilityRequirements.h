@interface HKFeatureAvailabilityRequirements : NSObject

+ (id)bloodOxygenRecordingsAreEnabled;
+ (id)heightIsPresent;
+ (id)currentCountryIsSupportedOnLocalDeviceForFeatureWithIdentifier:(id)a0;
+ (id)activeRemoteDeviceIsPresentWhenRequiredForRegionAvailabilityOrDeviceCapabilityForFeatureWithIdentifier:(id)a0;
+ (id)activeWatchIsNotUnderInternalDevelopmentImportExclusionForFeatureWithIdentifier:(id)a0;
+ (id)ageIsPresent;
+ (id)capabilityIsSupportedOnActiveRemoteDeviceForFeatureWithIdentifier:(id)a0;
+ (id)capabilityIsSupportedOnActiveWatchForFeatureWithIdentifier:(id)a0 supportedOnLocalDevice:(char)a1;
+ (id)capabilityIsSupportedOnAnyWatch:(id)a0 supportedOnLocalDevice:(char)a1;
+ (id)countryCodeIsPresentForFeatureWithIdentifier:(id)a0;
+ (id)countryIsSupportedOnActiveRemoteDeviceForFeatureWithIdentifier:(id)a0 isSupportedIfCountryListMissing:(char)a1;
+ (id)countryIsSupportedOnLocalDeviceAndPhoneForFeatureWithIdentifier:(id)a0 isSupportedIfCountryListMissing:(char)a1;
+ (id)countryIsSupportedOnLocalDeviceForFeatureWithIdentifier:(id)a0;
+ (id)countryIsSupportedOnWatchForFeatureWithIdentifier:(id)a0 isSupportedIfCountryListMissing:(char)a1;
+ (id)currentCountryCodeIsPresentForFeatureWithIdentifier:(id)a0;
+ (id)currentCountryIsSupportedOnActiveRemoteDeviceForFeatureWithIdentifier:(id)a0 isSupportedIfCountryListMissing:(char)a1;
+ (id)defaultHelpTileRequirementsForBackgroundDeliveredFeatureWithFeatureIdentifier:(id)a0 isAgeGatedUserDefaultsKey:(id)a1;
+ (id)defaultOnboardingEligibilityRequirementsForFeatureIdentifier:(id)a0;
+ (id)defaultTipsAppVisibilityRequirementsForBackgroundDeliveredFeatureWithFeatureIdentifier:(id)a0 isAgeGatedUserDefaultsKey:(id)a1;
+ (id)featureFlagIsEnabled:(char)a0;
+ (id)featureIsNotRemotelyDisabledWithIdentifier:(id)a0;
+ (id)featureIsOffWithIdentifier:(id)a0 isOffIfSettingIsAbsent:(char)a1;
+ (id)featureIsOffWithIdentifier:(id)a0 settingsKey:(id)a1 isOffIfSettingIsAbsent:(char)a2;
+ (id)featureIsOnWithIdentifier:(id)a0 isOnIfSettingIsAbsent:(char)a1;
+ (id)featureIsOnWithIdentifier:(id)a0 settingsKey:(id)a1 isOnIfSettingIsAbsent:(char)a2;
+ (id)fitnessTrackingIsEnabledInPrivacy;
+ (id)healthAppIsNotHidden;
+ (id)hearingAidCapabilityIsSupportedOnAnyPairedAirPods;
+ (id)hearingTestCapabilityIsSupportedOnAnyPairedAirPods;
+ (id)heartRateIsEnabledInPrivacy;
+ (id)isNotInPostPregnancy;
+ (id)localDeviceIsCapableOfPairingWithWatch;
+ (id)localDeviceIsCapableOfWatchPairing;
+ (id)mutuallyExclusiveFeatureIsOffWithIdentifier:(id)a0;
+ (id)noOngoingPregnancy;
+ (id)notAgeGatedForUserDefaultsKey:(id)a0;
+ (id)notInStoreDemoMode;
+ (id)notificationAuthorizedWithBundleIdentifier:(id)a0;
+ (id)notificationsOnForUserDefaultsKey:(id)a0;
+ (id)onboardedAtLeastDaysAgo:(long long)a0 featureIdentifier:(id)a1;
+ (id)onboardingAcknowledgedWithIdentifier:(id)a0;
+ (id)onboardingAcknowledgedWithIdentifier:(id)a0 settingsKey:(id)a1;
+ (id)onboardingNotAcknowledgedWithIdentifier:(id)a0;
+ (id)onboardingNotAcknowledgedWithIdentifier:(id)a0 settingsKey:(id)a1;
+ (id)onboardingRecordIsNotPresentForFeatureWithIdentifier:(id)a0;
+ (id)onboardingRecordIsNotPresentForMutuallyExclusiveFeatureWithIdentifier:(id)a0;
+ (id)onboardingRecordIsPresentForFeatureWithIdentifier:(id)a0;
+ (id)onboardingRecordsArePresentForPrerequisiteFeaturesWithIdentifiers:(id)a0;
+ (id)onboardingUserDefaultsKeyIsNotPresent:(id)a0;
+ (id)onboardingUserDefaultsKeyIsPresent:(id)a0;
+ (id)prerequisiteFeaturesAreOnWithFeatureSettings:(id)a0;
+ (id)profileIsNotFamilySetupPairingProfile;
+ (id)seedIsNotExpiredForFeatureWithIdentifier:(id)a0;
+ (id)someRegionIsSupportedForFeatureWithIdentifier:(id)a0;
+ (id)userDefaultsKeyIsNotPresent:(id)a0;
+ (id)walkingSteadinessCapabilityIsSupportedOnLocalDevice;
+ (id)watchAppIsInstalledForBundleIdentifier:(id)a0;
+ (id)watchLowPowerModeIsOff;
+ (id)wristDetectionIsEnabledForActiveWatch;

@end
