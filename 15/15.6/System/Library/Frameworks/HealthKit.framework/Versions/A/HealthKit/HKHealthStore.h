@class NSString, HKHealthStoreIdentifier, NSSet, HKHealthServicesManager, _HKXPCConnection, NSBundle, NSObject, NSMutableSet, HKDatabaseAccessibilityAssertionStoreKeeper, HKProfileIdentifier;
...
@interface HKHealthStore : NSObject <HKAnalyticsHealthDataSource, HKFeatureAvailabilityHealthDataSource, HKHealthStoreClientInterface, _HKXPCExportable, HKQueryDelegate, HKProxyProviderSource> {
    ...
    char _canConnectToSystemMachService;
    char _isUsingSystemEndpoint;
...
@property (class) char requireExplicitSystemMachServiceEnablement;
@property (class) char requireExplicitSystemMachServiceEnablement;
...
@property char canConnectToSystemMachService;
@property (copy, nonatomic) NSString *writeAuthorizationUsageDescriptionOverride;
@property (copy, nonatomic) NSString *readAuthorizationUsageDescriptionOverride;
@property (copy, nonatomic) NSString *clinicalReadAuthorizationUsageDescriptionOverride;
@property (copy, nonatomic) NSString *researchStudyUsageDescriptionOverride;
...
+ (char)_applicationHasRunningWorkout;
...
- (char)_prepareObjectsForSaving:(id)a0 errorOut:(id *)a1;
...
- (void)_saveObjects:(id)a0 atomically:(char)a1 skipInsertionFilter:(char)a2 completion:(id /* block */)a3;
- (void)_sendNextObjectBatch:(id)a0 skipInsertionFilter:(char)a1 lastRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a2 server:(id)a3 transaction:(id)a4 completion:(id /* block */)a5;
- (void)_serverProxyWithHandler:(id /* block */)a0 errorHandler:(id /* block */)a1;
- (char)_setActivityMoveMode:(long long)a0 error:(id *)a1;
- (void)_setBackgroundDeliveryFrequencyDataType:(id)a0 frequency:(long long)a1 withCompletion:(id /* block */)a2;
- (char)_setBiologicalSex:(long long)a0 error:(id *)a1;
- (char)_setBloodType:(long long)a0 error:(id *)a1;
- (char)_setBodyMassCharacteristicQuantity:(id)a0 error:(id *)a1;
- (char)_setCardioFitnessMedicationsUse:(unsigned long long)a0 error:(id *)a1;
- (char)_setCharacteristic:(id)a0 forDataType:(id)a1 error:(id *)a2;
- (void)_setConfigurationPropertyUsingBlock:(id /* block */)a0 resumeRequired:(char)a1;
- (void)_setDaemonPreferenceValue:(id)a0 forKey:(id)a1 completion:(id /* block */)a2;
- (char)_setDateOfBirthComponents:(id)a0 error:(id *)a1;
- (char)_setFitzpatrickSkinType:(long long)a0 error:(id *)a1;
- (char)_setHeightCharacteristicQuantity:(id)a0 error:(id *)a1;
- (char)_setLeanBodyMassCharacteristicQuantity:(id)a0 error:(id *)a1;
- (void)_setMirroredWorkoutSessionObserverStateToEnabled:(char)a0 handler:(id /* block */)a1;
- (void)_setPreferredUnit:(id)a0 forType:(id)a1 completion:(id /* block */)a2;
- (char)_setUserEnteredMenstrualCycleLengthCharacteristicQuantity:(id)a0 error:(id *)a1;
- (char)_setUserEnteredMenstrualPeriodLengthCharacteristicQuantity:(id)a0 error:(id *)a1;
- (char)_setWheelchairUse:(long long)a0 error:(id *)a1;
...
- (char)_supportsFeature:(unsigned long long)a0;
- (id)_synchronousServerProxyWithErrorHandler:(id /* block */)a0;
- (void)_throwIfAuthorizationDisallowedForSharing:(char)a0 types:(id)a1;
- (void)_throwIfClinicalTypesRequestedToShare:(id)a0;
- (void)_throwIfParentTypeNotRequestedForSharing:(char)a0 types:(id)a1;
...
- (void)_validateHealthDataPurposeStringsForSharingTypes:(id)a0 readingTypes:(id)a1 isResearchStudy:(char)a2;
...
- (void)fetchMedicalIDDataCreateIfNecessary:(char)a0 withCompletion:(id /* block */)a1;
...
- (void)requestAuthorizationToShareTypes:(id)a0 readTypes:(id)a1 shouldPrompt:(char)a2 completion:(id /* block */)a3;
...
- (void)saveObjects:(id)a0 skipInsertionFilter:(char)a1 completion:(id /* block */)a2;
