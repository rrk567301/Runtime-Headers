@class _CSMultiQuery, NSString, NSArray, CSUserQueryContext, CSSuggestionsRanker, CSRankingConfiguration, NSDictionary, NSObject, NSMutableArray, _CSContactSearch;
@protocol OS_dispatch_semaphore;

@interface CSUserQuery : CSSearchQuery {
    CSUserQueryContext *_clientContext;
    CSRankingConfiguration *_rankingConfig;
...
+ (double)computeScoreForNameUnigrams:(id)a0 queryUnigrams:(id)a1 queryString:(id)a2 locale:(id)a3 isContactsSuggestion:(char)a4;
+ (double)computeTopicalityScoreForName:(id)a0 inputName:(id)a1 numTokensInName:(unsigned long long)a2 isContactsSuggestion:(char)a3 queryUnigrams:(id)a4 isFirstName:(char)a5 locale:(id)a6;
+ (id)dedupSuggestions:(id)a0 queryContextScopeIsToPerson:(char)a1 seenContactInfoToSuggestion:(id)a2;
...
+ (id)dedupedSuggestions:(id)a0 queryContextScopeIsToPerson:(char)a1 options:(id)a2;
...
+ (id)parseResultWithSearchString:(id)a0 parseOptions:(id)a1 queryContext:(id)a2 isZKW:(char)a3;
...
+ (char)skipTextSemanticSearchForSearchString:(id)a0 queryContext:(id)a1 skipReason:(id *)a2;
...
+ (id)trimSuggestions:(id)a0 options:(id)a1 peopleSelectedScope:(long long)a2 isShortQuery:(char)a3;
