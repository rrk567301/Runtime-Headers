@class NSNumber, NSString, NSDictionary, NSSet, NSExtension, NSObject;
@protocol OS_dispatch_queue;

@interface _MDIndexExtension : NSObject

@property (retain, nonatomic) NSExtension *extension;
@property (retain, nonatomic) NSObject<OS_dispatch_queue> *queue;
@property (retain, nonatomic) NSString *containerPath;
@property (retain, nonatomic) NSString *containerID;
@property (retain, nonatomic) NSString *extensionID;
@property (nonatomic) char entitlementVerified;
@property (readonly) char isEnabled;
@property (readonly) char isInternal;
@property (readonly) char dontRunDuringMigration;
@property (readonly) NSDictionary *infoDictionary;
@property (readonly, nonatomic) NSNumber *isAppleCodeSigned;
@property (copy) NSSet *identifiers;
@property (copy) NSSet *supportedFileTypes;
@property (readonly) NSString *extensionLabel;

- (id)description;
- (void).cxx_destruct;
- (void)_performJob:(id)a0 completionBlock:(id /* block */)a1;
- (char)_verifyIntegrityWithHostContext:(id)a0;
- (id)initWithExtension:(id)a0 queue:(id)a1 containerPath:(id)a2 containerID:(id)a3;
- (id)initWithExtension:(id)a0 queue:(id)a1 containerPath:(id)a2 containerID:(id)a3 supportedFileTypes:(id)a4 extensionLabel:(id)a5 isAppleCodeSigned:(id)a6;
- (void)performJob:(id)a0 completionBlock:(id /* block */)a1;

@end
