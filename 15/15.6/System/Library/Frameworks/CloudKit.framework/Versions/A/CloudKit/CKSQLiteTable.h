@class NSString, NSArray, CKSQLiteDatabase, CKSQLiteTableGroup, NSDictionary, NSMutableArray, NSObject, NSNumber;

@interface CKSQLiteTable : NSObject <CKSQLiteDatabaseKeyValueProperties> {
    CKSQLiteDatabase *_db;
    NSMutableArray *_referencedGroups;
    NSDictionary *_referencedTablesByProperty;
    _Atomic unsigned long long _flags;
}

@property (weak, nonatomic) CKSQLiteTableGroup *tableGroup;
@property (retain, nonatomic) NSObject *databaseManagerData;
@property (retain, nonatomic) id tocTableEntry;
@property (readonly, copy, nonatomic) NSString *logicalTableName;
@property (readonly, copy, nonatomic) NSString *dbTableName;
@property (readonly, nonatomic) NSArray *primaryKeyProperties;
@property (readonly, nonatomic) char isIntegerPrimaryKeyTable;
@property (readonly, nonatomic) NSString *primaryKeyProperty;
@property (readonly, nonatomic) NSNumber *primaryKeyPropertyInfo;
@property (readonly, nonatomic) char databaseTableWasCreated;
@property (nonatomic) char logOperations;

+ (void)initialize;
+ (void)enumeratePropertyDataWithBlock:(id /* block */)a0;
+ (id)allPropertyNames;
+ (id)copyOfEntry:(id)a0;
+ (id)dbProperties;
+ (unsigned long long)dbVersion;
+ (void)decodeEntry:(id)a0 withCoder:(id)a1;
+ (id)descriptionOfEntry:(id)a0;
+ (id)descriptionOfProperties:(id)a0 from:(id)a1;
+ (void)encodeEntry:(id)a0 withCoder:(id)a1;
+ (char)entriesHaveEqualProperties:(id)a0 other:(id)a1 includePrimaryKeys:(char)a2;
+ (Class)entryClass;
+ (void)enumerateClassPropertyDictionariesWithBlock:(id /* block */)a0;
+ (id)flattenedDBProperties;
+ (unsigned long long)hashForEntry:(id)a0;
+ (id)nonKeyProperties;
+ (unsigned int)propertyCount;
+ (id)propertyInfo:(id)a0;
+ (void)setValue:(id)a0 forProperty:(id)a1 propertyInfo:(id)a2 inObject:(id)a3;
+ (char)shouldValidateEntryClass;
+ (Class)tableClassWithName:(id)a0 properties:(id)a1 version:(unsigned long long)a2 entryClass:(Class)a3;
+ (char)useNSCoding;
+ (id)valueForProperty:(id)a0 propertyInfo:(id)a1 inObject:(id)a2;
+ (id)valueForProperty:(id)a0 propertyInfo:(id)a1 inObject:(id)a2 allowFetchingUnsetPropertySentinel:(char)a3;

- (void)dealloc;
- (id)description;
- (id)init;
- (void).cxx_destruct;
- (unsigned long long)count:(id *)a0;
- (id)db;
- (id)deleteObject:(id)a0;
- (id)performTransaction:(id /* block */)a0;
- (id)insertObject:(id)a0;
- (void)setDbTableName:(id)a0;
- (void)cacheStatement:(id)a0;
- (char)hasEntry:(id)a0 error:(id *)a1;
- (id)predicateForMatchingPrimaryKeys;
- (id)UUIDValueForKey:(id)a0 error:(id *)a1;
- (void)_addPredicateForMatching:(id)a0 toStatement:(id)a1;
- (id)_bindProperties:(id)a0 valueObject:(id)a1 inStatement:(id)a2 includePrimaryKeys:(char)a3;
- (id)_fetchPropertiesUsingStatement:(id)a0 inObject:(id)a1 matchingDBProperties:(id)a2 label:(const struct { char *x0; char *x1; int x2; } *)a3;
- (id)_performEnumerationWithBlock:(id /* block */)a0 usingEnumerator:(id /* block */)a1;
- (id)_performInsertOrUpdateStatement:(id)a0 usingObject:(id)a1;
- (id)_predicateForMatchingProperties:(id)a0;
- (void)_setupActivityEntry:(id)a0;
- (id)_statementForFetchingEntriesMatchingObject:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2 setupBlock:(id /* block */)a3;
- (id)_valueDictionaryForProperties:(id)a0 inObject:(id)a1 nilPropertyError:(id *)a2 selForNilPropertyError:(SEL)a3;
- (void)activityTriggerWasRolledBack;
- (void)activityTriggered:(id)a0;
- (id)addPredicateToStatement:(id)a0 predicate:(id /* block */)a1;
- (void)addTransactionCompletionHandler:(id /* block */)a0;
- (id)checkConstraintForProperty:(id)a0;
- (id)compiledStatementWithLabel:(const struct { char *x0; char *x1; int x2; } *)a0 creationBlock:(id /* block */)a1;
- (unsigned long long)countOfEntriesMatching:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2 predicate:(id /* block */)a3;
- (id)createTableSQL;
- (id)dataValueForKey:(id)a0 error:(id *)a1;
- (id)dateValueForKey:(id)a0 error:(id *)a1;
- (unsigned long long)deleteAllEntries:(id *)a0;
- (unsigned long long)deleteEntriesMatching:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2 predicate:(id /* block */)a3;
- (id)deletePrimaryKeyValue:(id)a0;
- (id)didCreateDatabaseTable;
- (id)entriesMatchingObject:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2 setupBlock:(id /* block */)a3;
- (id)entriesWithValues:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2 setupBlock:(id /* block */)a3;
- (id)entryEnumerator:(id *)a0;
- (id)entryWithPrimaryKey:(id)a0 fetchProperties:(id)a1 label:(const struct { char *x0; char *x1; int x2; } *)a2 error:(id *)a3;
- (id)entryWithValues:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2 setupBlock:(id /* block */)a3;
- (id)enumerateEntriesWithBlock:(id /* block */)a0;
- (id)extraGroupTables:(id)a0;
- (id)fetchAllEntries:(id *)a0;
- (id)fetchAllProperties:(id)a0;
- (id)fetchProperties:(id)a0 inObject:(id)a1 matchingDBProperties:(id)a2 label:(const struct { char *x0; char *x1; int x2; } *)a3;
- (id)fetchProperties:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2;
- (id)finishInitializing;
- (char)hasEntries:(id *)a0;
- (id)initWithLogicalTableName:(id)a0;
- (id)insertObject:(id)a0 orUpdateProperties:(id)a1 label:(const struct { char *x0; char *x1; int x2; } *)a2;
- (id)maximumValueOfProperty:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2;
- (char)migrateDataFromTable:(id)a0 error:(id *)a1;
- (char)migrateEntryDataFromTable:(id)a0 entry:(id)a1 toEntry:(id)a2 addedProperties:(id)a3;
- (id)minimumValueOfProperty:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2;
- (id)newEntryObject;
- (id)newEntryObjectFromStatement:(id)a0 error:(id *)a1;
- (id)numberValueForKey:(id)a0 error:(id *)a1;
- (id)objectClassesForProperty:(id)a0;
- (id)performInTransaction:(id /* block */)a0;
- (id)primaryKeyValuesInEntry:(id)a0;
- (id)referenceProperties;
- (id)requestCallbackWithDate:(id)a0 coalescingInterval:(double)a1 minimumSeparation:(double)a2;
- (void)serialize:(id /* block */)a0;
- (id)setDataValue:(id)a0 forKey:(id)a1;
- (id)setDateValue:(id)a0 forKey:(id)a1;
- (id)setNumberValue:(id)a0 forKey:(id)a1;
- (unsigned long long)setProperties:(id)a0 valuesToStore:(id)a1 inEntriesMatching:(id)a2 label:(const struct { char *x0; char *x1; int x2; } *)a3 error:(id *)a4 predicate:(id /* block */)a5;
- (id)setStringValue:(id)a0 forKey:(id)a1;
- (id)setUUIDValue:(id)a0 forKey:(id)a1;
- (void)setValue:(id)a0 forProperty:(id)a1 propertyInfo:(id)a2 inObject:(id)a3;
- (id)setupReferencedTableCache;
- (long long)sizeOfProperty:(id)a0 inEntriesMatching:(id)a1 label:(const struct { char *x0; char *x1; int x2; } *)a2 error:(id *)a3 predicate:(id /* block */)a4;
- (long long)sizeOfProperty:(id)a0 inEntry:(id)a1 label:(const struct { char *x0; char *x1; int x2; } *)a2 error:(id *)a3;
- (long long)sizeOfProperty:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2;
- (id)statementForFetchingProperties:(id)a0 inEntriesMatchingColumns:(id)a1 error:(id *)a2;
- (id)stringValueForKey:(id)a0 error:(id *)a1;
- (long long)sumOfProperty:(id)a0 inEntriesMatching:(id)a1 label:(const struct { char *x0; char *x1; int x2; } *)a2 error:(id *)a3 predicate:(id /* block */)a4;
- (long long)sumOfProperty:(id)a0 label:(const struct { char *x0; char *x1; int x2; } *)a1 error:(id *)a2;
- (id)tableForReferenceProperty:(id)a0;
- (id)tableForReferenceProperty:(id)a0 block:(id /* block */)a1;
- (id)updateProperties:(id)a0 usingObject:(id)a1 label:(const struct { char *x0; char *x1; int x2; } *)a2;
- (id)updateUsingObject:(id)a0;
- (id)valueForProperty:(id)a0 propertyInfo:(id)a1 inObject:(id)a2;
- (id)wakeFromDatabase;

@end
