...
@property (nonatomic) char rampBatchInterval;
...
@property (readonly) char canShowProgress;
@property (nonatomic) char deleteContentAllowed;
@property (retain, nonatomic) PHImportOptions *options;
@property (retain, nonatomic) NSMutableArray *assets;
@property (retain, nonatomic) NSProgress *progress;
@property (readonly, nonatomic) char stalled;
...
@property (readonly, nonatomic) char canEject;
@property (readonly, nonatomic) char isConnectedDevice;
@property (readonly, nonatomic) char isLibrary;
@property (readonly, nonatomic, getter=isEjecting) char ejecting;
@property (readonly, nonatomic) char canAutolaunch;
@property (readonly, nonatomic) char isAvailable;
@property (readonly, nonatomic) NSString *volumePath;
@property (readonly, nonatomic) char canDeleteContent;
@property (copy, nonatomic) NSURL *autolaunchApplicationURL;
@property (readonly, nonatomic) PHPhotoLibrary *library;
@property (readonly, nonatomic) char isOptimizedCPLStorage;
...
- (char)isDone;
- (void)eject;
- (id)_adjustmentDataAssetFromAssets:(id)a0;
- (void)endWork;
- (char)isLivePhotoForImportIdentifier:(id)a0;
- (id)loadAssetsForLibrary:(id)a0 allowDuplicates:(char)a1 order:(long long)a2 batchInterval:(double)a3 atEnd:(id /* block */)a4;
- (char)_addSidecarAsset:(id)a0 toAsset:(id)a1;
- (id)_adjustmentSecondaryDataAssetFromAssets:(id)a0;
- (id)_findDatAssetInProcessedAssets:(id)a0 toPairWithMediaAsset:(id)a1 ifAssetIsAdjustment:(id)a2;
- (char)_pairDatSidecar:(id)a0 withMediaAssetIfHasAdjustments:(id)a1 orAaeInProcessedAssets:(id)a2;
...
- (void)beginWork;
- (char)confirmAsset:(id)a0 isBaseOf:(id)a1;
- (char)confirmAsset:(id)a0 isRenderOf:(id)a1;
- (char)confirmAsset:(id)a0 isSidecarOfAsset:(id)a1;
- (char)confirmAsset:(id)a0 isSocOf:(id)a1;
- (char)containsSupportedMediaWithImportExceptions:(id *)a0;
- (char)date:(id)a0 matchesDate:(id)a1;
- (id)deleteImportAssets:(id)a0 isConfirmed:(char)a1 atEnd:(id /* block */)a2;
- (id)deleteImportAssets:(id)a0 isConfirmed:(char)a1 isCancelable:(char)a2 atEnd:(id /* block */)a3;
- (void)dispatchAssetDataRequestAsync:(id)a0 usingBlock:(id /* block */)a1;
- (void)endBatch;
- (void)findDuplicatesOfItems:(id)a0 considerItemsInTheTrash:(char)a1;
- (char)isPairedSidecar:(id)a0;
- (id)loadAssetsForLibrary:(id)a0 allowDuplicates:(char)a1 order:(long long)a2 atEnd:(id /* block */)a3;
- (id)loadAssetsForLibrary:(id)a0 allowDuplicates:(char)a1 order:(long long)a2 batchSize:(unsigned long long)a3 atEnd:(id /* block */)a4;
- (id)loadAssetsForLibrary:(id)a0 allowDuplicates:(char)a1 order:(long long)a2 batchSize:(unsigned long long)a3 batchInterval:(double)a4 atEnd:(id /* block */)a5;
...
- (char)processPotentialJpegAsset:(id)a0 plusRawAsset:(id)a1;
