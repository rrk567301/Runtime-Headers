@class PHMemoryFeature, NSString, NSDictionary, NSData, NSDate, NSSet, NSArray;

@interface PHMemory : PHAssetCollection {
    NSString *_title;
    NSString *_localizedSubtitle;
    NSData *_photosGraphData;
    NSDictionary *_transientMemoryProperties;
    NSDate *_creationDate;
    NSDictionary *_movieAssetState;
    long long _storyColorGradeKind;
    NSString *_storyTitleCategory;
    NSString *_titleFontName;
    long long _syncedPlayCount;
    long long _syncedShareCount;
    long long _syncedViewCount;
    long long _pendingPlayCount;
    long long _pendingShareCount;
    long long _pendingViewCount;
    char _didLoadPhotosGraphProperties;
    long long _titleCategory;
}

@property (readonly, nonatomic, getter=isPresentedFromURLNavigation) char presentedFromURLNavigation;
@property (readonly, copy, nonatomic) NSString *URLNavigationSourceIdentifier;
@property (readonly, nonatomic) unsigned short userActionOptions;
@property (readonly, nonatomic) unsigned long long category;
@property (readonly, nonatomic) unsigned long long subcategory;
@property (readonly, nonatomic) unsigned long long notificationState;
@property (readonly, nonatomic) long long featuredState;
@property (readonly, nonatomic, getter=isRejected) char rejected;
@property (readonly, nonatomic, getter=isFavorite) char favorite;
@property (readonly, nonatomic) unsigned short pendingState;
@property (readonly, nonatomic) unsigned short creationType;
@property (readonly, nonatomic) unsigned short syndicatedContentState;
@property (readonly, nonatomic, getter=isUserCreated) char userCreated;
@property (readonly, nonatomic) NSString *subtitle;
@property (readonly, nonatomic) NSString *graphMemoryIdentifier;
@property (readonly, nonatomic) long long storyColorGradeKind;
@property (readonly, nonatomic) NSString *storyTitleCategory;
@property (readonly, nonatomic) NSData *movieData;
@property (readonly, nonatomic) long long photosGraphVersion;
@property (readonly, nonatomic) NSDictionary *photosGraphProperties;
@property (readonly, nonatomic) double score;
@property (readonly, nonatomic) NSDate *lastViewedDate;
@property (readonly, nonatomic) NSDate *lastMoviePlayedDate;
@property (readonly, nonatomic) NSDate *lastEnrichmentDate;
@property (readonly, nonatomic) NSSet *featuredPeopleIdentifiers;
@property (readonly, nonatomic) NSSet *featuredPersonLocalIdentifiers;
@property (readonly, nonatomic) NSData *assetListPredicate;
@property (readonly, nonatomic) char isContiguous;
@property (readonly, nonatomic) NSArray *blockableFeatures;
@property (readonly, nonatomic) PHMemoryFeature *blacklistedFeature;
@property (readonly, nonatomic) long long playCount;
@property (readonly, nonatomic) long long shareCount;
@property (readonly, nonatomic) long long viewCount;
@property (readonly, nonatomic) char isMustSee;
@property (readonly, nonatomic) char isStellar;
@property (readonly, nonatomic) char isGreat;
@property (readonly, nonatomic) char isCurrentlyRelevant;
@property (readonly, nonatomic) double currentRelevanceScore;
@property (readonly, nonatomic) char isBadgeable;
@property (readonly, nonatomic) NSArray *triggerTypes;
@property (readonly, nonatomic) NSDate *startDate;
@property (readonly, nonatomic) NSDate *endDate;
@property (readonly, nonatomic) NSDictionary *musicGenreDistribution;
@property (readonly, nonatomic) char isGenerative;
@property (readonly, nonatomic) long long storyColorGradeKindRaw;
@property (readonly, nonatomic) NSString *storyTitleCategoryRaw;
@property (readonly, nonatomic) unsigned short sharingComposition;

+ (Class)propertySetClassForPropertySet:(id)a0;
+ (id)fetchPredicateForSharingFilter:(unsigned short)a0;
+ (id)_composePropertiesToFetchWithHint:(unsigned long long)a0;
+ (unsigned long long)_contextualScoreForMemory:(id)a0;
+ (id)_fetchOptionsForTransientMemoryAssetsWithOptions:(id)a0;
+ (id)assetListPredicateFromQueryHintObjects:(id)a0;
+ (id)blockedPersonLocalIdentifiersInPhotoLibrary:(id)a0;
+ (void)clearCurrentMemoryForURLNavigation;
+ (id)corePropertiesToFetch;
+ (id)entityKeyMap;
+ (id)fetchBestRecentMemoryWithOptions:(id)a0;
+ (id)fetchBlockedMemoriesWithOptions:(id)a0;
+ (id)fetchMemoriesWithGraphMemoryIdentifiers:(id)a0 options:(id)a1;
+ (id)fetchMemoriesWithLocalIdentifiers:(id)a0 options:(id)a1;
+ (id)fetchMemoriesWithNilLastEnrichmentDateWithOptions:(id)a0;
+ (id)fetchMemoriesWithOptions:(id)a0;
+ (id)fetchMemoriesWithUserFeedbackWithOptions:(id)a0;
+ (id)fetchMemoryWithGraphMemoryIdentifier:(id)a0 options:(id)a1;
+ (id)fetchMostRelevantMemoryAtDate:(id)a0 options:(id)a1;
+ (id)fetchMostRelevantMemoryWithOptions:(id)a0;
+ (id)fetchPredicateFromComparisonPredicate:(id)a0 options:(id)a1;
+ (id)fetchPredicateFromUserCreatedComparisonPredicate:(id)a0;
+ (id)fetchType;
+ (id)filteredAndSortedPreviewAssetsByScoreFromAssets:(id)a0 targetSize:(struct CGSize { double x0; double x1; })a1;
+ (void)generateMemoriesWithOptions:(id)a0 completion:(id /* block */)a1;
+ (id)identifierCode;
+ (char)isTriggeredMemoryCategory:(unsigned long long)a0;
+ (id)managedEntityName;
+ (char)managedObjectSupportsCreationType;
+ (char)managedObjectSupportsPendingState;
+ (char)managedObjectSupportsRejectedState;
+ (char)managedObjectSupportsSharingComposition;
+ (char)managedObjectSupportsTrashedState;
+ (id)memoriesWithBlockedPersonFeatureInPhotoLibrary:(id)a0;
+ (id)mostRelevantMemoryInMemories:(id)a0 atDate:(id)a1;
+ (id)movieDataWithTitleFontName:(id)a0;
+ (id)previewCandidatesFromAssets:(id)a0;
+ (id)propertiesToFetchWithHint:(unsigned long long)a0;
+ (unsigned long long)propertyFetchHintsForPropertySets:(id)a0;
+ (id)propertySetAccessorsByPropertySet;
+ (id)propertySetsForPropertyFetchHints:(unsigned long long)a0;
+ (void)setCurrentMemoryForURLNavigation:(id)a0;
+ (id)stringForCategory:(unsigned long long)a0;
+ (id)stringForFeaturedState:(long long)a0;
+ (id)stringForNotificationState:(unsigned long long)a0;
+ (id)stringForSubcategory:(unsigned long long)a0;
+ (id)stringForTriggerType:(unsigned long long)a0;
+ (id)titleFontNameFromMovieData:(id)a0;
+ (id)transformValueExpression:(id)a0 forKeyPath:(id)a1;
+ (id)transientMemoryWithInfo:(id)a0 photoLibrary:(id)a1;

- (id)description;
- (void).cxx_destruct;
- (id)query;
- (id)creationDate;
- (char)isPending;
- (char)isTransient;
- (id)localizedSubtitle;
- (id)curationOfLength:(unsigned long long)a0 options:(id)a1 error:(id *)a2;
- (id)_createPropertyObjectOfClass:(Class)a0;
- (id)_createPropertyObjectOfClass:(Class)a0 preFetchedProperties:(id)a1;
- (id)_curatedAssetIDsWithLibrary:(id)a0;
- (double)_evaluatedDurationWithNumberOfStillPhotos:(unsigned long long)a0 numberOfLivePhotos:(unsigned long long)a1 numberOfVideos:(unsigned long long)a2 options:(id)a3;
- (id)_extendedCuratedAssetIDsWithLibrary:(id)a0;
- (unsigned long long)_fetchNumberOfAssetsWithType:(unsigned long long)a0 predicate:(id)a1;
- (void)_loadValuesFromPhotosGraphProperties;
- (unsigned long long)_numberOfAssetsWithType:(unsigned long long)a0 optionsValue:(id)a1 predicate:(id)a2;
- (id)_predicateForImages;
- (id)_predicateForLivePhotos;
- (id)_predicateForVideos;
- (id)_representativeAndCuratedAssetIDs;
- (unsigned long long)availableCurationLengthsWithOptions:(id)a0;
- (char)canContainAssets;
- (char)canPerformEditOperation:(long long)a0;
- (char)canShowAvalancheStacks;
- (Class)changeRequestClass;
- (char)collectionHasFixedOrder;
- (unsigned long long)currentCurationLengthWithOptions:(id)a0;
- (id)defaultSortDescriptor;
- (id)extractGenerativeMemoryMusicCuration;
- (id)fetchAssetsWithCurationOfLength:(unsigned long long)a0 options:(id)a1 error:(id *)a2;
- (id)fetchPreviewAssetForLength:(unsigned long long)a0 targetSize:(struct CGSize { double x0; double x1; })a1;
- (char)hasBlockableFeature;
- (id)initWithFetchDictionary:(id)a0 propertyHint:(unsigned long long)a1 photoLibrary:(id)a2;
- (char)isRelevantAtDate:(id)a0;
- (id)meaningLabels;
- (id)moodKeywords;
- (id)movieStateDataForAsset:(id)a0;
- (id)predicateForAllAssetsWithLibrary:(id)a0;
- (id)predicateForAllMomentsFromRepresentativeAssets;
- (id)predicateForAssetsContainedInMomentsFromRepresentativeAssets;
- (id)presentationHints;
- (id)queryForCuratedAssetsWithOptions:(id)a0;
- (id)queryForExtendedCuratedAssetsWithOptions:(id)a0;
- (id)queryForKeyAssetWithOptions:(id)a0;
- (id)rankedPreviewAssetsForLength:(unsigned long long)a0 targetSize:(struct CGSize { double x0; double x1; })a1;
- (id)rejectionCause;
- (double)relevanceScoreAtDate:(id)a0;
- (void)setupTransientMemory;
- (unsigned long long)suggestedMood;
- (long long)titleCategory;
- (id)titleFontName;
- (unsigned long long)titleFontNameHash;
- (id)transientMemoryStartDate;
- (id)transientRepresentativeAndCuratedAssetIDs;
- (id)userFeedbackProperties;

@end
