...
@property char progressive;
@property unsigned int replyPort;
@property char requestDone;
@property char requestDispatched;
@property char displayBundleIDFetched;
@property char noSenderRequested;
...
@property char forcedContentType;
...
@property char prefersDarkLoading;
...
@property char secondaryRequest;
@property char distantURLProtection;
@property (copy) NSString *appearanceName;
@property char darkAppearanceFallback;
@property char areProfessionalVideoWorkflowFormatReadersEnabled;
...
@property char shouldDownloadFault;
@property (readonly) char isCoordinating;
...
+ (char)supportsContentType:(id)a0;
+ (char)supportsFileAtURL:(id)a0 outContentType:(id *)a1;
...
- (char)_extensionExistsUsingExactTypeMatch:(char)a0;
- (int)_extensionType;
...
- (void)_setupTypesForSchemeHandlingForAdvancedUsage:(char)a0;
- (void)_setupTypesForViewBasedExtension:(unsigned long long)a0;
- (char)_shouldDownloadFault:(id)a0;
...
- (char)isRawImage;
- (char)isWithinContentTypeLimit:(id)a0;
- (id)loadingPlaceHolder_outShouldDarken:(char *)a0;
- (void)processProgressiveRendering:(id)a0 completionHandler:(id /* block */)a1;
- (void)receivedDataFromServer:(id)a0 attachmentURL:(id)a1 hasMoreData:(char)a2;
- (void)releasePort;
- (void)retrieveDataForProgressiveRendering:(id /* block */)a0;
- (void)retrieveDisplayBundleIDForAdvancedUsage:(char)a0 completionHander:(id /* block */)a1;
