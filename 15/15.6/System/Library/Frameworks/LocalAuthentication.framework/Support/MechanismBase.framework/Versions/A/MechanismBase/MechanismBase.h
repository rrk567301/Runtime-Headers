@class NSError, LAAssertionsProxy, NSArray, NSString, NSHashTable, NSMutableDictionary, NSDictionary, NSMutableArray, MechanismContext, NSNumber, EvaluationRequest;
@protocol LACContextExternalizing, LAUIDelegate;

@interface MechanismBase : NSObject <LAUIDelegate> {
    NSMutableArray *_mechanismAssertions;
    id /* block */ _replyToRun;
    MechanismContext *_mechanismContext;
    NSDictionary *_eventProcessing;
    NSMutableDictionary *_hints;
    LAAssertionsProxy *_touchIdAssertions;
    NSString *_touchIdAssertionReason;
    NSString *_pausedTouchIdAssertionReason;
    NSHashTable *_observers;
}

@property (class, readonly) unsigned int newInstanceId;

@property (retain, nonatomic) NSMutableDictionary *partialResult;
@property (readonly, nonatomic) unsigned int instanceId;
@property (readonly, nonatomic) long long eventIdentifier;
@property (readonly, nonatomic) long long remoteViewController;
@property (readonly, nonatomic) EvaluationRequest *request;
@property (readonly, weak, nonatomic) id<LACContextExternalizing> cachedExternalizationDelegate;
@property (readonly, nonatomic) NSDictionary *internalOptions;
@property (readonly, nonatomic) long long policy;
@property (readonly, nonatomic) NSDictionary *policyOptions;
@property (retain, nonatomic) NSDictionary *activationParams;
@property (weak, nonatomic) id<LAUIDelegate> eventsDelegate;
@property (readonly, nonatomic) unsigned int maxGlobalCredentialAge;
@property (retain, nonatomic) NSNumber *failureLimit;
@property (nonatomic) unsigned long long failures;
@property (readonly, nonatomic) char willTryToRecover;
@property (readonly, nonatomic, getter=isRunning) char running;
@property (readonly, nonatomic, getter=isRestartable) char restartable;
@property (readonly, nonatomic, getter=isLastRestartAttempt) char lastRestartAttempt;
@property (weak, nonatomic) MechanismBase *parent;
@property (weak, nonatomic) MechanismBase *rootMechanism;
@property (retain, nonatomic) MechanismBase *preCompanion;
@property (retain, nonatomic) MechanismBase *postCompanion;
@property (readonly, nonatomic) char precedesUI;
@property (readonly, nonatomic) char isMirroringUI;
@property (copy, nonatomic) id /* block */ showUIBlock;
@property (retain, nonatomic) NSError *silentFailure;
@property (readonly, nonatomic) NSArray *assertions;

- (id)description;
- (void).cxx_destruct;
- (id)externalizedContext;
- (void)event:(long long)a0 params:(id)a1 reply:(id /* block */)a2;
- (void)addAssertion:(id)a0;
- (void)removeEventObserver:(id)a0;
- (char)_isProcessedBy:(id)a0;
- (void)companionStateChanged:(id)a0 newState:(char)a1;
- (char)_acquireAssertionsWithReason:(id)a0 error:(id *)a1;
- (void)_dropAssertionsWithReason:(id)a0;
- (void)_dropTouchIdAssertionWithCompletion:(id /* block */)a0;
- (void)_eventWithParams:(id)a0 reply:(id /* block */)a1;
- (char)_shouldMirrorToDefaultUiWithProcessing:(id)a0;
- (void)_takeTouchIdAssertionIfNeededWithReason:(id)a0 completion:(id /* block */)a1;
- (void)_takeTouchIdAssertionWithReason:(id)a0 completion:(id /* block */)a1;
- (id)_touchIdAssertionsProxy;
- (void)addEventObserver:(id)a0;
- (id)additionalControllerInternalInfoForPolicy:(long long)a0;
- (id)availabilityEventsForPurpose:(long long)a0;
- (id)backgroundMechanismForEventProcessing:(id)a0;
- (id)bestEffortAvailableMechanismForRequest:(id)a0 error:(id *)a1;
- (char)canRecoverFromAvailabilityError:(id)a0 request:(id)a1;
- (void)cancelAuthentication;
- (void)cancelByParent:(id)a0 onSuccess:(char)a1;
- (id)descriptionFlags;
- (void)dispatchAsyncOnServerQueueIfRunning:(id /* block */)a0;
- (void)failAuthenticationWithError:(id)a0;
- (id)failuresInfoDictionaryWithError:(id)a0;
- (char)fallbackToIdentifier:(long long)a0;
- (id /* block */)fenceReplyWithTouchIdAssertions:(id /* block */)a0;
- (id)findMechanismToRetryWithEventIdentifier:(long long)a0;
- (id)findMechanismWithEventIdentifier:(long long)a0;
- (void)finishRunWithResult:(id)a0 error:(id)a1;
- (void)finishRunWithResult:(id)a0 error:(id)a1 skipReply:(char)a2;
- (void)handleUIEvent:(long long)a0 params:(id)a1;
- (id)initWithEventIdentifier:(long long)a0 remoteViewController:(long long)a1 cachedExternalizationDelegate:(id)a2 request:(id)a3;
- (id)initWithParams:(id)a0 request:(id)a1;
- (char)isAvailableForPurpose:(long long)a0 error:(id *)a1;
- (char)isTCCAllowedWithAuditTokenData:(id)a0 optionAuditTokenData:(id)a1 forcePrompt:(char)a2 auditTokenUsage:(id)a3 error:(id *)a4;
- (id)mechanismTreeDescription;
- (void)noResponseEventWithParams:(id)a0;
- (char)pause:(char)a0 forEvent:(long long)a1 error:(id *)a2;
- (void)prepareForRestart;
- (id)remoteAlertViewControllerName;
- (long long)remoteViewControllerForEventProcessing:(id)a0;
- (char)requiresRemoteViewControllerUiWithEventProcessing:(id)a0;
- (char)requiresUiWithEventProcessing:(id)a0;
- (void)responseEventWithParams:(id)a0 timeout:(double)a1 reply:(id /* block */)a2;
- (void)restartWithEventIdentifier:(long long)a0 lastAttempt:(char)a1;
- (void)runWithHints:(id)a0 eventsDelegate:(id)a1 reply:(id /* block */)a2;
- (void)setCredential:(id)a0 credentialType:(long long)a1 reply:(id /* block */)a2;
- (void)setShowingCoachingHint:(char)a0 reply:(id /* block */)a1;
- (void)subMechanismCanRestart:(id)a0;
- (void)succeedAuthenticationWithDefaultResult;
- (void)succeedAuthenticationWithResult:(id)a0;
- (id)tccError:(char)a0;
- (id)tccPreflightWithAuditTokenData:(id)a0 auditTokenUsage:(id)a1;
- (id)tccService;
- (void)willFinish;
- (void)yieldPartialResult:(id)a0;
- (void)yieldPartialResult:(long long)a0 value:(id)a1;

@end
