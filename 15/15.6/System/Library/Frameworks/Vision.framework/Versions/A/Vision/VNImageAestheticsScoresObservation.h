...
@property (readonly, nonatomic) char isUtility;
@property (readonly, nonatomic) float overallScore;

+ (char)supportsSecureCoding;
+ (id)defaultOriginatingRequestClassNameForRequestRevision:(unsigned long long)a0;
+ (id)observationForOriginatingRequestSpecifier:(id)a0 isUtility:(char)a1 overallScore:(float)a2 aestheticScore:(float)a3 failureScore:(float)a4 junkNegativeScore:(float)a5 junkTragicFailureScore:(float)a6 poorQualityScore:(float)a7 nonMemorableScore:(float)a8 screenShotScore:(float)a9 receiptOrDocumentScore:(float)a10 textDocumentScore:(float)a11 error:(id *)a12;
