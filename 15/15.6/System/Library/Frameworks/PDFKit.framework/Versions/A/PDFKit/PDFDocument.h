@class NSURL, NSMutableDictionary, PDFOutline, PDFAKDocumentAdaptor, NSDictionary, NSObject, NSMutableIndexSet, PDFAKPageOverlayViewProvider, NSMutableArray, NSOrderedSet, PDFRenderingProperties, NSString, PDFForm, NSIndexSet, PDFSelection, PDFPasswordProtectedEditsController, NSArray;
@protocol PDFDocumentPageChangeDelegate, PDFAKControllerDelegateProtocol, OS_dispatch_queue, PDFDocumentDelegate;

@interface PDFDocument : NSObject <NSCopying> {
    struct CGPDFDocument { } *_document;
    NSURL *_documentURL;
    char _createdWithHighLatencyDataProvider;
    NSMutableArray *_pages;
    NSOrderedSet *_pagesOrderedSet;
    NSMutableDictionary *_pageIndices;
    NSMutableDictionary *_pageDictionaryIndices;
    NSMutableIndexSet *_preloadingPageIndexes;
    NSArray *_accessibilityPageElements;
    BOOL _subclassOverridesPageAtIndex;
    id _delegate;
    BOOL _respondsToDidUnlock;
    BOOL _respondsToDidFindMatch;
    BOOL _respondsToDidBeginDocumentFind;
    BOOL _respondsToDidEndDocumentFind;
    BOOL _respondsToDidBeginPageFind;
    BOOL _respondsToDidEndPageFind;
    BOOL _respondsToDidMatchString;
    BOOL _respondsToPrintJobTitle;
    BOOL _respondsToClassForPage;
    BOOL _respondsToClassForAnnotationType;
    BOOL _respondsToClassForAnnotationClass;
    BOOL _respondsToShouldReadAKInkAnnotations;
    BOOL _respondsToHandleTabFrom;
    BOOL _respondsToDidReceiveAnalysis;
    unsigned long long _pageCount;
    int _majorVersion;
    int _minorVersion;
    BOOL _isEncrypted;
    BOOL _isUnlocked;
    unsigned int _accessPermissions;
    BOOL _allowsPrinting;
    BOOL _allowsCopying;
    BOOL _allowsDocumentChanges;
    BOOL _allowsDocumentAssembly;
    BOOL _allowsContentAccessibility;
    BOOL _allowsCommenting;
    BOOL _allowsFormFieldEntry;
    long long _permission;
    NSString *_ownerPassword;
    NSString *_userPassword;
    NSDictionary *_attributes;
    PDFOutline *_outline;
    NSObject<OS_dispatch_queue> *_textExtractionQueue;
    NSObject<OS_dispatch_queue> *_formFillingQueue;
    BOOL _finding;
    int _findModel;
    NSArray *_findStrings;
    PDFSelection *_findInstance;
    unsigned long long _findOptions;
    long long _findPageIndex;
    long long _findCharIndex;
    long long _lastFindPageIndex;
    long long _lastFindCharIndex;
    NSMutableArray *_findResults;
    BOOL _dataDetectorsEnabled;
    PDFForm *_formData;
    char _formDataLoaded;
    NSArray *_documentCatalogMetadata;
    NSString *_xmpNameSpace;
    NSString *_xmpPrefix;
    NSString *_xmpRootPath;
    id<PDFDocumentPageChangeDelegate> _pageChangeDelegate;
    BOOL _documentChanged;
    BOOL _documentHasBurnInAnnotations;
    unsigned long long _documentRedactionCount;
    BOOL _documentHasPageWithApplicationData;
    struct { BOOL pagesAdded; BOOL blankPagesAdded; BOOL pagesRemoved; BOOL pagesExchanged; } _pagesChanged;
    NSDictionary *_appendedAttributes;
    NSIndexSet *_initialBookmarkedPageIndices;
    NSMutableIndexSet *_bookmarkedPages;
    PDFAKDocumentAdaptor *_akDocumentAdaptor;
    id<PDFAKControllerDelegateProtocol> _pdfAKControllerDelegateForDeferredSetup;
    PDFAKPageOverlayViewProvider *_akPageOverlayViewProvider;
    PDFRenderingProperties *_renderingProperties;
    BOOL _limitedSearch;
    NSObject<OS_dispatch_queue> *_asyncSearchQueue;
    PDFPasswordProtectedEditsController *_passwordProtectedEditsController;
}

@property (readonly, nonatomic) NSURL *documentURL;
@property (readonly, nonatomic) struct CGPDFDocument { } *documentRef;
@property (copy, nonatomic) NSDictionary *documentAttributes;
@property (readonly, nonatomic) long long majorVersion;
@property (readonly, nonatomic) long long minorVersion;
@property (readonly, nonatomic) char isEncrypted;
@property (readonly, nonatomic) char isLocked;
@property (readonly, nonatomic) char allowsPrinting;
@property (readonly, nonatomic) char allowsCopying;
@property (readonly, nonatomic) char allowsDocumentChanges;
@property (readonly, nonatomic) char allowsDocumentAssembly;
@property (readonly, nonatomic) char allowsContentAccessibility;
@property (readonly, nonatomic) char allowsCommenting;
@property (readonly, nonatomic) char allowsFormFieldEntry;
@property (readonly, nonatomic) unsigned long long accessPermissions;
@property (readonly, nonatomic) long long permissionsStatus;
@property (readonly, nonatomic) NSString *string;
@property (weak, nonatomic) id<PDFDocumentDelegate> delegate;
@property (retain, nonatomic) PDFOutline *outlineRoot;
@property (readonly, nonatomic) unsigned long long pageCount;
@property (readonly, nonatomic) Class pageClass;
@property (readonly, nonatomic) char isFinding;
@property (readonly, nonatomic) PDFSelection *selectionForEntireDocument;

+ (id)_getDocumentID:(struct CGPDFDocument { } *)a0;
+ (char)isValidPassword:(id)a0;
+ (char)pdfDocumentAppendModeActiveForThisThread;
+ (void)setPDFDocumentAppendModeActiveForThisThread:(char)a0;

- (void)dealloc;
- (id)copyWithZone:(struct _NSZone { } *)a0;
- (id)init;
- (void).cxx_destruct;
- (id)initWithData:(id)a0;
- (id)initWithProvider:(struct CGDataProvider { } *)a0;
- (id)initWithURL:(id)a0;
- (id)dataRepresentationWithOptions:(id)a0;
- (id)dataRepresentation;
- (void)_commonInit;
- (void)redo:(id)a0;
- (void)setDocument:(struct CGPDFDocument { } *)a0;
- (void)undo:(id)a0;
- (char)writeToFile:(id)a0;
- (id)accessibilityChildren:(id)a0;
- (id)pageAtIndex:(unsigned long long)a0;
- (char)writeToURL:(id)a0;
- (void)revert;
- (id)findString:(id)a0 fromSelection:(id)a1 withOptions:(unsigned long long)a2;
- (id)getPrintOperationForPrintInfo:(id)a0 autoRotate:(char)a1;
- (unsigned long long)indexForPage:(id)a0;
- (id)formData;
- (void)setFormData:(id)a0;
- (void)didMatchString:(id)a0;
- (id)akController;
- (void)setPDFAKControllerDelegate:(id)a0;
- (char)unlockWithPassword:(id)a0;
- (char)validateRedo:(id)a0;
- (char)validateUndo:(id)a0;
- (char)writeToConsumer:(struct CGDataConsumer { } *)a0 withOptions:(id)a1;
- (id)annotationsForFieldName:(id)a0;
- (id)findString:(id)a0 withOptions:(unsigned long long)a1;
- (id)namedDestination:(id)a0;
- (void)preloadDataOfPagesInRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a0 onQueue:(id)a1 completion:(id /* block */)a2;
- (void)resetFormFields:(id)a0;
- (id)selectionFromPage:(id)a0 atCharacterIndex:(unsigned long long)a1 toPage:(id)a2 atCharacterIndex:(unsigned long long)a3;
- (id)selectionFromPage:(id)a0 atPoint:(struct CGPoint { double x0; double x1; })a1 toPage:(id)a2 atPoint:(struct CGPoint { double x0; double x1; })a3;
- (id)selectionFromPage:(id)a0 atPoint:(struct CGPoint { double x0; double x1; })a1 toPage:(id)a2 atPoint:(struct CGPoint { double x0; double x1; })a3 withGranularity:(unsigned long long)a4;
- (void)setEnableDataDetectors:(char)a0;
- (void)setHasHighLatencyDataProvider:(char)a0;
- (void)insertPage:(id)a0 atIndex:(unsigned long long)a1;
- (char)writeToURL:(id)a0 withOptions:(id)a1;
- (id)PDFAKControllerDelegate;
- (id)bookmarkedPages;
- (char)documentChanged;
- (id)_SASLSanitize:(id)a0;
- (id)_addEncryptionFromOptions:(id)a0;
- (id)PDFDocumentPrintJobTitle:(id)a0;
- (id)_dataFromXMP:(struct __CFData { } *)a0 withRootPath:(id)a1 keys:(id)a2;
- (void)coreFindString:(id)a0;
- (id)_bookmarksCatalog;
- (BOOL)_canSaveWithEncryption;
- (struct CGPDFName { } *)_convertCFStringToCGPDFName:(void *)a0 forKey:(struct __CFString { } *)a1;
- (id)_convertWriteOptions:(id)a0;
- (struct CGPDFAnnotation { } *)_createCGPDFAnnotationForAnnotation:(id)a0;
- (void)_createDocumentWithDataProvider:(struct CGDataProvider { } *)a0;
- (struct __CFDictionary { } *)_createInfoDictionary;
- (id)_documentCatalogMetadataForRootPath:(id)a0 withKeys:(id)a1;
- (id)_findString:(id)a0 fromSelection:(id)a1 withOptions:(unsigned long long)a2;
- (id)_getNearestOutline:(id)a0 forDestination:(id)a1;
- (BOOL)_isCommonlyMappedToNothing:(id)a0;
- (BOOL)_isNonAsciiSpace:(id)a0;
- (id)_namedDestination:(id)a0 forNameDictionary:(struct CGPDFDictionary { } *)a1;
- (void)_notifyAKAdaptorPagePlaceholder:(id)a0 wasReplacedWithPage:(id)a1 atIndex:(unsigned long long)a2;
- (id)_rawPageAtIndex:(unsigned long long)a0;
- (void)_sanitizeAnnotationArray:(struct __CFArray { } *)a0;
- (void)_sanitizeAnnotationDictionary:(struct __CFDictionary { } *)a0;
- (void)_setDocumentCatalogMetadata:(id)a0 withNSpace:(id)a1 prefix:(id)a2 rootPath:(id)a3;
- (BOOL)_writeAppendMode:(struct CGDataConsumer { } *)a0;
- (id)accessibilityPageElements;
- (id)accessibilityVisibleChildren:(id)a0;
- (id)akDocumentAdaptor;
- (Class)annotationSubclassForClass:(Class)a0;
- (Class)annotationSubclassForType:(id)a0;
- (void)asyncFindString:(id)a0 withDelegate:(id)a1 onQueue:(id)a2;
- (void)asyncFindStrings:(id)a0 withDelegate:(id)a1 onQueue:(id)a2;
- (void)asyncFindStrings:(id)a0 withOptions:(unsigned long long)a1 withDelegate:(id)a2 onQueue:(id)a3;
- (void)beginBatchPageChanges;
- (void)beginFindString:(id)a0 withOptions:(unsigned long long)a1;
- (void)beginFindStrings:(id)a0 withOptions:(unsigned long long)a1;
- (char)bookmarksChanged;
- (void)callDelegateDidReceiveAnalysis:(id)a0 forPage:(id)a1;
- (char)callDelegateHandleTabFrom:(id)a0 direction:(unsigned long long)a1;
- (char)callShouldReadAKInkAnnotations;
- (char)canSaveWithAppendModeUsingOptions:(id)a0;
- (char)canSaveWithTextFromOCR;
- (void)cancelFindString;
- (char)checkForPermissionToChangePermissionsWithWindowForSheet:(id)a0;
- (char)checkForPermissions:(unsigned long long)a0 windowForSheet:(id)a1;
- (void)cleanupAfterPrintOperation:(id)a0;
- (void)cleanupFind;
- (void)coreFindStrings:(id)a0;
- (char)createdWithWithHighLatencyDataProvider;
- (void)decrementRedactionCount;
- (char)enableDataDetectors;
- (void)enableDocumentMRUMode:(BOOL)a0;
- (void)enableLimitedSearch;
- (void)endBatchPageChanges;
- (void)exchangePageAtIndex:(unsigned long long)a0 withPageAtIndex:(unsigned long long)a1;
- (char)findOnPage;
- (id)findPageWithCGPDFDictionaryPageRef:(struct CGPDFDictionary { } *)a0;
- (id)findStringPerPage:(id)a0 fromSelection:(id)a1 withOptions:(unsigned long long)a2;
- (id)findStrings:(id)a0 withinSelection:(id)a1 withOptions:(unsigned long long)a2;
- (id)formFillingQueue;
- (char)hasHighLatencyDataProvider;
- (void)incrementRedactionCount;
- (char)isLinearized;
- (char)isSimpleSelectionEnabled;
- (id)linearizedRangesForPage:(id)a0;
- (struct __CFData { } *)newXMPFromData:(id)a0 preserveExistingXMPMetadata:(char)a1;
- (void)normalizeFindOptions:(unsigned long long)a0;
- (id)outlineItemForSelection:(id)a0;
- (id)pageAnnotationChanges;
- (id)passwordProtectedEditsController;
- (id)passwordUsedForUnlocking;
- (id)printOperationForPrintInfo:(id)a0 scalingMode:(long long)a1 autoRotate:(char)a2;
- (void)removePageAtIndex:(unsigned long long)a0;
- (id)renderingProperties;
- (void)requirePasswordsIfNeededUsingPresentingViewController:(id)a0 completion:(id /* block */)a1;
- (void)resetAccessibilityTree;
- (void)resetPageAnnotationChanges;
- (void)scheduledFindOnPage:(id)a0;
- (id)selectionFromPage:(id)a0 atPoint:(struct CGPoint { double x0; double x1; })a1 toPage:(id)a2 atPoint:(struct CGPoint { double x0; double x1; })a3 type:(int)a4;
- (void)setBookmarked:(BOOL)a0 atPageIndex:(unsigned long long)a1;
- (void)setDocumentHasBurnInAnnotations:(char)a0;
- (void)setHasPageWithApplicationData:(char)a0;
- (void)setPageChangeDelegate:(id)a0;
- (void)setPrintScalingMode:(long long)a0 forPrintOperation:(id)a1;
- (void)setRenderingProperties:(id)a0;
- (void)setShowsAnnotations:(char)a0;
- (void)setupAKDocumentAdaptorIfNecessaryWithView:(id)a0;
- (char)showsAnnotations;
- (id)textExtractionQueue;
- (void)updateBookmarksInPDFDocument;
- (char)viewPreferenceRightToLeft;
- (char)writeToFile:(id)a0 withOptions:(id)a1;

@end
