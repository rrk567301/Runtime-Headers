@class LPTextViewStyle, NSFont;

@interface LPQuotedTextViewStyle : LPTextViewStyle <NSCopying>

@property (retain, nonatomic) NSFont *longPullQuoteFont;
@property (nonatomic) unsigned int maximumLinesToConsiderShort;
@property (nonatomic) char showQuoteIndicator;
@property (nonatomic) char showCharacterLimitIndicator;
@property (readonly, retain, nonatomic) LPTextViewStyle *characterLimitIndicatorStyle;

- (id)copyWithZone:(struct _NSZone { } *)a0;
- (void).cxx_destruct;
- (id)adjustedForString:(id)a0;
- (id)initWithPlatform:(long long)a0 sizeClass:(unsigned long long)a1 fontScalingFactor:(double)a2;

@end
