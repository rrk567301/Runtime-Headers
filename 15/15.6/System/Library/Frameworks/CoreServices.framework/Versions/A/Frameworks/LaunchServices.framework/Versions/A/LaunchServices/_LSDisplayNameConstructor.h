...
@property (class, nonatomic) char showAllExtensions;
...
+ (void)getExtensionRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; } *)a0 secondaryExtensionRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; } *)a1 fromFileName:(id)a2 considerConfusables:(char)a3;
+ (unsigned char)prepareMimicWithPopulator:(id)a0 error:(id *)a1;
+ (void)setShowAllExtensions:(char)a0;
...
- (char)canSetExtensionHiddenWithContext:(struct LSContext { id x0; } *)a0;
- (id)cleanSecondaryExtension:(id)a0;
- (id)combineBaseName:(id)a0 extension:(id)a1;
- (void)getTransformedBaseName:(id *)a0 extension:(id *)a1 needsBiDiControlCharacters:(char[2])a2;
- (void)getUnlocalizedBaseName:(id *)a0 extension:(id *)a1 requiresAdditionalBiDiControlCharacters:(char *)a2;
- (void)initContentBitsWithDisplayName:(id)a0 treatAsFSName:(char)a1;
- (void)initNamePartsWithDisplayName:(id)a0;
- (void)initNodeBitsWithContextIfNeeded:(struct Context { struct LSContext *x0; struct LSContext { id x0; } x1; BOOL x2; id x3; } *)a0 node:(id)a1 isDirectory:(char)a2 bundleClass:(const unsigned int *)a3;
- (id)initWithContextIfNeeded:(struct Context { struct LSContext *x0; struct LSContext { id x0; } x1; BOOL x2; id x3; } *)a0 node:(id)a1 isDirectory:(char)a2 bundleClass:(const unsigned int *)a3 desiredDisplayName:(id)a4 treatAsFSName:(char)a5;
- (id)insertCompleteNameBiDiControlCharacters:(id)a0;
- (id)insertNameComponentBiDiControlCharacters:(id)a0;
- (char)isStringNaturallyRTL:(id)a0;
- (char)mayHideExtensionWithContextIfNeeded:(struct Context { struct LSContext *x0; struct LSContext { id x0; } x1; BOOL x2; id x3; } *)a0;
- (char)showExtensionWithContextIfNeeded:(struct Context { struct LSContext *x0; struct LSContext { id x0; } x1; BOOL x2; id x3; } *)a0 asIfShowingAllExtensions:(id)a1;
- (id)transformBeforeCombining:(id)a0 needsBiDiControlCharacters:(char *)a1;
