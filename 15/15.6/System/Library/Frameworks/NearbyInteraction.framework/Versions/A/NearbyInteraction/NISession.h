...
@property (readonly, getter=isPreciseRangingAllowed) char preciseRangingAllowed;
@property (readonly, getter=isExtendedDistanceMeasurementAllowed) char extendedDistanceMeasurementAllowed;
@property (readonly, getter=isRangingLimitExceeded) char rangingLimitExceeded;
...
+ (id)setLocalDeviceCanInteract:(char)a0 withDiscoveryTokens:(id)a1;
...
- (void)_interruptSessionWithInternalReason:(long long)a0 cachedInterruption:(char)a1;
- (void)_invalidateInternal;
- (void)_invalidateSessionAndNotifyError:(id)a0;
- (char)_isInternalClient;
- (void)_logDurationAndSubmit:(char)a0;
- (void)_logTime;
...
- (void)_performBlockOnDelegateQueue:(id /* block */)a0 ifRespondsToSelector:(SEL)a1 evenIfNotRunning:(char)a2;
...
- (char)_shouldReConnectToDaemonAfterCrash;
- (char)_shouldReRunSessionAfterSessionInterruptionEnded;
- (void)_submitErrorMetric:(id)a0;
- (id)_synchronousRemoteObject;
- (char)_tryToRecoverFromFailure:(id)a0;
...
- (void)didReceiveRangingAuthRecommendation:(char)a0 forObject:(id)a1;
...
- (void)didUpdateHomeDeviceUWBRangingAvailability:(char)a0;
...
- (void)systemDidUpdateResourceUsageLimitExceeded:(char)a0 forSessionConfigurationTypeWithName:(id)a1;
