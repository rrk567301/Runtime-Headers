...
@property (readonly, nonatomic) char canBeDropped;
@property (readonly, nonatomic) char isWasteful;
...
@property (readonly, nonatomic) char incompatibleVersion;
...
- (char)addExternalReferenceToObjectOrLazyReference:(id)a0 isWeak:(char)a1 externalReferenceMap:(id)a2 delegate:(id)a3;
- (void)continueDiscardingContentIfPossibleUsingBlock:(id /* block */)a0;
- (void)discardContentIfPossibleFromNSCache:(char)a0;
...
- (void)saveToMessage:(void *)a0 saveToken:(unsigned long long)a1 writtenComponentInfo:(const struct WrittenComponentInfo { id x0; id x1; char x2; char x3; id x4; char x5; char x6; char x7; id x8; id x9; char x10; unsigned char x11; long long x12; unsigned long long x13; unsigned long long x14; unsigned long long x15; unsigned long long x16; char x17; id x18; id x19; id x20; id x21; id x22; id x23; id x24; id x25; } *)a2;
...
- (void)setModified:(char)a0 forObject:(id)a1;
- (void)setModified:(char)a0 forObject:(id)a1 isDocumentUpgrade:(char)a2;
- (void)setModifiedImpl:(char)a0 forObject:(id)a1;
- (void)setPackageIdentifier:(unsigned char)a0 preferredLocator:(id)a1 locator:(id)a2 isStoredOutsideObjectArchive:(char)a3 compressionAlgorithm:(long long)a4 rootObjectOrNil:(id)a5 archivedObjects:(id)a6 externalReferenceMap:(id)a7 persistedComponentDataReferenceMap:(id)a8 ambiguousReferences:(id)a9 documentReadVersion:(unsigned long long)a10 documentWriteVersion:(unsigned long long)a11 componentReadVersion:(unsigned long long)a12 componentRequiredVersion:(unsigned long long)a13 canBeDropped:(char)a14 isWasteful:(char)a15 requiredPackageIdentifier:(unsigned char)a16 featureInfos:(id)a17 componentObjectUUIDMap:(id)a18 objectReferenceMap:(id)a19 saveToken:(unsigned long long)a20 encodedLength:(unsigned long long)a21 lastModificationDate:(id)a22 wasCopied:(char)a23 wasModifiedDuringWrite:(char)a24;
