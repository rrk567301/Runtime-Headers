...
- (void)addDataFromPackage:(id)a0 packageURL:(id)a1 dataInfo:(const void *)a2 packageMetadata:(id)a3 documentResourceInfo:(id)a4 areExternalReferencesAllowed:(char)a5;
- (id)addNewDataForStorage:(id)a0 digest:(id)a1 hasValidatedDigestMatch:(char)a2 filename:(id)a3;
- (char)attemptToMaterializeData:(id)a0 dataURL:(id)a1 dataURLType:(long long)a2;
- (id)dataForDigestImpl:(id)a0 documentResourceInfo:(id)a1 skipDocumentResourcesLookup:(char)a2 expectedLength:(unsigned long long)a3 accessorBlock:(id /* block */)a4;
- (id)dataForDocumentResourceInfo:(id)a0 fromFileURL:(id)a1;
- (id)dataForExistingData:(id)a0 digest:(id)a1 hasValidatedDigestMatch:(char)a2 filename:(id)a3 temporaryDataStorageURL:(id)a4;
- (id)dataFromExternalReferenceURL:(id)a0 filename:(id)a1 useFileCoordination:(char)a2;
- (void)dataFromFileURL:(id)a0 filename:(id)a1 context:(id)a2 completionQueue:(id)a3 completion:(id /* block */)a4;
- (id)dataFromFileURL:(id)a0 filename:(id)a1 useFileCoordination:(char)a2;
- (id)dataFromReadChannel:(id)a0 filename:(id)a1 dataURL:(id)a2 canLink:(char)a3;
- (void)dataFromReadChannel:(id)a0 filename:(id)a1 dataURL:(id)a2 canLink:(char)a3 completion:(id /* block */)a4;
- (id)dataFromURL:(id)a0 filename:(id)a1 useFileCoordination:(char)a2;
- (id)dataWithStorage:(id)a0 digest:(id)a1 hasValidatedDigestMatch:(char)a2 filename:(id)a3 skipDocumentResourcesLookup:(char)a4 accessorBlock:(id /* block */)a5;
- (id)dataWithTemporaryDataStorageURL:(id)a0 digest:(id)a1 hasValidatedDigestMatch:(char)a2 filename:(id)a3;
...
- (void)findExistingDataForReadChannel:(id)a0 dataURL:(id)a1 dataURLType:(long long)a2 temporaryDataStorageURL:(id)a3 keepTemporaryFileOnSuccess:(char)a4 completion:(id /* block */)a5;
- (char)hasIncompleteDataIncludeUnmaterialized:(char)a0 includeMissing:(char)a1 includeExternalReference:(char)a2;
- (char)linkOrCloneTemporaryURL:(id)a0 fromURL:(id)a1 canLink:(char)a2;
- (void)loadFromPackage:(id)a0 packageURL:(id)a1 packageMetadata:(id)a2 areExternalReferencesAllowed:(char)a3 dataIdentifiersIndexSet:(id *)a4;
- (char)migrateDataToTemporaryStorageFromPackage:(id)a0;
- (int)openTemporaryURL:(id)a0;
- (id)preferredFilenameForDocumentResourceInfo:(id)a0;
- (char)prepareSaveWithOldPackage:(id)a0 saveOperationState:(id)a1;
- (char)prepareTemporaryDataStorageForData:(id)a0 temporaryDataStorage:(id *)a1;
- (id)remoteDataWithURL:(id)a0 digest:(id)a1 filename:(id)a2 length:(unsigned long long)a3 canDownload:(char)a4 downloadPriority:(long long)a5 uploadStatus:(long long)a6 modificationDate:(id)a7;
- (char)shouldAttemptToMaterializeData:(id)a0 expectedLength:(unsigned long long)a1;
- (id)temporaryDataStorageURLForFilename:(id)a0;
- (id)unmaterializedRemoteDataIncludingExpiredUploads:(char)a0 includingDataNotInDocument:(char)a1;
- (void)willCreateData;
