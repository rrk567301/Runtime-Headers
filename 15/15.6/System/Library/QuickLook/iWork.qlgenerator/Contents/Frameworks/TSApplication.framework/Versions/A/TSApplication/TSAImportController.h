@class NSURL, SFUCryptoKey, TSPObjectContext, TSUImage, NSDictionary, NSObject, TSPDocumentLoadValidationPolicy, TSUTemporaryDirectory, NSMapTable, NSMutableArray, NSProgress, NSString, NSSet, TSPDocumentSaveValidationPolicy, NSMutableSet, NSOperationQueue, TSADocumentRoot, NSUUID;
...
@property (readonly, nonatomic) char shouldUpdateAdditionalResourceRequestsAfterImport;
...
@property (readonly, nonatomic) char shouldSaveDocumentPreviewImage;
...
@property (nonatomic) char temporaryURLShouldBeAppropriateForReplacingSourcePath;
...
@property (readonly, nonatomic) char shouldSuppressAssertionsAboutDeprecatedWPStyleProperties;
...
- (void)_continueImportWithProgress:(id)a0 success:(char)a1 error:(id)a2 completedSteps:(int)a3 completionQueue:(id)a4 completionHandler:(id /* block */)a5;
- (void)_performImportWithProgress:(id)a0 completedSteps:(int)a1 completionQueue:(id)a2 completionHandler:(id /* block */)a3;
- (char)_saveContextToTemporaryURL:(id)a0 passphrase:(id)a1 originalURL:(id)a2 documentUUID:(id)a3 error:(id *)a4;
...
- (void)finishImportWithProgress:(id)a0 success:(char)a1 error:(id)a2 completionQueue:(id)a3 completionHandler:(id /* block */)a4;
