...
@property (nonatomic) struct { char x0; char x1; char x2; char x3; char x4; char x5; char x6; char x7; long long x8; unsigned long long x9; } defaultLayoutSettings;
@property (readonly, nonatomic) TSCHChartLayoutCache *sceneAreaLayoutItemCache;
@property (readonly, nonatomic) char supportsAndHasRoundedCorners;
@property (readonly, nonatomic) char supportsRoundedCorners;
@property (readonly, nonatomic) char hasRoundedCorners;
@property (readonly, nonatomic) char supportsAndHasStackedSummaryLabels;
@property (readonly, nonatomic) char supportsStackedSummaryLabels;
@property (readonly, nonatomic) char hasStackedSummaryLabels;
@property (readonly, nonatomic) float maximumExplosion;
@property (readonly, nonatomic) char isSingleCircleSpecialCase;
@property (readonly, nonatomic) char containsCalloutLines;
...
@property (nonatomic) char needsCalcEngineDependentImport;
@property (nonatomic) char needsCalcEngineDependentUpgrade;
...
@property (nonatomic) char isOlderThanSage;
...
@property (nonatomic) char appearancePreservedForPreset;
...
+ (int)adjustNumberFormatType:(int)a0 forChartModel:(id)a1 gridValueType:(int)a2 hasCustomFormat:(char)a3;
+ (double)beginValueForStackedBarSeries:(id)a0 groupIndex:(unsigned long long)a1 unitSpaceIntercept:(double)a2 relativelyPositive:(char)a3 valueAxis:(id)a4;
...
+ (char)updateInitialLabelExplosionIfNeededForChartType:(id)a0 seriesNonStyles:(inout id *)a1 stylePreset:(id)a2 rowCount:(unsigned long long)a3 columnCount:(unsigned long long)a4;
...
- (void)preserveAppearanceForCurrentPresetAdoptingStylesheet:(char)a0;
...
- (struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })chartBodyBoundsForSageImportWithSageChartType:(char)a0 isSpiceDoc:(char)a1;
...
- (id)create3DSceneWithLayoutSettings:(const struct { char x0; char x1; char x2; char x3; char x4; char x5; char x6; char x7; long long x8; unsigned long long x9; } *)a0 styleProvidingSource:(id)a1;
...
- (char)hasSummaryLabelCustomFormatForGridValueType:(int)a0;
...
- (id)infoGeometryForDesiredCircumscribingGeometry:(id)a0 omitLabelPlacement:(char)a1;
...
- (char)isSingleCircleSpecialCaseOutSeries:(id *)a0;
...
- (id)newChartStylePresetByExampleWithPresetIndex:(unsigned long long)a0 withSeriesCount:(unsigned long long)a1 forTheme:(id)a2 designMode:(char)a3;
...
- (char)p_containsCalloutLinesWithModelSync:(char)a0;
...
- (id)p_infoGeometryForGeometry:(id)a0 isCircumscribing:(char)a1 omitLabelPlacement:(char)a2;
- (id)p_init;
- (void)p_invalidateCachesInLayouts:(id)a0;
- (char)p_isDownsampledStackedChart;
...
- (char)p_supportsRoundedCornersWithModelSync:(char)a0;
- (id)p_swapTuplesForApplyingPreset:(id)a0 preservingAppearance:(char)a1;
...
- (id)referenceLineStyleForAxisID:(id)a0 atIndex:(unsigned long long)a1 privateStyleOnly:(char)a2;
- (char)referenceLineStyleIsPrivate:(id)a0;
...
- (char)seriesStyleIsPrivate:(id)a0;
- (void)setCategoryAxisNonstyle:(id)a0 atIndex:(unsigned long long)a1;
- (void)setChartNonstyle:(id)a0;
- (void)setChartType:(id)a0 andSetLegendDefaults:(char)a1 gridRowIds:(id)a2 gridColumnIds:(id)a3 forDocumentLocale:(id)a4;
- (void)setDefaultLegendPositionIfNeededWithOptionalLayout:(id)a0;
- (void)setGeometry:(id)a0 clearObjectPlaceholderFlag:(char)a1;
- (void)setGeometry:(id)a0 omitLegendResize:(char)a1;
- (void)setGeometry:(id)a0 omitLegendResize:(char)a1 clearObjectPlaceholderFlag:(char)a2;
- (void)setInfoGeometryByUpdatingLegendGeometryAccommodatedForInitialLayoutGeometry:(id)a0;
- (void)setPreset:(id)a0 forceOwning:(char)a1;
...
- (id)summaryLabelCustomFormatForGridValueType:(int)a0;
- (id)summaryLabelDataFormatter;
...
- (void)upgradeAxisLabelFormatWithValuePrefix:(id)a0 valueSuffix:(id)a1 valueUseSeparator:(char)a2 horizontalPrefix:(id)a3 horizontalSuffix:(id)a4 horizontalUseSeparator:(char)a5;
- (void)upgradeChartRoundedCornerRadiusWithValue:(double)a0;
- (void)upgradeStackedSummaryLabelPropertiesWithTitleParagraphStyleIndex:(unsigned long long)a0;
