...
@property (class, readonly, nonatomic) char doesTopMostContainerSupportRTLOrigin;
@property (class, readonly, nonatomic) char shouldShowImportedDataNotificationsOnOpen;

@property (nonatomic) char hasPreUFFVersion;
@property (nonatomic) char didLoadDocumentFromTemplate;
@property (retain, nonatomic) id<TSAUIState> uiState;
@property (readonly, nonatomic) char isMultiPageForQuickLook;
...
@property (readonly, nonatomic) char documentLocaleWasUpdated;
...
@property (nonatomic) char needsMediaCompatibilityUpgrade;
@property (nonatomic) char collaborativeMediaCompatibilityUpgradeDidFail;
...
+ (id)_localizedStringKeyForValue:(double)a0 templateName:(id)a1 tableInfo:(id)a2 unlocalizedTableName:(id)a3 cellAddress:(struct TSUCellCoord { unsigned int x0; unsigned short x1; char x2; char x3; })a4 defaultValue:(out id *)a5;
...
+ (void)writePreviewImage:(id)a0 toPath:(id)a1 withIntermediateDirectories:(char)a2 name:(id)a3 group:(id)a4 queue:(id)a5 completion:(id /* block */)a6;
+ (char)writePreviewImagesToPackageDataWriter:(id)a0 scalableImage:(id)a1;
+ (char)writePreviewImagesToPackageDataWriter:(id)a0 scalableImage:(id)a1 group:(id)a2 queue:(id)a3;
+ (char)writePreviewImagesToPath:(id)a0 scalableImage:(id)a1;
+ (char)writePreviewImagesToPath:(id)a0 scalableImage:(id)a1 group:(id)a2 queue:(id)a3;
...
- (id)commandForPropagatingPresetChangeCommand:(id)a0 alwaysPreserveAppearance:(char)a1;
- (id)customFormatList;
...
- (char)exportToPath:(id)a0 exporter:(id)a1 error:(id *)a2;
- (void)fulfillPasteboardPromises;
- (char)hasICloudConflict;
- (void)importerDidFinish:(id)a0;
- (void)initializeForImport;
- (void)insertTextPresetDisplayItemsPreservingGrouping:(id)a0 insertAtBeginningOfGroup:(char)a1;
...
- (id)p_parseNumberOutOfBasename:(id)a0 hasNumber:(char *)a1 number:(unsigned long long *)a2;
...
- (char)hasRemoteDataSource;
- (id)makeIsolatedStyleMapper;
- (char)objectsNeedToBeMigrated:(id)a0;
- (void)p_addCaptionShapeStyles;
- (id)p_captureViewStateForImport:(char)a0;
...
- (id)readBuildVersionHistoryFromDiskHasPreUFFVersion:(char)a0;
