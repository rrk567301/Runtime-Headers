...
@interface TSTTableTileRowInfo : NSObject <TSTCompatibilityVersionProviding> {
    struct TSTTableTileRowBuffer { struct { char *cellData; unsigned long long cellBufferSize; unsigned long long allocatedCellBufferSize; unsigned short *offsets; unsigned short offsetBufferCount; char wideOffsets; } _private; } _currentData;
    struct TSTTableTileRowBuffer { struct { char *cellData; unsigned long long cellBufferSize; unsigned long long allocatedCellBufferSize; unsigned short *offsets; unsigned short offsetBufferCount; char wideOffsets; } _private; } _preBNCData;
...
@property (readonly, nonatomic) char usesWideOffsets;
@property (readonly, nonatomic) unsigned long long cellCount;
@property (readonly, nonatomic) unsigned short maxColumnIndex;
@property (readonly, nonatomic) char bncStorageBufferExists;
@property (readonly, nonatomic) unsigned long long archivingCompatibilityVersion;

+ (id)rowInfoFromArchive:(const void *)a0 fileFormatVersion:(unsigned long long)a1;
+ (id)rowInfoWithTileRowIndex:(unsigned int)a0 wideOffsets:(char)a1;
...
- (void)enumerateStoragesInColumnRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a0 getPreBNC:(char)a1 withBlock:(id /* block */)a2;
- (void)enumerateStoragesInColumnRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a0 withBlock:(id /* block */)a1;
- (id)initFromArchive:(const void *)a0 fileFormatVersion:(unsigned long long)a1;
- (id)initWithTileRowIndex:(unsigned int)a0 wideOffsets:(char)a1;
...
- (char)searchCellStorageRefAtColumnIndex:(unsigned short)a0 searchMask:(unsigned long long)a1;
- (long long)setCell:(id)a0 atIndex:(unsigned short)a1;
- (void)unlockForConcurrentAccess;
- (char)validateWithResult:(id *)a0;
