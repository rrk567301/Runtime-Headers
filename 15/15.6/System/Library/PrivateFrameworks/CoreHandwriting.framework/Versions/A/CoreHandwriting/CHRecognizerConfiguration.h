...
+ (char)isLocaleSupported:(id)a0 withMode:(int)a1;
+ (char)isSupportedCyrillicScriptLocale:(id)a0 withMode:(int)a1;
+ (char)isSupportedLatinScriptLocale:(id)a0 withMode:(int)a1;
...
+ (char)shouldAddSpaceToNeutralQuotationMarks:(id)a0;
+ (char)shouldAdjustGroupingForLocale:(id)a0;
+ (char)shouldAdjustGroupingHeuristicsForVeryComplexCharacters:(id)a0;
+ (char)shouldAllowSpaceInsertionGestureForLocale:(id)a0;
+ (char)shouldApplyLatinSpacingForLocale:(id)a0;
+ (char)shouldApplyThaiSpacingForLocale:(id)a0;
+ (char)shouldForceNoAlternativeRecognitionCandidates:(id)a0;
+ (char)shouldInsertSpaceBetweenPreviousChar:(id)a0 andNextChar:(id)a1 inLocale:(id)a2 inString:(id)a3 atRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a4 withQuoteCount:(id)a5;
+ (char)shouldPerformStrictCandidateFiltering:(id)a0;
+ (char)shouldRefinePrefixQueryMatchesForLocale:(id)a0;
+ (char)shouldRemoveSpaceBetweenPreviousChar:(id)a0 andNextChar:(id)a1 inLocale:(id)a2;
+ (char)shouldSwapCyrillicTopResult:(id)a0 topRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a1 englishResult:(id)a2 englishRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a3 prevousGroupTopString:(id)a4;
+ (char)shouldUseFullWidthSpaceBasedOnContextForLocale:(id)a0;
+ (char)shouldUseOriginalTokensForLocale:(id)a0;
+ (char)shouldUseStrokeReorderingForMode:(int)a0;
...
- (char)shouldRecoverCyrillicCandidates;
...
- (char)hasSamePostProcessingAsConfiguration:(id)a0;
- (char)hasSameResourcesAsConfiguration:(id)a0;
- (id)initWithMode:(int)a0 locale:(id)a1 contentType:(int)a2 autoCapitalizationMode:(int)a3 autoCorrectionMode:(int)a4 baseWritingDirection:(int)a5 enableCachingIfAvailable:(char)a6 enableGen2ModelIfAvailable:(char)a7 enableGen2CharacterLMIfAvailable:(char)a8;
- (id)initWithMode:(int)a0 locale:(id)a1 contentType:(int)a2 enableCachingIfAvailable:(char)a3 enableGen2ModelIfAvailable:(char)a4 enableGen2CharacterLMIfAvailable:(char)a5;
- (char)isEqualToRecognizerConfiguration:(id)a0;
...
- (void *)newCharacterLanguageModelAndMap:(void *)a0 force:(char)a1;
...
- (void *)newWordLanguageModelForRecognizer:(id)a0 async:(char)a1 synchronizationQueue:(id)a2;
...
- (char)shouldAddAlternativeWidthCandidates;
...
- (char)shouldConvertKanaInPostProcessing;
- (char)shouldCreatePseudoTokens;
...
- (char)shouldFilterLowProbabilityTranscriptionPaths;
...
- (char)shouldLoadCJKLexicons;
- (char)shouldMarkMultiWordOVS;
- (char)shouldMarkNoWhiteSpaceColumnsChangeable;
- (char)shouldPenalizeLetterInsertion;
- (char)shouldPenalizePrefixes;
- (char)shouldPerformArabicPostProcessing;
...
- (char)shouldPromoteCJKCommonCharacters;
- (char)shouldPromoteInLexiconCandidates;
- (char)shouldRefineCandidates;
- (char)shouldRefineChineseCharacterCandidates;
- (char)shouldRelaxFinalCandidatesThresholding;
- (char)shouldRemoveInvalidArabicDiacritics;
- (char)shouldRemoveSpacesFromHashtagsAndMentions;
- (char)shouldReorderSCTCConfusion;
- (char)shouldReplaceInvalidTokenIDs;
- (char)shouldRunLexiconCorrectionPostProcessingStep;
...
- (char)shouldTransliterateSentencesSCTC;
