...
@interface GEOResourceManifestDownloadTask : NSObject <GEODataURLSessionTaskDelegate> {
    struct GEOOnce_s { struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } lock; char didRun; } _started;
    struct GEOOnce_s { struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } lock; char didRun; } _finished;
...
- (void)_finishWithUpdatedManifest:(id)a0 newETag:(id)a1 isExistingManifestCurrent:(char)a2 error:(id)a3;
