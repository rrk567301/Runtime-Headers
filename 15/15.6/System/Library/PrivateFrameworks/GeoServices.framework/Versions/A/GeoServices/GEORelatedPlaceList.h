@class NSArray, GEOFollowUpRequestMetadata;

@interface GEORelatedPlaceList : NSObject {
    NSArray *_placeTemplates;
    int _type;
}

@property (readonly, nonatomic) char hasInitialData;
@property (readonly, nonatomic) NSArray *placeTemplates;
@property (readonly, nonatomic) int type;
@property (readonly, nonatomic) GEOFollowUpRequestMetadata *followUpRequestMetadata;
@property (readonly, nonatomic) unsigned long long numberOfInlineItems;
@property (readonly, nonatomic) NSArray *mapIdentifiers;

- (void).cxx_destruct;
- (id)initWithRelatedPlace:(id)a0;
- (id)initWithTemplatePlace:(id)a0 attributedMap:(id)a1;
- (id)initWithTemplates:(id)a0 type:(int)a1 hasInitialData:(char)a2 numberOfInlineItem:(unsigned long long)a3 followUpRequestMetadata:(id)a4;

@end
