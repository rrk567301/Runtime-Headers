...
@property (nonatomic) char warmFace;
...
+ (id)inputKeys;
+ (id)outputKeys;
+ (struct DGRGBf_t { float x0; float x1; float x2; })YIQFromFaceColor:(id)a0 forRAW:(char)a1;
...
- (struct DGRGBf_t { float x0; float x1; float x2; })_inputFaceColorYIQValueForRAW:(char)a0;
