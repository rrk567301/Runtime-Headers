@class FCPuzzleTypeController, NSString, FCSubscriptionList, NSHashTable, FCNotificationController, FCUserInfo, FCThreadSafeMutableDictionary, FCTagController;
@protocol FCPurchaseProviderType, FCCoreConfigurationManager, FCNewsAppConfigurationManager, FCLocalChannelsProvider;

@interface FCSubscriptionController : NSObject <FCSubscriptionListObserving, FCAppActivityObserving> {
    FCSubscriptionList *_subscriptionList;
    FCTagController *_tagController;
    FCPuzzleTypeController *_puzzleTypeController;
    FCThreadSafeMutableDictionary *_subscribedTagsByTagID;
    NSHashTable *_observers;
    FCNotificationController *_notificationController;
    id<FCPurchaseProviderType> _purchaseProvider;
    id<FCCoreConfigurationManager> _configurationManager;
    id<FCNewsAppConfigurationManager> _appConfigurationManager;
    FCUserInfo *_userInfo;
}

@property (retain, nonatomic) id<FCLocalChannelsProvider> localChannelsProvider;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (id)init;
- (void).cxx_destruct;
- (void)removeObserver:(id)a0;
- (void)addObserver:(id)a0;
- (id)subscriptions;
- (char)addIgnoredSubscriptionForTagID:(id)a0;
- (char)hasIgnoredSubscriptionForTagID:(id)a0;
- (id)appleNewsNotificationChannelIDs;
- (id)groupableTagIDs;
- (id)tagIDsWithType:(unsigned long long)a0;
- (void)_purchaseListDidAddTagIDs:(id)a0;
- (void)_purchaseListDidRemoveTagIDs:(id)a0;
- (void)activityObservingApplicationDidEnterBackground;
- (void)addAutoFavoriteSubscriptionForTagIDs:(id)a0 groupableSubscriptionForTagIDs:(id)a1 originProvider:(id /* block */)a2 completion:(id /* block */)a3;
- (char)addIgnoredSubscriptionForTagID:(id)a0 groupID:(id)a1;
- (char)addMutedSubscriptionForTagID:(id)a0;
- (char)addMutedSubscriptionForTagID:(id)a0 groupID:(id)a1;
- (char)addSubscriptionToTag:(id)a0 error:(id *)a1;
- (char)addSubscriptionToTag:(id)a0 notificationsEnabled:(char)a1 error:(id *)a2;
- (char)addSubscriptionToTag:(id)a0 options:(long long)a1 error:(id *)a2;
- (id)allSubscribedTagIDs;
- (char)appendSubscriptionToTag:(id)a0 notificationsEnabled:(char)a1 error:(id *)a2;
- (id)autoFavoriteTagIDs;
- (id)cachedSubscribedTags;
- (char)canAddSubscription;
- (char)canAddSubscriptionWithError:(id *)a0;
- (void)fetchAllTagsWithCallbackQueue:(id)a0 maximumCachedAge:(double)a1 qualityOfService:(long long)a2 completion:(id /* block */)a3;
- (void)fetchSubscribedTagsWithCallbackQueue:(id)a0 preferCache:(char)a1 completion:(id /* block */)a2;
- (char)hasAutoFavoriteSubscriptionForTagID:(id)a0;
- (char)hasMutedSubscriptionForTagID:(id)a0;
- (char)hasNotificationsEnabledForTag:(id)a0;
- (char)hasSubscriptionToTag:(id)a0;
- (char)hasSubscriptionToTagID:(id)a0;
- (id)ignoredTagIDs;
- (id)initWithSubscriptionList:(id)a0 tagController:(id)a1 puzzleTypeController:(id)a2 notificationController:(id)a3 purchaseProvider:(id)a4 configurationManager:(id)a5 appConfigurationManager:(id)a6 appActivityMonitor:(id)a7 userInfo:(id)a8;
- (char)isAppleNewsNotificationForTagID:(id)a0;
- (char)isChannelPermanentForTagID:(id)a0;
- (char)moveSubscriptionForTagID:(id)a0 beforeTagID:(id)a1;
- (id)mutedTagIDs;
- (id)newlySubscribedTagIDsInDateRange:(id)a0;
- (id)orderedCachedSubscribedTags;
- (id)permanentChannelIDs;
- (id)portraitExcludedAutoFavoriteTagIDs;
- (void)prewarmFromCacheWithCallbackQueue:(id)a0 completion:(id /* block */)a1;
- (void)refreshAndNotifyAboutSubscribedTags;
- (void)removeAllAutoFavoriteSubscriptions:(id /* block */)a0;
- (char)removeIgnoredSubscriptionForTagID:(id)a0;
- (char)removeIgnoredSubscriptionForTagID:(id)a0 groupID:(id)a1;
- (void)removeSubscriptionForTagID:(id)a0 type:(unsigned long long)a1;
- (void)removeSubscriptionToTag:(id)a0;
- (void)reorderSubscriptionOrderForOrderedIdentifiers:(id)a0;
- (char)setNotificationsEnabled:(char)a0 forTag:(id)a1 error:(id *)a2;
- (char)setNotificationsEnabled:(char)a0 forTagID:(id)a1 error:(id *)a2;
- (id)subscribedTagForTagID:(id)a0;
- (id)subscribedTagIDs;
- (id)subscribedTagIDsWithNotificationsEnabled;
- (id)subscriptionForTag:(id)a0;
- (id)subscriptionForTagID:(id)a0;
- (void)subscriptionList:(id)a0 didAddSubscriptions:(id)a1 changeSubscriptions:(id)a2 moveSubscriptions:(id)a3 removeSubscriptions:(id)a4;
- (void)subscriptionListDidResetToEmpty:(id)a0;
- (void)subscriptionListDidStopSyncingRemoteChanges:(id)a0;
- (void)subscriptionListWillStartSyncingRemoteChanges:(id)a0;
- (id)subscriptionSurfacingHeadline:(id)a0;
- (void)subscriptionsWithCompletion:(id /* block */)a0;
- (id)subscriptionsWithType:(unsigned long long)a0;

@end
