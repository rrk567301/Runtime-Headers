...
@property (nonatomic) char recordedAnyFatalFailures;
@property (nonatomic) char haveMigratedAnySyncingAccounts;
@property (nonatomic) char haveMigratedAnyLocalCalendarsOrGroups;
...
@property (nonatomic) char haveDeletedExistingLocalCalendars;
...
- (char)_createAccountsForSubscriptionInfos:(id)a0 inParentAccount:(id)a1;
- (char)_createChildAccountForDelegatePrincipals:(id)a0 withParentAccount:(id)a1;
- (char)_createChildDelegateAccounts;
- (char)_createPublicationAccounts;
- (void)_createSpecialCalendarsForCalDAVPrincipal:(id)a0 inStore:(void *)a1;
- (void *)_createStoreForPrincipalWithUid:(id)a0;
- (char)_createSubscriptionAccounts;
...
- (char)_patchAccountIdsForCreatedPublishedCalendars;
- (char)_patchAccountIdsForCreatedSubscribedCalendars;
- (char)_patchDelegateOwnersForCreatedStores;
- (char)_patchExternalIdsForCreatedStores;
- (char)_processAccountChanges;
- (char)_processAccountsToDelete;
- (void)_recordAccountIdentifierToBeDeleted:(id)a0;
- (void)_recordMigrationFailure:(id)a0 isFatal:(char)a1;
- (void)_recordMigrationFailureWithDescription:(id)a0 inStage:(unsigned long long)a1 underlyingError:(id)a2 relatedTo:(id)a3;
- (void)_recordMigrationFailureWithDescription:(id)a0 inStage:(unsigned long long)a1 underlyingError:(id)a2 relatedTo:(id)a3 isFatal:(char)a4;
- (void)_removeItemsToDeleteContainedByDirectory:(id)a0;
- (char)_saveAccountChanges;
- (char)_saveCalendarDatabase:(struct CalDatabase { struct __CFRuntimeBase { unsigned long long x0; _Atomic unsigned long long x1; } x0; int x1; struct CPRecordStore *x2; struct CalEventOccurrenceCache *x3; struct CalScheduledTaskCache *x4; void *x5; void *x6; struct __CFDictionary *x7; struct __CFDictionary *x8; struct os_unfair_lock_s { unsigned int x0; } x9; unsigned int x10; unsigned int x11; struct __CFArray *x12; struct __CFString *x13; struct __CFArray *x14; int x15; int x16; struct __CFString *x17; struct __CFURL *x18; struct __CFString *x19; struct __CFString *x20; unsigned long long x21; int x22; int x23; int x24; id /* block */ x25; struct _opaque_pthread_mutex_t { long long x0; char x1[56]; } x26; BOOL x27; struct __CFArray *x28; BOOL x29; struct __CFSet *x30; id x31; id x32; id x33; int x34; id x35; id x36; id x37; char *x38; unsigned int x39; unsigned int x40; int x41; unsigned long long x42; BOOL x43; BOOL x44; BOOL x45; BOOL x46; BOOL x47; BOOL x48; BOOL x49; } *)a0;
- (char)_setAccountProperties;
- (char)_setCalDAVPrincipalsAccountProperties;
- (char)_setExchangePrincipalsAccountProperties;
- (char)_setPublishedCalendarAccountProperties;
- (void)_setSelfAttendeeOnEvent:(void *)a0 withAccountInfo:(id)a1;
- (char)_setSubscribedCalendarAccountProperties;
- (char)_updateAccount:(id)a0 withCalDAVPrincipal:(id)a1;
- (char)_updateAccount:(id)a0 withExchangePrincipal:(id)a1 delegatePrincipals:(id)a2;
- (char)_updateCreatedDatabaseItems;
...
- (void)setHolidayAccountEnabled:(char)a0;
