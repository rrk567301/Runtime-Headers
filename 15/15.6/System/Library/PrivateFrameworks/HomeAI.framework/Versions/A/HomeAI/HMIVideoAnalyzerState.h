...
@property (readonly) char monitored;
...
@property (readonly, getter=isThermalAndSystemResourceUsageLevelIgnored) char thermalAndSystemResourceUsageLevelIgnored;
@property (readonly) char encode;
@property (readonly) char encoder;
...
- (void).cxx_destruct;
- (void)check;
- (id)initWithConfiguration:(id)a0 dynamicConfiguration:(id)a1 identifier:(id)a2 monitored:(char)a3 analysisFPS:(double)a4 timeSinceAnalyzerStarted:(double)a5 timeSinceLastFragmentWasReceived:(double)a6 bufferFillRatio:(double)a7 bufferSize:(unsigned long long)a8 delay:(double)a9 currentPTS:(struct { long long x0; int x1; unsigned int x2; long long x3; })a10 numDecodedSamples:(unsigned long long)a11 numDidAnalyzeFrames:(unsigned long long)a12 numDidAnalyzeFragments:(unsigned long long)a13 numDidAnalyzePackages:(unsigned long long)a14 numDidCreateTimelapseFragments:(unsigned long long)a15 averageAnalysisTime:(double)a16 encode:(char)a17 encoder:(char)a18;
