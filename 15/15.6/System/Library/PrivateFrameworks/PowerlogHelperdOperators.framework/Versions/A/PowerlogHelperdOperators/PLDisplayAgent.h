...
@property char alsPluginKeyRegistered;
@property (retain) PLXPCListenerOperatorComposition *multitouchXPCListener;
@property char isMultitouchLoggingEnabled;
...
@property char isMIEActive;
@property char firstEntryOnInit;
@property char wasDisplayOn;
@property char isDisplayOnNow;
@property char isDisplayHighBrightness;
@property char isFirstTimeModeling;
...
+ (char)shouldLogALSPowerSaved;
+ (char)shouldLogAPL;
+ (char)shouldLogBLR;
+ (char)shouldLogBacklightControl;
+ (char)shouldLogDisplay;
+ (char)shouldLogFromDCP;
+ (char)shouldLogKeyboardBrightness;
+ (char)shouldLogLCD;
+ (char)shouldLogLuxStats;
+ (char)shouldLogTouch;
+ (char)shouldLogUserBrightness;
+ (char)shouldModelPowerFromAPL;
+ (char)shouldModelPowerFromIOMFB;
...
- (char)isAlsEnabled;
- (char)listenForAlsPluginMatchingKeys:(struct __IOHIDEventSystemClient { } *)a0 withKeys:(const void **)a1 withValues:(const void **)a2 withCount:(unsigned long long)a3;
...
- (void)logEventBackwardUserTouch:(char)a0;
...
- (void)logEventPointDisplayForBlock:(id)a0 isActive:(char)a1 withEntryDate:(id)a2;
...
- (char)updateDisplayIOReportStats:(id)a0 logZeroFramesOnlyOnce:(char)a1;
