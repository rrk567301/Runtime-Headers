...
@property (nonatomic) char allowStreaming;
@property (nonatomic) char onDemand;
@property (nonatomic) float maxHighlightDuration;
@property (nonatomic) char faceDominated;
...
+ (char)canAnalyzeUndegraded:(id)a0 withResources:(id)a1;
+ (char)enableAudioVideoFusion;
+ (char)getEnableMovieHumanAction;
+ (float)getMaximumHighlightInSec;
+ (BOOL)isHeuristicStickerScoreEnabled;
+ (char)persistAudioEmbedding;
+ (void)purgeCachedModels;

- (void).cxx_destruct;
- (id)analyzeAsset:(id /* block */)a0 streamed:(char *)a1;
