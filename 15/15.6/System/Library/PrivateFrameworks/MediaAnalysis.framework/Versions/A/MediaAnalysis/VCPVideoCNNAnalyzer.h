@class VCPEspressoV2Data, VCPVideoTransformerBackbone, VCPVideoCNNBackbone, VCPVideoEmbeddings, VCPTransforms, VCPAdaptiveSegmentAnalyzer, VCPVideoCaptionAnalyzer, NSMutableArray, VCPEmbeddingSummarizationAnalyzer, NSString, VCPVideoCNNAutoplay, VCPVideoCNNHighlight, VCPVideoPersonDetector, VCPVideoCNNCameraMotion, VCPVideoCNNQuality;

@interface VCPVideoCNNAnalyzer : VCPVideoAnalyzer {
    VCPVideoCNNBackbone *_backbone;
    VCPVideoTransformerBackbone *_videoTransformerBackbone;
    VCPTransforms *_transformImage;
    VCPTransforms *_transformImageMUBB;
    NSMutableArray *_tasks;
    NSMutableArray *_postTasks;
    NSMutableArray *_privateTasks;
    VCPVideoEmbeddings *_videoEmbeddings;
    VCPVideoEmbeddings *_videoEmbeddingsX3D;
    VCPAdaptiveSegmentAnalyzer *_adaptiveSegmentAnalyzer;
    VCPEmbeddingSummarizationAnalyzer *_embeddingSummarizationAnalyzer;
    VCPVideoCaptionAnalyzer *_videoCaptionAnalyzer;
    float *_inputData;
    float *_inputDataSettling;
    VCPEspressoV2Data *_inputDataForTransformer;
    VCPEspressoV2Data *_inputDataBackup;
    int _inputWidth;
    int _inputHeight;
    int _inputWidthMUBB;
    int _inputHeightMUBB;
    int _mubbSampleScale;
    struct { long long value; int timescale; unsigned int flags; long long epoch; } _timeLastProcess;
    struct { long long value; int timescale; unsigned int flags; long long epoch; } _timeLastDetection;
    struct { long long value; int timescale; unsigned int flags; long long epoch; } _timeStart;
    int _validFrames;
    int _validFramesSettling;
    char _enoughFrames;
    char _enoughFramesSettling;
    char _enoughFramesTransformer;
    VCPVideoPersonDetector *_personDetector;
    NSString *_resConfig;
    VCPVideoCNNAutoplay *_autoplay;
    VCPVideoCNNCameraMotion *_cameraMotion;
    VCPVideoCNNQuality *_quality;
    VCPVideoCNNHighlight *_highlight;
    struct CGRect { struct CGPoint { double x; double y; } origin; struct CGSize { double width; double height; } size; } _regionCrop;
    struct { long long value; int timescale; unsigned int flags; long long epoch; } _timeEnd;
    char _postInference;
    float _minProcessingInterval;
    float _photoOffset;
}

@property (readonly, nonatomic) char highlightEnabled;
@property (readonly, nonatomic) char postInference;
@property (readonly, nonatomic) float minProcessingInterval;
@property (readonly, nonatomic) struct { long long x0; int x1; unsigned int x2; long long x3; } timeStart;

+ (char)forcePersonDetection;
+ (char)isAdaptiveSegmentEnabled;
+ (char)isMLHighlightEnabled;
+ (char)isMLSettlingEffectPregatingEnabled;
+ (char)isMUBackboneEnabled;
+ (char)isVideoSegmentCaptionEnabled;

- (void)dealloc;
- (void).cxx_destruct;
- (id)results;
- (int)analyzeFrame:(struct __CVBuffer { } *)a0 withTimestamp:(struct { long long x0; int x1; unsigned int x2; long long x3; })a1 andDuration:(struct { long long x0; int x1; unsigned int x2; long long x3; })a2 flags:(unsigned long long *)a3;
- (int)configForAspectRatio:(float)a0;
- (void)copyFrames;
- (int)copyImage:(struct __CVBuffer { } *)a0 withChannels:(int)a1 settling:(char)a2;
- (int)copyMUBBImage:(struct __CVBuffer { } *)a0 withChannels:(int)a1;
- (int)finishAnalysisPass:(struct { struct { long long x0; int x1; unsigned int x2; long long x3; } x0; struct { long long x0; int x1; unsigned int x2; long long x3; } x1; })a0;
- (id)initWithTimeOfInteret:(id)a0 frameRate:(float)a1 isLivePhoto:(char)a2 phFaces:(id)a3 timeRange:(struct { struct { long long x0; int x1; unsigned int x2; long long x3; } x0; struct { long long x0; int x1; unsigned int x2; long long x3; } x1; })a4 withEmbeddings:(id)a5 requestedAnalyses:(unsigned long long)a6 photoOffset:(float)a7;
- (struct { long long x0; int x1; unsigned int x2; long long x3; })isAnalysisResultNeeded:(struct { long long x0; int x1; unsigned int x2; long long x3; })a0;
- (int)loadAnalysisResults:(id)a0 audioResults:(id)a1;
- (int)loadAnalysisResultsFrom:(id)a0 actionAnalyzer:(id)a1 atTime:(struct { long long x0; int x1; unsigned int x2; long long x3; })a2;
- (int)loadFullAnalysisResults:(id)a0 actionAnalysisResults:(id)a1 predictedTimeRange:(struct { struct { long long x0; int x1; unsigned int x2; long long x3; } x0; struct { long long x0; int x1; unsigned int x2; long long x3; } x1; })a2 timestamp:(struct { long long x0; int x1; unsigned int x2; long long x3; })a3;
- (id)privateResults;
- (int)runTasks:(struct { long long x0; int x1; unsigned int x2; long long x3; })a0 duration:(struct { long long x0; int x1; unsigned int x2; long long x3; })a1 persons:(id)a2 regionCrop:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a3;

@end
