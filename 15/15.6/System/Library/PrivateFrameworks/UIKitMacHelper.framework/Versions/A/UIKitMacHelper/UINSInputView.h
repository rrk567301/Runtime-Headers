...
@interface UINSInputView : NSView <NSTextInputClient, NSServicesMenuRequestor, UINSKeyboardEventHandler> {
    ...
    char _trackingDragLocally;
    char _gameControllerInputAlternativeActive;
    ...
    char _performingKeyEquivalent;
    char _waitingForMaybeDidBecomeFirstResponder;
...
- (char)_isNSTextInputContextiOSMacClient;
...
- (char)_chooseNewKeyViewWithDirection:(unsigned long long)a0;
- (void)_consumeMouseEventsUntilMouseUp;
- (char)_isResentEvent:(id)a0;
...
- (char)performDefaultBehaviorForEvent:(id)a0;
- (id)uinsFontManager;
- (void)forwardPrepareForDragOperation:(id)a0;
- (char)_areWeAppKitFirstResponder;
...
- (void)_forwardKeyboardAction:(id)a0 propagateUpHostResponderChain:(char)a1;
- (char)_handleEventByInputMethod:(id)a0;
- (void)_installMouseTracking;
- (char)_isSpeaking;
- (char)_isValidKeyView:(id)a0;
- (char)_keyboardDelegateRequiresKeyEvents;
...
- (char)_sendKeyEvent:(id)a0 isDown:(char)a1;
- (char)_shouldExcludeEventFromInputContext:(id)a0;
- (char)_validateMenuItemForWritingTools:(id)a0;
- (id)_writingToolsDelegateForResponder:(id)a0;
- (char)acceptKeyViewHandoff:(char)a0;
...
- (char)forwardPerformDragOperation:(id)a0;
- (void)forwardUINSEventToAppKit:(id)a0;
- (char)handleEventByInputContext:(id)a0;
- (char)handleEventByInputMethod:(id)a0;
