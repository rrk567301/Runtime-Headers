...
@property char topHitIsInOfLastResponse;
...
@property (nonatomic) char prefersLocalUserTypedSuggestion;
@property (nonatomic) char wantsSuggestions;
...
@property (nonatomic) char forceStableResults;
@property char isAdvancedQuerySyntax;
@property char isMath;
@property char isPeopleSearch;
@property (retain) NSDictionary *nlpQueryCategoryPreference;
@property (retain) NSString *nlpQueryConfidence;
@property (readonly) char parsecFinished;
@property (readonly) char metadataFinished;
@property (readonly) char corespotlightFinished;
...
@property (nonatomic) char parsecCameLaterThanSRT;
@property (nonatomic) char hasSentTopHit;
@property int source;
...
- (void)buildSyndicatedSectionWithSections:(id)a0 queryContext:(id)a1 isPeopleSearch:(char)a2;
...
- (void)_sendResponseKind:(int)a0 withSections:(id)a1 withServerSuggestions:(id)a2 withLocalSuggestions:(id)a3 toQuery:(id)a4 topHitIsIn:(char)a5 delay:(double)a6 internal:(char)a7;
...
- (void)filterPreferedSectionResults:(id)a0 isAdvancedQuerySyntax:(char)a1;
- (id)findDistinctPathIdentifiers;
- (void)finishWithResponseKind:(int)a0 sections:(id)a1 serverSuggestions:(id)a2 localSuggestions:(id)a3 groupedResults:(id)a4 userQueryString:(id)a5 topHitIsIn:(char)a6;
- (void)markAsEngaged:(id)a0 forQueryString:(id)a1 timestamp:(double)a2 preview:(char)a3;
- (void)markAsUsed:(id)a0 subItem:(char)a1;
...
- (void)rankAndPrune:(id)a0 maxResults:(unsigned long long)a1 totalResultCount:(long long *)a2 query:(id)a3 currentLocale:(id)a4 isAdvancedQuerySyntax:(char)a5 currentTime:(double)a6;
