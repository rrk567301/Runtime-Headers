...
@property (nonatomic) char alwaysSaveRecordZones;
@property (nonatomic) unsigned long long maxPendingFetchedRecords;
@property (nonatomic, getter=isAutomaticSyncingEnabled) char automaticSyncingEnabled;
@property (weak, nonatomic) id<SYDSyncManagerDelegate> delegate;
@property (retain, nonatomic) CKSyncEngine *engine;
@property (readonly, nonatomic) char useSyncAnchor;
@property (nonatomic) char disableAutomaticRetries;
...
+ (char)shouldIgnoreFetchError:(id)a0;
...
- (char)getSyncEngineStateSerialization:(id *)a0 error:(id *)a1;
- (void)handleSyncEngineStateUpdate:(id)a0;
- (char)hasPendingChangesToUpload;
- (char)hasSyncEngine;
- (id)initWithContainer:(id)a0 coreDataStore:(id)a1 storeType:(long long)a2 testConfiguration:(id)a3;
- (char)initializeSyncEngineWithError:(id *)a0;
- (char)isIdentityLostError:(id)a0;
...
- (char)removeObjectForKey:(id)a0 inStoreWithIdentifier:(id)a1 error:(id *)a2;
- (char)removeObjectForKey:(id)a0 inStoreWithIdentifier:(id)a1 excludeFromChangeTracking:(char)a2 didRemoveValue:(char *)a3 error:(id *)a4;
- (char)removeObjectForKey:(id)a0 inStoreWithIdentifier:(id)a1 excludeFromChangeTracking:(char)a2 error:(id *)a3;
...
- (char)setObject:(id)a0 forKey:(id)a1 inStoreWithIdentifier:(id)a2 error:(id *)a3;
- (char)setObject:(id)a0 forKey:(id)a1 inStoreWithIdentifier:(id)a2 excludeFromChangeTracking:(char)a3 enforceQuota:(char)a4 didChangeValue:(char *)a5 error:(id *)a6;
- (char)setObject:(id)a0 forKey:(id)a1 inStoreWithIdentifier:(id)a2 excludeFromChangeTracking:(char)a3 enforceQuota:(char)a4 error:(id *)a5;
- (char)shouldRetryAfterModifyError:(id)a0;
