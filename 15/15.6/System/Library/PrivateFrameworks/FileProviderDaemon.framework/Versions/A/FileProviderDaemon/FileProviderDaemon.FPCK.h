...
@interface FileProviderDaemon.FPCK : NSObject <FPFSCheckerDelegate> {
...
+ (char)dumpDatabaseAt:(id)a0 fullDump:(char)a1 writeTo:(id)a2 error:(id *)a3;

- (void)dealloc;
- (id)init;
- (void).cxx_destruct;
- (char)checker:(id)a0 handleItem:(int)a1 itemStatus:(const struct { BOOL x0; BOOL x1; BOOL x2; BOOL x3; BOOL x4; BOOL x5; BOOL x6; BOOL x7; BOOL x8; BOOL x9; BOOL x10; struct fpfs_fileattrs *x11; struct stat *x12; unsigned int x13; } *)a2 under:(id)a3 brokenInvariants:(unsigned long long)a4;
- (char)checker:(id)a0 handleLockedItemAtPath:(id)a1 handle:(const struct fpfs_item_handle { unsigned long long x0; unsigned long long x1; unsigned int x2; unsigned int x3; char *x4; int x5; unsigned int x6; } *)a2;
- (id)initWithDatabasesBackupsPaths:(id)a0 volumeRole:(unsigned int)a1 providerDomainID:(id)a2 domainUserInfo:(id)a3 reason:(unsigned long long)a4 usingFPFS:(char)a5 iCDPackageDetection:(char)a6 useShouldPause:(char)a7 shouldPause:(id /* block */)a8 sendDiagnostics:(id /* block */)a9 saveCheckpoint:(id /* block */)a10 isInvalidated:(id /* block */)a11;
- (char)launchFromURLs:(id)a0 options:(unsigned long long)a1 error:(id *)a2 resultHandler:(id /* block */)a3;
