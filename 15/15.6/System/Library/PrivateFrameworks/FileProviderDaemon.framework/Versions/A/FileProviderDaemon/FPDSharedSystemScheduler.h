...
@interface FPDSharedSystemScheduler : NSObject {
    ...
    char _preventRunning;
    char _manualScheduling;
    char _wasCheckedIn;
    char _isExpired;
}

@property (class, readonly) NSObject<OS_dispatch_queue> *queue;
@property (class) char disableDelayedUnregistration;

@property char preventRunning;
@property (getter=useManualScheduling) char manualScheduling;
@property (readonly) char hasRegisteredExecutors;
...
+ (void)checkIn;
+ (char)_backgroundTaskCriteriaEqualWithFirst:(id)a0 second:(id)a1;
...
- (char)canOrIsForcedToRunCheckingDeferral:(char)a0 reason:(id *)a1;
- (char)canRunCheckingDeferral:(char)a0 reason:(id *)a1;
...
- (char)runIfPossible:(id /* block */)a0;
