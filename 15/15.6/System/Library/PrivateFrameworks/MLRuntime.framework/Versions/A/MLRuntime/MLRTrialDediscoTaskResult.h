@class NSString, NSDictionary;

@interface MLRTrialDediscoTaskResult : MLRTrialTaskResult

@property (readonly, copy, nonatomic) NSString *namespaceName;
@property (readonly, copy, nonatomic) NSString *recipeFactorName;
@property (readonly, copy, nonatomic) NSDictionary *additionalKeyVariables;

+ (id)baseKeyFromFormat:(id)a0 variables:(id)a1;

- (id)description;
- (void).cxx_destruct;
- (char)record:(id)a0 data:(id)a1 encodingSchema:(id)a2 metadata:(id)a3 errorOut:(id *)a4;
- (id)initWithJSONResult:(id)a0 identifier:(id)a1;
- (id)initWithJSONResult:(id)a0 namespaceName:(id)a1 factorName:(id)a2 additionalKeyVariables:(id)a3;
- (char)submitForTask:(id)a0 error:(id *)a1;
- (char)submitWithTRIClient:(id)a0 error:(id *)a1;
- (id)variablesFromTrialClient:(id)a0;

@end
