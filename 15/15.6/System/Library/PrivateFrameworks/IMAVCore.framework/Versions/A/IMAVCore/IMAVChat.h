...
@property (nonatomic, setter=setRemoteMute:) char isRemoteMute;
@property (nonatomic) char isSendingAudio;
@property (readonly, nonatomic) char didRemoteMute;
@property (nonatomic, setter=setPaused:) char isPaused;
@property (readonly, nonatomic) char hasReceivedFirstFrame;
@property (nonatomic) char isSendingVideo;
@property (readonly, nonatomic) char didRemotePause;
@property (readonly, nonatomic) char _allParticipantsUsingICE;
...
@property (readonly, nonatomic) char _usesRelay;
@property (readonly, nonatomic) char _isProxy;
...
@property (nonatomic, setter=_setIsVideoInterrupted:) char _isVideoInterrupted;
@property (nonatomic, setter=_setIsAudioInterrupted:) char _isAudioInterrupted;
...
@property (nonatomic, setter=_setNeedsAudioRestart:) char _needsAudioRestart;
@property (nonatomic, setter=_setNeedsVideoRestart:) char _needsVideoRestart;
...
@property (nonatomic, setter=_setConnectionStarted:) char _connectionStarted;
...
@property (nonatomic, getter=isTerminating, setter=_setTerminating:) char terminating;
@property (nonatomic, setter=_setDidRemoteMute:) char didRemoteMute;
@property (nonatomic, setter=_setDidRemotePause:) char didRemotePause;
@property (nonatomic, setter=_setAirplaneModeEnabled:) char airplaneModeEnabled;
@property (nonatomic, setter=_setMetadataFinalized:) char metadataFinalized;
...
@property (readonly, nonatomic) char isUsingWifi;
...
+ (id)_avChatWithMatchingIMHandles:(id)a0 video:(char)a1;
+ (id)_chatList;
+ (id)_chatListLock;
...
+ (int)systemSupportsNewOutgoingConferenceTo:(id)a0 isVideo:(char)a1;
...
- (char)_moveVCPartyID:(id)a0 toIndex:(unsigned int)a1 inCount:(unsigned int)a2;
- (void)_noteFirstFrame;
- (void)_postStateToDelegateIfNecessary;
- (void)conferencePersonWithID:(id)a0 didMute:(char)a1;
...
- (void)_cacheBool:(char)a0 forKey:(id)a1;
- (char)_cachedBoolForKey:(id)a0;
...
- (void)_conferenceOtherParticipant:(id)a0 didConnect:(char)a1;
...
- (void)_handleGenericAVMessageFromParticipant:(id)a0 type:(unsigned int)a1 userInfo:(id)a2 handled:(char)a3;
...
- (char)_hasCachedBoolForKey:(id)a0;
- (id)_initOutgoingTo:(id)a0 isVideo:(char)a1 isRelayed:(char)a2 GUID:(id)a3;
- (id)_initWith:(id)a0 invitedBy:(id)a1 sessionID:(unsigned int)a2 GUID:(id)a3 video:(char)a4 extraProperties:(id)a5;
- (id)_initWith:(id)a0 invitedBy:(id)a1 sessionID:(unsigned int)a2 isRelayed:(char)a3 GUID:(id)a4 video:(char)a5 extraProperties:(id)a6;
...
- (void)_postParticipantMediaChangeNotification:(id)a0 cameraChanged:(char)a1 orientationChanged:(char)a2 aspectChanged:(char)a3 cameraWillSwitch:(char)a4 camera:(unsigned int)a5 orientation:(unsigned int)a6 aspect:(struct CGSize { double x0; double x1; })a7;
- (char)_processVCResponseDict:(id)a0;
...
- (void)_submitCallStartedLoggingWithRecipientID:(id)a0 isCaller:(char)a1 isVideo:(char)a2;
- (id)_timings;
- (void)_updateIMHandleInBuddyList:(id)a0;
- (void)_vccInitDidFinish:(id)a0;
- (void)beginChat;
- (void)conferenceAVConferenceCallID:(long long)a0 didConnect:(char)a1;
- (void)conferenceDidPauseAudioWithCallID:(long long)a0 didPauseAudio:(char)a1;
- (void)conferenceDidStopWithCallID:(long long)a0 error:(id)a1;
- (void)conferencePersonWithID:(id)a0 didDegrade:(char)a1;
- (void)conferencePersonWithID:(id)a0 didPause:(char)a1;
- (void)conferencePersonWithID:(id)a0 localIPDidChange:(id)a1;
- (void)conferencePersonWithID:(id)a0 mediaDidStall:(char)a1;
...
- (id)initIncomingFrom:(id)a0 isVideo:(char)a1 callerProperties:(id)a2;
- (id)initOutgoingTo:(id)a0 isVideo:(char)a1;
- (id)initOutgoingTo:(id)a0 isVideo:(char)a1 isRelayed:(char)a2;
- (id)initOutgoingTo:(id)a0 isVideo:(char)a1 isRelayed:(char)a2 GUID:(id)a3;
