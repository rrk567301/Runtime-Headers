...
@interface MOMediaView : NSView <NSScrollViewDelegate> {
    ...
    struct { char needsReload; char reloading; char waitingForVisibleContent; char viewIsPrepared; char inModalSelectionMode; char modalSelectionModeSavedAllowsContinuousSelectionState; char modalSelectionModeSavedAllowsPaintingSelectionState; char modalSelectionModeSavedLassoSelectsInverse; char dataSourceNumberOfSections; char dataSourceMediaSectionAtIndexPath; char dataSourceCellReuseIdentifierAtIndexPath; char dataSourceSupplementaryViewReuseIdentifierOfKindAtIndexPath; char dataSourceCanMoveItemsAtIndexPaths; char dataSourceAllowedDropPositionsForItemsAtIndexPathsMovedToIndexPath; char dataSourceDragOperationForItemsAtIndexPathsMovedOntoItemAtIndexPath; char dataSourceMoveItemsAtIndexPathsToIndexPathsDropPosition; char dataSourceShouldExchangeItemsAtIndexPathsWithProposedIndexPaths; char dataSourceExchangeItemsAtIndexPathsWithIndexPaths; char delegateTargetContentOffsetForProposedContentOffset; char delegateWillUpdateCell; char delegateDidUpdateCell; char delegateWillUpdateSupplementaryView; char delegateDidUpdateSupplementaryView; char delegateWillDisplayCell; char delegateDidEndDisplayingCell; char delegateShouldSelectItem; char delegateShouldDeselectItem; char delegateDidSelectItem; char delegateDidDeselectItem; char delegateSelectionWillChange; char delegateSelectionDidChange; char delegateSelectionWillAddAndRemove; char delegateSelectionDidAddAndRemove; char delegateSectionsForSelectAllAction; char delegateBackgroundWasClicked; char delegateBackgroundWasRightClicked; char delegateWillBeginScrolling; char delegateDidScroll; char delegateDidEndScrolling; char delegateDidEndScrollingAnimation; char delegateMouseDownWithEvent; char delegateItemWasDoubleClickedAtIndexPathWithEvent; char delegateItemWasRightClickedAtIndexPathWithEvent; char delegateLoadingThumbnail; char delegateUpdatesLayoutOnDrag; char delegateImageForDraggedItemAtIndexPath; char delegatePasteboardWriterForItemAtIndexPath; char delegateDraggingItemForIndexPathProposedDraggingItem; char delegatePreferredDraggingFormation; char delegateCreatedDraggingSessionForItemsAtIndexPaths; char delegateDraggingSessionSourceOperationMaskForDraggingContext; char delegateDraggingSessionWillBeginAtPoint; char delegateDraggingSessionMovedToPoint; char delegateDraggingSessionEndedAtPointDragOperation; char delegateDragSourceIdentifier; char delegatePrepareForDragOperation; char delegatePerformDragOperation; char delegateDraggingEntered; char delegateDraggingUpdated; char delegateDraggingExited; char delegateDraggingEnded; } _mediaViewFlags;
...
@property (nonatomic) char allowsMotionBlurScrolling;
@property (nonatomic) char showsLowThumbnails;
@property (nonatomic) char showsBestThumbnails;
@property (nonatomic) char showsThumbnailsIndicators;
...
@property char isCancellingAllOperations;
...
@property (nonatomic) char allowsBestOnDeceleration;
@property (nonatomic) char fadesBestOnUpdates;
@property (nonatomic) unsigned long long thumbnailMode;
@property (nonatomic) char usesBestNearestInterpolation;
@property (nonatomic) char usesSmoothScrolling;
@property (nonatomic) char animateSelection;
@property (nonatomic) char rearrangingEnabled;
@property (nonatomic) char rearrangingAllowAutoscroll;
@property (nonatomic) char externalDropEnabled;
@property (readonly, nonatomic, getter=isLassoSelectionInProgress) char lassoSelectionInProgress;
...
- (char)collectionView:(id)a0 canMoveItemsAtIndexPaths:(id)a1;
...
- (void)collectionView:(id)a0 indexPathsForSelectedItemsDidAdd:(id)a1 remove:(id)a2 animated:(char)a3;
- (void)collectionView:(id)a0 indexPathsForSelectedItemsWillAdd:(id)a1 remove:(id)a2 animated:(char)a3;
...
- (char)collectionView:(id)a0 performDragOperation:(id)a1;
- (char)collectionView:(id)a0 prepareForDragOperation:(id)a1;
- (char)collectionView:(id)a0 shouldExchangeItemsAtIndexPaths:(id)a1 withProposedIndexPaths:(id)a2;
...
- (char)collectionViewUpdatesLayoutOnDrag:(id)a0;
...
- (char)_mediaViewBackgroundHitByEvent:(id)a0;
...
- (char)_canShowBestThumbnailsForCell:(id)a0;
- (char)_closeEnoughToLoadBestThumbnailForCell:(id)a0;
...
- (void)_setMediaViewLayout:(id)a0 animated:(char)a1 completion:(id /* block */)a2;
...
- (void)_updateCollectionViewWithMediaViewLayout:(id)a0 animated:(char)a1 completion:(id /* block */)a2;
...
- (void)_viewDidPrepare:(char)a0;
- (void)_viewWillPrepare;
- (void)_waitUntilVisibleContentIsPrepared:(double)a0 includingBest:(char)a1 completion:(id /* block */)a2;
...
- (void)deselectItemsInSection:(long long)a0 animated:(char)a1;
...
- (char)hasBestThumbnailCellOperationForCell:(id)a0;
- (char)hasBestThumbnailPrefetchOperationForIndexPath:(id)a0;
- (char)hasLowThumbnailCellOperationForCell:(id)a0;
- (char)hasLowThumbnailPrefetchOperationForIndexPath:(id)a0;
- (char)hasMainQueueThumbnailOperationForContext:(id)a0;
- (char)hasMainThreadRunLoopBlockWithToken:(id)a0;
...
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 mediaViewLayout:(id)a1 responsiveScrolling:(char)a2;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 responsiveScrolling:(char)a1;
- (void)installMemoryPressureListener;
- (void)invalidateAllMemoryCaches;
- (void)invalidateThumbnailsMemoryCaches;
- (char)isVisibleContentPrepared:(char)a0;
...
- (void)prefetchAdjacentThumbnails:(char)a0;
...
- (void)selectItemsInSection:(long long)a0 animated:(char)a1;
- (void)setConcurrencyEnabled:(char)a0;
- (void)setMediaViewLayout:(id)a0 animated:(char)a1 completion:(id /* block */)a2;
- (void)setModalSelectionMode:(char)a0;
- (void)setMotionBlurRadius:(double)a0 animated:(char)a1;
- (char)shouldFadeBestThumbnailsForCell:(id)a0;
