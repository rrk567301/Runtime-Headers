...
+ (char)canAcceptPendingInvitationForAlbum:(id)a0 error:(id *)a1;
...
+ (char)prepareToEnqueueBatchOfOriginalAssetUUIDs:(id)a0 withBatchCommentText:(id)a1 assetsSharingInfos:(id)a2 customExportsInfo:(id)a3 andTrimmedVideoPathInfo:(id)a4 sharedAlbum:(id)a5 isNewAlbum:(char)a6 error:(id *)a7;
...
+ (char)sharedStreamsEnabledForPhotoLibraryURL:(id)a0;
...
+ (char)_derivativeExistsForAsset:(id)a0 cloudPlaceholderKind:(short)a1;
+ (id)_downloadRequestQueue;
+ (char)_isUserLoggedIntoiCloud;
...
+ (char)_recentlyRequestedDownloadAsset:(id)a0 cloudPlaceholderKind:(short)a1;
+ (void)_resetServerSideConfigurationDictionary;
+ (char)_retrieveURLsFromAssetWithUUID:(id)a0 withExportedFileURL:(id)a1 primaryURL:(id *)a2 videoComplementURL:(id *)a3;
...
+ (char)accountMatchesEmail:(id)a0;
+ (void)accountSettingsChanged;
+ (void)applicationIsInForeground:(char)a0 photoLibraryURL:(id)a1;
+ (char)canCreateStreamInPhotoLibrary:(id)a0 error:(id *)a1;
+ (char)canSetUserCloudSharedLiked:(char)a0 forAssets:(id)a1 error:(id *)a2;
...
+ (id)directoryPathForDownloadActivityForAlbumGUID:(id)a0 create:(char)a1;
+ (id)directoryPathForUploadActivityForAlbumGUID:(id)a0 create:(char)a1;
+ (void)downloadAsset:(id)a0 cloudPlaceholderKind:(short)a1 shouldPrioritize:(char)a2 shouldExtendTimer:(char)a3;
...
+ (char)hasPhoneInvitationForAlbum:(id)a0;
+ (char)hasReachedLimitOfOwnedStreamsInLibrary:(id)a0;
+ (char)hasReachedLimitOfSubscribedStreamsInLibrary:(id)a0;
...
+ (void)markAlbumGUIDAsViewed:(id)a0 clearUnseenAssetsCount:(char)a1;
...
+ (char)processExportedFileURL:(id)a0 assetUUID:(id)a1 customExportsInfo:(id)a2;
+ (void)pruneCloudSharingContentIfNecessaryInLibrary:(id)a0;
+ (unsigned long long)purgeSpace:(unsigned long long)a0;
+ (char)removeCloudSharingDirectories:(id *)a0;
...
+ (char)sharedStreamsExplictlyDisabledForPhotoLibraryURL:(id)a0;
+ (id)sharingDisplayNameIncludingEmail:(char)a0 allowsEmail:(char)a1;
+ (id)sharingFirstName;
...
+ (void)writeAlbumNameBreadCrumb:(id)a0 forAlbumGUID:(id)a1 isUpload:(char)a2;
