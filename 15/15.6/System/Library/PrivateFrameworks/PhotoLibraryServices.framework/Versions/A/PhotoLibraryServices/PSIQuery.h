@class NSDate, NSString, NSArray, NSSet, PSIParse, NSDictionary, NSMutableDictionary, NSMutableSet, NSIndexSet, PSITokenizer;
...
@property (readonly, nonatomic) char useWildcardText;
...
@property (nonatomic) char legacyDisableTopAssetsAndTopCollections;
...
@property (readonly, nonatomic) char implicitTokenizationIncludeBaseParseResults;
...
@property (readonly, nonatomic) char enableExactMatchPrioritization;
@property (nonatomic) char includeAllFruitfulMatchedGroups;
@property (nonatomic) char includeAllQueryMatchedGroups;
...
@property (readonly, copy, nonatomic) NSIndexSet *modelTypes;
@property (readonly, copy, nonatomic) NSString *queryAssetUUID;
...
+ (char)_shouldApplyFilterTokensForGroupArrays:(id)a0 searchTokens:(id)a1 filterGroupIds:(id)a2;
...
+ (char)enumerateDatedParsesWithParse:(id)a0 currentTokenIndex:(unsigned long long)a1 potentialComboAttributes:(id)a2 usingBlock:(id /* block */)a3;
+ (id)parsedDatesFromText:(id)a0;
+ (id)searchResultFromArrayOfGroupArrays:(id)a0 searchTokens:(id)a1 searchResultTypes:(unsigned long long)a2 suggestionType:(unsigned long long)a3 scopedSearchIdentifiers:(id)a4 normalizedSearchText:(id)a5 enableExactMatchPrioritization:(char)a6 includeAllQueryMatchedGroups:(char)a7 includeAllFruitfulMatchedGroups:(char)a8 suggestionLimit:(unsigned long long)a9 query:(id)a10 delegate:(id)a11;
+ (struct _NSRange { unsigned long long x0; unsigned long long x1; })searchTextExtendedRangeForParse:(id)a0;
+ (char)tokenIsEligibleForDateParsing:(id)a0;
...
- (id)processParse:(id)a0 disableWildcardMatchesForUserControlledCategories:(char)a1;
- (char)_enumerateImplicitlyTokenizedParsesWithBaseParse:(id)a0 usingBlock:(id /* block */)a1;
- (struct __CFSet { } *)_idsOfGroupsMatchingString:(id)a0 categories:(id)a1 textIsSearchable:(char)a2 isFilterTypeToken:(char)a3;
...
- (id)initWithEmbeddingModelTypes:(id)a0 assetUUID:(id)a1 delegate:(id)a2;
- (id)initWithQueryText:(id)a0 suggestionType:(unsigned long long)a1 suggestionLimit:(unsigned long long)a2 resultTypes:(unsigned long long)a3 delegate:(id)a4 libraryScope:(unsigned long long)a5;
- (id)initWithQueryTokens:(id)a0 searchText:(id)a1 useWildcardText:(char)a2 delegate:(id)a3 libraryScope:(unsigned long long)a4;
- (id)initWithQueryTokensV2:(id)a0 searchText:(id)a1 scopedSearchIdentifiers:(id)a2 searchResultTypes:(unsigned long long)a3 searchSuggestionType:(unsigned long long)a4 useWildcardText:(char)a5 enableExactMatchPrioritization:(char)a6 delegate:(id)a7 libraryScope:(unsigned long long)a8 suggestionLimit:(unsigned long long)a9;
- (void)processDates;
- (void)processImplicitTokenization;
- (void)processWordEmbeddings;
- (char)recursiveAddToGroupResults:(id)a0 aggregate:(id)a1 atIndex:(unsigned long long)a2 outOf:(unsigned long long)a3 inGroupArrays:(id)a4 dateFilter:(id)a5 datedTokens:(id)a6 disableWildcardMatchesForUserControlledCategories:(char)a7;
- (void)runFetchEmbeddingQueryWitBatchHandler:(id /* block */)a0;
- (id)runLookupIdentifierQuery;
- (void)runWithResultsHandler:(id /* block */)a0 disableWildcardMatchesForUserControlledCategories:(char)a1;
