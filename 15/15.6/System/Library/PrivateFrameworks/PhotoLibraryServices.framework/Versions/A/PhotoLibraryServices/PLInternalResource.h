...
+ (char)batchResetFileIDInManagedObjectContext:(id)a0 error:(id *)a1;
...
+ (char)deleteObsoleteResourcesInManagedObjectContext:(id)a0 error:(id *)a1;
...
+ (char)isCPLJPEGThumbnailResource:(id)a0;
...
+ (id)prefetchResourcePredicateForCPLResourceType:(unsigned long long)a0 isAssetResource:(char)a1 maxRetry:(unsigned long long)a2 excludeDynamicResources:(char)a3 additionalResourcePredicates:(id)a4;
+ (id)prunePredicateForCPLResourceTypes:(id)a0;
+ (id)purgeablePushedPredicateForCPLResourceTypes:(id)a0 urgency:(long long)a1;
+ (void)resetCloudResourcesStateForCloudInManagedObjectContext:(id)a0 hardReset:(char)a1;
...
+ (char)supportsTrashedStateForResourceIdentity:(id)a0;
...
- (char)repairResourceValidationErrors:(id)a0 managedObjectContext:(id)a1;
- (id)scopedIdentifier;
...
- (char)copyPurgeabilityFromFileURL:(id)a0;
- (id)cplFileURL;
- (id)cplResourceForFileURL:(id)a0 createDirectoryIfNeeded:(char)a1;
...
- (id)cplResourceIncludeFile:(char)a0 createDirectoryIfNeeded:(char)a1;
- (void)deleteResource;
- (void)ensureInitialValuesForSyndication;
- (id)expungeableResourceStateRepresentation;
- (char)hasInvalidFileID;
...
- (char)isCPLOriginalResourceAssetHasAdjustments:(char)a0;
- (char)isDefaultOrientation;
- (char)isEquivalentToFingerprint:(id)a0 andStableHash:(id)a1 fingerprintContext:(id)a2;
...
- (void)setSyndicationLocalAvailabilityWithAvailable:(char)a0;
- (void)setSyndicationLocalAvailabilityWithAvailable:(char)a0 additionalFlags:(unsigned short)a1;
