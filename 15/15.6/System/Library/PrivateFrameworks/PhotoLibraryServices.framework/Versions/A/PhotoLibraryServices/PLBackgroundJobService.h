...
@interface PLBackgroundJobService : NSObject <PFCameraViewfinderSessionWatcherDelegate, PFStateCaptureProvider, PLBackgroundJobLibraryCoordinatorDelegate> {
    ...
    char _needsActivityUnregistrationOnFirstLaunch;
...
+ (char)_verifyActivityHandlerInvokedWithCorrectIdentifier:(id)a0 forCriteria:(id)a1;
+ (void)_removeAllBundlesFromUserDefaultsWithoutLocking;
+ (char)_stateIsReadyForRegistration:(unsigned long long)a0;
+ (char)_stateIsReadyToRun:(unsigned long long)a0;
+ (char)verifyStateTransitionFromState:(unsigned long long)a0 toState:(unsigned long long)a1;
...
- (char)_activityIsRegisteredForCriteria:(id)a0;
...
- (char)_signalNeededOnBundle:(id)a0;
- (void)_simulateXPCShouldDefer:(char)a0;
...
- (void)_unregisterActivityIfNeededShouldConsiderDeferring:(char)a0;
- (void)_unregisterActivityIfNeededShouldConsiderDeferring_enqueue:(char)a0;
- (void)_updateCameraForegroundState:(char)a0;
