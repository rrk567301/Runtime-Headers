...
@interface PLCameraCaptureTaskConstraintCoordinator : NSObject <PFCameraViewfinderSessionWatcherDelegate> {
    ...
    char _delayedDeactivationInProgress;
    PLDelayedActionTimer *_failsafeTimer;
    char _failsafeTimerEnabled;
...
+ (char)_shouldUseFailsafeTimerForRole:(long long)a0;
+ (char)_shouldWatchCameraViewfinderForRole:(long long)a0;
...
- (void)_invalidateTransactionAndStopConstrainingTasksWithDelayedDeactivation:(char)a0;
