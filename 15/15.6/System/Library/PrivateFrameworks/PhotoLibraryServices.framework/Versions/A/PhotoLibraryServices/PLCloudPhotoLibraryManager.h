...
@interface PLCloudPhotoLibraryManager : NSObject <PLCloudPersistentHistoryChangeTrackerDelegate, PLBackgroundJobWorkerCoordinatorDelegate, PLCloudPersistentHistoryMigratorContext, CPLRecordComputeStateDelegate, CPLResourceProgressDelegate, CPLLibraryManagerDelegate, PLCloudUserSessionHandling> {
    ...
    char _unsafe_readyForAnalysis;
    char _opportunisticTasksWasDisabled;
    ...
    char _hasCompletedInitialDownload;
    unsigned long long _handlePrefetchModeChangeGeneration;
    char _hasReportedLibrarySizeToDAS;
}
...
+ (char)needResetSyncErrorType:(id)a0;
...
- (char)isConnectedToCloudChangeTracker;
- (void)libraryManagerHasStatusChanges:(id)a0;
- (id)_assetResourceForAsset:(id)a0 resourceType:(unsigned long long)a1 masterResourceOnly:(char)a2 isPhoto:(char *)a3;
...
- (id)recoveryManager;
- (void)_appWithBundleIdentifier:(id)a0 changedStateToForeground:(char)a1;
...
- (char)_canExternallyTransitionToNewLibraryStateIgnoringPause:(char)a0;
- (void)_checkAndMarkPurgeableResources:(id)a0 checkIfSafe:(char)a1 checkServerIfNecessary:(char)a2 urgency:(long long)a3 completionHandler:(id /* block */)a4;
...
- (char)_connectToCloudChangeTrackerWithContext:(id)a0;
...
- (void)_disableiCPLWillBecomeNonSyncablePhotoLibrary:(char)a0 resetMode:(long long)a1;
- (void)_doPause;
...
- (void)_handleFinalizeSessionError:(id)a0 commitError:(id)a1 uploadBatchContainer:(id)a2 needResetSync:(char)a3 forTransaction:(id)a4 inLibrary:(id)a5;
...
- (char)_isPausedForDownloadRequestHighPriority:(char)a0;
- (char)_libraryModeSupportsEnableiCPL;
- (void)_linkFileFrom:(id)a0 to:(id)a1;
- (void)_markPurgeableNextBatchOfResources:(id)a0 urgency:(long long)a1 checkIfSafe:(char)a2 inLibrary:(id)a3 completionHandler:(id /* block */)a4;
...
- (void)_notifyCPLStatus:(id)a0 didChange:(char)a1 andCPLConfiguration:(id)a2;
...
- (char)_shouldResetICloudBackupExclusionPaths;
...
- (void)_updateWithCPLResource:(id)a0 isHighPriority:(char)a1 inLibrary:(id)a2 completionHandler:(id /* block */)a3;
...
- (void)downloadAsset:(id)a0 resourceType:(unsigned long long)a1 masterResourceOnly:(char)a2 highPriority:(char)a3 clientBundleID:(id)a4 proposedTaskIdentifier:(id)a5 taskDidBeginHandler:(id /* block */)a6 progressBlock:(id /* block */)a7 completionHandler:(id /* block */)a8;
- (void)downloadResource:(id)a0 highPriority:(char)a1 clientBundleID:(id)a2 proposedTaskIdentifier:(id)a3 taskDidBeginHandler:(id /* block */)a4 progressBlock:(id /* block */)a5 completionHandler:(id /* block */)a6;
...
- (void)downloadResourceInMemoryForAsset:(id)a0 resourceType:(unsigned long long)a1 masterResourceOnly:(char)a2 clientBundleID:(id)a3 proposedTaskIdentifier:(id)a4 taskDidBeginHandler:(id /* block */)a5 completionHandler:(id /* block */)a6;
...
- (void)fetchComputeStatesForRecordsWithScopedIdentifiers:(id)a0 onDemand:(char)a1 completionHandler:(id /* block */)a2;
...
- (void)getCPLStateForDebug:(char)a0 completionHandler:(id /* block */)a1;
...
- (char)isResourceTransferTaskAliveWithTaskWithIdentifier:(id)a0;
...
- (void)markPurgeableResourcesMatchingPredicate:(id)a0 urgency:(long long)a1 checkIfSafe:(char)a2 inLibrary:(id)a3 completionHandler:(id /* block */)a4;
- (void)markPurgeableResourcesMatchingPredicate:(id)a0 urgency:(long long)a1 inLibrary:(id)a2 completionHandler:(id /* block */)a3;
- (char)overrideSystemBudgetsForSyncSession:(char)a0 pauseReason:(id)a1 forSystemBudgets:(unsigned long long)a2;
...
- (void)sendBudgetOverrideAnalytics:(char)a0 pauseReason:(id)a1;
- (void)setCPLConfiguration:(id)a0;
- (void)setCPLLibrary:(id)a0;
- (void)setCPLStatus:(id)a0;
- (void)setPause:(char)a0 reason:(short)a1;
