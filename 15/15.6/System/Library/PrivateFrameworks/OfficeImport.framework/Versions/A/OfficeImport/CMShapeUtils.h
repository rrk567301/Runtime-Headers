...
+ (float)intersectionAngleNextToAngle:(float)a0 isAfter:(char)a1 nodeSize:(struct CGSize { double x0; double x1; })a2;
+ (char)isShapeALine:(id)a0;
+ (double)mapAdjustCoord:(struct OADAdjustCoord { char x0; int x1; })a0 geometry:(id)a1;
+ (int)mapFormulaKeywordValue:(int)a0 geometry:(id)a1;
+ (double)mapFormulaOfType:(int)a0 argValues:(double *)a1 isEscher:(char)a2;
+ (double)milliMinutesToRad:(int)a0;
+ (double)nativeAngleToRad:(int)a0 isEscher:(char)a1;
+ (float)normalizedAngle:(float)a0;
+ (int)radToFixedPointDeg:(double)a0;
+ (int)radToMilliMinutes:(double)a0;
+ (int)radToNativeAngle:(double)a0 isEscher:(char)a1;
