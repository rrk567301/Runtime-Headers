@class NSData, NSString, NSArray, NSURL, DMCEnrollmentDirtyState, NSMutableDictionary, NSDictionary, MDMESSODetails, DMCObliterationShelter, NSNumber, DMCServiceDiscoveryHelper;
@protocol DMCEnrollmentFlowDelegate, DMCEnrollmentFlowMCBridge, DMCEnrollmentFlowRMBridge, DMCEnrollmentFlowPresenter;

@interface DMCEnrollmentFlowController : DMCEnrollmentFlowControllerBase

@property (retain, nonatomic) DMCEnrollmentDirtyState *dirtyState;
@property (retain, nonatomic) id<DMCEnrollmentFlowPresenter> presenter;
@property (retain, nonatomic) id<DMCEnrollmentFlowMCBridge> managedConfigurationHelper;
@property (retain, nonatomic) id<DMCEnrollmentFlowRMBridge> rmStoreHelper;
@property (copy, nonatomic) id /* block */ enrollmentCompletionHandler;
@property (nonatomic) unsigned long long enrollmentType;
@property (nonatomic) char restartIfFail;
@property (nonatomic) char cleanupIfFail;
@property (nonatomic) char isRenewalFlow;
@property (nonatomic) char isPostRestoration;
@property (nonatomic) char isPostBuddy;
@property (retain, nonatomic) DMCServiceDiscoveryHelper *serviceDiscoveryHelper;
@property (nonatomic) char hasExecutedUIStep;
@property (nonatomic) char noCompletionScene;
@property (copy, nonatomic) NSArray *anchorCertificateRefs;
@property (nonatomic) unsigned long long enrollmentMethod;
@property (retain, nonatomic) NSString *devicePasscode;
@property (retain, nonatomic) NSData *devicePasscodeData;
@property (nonatomic) unsigned long long devicePasscodeDataType;
@property (retain, nonatomic) NSString *username;
@property (retain, nonatomic) NSString *managedAppleID;
@property (retain, nonatomic) NSArray *servers;
@property (nonatomic) unsigned long long serverInformationSource;
@property (retain, nonatomic) NSURL *serviceURL;
@property (retain, nonatomic) NSDictionary *authParams;
@property (retain, nonatomic) NSString *enrollmentURL;
@property (retain, nonatomic) NSDictionary *authTokens;
@property (retain, nonatomic) NSString *personaID;
@property (retain, nonatomic) NSMutableDictionary *authenticationResults;
@property (retain, nonatomic) NSString *altDSID;
@property (retain, nonatomic) NSString *rmAccountIdentifier;
@property (retain, nonatomic) NSData *profileData;
@property (retain, nonatomic) NSString *profileIdentifier;
@property (nonatomic) char isProfileLocked;
@property (retain, nonatomic) NSString *organizationName;
@property (retain, nonatomic) NSString *friendlyName;
@property (retain, nonatomic) NSArray *serverCapabilities;
@property (retain, nonatomic) MDMESSODetails *essoDetails;
@property (retain, nonatomic) NSString *appBundleID;
@property (retain, nonatomic) NSNumber *essoAppITunesStoreID;
@property (retain, nonatomic) NSString *ESSOConfigurationProfileIdentifier;
@property (retain, nonatomic) NSArray *extensionIdentifiersFromConfig;
@property (retain, nonatomic) NSString *wifiProfileIdentifier;
@property (retain, nonatomic) NSDictionary *softwareUpdateInfo;
@property (retain, nonatomic) NSDictionary *cloudConfig;
@property (retain, nonatomic) NSString *password;
@property (retain, nonatomic) DMCObliterationShelter *obliterationShelter;
@property (nonatomic) char isReturnToService;
@property (weak) id<DMCEnrollmentFlowDelegate> delegate;

+ (id)enrollmentFlowController;
+ (id)enrollmentFlowControllerWithPresenter:(id)a0 managedConfigurationHelper:(id)a1;
+ (id)_createEnrollmentNotAllowedDuringSDPError;
+ (id)_createEnterpriseApplicationExistsErrorWithAppName:(id)a0;
+ (id)_createEnterpriseApplicationMissingEntitlementsError;
+ (id)_createGeneralError;
+ (id)_createGeneralSignInError;
+ (id)_createInvalidCloudConfigError;
+ (id)_createInvalidErSSODeclarationsError;
+ (id)_createMissingAppleAccountForUserEnrollmentError;
+ (id)_createMissingEnterpriseApplicationBundleIDError;
+ (id)_createMissingEnterpriseApplicationError;
+ (id)_createMissingRMAccountError;
+ (id)_createUnsupportedFeatureError;
+ (id)chooseServerFromUAORGOServers:(id)a0 error:(id *)a1;
+ (id)enrollmentFlowControllerWithPresenter:(id)a0 managedConfigurationHelper:(id)a1 rmStoreHelper:(id)a2;

- (void)dealloc;
- (id)init;
- (void).cxx_destruct;
- (void)continueMacBuddyORGOEnrollmentFlowWithAuthenticationResults:(id)a0 serviceURL:(id)a1 bearerToken:(id)a2 profileData:(id)a3 completionHandler:(id /* block */)a4;
- (void)reauthBYODEnrollmentFlowRestartIfFail:(char)a0 rmAccountIdentifier:(id)a1 completionHandler:(id /* block */)a2;
- (void)renewORGOEnrollmentWithServiceURL:(id)a0 anchorCertificates:(id)a1 restartIfFail:(char)a2 cleanupIfFail:(char)a3 completionHandler:(id /* block */)a4;
- (void)startBYODEnrollmentFlowRestartIfFail:(char)a0 completionHandler:(id /* block */)a1;
- (void)startMacBuddyORGOEnrollmentFlowWithServiceURL:(id)a0 anchorCertificates:(id)a1 restartIfFail:(char)a2 completionHandler:(id /* block */)a3;
- (void)startPostBuddyORGOEnrollmentFlowWithServiceURL:(id)a0 anchorCertificates:(id)a1 restartIfFail:(char)a2 completionHandler:(id /* block */)a3;
- (void)startUAORGOEnrollmentFlowWithServiceURL:(id)a0 anchorCertificates:(id)a1 restartIfFail:(char)a2 completionHandler:(id /* block */)a3;
- (void)_askForMDMUsernameAndCredential;
- (id)_translatedErrorStringForError:(id)a0;
- (id)_ADxE_thirdParty_firstPartSteps;
- (id)_findServerWithVersion:(id)a0 servers:(id)a1;
- (id)_ADE_RTS_commonSteps;
- (id)_ADE_commonSteps;
- (id)_ADE_deviceEnrollment_legacy_additionalNativeAuth_steps;
- (id)_ADE_deviceEnrollment_legacy_steps;
- (id)_ADE_deviceEnrollment_softwareUpdate_steps;
- (id)_ADE_deviceEnrollment_webURL_steps;
- (id)_ADE_postEnrollmentSteps;
- (id)_ADxE_ABE_ESSO_firstPartSteps;
- (id)_ADxE_ABE_ESSO_secondPartSteps_default;
- (id)_ADxE_ABE_ESSO_secondPartSteps_orgToken;
- (id)_ADxE_ABE_firstPartSteps;
- (id)_ADxE_ABE_secondPartSteps_default;
- (id)_ADxE_ABE_secondPartSteps_orgToken;
- (id)_ADxE_ESSO_firstPart_commonSteps;
- (id)_ADxE_ESSO_installAppSteps;
- (id)_ADxE_ESSO_postEnrollmentSteps;
- (id)_ADxE_ESSO_secondPart_commonSteps_default;
- (id)_ADxE_ESSO_secondPart_commonSteps_orgToken;
- (id)_ADxE_commonSteps;
- (id)_ADxE_consentAndCreatePersonaSteps;
- (id)_ADxE_postEnrollmentSteps;
- (id)_ADxE_reauthSteps;
- (id)_ADxE_secondPart_commonSteps_default;
- (id)_ADxE_secondPart_commonSteps_orgToken;
- (id)_ADxE_thirdParty_ESSO_firstPartSteps;
- (id)_ADxE_thirdParty_ESSO_secondPartSteps_default;
- (id)_ADxE_thirdParty_ESSO_secondPartSteps_orgToken;
- (id)_ADxE_thirdParty_fetchEnrollmentProfileSteps;
- (id)_ADxE_thirdParty_secondPartSteps_default;
- (id)_ADxE_thirdParty_secondPartSteps_orgToken;
- (id)_MAID_fetchEnrollmentProfileSteps;
- (id)_MDM_SharediPad_commonSteps;
- (id)_ORGO_MACBuddy_commonSteps;
- (id)_ORGO_MACBuddy_firstPartSteps;
- (id)_ORGO_MACBuddy_secondPartSteps;
- (id)_ORGO_MAIDEnrollmentSteps;
- (id)_ORGO_MAIDEnrollmentSteps_orgToken;
- (id)_ORGO_commonSteps;
- (id)_ORGO_deviceEnrollmentSteps;
- (id)_PDUE_UserEnrollment_commonSteps;
- (id)_Watch_deviceEnrollmentSteps;
- (void)_addNotification;
- (void)_analyzeCloudConfig:(id)a0 enrollmentType:(unsigned long long)a1 isReturnToService:(char)a2 obliterationShelter:(id)a3;
- (void)_analyzeESSODetails:(id)a0;
- (id)_appNameWithBundleID:(id)a0;
- (char)_appWithBundleIDExists:(id)a0;
- (void)_askForPasscodeIfNeeded;
- (void)_askForRestoreChoiceWithMAID:(id)a0 personaID:(id)a1;
- (void)_askForUserConsentWithProfileData:(id)a0 managedAppleID:(id)a1 cloudConfig:(id)a2 isReturnToService:(char)a3 enrollmentType:(unsigned long long)a4;
- (void)_askForUsernameWithDefaultUsername:(id)a0;
- (void)_authenticateMAID:(id)a0 authenticationResults:(id)a1 enrollmentType:(unsigned long long)a2 personaID:(id)a3 ephemeral:(char)a4 silent:(char)a5 isRenewalFlow:(char)a6;
- (void)_awaitESSOConfigActivationWithExtensionIdentifiers:(id)a0;
- (void)_blockAppInstalls;
- (void)_blockAppInstallsIfNecessary;
- (id)_bundleIDsFromAppIDs:(id)a0;
- (char)_canSkipMDMEnrollmentWithEnrollmentType:(unsigned long long)a0;
- (void)_chooseEnrollmentTypeWithAvailableServers:(id)a0 serverInformationSource:(unsigned long long)a1;
- (void)_cleanupDirtyState:(id)a0;
- (id)_commonStepsForEnrollmentType:(unsigned long long)a0 isReturnToService:(char)a1;
- (id)_convertErrorToHumanReadableError:(id)a0;
- (void)_correlateMAIDWithAltDSID:(id)a0 withRMAccount:(id)a1 isProfileLocked:(char)a2 organizationName:(id)a3 friendlyName:(id)a4 personaID:(id)a5;
- (void)_createEnterprisePersonaWithDevicePasscode:(id)a0 devicePasscodeData:(id)a1 devicePasscodeDataType:(unsigned long long)a2;
- (void)_createRMAccountIfNeededWithSignInUserIdentifier:(id)a0 assignedMAID:(id)a1 existingRMAccountID:(id)a2 enrollmentType:(unsigned long long)a3 enrollmentMethod:(unsigned long long)a4 isRenewalFlow:(char)a5;
- (id)_createRMAccountSteps;
- (void)_detectAuthTypeWithServiceURL:(id)a0 enrollmentType:(unsigned long long)a1 isPostBuddy:(char)a2 machineInfo:(id)a3 anchorCertificateRefs:(id)a4;
- (void)_displayManagementDetailsOverviewWithProfileData:(id)a0 managedAppleID:(id)a1;
- (id)_enrollmentSteps;
- (void)_enrollmentTypeAuthorizedBySDP:(unsigned long long)a0 completion:(id /* block */)a1;
- (void)_ensureDeviceActivation;
- (void)_ensureWiFiConnectionWithWiFiProfile:(id)a0;
- (id)_ephemeralAuthSteps;
- (id)_errorTranslation;
- (void)_exchangeMAIDForBearerTokenWithRMAccountIdentifier:(id)a0 authParams:(id)a1 anchorCertificateRefs:(id)a2;
- (void)_extensionIDsFromDeclarationProfiles;
- (char)_extractAndVerifyPropertiesFromProfileData:(id)a0 enrollmentMethod:(unsigned long long)a1 isESSO:(char)a2 essoAppITunesStoreID:(id)a3 error:(id *)a4;
- (id)_fetchAndAnalyzeCloudConfigSteps;
- (void)_fetchAppAttributesWithITunesItemID:(id)a0 completionHandler:(id /* block */)a1;
- (void)_fetchBundleIDsWithiTunesItemID:(id)a0 debuggingAppIDs:(id)a1 completionHandler:(id /* block */)a2;
- (void)_fetchCloudConfigWithEnrollmentType:(unsigned long long)a0 isReturnToService:(char)a1;
- (void)_fetchEnrollmentProfileFromServiceURL:(id)a0 authTokens:(id)a1 machineInfo:(id)a2 anchorCertificateRefs:(id)a3 enrollmentType:(unsigned long long)a4 enrollmentMethod:(unsigned long long)a5 isESSO:(char)a6 essoAppITunesStoreID:(id)a7 isReturnToService:(char)a8;
- (void)_fetchEnrollmentProfileFromServiceURL:(id)a0 username:(id)a1 password:(id)a2 machineInfo:(id)a3 anchorCertificateRefs:(id)a4;
- (void)_fetchEnrollmentProfileFromWebURL:(id)a0 machineInfo:(id)a1 anchorCertificateRefs:(id)a2;
- (void)_fetchServerInformationWithUsername:(id)a0;
- (void)_flowTerminatedWithError:(id)a0 canceled:(char)a1;
- (id)_guardAgainstConflictingAccountsWithUsername:(id)a0 altDSID:(id)a1;
- (char)_handleNeedCredentialsError:(id)a0;
- (char)_handleSoftwareUpdateRequiredError:(id)a0;
- (void)_initiateDEPPushTokenSync;
- (void)_installESSOConfigurationProfile:(id)a0 devicePasscode:(id)a1 personaID:(id)a2;
- (void)_installESSOConfigurationWithProfileData:(id)a0 declarations:(id)a1 devicePasscode:(id)a2 personaID:(id)a3;
- (void)_installESSODeclarations:(id)a0 devicePasscode:(id)a1 chosenBundleID:(id)a2 personaID:(id)a3 enrollmentType:(unsigned long long)a4;
- (void)_installEnrollmentProfile:(id)a0 devicePasscode:(id)a1 personaID:(id)a2 rmAccountIdentifier:(id)a3 isESSO:(char)a4 essoAppITunesStoreID:(id)a5 essoConfigurationProfile:(id)a6 wifiProfileIdentifier:(id)a7 enrollmentType:(unsigned long long)a8 isReturnToService:(char)a9;
- (void)_installEnterpriseApplication:(id)a0 debuggingAppIDs:(id)a1 personaID:(id)a2;
- (char)_isBYODEnrollment:(unsigned long long)a0;
- (char)_isORGOEnrollment:(unsigned long long)a0;
- (void)_linkESSOStore:(id)a0 rmAccountIdentifier:(id)a1 profileIdentifier:(id)a2;
- (void)_manageApplicationWithAppBundleID:(id)a0 essoDetails:(id)a1 organization:(id)a2 personaID:(id)a3;
- (id)_nameForStep:(unsigned long long)a0;
- (id)_permanentAuthSteps;
- (id)_postEnrollmentSteps;
- (void)_preflightEnrollmentWithEnrollmentType:(unsigned long long)a0 isRenewalFlow:(char)a1 isPostRestoration:(char)a2 isPostBuddy:(char)a3;
- (void)_processAuthServicesWithAuthParams:(id)a0 enrollmentMethod:(unsigned long long)a1 username:(id)a2;
- (void)_promoteiCloudWithAltDSID:(id)a0;
- (void)_promptForSoftwareUpdateWithSoftwareUpdateInfo:(id)a0;
- (id)_readDirtyStateFromDisk;
- (void)_reauthenticationCompleteWithRMIdentifier:(id)a0;
- (void)_receivedApplicationWillTerminateNotification:(id)a0;
- (void)_requestiCloudSignInConfirmation;
- (void)_resetToInitialSteps;
- (void)_restoreLanguage:(id)a0 locale:(id)a1;
- (void)_restoreSetAsideProfiles;
- (char)_shouldDoReturnToService;
- (id)_signInMAIDSteps;
- (void)_signInMAIDWithAuthenticationResult:(id)a0 enrollmentType:(unsigned long long)a1 personaID:(id)a2 isRenewalFlow:(char)a3;
- (id)_silentAuthSteps;
- (void)_startEnrollmentFlowWithType:(unsigned long long)a0 anchorCertificates:(id)a1 restartIfFail:(char)a2 completionHandler:(id /* block */)a3;
- (id)_stepsWithUI;
- (void)_storeCloudConfig:(id)a0;
- (void)_trackDirtyAppBundleIDs:(id)a0;
- (id)_trustedErrors;
- (void)_unblockAppInstallsIfNecessaryWithCaller:(id)a0;
- (void)_unblockAppInstallsWithCaller:(id)a0;
- (void)_updateAccountsWithRMIdentifier:(id)a0 managedAppleID:(id)a1 profileIdentifier:(id)a2 organizationName:(id)a3 enrollmentURL:(id)a4 personaID:(id)a5;
- (void)_updateCloudConfigWithRMAccountIdentifier:(id)a0;
- (void)_updateCredentialForAccount:(id)a0 authTokens:(id)a1;
- (void)_updateRMAccountWithIdentifier:(id)a0 authTokens:(id)a1 personaID:(id)a2;
- (void)_updateRMAccountWithIdentifier:(id)a0 authTokens:(id)a1 profileIdentifier:(id)a2 organizationName:(id)a3 personaID:(id)a4 enrollmentMethod:(unsigned long long)a5;
- (void)_updateRMAccountWithIdentifier:(id)a0 authenticationResults:(id)a1 enrollmentMethod:(unsigned long long)a2;
- (char)_validEnrollmentMode:(id)a0 enrollmentMethod:(unsigned long long)a1;
- (char)_validateBundleID:(id)a0 personaID:(id)a1 requiredEntitlements:(id)a2 error:(id *)a3;
- (char)_verifyAccountsInformationWithAltDSID:(id)a0 personaID:(id)a1;
- (void)_waitForDeviceConfiguration;
- (void)_waitForESSODeclarations;
- (void)_workerQueue_cleanupCachedValues;
- (void)_workerQueue_flowCompleted;
- (void)_workerQueue_performFlowStep:(unsigned long long)a0;
- (void)_workerQueue_stopMonitorAndEmptyDiskValues;
- (void)_writeDirtyStateToDisk:(id)a0;
- (void)cleanUpDirtyState;
- (void)continueMacBuddyORGOEnrollmentFlowWithAuthentiactionResults:(id)a0 serviceURL:(id)a1 bearerToken:(id)a2 profileData:(id)a3 completionHandler:(id /* block */)a4;
- (id)initWithPresenter:(id)a0 managedConfigurationHelper:(id)a1;
- (id)initWithPresenter:(id)a0 managedConfigurationHelper:(id)a1 rmStoreHelper:(id)a2;
- (void)restartORGOEnrollmentWithServiceURL:(id)a0 anchorCertificates:(id)a1 isPostRestoration:(char)a2 restartIfFail:(char)a3 cleanupIfFail:(char)a4 completionHandler:(id /* block */)a5;
- (void)startInBuddyEnrollmentFlowRestartIfFail:(char)a0 completionHandler:(id /* block */)a1;
- (void)startORGOEnrollmentFlowWithServiceURL:(id)a0 anchorCertificates:(id)a1 restartIfFail:(char)a2 completionHandler:(id /* block */)a3;
- (void)startWatchEnrollmentFlowWithServiceURL:(id)a0 anchorCertificates:(id)a1 restartIfFail:(char)a2 completionHandler:(id /* block */)a3;
- (void)terminateEnrollmentFlow;
- (void)updateMDMUserEnrollmentWithManagedAppleID:(id)a0 profileIdentifier:(id)a1 organizationName:(id)a2 personaID:(id)a3 completionHandler:(id /* block */)a4;
- (void)updateSharediPadUserChannelWithManagedAppleID:(id)a0 profileIdentifier:(id)a1 organizationName:(id)a2 enrollmentURL:(id)a3 completionHandler:(id /* block */)a4;

@end
