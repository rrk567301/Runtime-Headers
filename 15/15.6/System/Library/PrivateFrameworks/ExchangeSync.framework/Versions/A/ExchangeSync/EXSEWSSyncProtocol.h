@class EWSAttachmentManager, NSString, EXSEWSProtocolSubscription, NSCondition, NSMutableSet, NSObject, EWSExchangeServiceBinding;
@protocol OS_os_log;

@interface EXSEWSSyncProtocol : EXSSyncProtocol <EXSNetworkControllerDelegate, EXSProtocolSubscriptionDelegate> {
    NSCondition *_networkAvailableCondition;
    NSMutableSet *_foldersToSyncAfterPush;
}

@property (retain, nonatomic) NSObject<OS_os_log> *log;
@property char networkAvailable;
@property (nonatomic) char activelyPolling;
@property (retain, nonatomic) EWSExchangeServiceBinding *bindingForData;
@property (retain, nonatomic) EWSExchangeServiceBinding *bindingForSync;
@property (retain, nonatomic) EXSEWSProtocolSubscription *ewsSubscription;
@property (retain, nonatomic) EWSAttachmentManager *attachmentManager;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

+ (long long)EXSResponseTypeFromEWSResponseType:(long long)a0;
+ (id)accountGenericLog;
+ (id)convertEWSAttendeesToEXSAttendees:(id)a0;
+ (id)convertEWSOccurrencesToEXSOccurrences:(id)a0;
+ (id)convertEWSReminderDataToEXSAlarm:(id)a0;
+ (id)createChangeItemForItemType:(id)a0 changeType:(long long)a1 changeSource:(id)a2 accountID:(long long)a3;
+ (id)createDeleteFolderFieldForField:(long long)a0;
+ (id)delegateFoldersToWatchIn:(id)a0 withDataManager:(id)a1;
+ (long long)deriveAttendeeResponseFrom:(long long)a0;
+ (long long)deriveEWSMailBoxTypeFrom:(long long)a0;
+ (id)deriveEXSAttachmentsFromEWSCalendarItem:(id)a0;
+ (id)deriveEXSEmailAddressFromEWSEmailAddressType:(id)a0;
+ (long long)deriveMailBoxTypeFrom:(long long)a0;
+ (id)deriveWellKnownEWSAttendeeResponseFrom:(long long)a0;
+ (id)fetchExceptionStartDates:(id)a0;
+ (long long)folderItemTypeFromFolder:(id)a0;
+ (long long)getChangeItemItemTypeFromFolder:(id)a0;
+ (id)itemPropertiesFromCalendarItemType:(id)a0;
+ (id)itemPropertiesFromNotesItemType:(id)a0;
+ (id)itemPropertiesFromTaskType:(id)a0;
+ (void)setRecurrenceProperties:(id)a0 forPattern:(id)a1 range:(id)a2;

- (void).cxx_destruct;
- (void)accountChanged;
- (void)shutdown;
- (void)_startPolling;
- (id)hostnames;
- (void)subscription:(id)a0 failedWithError:(id)a1;
- (void)startup;
- (void)downloadAttachmentWithRequestID:(id)a0 attachmentUUID:(id)a1 responseManager:(id)a2;
- (void)_foldersChanged;
- (void)didUnsubscribe:(id)a0;
- (void)refreshBindings;
- (char)sendGetGrantedDelegatesRequest:(id)a0;
- (id)_detachedItemIDsToLoadFromChangeItems:(id)a0 parentIdDictionary:(id)a1;
- (id)_encodableInfoFromDictionary:(id)a0;
- (char)_executeEWSOperation:(id)a0 withChangeItem:(id)a1 error:(id *)a2;
- (void)_fetchChangeKeyForItem:(id)a0;
- (id)_itemShapeAdditionalPropertiesForFolderType:(long long)a0;
- (long long)_itemShapeBodyTypeForFolderType:(long long)a0;
- (id)_newScheduleCriteria;
- (void)_postSyncStateChangedNotificationWithStartDate:(id)a0 endDate:(id)a1 error:(id)a2;
- (id)_processDetachedItemProperties:(id)a0 parentIdDictionary:(id)a1 previouslyUnknownAttachments:(id)a2;
- (id)_processEWSSyncFolderItemsResponseMessage:(id)a0 withItemProperties:(id)a1 forFolder:(id)a2 overrideChangeKey:(char)a3 attachmentsToAutodownload:(id)a4;
- (void)_stopPolling;
- (void)_syncItemsForFolder:(id)a0 initiatedBy:(id)a1 overrideChangeKey:(char)a2 dispatchGroup:(id)a3;
- (void)_syncItemsForFolder:(id)a0 initiatedBy:(id)a1 overrideChangeKey:(char)a2 dispatchGroup:(id)a3 completionHandler:(id /* block */)a4;
- (void)beginPushingChangesToServer;
- (char)canBatchChangeItemsForPush;
- (char)canStoreChangeTokens;
- (id)changeSourceID;
- (id)createChangeItemForDeleteFolder:(id)a0 itemType:(long long)a1;
- (id)createChangeItemForDeleteFolderType:(id)a0 itemType:(long long)a1;
- (id)createChangeItemForFolder:(id)a0 changeType:(long long)a1;
- (id)createChangeItemForFolderType:(id)a0 changeType:(long long)a1;
- (id)createChangeItemForItemType:(id)a0 changeType:(long long)a1 previouslyUnknownAttachments:(id)a2;
- (id)createChangeItemForModificationTimeUpdate:(id)a0 itemToUpdate:(id)a1;
- (void)didSubscribe:(id)a0;
- (void)endPushingChangesToServer;
- (char)extendedPropertyChangesExistInEWSFolder:(id)a0 folder:(id)a1;
- (id)findAttachmentsWithUploadSuppressedForEventWithInternalID:(id)a0;
- (void)fireDownloadCompleteForAttachmentWithRequestKey:(id)a0;
- (long long)folderItemTypeFromFolderClass:(id)a0;
- (long long)getDelegateFolderPermissionsForEmailSynchronously:(id)a0 error:(id *)a1;
- (void)hostnameIsNotReachable:(id)a0;
- (void)hostnameIsReachable:(id)a0;
- (id)initWithAccount:(id)a0 dataManager:(id)a1 dispatchWorkloop:(id)a2;
- (id)lastProcessedChangeToken;
- (id)loadFolderPropertiesForFolderIDs:(id)a0 error:(id *)a1;
- (id)loadItemPropertiesForItemIDs:(id)a0 withFolderType:(long long)a1 error:(id *)a2;
- (void)locateDeletedItemFolderIDAfterMigration;
- (void)locateImportantFolders;
- (void)logServerCallOfType:(id)a0 withPrivateIdentifier:(id)a1 withResponse:(id)a2;
- (void)logServerCallOfType:(id)a0 withPublicIdentifier:(id)a1 withPrivateIdentifier:(id)a2 withResponse:(id)a3;
- (void)logServerCallOfType:(id)a0 withPublicIdentifier:(id)a1 withResponse:(id)a2;
- (void)logServerCallOfType:(id)a0 withResponse:(id)a1;
- (void)officeOnlineSubscriptionReceivedHeartbeatPing;
- (void)performAutodiscoveryForAccount:(id)a0 accountStore:(id)a1;
- (void)performCredentialRenewalForAccount:(id)a0 accountStore:(id)a1;
- (void)prepareAttachmentsForDetachment:(id)a0;
- (char)pushChangeItems:(id)a0 withTrackingToken:(id)a1;
- (char)pushFolderCreated:(id)a0 trackingToken:(id)a1 error:(id *)a2;
- (char)pushFolderDeleted:(id)a0 trackingToken:(id)a1 error:(id *)a2;
- (char)pushFolderUpdated:(id)a0 trackingToken:(id)a1 error:(id *)a2;
- (char)pushItemCreated:(id)a0 trackingToken:(id)a1 error:(id *)a2;
- (char)pushItemDeleted:(id)a0 trackingToken:(id)a1 error:(id *)a2;
- (char)pushItemUpdated:(id)a0 trackingToken:(id)a1 error:(id *)a2;
- (char)reconcileAttachmentChangesWithServer:(id)a0 forChangeItem:(id)a1;
- (void)repushFolders:(id)a0 completionHandler:(id /* block */)a1;
- (void)resyncFolderHierarchyInitiatedBy:(id)a0 dispatchGroup:(id)a1;
- (void)resyncItemsForFolder:(id)a0 initiatedBy:(id)a1 dispatchGroup:(id)a2;
- (id)retrieveLatestPropertiesForFolder:(id)a0;
- (void)sendAvailabilityRequestWithComponents:(id)a0;
- (id)sendEWSRequest:(id)a0 error:(id *)a1;
- (char)sendGrantedDelegateRequest:(id)a0;
- (char)sendSearchDirectoryRequest:(id)a0;
- (void)startBindings;
- (void)stopBindings;
- (void)subscription:(id)a0 foldersChanged:(char)a1 folderEvents:(id)a2 itemEvents:(id)a3;
- (char)supportsSchema2010;
- (char)supportsSchema2010SP2;
- (char)supportsSchema2013SP1;
- (void)syncAllFolderItems;
- (void)syncAllFolderItemsForDataclasses:(long long)a0 withDispatchGroup:(id)a1;
- (void)syncAttachmentMetadataWithDB:(id)a0 forItem:(id)a1 previouslyUnknownAttachments:(id)a2;
- (void)syncCalendarFolderInitiatedBy:(id)a0 dispatchGroup:(id)a1;
- (void)syncFolderHierarchyForFolderIDs:(id)a0 initiatedBy:(id)a1 dispatchGroup:(id)a2;
- (void)syncFolderHierarchyInitiatedBy:(id)a0 dispatchGroup:(id)a1;
- (void)syncItemsForFolder:(id)a0 initiatedBy:(id)a1 dispatchGroup:(id)a2;
- (id)syncProtocolIdentifier;
- (void)systemWasWokenFromSleep;

@end
