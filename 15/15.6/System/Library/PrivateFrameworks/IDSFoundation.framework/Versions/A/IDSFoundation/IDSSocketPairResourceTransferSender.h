@class NSString, NSDictionary, NSData, NSDate;

@interface IDSSocketPairResourceTransferSender : IDSSocketPairMessage <IDSSocketPairMessageProtocol> {
    NSString *_resourcePath;
    NSDictionary *_metadata;
    NSDictionary *_resourceAttributes;
    int _fileDescriptor;
    char _expectsPeerResponse;
    char _wantsAppAck;
    char _compressPayload;
    char _compressed;
    char _didWakeHint;
    NSString *_peerResponseIdentifier;
    NSDate *_expiryDate;
}

@property (readonly, nonatomic) char sentFirstMessage;
@property (readonly, nonatomic) char isDone;
@property (nonatomic) unsigned int maxChunkSize;
@property (nonatomic) char resumeResourceTransfers;
@property (nonatomic) unsigned long long nextByte;
@property (readonly, nonatomic) unsigned long long totalBytes;
@property (nonatomic) unsigned int sequenceNumber;
@property (nonatomic) unsigned short streamID;
@property (readonly, nonatomic) NSString *messageUUID;
@property (readonly, nonatomic) NSData *data;
@property (readonly, nonatomic) char wantsAppAck;
@property (readonly, nonatomic) char expectsPeerResponse;
@property (readonly, nonatomic) char didWakeHint;
@property (readonly, nonatomic) NSString *peerResponseIdentifier;
@property (retain, nonatomic) NSDate *expiryDate;

- (void)dealloc;
- (id)description;
- (void).cxx_destruct;
- (void)reset;
- (unsigned char)command;
- (id)nextMessage;
- (id)nextMessage_old;
- (void)closeFileAndMarkDone;
- (id)initWithResourceAtPath:(id)a0 metadata:(id)a1 sequenceNumber:(unsigned int)a2 streamID:(unsigned short)a3 expectsPeerResponse:(char)a4 wantsAppAck:(char)a5 compressPayload:(char)a6 compressed:(char)a7 didWakeHint:(char)a8 peerResponseIdentifier:(id)a9 messageUUID:(id)a10 expiryDate:(id)a11;
- (id)readNextBytes;
- (char)readNextBytes:(id *)a0 byteOffset:(unsigned long long *)a1;

@end
