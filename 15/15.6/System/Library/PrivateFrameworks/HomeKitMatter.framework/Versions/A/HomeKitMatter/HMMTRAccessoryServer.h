@class NSHashTable, NSFileHandle, NSNumber, HMFTimer, HMMTRControllerWrapper, HMMTRAccessoryReachabilityManager, NSDictionary, NSMutableArray, NSString, HMMTRSyncClusterDoorLock, NSError, MTRDeviceController, HMMTRPairing, NSArray, NSMutableDictionary, HMMTRAccessoryPairingEndContext, HMMTRAccessoryServerBrowser, HMMTROTAApplyUpdateTimer, HMMTRHAPEnumerator, HMMTROTAAnnounceTimer, HMMTRFirmwareUpdateStatus, HMMTRDuration, HMMTRStorage, HMMTRMetricBundle, NSData, HMMTRFabric, HMFActivity, HMMTROTAApplyUpdateRequestTimer, HMMTRAttributeReportDistributor, NSUUID, MTRDevice, HMFFuture, HAPAccessory;
@protocol HMMTRAccessoryServerDiagnosticsEventDelegate, HMMTRAccessoryServerDelegate, HMMTRReportObserver;

@interface HMMTRAccessoryServer : HAPAccessoryServer <MTRDeviceControllerDelegate, HMFLogging, MTRDeviceDelegate, HMFTimerDelegate> {
    NSNumber *_matterDeviceTypeID;
}

@property (weak, nonatomic) HMMTRStorage *storage;
@property (retain, nonatomic) HMMTRAccessoryReachabilityManager *reachabilityManager;
@property (copy, nonatomic) id /* block */ stageDeviceCredentialHandler;
@property (copy, nonatomic) id /* block */ stageWiFiScanResultsHandler;
@property (copy, nonatomic) id /* block */ stageThreadScanResultsHandler;
@property (copy, nonatomic) id /* block */ stageProgressUpdateHandler;
@property (nonatomic) char stageNetworkScanRequested;
@property (copy, nonatomic) id /* block */ stageCompletion;
@property (retain, nonatomic) HMFTimer *pairingTimer;
@property (retain, nonatomic) HMFTimer *stateCaptureConfigChangeTimer;
@property (retain, nonatomic) HMFTimer *stateCaptureDeviceConnectedTimer;
@property (nonatomic) unsigned long long pairingProgress;
@property (nonatomic) char commissioningSessionEstablished;
@property (nonatomic) char commissioningCompleted;
@property unsigned int networkProvisioningFailures;
@property (retain, nonatomic) HMMTRPairing *currentPairingInfo;
@property (nonatomic) unsigned long long partsListStateCaptureHandle;
@property (retain, nonatomic) NSString *partsListStateCaptureInformation;
@property (nonatomic) unsigned long long deviceConnectedStateHandle;
@property (retain, nonatomic) NSString *deviceConnectedStateCaptureInformation;
@property (nonatomic) char pairingUsingMatterSupport;
@property (retain, nonatomic) NSData *originalPairingAttemptRootCert;
@property (retain, nonatomic) NSData *originalPairingAttemptOperationalCert;
@property (nonatomic) char shouldPairWithoutResident;
@property (nonatomic) char shouldReestablishSubscription;
@property (nonatomic) unsigned long long operationDisabledReason;
@property (retain, nonatomic) NSArray *lastPendingFabricLabel;
@property (retain, nonatomic) NSError *lastFabricLabelUpdateError;
@property (retain, nonatomic) NSData *defaultThreadOperationalDataset;
@property (nonatomic) char defaultThreadOperationalDatasetRetrieved;
@property (copy, nonatomic) id /* block */ defaultThreadOperationalDatasetReceiveBlock;
@property (retain, nonatomic) HMMTRMetricBundle *currentMetricBundle;
@property (retain, nonatomic) HMMTRDuration *maxMetricDuration;
@property (retain, nonatomic) NSMutableDictionary *pairingDurationDictionary;
@property (readonly, nonatomic) HMMTRHAPEnumerator *hapEnumerator;
@property (nonatomic) char servicesEnumerated;
@property (nonatomic) char serviceEnumerationInProgress;
@property (readonly) NSMutableArray *serviceEnumerationFromDeviceCompletionHandlers;
@property (nonatomic) char fullServiceEnumerationPendingCachePrimedEvent;
@property (retain) HMMTRSyncClusterDoorLock *doorLockCluster;
@property (retain) HMFFuture *doorLockClusterFuture;
@property (retain, nonatomic) NSNumber *softwareVersionNumber;
@property (retain, nonatomic) NSString *softwareVersionString;
@property (retain, nonatomic) NSMutableDictionary *clusterIDCharacteristicMap;
@property (retain, nonatomic) NSMutableDictionary *attributeTimers;
@property (nonatomic) long long linkLayerType;
@property (retain) NSUUID *initialMTRDeviceStateTimeoutId;
@property (nonatomic) char controllerRevokeHandlerRegistered;
@property (retain, nonatomic) NSNumber *commissioneeNetworkCommissioningClusterEndpoint;
@property (retain, nonatomic) NSArray *commissioneeNetworks;
@property (nonatomic) char commissioneeHasActiveNetwork;
@property (readonly, nonatomic) NSNumber *objectID;
@property (readonly, nonatomic) NSMutableDictionary *pendingMTRDeviceReadReadyHandlers;
@property (readonly, copy, nonatomic) NSHashTable *reportObservers;
@property (weak, nonatomic) HMMTRAccessoryServerBrowser *browser;
@property (retain, nonatomic) HAPAccessory *primaryAccessory;
@property (retain, nonatomic) NSNumber *nodeID;
@property (retain, nonatomic) NSNumber *fabricID;
@property (retain, nonatomic) NSUUID *commissioningID;
@property (retain, nonatomic) NSNumber *vendorID;
@property (retain, nonatomic) NSNumber *productID;
@property (retain, nonatomic) NSString *vendorNameFromDcl;
@property (retain, nonatomic) NSString *productNameFromDcl;
@property (retain, nonatomic) NSNumber *deviceTypeIDFromDcl;
@property (retain, nonatomic) NSNumber *discriminator;
@property (nonatomic) char discriminatorIsOriginatedFromShort;
@property (retain, nonatomic) NSNumber *setUpPINCode;
@property (retain, nonatomic) NSString *setupPayloadString;
@property (retain, nonatomic) NSString *onboardingSetupPayloadString;
@property (retain, nonatomic) NSData *topology;
@property (nonatomic) long long pairedState;
@property (retain) NSData *otaUpdateToken;
@property (retain) NSFileHandle *otaFileHandle;
@property (retain) NSNumber *otaFileOffset;
@property (retain, nonatomic) HMMTROTAApplyUpdateTimer *otaUpdateTimer;
@property (retain, nonatomic) HMMTROTAAnnounceTimer *otaAnnounceTimer;
@property (retain, nonatomic) HMMTROTAApplyUpdateRequestTimer *otaApplyUpdateRequestTimer;
@property (retain, nonatomic) HMMTRFirmwareUpdateStatus *matterFirmwareUpdateStatus;
@property (nonatomic) char storageUpdatePending;
@property (nonatomic) char locallyDiscovered;
@property (nonatomic) char discoveredOverBLE;
@property (nonatomic) char bleScanPending;
@property (nonatomic, getter=isCertified) char certified;
@property (nonatomic) char knownToSystemCommissioner;
@property (nonatomic) char hasPriorSuccessfulPairing;
@property (nonatomic) char serviceEnumerationFailed;
@property (weak, nonatomic) id<HMMTRAccessoryServerDiagnosticsEventDelegate> diagnosticsEventDelegate;
@property (readonly, nonatomic) char accessoryWithSameDiscriminatorDiscovered;
@property (nonatomic) char delayDiscovery;
@property (readonly, nonatomic) MTRDeviceController *deviceController;
@property (retain) MTRDevice *matterDevice;
@property (copy) id /* block */ chipReportHandler;
@property (readonly, nonatomic) NSError *removeReason;
@property (readonly, nonatomic) HMMTRAccessoryPairingEndContext *pairingEndContextWhenRemove;
@property (nonatomic) char removalInProgress;
@property (nonatomic) char blockInvalidation;
@property (nonatomic, getter=isWEDDevice) char wedDevice;
@property (retain, nonatomic) NSString *eMACAddress;
@property (nonatomic) char requiresThreadRouter;
@property (nonatomic) char deferStartThreadForPairing;
@property (readonly, nonatomic) NSMutableDictionary *characteristicProtocolMappingStates;
@property (retain, nonatomic) NSError *commissioningFailureOverridingError;
@property (retain, nonatomic) HMFTimer *residentReachabilityUpdateWaitTimer;
@property (nonatomic) char mtrDeviceStateReported;
@property (nonatomic) char mtrDeviceConnected;
@property (retain, nonatomic) NSNumber *bridgedAccessoryReachabilityReaderTimeoutNSecs;
@property (readonly, nonatomic) HMMTRAttributeReportDistributor *reportDistributor;
@property (copy) id /* block */ descriptorClusterManager;
@property (nonatomic) char operationDisabled;
@property (readonly, nonatomic) HAPAccessory *primaryAccessoryForServer;
@property (readonly, nonatomic) HMMTRPairing *currentPairing;
@property (weak, nonatomic) id<HMMTRAccessoryServerDelegate> chipDelegate;
@property (weak, nonatomic) id<HMMTRReportObserver> hmdHAPAccessoryDelegate;
@property (readonly) NSData *rootPublicKey;
@property (retain) NSUUID *fabricUUID;
@property (retain) HMMTRFabric *pairingTargetFabric;
@property (readonly, nonatomic) HMFActivity *activity;
@property (retain, nonatomic) NSNumber *supportedLinkLayerTypes;
@property (retain) HMMTRControllerWrapper *controllerWrapper;
@property (retain, nonatomic) NSDictionary *endpointToDeviceTypesMap;
@property (readonly) char hasPreferredLocalLink;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

+ (id)shortDescription;
+ (id)logCategory;
+ (char)certificateSubjectHasCATs:(id)a0;
+ (void)propagateCharactersticValuesToAccessory:(id)a0;

- (void)dealloc;
- (char)isEqual:(id)a0;
- (void).cxx_destruct;
- (void)disable;
- (void)disconnect;
- (id)deviceID;
- (id)_processEvent:(id)a0;
- (id)privateDescription;
- (char)isDisabled;
- (long long)linkType;
- (char)isPaired;
- (char)isStaged;
- (char)isPairingInProgress;
- (id)deviceController;
- (void)disconnectWithError:(id)a0;
- (void)dispatchAfter:(unsigned long long)a0 block:(id /* block */)a1;
- (id)logIdentifier;
- (id)attributeDescriptions;
- (void)timerDidFire:(id)a0;
- (long long)communicationProtocol;
- (id)matterDeviceTypeID;
- (void)updateAccessoryName:(id)a0;
- (void)_updateMetrics;
- (char)matchesSetupID:(id)a0;
- (char)_delegateRespondsToSelector:(SEL)a0;
- (void)_notifyDelegateOfPairingProgress:(long long)a0;
- (void)addPairing:(id)a0 completionQueue:(id)a1 completionHandler:(id /* block */)a2;
- (void)discoverAccessories;
- (char)doesPeriodicSessionChecks;
- (void)enableEvents:(char)a0 forCharacteristics:(id)a1 withCompletionHandler:(id /* block */)a2 queue:(id)a3;
- (void)handleUpdatesForCharacteristics:(id)a0 stateNumber:(id)a1;
- (void)identifyWithCompletion:(id /* block */)a0;
- (char)isKnownToSystemCommissioner;
- (void)listPairingsWithCompletionQueue:(id)a0 completionHandler:(id /* block */)a1;
- (char)matchesSetupID:(id)a0 serverIdentifier:(id)a1;
- (unsigned long long)numActiveSessionClients;
- (char)pingSupported;
- (id)reachabilityChangedReason;
- (void)readCharacteristicValues:(id)a0 timeout:(double)a1 completionQueue:(id)a2 completionHandler:(id /* block */)a3;
- (void)removePairing:(id)a0 completionQueue:(id)a1 completionHandler:(id /* block */)a2;
- (char)removePairingForCurrentControllerOnQueue:(id)a0 completion:(id /* block */)a1;
- (unsigned long long)sessionCheckInterval;
- (void)startPairingWithRequest:(id)a0;
- (char)stopPairingWithError:(id *)a0;
- (char)tryPairingPassword:(id)a0 onboardingSetupPayloadString:(id)a1 error:(id *)a2;
- (void)writeCharacteristicValues:(id)a0 timeout:(double)a1 completionQueue:(id)a2 completionHandler:(id /* block */)a3;
- (void)device:(id)a0 receivedAttributeReport:(id)a1;
- (id)collectDiagnosticsForAccessory:(id)a0;
- (void)startPairingMetricWithActivity:(id)a0;
- (id)accessoryProductID;
- (id)accessoryVendorID;
- (id)chipNodeId;
- (id)createDoorLockClusterObjectWithFlow:(id)a0;
- (id)currentWiFiNetworkInfo;
- (void)device:(id)a0 receivedEventReport:(id)a1;
- (void)device:(id)a0 stateChanged:(unsigned long long)a1;
- (void)deviceCachePrimed:(id)a0;
- (void)deviceConfigurationChanged:(id)a0;
- (void)dispatchBlock:(id /* block */)a0;
- (void)fetchColorControlClusterForHapAccessory:(id)a0 completionHandler:(id /* block */)a1;
- (void)fetchLastKnownPairingsWithCompletionHandler:(id /* block */)a0;
- (void)fetchPairingsWithCompletionHandler:(id /* block */)a0;
- (void)finishPairing;
- (id)getConnectNetworkIDForAccessory:(id)a0;
- (id)getOperationalNetworkAddressForAccessory:(id)a0;
- (id)getThreadNetworkLinkQualityForAccessory:(id)a0;
- (id)getWiFiNetworkLinkQualityForAccessory:(id)a0;
- (void)handleCHIPRemoteRequest:(id)a0 completion:(id /* block */)a1;
- (void)handleThreadNetworkStateChangedNotification:(id)a0;
- (char)isLocallyDiscoveredServer;
- (void)openPairingWindowForDuration:(double)a0 completionHandler:(id /* block */)a1;
- (void)openPairingWindowWithPINForDuration:(double)a0 completionHandler:(id /* block */)a1;
- (id)readPastDiagnosticEventsFromAccessory:(id)a0 fromEventNumber:(id)a1;
- (void)readSpecificationVersionWithCompletionHandler:(id /* block */)a0;
- (void)removeAllPairingsWithCompletionHandler:(id /* block */)a0;
- (void)removePairing:(id)a0 completionHandler:(id /* block */)a1;
- (void)resetThreadNetworkDiagnosticsCountForAccessory:(id)a0;
- (void)resetWiFiNetworkDiagnosticsCountForAccessory:(id)a0;
- (void)setCHIPReportHandler:(id /* block */)a0;
- (void)updateFabricLabel:(id)a0 completionHandler:(id /* block */)a1;
- (void)_deviceInternalStateChanged:(id)a0;
- (void)_handleEventReport:(id)a0;
- (void)controller:(id)a0 commissioningComplete:(id)a1 nodeID:(id)a2 metrics:(id)a3;
- (void)controller:(id)a0 commissioningSessionEstablishmentDone:(id)a1;
- (void)controller:(id)a0 statusUpdate:(long long)a1;
- (void)setupReporting;
- (id)_categoryFromTopology:(id)a0 endpoint:(id)a1;
- (void)_deregisterStateCaptureHandlers;
- (char)_handleUnmappedAttributeReport:(id)a0;
- (void)_pairOnSystemCommissionerFabric;
- (id)mtrDeviceWithNodeID:(id)a0 controller:(id)a1;
- (void)_buildHAPCategoriesFromCHIPWithCompletionHandler:(id /* block */)a0;
- (id)_categoryFromAccessoryInfo:(id)a0 endpoint:(id)a1;
- (void)_clearCommissioneeInfoAfterPairingCompletion;
- (void)_collectNetworkCredentials:(id)a0;
- (void)_commissionWithParams:(id)a0;
- (void)_controller:(id)a0 commissioningComplete:(id)a1 nodeID:(id)a2 abstractMetrics:(id)a3;
- (char)_controllerIsOnWiFiNetworkWhichSupportsBothBands;
- (void)_createFirmwareUpdateServiceWithInstanceID:(long long)a0 device:(id)a1 completionHandler:(id /* block */)a2;
- (void)_deleteAccessoryServerData;
- (void)_deregisterDeviceConnectedStateCaptureHandler;
- (void)_deregisterPartsListStateCaptureHandler;
- (void)_deregisterStateCaptureHandlerWithHandle:(unsigned long long)a0;
- (void)_disconnectWithError:(id)a0;
- (id)_distinctEndpointsHavingHapServicesForTopology:(id)a0;
- (id)_endCurrentMetricTimeWithName:(id)a0;
- (void)_endpointForOTARequestorWithTopology:(id)a0 mtrDevice:(id)a1 callbackQueue:(id)a2 completionHandler:(id /* block */)a3;
- (void)_fetchAdditionalThreadNetworkInformationFromDevice:(id)a0 completion:(id /* block */)a1;
- (void)_fetchAdditionalThreadNetworkInformationWithCompletion:(id /* block */)a0;
- (void)_fetchCurrentPairingWithCompletionHandler:(id /* block */)a0;
- (void)_fetchSerialNumberWithCompletionHandler:(id /* block */)a0;
- (id)_fetchSupportedThreadFeatures;
- (void)_finalizePairing;
- (void)_finishMaximumDurationCollection;
- (void)_flushMTRDeviceReadReadyHandlers;
- (void)_getCommissioneeHasActiveNetworkWithNetworkCommissioningCluster:(id)a0 completion:(id /* block */)a1;
- (void)_getCommissioneeNetworkCommissioningClusterEndpointWithDevice:(id)a0 completion:(id /* block */)a1;
- (id)_getFirstNonOtherCategory:(id)a0;
- (long long)_getLinkLayerTypeForAccessory:(id)a0;
- (id)_getOperationalHardwareAddressFromReadValue:(id)a0;
- (id)_getOperationalNetworkAddressForAccessory:(id)a0;
- (id)_getSupportedLinkLayerTypesForAccessory:(id)a0;
- (id)_getThreadHardwareAddressFromReadValue:(id)a0;
- (void)_handleAddNocCompletionDuringSystemCommissionerFabricCommissioningWithDispatchGroup:(id)a0 fabricID:(id)a1 rootCACert:(id)a2 ipk:(id)a3 controllerNodeID:(id)a4 commissioneeNodeID:(id)a5 error:(id)a6;
- (void)_handleAddTrustedRootCertificateCompletionDuringSystemCommissionerFabricCommissioningWithDispatchGroup:(id)a0 fabricID:(id)a1 rootCACert:(id)a2 operationalPublicKey:(id)a3 ipk:(id)a4 controllerNodeID:(id)a5;
- (void)_handleArmFailSafeResponseDuringSystemCommissionerFabricCommissioningWithDispatchGroup:(id)a0 error:(id)a1;
- (void)_handleCSRResponseDuringSystemCommissionerFabricCommissioningWithDispatchGroup:(id)a0 responseParams:(id)a1 error:(id)a2;
- (void)_handleDiagnosticsEvent:(id)a0;
- (void)_handleNewSystemCommissionerFabricNodeID:(id)a0 dispatchGroup:(id)a1 fabricID:(id)a2 rootCACert:(id)a3 operationalPublicKey:(id)a4 ipk:(id)a5 controllerNodeID:(id)a6 device:(id)a7;
- (void)_handlePairOnSystemCommissionerFabricSuccess;
- (void)_handlePairingFailureWithError:(id)a0 context:(id)a1;
- (void)_handleSystemCommissionerRootCertificate:(id)a0 fabricID:(id)a1 ipk:(id)a2 controllerNodeID:(id)a3 dispatchGroup:(id)a4 csrResponseParams:(id)a5 device:(id)a6;
- (void)_handleThreadRadioStateChanged;
- (void)_handleUnreachableStateWithError:(id)a0;
- (id)_hapAccessoryAtEndpoint:(id)a0;
- (char)_ignoreAttributeReport:(id)a0;
- (void)_initAccessoriesForServer;
- (char)_isBridgedBasicInformation:(id)a0 reportClusterID:(id)a1;
- (char)_legacyHMDHAPAccessoryDelegateShouldHandleEvent:(id)a0;
- (void)_metricCollectionWithError:(id)a0 description:(id)a1 progressState:(long long)a2;
- (void)_notifyDelegateOfAccessoryMatchingCommissioningDiscriminatorDiscovered;
- (void)_notifyDelegateOfMTRMetrics:(id)a0;
- (void)_notifyDelegateOfMatterAccessoryIsWEDAccessory:(id)a0;
- (void)_notifyDelegateOfMatterAccessoryThreadCapabilities:(id)a0;
- (void)_notifyDelegateOfMatterAccessoryVendorID:(id)a0 productID:(id)a1 deviceType:(id)a2;
- (void)_notifyDelegateOfPairingProgress:(long long)a0 error:(id)a1;
- (void)_notifyDelegateOfPairingStep:(unsigned long long)a0;
- (void)_notifyDelegateOfSupportedLinkLayerTypes:(id)a0;
- (void)_onUnpairFinishedWithError:(id)a0 queue:(id)a1 completion:(id /* block */)a2;
- (void)_openPairingWindowForDuration:(double)a0 completionHandler:(id /* block */)a1;
- (void)_openPairingWindowWithPINForDuration:(double)a0 completionHandler:(id /* block */)a1;
- (void)_pairOnSystemCommissionerFabricUnlessAlreadyPairedWithRootPublicKey:(id)a0;
- (void)_pairingComplete:(id)a0 context:(id)a1;
- (id)_pairingSetupPayload;
- (void)_persistAccessoryServerData;
- (char)_populateCommissioningParameterCountryCode:(id)a0;
- (id)_populateHandlingForCharacteristics:(id)a0;
- (void)_populateServiceForCharacteristic:(id)a0;
- (void)_populateThreadCredentials:(id)a0 completion:(id /* block */)a1;
- (void)_populateWifiCredentials:(id)a0 scanResults:(id)a1 completion:(id /* block */)a2;
- (void)_prepareThreadCredentials;
- (id)_progressStateToString:(long long)a0;
- (void)_queryBridgedAccessoryAndUnreachablePerCacheForCharacteristic:(id)a0 completion:(id /* block */)a1;
- (void)_readCharacteristicValueFromCacheAfterConfirmingBridgedAccessroyReachabilityWithCharacteristic:(id)a0 responseHandler:(id /* block */)a1;
- (void)_readCharacteristicValueFromCacheWithCharacteristic:(id)a0 responseHandler:(id /* block */)a1;
- (void)_readCharacteristicValues:(id)a0 timeout:(double)a1 skipCache:(char)a2 sendNotification:(char)a3 completionQueue:(id)a4 completionHandler:(id /* block */)a5;
- (id)_readFixedCharacteristicValue:(id)a0;
- (id)_readLocallyMaintainedCharacteristicValue:(id)a0;
- (id)_readPastEventsFromAccessory:(id)a0 forClusters:(id)a1;
- (void)_rebuildHAPServicesFromCHIPWithCompletionHandler:(id /* block */)a0;
- (unsigned long long)_registerStateCaptureHandlerWithStateCaptureInformation:(id)a0;
- (void)_removeSharedAdminControllerNodeIDFromACLWithCompletion:(id /* block */)a0;
- (void)_reportPairingComplete;
- (void)_restoreCommissioneeInfoBeforeNextPairingAttempt;
- (void)_retryPairing;
- (void)_setCategoryForPrimaryAccessory:(id)a0;
- (void)_setSupportedLinkLayerTypes:(id)a0;
- (void)_setupMatterDevice;
- (void)_setupPairingProperties;
- (char)_shouldUpdateUnreachableState;
- (void)_startCurrentMetricTimeWithName:(id)a0;
- (void)_startInitialReachableStateTimerWithCompletion:(id /* block */)a0;
- (void)_startLocallyDiscoveredAccessoryServerPairingWithRequest:(id)a0 fabricID:(id)a1;
- (void)_startPairingWithError:(id *)a0 pairingEndContext:(id *)a1;
- (void)_startPairingWithReadyToCancelHandler:(id /* block */)a0 error:(id *)a1 pairingEndContext:(id *)a2;
- (void)_unpair:(id)a0 completion:(id /* block */)a1;
- (void)_updateAdditionalCharacteristicsFromCharacteristicUpdate:(id)a0 service:(id)a1 path:(id)a2 completionHandler:(id /* block */)a3;
- (void)_updateAttributeTimer:(id)a0 report:(id)a1 timeout:(double)a2 server:(id)a3;
- (void)_updateDefaultEntriesForBridgedClusterIDCharacteristicMap:(id)a0;
- (void)_updateDefaultEntriesForClusterIDCharacteristicMap:(id)a0;
- (void)_updateDelegateOfConnectionStatus:(char)a0 withError:(id)a1;
- (void)_updateMetricWithProgressState:(long long)a0 error:(id)a1;
- (void)_updatedCharacteristicsForAttributeReport:(id)a0 completionHandler:(id /* block */)a1;
- (void)_updatedCharacteristicsForEventReport:(id)a0 completionHandler:(id /* block */)a1;
- (void)_writeCharacteristicValues:(id)a0 responseTuples:(id)a1 completionQueue:(id)a2 completionHandler:(id /* block */)a3;
- (void)abortStagingWithError:(id)a0 context:(id)a1;
- (char)accessoryLinkLayerTypeIsWifiOrEthernet;
- (char)accessoryWithSameDiscriminatorDiscovered;
- (void)addMetricsWithDuration:(id)a0 metricsKey:(id)a1;
- (void)addReportObserver:(id)a0;
- (void)announceOtaProvider:(id)a0 providerEndpoint:(id)a1 immediateAnnouncement:(char)a2 delayCounter:(long long)a3 completionHandler:(id /* block */)a4;
- (void)commitStagedPairing;
- (void)configureDefaultRequiresThreadRouter;
- (id)connectedNetworkIDFromReadValue:(id)a0;
- (struct os_state_data_s { unsigned int x0; union { unsigned int x0 : 32; unsigned int x1; } x1; struct os_state_data_decoder_s { char x0[64]; char x1[64]; } x2; char x3[64]; unsigned char x4[0]; } *)createStateData:(id)a0 data:(id)a1;
- (id)currentDeviceTypeFromDCL;
- (id)dataOfReadValue:(id)a0;
- (id)deviceTopology;
- (void)didUpdateReachability:(char)a0;
- (char)doesMatchDiscriminator:(id)a0;
- (id)dumpState:(id)a0;
- (id)endPointForHapAccessory:(id)a0;
- (id)endpointForDiagnosticCluster:(id)a0 clusterName:(id)a1 amongEndpoints:(id)a2 topology:(id)a3 device:(id)a4 definitelyUnsupported:(char *)a5;
- (id)endpointsForHAPAccessory:(id)a0 topology:(id)a1;
- (void)enumerateHAPServices:(id /* block */)a0;
- (id)errorCountOfThreadNetworkDiagnostics:(id)a0;
- (id)errorCountOfWiFiNetworkDiagnostics:(id)a0;
- (void)fetchAndNotifyCharacteristics:(id)a0 timeout:(double)a1 completionQueue:(id)a2 completionHandler:(id /* block */)a3;
- (void)fetchExtendedMACAddressFromDevice:(id)a0 completion:(id /* block */)a1;
- (void)fetchSoftwareVersion:(char)a0 completionHandler:(id /* block */)a1;
- (void)fetchWEDSupportInformationFromDevice:(id)a0 completion:(id /* block */)a1;
- (id)generalDiagnosticsClusterFromEndpoints:(id)a0 topology:(id)a1 device:(id)a2 definitelyUnsupported:(char *)a3;
- (void)generateAccessoryConfigurationForReason:(id)a0 completionHandler:(id /* block */)a1;
- (long long)getLinkLayerTypeForAccessory:(id)a0;
- (id)getSupportedLinkLayerTypesForAccessory:(id)a0;
- (id)getSupportedLinkLayerTypesFromNetworkCommissioningClusterFeatureMap:(id)a0;
- (void)handleCHIPRemoteRequest:(id)a0 device:(id)a1 completion:(id /* block */)a2;
- (void)handleDownloadLogCommand:(id)a0 device:(id)a1 completion:(id /* block */)a2;
- (void)handleFirmwareUpdateStatusChange:(long long)a0;
- (void)handleRemoveFromBrowser;
- (void)handleThreadDirectConnectionSleepyTypeChange:(char)a0;
- (void)handleWEDDisconnectedWhileNotPairing;
- (char)hasNetworkProvisioningFailed:(unsigned int)a0;
- (id)highestRSSIofThreadNetworkDiagnostics:(id)a0;
- (id)initWithKeystore:(id)a0 browser:(id)a1;
- (void)isDiscoverableWithCompletion:(id /* block */)a0;
- (char)isLocallyDiscoveredAndMatchesDiscriminatorOfSetupPayload:(id)a0;
- (char)isPairedInStorage;
- (char)isReadyToReadFromMTRDevice;
- (void)markAsSubscribed;
- (void)markForResubscription;
- (id)mergeExistingAclEntries:(id)a0 withAdminNodes:(id)a1 regularUserNodes:(id)a2;
- (id)mergeExistingAclEntries:(id)a0 withNewNodes:(id)a1 withPrivilege:(unsigned char)a2;
- (id)mtrBaseDeviceWithNodeID:(id)a0 controller:(id)a1;
- (id)networkCommissioningClusterFromEndpoints:(id)a0 topology:(id)a1 device:(id)a2 definitelyUnsupported:(char *)a3;
- (void)notifyDelegateOfAccessoryMatchingCommissioningDiscriminatorDiscovered;
- (void)notifyMatterFirmwareRevisionNumberCharacteristicChanged;
- (id)numberOfReadValue:(id)a0;
- (id)packetCountOfThreadNetworkDiagnostics:(id)a0;
- (id)packetCountOfWiFiNetworkDiagnostics:(id)a0;
- (id)populateACLEntriesAfterPairing;
- (id)populateACLEntriesForPairing;
- (void)processAttributeReport:(id)a0;
- (id)protocolMappingStateForCharacteristic:(id)a0;
- (void)queueAccessoryOperation:(id /* block */)a0 highPriority:(char)a1 completion:(id /* block */)a2;
- (void)readPairingWindowStatusWithCompletionHandler:(id /* block */)a0;
- (void)refreshThreadCapabilitiesWithCompletion:(id /* block */)a0;
- (id)removeNode:(id)a0 withPrivilge:(unsigned char)a1 fromExistingAclEntries:(id)a2;
- (void)removeReportObserver:(id)a0;
- (void)setClusterIDForCharacteristic:(id)a0 endpointID:(id)a1 clusterID:(id)a2;
- (void)setDiagnosticsEventHandler:(id)a0;
- (void)setRemoveReason:(id)a0 pairingEndContextWhenRemove:(id)a1;
- (void)setupThreadPairing;
- (void)startStagedPairingWithCompletion:(id /* block */)a0;
- (void)startStagedPairingWithDeviceCredentialHandler:(id /* block */)a0 wifiScanResultsHandler:(id /* block */)a1 threadScanResultsHandler:(id /* block */)a2 readyToCancelHandler:(id /* block */)a3 progressUpdateHandler:(id /* block */)a4 scanningNetworks:(char)a5 completion:(id /* block */)a6;
- (id)stringOfReadValue:(id)a0;
- (id)threadNetworkDiagnosticsClusterFromEndpoints:(id)a0 topology:(id)a1 device:(id)a2 definitelyUnsupported:(char *)a3;
- (void)triggerEstablishingMatterSubscription;
- (void)updateAccessoryControlToAdministratorNodes:(id)a0 sharedUserNodes:(id)a1 completion:(id /* block */)a2;
- (void)updateAccessoryControlToIncludeAdministratorNodes:(id)a0 sharedUserNodes:(id)a1 completion:(id /* block */)a2;
- (void)updateAccessoryControlToRemoveAdministratorNode:(id)a0 completion:(id /* block */)a1;
- (void)updateAllCharacteristicValuesPostHAPServiceEnumeration:(id /* block */)a0;
- (void)updateAllCharacteristicValuesPostHAPServiceEnumerationForAccessory:(id)a0 completion:(id /* block */)a1;
- (void)updateDefaultOtaProvider:(id)a0 providerEndpoint:(id)a1 completionHandler:(id /* block */)a2;
- (char)updateProductAttributes;
- (void)updateSoftwareVersion:(id)a0;
- (void)updateVidPidWithAttestationDeviceInfo:(id)a0;
- (void)validateAttestationDeviceInfo:(id)a0 error:(id)a1 completion:(id /* block */)a2;
- (id)wifiNetworkDiagnosticsClusterFromEndpoints:(id)a0 topology:(id)a1 device:(id)a2 definitelyUnsupported:(char *)a3;
- (id)wifiNetworkInfoForSSID:(id)a0;

@end
