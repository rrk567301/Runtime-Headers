@class WBSPair, NSMutableDictionary, NSLocale, NSObject, WBSAutoFillQuirksManager, NSMutableArray, WBSKeychainCredentialNotificationMonitor, WBSAuthenticationServicesAgentProxy;
@protocol OS_dispatch_queue;

@interface WBSFormDataController : NSObject {
    WBSKeychainCredentialNotificationMonitor *_keychainMonitor;
    id _keychainNotificationRegistrationToken;
    NSMutableDictionary *_completionDB;
    NSMutableDictionary *_valuesDB;
    NSMutableDictionary *_preferredLabelsMap;
    unsigned long long _completionDBSize;
    NSMutableDictionary *_domainToLastUsedUsernameAndProtectionSpace;
    NSMutableArray *_recentlyUsedAutoFillSets;
    NSMutableDictionary *_preferredLabelForUniqueIDOfPersonMap;
    WBSPair *_cachedBirthdayAndLocalizedStrings;
    NSObject<OS_dispatch_queue> *_internalQueue;
    NSMutableDictionary *_mapOfHighLevelDomainToLastAutomaticFormSubmission;
    NSMutableArray *_recentAutoFillEvents;
    id _keybagCallbackToken;
    char _shouldShowPasswordsAppOnboarding;
    WBSAuthenticationServicesAgentProxy *_agentProxy;
}

@property (readonly, nonatomic) WBSAutoFillQuirksManager *autoFillQuirksManager;
@property (retain, nonatomic) NSLocale *locale;

+ (char)isNameProperty:(id)a0;
+ (id)stringWithAddressBookValue:(id)a0 key:(id)a1;
+ (char)_password:(id)a0 appearsToBeASixDigitCodeAppendedToPassword:(id)a1;
+ (char)_password:(id)a0 appearsToBeMoreThanOneSymbolAppendedToPassword:(id)a1;
+ (id)addressBookAddressPropertyKey;
+ (id)allAddressBookAddressComponentKeys;
+ (id)allAddressBookNonAddressPropertyKeys;
+ (id)allSynonymsForMatch:(id)a0;
+ (long long)availableManualAutoFillActionForTextField:(id)a0 form:(id)a1 outUsernameElementUniqueID:(id *)a2 outPasswordElementUniqueID:(id *)a3 outConfirmPasswordElementUniqueID:(id *)a4;
+ (char)canAutocompleteTextField:(id)a0 inForm:(id)a1;
+ (char)contactIsMe:(id)a0;
+ (id)contactKeyForString:(id)a0;
+ (id)continuingFieldsInFormControls:(id)a0 startingAtIndex:(unsigned long long)a1;
+ (id)controlsConsideredByAutoFillInForm:(id)a0;
+ (char)convertNumber:(id)a0 toAutoFillFormType:(unsigned long long *)a1;
+ (id)domainFromURL:(id)a0;
+ (id)dontSaveMarker;
+ (id)fieldToFocusBeforeSubmittingForm:(id)a0;
+ (char)formContainsCreditCardData:(id)a0;
+ (char)formContainsCreditCardNumberField:(id)a0;
+ (char)formContainsDateFields:(id)a0 matchingAddressBookMatch:(id)a1;
+ (char)isAddressBookAutoFillAllowedInFormWithType:(unsigned long long)a0;
+ (char)isFieldUnidentified:(id)a0;
+ (char)isUsernameFieldInLoginFormWithTextFieldMetadata:(id)a0 formMetadata:(id)a1;
+ (id)lastFieldInControls:(id)a0 inForm:(id)a1;
+ (id)localizedLowercaseContactProperty:(id)a0;
+ (id)metadataForControlWithUniqueID:(id)a0 inForm:(id)a1;
+ (id)nextFieldAfterControls:(id)a0 inForm:(id)a1;
+ (char)password:(id)a0 shouldBeConsideredEqualToExistingPassword:(id)a1;
+ (char)shouldDisplayHideMyEmailForControl:(id)a0;
+ (char)shouldDisplayOneTimeCodeForControl:(id)a0 inForm:(id)a1;
+ (id)specifierForAddressBookLabel:(id)a0;
+ (id)specifierForControl:(id)a0;
+ (char)stringLooksLikeCreditCardNumber:(id)a0;
+ (char)textFieldLooksLikeCreditCardFormField:(id)a0;
+ (char)textFieldLooksLikeCreditCardNumericFormField:(id)a0;
+ (id)valueOfControlWithUniqueID:(id)a0 inForm:(id)a1;
+ (id)valuesFromUser:(id)a0 password:(id)a1 forLoginOrChangePasswordForm:(id)a2;

- (void)dealloc;
- (id)init;
- (void).cxx_destruct;
- (void)_currentLocaleDidChange:(id)a0;
- (id)matchesForControl:(id)a0 atURL:(id)a1 matchingPartialString:(id)a2 autoFillDataType:(long long)a3 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a4;
- (void)updateLastUsedUsernameAndExtractUsernameAndPasswordFromForm:(id)a0 shouldPreferAnnotatedCredentials:(char)a1 atURL:(id)a2 username:(id *)a3 password:(id *)a4 inContext:(id)a5;
- (void)_addControlIDToAutoFillValuesToAutoFillItems:(id)a0 formMetadata:(id)a1 startingAtIndex:(unsigned long long)a2;
- (unsigned long long)_addMatchesForControl:(id)a0 startingAtIndex:(unsigned long long)a1 formMetadata:(id)a2 fromExistingMatches:(id)a3 fromAllMatchesIfNecessary:(id)a4 addToFoundMatches:(id)a5 addToAutoFillValues:(id)a6 multiRoundAutoFillManager:(id)a7 propertyToIdentifierMapForFoundMatches:(id)a8 shouldUseExistingMatchesToFillFocusedField:(char)a9;
- (id)_autoFillSetFromMatches:(id)a0 label:(id)a1 contact:(id)a2 form:(id)a3;
- (id)_cachedLocalizedStringsForBirthdate:(id)a0;
- (id)_completionDB;
- (char)_dateIsWithinGracePeriodForNotAutomaticallySubmittingLoginForms:(id)a0;
- (id)_domainsWithPreviousDataOnInternalQueue;
- (void)_fillPhoneNumber:(id)a0 intoValues:(id)a1 controls:(id)a2 formTextSample:(id)a3 multiRoundAutoFillManager:(id)a4;
- (id)_formKeyForMapOfHighLevelDomainToLastAutomaticFormSubmission:(id)a0 formMetadata:(id)a1;
- (char)_hasMatchingAutoFillEventForUsername:(id)a0 withinTimeInterval:(double)a1 inTabWithID:(id)a2 currentURL:(id)a3 shouldAllowPrivateTabs:(char)a4 shouldRemoveMatchingEvent:(char)a5 shouldIncludeExternalCredentialEvents:(char)a6;
- (unsigned long long)_indexForControlWithUniqueID:(id)a0 inForm:(id)a1;
- (id)_keyToLookUpInAnnotationsForPasswordForFormMetadata:(id)a0;
- (id)_knownUsernamesWithPasswordsForURL:(id)a0 inContext:(id)a1;
- (id)_lastUsedUsernameForDomain:(id)a0;
- (void)_loadCompletionDBIfNeededOnInternalQueue;
- (char)_matchHasPreferredIdentifierOrShouldBeFilledInMultiRoundAutoFill:(id)a0 specifier:(id)a1 multiRoundAutoFillManager:(id)a2 contact:(id)a3;
- (id)_matchesForControl:(id)a0 inDomain:(id)a1 matchingPartialString:(id)a2 autoFillDataType:(long long)a3 preferredLabel:(id)a4 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a5;
- (id)_matchesForControl:(id)a0 inDomain:(id)a1 matchingPartialString:(id)a2 autoFillDataType:(long long)a3 preferredLabel:(id)a4 contact:(id)a5 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a6;
- (id)_nextFieldToFocusAfterFillingFieldAtIndex:(unsigned long long)a0 inForm:(id)a1;
- (id)_phoneNumberCandidates:(id)a0 fillingMultipleFields:(char)a1;
- (id)_recentlyUsedAutoFillDictionaries;
- (void)_removeExpiredAutoFillEvents:(id)a0;
- (void)_removeStaleEntriesFromMapOfHighLevelDomainToLastAutomaticFormSubmission;
- (void)_setLastUsedUsername:(id)a0 andProtectionSpace:(id)a1 forDomain:(id)a2;
- (id)_singleFieldPhoneNumberCandidates:(id)a0;
- (id)_valuesForStandardForm:(id)a0 inDomain:(id)a1 autoFillDataType:(long long)a2 matches:(id *)a3 preferredLabel:(id)a4 multiRoundAutoFillManager:(id)a5 wantAllMatches:(char)a6 contact:(id)a7 existingMatches:(id)a8 shouldUseExistingMatchesToFillFocusedField:(char)a9 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a10;
- (void)addABMatchesForValueSpecifier:(id)a0 matchingPartialString:(id)a1 toArray:(id)a2 preferredLabel:(id)a3 contact:(id)a4 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a5;
- (void)addAllAddressDataIfNecessary:(id)a0 contactLabel:(id)a1 contact:(id)a2;
- (void)addPreviousDataMatchesForFieldWithName:(id)a0 inDomain:(id)a1 matchingPartialString:(id)a2 toArray:(id)a3;
- (void)addRecentlyUsedAutoFillSet:(id)a0 appendToTheEnd:(char)a1;
- (unsigned long long)addValuesForStandardFormControls:(id)a0 startingAtIndex:(unsigned long long)a1 fromAutoFillItem:(id)a2 toDictionary:(id)a3 formTextSample:(id)a4 multiRoundAutoFillManager:(id)a5;
- (unsigned long long)addValuesForStandardFormControlsInForm:(id)a0 startingAtIndex:(unsigned long long)a1 fromAutoFillItem:(id)a2 toDictionary:(id)a3 multiRoundAutoFillManager:(id)a4;
- (char)addressBookHasDataForLabel:(id)a0;
- (id)addressBookMatchesForFullNameForContact:(id)a0;
- (id)addressBookMatchesForProperty:(id)a0 key:(id)a1 label:(id)a2;
- (id)addressBookMatchesForProperty:(id)a0 key:(id)a1 label:(id)a2 partialString:(id)a3 contact:(id)a4 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a5;
- (id)allFormDataForSaving;
- (id)annotationsFromUsername:(id)a0 forLoginOrChangePasswordForm:(id)a1;
- (long long)autoFillActionForFormType:(unsigned long long)a0 onURL:(id)a1 shouldSubmitAfterFilling:(char)a2;
- (id)autoGeneratedPasswordForURL:(id)a0 respectingPasswordRequirements:(id)a1 maxLength:(unsigned long long)a2;
- (id)bestAddressBookLabelForFormMetadata:(id)a0 formControlValue:(id)a1;
- (id)bestMatchForControl:(id)a0 inDomain:(id)a1 matchingPartialString:(id)a2 autoFillDataType:(long long)a3 preferredLabel:(id)a4 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a5;
- (id)bestMatchForControl:(id)a0 inDomain:(id)a1 matchingPartialString:(id)a2 autoFillDataType:(long long)a3 preferredLabel:(id)a4 contact:(id)a5 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a6;
- (char)canApplyAutoFillGracePeriodForUsername:(id)a0 inTabWithID:(id)a1 currentURL:(id)a2;
- (void)canAutomaticallyRegisterPasskeyForUsername:(id)a0 inTabWithID:(id)a1 currentURL:(id)a2 completionHandler:(id /* block */)a3;
- (id)cascadingAddressMatchesForMatch:(id)a0 contact:(id)a1;
- (void)clearPreviousDataDatabaseItemsAddedAfterDate:(id)a0;
- (void)clearPreviousDataForDomain:(id)a0;
- (id)completionDBPath;
- (id)contactAutoFillSetForRecentlyUsedAutoFillSet:(id)a0 contact:(id)a1 form:(id)a2;
- (void)didAutomaticallySubmitFormWhenFillingOnURL:(id)a0 formMetadata:(id)a1;
- (void)didFillPasswordForUsername:(id)a0 fromProviderWithBundleIdentifier:(id)a1 inTabWithID:(id)a2 currentURL:(id)a3 isPrivate:(char)a4 requester:(id)a5;
- (id)domainsWithPreviousData;
- (void)domainsWithPreviousDataChanged;
- (id)encryptOrDecryptData:(id)a0 encrypt:(char)a1;
- (id)exactFQDNAndOtherSavedAccountMatchesForForm:(id)a0 atURL:(id)a1 webFrameIdentifier:(id)a2 savedAccountContext:(id)a3;
- (char)frameIsKnownToAskForCredentialsFromOtherServicesFromAncestorFrameURLs:(id)a0;
- (void)getFormFieldValues:(id *)a0 andFieldToFocus:(id *)a1 andCreditCardDataTypesThatWillBeFilled:(id *)a2 forCreditCardForm:(id)a3 fromCreditCardData:(id)a4;
- (void)getFormFieldValues:(id *)a0 andFieldToFocus:(id *)a1 withSingleCreditCardData:(id)a2 inField:(id)a3 inForm:(id)a4;
- (void)getSavedAccountMatchesWithCriteria:(id)a0 synchronously:(char)a1 completionHandler:(id /* block */)a2;
- (char)hasUserDeniedAccessToCredential:(id)a0 inProtectionSpace:(id)a1;
- (id)infoForDomain:(id)a0;
- (id)initWithAggressiveKeychainCaching:(char)a0;
- (char)isControlASelectElement:(id)a0;
- (char)isPasswordFieldForUserCredentialsWithMetadata:(id)a0 formMetadata:(id)a1;
- (id)lastUsedUsernameWithPasswordForURL:(id)a0 inContext:(id)a1;
- (void)loadCompletionDBIfNeeded;
- (id)matchesForControl:(id)a0 atURL:(id)a1 matchingPartialString:(id)a2 autoFillDataType:(long long)a3 contact:(id)a4 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a5;
- (id)metadataOfActiveFormOrBestFormForPageLevelAutoFill:(id)a0 frame:(struct OpaqueFormAutoFillFrame **)a1;
- (id)metadataOfBestFormForStreamlinedLogin:(id)a0 frame:(struct OpaqueFormAutoFillFrame **)a1;
- (void)notifyKeychainWasDirectlyAffectedBySafari;
- (id)orderedHomeAndWorkSetsForContact:(id)a0 form:(id)a1;
- (char)preferredIdentifierExistsForProperty:(id)a0 withContact:(id)a1;
- (id)preferredIdentifierForProperty:(id)a0 withContact:(id)a1;
- (void)pruneCompletionDB;
- (id)recentlyUsedAutoFillSets;
- (void)saveCompletionDBSoon;
- (void)saveRecentlyUsedAutoFillSetWithMatchesToFill:(id)a0 matchesForDoNotFill:(id)a1;
- (id)savedAccountUsingDomainsToConsiderIdenticalWithUsername:(id)a0 url:(id *)a1 host:(id *)a2;
- (void)setInfo:(id)a0 forDomain:(id)a1;
- (void)setPreferredIdentifier:(id)a0 forProperty:(id)a1 withContact:(id)a2;
- (char)shouldAutoFillFromAddressBook;
- (char)shouldAutoFillFromPreviousData;
- (char)shouldAutoFillPasswords;
- (BOOL)shouldForceUSLocaleForAutoFillFillingTest;
- (char)shouldSubmitForm:(id)a0 withUser:(id)a1 password:(id)a2 onURL:(id)a3;
- (id)substituteCredential:(id)a0 inProtectionSpace:(id)a1;
- (char)textFieldIsEligibleForAutomaticStrongPassword:(id)a0 form:(id)a1 ignorePreviousDecision:(char)a2 textFieldCurrentlyContainsStrongPassword:(char *)a3;
- (char)textFieldMetadataMeetsRequirementsForAutomaticStrongPasswordTreatment:(id)a0 form:(id)a1;
- (id)uniqueIDOfContact:(id)a0;
- (id)valuesForContactFormWithMetadata:(id)a0 inDomain:(id)a1 matches:(id *)a2 multiRoundAutoFillManager:(id)a3;
- (id)valuesForContactFormWithMetadata:(id)a0 inDomain:(id)a1 matches:(id *)a2 multiRoundAutoFillManager:(id)a3 contact:(id)a4;
- (id)valuesForContactFormWithMetadata:(id)a0 matches:(id *)a1 multiRoundAutoFillManager:(id)a2 existingMatches:(id)a3 contact:(id)a4;
- (id)valuesForContactFormWithMetadata:(id)a0 matches:(id *)a1 multiRoundAutoFillManager:(id)a2 existingMatches:(id)a3 shouldUseExistingMatchesToFillFocusedField:(char)a4 contact:(id)a5;
- (id)valuesForFormWithMetadata:(id)a0 hideMyEmailRecord:(id)a1;
- (id)valuesForStandardForm:(id)a0 inDomain:(id)a1 autoFillDataType:(long long)a2 matches:(id *)a3 preferredLabel:(id)a4 multiRoundAutoFillManager:(id)a5;
- (id)valuesForStandardForm:(id)a0 inDomain:(id)a1 autoFillDataType:(long long)a2 matches:(id *)a3 preferredLabel:(id)a4 multiRoundAutoFillManager:(id)a5 contact:(id)a6 allowingIdentifiedAddressBookLabelToOverridePreferredIdentifier:(char)a7;
- (void)willSubmitFormWithCredentials:(id)a0 shouldPreferAnnotatedCredentials:(char)a1 atURL:(id)a2 username:(id *)a3 password:(id *)a4 inContext:(id)a5;
- (void)willSubmitFormWithCredentials:(id)a0 shouldPreferAnnotatedCredentials:(char)a1 atURL:(id)a2 username:(id *)a3 password:(id *)a4 inContext:(id)a5 shouldShowPasswordsAppOnboarding:(char)a6;

@end
