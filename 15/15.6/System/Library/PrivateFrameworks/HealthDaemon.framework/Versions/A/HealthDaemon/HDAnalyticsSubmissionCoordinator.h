...
- (void)cloudSync_reportFullSyncMetricsWithReason:(id)a0 shard:(id)a1 daysSincePreviousFullSync:(id)a2 totalDuration:(double)a3 activeDuration:(double)a4 numberOfRuns:(id)a5 incomplete:(char)a6;
...
- (void)authorization_reportAuthRequestsFromBundleIdentifier:(id)a0 isExtension:(char)a1 isAppleWatch:(char)a2 shouldPrompt:(char)a3;
...
- (void)cloudCache_reportDailyCacheAnalyticsWithPushTargets:(long long)a0 pullTargets:(long long)a1 sharedDBSummarySharingZones:(long long)a2 privateDBSummarySharingZones:(long long)a3 unifiedZoneInSharedDB:(char)a4 unifiedZoneInPrivateDB:(char)a5 deviceContexts:(unsigned long long)a6 deviceKeys:(unsigned long long)a7 nilSyncIdentities:(unsigned long long)a8;
- (void)cloudCache_reportWeeklyAnalyticsWithCacheValidation:(id)a0;
- (void)cloudSync_newChildAdded:(char)a0;
...
- (void)cloudSync_reportDailyAnalyticForProfileType:(long long)a0 syncEnabled:(char)a1 syncDisabledError:(id)a2 primaryContainer:(id)a3 restoreComplete:(char)a4 timeSinceRestoreStart:(id)a5 journalSize:(id)a6 oldestJournalAge:(id)a7 timeSinceLastSuccessfulPull:(id)a8 timeSinceLastSuccessfulPush:(id)a9 timeSinceLastSuccessfulLitePush:(id)a10 timeSinceLastPushForwardProgress:(id)a11 storeCount:(long long)a12 zoneCount:(long long)a13 hasUnifiedZone:(char)a14 hasMasterZone:(char)a15 hasMedicalID:(char)a16 errorRequiringUserAction:(id)a17 childSyncIdentityCount:(long long)a18 canUpgradeToSyncIdentity:(char)a19 startedUpgradeToSyncIdentity:(char)a20 completedUpgradeToSyncIdentity:(char)a21 iPhoneCount:(id)a22 iPadCount:(id)a23 watchCount:(id)a24 visionProCount:(id)a25 totalSyncsCount:(id)a26 successfulSyncPercentageCount:(id)a27;
...
- (void)cloudSync_reportRebaseTriggeredByDeletionByUser:(char)a0;
- (void)cloudSync_reportRestoreForProfile:(id)a0 startDate:(id)a1 endDate:(id)a2 duration:(double)a3 finishedJournalMerge:(char)a4;
...
- (void)database_reportSQLiteCorruptionWithExtendedErrorCode:(int)a0 type:(long long)a1 profile:(id)a2 description:(id)a3 sqlStatement:(id)a4 failedObliterationAttempt:(char)a5;
...
- (char)isEventUsed:(id)a0;
- (char)isImprovedHealthAndActivityEnabled;
- (void)maintenanceCoordinator_reportCoreAnalyticsWithOperationName:(id)a0 database:(id)a1 pendingOperationsCount:(long long)a2 activeOperationsCount:(long long)a3 timeUntilStart:(long long)a4 canceled:(char)a5 timedOut:(char)a6 elapsedTime:(long long)a7 isImmediateRequest:(char)a8 async:(char)a9;
- (void)medicalID_dailyReport:(id)a0 sosContactStatus:(char)a1 profileType:(long long)a2;
...
- (char)periodicActivityRequiresProtectedData:(id)a0;
...
- (void)summarySharing_reportSetupOperation:(id)a0 success:(char)a1 error:(id)a2;
...
- (void)workout_reportMirroringEventWithStartDuration:(double)a0 stopDuration:(double)a1 mirroringDuration:(double)a2 numOfSendDataRequests:(long long)a3 maxTimeToSendData:(double)a4 minTimeToSendData:(double)a5 avgTimeToSendData:(double)a6 isFirstParty:(char)a7;
- (void)workout_reportWorkoutCondenserEventWithReason:(long long)a0 batchSize:(long long)a1 hasWatchSource:(char)a2 duration:(double)a3 success:(char)a4 error:(id)a5 workoutsToCondense:(long long)a6 workoutsToRecondense:(long long)a7 condensedWorkouts:(long long)a8 processedWorkouts:(long long)a9 createdSeries:(long long)a10 deletedSamples:(long long)a11;
- (void)workout_reportWorkoutEventWithHeartBeatFailures:(long long)a0 workoutDuration:(double)a1 isFirstParty:(char)a2;
