...
@interface HDCloudSyncCoordinator : NSObject <HDCloudSyncManagerTaskQueueDelegate, HDDiagnosticObject, HDHealthDaemonReadyObserver, HDDatabaseJournalMergeObserver, HDProfileReadyObserver> {
    ...
    char _lock_cloudSyncEnabled;
    char _lock_hasComputedCloudSyncEnabled;
    char _lock_cloudSyncSupportEnabled;
    char _lock_cloudSyncSupportValueLoaded;
    char _lock_syncInProgress;
...
- (char)canPerformCloudSyncWithError:(id *)a0;
...
- (char)hasActiveSyncTask;
- (char)markAllClientSyncRequestsAsInProgress:(id *)a0;
- (id)mergeCloudSyncJournalsAndNotifyObservers:(id)a0 syncRequestCompleted:(id)a1 success:(char)a2 error:(id)a3;
- (id)mergeCloudSyncJournalsWithCompletion:(id /* block */)a0;
- (double)nextCloudKitOperationDelay;
- (void)notifyObservers:(id)a0 syncRequestCompleted:(id)a1 success:(char)a2 error:(id)a3;
...
- (char)scheduleSyncForAllProfilesViaGatedXPCActivity:(char)a0 context:(id)a1 reason:(id)a2 error:(id *)a3;
- (char)shouldSyncProfile:(id)a0;
- (id)stateSyncDomainForSubscriptionIdentifier;
- (id)stateSyncEntityClasses;
- (void)syncAllProfilesViaGatedXPCActivity:(char)a0 context:(id)a1 reason:(id)a2 completion:(id /* block */)a3;
