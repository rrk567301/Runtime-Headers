@class NSURL, CUBonjourAdvertiser, NSMutableDictionary, CUCoalescer, RPIdentity, NSObject, RPFileTransferResumeState, CUTCPServer, NSMutableArray, RPFileTransferProgress, CUBonjourDevice, NSString, CUBonjourBrowser, RPConnection, CUWiFiManager, NSMutableSet, NSData;
...
- (char)isResumable;
- (char)prepareTemplateAndReturnError:(id *)a0;
...
- (void)_updateLargeFilesTaskResumeState:(id)a0 data:(id)a1 sendFlags:(unsigned int)a2 error:(id)a3 complete:(char)a4;
- (id)resumeStateDict;
- (char)_activateAndReturnError:(id *)a0;
- (char)_activateSourceAndReturnError:(id *)a0;
- (char)_activateTargetAndReturnError:(id *)a0;
...
- (void)_largeFileReceiveTaskRespond:(id)a0 error:(id)a1 complete:(char)a2 responseHandler:(id /* block */)a3;
- (void)_largeFileReceiveTaskRun:(id)a0 data:(id)a1 sendFlags:(unsigned int)a2 responseHandler:(id /* block */)a3;
- (char)_largeFileReceiveTaskUpdateSHA256CtxFromFileItem:(id)a0 length:(unsigned long long)a1 error:(id *)a2;
...
- (void)_largeFileSendTaskResponse:(id)a0 error:(id)a1 end:(char)a2 xid:(unsigned int)a3;
- (void)_largeFileSendTaskSend:(id)a0 data:(id)a1 end:(char)a2 xid:(unsigned int)a3;
- (void)_largeFileSendTaskStart:(id)a0;
- (void)_metricAddFileSize:(long long)a0;
- (id)_modTimeForFileWithPath:(const char *)a0 error:(id *)a1;
- (int)_openReadFileItem:(id)a0 error:(id *)a1;
- (int)_openReadPath:(const char *)a0 error:(id *)a1;
- (int)_openWriteFileItem:(id)a0 size:(long long)a1 error:(id *)a2;
- (int)_openWriteFileItem:(id)a0 size:(long long)a1 truncate:(char)a2 error:(id *)a3;
- (int)_openWritePath:(id)a0 size:(long long)a1 error:(id *)a2;
- (int)_openWritePath:(id)a0 size:(long long)a1 truncate:(char)a2 error:(id *)a3;
- (void)_prefsChanged;
- (char)_prepareItem:(id)a0 error:(id *)a1;
- (void)_processFinish;
- (void)_processReceivedItem:(id)a0 responseHandler:(id /* block */)a1;
- (void)_processReceivedItems:(id)a0 responseHandler:(id /* block */)a1;
- (char)_readFD:(int)a0 buffer:(char *)a1 size:(unsigned long long)a2 error:(id *)a3;
...
- (void)_resumeStateReceiveRequest:(id)a0 responseHandler:(id /* block */)a1;
- (void)_scheduleItems;
...
- (char)_smallFilesReceiveTaskFileItem:(id)a0 error:(id *)a1;
...
- (char)_truncateFD:(int)a0 size:(long long)a1 error:(id *)a2;
- (void)_updateSmallFilesTaskResumeState:(id)a0 error:(id)a1;
- (void)_updateWiFi;
- (char)_writeFD:(int)a0 buffer:(const char *)a1 size:(unsigned long long)a2 error:(id *)a3;
