...
@interface MTLLegacySVDevice : MTLToolsDevice {
    struct LegacySVMetalBufferHeap { struct mutex { struct _opaque_pthread_mutex_t { long long __sig; char __opaque[56]; } __m_; } _mutex; struct vector<id<MTLBuffer>, std::allocator<id<MTLBuffer>>> { __end_ **__begin_; __end_cap_ **x0; struct __compressed_pair<id<MTLBuffer> *, std::allocator<id<MTLBuffer>>> { id *__value_; } x1; } _buffers; struct vector<unsigned int, std::allocator<unsigned int>> { unsigned int *__begin_; unsigned int *__end_; struct __compressed_pair<unsigned int *, std::allocator<unsigned int>> { unsigned int *__value_; } __end_cap_; } _freeList; int _currentFreeIndex; unsigned long long _totalMemoryAllocated; unsigned long long _totalMemoryInUse; unsigned long long _bufferLength; MTLLegacySVDevice *_device; } bufferHeap;
...
@property (readonly, nonatomic) char supportsResourceUsageValidation;
...
- (void)setWritableHeapsEnabled:(char)a0;
