...
@interface SASampleStore : NSObject <NSSecureCoding> {
    ...
    char _isRootInstalled;
    char _isAppleInternal;
    char _appleInternalIsKnown;
    char _isInternalCarryDevice;
    char _hasInternalCarryDeviceDistinction;
    char _hasHardwareKeyboardInfo;
    char _hardwareKeyboard;
    char _hasEnterprisePersona;
    char _hidEventsHaveHighFidelityTimelines;
...
@property char kPerfPETParsePastLastStackshot;
@property char keepStateBetweenSampleIndexes;
@property char haveKPerfSched;
...
@property char keepMicrostackshotsWithoutLoadInfo;
...
@property char shouldGatherKextStat;
...
@property (readonly) char haveCPUClusterInfo;
...
@property (readonly) char isRootInstalled;
@property (readonly) NSArray *installedRootNames;
@property (readonly) char isAppleInternal;
@property (readonly) char appleInternalIsKnown;
@property (readonly) char isInternalCarryDevice;
@property (readonly) char hasInternalCarryDeviceDistinction;
...
@property (readonly) char hasHardwareKeyboardInfo;
@property (readonly) char hardwareKeyboard;
@property (readonly) unsigned long long gpuRestartCount;
@property (readonly) unsigned long long gpuRestartLastMachAbs;
@property (readonly) char hasEnterprisePersona;
...
+ (char)canOpenFileAsKTraceFile:(const char *)a0 errorOut:(id *)a1;
...
- (unsigned long long)addKCDataStackshots:(id)a0 createSeparateSamplePerStackshot:(char)a1;
...
- (id)binaryFormat;
- (char)cacheLoadInfoForProcess:(int)a0;
...
- (char)isECoreForCPUNum:(unsigned int)a0;
- (char)isPCoreForCPUNum:(unsigned int)a0;
- (char)kPerfPETParsePastLastStackshot;
- (double)kPerfPETSampleIntervalLimit;
- (id)kperfTriggers;
- (char)parseKTraceFile:(const char *)a0 warningsOut:(id)a1 errorOut:(id *)a2;
- (char)parseStackshotsFromKTraceFile:(const char *)a0 warningsOut:(id)a1 errorOut:(id *)a2;
- (void)postprocess;
...
- (void)setHaveKPerfSched:(char)a0;
- (void)setKPerfPETParsePastLastStackshot:(char)a0;
- (void)setKperfTriggers:(id)a0;
- (char)setTargetProcessWithHint:(id)a0;
