...
@interface VMUVMRegion : NSObject {
    ...
    char isExclaveRegion;
    unsigned int exclaveFlags;
}

+ (void)initialize;
+ (id)columnHeadersWithOptions:(unsigned long long)a0 maximumLength:(unsigned long long)a1;
+ (id)columnHeadersWithOptions:(unsigned long long)a0 maximumLength:(unsigned long long)a1 memorySizeDivisor:(unsigned int)a2 hasFractionalPageSizes:(char)a3;
...
- (id)descriptionWithOptions:(unsigned long long)a0 maximumLength:(unsigned long long)a1 memorySizeDivisor:(unsigned int)a2 hasFractionalPageSizes:(char)a3;
...
- (char)hasSameInfoAsRegion:(id)a0;
- (char)ignoreRegionDuringScanning;
...
- (char)isActivityTracing;
- (char)isAnalysisToolRegion;
- (char)isFoundation;
- (char)isJavascriptJitExecutableAllocator;
- (char)isKernelPageTableMemory;
- (char)isMallocHeapRelated;
- (char)isNullRegion;
- (char)isOwnedUnmappedMemory;
- (char)isRootRegion;
- (char)isSanitizer;
- (char)isSpecialPhysFootprintRegion;
- (char)isUnsharedPmap;
- (char)isUntaggedRegion;
