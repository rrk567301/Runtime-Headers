@class NSTextField, LAUILockImageView, NSString, LACUIAuthenticationViewModel, LAContext, NSColor, NSTextView, NSImage, NSLayoutConstraint, LAUIShakeAnimation;
@protocol _LAUIAuthenticationPasswordField, LAUIAuthenticationViewControllerDelegate;

@interface LAUIAuthenticationViewController : NSViewController <LAUIUserPasswordFieldRemoteProtocol, LAUIPasswordFieldDelegate, LACUIAuthenticationViewModelDelegate> {
    LAUILockImageView *_lockImageView;
    NSTextField *_titleLabel;
    NSTextField *_subtitleLabel;
    id<_LAUIAuthenticationPasswordField> _passwordField;
    NSTextView *_hintTextView;
    NSLayoutConstraint *_hintTextViewHeightConstraint;
    NSLayoutConstraint *_lockImageCenterX;
    LACUIAuthenticationViewModel *_viewModel;
    char _touchIDMatch;
    LAUIShakeAnimation *_shakeAnimation;
    LAContext *_lastAuthenticatedContext;
    char _supportsConcurrentEvaluations;
}

@property (nonatomic) unsigned long long enabledMechanisms;
@property (nonatomic) unsigned long long activeMechanisms;
@property (nonatomic) unsigned long long runningMechanisms;
@property (copy, nonatomic) id /* block */ resultHandler;
@property (copy, nonatomic) id /* block */ appPasswordHandler;
@property (retain, nonatomic) NSString *authTitle;
@property (retain, nonatomic) NSString *authHint;
@property (retain, nonatomic) NSString *authDFRPrompt;
@property (retain, nonatomic) NSColor *authTitleColor;
@property (retain, nonatomic) NSColor *authSubTitleColor;
@property (copy, nonatomic) NSString *callerIconPath;
@property (copy, nonatomic) NSString *callerIconBundlePath;
@property (retain, nonatomic) NSImage *customIcon;
@property (nonatomic) struct CGPoint { double x; double y; } badgeOffset;
@property (copy, nonatomic) NSString *localizedCallerName;
@property (readonly, nonatomic) LAContext *context;
@property (nonatomic) char shouldAutoFocusPasswordField;
@property (weak, nonatomic) id<LAUIAuthenticationViewControllerDelegate> delegate;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (void)dealloc;
- (void).cxx_destruct;
- (id)initWithCoder:(id)a0;
- (void)observeValueForKeyPath:(id)a0 ofObject:(id)a1 change:(id)a2 context:(void *)a3;
- (id)initWithNibName:(id)a0 bundle:(id)a1;
- (void)viewDidLayout;
- (void)viewWillAppear;
- (char)isMechanismEnabled:(unsigned long long)a0;
- (void)cancelAuthentication;
- (void)_setupUI:(char)a0;
- (void)_updateUI;
- (void)clearPasswordField;
- (unsigned long long)_filterAvailableMechanismsFrom:(unsigned long long)a0;
- (void)_setupObservers;
- (void)setAuthenticationResultHandler:(id /* block */)a0;
- (id)_attributedPasswordPlaceholderString;
- (id)_getViewModel;
- (void)_handleUINotification:(id)a0;
- (void)_invalidateViewModel;
- (char)_isVisibleToUser;
- (void)_mechanism:(unsigned long long)a0 enable:(char)a1 reply:(id /* block */)a2;
- (long long)_policyForBackgroundMechanisms;
- (void)_restartBackgroundMechanisms;
- (void)_saveLastAuthenticatedContext:(id)a0;
- (void)_setupMechanisms:(unsigned long long)a0;
- (void)_shakeUI:(id)a0;
- (void)_startBackgroundMechanisms;
- (void)_startOrPauseBackgroundMechanisms;
- (void)_stopBackgroundMechanisms;
- (void)_updateHintHeight;
- (void)authenticationViewModel:(id)a0 didUpdateActiveMechanisms:(id)a1;
- (void)authenticationViewModel:(id)a0 succeededWithBiometry:(char)a1;
- (void)authenticationViewModelDidReceiveBiometryMatch:(id)a0;
- (void)authenticationViewModelDidReceiveBiometryNoMatch:(id)a0;
- (char)cmdEnterOrReturnPressed;
- (void)didSubmitWrongPassword:(char)a0;
- (void)didVerifyPassword;
- (void)disableMechanism:(unsigned long long)a0 reply:(id /* block */)a1;
- (void)enableMechanism:(unsigned long long)a0 reply:(id /* block */)a1;
- (void)externalizedContextInReply:(id /* block */)a0;
- (void)focusPasswordField;
- (id)initWithMechanisms:(unsigned long long)a0;
- (char)isMechanismActive:(unsigned long long)a0;
- (char)isMechanismAvailable:(unsigned long long)a0;
- (char)isMechanismRunning:(unsigned long long)a0;
- (char)isPasswordFieldFocused;
- (void)requestCancelAuthentication;
- (void)setAppPasswordAuthenticationHandler:(id /* block */)a0;
- (void)startBackgroundMechanisms;
- (void)submitPasswordField;

@end
