...
@interface NSTextLayoutManager : NSObject <NSTextViewportElementProvider, NSSecureCoding, NSTextSelectionDataSource> {
    ...
    struct { id x0; id x1; double x2; long long x3; long long x4; id x5; char x6; char x7; char x8; } *_activeEnumerationCache;
    char _isProcessingRenderingAttributesMethod;
    _NSTextRunStorage *_textAnimationContextStorage;
    char _delegateWithTextSelectionNotification;
    char _delegateWithTextContainerQuery;
    char _delegateWithSimpleTextContainerQuery;
    char _delegateWithTruncationToken;
    struct { NSTextContainer *textContainer; id<NSTextLocation> startLocation; NSTextRange *textRange; struct CGRect { struct CGPoint { double x; double y; } origin; struct CGSize { double width; double height; } size; } usageBounds; unsigned long long entryIndex; char containerFilled; char anchoredLocation; } _lastTextContainerEntry;
    struct { id x0; id x1; id x2; struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x3; unsigned long long x4; char x5; char x6; } *_textContainerEntries;
    struct { id x0; id x1; id x2; struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x3; unsigned long long x4; char x5; char x6; } *_textContainerEntriesAccessHint;
...
@property (readonly) char lastContainerFilled;
...
@property char rendersBackgroundColorAttribute;
...
@property (readonly) char hasNonSimpleRectangularTextContainer;
...
+ (char)shouldBeUsedForNSTextView;
+ (char)_usesOutlinedHighlightByDefault;
+ (char)usesHyphenation;
+ (void)_setThrowsOnAssertionError:(char)a0;
+ (void)_setValidatesInternalCaches:(char)a0;
...
- (void)enumerateTemporaryAttributesFromLocation:(id)a0 reverse:(char)a1 usingBlock:(id /* block */)a2;
...
- (void)setUsesFontLeading:(char)a0;
...
- (char)usesHyphenation;
- (char)_addTextAnimationContext:(id)a0;
- (void)_addTextContainer:(id)a0;
- (void)_addTextContainerForLocation:(id)a0;
- (char)_adjustedTextLayoutFragmentBoundaryTextRangeForTextRange:(id)a0 location:(out id *)a1 endLocation:(out id *)a2;
- (char)_appendTextContainer;
...
- (void)_enumerateTextSegmentAndCategoryInRange:(id)a0 type:(long long)a1 options:(unsigned long long)a2 allowsZeroWidthExtensions:(char)a3 usingBlock:(id /* block */)a4;
...
- (void)_fillLayoutToLocation:(id)a0 enumerationCache:(struct { id x0; id x1; double x2; long long x3; long long x4; id x5; char x6; char x7; char x8; } *)a1;
- (id)_fillSoftInvalidationToLocation:(id)a0 verticalOffset:(double)a1;
- (char)_hasLayoutForLocation:(id)a0;
- (void)_invalidateLayoutForTextRange:(id)a0 hard:(char)a1;
...
- (char)_setSoftInvalidationLocation:(id)a0 onlyIfUpstream:(char)a1;
- (char)_shouldDrawTextCorrectionMarkerWithType:(long long)a0 inRange:(id)a1;
...
- (char)allowsAddingTextContainer;
...
- (void)enumerateContainerBoundariesFromLocation:(id)a0 reverse:(char)a1 usingBlock:(id /* block */)a2;
...
- (char)flushTextLayoutFragmentsFromLocation:(id)a0 direction:(char)a1;
...
- (void)processLayoutInvalidationForTextRange:(id)a0 synchronizing:(char)a1;
...
- (void)setAllowsFontSubstitutionAffectingVerticalMetrics:(char)a0;
...
- (void)setUsesDefaultHyphenation:(char)a0;
- (void)setUsesHyphenation:(char)a0;
...
- (id)textLayoutFragmentForPosition:(struct CGPoint { double x0; double x1; })a0 inTextContainerAtIndex:(long long)a1 avoidsAdditionalLayout:(char)a2;
- (long long)textLayoutOrientationAtLocation:(id)a0;
- (id)textRangeForSelectionGranularity:(long long)a0 enclosingLocation:(id)a1;
- (void)updateLayoutWithTextLayoutFragment:(id)a0 addition:(char)a1;
