...
@property (readonly) char isAssociatedWithHostWindow;
...
@property char settingFirstResponder;
...
@property char reportedLackOfAccessibilityParent;
@property char hadAccessibilityParent;
@property char hostWindowIsKey;
...
@property (readonly) char remoteViewDidMoveInProgress;
...
+ (char)anyInstanceResumingSuspendedMessages;
+ (id)auxiliaryProxyFor:(const char *)a0 async:(char)a1;
+ (char)beginFreeWindowRendezvous:(id)a0;
+ (void)choseNotToRendezvous:(id)a0 because:(const char *)a1;
+ (char)considerWindowForRendezvous:(id)a0;
...
+ (char)responder:(id)a0 isFieldEditorFor:(id)a1;
...
- (char)dragWindow:(id)a0 relativeToMouseDown:(struct CGPoint { double x0; double x1; })a1;
...
- (void)maintainNotificationsForWindow:(char)a0;
...
- (void)_bootstrap:(const struct { id x0; id x1; id x2; id x3; id x4; double x5; struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x6; unsigned long long x7; unsigned int x8; char x9; char x10; char x11; char x12; id x13; char x14; } *)a0 replyData:(id)a1 completion:(id /* block */)a2;
...
- (void)_willBecomeAssociatedWithHostWindow:(unsigned int)a0 atLevel:(long long)a1 isFunctionRow:(char)a2;
...
- (char)activateTSM:(const char *)a0;
...
- (void)afterResignKey;
- (void)allowAppNap:(char)a0;
- (void)allowKeyboardEvents:(char)a0;
- (char)animateWindow:(id)a0 frame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a1;
...
- (void)beginHostSheet:(id)a0 isCritical:(char)a1 completion:(id /* block */)a2;
- (void)beginTrackingLoop:(id)a0;
- (void)bootstrap:(struct { id x0; id x1; id x2; id x3; id x4; double x5; struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x6; unsigned long long x7; unsigned int x8; char x9; char x10; char x11; char x12; id x13; char x14; })a0 withReply:(id /* block */)a1;
...
- (char)changeWindowFrameFromConstraintsIfNecessary;
...
- (char)deactivateTSM:(const char *)a0;
- (void)defaultResizeBehavior;
- (void)didBecomeAssociatedWithHostWindow:(unsigned int)a0 withKeyness:(struct { char x0; char x1; })a1 isFirstResponder:(char)a2 inActiveApp:(char)a3;
...
- (char)ensureWindow:(id)a0 hasCorrectOrigin:(struct CGPoint { double x0; double x1; })a1;
- (char)ensureWindowHasCorrectOrigin:(id)a0;
- (void)evaluateKeynessOfRemoteViews;
- (char)eventResizingRegionContainsPoint:(struct CGPoint { double x0; double x1; })a0;
...
- (void)forwardPotentialCommandEquivalentToAuxService:(id)a0 eventDeliveredByAppKit:(char)a1 fullDispatch:(char)a2 reply:(id /* block */)a3;
...
- (void)hostWindowBecameKey:(struct { char x0; char x1; })a0 inActiveApp:(char)a1 firstResponderState:(unsigned char)a2 isContentView:(char)a3;
- (void)hostWindowChangedKeyness:(id /* block */)a0;
- (char)hostWindowIsKey:(id)a0;
...
- (void)hostWindowResignedKey:(char)a0;
- (void)hostWindowResignedKey:(struct { char x0; char x1; })a0 focusOnly:(char)a1 inActiveApp:(char)a2 isContentView:(char)a3;
...
- (char)isRunningADocModalAttachedSheet:(id)a0;
- (char)makeFirstResponder:(id)a0 ofWindow:(id)a1;
...
- (void)remoteViewBecameFirstResponder:(unsigned long long)a0 forWindowsWithKeyness:(struct { char x0; char x1; })a1 inActiveApp:(char)a2 inVisibleWindow:(char)a3 reply:(id /* block */)a4;
- (unsigned char)remoteViewBecameFirstResponder:(unsigned long long)a0 forWindowsWithKeyness:(struct { char x0; char x1; })a1 inVisibleWindow:(char)a2;
- (void)remoteViewCaresAboutTouchBars:(char)a0;
...
- (void)remoteViewIsContentView:(char)a0;
...
- (void)remoteViewResignedFirstResponderInActiveApp:(char)a0;
...
- (void)requestFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 transaction:(id)a1 animate:(char)a2 async:(char)a3 completion:(id /* block */)a4;
...
- (char)scrollViewSeparatorTrackingAdapter:(id)a0 qualifiesForWindow:(id)a1;
...
- (char)sendEvent:(id)a0 to:(id)a1;
- (void)sendMinMaxWindowSizesToHostIfNeeded:(id)a0;
- (void)sendMinMaxWindowSizesToHostIfNeeded:(id)a0 now:(char)a1;
...
- (char)setAppearanceOfWindow:(id)a0;
...
- (void)setValidatesSecurityOfSuperlayers:(char)a0 onWindow:(id)a1;
- (void)setViewVibrantBlendingStyle:(unsigned long long)a0;
- (char)setWindow:(id)a0 frame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a1 display:(char)a2 stashSize:(char)a3;
- (void)setWindow:(id)a0 styleMask:(unsigned long long)a1;
- (void)shakeWindow:(id)a0;
- (char)sharesFirstResponderWithAnyChild;
- (char)shouldAllowAnimationForDefaultResizeBehavior;
- (char)shouldDrawSubviewsIntoLayer;
- (void)showTouchBarPopover:(id)a0 fromItem:(id)a1 withOverlayIdentifier:(id)a2 withCloseButton:(char)a3 withControlStrip:(char)a4 withOptions:(id)a5;
...
- (char)validateTargetAndAction:(struct { id x0; SEL x1; id x2; } *)a0 validateMenuItem:(struct { char x0; char x1; } *)a1 validateUserInterfaceItem:(struct { char x0; char x1; } *)a2;
...
- (void)willBecomeAssociatedWithHostWindow:(unsigned int)a0 atLevel:(long long)a1 isFunctionRow:(char)a2;
...
- (void)withHostWindowFrameAnimationInProgress:(char)a0 perform:(id /* block */)a1;
- (void)withResumingSuspendedMessagesInProgress:(id /* block */)a0;
- (void)wrap:(id)a0 swizzledSetFrameCommon:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a1 display:(char)a2 stashSize:(char)a3;
