...
@protocol OS_dispatch_queue, PXAVResourceReclamationController, ISBasePlayerDelegate;
...
@interface ISBasePlayer : ISObservable <ISChangeObserver, ISWrappedAVPlayerDelegate, PXAVResourceReclamationObserver> {
    ...
    struct { char videoPlayer; char videoPlayerVolume; char contentFromPlayerItem; char willPlayToEndObserver; char willPlayToPhotoObserver; char status; } _isValid;
    struct { char wrappedVideoPlayerForPlayer; char playerWillRelinquishVideoPlayer; char videoPlayerForPlayer; } _delegateRespondsTo;
...
@property (readonly, nonatomic) char videoLayersReadyForDisplay;
...
- (char)behaviorIsVideoReadyForDisplay:(id)a0;
...
- (char)behavior:(id)a0 prerollVideoAtRate:(float)a1 completionHandler:(id /* block */)a2;
- (void)behavior:(id)a0 removeTimeObserver:(id)a1;
- (char)behavior:(id)a0 seekVideoPlayerToTime:(struct { long long x0; int x1; unsigned int x2; long long x3; })a1 completionHandler:(id /* block */)a2;
- (char)behavior:(id)a0 seekVideoPlayerToTime:(struct { long long x0; int x1; unsigned int x2; long long x3; })a1 toleranceBefore:(struct { long long x0; int x1; unsigned int x2; long long x3; })a2 toleranceAfter:(struct { long long x0; int x1; unsigned int x2; long long x3; })a3 completionHandler:(id /* block */)a4;
