@class NSUUID, NSString, MTSyncInfo, NSDate, NSSet, MTChannel, NSData, NSOrderedSet, MTInterest, MTCategory, MTAdvancedPlaybackSettings;
...
@property (readonly, nonatomic) char playbackNewestToOldest;
@property (nonatomic) char feedUpdateNeedsRetry;
@property (nonatomic) char hasBeenSynced;
@property (nonatomic) char importing;
@property (nonatomic) char isTransitioningFromImplicit;
@property (nonatomic, getter=isAuthenticatedDark) char authenticatedDark;
@property (readonly, nonatomic, getter=isDark) char dark;
@property (readonly, nonatomic, getter=isPastAutodownloadOrEpisodeLimitDark) char autodownloadOrEpisodeLimitDark;
@property (nonatomic) char needsArtworkUpdate;
@property (nonatomic, readonly) long long syncType;
@property (nonatomic, readonly) char shouldBeDeleted;
@property (nonatomic, readonly) char currentEpisodeLimitAllowsAutomaticDownloads;
@property (nonatomic, readonly) char currentEpisodeLimitIsNextNEpisodes;
@property (nonatomic, readonly) char shouldBeHiddenFromUpNext;
@property (nonatomic) double addedDate;
@property (retain, nonatomic) NSDate *latestExitFromDarkDownloads;
@property (retain, nonatomic) NSString *author;
@property (nonatomic) char autoDownloadEnabled;
...
@property (nonatomic) char notifications;
...
@property (nonatomic) char showPlacardForRemovePlayedEpisodes;
...
@property (nonatomic) char isImplicitlyFollowed;
@property (nonatomic) char isHiddenOrImplicitlyFollowed;
...
@property (readonly, nonatomic) char autoDownload;
@property (nonatomic) int autoDownloadType;
@property (nonatomic) long long keepEpisodes;
@property (nonatomic) char orphanedFromCloud;
@property (nonatomic) char showPlacardForOrphanedFromCloud;
@property (nonatomic) char showPlacardForSavedEpisodes;

+ (long long)defaultShowType;
+ (id)predicateForAutoDownloadEnabled;
+ (id)predicateForHiddenPodcastUuids:(id)a0;
+ (id)predicateForPodcastWithFeedUrl:(id)a0;
+ (id)predicateForPodcastToAutoRemove;
+ (id)_predicateForShowsWithPlayableFreeEpisodes;
+ (char)autoDownloadDefaultValue;
...
+ (id)importedShowArtworkURLFor:(id)a0;
+ (id)insertNewPodcastInManagedObjectContext:(id)a0 subscribed:(char)a1 feedUrl:(id)a2 showType:(long long)a3 showTypeInFeed:(id)a4 title:(id)a5 author:(id)a6 provider:(id)a7 imageUrl:(id)a8 description:(id)a9 adamId:(long long)a10 displayType:(id)a11 showSpecificUpsellCopy:(id)a12;
+ (id)insertNewPodcastInManagedObjectContext:(id)a0 subscribed:(char)a1 feedUrl:(id)a2 showType:(long long)a3 showTypeInFeed:(id)a4 title:(id)a5 author:(id)a6 provider:(id)a7 imageUrl:(id)a8 description:(id)a9 displayType:(id)a10 showSpecificUpsellCopy:(id)a11;
+ (id)insertNewPodcastInManagedObjectContext:(id)a0 subscribed:(char)a1 feedUrl:(id)a2 showType:(long long)a3 title:(id)a4 author:(id)a5 provider:(id)a6 imageUrl:(id)a7 description:(id)a8;
+ (char)isRedirectURL:(id)a0;
+ (id)podcastUuidForEpisodeUuid:(id)a0 ctx:(id)a1;
+ (id)predicateForAreMediaAPI:(char)a0;
+ (id)predicateForEntitledShows;
+ (id)predicateForExcludingExplicitBasedOnSetting;
+ (id)predicateForHasBeenSynced:(char)a0;
...
+ (id)predicateForImporting:(char)a0;
...
+ (id)predicateForPaidSubscriptionActive:(char)a0;
...
+ (id)predicateForPodcastsNeedingRetry:(char)a0;
...
+ (id)sortDescriptorsForNewestToOldest:(char)a0;
+ (id)sortDescriptorsForNewestToOldestFirstTimeAvailable:(char)a0;
+ (char)sortOrderAscForShowType:(long long)a0;
+ (id)storeCleanURLForAdamID:(long long)a0;
...
- (id)updatedAtString;
- (id)_episodeNextToEpisode:(id)a0 after:(char)a1 usePlayOrder:(char)a2 restrictToUserEpisodes:(char)a3 excludePlayed:(char)a4 excludeExplicit:(long long)a5 episodeTypeFilter:(long long)a6;
- (id)_episodesNextTo:(double)a0 after:(char)a1 usePlayOrder:(char)a2 sortAsc:(char)a3 restrictToUserEpisodes:(char)a4 excludePlayed:(char)a5 excludeExplicit:(long long)a6 episodeTypeFilter:(long long)a7 limit:(long long)a8;
- (id)_episodesNextTo:(double)a0 after:(char)a1 usePlayOrder:(char)a2 sortAsc:(char)a3 sortByEpisodeNumber:(char)a4 filter:(id)a5 limit:(long long)a6;
- (id)_episodesNextToEpisode:(id)a0 after:(char)a1 usePlayOrder:(char)a2 restrictToUserEpisodes:(char)a3 excludePlayed:(char)a4 excludeExplicit:(long long)a5 episodeTypeFilter:(long long)a6 limit:(long long)a7;
- (id)_latestOrOldestEpisode:(char)a0 sortDate:(unsigned long long)a1 filter:(id)a2;
- (id)_latestOrOldestEpisode:(char)a0 sortDate:(unsigned long long)a1 restrictToUserEpisodes:(char)a2 excludePlayed:(char)a3 excludeExplicit:(long long)a4;
- (id)_latestOrOldestEpisode:(char)a0 sortDate:(unsigned long long)a1 restrictToUserEpisodes:(char)a2 excludePlayed:(char)a3 excludeExplicit:(long long)a4 episodeTypeFilter:(long long)a5;
- (id)_latestOrOldestEpisode:(char)a0 sortDate:(unsigned long long)a1 restrictToUserEpisodes:(char)a2 excludePlayed:(char)a3 excludeExplicit:(long long)a4 excludingEpisodeUuid:(id)a5;
- (id)_latestOrOldestEpisode:(char)a0 sortDate:(unsigned long long)a1 restrictToUserEpisodes:(char)a2 excludePlayed:(char)a3 excludeExplicit:(long long)a4 excludingEpisodeUuid:(id)a5 episodeTypeFilter:(long long)a6;
- (id)_latestOrOldestEpisode:(char)a0 sortDate:(unsigned long long)a1 restrictToUserEpisodes:(char)a2 playStateFilter:(long long)a3 excludeExplicit:(long long)a4 excludingEpisodeUuid:(id)a5 episodeTypeFilter:(long long)a6;
- (void)applyFollowedShowSettingsFor:(long long)a0;
- (void)applyUnfollowedShowSettings;
- (id)bestAvailableStoreCleanURL;
- (id)bestFeedURLExcludingRedirectURL:(char)a0;
- (void)calculateEpisodicNewEpisodeCountIn:(id)a0;
- (void)calculateNewEpisodeCountIn:(id)a0 serialShowsUseExperimentalRules:(char)a1;
...
- (id)episodesInSeasonNumber:(long long)a0 lowestNumberedEpisodesFirst:(char)a1 filter:(id)a2 limit:(long long)a3;
- (id)episodesInSeasonNumber:(long long)a0 oldestEpisodesFirst:(char)a1;
- (id)episodesInSeasonNumber:(long long)a0 oldestEpisodesFirst:(char)a1 excludePlayed:(char)a2 excludeExplicit:(long long)a3 episodeTypeFilter:(long long)a4 limit:(unsigned long long)a5;
- (id)episodesInSeasonNumber:(long long)a0 oldestEpisodesFirst:(char)a1 excludePlayed:(char)a2 excludeExplicit:(long long)a3 limit:(long long)a4;
- (id)episodesInSeasonNumber:(long long)a0 oldestEpisodesFirst:(char)a1 filter:(id)a2 limit:(long long)a3;
- (id)episodesPublishedAfter:(double)a0 filter:(id)a1 limit:(long long)a2 sortAsc:(char)a3;
...
- (double)playbackRateResolvedValue;
- (char)requestsAreNonAppInitiated;
- (void)resetGoDarkWithReason:(unsigned long long)a0;
- (id)seasonTrailerInSeason:(long long)a0;
- (void)setIsImplicitlyFollowed:(char)a0;
