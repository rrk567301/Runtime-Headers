...
@property (nonatomic) char skipIndexExclusionCheck;
@property (nonatomic) char clientStateFetched;
@property (nonatomic) char enableSpotlightVerification;
...
@property (nonatomic, getter=isIndexingDisabledForPPT) char indexingDisabledForPPT;
...
@property (readonly, getter=isPerformingBlockAffectingDataSourceAndIndex) char performingBlockAffectingDataSourceAndIndex;
@property (getter=isPersistenceAvailable) char persistenceAvailable;
@property (readonly, nonatomic) EFObservable *powerObservable;
@property (readonly, nonatomic, getter=isPluggedIn) char pluggedIn;
@property (readonly, nonatomic) char canIndexAttachments;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
@property (readonly) unsigned long long signpostID;
@property (nonatomic, getter=isDataSourceIndexingPermitted) char dataSourceIndexingPermitted;

+ (id)_localClientState;
+ (id)_localClientStateURL;
+ (id)_reindexReasonString:(long long)a0;
+ (void)_saveLocalClientState:(id)a0;
+ (char)isIncrementalIndexingType:(long long)a0;
+ (char)isValidTransaction:(long long)a0;
...
- (void)_dataSourcePrepareToIndexItems:(id)a0 fromRefresh:(char)a1 withCompletion:(id /* block */)a2;
- (void)_dataSourceScheduleWork:(id /* block */)a0;
- (void)_dataSourceVerifyRepresentativeSampleWithCompletion:(id /* block */)a0;
- (void)_doIndexItems:(id)a0 fromRefresh:(char)a1 immediately:(char)a2;
...
- (void)_indexItems:(id)a0 fromRefresh:(char)a1 immediately:(char)a2;
...
- (char)_processIndexingBatch:(id)a0 clientState:(id)a1 itemsNotIndexed:(id)a2;
...
- (void)_queueRefresh;
- (void)_queueTransitionFromRefresh:(char)a0;
...
- (void)_scheduleProcessPendingItemsFromRefresh:(char)a0;
...
- (void)indexAttachmentsForMessageWithIdentifier:(id)a0 immediately:(char)a1;
- (void)indexItems:(id)a0;
- (void)indexItems:(id)a0 fromRefresh:(char)a1 immediately:(char)a2;
- (void)indexItems:(id)a0 immediately:(char)a1;
...
- (void)removeItemsWithIdentifiers:(id)a0 reasons:(id)a1 fromRefresh:(char)a2;
