...
@interface EQKitMathMLParser : NSObject {
    ...
    struct basic_string<wchar_t, std::char_traits<wchar_t>, std::allocator<wchar_t>> { struct __compressed_pair<std::basic_string<wchar_t>::__rep, std::allocator<wchar_t>> { union __rep { struct __short { struct { unsigned char __is_long_ : 1; unsigned char __size_ : 7; } ; char __padding_[3]; int __data_[5]; } __s; struct __long { struct { unsigned char __is_long_ : 1; unsigned long __cap_ : 63; } ; unsigned long long __size_; int *__data_; } __l; } __value_; } __r_; } mAttribution;
}
...
- (char)isElement:(int)a0 allowedInState:(int)a1;
