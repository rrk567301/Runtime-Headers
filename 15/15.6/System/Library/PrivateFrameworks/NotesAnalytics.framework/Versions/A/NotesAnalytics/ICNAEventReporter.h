@class NSMutableDictionary, NSString, NSMutableSet, ICNASubTracker, NSWindow;
...
+ (char)activityTypeHasUnknownShareFlow:(id)a0;
+ (id)analyticsTypeUTIAttachment:(id)a0;
+ (char)attachmentHasDeepLinkSafariHighlight:(id)a0;
+ (id)filteredActivityType:(id)a0;
+ (id)inlineAttachmentReportForModernNote:(id)a0 faultOutInlineAttachmentsAfterDone:(char)a1;
...
- (id)initForAudioWithSubTrackerName:(id)a0;
...
- (id)audioAttachmentDataForModernAttachment:(id)a0;
- (id)audioRecordingDataForModernAttachment:(id)a0 appBackgroundOccurred:(char)a1 noteMultitaskingOccurred:(char)a2 audioAttachmentDuration:(id)a3;
- (id)audioTranscriptInteractionDataWithSummaryViewed:(char)a0 transcriptViewed:(char)a1 interactionTargets:(id)a2 interactionTypes:(id)a3;
- (id)bioAuthDataForModernNote:(id)a0;
- (id)collaborationActionDataIsInviting:(char)a0 inviteStep:(long long)a1;
...
- (id)folderViewEventDataObjectsForNoteContainer:(id)a0 isInGridMode:(char)a1;
- (id)folderViewEventDataObjectsForVirtualFolder:(id)a0 isInGridMode:(char)a1 context:(id)a2;
- (id)initWithSubTrackerName:(id)a0;
- (id)initWithSubTrackerName:(id)a0 view:(id)a1;
- (id)initWithSubTrackerName:(id)a0 window:(id)a1;
- (id)layoutDataForIsInGridMode:(char)a0 isTimelineView:(char)a1;
- (long long)linkContentTypeEnumForURL:(id)a0;
- (id)noteAccessDataForHTMLNote;
- (id)noteAccessDataForModernNote:(id)a0;
- (id)noteCollapsibleSectionDataForHTMLNote:(id)a0;
...
- (void)startFolderViewEventDurationTrackingForNoteContainer:(id)a0 isInGridMode:(char)a1;
- (void)startFolderViewEventDurationTrackingForVirtualFolder:(id)a0 isInGridMode:(char)a1 context:(id)a2;
- (void)startNoteViewEventDurationTrackingForHTMLNote:(id)a0;
- (void)startNoteViewEventDurationTrackingForModernNote:(id)a0 reportCoreAnalytics:(char)a1;
...
- (void)submitAddLinkEvent:(id)a0 addApproach:(long long)a1 contentType:(long long)a2 isTokenized:(char)a3;
- (void)submitAddLinkEvent:(id)a0 addApproach:(long long)a1 contentType:(long long)a2 isTokenized:(char)a3 noteData:(id *)a4 noteContentData:(id *)a5 noteAccessData:(id *)a6;
...
- (void)submitAudioRecordingEventWithAttachment:(id)a0 appBackgroundOccurred:(char)a1 noteMultitaskingOccurred:(char)a2 audioAttachmentDuration:(id)a3;
- (void)submitAudioTranscriptInteractionEventSummaryViewed:(char)a0 transcriptViewed:(char)a1 interactionTargets:(id)a2 interactionTypes:(id)a3 audioAttachment:(id)a4;
- (void)submitBiometricsFailEventForModernNote:(id)a0;
- (void)submitChangePasswordEventForModernAccount:(id)a0 isReset:(char)a1;
...
- (void)submitCollabActivityBrowseEventForObject:(id)a0 contextPath:(long long)a1 clickContext:(long long)a2 mode:(long long)a3 isCurrentUserMentioned:(char)a4 hasRecentUpdates:(char)a5;
- (void)submitCollabNotificationEventWithAction:(long long)a0;
- (void)submitCollabSummaryEventForObject:(id)a0 withUpdates:(char)a1;
- (void)submitCollaborationActionEventForCloudSyncingObject:(id)a0 share:(id)a1 isInviting:(char)a2 inviteStep:(long long)a3;
- (void)submitDocScanEventForNote:(id)a0 pageCount:(unsigned long long)a1;
- (void)submitEverNoteImportEventWithCount:(unsigned long long)a0 isSuccessful:(char)a1 intoAccount:(id)a2;
...
- (void)submitFindInNoteEventWasSearchQueryEntered:(char)a0 htmlNote:(id)a1;
- (void)submitFindInNoteEventWasSearchQueryEntered:(char)a0 modernNote:(id)a1;
- (void)submitFindInTranscriptEventWasSearchQueryEntered:(char)a0 audioAttachment:(id)a1;
...
- (void)submitFolderViewEventForNoteContainer:(id)a0 isInGridMode:(char)a1;
- (void)submitFolderViewEventForVirtualFolder:(id)a0 isInGridMode:(char)a1 context:(id)a2;
- (void)submitFullscreenDrawingToInlineDrawingEventForNote:(id)a0 drawing:(id)a1 drawingID:(id)a2 method:(long long)a3;
- (void)submitHashtagAddEventForNote:(id)a0 tokenContentIdentifier:(id)a1 viaAutoComplete:(char)a2;
...
- (void)submitMentionAddEventForNote:(id)a0 mentionID:(id)a1 participantID:(id)a2 viaAutoComplete:(char)a3;
- (void)submitMentionDeleteEvent:(id)a0;
- (void)submitMoveCheckedItemsToBottomSwitchEventWithNewState:(char)a0 isInApp:(char)a1;
- (void)submitNoteActionMenuEventForNoteBrowserWithUsageType:(long long)a0 layout:(long long)a1 activityType:(id)a2 isTimelineView:(char)a3;
...
- (void)submitNoteEditEventForModernNote:(id)a0 fromQuickNote:(char)a1;
...
- (void)submitNoteSharrowEventForModernNote:(id)a0 activityType:(id)a1 collaborationSelected:(char)a2 countOfCollaboratorsAdded:(long long)a3 countOfCollaboratorsRemoved:(long long)a4 startInvitedCount:(long long)a5 startAcceptedCount:(long long)a6 endInvitedCount:(long long)a7 endAcceptedCount:(long long)a8;
- (void)submitNoteViewEventForModernNote:(id)a0 referringNote:(id)a1 collapsibleSectionAffordanceExposures:(long long)a2 collapsibleSectionAffordanceUsages:(long long)a3;
- (void)submitNotesToPagesEventWithContextPath:(long long)a0;
- (void)submitOnboardingScreenViewEventWithType:(long long)a0;
...
- (void)submitSnapshotCompletionEventIsSuccessful:(char)a0;
- (void)submitSnapshotRegisterEventIsSuccessful:(char)a0;
...
- (void)submitTagBrowserSelectionEventWithSelectedTagCount:(unsigned long long)a0 isAllTagsSelected:(char)a1;
