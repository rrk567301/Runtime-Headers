...
@property (nonatomic) char fromMicrosoft;
@property (nonatomic) char isMalformed;
...
@property (readonly, nonatomic) char hasDataAvailable;
@property (readonly, copy, nonatomic) NSData *decodedData;
@property (copy, nonatomic) NSString *type;
@property (copy, nonatomic) NSString *subtype;
@property (readonly, nonatomic) char isDirectoryAttachment;
...
@property (nonatomic) char skipSignatureVerification;
...
@property (readonly, nonatomic) char isCMSPart;
...
+ (id)_encryptedDataForContentData:(id)a0 senderCertificate:(struct __SecCertificate { } *)a1 senderCapabilities:(id)a2 recipients:(id)a3 isAuthenticated:(char *)a4 error:(id *)a5;
...
- (void)getNumberOfAttachments:(unsigned int *)a0 metadata:(id)a1 numberOfTNEFAttachments:(unsigned int *)a2 isSigned:(char *)a3 isEncrypted:(char *)a4;
...
- (id)_cmsExtractedContentIsEncrypted:(char *)a0 isSigned:(char *)a1 error:(id *)a2;
...
- (id)_decodeEncodedData:(id)a0 detachedContentData:(id)a1 isEncrypted:(char *)a2 isSigned:(char *)a3 encryptionError:(id *)a4 signingError:(id *)a5;
...
- (id)_errorForEncryption:(char)a0 underlyingError:(id)a1;
- (id)_extractCMSContentIfNecessaryIsEncrypted:(char *)a0 isSigned:(char *)a1;
...
- (id)_getAttachment;
- (void)_getAttachmentsAndAddToCount:(unsigned int *)a0 isSigned:(char *)a1 isEncrypted:(char *)a2 attachments:(id)a3 metadata:(id)a4 numberOfTNEFAttachments:(unsigned int *)a5;
...
- (char)_inferIsEncrypted;
- (char)_isMessageRFC822;
- (char)_isMultipartSigned;
- (char)_isTypeCode:(long long)a0 subtypeCode:(long long)a1;
- (void)_mergeWithSecurityInformation:(id)a0;
- (id)_messageBodyForContentPart:(id)a0 expectEncryptedSubject:(char)a1 outSubject:(id *)a2 outError:(id *)a3;
- (id)_messageBodyFromDecodedContents:(id)a0;
- (char)_needsSignatureVerification:(id *)a0;
- (struct _CMSDecoder { } *)_newCMSDecoderWithMimePart:(id)a0 error:(id *)a1;
- (id)_nonMultipartPart;
- (char)_parseHeadersWithEncodingHint:(unsigned long long)a0 hasVisualEncoding:(char *)a1;
- (char)_parseHeadersWithEncodingHint:(unsigned long long)a0 hasVisualEncoding:(char *)a1 depth:(unsigned long long)a2;
- (char)_parseSubpartsWithEncodingHint:(unsigned long long)a0 hasVisualEncoding:(char *)a1 depth:(unsigned long long)a2;
- (id)_parseUUEncodedPartsWithEncodingHint:(unsigned long long)a0;
- (id)_partThatIsAttachment;
- (id)_remoteAttachmentsByURL;
- (void)_setCMSExtractedContent:(id)a0 isEncrypted:(char)a1 isSigned:(char)a2 error:(id)a3;
...
- (char)needToIncludeEncryptionCertificteToSignedMessageForRawSender:(id)a0 identity:(struct __SecIdentity { } *)a1;
