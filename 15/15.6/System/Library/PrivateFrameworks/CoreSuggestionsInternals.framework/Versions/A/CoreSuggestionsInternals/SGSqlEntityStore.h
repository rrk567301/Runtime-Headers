...
@interface SGSqlEntityStore : NSObject <CSSearchableIndexDelegate> {
    ...
    char _entityMigrationRequiresMergeBlobUpdate;
...
@property (readonly, nonatomic) char isEphemeral;
@property (readonly, nonatomic) char waitForMigrations;
...
+ (id)_initializeDatabaseNolock:(id)a0 withProtection:(char)a1 sharedLock:(id)a2 newDatabaseCreated:(char *)a3;
+ (id)_recreateCorruptDatabase:(id)a0 sharedLock:(id)a1 withProtection:(char)a2;
+ (void)addPathToFutureDbSchemas:(id)a0;
+ (char)canStoreEventsForAccountType:(id)a0;
+ (id)cnContactIdentifiersSpotlightQueryStringForContact:(id)a0;
+ (char)contactContainsEphemeralId:(id)a0;
+ (id)defaultHarvestStore;
+ (id)defaultPathCreatingSuggestionsDirectoryIfNeeded:(char)a0;
+ (id)defaultSnippetPath;
+ (void)deletePreparedSqlStoreInMemory;
+ (id)escapeLike:(id)a0;
+ (id)globPrefix:(id)a0;
+ (id)initializeDatabase:(id)a0 withProtection:(char)a1 sharedLock:(id)a2 newDatabaseCreated:(char *)a3;
+ (char)isEphemeralIdentifier:(long long)a0;
+ (id)journalNameForDbPath:(id)a0;
+ (id)likePrefix:(id)a0;
+ (char)pathHasFutureDbSchema:(id)a0;
...
+ (id)sqlStoreInMemory;
+ (id)sqlStoreWithEntityDbPath:(id)a0 snippetDbPath:(id)a1 isEphemeral:(char)a2 executeJournals:(char)a3;
+ (id)sqlStoreWithEntityDbPath:(id)a0 snippetDbPath:(id)a1 isEphemeral:(char)a2 sharedLock:(id)a3 executeJournals:(char)a4;
+ (id)sqlStoreWithEntityDbPathNoMigrate:(id)a0 snippetDbPath:(id)a1 isEphemeral:(char)a2 executeJournals:(char)a3;
+ (id)storeWithDefaultLocationAndExecuteJournals:(char)a0;
...
- (id)suggestContactsByMasterEntityQuery:(id)a0 withSnippets:(char)a1 limit:(unsigned long long)a2 bindings:(id /* block */)a3;
- (long long)commitSaliency:(id)a0;
- (char)confirmContactByRecordID:(id)a0 error:(id *)a1;
- (void)deleteInteractionsWithBundleId:(id)a0;
- (char)deleteURLsFromBundleIdentifier:(id)a0;
...
- (char)_analyzeMergeBlobsIncremental:(id)a0;
- (void)_clearPendingJobs;
- (id)_computeBlobsRaw:(id *)a0;
- (id)_computePseudoContactContentHash:(id)a0 contactDetails:(id)a1;
- (id)_contactForMasterEntityId:(id)a0 withSnippets:(char)a1 filterConfirmRejectDetails:(char)a2;
...
- (id)_contactsForMasterEntityIds:(id)a0 withSnippets:(char)a1;
- (void)_contentWriteEnrichmentToDb:(id)a0;
- (char)_createEntityFTSTableWithName:(id)a0;
...
- (void)_deleteIfNoSpotlightReferences:(id)a0 addToRecentlyDeletedCache:(char)a1;
...
- (void)_enumerateContactsForMasterEntityIds:(id)a0 withSnippets:(char)a1 filterConfirmRejectDetails:(char)a2 usingBlock:(id /* block */)a3;
- (void)_enumerateContactsForMasterEntityIds:(id)a0 withSnippets:(char)a1 usingBlock:(id /* block */)a2;
...
- (char)_isBlobAnalysisQueueEmpty;
- (char)_isIdentityStoreDirty;
- (char)_isStringInNoContactsCache:(id)a0;
...
- (char)_prepareToCopyFrom:(id)a0 toNewTableNamed:(id)a1;
- (char)_processBatchOfSerializedContactJobs;
- (struct _PASDBIterAction_ { char x0; })_processContactMatchWithContacts:(id)a0 matches:(id)a1 limit:(long long)a2 masterEntityId:(id)a3 detailEntityId:(id)a4 contactEntityId:(id)a5 matchinfoData:(id)a6 tokens:(id)a7 requireName:(char)a8;
...
- (char)_renameTable:(id)a0 to:(id)a1;
- (void)_reportDbStatsBackground;
- (char)_runInitMigrationTasks:(id)a0 noMigrate:(char)a1;
...
- (char)_shouldContinueReindex:(int)a0;
- (void)_slowCopyFromTable:(id)a0 toTable:(id)a1 startingAtEntityId:(unsigned long long)a2;
- (char)_tombstoneExistsForBundleIdentifier:(id)a0;
- (void)_trimSerializedContacts;
- (id)_uniqueGoodDetailMatchFrom:(id)a0 toDetails:(id)a1 nonUnique:(char *)a2;
- (struct _PASDBIterAction_ { char x0; })_updateURLs:(id)a0 stmt:(id)a1;
...
- (char)addSpotlightReferenceToEmailIfFullyDownloaded:(id)a0 withBundleIdentifier:(id)a1 uniqueIdentifier:(id)a2 domainIdentifier:(id)a3;
...
- (void)closeDatabases;
- (id)cnContactMatchesForRecordId:(id)a0 found:(char *)a1;
...
- (char)confirm:(char)a0 orRejectContactByRecordID:(id)a1 error:(id *)a2;
- (char)confirmContactDetailByRecordID:(id)a0 error:(id *)a1;
- (void)confirmFieldValues:(id)a0 forEntity:(id)a1;
- (char)confirmRealtimeContact:(id)a0 error:(id *)a1;
- (char)confirmRealtimeEvent:(id)a0 error:(id *)a1;
- (char)confirmStoredEvent:(id)a0 error:(id *)a1;
- (char)confirmStoredReminder:(id)a0 error:(id *)a1;
...
- (char)contactIsDisplayable:(id)a0;
- (char)contactIsSignificant:(id)a0;
- (char)contactIsSignificant:(id)a0 usingCoreRecents:(char)a1;
- (char)contactIsSignificantOrSignificanceIsDisabled:(id)a0;
- (char)contactIsSignificantOrSignificanceIsDisabled:(id)a0 usingCoreRecents:(char)a1;
...
- (char)corruptionMarkerPresent;
- (id)curatedRecordIds;
- (char)databasecheck_BrokenEntityIDReferences;
- (char)databasecheck_IntegrityCheck;
- (char)databasecheck_contactMergeGroupConsistency;
...
- (void)deleteEntitiesByDuplicateKey:(id)a0 emitChangeNotifications:(char)a1;
- (void)deleteEntitiesByDuplicateKey:(id)a0 preserveEventConfirmationHistory:(char)a1 emitChangeNotifications:(char)a2;
...
- (void)deleteItemsWithEntityTag:(id)a0 preserveEventConfirmationHistory:(char)a1;
...
- (char)deleteURLsFromBundleIdentifier:(id)a0 documentIdentifiers:(id)a1;
- (char)deleteURLsFromBundleIdentifier:(id)a0 groupIdentifierGlobs:(id)a1;
- (char)deleteURLsOlderThanDate:(id)a0 deletionCount:(long long *)a1;
...
- (id)entityFromSqlResult:(struct sqlite3_stmt { } *)a0 withSnippets:(char)a1;
...
- (void)enumerateAllContactsWithExtractionStartDate:(id)a0 extractionEndDate:(id)a1 withSnippets:(char)a2 limitTo:(unsigned long long)a3 usingBlock:(id /* block */)a4;
- (void)enumerateAllContactsWithSnippets:(char)a0 limitTo:(unsigned long long)a1 usingBlock:(id /* block */)a2;
- (void)enumerateContactsByMasterEntityQuery:(id)a0 withSnippets:(char)a1 limit:(unsigned long long)a2 bindings:(id /* block */)a3 usingBlock:(id /* block */)a4;
- (id)eventFromSqlResult:(struct sqlite3_stmt { } *)a0;
- (char)eventIsCancelled:(id)a0;
...
- (id)initWithEntityDbPath:(id)a0 snippetDbPath:(id)a1 isEphemeral:(char)a2 sharedLock:(id)a3 executeJournals:(char)a4 noMigrate:(char)a5;
- (long long)insertEvent:(id)a0;
- (void)insertEventTagsAndLocations:(id)a0 primaryKey:(long long)a1;
- (char)isRecentlyDeleted:(id)a0;
...
- (id)loadContactForStorageContact:(id)a0 usingSerializedContactCache:(char)a1;
...
- (char)mayHaveSentMessageToEmail:(id)a0;
- (char)mayHaveSentMessageToHandle:(id)a0;
...
- (char)performDatabaseCheck;
- (char)performIntegrityCheckOnly;
...
- (void)prepMigrator;
- (char)processContactEnrichment:(id)a0 recordId:(id)a1;
- (char)processPseudoContactEntity:(id)a0 recordId:(id)a1;
...
- (char)realtimeContactIsSignificant:(id)a0;
- (char)realtimeContactIsSignificantOrSignificanceIsDisabled:(id)a0;
- (void)rebuildContactDetailsCacheWithShouldContinueBlock:(id /* block */)a0;
- (char)rebuildIdentityTables;
- (id)recentURLsWithLimit:(long long)a0;
- (char)recentsContainSomeOfEmails:(id)a0 phoneNumbers:(id)a1 instantMessageAddresses:(id)a2;
- (void)refreshSuggestionsContact:(id)a0;
- (char)registerAndLinkIdentity:(id)a0 recordId:(id)a1 phones:(id)a2 socialProfiles:(id)a3 email:(id)a4 curated:(char)a5;
- (char)registerAndLinkIdentity:(id)a0 recordId:(id)a1 phones:(id)a2 socialProfiles:(id)a3 email:(id)a4 curated:(char)a5 isSent:(char)a6;
...
- (char)registerURLFeedback:(unsigned char)a0 absoluteURL:(id)a1;
- (id)reimportRequestDBStats;
- (char)reimportRequestsContainBundleIdentifier:(id)a0 uniqueIdentifier:(id)a1;
...
- (char)rejectContactByRecordID:(id)a0 error:(id *)a1;
- (char)rejectContactDetailByRecordID:(id)a0 error:(id *)a1;
- (id)rejectFieldValuesForRecordId:(id)a0;
- (char)rejectRealtimeContact:(id)a0 error:(id *)a1;
- (char)rejectRealtimeEvent:(id)a0 error:(id *)a1;
- (char)rejectReminderByKey:(id)a0 error:(id *)a1;
- (char)rejectStoredEvent:(id)a0 error:(id *)a1;
- (char)rejectStoredReminder:(id)a0 error:(id *)a1;
- (char)reminderAlarmTriggeredForStorageReminder:(id)a0 error:(id *)a1;
...
- (id)sortedSaliencyResultsRestrictedToMailboxTypes:(id)a0 mailboxIds:(id)a1 receivedOnOrAfter:(id)a2 ascending:(char)a3 limit:(unsigned long long)a4;
...
- (id)storageContactByMasterEntityId:(long long)a0 withSnippets:(char)a1;
- (id)storageContactByRecordId:(id)a0 withSnippets:(char)a1;
...
- (id)suggestContactByRecordId:(id)a0 withSnippets:(char)a1 filterConfirmRejectDetails:(char)a2;
...
- (id)suggestContactMatchesByEmailAddress:(id)a0 isMaybe:(char)a1 onlySignificant:(char)a2;
- (id)suggestContactMatchesByPhoneNumber:(id)a0;
- (id)suggestContactMatchesByPhoneNumber:(id)a0 isMaybe:(char)a1 onlySignificant:(char)a2;
- (id)suggestContactMatchesBySocialProfile:(id)a0;
- (id)suggestContactMatchesBySocialProfile:(id)a0 isMaybe:(char)a1 onlySignificant:(char)a2;
- (id)suggestContactMatchesByType:(long long)a0 searchString:(id)a1 isMaybe:(char)a2 onlySignificant:(char)a3;
...
- (id)suggestContactsWithContact:(id)a0 withSnippets:(char)a1 filterConfirmRejectDetails:(char)a2 limitTo:(unsigned long long)a3;
...
- (char)syncNewBlobsIfChanged;
- (char)tombstoneExistsForSpotlightReferenceWithBundleIdentifier:(id)a0 uniqueIdentifier:(id)a1 domainIdentifier:(id)a2;
- (id)topSalienciesForMailboxId:(id)a0 limit:(long long)a1;
- (char)trimURLTableDownToLimit:(long long)a0 deletionCount:(long long *)a1;
- (char)trimURLs;
...
- (void)writeIdentityBlobs:(id)a0 forceRewrite:(char)a1;
