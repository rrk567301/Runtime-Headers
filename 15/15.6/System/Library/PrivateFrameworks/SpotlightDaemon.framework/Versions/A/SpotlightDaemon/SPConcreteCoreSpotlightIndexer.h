...
@property (nonatomic) char onDemandOpen;
...
@property (nonatomic) char finishedDrainingJournal;
...
+ (char)_spellingCorrectionConditional:(int)a0 isSingleToken:(BOOL)a1 isPreviousTokenCorrected:(BOOL)a2 isLastToken:(BOOL)a3 tokenLength:(unsigned long long)a4;
...
+ (char)dumpCrashStates:(const char *)a0 toFile:(id)a1;
+ (id)fetchItemForURL:(id)a0;
+ (id)fetchParentsForItemID:(id)a0 recursively:(char)a1 timeout:(unsigned long long)a2;
...
- (int)openIndexForUpgradeSynchronous:(char)a0;
...
- (void)_expireCorruptIndexFilesWithPath:(id)a0 keepLatest:(char)a1;
- (void)_fetchAccumulatedStorageSizeForBundleId:(id)a0 completionHandler:(id /* block */)a1;
- (char)_hasPurgeableTouchFile;
...
- (char)_removePurgeableTouchFile;
- (void)_saveCorruptIndexWithPath:(id)a0 shouldSendABC:(char)a1 fullyCreated:(char)a2 markedPurgeable:(char)a3;
- (void)_scheduleStringsCleanupForBundleID:(id)a0;
- (void)_sendIndexDropABCEvent:(char)a0 markedPurgeable:(char)a1;
- (char)_shouldNotifyForSearchableItemUpdates:(id)a0;
- (char)_startInternalQueryWithIndex:(struct __SI { } *)a0 query:(id)a1 fetchAttributes:(id)a2 forBundleIds:(id)a3 maxCount:(unsigned long long)a4 resultsHandler:(id /* block */)a5;
- (char)_startInternalQueryWithIndex:(struct __SI { } *)a0 query:(id)a1 fetchAttributes:(id)a2 forBundleIds:(id)a3 maxCount:(unsigned long long)a4 resultsHandler:(id /* block */)a5 resultQueue:(id)a6;
- (char)_startInternalQueryWithIndex:(struct __SI { } *)a0 query:(id)a1 fetchAttributes:(id)a2 forBundleIds:(id)a3 maxCount:(unsigned long long)a4 resultsHandler:(id /* block */)a5 resultQueue:(id)a6 postFilter:(id)a7;
- (char)_startInternalQueryWithIndex:(struct __SI { } *)a0 query:(id)a1 fetchAttributes:(id)a2 forBundleIds:(id)a3 maxCount:(unsigned long long)a4 resultsHandler:(id /* block */)a5 resultQueue:(id)a6 postFilter:(id)a7 clientBundleID:(id)a8;
- (char)_startInternalQueryWithIndex:(struct __SI { } *)a0 query:(id)a1 fetchAttributes:(id)a2 forBundleIds:(id)a3 resultsHandler:(id /* block */)a4;
- (char)_startInternalQueryWithIndex:(struct __SI { } *)a0 query:(id)a1 fetchAttributes:(id)a2 forBundleIds:(id)a3 resultsHandler:(id)a4 resultQueue:(id)a5;
- (char)_startInternalQueryWithIndex:(struct __SI { } *)a0 query:(id)a1 fetchAttributes:(id)a2 resultsHandler:(id /* block */)a3;
...
- (void)deleteAllSearchableItemsForBundleID:(id)a0 fromClient:(id)a1 shouldGC:(char)a2 completionHandler:(id /* block */)a3;
...
- (char)denyOperationOnAssertedIndex:(char *)a0;
- (void)dirty:(char)a0;
...
- (void)fetchMeCard:(char)a0 isNotCreateNewIndex:(char)a1;
...
- (void)indexFromBundle:(id)a0 personaID:(id)a1 options:(long long)a2 items:(id)a3 itemsText:(id)a4 itemsHTML:(id)a5 clientState:(id)a6 expectedClientState:(id)a7 clientStateName:(id)a8 deletes:(id)a9 canCreateNewIndex:(char)a10 completionHandler:(id /* block */)a11;
- (id)indexLossAnalyticsDictWithPreviousIndexCreationDate:(long long)a0 size:(long long)a1 openingInReadOnly:(char)a2 fullyCreated:(char)a3 markedPurgeable:(char)a4 vectorIndexDrop:(id)a5 forAnalytics:(char)a6;
...
- (void)issuePhotosReindexIfNeeded:(char)a0;
- (char)issuePriorityIndexFixup;
- (void)issuePriorityIndexFixupOff;
- (void)issuePriorityIndexFixupOn;
- (void)issueRepair;
- (void)issueSplit;
- (void)markDirectoryAtomicallyPurgeable:(id)a0 purgeableOrNot:(char)a1;
...
- (int)openIndex:(char)a0;
- (int)openIndex:(char)a0 shouldReindexAll:(char)a1;
- (int)openIndex:(char)a0 shouldReindexAll:(char)a1 readOnly:(char)a2;
- (int)openIndex:(char)a0 shouldReindexAll:(char)a1 readOnly:(char)a2 forcePC:(id)a3;
- (int)openJWLIndex;
...
- (void)processImportForBundleID:(id)a0 withURLs:(id)a1 contentTypes:(id)a2 sandboxExtensions:(id)a3 andIdentifiers:(id)a4 options:(long long)a5 inGroup:(id)a6 additionalAttributes:(id)a7 computeUpdaterAttributesAfterImport:(char)a8;
- (id)purgeableIndexTouchFilePath;
- (void)readyIndex:(char)a0;
- (char)reindexAllStarted;
- (void)reindexAttributes:(id)a0 ofItemsMatchingQuery:(id)a1 indexAttrName:(id)a2 withVersion:(unsigned long long)a3 perItemCompletionAttribute:(id)a4 completionValue:(char)a5 alwaysReindexWithCompletionAttribute:(BOOL)a6 force:(BOOL)a7 postFilter:(id)a8;
...
- (void)setProperty:(id)a0 forKey:(id)a1 sync:(char)a2;
- (int)shouldNotLogIndexDrop:(id)a0 ignoreParentDirectoryAge:(char)a1;
...
- (char)unpurgeOnceTouchFileExists;
- (void)updateContainersAndScores:(char)a0;
- (void)updateDerivedFromToIsMeNameAdded:(char)a0;
- (void)updateDerivedFromToIsMeNameUpdated:(char)a0;
- (void)updateDerivedFromToMeEmailAdded:(char)a0;
- (void)updateDerivedFromToMeEmailUpdated:(char)a0;
- (void)updateDerivedIsFromToMe:(char)a0;
- (void)updateDerivedIsMe:(char)a0;
- (void)updateDerivedIsMe:(char)a0 nameTokens:(id)a1 alias:(id)a2 onlyIfNotAlready:(BOOL)a3;
- (void)updateDerivedIsMeIfNotAlready:(char)a0;
- (void)updateDerivedIsMeNot:(char)a0 nameTokens:(id)a1 alias:(id)a2;
- (void)updateDerivedIsMeTextContentMatch:(char)a0 nameTokens:(id)a1 alias:(id)a2 onlyIfNotAlready:(BOOL)a3;
- (void)updateDerivedIsMeTextContentMatchNot:(char)a0 nameTokens:(id)a1 alias:(id)a2;
- (void)updateEmailContentURLAttr:(char)a0;
- (void)updateEmailLocalParts:(char)a0;
- (void)updateIndexRankingDates:(char)a0;
- (void)updateKnownBundles:(id)a0 group:(id)a1;
- (void)updateMeCardInfo:(id)a0 middleName:(id)a1 familyName:(id)a2 emailAddresses:(id)a3 isFirstTimeCheck:(char)a4 isNotCreateNewIndex:(char)a5;
- (void)updateNotes:(char)a0;
- (void)updateRankingDates:(char)a0;
- (void)validateConcreteIndexer:(char)a0 outFileDescriptor:(int)a1;
...
- (char)writeDiagnostic:(id)a0 bundleID:(id)a1 identifier:(id)a2;
- (void)writeIndexCreationDate:(long long)a0;
- (void)writeIndexDropAnalyticsDate:(long long)a0;
- (void)writeIndexLossEventToFile:(id)a0 vector:(char)a1;
