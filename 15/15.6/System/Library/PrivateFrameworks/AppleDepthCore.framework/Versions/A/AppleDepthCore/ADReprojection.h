@interface ADReprojection : NSObject {
    ...
    char _singlePassOptimizationUsed;
    struct __CVBuffer { } *_warpMap;
}

@property (nonatomic) char singlePassReprojectionAllowed;
@property (nonatomic) char fullPassReprojectionAllowed;
...
- (char)updateCapacity:(unsigned long long)a0;
...
- (long long)warpDepth:(struct __CVBuffer { } *)a0 srcCalibration:(id)a1 dstCalibration:(id)a2 poseTransform:(struct { void /* unknown type, empty encoding */ x0[4]; })a3 warpedDepth:(struct __CVBuffer { } *)a4;
...
- (char)isCalibrationUndistorted:(id)a0;
...
- (long long)updateWarpMapWithDepth:(struct __CVBuffer { } *)a0 srcCalibration:(id)a1 dstCalibration:(id)a2 sourceToDestinationTransform:(struct { void /* unknown type, empty encoding */ x0[4]; })a3 warpedDepth:(struct __CVBuffer { } *)a4 warpMap:(struct __CVBuffer { } *)a5;
