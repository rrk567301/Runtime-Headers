...
@interface SymptomExpertSystemHandler : ExpertSystemHandlerCore <ProbeManagerDelegate, CLIPSShimDelegate, SymptomsFileCleanerDelegate> {
    ...
    struct { char *relayName; char symptomLike; char scalarValues; } wifiRelayContext;
    struct { char *relayName; char symptomLike; char scalarValues; } wifiScalarRelayContext;
    struct { char *relayName; char symptomLike; char scalarValues; } cellRelayContext;
    struct { char *relayName; char symptomLike; char scalarValues; } cellScalarRelayContext;
    struct { char *relayName; char symptomLike; char scalarValues; } systemRelayContext;
    struct { char *relayName; char symptomLike; char scalarValues; } systemScalarRelayContext;
    struct { char *relayName; char symptomLike; char scalarValues; } systemSymptomRelayContext;
    struct { char *relayName; char symptomLike; char scalarValues; } hardwareRelayContext;
    NSString *_airdropID;
...
@property (nonatomic) char carrierSeedBuild;
@property (nonatomic) char internalOrCarrierSeedBuild;
@property (nonatomic) char customerSeedBuild;
...
+ (char)parseEndpoint:(id)a0 intoURL:(id *)a1 host:(id *)a2 scheme:(id *)a3 path:(id *)a4;
...
- (void *)assertFactString:(id)a0 moduleName:(id)a1 run:(char)a2;
...
- (char)didInstallProfileForKey:(id)a0;
...
- (char)loadModule:(id)a0 requiresBasebandModule:(char)a1;
...
- (char)postCoreAnalyticsEvent:(id)a0 forEventName:(id)a1;
- (id)primaryComponent:(id)a0 obfuscationType:(id)a1;
- (void)probeFactString:(id)a0 module:(id)a1 goIntoDiagnosing:(char)a2 run:(char)a3;
- (void)probeOutputFilePaths:(id)a0 forDiagSessionUUID:(id)a1;
- (void)probeStatusUpdate:(id)a0;
- (void)processRelayStateChange:(id)a0 key:(id)a1 relay:(id)a2 scalar:(char)a3;
- (void)processWiFiAssociationChange:(char)a0 atTime:(id)a1;
...
- (void)retractFact:(void *)a0 moduleName:(id)a1 run:(char)a2;
- (void)retractFacts:(id)a0 runAfterLastFact:(char)a1;
