...
@property (nonatomic) char delegateToProxyAnalytics;
@property (nonatomic) char removeOldRecordsNow;
...
+ (unsigned int)_constructRxSignalExemptionsBitmapFromHint:(char)a0 reasons:(id)a1;
...
+ (char)getDNSCountsOn:(unsigned char)a0 total:(long long *)a1 impacted:(long long *)a2;
+ (id)getNetworkExtensionStateRelay;
+ (char)hasAnyNetworkAttachmenOnLOI:(long long)a0;
+ (void)hasNetworkAttachmentOn:(unsigned char)a0 isAny:(char)a1 isBuiltin:(char)a2 scopedToLOI:(long long)a3 hasCustomSignature:(id)a4 queue:(id)a5 reply:(id /* block */)a6;
...
- (char)_getDNSCountsOn:(unsigned char)a0 total:(long long *)a1 impacted:(long long *)a2;
...
- (void)_armFatalSuspector:(id)a0 isActive:(char)a1;
...
- (char)_determineIfThisIsWiFiFirstAttachment:(id)a0;
...
- (void)_hasNetworkAttachmentOn:(unsigned char)a0 isAny:(char)a1 isBuiltin:(char)a2 scopedToLOI:(long long)a3 hasCustomSignature:(id)a4 queue:(id)a5 reply:(id /* block */)a6;
...
- (char)_isRadioTechnologySubpar:(unsigned char)a0;
...
- (void)_performPeriodicTasks:(char)a0;
...
- (char)_recordOfActiveFatalSuspector:(long long)a0;
...
- (void)_trainModelAndScore:(char)a0;
