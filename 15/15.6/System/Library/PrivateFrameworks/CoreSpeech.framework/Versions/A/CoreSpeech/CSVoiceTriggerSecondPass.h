...
@property (nonatomic) char shouldUsePHS;
...
@property (nonatomic) char isStartSampleCountMarked;
...
@property (nonatomic) char secondPassHasMadeDecision;
...
@property (nonatomic) char hasLoggedSecondPass;
...
@property (nonatomic) char supportsMph;
...
@property (nonatomic) char isPSRAudioFlushed;
...
@property (nonatomic) char isAdBlockerMatchedBeforeSetResultCompletion;
@property (nonatomic) char cancelSecondPassTrigger;
@property (nonatomic) struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } secondpassTriggerCancellationLock;
...
- (void)_calculateRecordingTimeForAOPTriggerFromFirstPassInfo:(id)a0 completion:(id /* block */)a1;
- (void)_clearSecondPassCompletionWatchDog;
- (void)_clearTriggerCandidate;
- (id)_constructVTEIFromExclaveKeywordResult:(unsigned long long)a0 speakerDetectionResult:(unsigned long long)a1 phraseId:(unsigned int)a2 triggerTimestamp:(unsigned long long)a3 triggerStartSampleCount:(unsigned long long)a4 triggerEndSampleCount:(unsigned long long)a5;
- (void)_didStartAudioStream:(char)a0;
...
- (void)_handleAudioChunk:(id)a0 shouldPreprocessedByZeroFilter:(char)a1;
- (void)_handlePHSResults:(id)a0 voiceTriggerEventInfo:(id)a1 forPhId:(unsigned long long)a2;
- (void)_handleResultCompletion:(unsigned long long)a0 voiceTriggerInfo:(id)a1 isSecondChanceCandidate:(char)a2 error:(id)a3;
...
- (void)_handleVoiceTriggerFirstPassFromExclave:(id)a0 audioProviderUUID:(id)a1 completion:(id /* block */)a2;
...
- (char)_isBuiltInFirstPassSource:(unsigned long long)a0;
- (char)_isFirstPassSourceExclave:(unsigned long long)a0;
...
- (void)_logUptimeWithVTSwitchChanged:(char)a0 VTEnabled:(char)a1;
- (void)_mmapModelForPreWarm;
- (void)_notifySecondPassReject:(id)a0 result:(unsigned long long)a1 isSecondChanceCandidate:(char)a2;
- (void)_prepareStartAudioStream;
- (void)_processSecondPassInExclave:(id /* block */)a0 rejectBlock:(id /* block */)a1;
- (void)_reportDiagnosticsForDelayedVTLaunchIfNeeded:(float)a0;
...
- (char)_shouldLogMediaplayState:(unsigned long long)a0;
- (char)_shouldRequestSingleChannelFromAudioProvider;
...
- (id)constructVTEIFromExclaveKeywordResult:(unsigned long long)a0 speakerDetectionResult:(unsigned long long)a1 phraseId:(unsigned int)a2 triggerTimestamp:(unsigned long long)a3 triggerStartSampleCount:(unsigned long long)a4 triggerEndSampleCount:(unsigned long long)a5;
- (id)convertSecureVoiceTriggerKeywordDetectionResultTypeToString:(unsigned long long)a0;
- (id)convertSecureVoiceTriggerSpeakerDetectionResultTypeToString:(unsigned long long)a0;
- (void)forceCancelSecondPassTrigger;
- (void)handleVoiceTriggerSecondPassFrom:(id)a0 completion:(id /* block */)a1;
- (id)initWithFirstPassSource:(unsigned long long)a0 phsEnabled:(char)a1 speechManager:(id)a2 supportsMphDetection:(char)a3 secondPassQueue:(id)a4;
- (void)selfTriggerDetector:(id)a0 didDetectSelfTrigger:(id)a1;
- (void)setSupportsMultiPhraseDetection:(char)a0;
