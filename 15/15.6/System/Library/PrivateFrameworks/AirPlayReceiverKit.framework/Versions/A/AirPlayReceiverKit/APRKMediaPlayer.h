...
@interface APRKMediaPlayer : NSObject <APRKResourceLoaderHelperDelegate, APRKContentKeyHelperDelegate, AVPlayerItemMetadataCollectorPushDelegate> {
    ...
    char _isFirstSetRateReceived;
    char _isInvalidated;
    char _disableAutoResumeAfterSeek;
    char _isWaitingToSetRateFromSenderAfterSeek;
...
@property (readonly, nonatomic) char isInterstitialPlayer;
...
- (void)_addPendingSeekID:(int)a0 withMessage:(id)a1 forItem:(id)a2 ignoreSeekCompletion:(char)a3;
- (char)_attemptToSetSelectedMediaArray:(id)a0;
...
- (void)_sendUpstreamPlaybackStateMessageWithPlaybackStateString:(id)a0 stoppedBecauseInterrupted:(char)a1;
- (id)_serializeTimeRanges:(id)a0;
- (void)_setPropertyWithDictionary:(id)a0;
- (void)_setRateWithDictionary:(id)a0;
- (char)_shouldIgnoreSeekCompletionForSeekID:(int)a0 forItem:(id)a1;
...
- (void)_updatePlaybackStateWithState:(long long)a0 stoppedBecauseInterrupted:(char)a1 shouldSendUpstreamMessage:(char)a2;
- (void)addDateRangeCollectorToItem:(id)a0;
- (void)contentKeyHelper:(id)a0 didGenerateSecureStopRecordPayload:(id)a1;
- (id)initWithP2PWiFiSupport:(char)a0 isInterstitialPlayer:(char)a1 playerSessionID:(int)a2;
...
- (char)shouldDelegateToInterstitialPlayerForMessageAndID:(id)a0 sessionID:(int)a1;
