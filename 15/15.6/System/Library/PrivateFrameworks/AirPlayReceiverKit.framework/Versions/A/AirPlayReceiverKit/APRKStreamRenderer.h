...
@protocol APRKStreamRendererDelegate, AirPlayReceiverUIDelegate, APRKStreamRendererSampleBufferDelegate, OS_dispatch_source, NSObject, OS_dispatch_queue;
...
@interface APRKStreamRenderer : NSObject <APRKMediaPlayerMessagingDelegate> {
    ...
    char _isStopping;
    char _isWiredLink;
    char _isP2PWiFiSession;
    char _canRecord;
    char _useCALayerForMirroring;
    char _deferLayerRendering;
    char _shouldRecordFrames;
    char _isAudioActive;
    char _mirroringNeedsKeyFrame;
    char _isMirroringVideoStreamPaused;
    char _isScreenCleared;
...
@property (readonly, nonatomic) char canRecord;
@property (readonly, nonatomic) char isRecording;
@property (nonatomic) char isPaused;
@property (nonatomic) char isMirroringVideoStreamPaused;
@property (nonatomic) char isMirroringAudioStreamPaused;
@property (readonly, nonatomic) struct CGSize { double x0; double x1; } videoFrameSize;
@property (nonatomic) char autoRotateLayerFollowingClientRotation;
...
- (id)mediaPlayer;
- (void)setCanRecord:(char)a0;
- (id)HUDLayer;
- (int)_enqueueVideoFrameForRendering:(struct opaqueCMSampleBuffer { } *)a0;
- (float)mirroringVolume;
- (void)setIsWiredLink:(char)a0;
- (id)videoQueuePerformanceDictionary;
- (void)_cleanupInternalPlayer;
- (void)_cleanupPreviousRecordingIfExisting;
- (int)_enqueueSampleBufferForRecording:(struct opaqueCMSampleBuffer { } *)a0 isAudioSBuf:(char)a1;
- (short)_ensureFigVideoQueue;
- (void)_ensureInternalPlayerFor:(int)a0;
- (char)_isRotatedTransform:(unsigned long long)a0;
- (char)_performStartRecordingWithOutputURL:(id)a0;
- (char)_performStopRecording;
- (void)_performUIControllerActionWithBlock:(id /* block */)a0;
- (char)_receiverSupportsMirroring;
- (void)_registerForFigVideoQueueNotifications;
- (char)_sampleBufferRepresentsKeyFrame:(struct opaqueCMSampleBuffer { } *)a0;
- (char)_shouldFlip:(unsigned int)a0;
...
- (id)demoDeviceInfo;
- (id)ensembleInfo;
- (id)initWithUniqueID:(id)a0 clientName:(id)a1 UIController:(struct OpaqueAPReceiverUIController { } *)a2 useCALayerForMirroring:(char)a3;
- (char)isP2PWiFi;
- (char)isWiredLink;
...
- (char)processAudioSampleBuffer:(struct opaqueCMSampleBuffer { } *)a0;
- (void)processHidePasscodePromptRequest;
- (void)processSenderUIEvent:(id)a0;
- (void)processShowPasscodePromptRequest:(id)a0;
- (void)processStartAudioSessionRequestWithSessionID:(unsigned int)a0 isScreenAudio:(char)a1;
...
- (char)processStopAudioSessionRequestWithSessionID:(unsigned int)a0;
- (char)processStopScreenPresentationWithSessionID:(unsigned int)a0;
- (char)processStopVideoPlaybackRequestWithSessionID:(unsigned int)a0;
- (char)processVideoSampleBuffer:(struct opaqueCMSampleBuffer { } *)a0;
- (unsigned int)screenSessionCount;
- (void)setDeferLayerRendering:(char)a0;
- (void)setDemoDeviceInfo:(id)a0;
- (void)setEnsembleInfo:(id)a0;
- (void)setIsP2PWiFi:(char)a0;
...
- (char)startRecordingWithOutputURL:(id)a0;
