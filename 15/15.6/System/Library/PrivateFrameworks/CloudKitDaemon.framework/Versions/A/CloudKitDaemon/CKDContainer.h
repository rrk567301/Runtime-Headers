@class CKDAppContainerTuple, CKDPCSManager, CKDZoneGatekeeper, CKContainerID, NSString, CKDProcessScopedClientProxy, CKDPCSFetchAggregator, CKDAnonymousSharingManager, NSMutableSet, CKDLogicalDeviceScopedClientProxy, CKDPCSCache, NSMutableDictionary, CKDIdentityCache, NSArray, CKDServerConfiguration, NSRecursiveLock, CKDDirectoryContext, NSObject, CKEntitlements, CKDMMCS, NSOperationQueue, CKContainerOptions, CKDLogicalDeviceContext, CKDAppContainerAccountTuple, CKDApplicationID, CKDAccount, CKAccountOverrideInfo, CKDContainerServerInfo, CKDPublicIdentityLookupService, CKDRecordCache, NSURL, NSHashTable;
@protocol _TtP14CloudKitDaemon21CKDTestServerProtocol_, OS_dispatch_workloop, OS_dispatch_queue, NSObject;
...
@property (readonly, nonatomic) id<_TtP14CloudKitDaemon21CKDTestServerProtocol_> testServer;
...
@property char isClientMainBundleAppleExecutable;
@property (retain, nonatomic) NSHashTable *clientContainers;
@property char isForClouddInternalUse;
...
@property (readonly, nonatomic) char hasValidatedEntitlements;
...
@property (readonly, nonatomic) char hasTCCAuthorization;
@property (readonly, nonatomic) char canAuthWithCloudKit;
...
- (void)submitClientEventMetric:(id)a0 completeWhenQueued:(char)a1 completionHandler:(id /* block */)a2;
- (void)suggestedMergeableDeltaSizeWithCompletionHandler:(id /* block */)a0;
- (void)setServerInfo:(id)a0;
- (void)performRequest:(id)a0;
- (void)_clearCaches;
- (void)_reloadAccount:(char)a0;
- (id)CKStatusReportArrayIncludingSharedOperations:(char)a0;
- (long long)_accountStatus;
- (long long)_applicationPermissionStatusFromUserPrivacySetting:(long long)a0;
- (void)_broadcastUpdateIfNeededForRecord:(id)a0 recordXPCMetadata:(id)a1 recordID:(id)a2 isDeleted:(char)a3 error:(id)a4;
...
- (void)_fetchContainerServerInfoForOperation:(id)a0 requireUserIDs:(char)a1 requireEnvironment:(char)a2 completionHandler:(id /* block */)a3;
...
- (char)_isAdopterAppleInternal;
- (char)_isContainerAuthorizedForOperation:(id)a0 error:(id *)a1;
- (void)_loadApplicationContainerPathAndType;
- (void)_lockedHandleTCCAuthorizationChangedEvent:(long long)a0;
- (long long)_lockedHasTCCAuthorization:(char)a0;
...
- (void)_setApplicationPermission:(unsigned long long)a0 enabled:(char)a1 completionHandler:(id /* block */)a2;
...
- (long long)hasTCCAuthorization:(char)a0;
- (char)isEligibleForBroadcastingToContainer:(id)a0;
...
- (void)setContainerAvailable:(char)a0;
- (char)setupAssetTransfers:(id *)a0;
- (char)setupMMCSWrapper:(id *)a0;
