...
@property (nonatomic) char pendingOriginatedMessagesTimer;
@property (retain, nonatomic) IMMultiQueue *incomingMessageMultiQueue;
@property (nonatomic) char isRegisteredForCTPhoneNumberNotifications;
...
- (char)_replicationDisabledByServerBag;
- (char)_canLocallySendWithPhoneNumber:(id)a0;
...
- (char)_allowedToGeneratePreviewForTransfer:(id)a0 smsMessage:(id)a1;
- (char)_allowedToProcessMessageFromToken:(id)a0 fromID:(id)a1 reflectOnly:(char)a2 isInProxyMode:(char)a3;
...
- (char)_canSendWithCallerID:(id)a0;
- (void)_carrierReportJunkMetricsForSMSWithSubject:(id)a0 isGroupchat:(char)a1 hasAttachments:(char)a2;
...
- (char)_disableScreenTimeRestrictionsForMessageItem:(id)a0 chat:(id)a1 watchSendsOverCellular:(char)a2 emergencyNumbersFoundInRecipients:(long long *)a3;
- (void)_enqueueBlock:(id /* block */)a0 withTimeout:(double)a1;
- (id)_extractSMSSenderAddress:(id)a0;
- (void)_finishSendingMessageItem:(id)a0 chatIdentifier:(id)a1 chatStyle:(unsigned char)a2 foundChat:(id)a3 didSendSMS:(char)a4 attemptingReplication:(char)a5 shouldForceFail:(char)a6 watchSendsOverCellular:(char)a7 forceReflection:(char)a8 callerID:(id)a9;
- (void)_generatePreviewForTransfer:(id)a0 message:(id)a1;
- (long long)_iMessageCapabilityForSIMID:(id)a0;
- (char)_isActiveServiceForRelay;
- (void)_markFromStorageIfNeeded:(id)a0 messageGUID:(id)a1;
- (void)_markMessageAsSent:(id)a0;
- (void)_markMessageAsSent:(id)a0 wasInterworked:(char)a1;
- (id)_messageDictionaryToRelayWithIncomingMessageDictionary:(id)a0 smsMessage:(id)a1;
- (void)_messageTimedOut:(id)a0;
- (char)_newFeatureRequiredByMessageItem:(id)a0 chatIdentifier:(id)a1;
- (double)_periodicReplicationTimeInterval;
- (void)_processReceivedDictionary:(id)a0 storageContext:(id)a1 receivedViaRelay:(char)a2 withCompletionBlock:(id /* block */)a3;
- (void)_processReceivedDictionaryInBlastDoor:(id)a0 storageContext:(id)a1 receivedViaRelay:(char)a2 withCompletionBlock:(id /* block */)a3;
- (void)_processReceivedSMSMessage:(id)a0 storageContext:(id)a1 receivedViaRelay:(char)a2 completionBlock:(id /* block */)a3;
...
- (void)_relayMessage:(id)a0 chat:(id)a1 didSendSMS:(char)a2 attemptingReplication:(char)a3 forceReflection:(char)a4 relayToWatchOnly:(char)a5 shouldDisableScreenTimeRestrictions:(char)a6 callerID:(id)a7;
- (void)_releasePendingMessagesAndProcessReceivedSMSMessage:(id)a0 storageContext:(id)a1 receivedViaRelay:(char)a2 completionBlock:(id /* block */)a3;
- (void)_removeOriginatedMessage:(id)a0;
- (id)_replicatedMessageGUIDToReleaseFromMessageDictionary:(id)a0;
- (char)_replicationRequiredForFeaturesUsedByMessageBody:(id)a0;
- (void)_resetAndBroadcastBlackholeErrorStateIfNeededForChatIdentifier:(id)a0 chatStyle:(unsigned char)a1;
- (char)_sendMessageOverAlternatePath:(id)a0 toChat:(id)a1;
...
- (char)_shouldFailSMSMessageForTesting:(id)a0 chatIdentifier:(id)a1 chatStyle:(unsigned char)a2;
- (char)_shouldForceReplicationForStaleChatWithLastKnownReplicationDate:(id)a0;
- (char)_shouldReplicateMessageItem:(id)a0 chatStyle:(unsigned char)a1 lastKnownReplicationDate:(id)a2 callerID:(id)a3;
- (char)_sosReplicationDisabledByServerBag;
...
- (void)_updateShouldForceToSMSForChatWithIdentifier:(id)a0 style:(unsigned char)a1 shouldForceToSMS:(char)a2;
...
- (char)allowedToSendMessage:(id)a0 toChat:(id)a1;
- (id)chatForIncomingMessageContext:(id)a0 createIfNotExists:(char)a1;
- (id)chatForOutgoingMessage:(id)a0 fromIdentifier:(id)a1 isInProxyMode:(char)a2 createIfNotExists:(char)a3;
- (id)compatibleMessageToSendForMessageItem:(id)a0;
- (id)createHandleInfoFor:(id)a0;
- (id)createHandleInfoForParticipants:(id)a0 unformattedIDs:(id)a1 countryCodes:(id)a2 isGroupChat:(char)a3;
...
- (void)didReceiveDisplayNameChange:(id)a0 guid:(id)a1 fromID:(id)a2 toIdentifier:(id)a3 forChat:(id)a4 style:(unsigned char)a5 account:(id)a6 shouldRelay:(char)a7;
...
- (void)handler:(id)a0 incomingGroupMutationMessage:(id)a1 timeStamp:(id)a2 storageContext:(id)a3 fromToken:(id)a4 fromID:(id)a5 isInProxyMode:(char)a6;
...
- (void)handler:(id)a0 localMessageRead:(id)a1 readByMe:(char)a2 timeStamp:(id)a3 storageContext:(id)a4;
- (void)handler:(id)a0 localMessageSent:(id)a1 wasInterworked:(char)a2 storageContext:(id)a3;
- (void)handler:(id)a0 localOutgoingDownloadMessage:(id)a1 isBeingReplayed:(char)a2 storageContext:(id)a3;
- (void)handler:(id)a0 localOutgoingMessage:(id)a1 isBeingReplayed:(char)a2 storageContext:(id)a3;
- (void)handler:(id)a0 messageIDRead:(id)a1 readByMe:(char)a2 timeStamp:(id)a3 toIdentifier:(id)a4 fromIdentifier:(id)a5 fromToken:(id)a6 reflectOnly:(char)a7 storageContext:(id)a8;
...
- (void)handler:(id)a0 messageIDSent:(id)a1 wasInterworked:(char)a2 toIdentifier:(id)a3 fromIdentifier:(id)a4 timeStamp:(id)a5 storageContext:(id)a6;
- (void)handler:(id)a0 outgoingDownloadMessage:(id)a1 toIdentifier:(id)a2 fromIdentifier:(id)a3 fromToken:(id)a4 messageGUID:(id)a5 timeStamp:(id)a6 isBeingReplayed:(char)a7 isInProxyMode:(char)a8 storageContext:(id)a9;
- (void)handler:(id)a0 outgoingPlainTextMessage:(id)a1 toIdentifier:(id)a2 fromIdentifier:(id)a3 fromToken:(id)a4 messageGUID:(id)a5 timeStamp:(id)a6 isBeingReplayed:(char)a7 isInProxyMode:(char)a8 storageContext:(id)a9;
- (void)handler:(id)a0 outgoingPlainTextMessage:(id)a1 toIdentifier:(id)a2 fromIdentifier:(id)a3 fromToken:(id)a4 messageGUID:(id)a5 timeStamp:(id)a6 isBeingReplayed:(char)a7 storageContext:(id)a8;
- (char)incomingMessageHasTooManyRecipients:(id)a0;
- (id)initWithAccount:(id)a0 service:(id)a1 replicatingForSession:(id)a2;
- (char)isGroupMessagingEnabledFor:(id)a0;
...
- (void)relayDisplayNameChange:(id)a0 forChat:(id)a1 fromID:(id)a2 messageID:(id)a3 didOccurLocally:(char)a4;
- (char)replicateMessage:(id)a0 chatIdentifier:(id)a1 chatStyle:(unsigned char)a2 lastKnownReplicationDate:(id)a3 callerID:(id)a4;
- (id)replicationPhoneNumbers;
- (char)replicationRequiredForFeaturesUsedByMessageItem:(id)a0;
- (char)replicationRequiredForMessageBodyAttributeName:(id)a0;
- (char)replicationRequiredForMultipleMessageParts;
- (char)replicationRequirementsSatisfiedForPhoneNumber:(id)a0 simID:(id)a1;
- (char)responsibleForMessageID:(id)a0;
...
- (char)shouldExcludeChatFromRelay:(id)a0;
