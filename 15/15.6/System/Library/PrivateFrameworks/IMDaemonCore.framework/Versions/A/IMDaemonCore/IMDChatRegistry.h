...
@property (nonatomic) char hasDumpedLogsForNoExisitingGroup;
...
- (id)aliasToCNIDMap;
- (unsigned long long)markAsSpamForIDs:(id)a0 style:(unsigned char)a1 onServices:(id)a2 chatID:(id)a3 queryID:(id)a4 autoReport:(char)a5;
...
- (char)filterediMessageChatsExist;
...
- (id)_addChatFromCacheMiss:(id)a0 preferExistingChat:(char)a1;
- (id)_addChatsFromCacheMisses:(id)a0 preferExistingChats:(char)a1;
...
- (char)_chat:(id)a0 isDuplicateOfChat:(id)a1;
...
- (id)_chatInfoInRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a0 wantsBlackholed:(char)a1;
- (id)_chatsWithBlackholed:(char)a0;
- (void)_checkForContactChanges;
- (long long)_compareDuplicateChatsWithFirstChat:(id)a0 secondChat:(id)a1;
- (char)_contactsBasedMergingEnabled;
...
- (char)_ensureNoExistingGroupForSyncData:(id)a0;
- (void)_enumeratePersonMergedChatsWithMaximumNumberOfChats:(long long)a0 usingChats:(id)a1 useCachedChatGroups:(char)a2 includingPinnedChatIdentifiers:(id)a3 usingBlock:(id /* block */)a4;
...
- (id)_existingChatsForIDs:(id)a0 serviceName:(id)a1 displayName:(id)a2 groupID:(id)a3 useOriginalGroupID:(char)a4 style:(unsigned char)a5;
- (char)_existingItemRequiresUpgrade:(id)a0 itemFromCKRecord:(id)a1;
...
- (id)_findChatWinnerInDuplicateChatArray:(id)a0 fixDisplayName:(char *)a1;
...
- (void)_forceReloadChats:(char)a0;
- (id)_generateCurrentAliasToCNIDDictionary;
- (id)_handlesWithChangedContactsOriginalMap:(id)a0 newMap:(id)a1;
- (char)_hasInconsistentGroupPhotoGUIDAcrossMergedChats:(id)a0;
- (char)_hasInconsistentJoinStateAcrossMergedChats:(id)a0;
- (char)_hasSavedContactCardForHandle:(id)a0;
...
- (id)_newestGroupChatMatchingParticipants:(id)a0 displayName:(id)a1 strictDisplayNameMatching:(char)a2 serviceName:(id)a3 toIdentifier:(id)a4;
...
- (void)_reportMessageDictionaryForMessages:(id)a0 withLastAddressedHandle:(id)a1 maxMessageLength:(unsigned long long)a2 isAutoReport:(char)a3 withChat:(id)a4 maxMessagesToReport:(unsigned long long)a5 totalMessageCount:(unsigned long long *)a6 isJunkReportedToCarrier:(char)a7 reportReason:(unsigned long long)a8;
- (void)_reportMessageDictionaryForMessages:(id)a0 withLastAddressedHandle:(id)a1 maxMessageLength:(unsigned long long)a2 isAutoReport:(char)a3 withChat:(id)a4 maxMessagesToReport:(unsigned long long)a5 totalMessageCount:(unsigned long long *)a6 notifyInternalSecurity:(char)a7 isJunkReportedToCarrier:(char)a8 reportReason:(unsigned long long)a9;
...
- (void)_setInitialLoadForTesting:(char)a0;
...
- (void)addChat:(id)a0;
- (void)addChat:(id)a0 firstLoad:(char)a1 store:(char)a2;
- (void)addIMDHandleToRegistry:(id)a0;
- (void)addMessage:(id)a0 toChat:(id)a1;
- (void)addMessage:(id)a0 toChat:(id)a1 deferSpotlightIndexing:(char)a2;
...
- (id)chatForHandle:(id)a0 account:(id)a1 chatIdentifier:(id)a2 guid:(id)a3 lastAddressedHandle:(id)a4 lastAddressedSIMID:(id)a5 isBlackholed:(char)a6;
- (id)chatForHandles:(id)a0 account:(id)a1 chatIdentifier:(id)a2 style:(unsigned char)a3 groupID:(id)a4 originalGroupID:(id)a5 displayName:(id)a6 guid:(id)a7 lastAddressedHandle:(id)a8 lastAddressedSIMID:(id)a9 isBlackholed:(char)a10;
...
- (void)checkBlackholedChatsExistAfterUpdatingChatWithAdd:(char)a0;
...
- (id)existingChatsFilteredUsingPredicate:(id)a0 sortedUsingLastMessageDateAscending:(char)a1 limit:(unsigned long long)a2;
- (id)existingChatsFilteredUsingPredicate:(id)a0 sortedUsingLastMessageDateAscending:(char)a1 olderThan:(id)a2 limit:(unsigned long long)a3;
...
- (char)filteredChatsExistForServices:(id)a0;
- (id)groupChatsBasedOnIdentity:(id)a0;
- (id)groupChatsBasedOnIdentityUsingCacheIfApplicable:(id)a0;
- (char)handleMessageUpdate:(id)a0;
- (void)handleMessageUpdateConflictType:(id)a0 serverRecord:(id)a1 localRowID:(long long)a2;
- (char)hasKnownSenderChatWithChatIdentifier:(id)a0;
- (void)invalidatePersonCentricGroupedChatsCache;
- (char)isBeingSetup;
- (char)loadChatsWithCompletionBlock:(id /* block */)a0;
- (unsigned long long)markAsSpamForIDs:(id)a0 style:(unsigned char)a1 onServices:(id)a2 chatID:(id)a3 queryID:(id)a4 autoReport:(char)a5 isJunkReportedToCarrier:(char)a6;
- (unsigned long long)markAsSpamForIDs:(id)a0 style:(unsigned char)a1 onServices:(id)a2 chatID:(id)a3 queryID:(id)a4 autoReport:(char)a5 isJunkReportedToCarrier:(char)a6 reportReason:(unsigned long long)a7;
...
- (id)newestExistingChatForcingGroupIDUpdateWithOriginalGroupID:(id)a0 onService:(id)a1;
- (id)newestExistingChatWithOriginalGroupID:(id)a0 onService:(id)a1;
- (void)pendingAlternateID:(id)a0 forHandle:(id)a1;
- (id)personCentricGroupedChatsArrayWithMaximumNumberOfChats:(long long)a0 skipsLastMessageLoading:(char)a1 usingChats:(id)a2 useCachedChatGroups:(char)a3 includingPinnedChatIdentifiers:(id)a4 repairInconsistentMergedChats:(char)a5;
...
- (char)removeIMDHandleFromRegistry:(id)a0;
...
- (void)updateLastMessageForChat:(id)a0 hintMessage:(id)a1 historyQuery:(char)a2;
- (void)updateMeCardHasUpdatedForAllChats;
- (void)updateNotificationUnreadCountForChat:(id)a0;
- (char)updateProperties:(id)a0 chat:(id)a1 style:(unsigned char)a2;
- (void)updateStateForChat:(id)a0 forcePost:(char)a1;
- (void)updateStateForChat:(id)a0 forcePost:(char)a1 shouldRebuildFailedMessageDate:(char)a2;
- (void)updateStateForChat:(id)a0 forcePost:(char)a1 shouldRebuildFailedMessageDate:(char)a2 setUnreadCountToZero:(char)a3;
- (void)updateStateForChat:(id)a0 forcePost:(char)a1 shouldRebuildFailedMessageDate:(char)a2 shouldCalculateUnreadCount:(char)a3;
- (void)updateStateForChat:(id)a0 fromMessage:(id)a1 toMessage:(id)a2 forcePost:(char)a3 hintMessage:(id)a4 shouldRebuildFailedMessageDate:(char)a5 shouldCalculateUnreadCount:(char)a6 setUnreadCountToZero:(char)a7;
- (void)updateStateForChat:(id)a0 hintMessage:(id)a1 shouldRebuildFailedMessageDate:(char)a2;
- (void)updateStateForChat:(id)a0 hintMessage:(id)a1 shouldRebuildFailedMessageDate:(char)a2 setUnreadCountToZero:(char)a3;
- (void)updateStateForChat:(id)a0 hintMessage:(id)a1 shouldRebuildFailedMessageDate:(char)a2 shouldCalculateUnreadCount:(char)a3;
- (char)updateSyncedSyndicationRanges:(id)a0 forGUID:(id)a1;
- (char)updateUnreadCountForChat:(id)a0;
