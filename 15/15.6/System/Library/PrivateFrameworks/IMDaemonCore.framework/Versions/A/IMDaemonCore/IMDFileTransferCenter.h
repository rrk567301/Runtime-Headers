...
- (char)markAttachment:(id)a0 sender:(id)a1 recipients:(id)a2 isIncoming:(char)a3;
...
- (char)registerGUID:(id)a0 forNewOutgoingTransferWithLocalURL:(id)a1;
...
- (void)_addTransfer:(id)a0 forGUID:(id)a1 shouldNotify:(char)a2;
...
- (id)_dictionaryRepresentationsForFileTransfers:(id)a0 toSave:(char)a1;
...
- (void)_handleFileTransfer:(id)a0 acceptedWithPath:(id)a1 autoRename:(char)a2 overwrite:(char)a3 options:(long long)a4 postNotification:(char)a5;
- (void)_handleFileTransfer:(id)a0 createdWithProperties:(id)a1 withAuditToken:(struct { unsigned int x0[8]; })a2;
- (void)_handleFileTransfer:(id)a0 updatedWithProperties:(id)a1 isStickerReposition:(char)a2 repositionedLocally:(char)a3;
- (void)_handleFileTransferForStickerRepositionWithGUID:(id)a0 updatedWithProperties:(id)a1 fromLocalChange:(char)a2;
- (void)_handleFileTransferRemoved:(id)a0;
- (void)_handleFileTransferStopped:(id)a0;
- (void)_handleFileTransfers:(id)a0 autoRename:(char)a1 overwrite:(char)a2 options:(long long)a3 postNotification:(char)a4;
- (char)_hasActiveTransferWithGUID:(id)a0;
- (void)_hubbleTransferCompleted:(id)a0 withGUID:(id)a1 error:(id)a2 suggestedRetryGUID:(id)a3;
- (char)_isFileTransferWithGUIDFromKnownSender:(id)a0;
...
- (id)_progressForTransferGUID:(id)a0 allowCreate:(char)a1 path:(id)a2;
...
- (char)_shouldDownloadAssetForTransfer:(id)a0 forMessageItem:(id)a1;
...
- (void)endTransfer:(id)a0 endState:(long long)a1 updateByteCounts:(char)a2;
...
- (id)guidForNewIncomingTransferWithFilename:(id)a0 isDirectory:(char)a1 totalBytes:(unsigned long long)a2 hfsType:(unsigned int)a3 hfsCreator:(unsigned int)a4 hfsFlags:(unsigned short)a5;
- (id)guidForNewOutgoingTransferWithFilename:(id)a0 isDirectory:(char)a1 totalBytes:(unsigned long long)a2 hfsType:(unsigned int)a3 hfsCreator:(unsigned int)a4 hfsFlags:(unsigned short)a5;
...
- (void)makeNewIncomingTransferWithGUID:(id)a0 filename:(id)a1 isDirectory:(char)a2 totalBytes:(unsigned long long)a3 hfsType:(unsigned int)a4 hfsCreator:(unsigned int)a5 hfsFlags:(unsigned short)a6;
- (void)makeNewOutgoingTransferWithGUID:(id)a0 filename:(id)a1 isDirectory:(char)a2 totalBytes:(unsigned long long)a3 hfsType:(unsigned int)a4 hfsCreator:(unsigned int)a5 hfsFlags:(unsigned short)a6;
- (void)markTransferAsNotSuccessfullyDownloadedFromCloud:(id)a0;
- (void)markTransferAsNotSyncedSuccessfully:(id)a0;
- (char)populateLocalURLsForTransfer:(id)a0 fromCKRecord:(id)a1;
...
- (void)setUpdatedStickerUserInfo:(id)a0 forTransfer:(id)a1 repositioningFromLocalChange:(char)a2;
- (void)startFinalizingTransfer:(id)a0;
- (void)startFinalizingTransfer:(id)a0 updateByteCounts:(char)a1;
...
- (id)updateTransfersWithCKRecord:(id)a0 recordWasFetched:(char)a1 downloadAsset:(char *)a2;
