...
- (id)keyRequestByID:(unsigned long long)a0;
...
- (void)_handleRequest:(struct __CFDictionary { } *)a0 withRequestID:(unsigned long long)a1 fromHandler:(struct OpaqueFigCustomURLHandler { } *)a2 willHandleRequest:(char *)a3;
- (void)_handleSecureStopDidFinalizeRecordCallback;
- (void)_handleUpdateToPersistentKey:(id)a0 forKeyIdentifier:(id)a1;
- (id)_internalQueue;
- (void)_invokeDelegateCallbackWithBlock:(id /* block */)a0 synchronouslyWhenDelegateQueueIsNULL:(char)a1;
- (char)_makeContentKeyGroupWithIDOut:(unsigned long long *)a0 error:(id *)a1;
- (unsigned long long)_processContentKeyRequestWithIdentifier:(id)a0 encryptionMethod:(int)a1 supportedProtocolVersions:(id)a2 options:(id)a3 groupID:(unsigned long long)a4 error:(id *)a5;
- (void)_processContentKeyRequestWithIdentifier:(id)a0 initializationData:(id)a1 options:(id)a2 groupID:(unsigned long long)a3;
- (void)_removeContentKeyRequestForCryptorUUID:(id)a0 cryptorKeyRequestID:(unsigned long long)a1;
- (char)_renewKeyWithIdentifier:(id)a0 encryptionMethod:(int)a1 initializationData:(id)a2 contentIdentifier:(id)a3 keyIDFromInitializationData:(id)a4 options:(id)a5 error:(id *)a6;
...
- (void)contentKeyRequestDidSucceedWithBoss:(struct OpaqueFigContentKeyBoss { } *)a0 keySpecifier:(struct OpaqueFigContentKeySpecifier { } *)a1 requestID:(unsigned long long)a2 cryptor:(struct OpaqueFigCPECryptor { } *)a3;
- (struct OpaqueFigCPECryptor { } *)copyCryptorForCryptKeyAttributes:(id)a0;
- (char)copyCryptorForFormatDescription:(struct opaqueCMFormatDescription { } *)a0 cryptorOut:(const struct OpaqueFigCPECryptor **)a1 errorOut:(id *)a2;
- (struct OpaqueFigCPECryptor { } *)copyCryptorForInitializationData:(id)a0;
- (int)createAndInstallCustomURLHandlerForAsset:(id)a0 outHandler:(const struct OpaqueFigCustomURLHandler **)a1;
- (struct OpaqueFigCPECryptor { } *)createCryptorIfNecessaryForInitializationData:(id)a0 formatDescription:(struct opaqueCMFormatDescription { } *)a1 error:(id *)a2;
- (unsigned long long)defaultGroupID;
- (void)didProvideKeyRequestWithBoss:(struct OpaqueFigContentKeyBoss { } *)a0 keySpecifier:(struct OpaqueFigContentKeySpecifier { } *)a1 requestID:(unsigned long long)a2 renewalRequest:(char)a3 supportsOfflineKey:(char)a4;
- (void)expire;
- (void)externalProtectionStateChangedCallbackWithBoss:(struct OpaqueFigContentKeyBoss { } *)a0 keySpecifier:(struct OpaqueFigContentKeySpecifier { } *)a1;
- (char)hasProtector;
- (id)initWithKeySystem:(id)a0 storageDirectoryAtURL:(id)a1 internal:(char)a2;
- (id)initWithKeySystem:(id)a0 storageDirectoryAtURL:(id)a1 legacyWebKitCompatibilityMode:(char)a2;
...
- (void)issueContentKeyRequestWithCustomURLHandler:(struct OpaqueFigCustomURLHandler { } *)a0 identifier:(id)a1 requestInfo:(struct __CFDictionary { } *)a2 requestID:(unsigned long long)a3 providesPersistableKey:(char)a4;
- (void)issueContentKeyRequestWithPreloadingRequestOptions:(id)a0 identifier:(id)a1 initializationData:(id)a2 providesPersistableKey:(char)a3;
- (void)issueContentKeyRequests:(id)a0 forInitializationData:(id)a1;
- (void)issuePersistableContentKeyRequest:(id)a0;
- (void)issuePersistableContentKeyRequestForKeySpecifier:(struct OpaqueFigContentKeySpecifier { } *)a0 initializationData:(id)a1 keyIDFromInitializationData:(id)a2 contentIdentifier:(id)a3 requestID:(unsigned long long)a4 preloadingRequestOptions:(id)a5 identifier:(id)a6;
...
- (void)processContentKeyRequestForPSSHEntries:(id)a0 initializationData:(id)a1 groupID:(unsigned long long)a2 options:(id)a3;
- (void)processContentKeyRequestForSinf:(id)a0 initializationData:(id)a1 groupID:(unsigned long long)a2 options:(id)a3;
- (void)processContentKeyRequestForTransportStreamWithCodecType:(id)a0 initializationData:(id)a1 groupID:(unsigned long long)a2 options:(id)a3;
