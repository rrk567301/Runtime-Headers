...
@property (readonly, nonatomic, getter=isAuthorizationRequiredForPlayback) char authorizationRequiredForPlayback;
@property (readonly, nonatomic, getter=isApplicationAuthorizedForPlayback) char applicationAuthorizedForPlayback;
@property (readonly, nonatomic, getter=isContentAuthorizedForPlayback) char contentAuthorizedForPlayback;
@property (readonly, nonatomic) long long contentAuthorizationRequestStatus;
@property (nonatomic, getter=_isExternalProtectionRequiredForPlayback, setter=_setExternalProtectionRequiredForPlayback:) char _externalProtectionRequiredForPlayback;
@property (readonly, nonatomic) NSString *_applicationAuthorizationChallengeProvider;
@property (readonly, nonatomic, getter=_isRental) char _rental;
@property (readonly, nonatomic) NSDate *_rentalStartDate;
@property (readonly, nonatomic) NSDate *_rentalExpirationDate;
@property (readonly, nonatomic, getter=_isRentalPlaybackStarted) char _rentalPlaybackStarted;
...
@property (nonatomic) char automaticallyHandlesInterstitialEvents;
...
- (void)dispatchIVarWrite:(char)a0 locked:(char)a1 block:(id /* block */)a2;
...
- (void)setSuppressesAudioOnlyVariants:(char)a0;
- (int)_CreateSeekID;
- (void)_addFoldedTBListeners;
- (void)_addInterstitialEventCollector;
- (void)_addInterstitialEventCollectorLocked:(char)a0;
- (void)_addLegibleOutput:(id)a0;
- (void)_addMetadataCollector:(id)a0;
- (void)_addMetadataCollector:(id)a0 locked:(char)a1;
- (void)_addMetadataOutput:(id)a0;
- (void)_addRenderedLegibleOutput:(id)a0;
- (void)_addSyncLayer:(id)a0;
- (char)_addToPlayQueueOfFigPlayerOfPlayer:(id)a0 afterFigPlaybackItemOfItem:(id)a1;
...
- (char)_configurePlaybackItemAndReturnError:(id *)a0;
...
- (struct OpaqueFigPlaybackItem { } *)_copyFigPlaybackItemLocked:(char)a0;
...
- (char)_getCachedPresentationSize:(struct CGSize { double x0; double x1; } *)a0;
...
- (char)_hasSelectionInCachedMediaSelectionGroup:(id)a0;
...
- (void)_postSeekCompletionNotificationWithSeekID:(int)a0 andResult:(char)a1;
...
- (char)_selectMediaOption:(id)a0 inMediaSelectionGroup:(id)a1;
...
- (void)_setParticipatesInCoordinatedPlayback:(char)a0;
- (void)_setPlaybackCoordinator:(id)a0;
- (void)_setSuppressesVideoLayers:(char)a0;
- (void)_setTemplatePlayerItem:(id)a0;
- (void)_setTimeJumpedNotificationIncludesExtendedDiagnosticPayload:(char)a0;
...
- (char)_speedRampDataWasSet;
- (char)_suppressesVideoLayers;
- (char)_suppressionForOutputs:(id)a0;
- (id)_syncLayers;
- (char)_timeJumpedNotificationIncludesExtendedDiagnosticPayload;
...
- (void)_updateCanPlayAndCanStepPropertiesWhenReadyToPlayWithNotificationPayload:(id)a0 updateStatusToReadyToPlay:(char)a1;
...
- (void)addMediaDataCollector:(id)a0 locked:(char)a1;
...
- (char)playerAppliesAutomaticMediaSelectionToGroup:(id)a0;
...
- (void)removeMediaDataCollector:(id)a0 locked:(char)a1;
...
- (char)seekToDate:(id)a0;
- (char)seekToDate:(id)a0 completionHandler:(id /* block */)a1;
...
- (void)setAggressivelyCachesVideoFrames:(char)a0;
- (void)setAllowProgressiveResume:(char)a0;
- (void)setAllowProgressiveStartup:(char)a0;
- (void)setAllowProgressiveSwitchUp:(char)a0;
- (void)setAllowedAudioSpatializationFormats:(unsigned long long)a0;
- (void)setAllowsExtendedReadAhead:(char)a0;
- (void)setAlwaysMonitorsPlayability:(char)a0;
- (void)setAppliesPerFrameHDRDisplayMetadata:(char)a0;
- (void)setAudioMix:(id)a0;
- (void)setAudioSpatializationAllowed:(char)a0;
- (void)setAudioTapProcessor:(struct opaqueMTAudioProcessingTap { } *)a0;
- (void)setAudioTimePitchAlgorithm:(id)a0;
- (void)setAutoSwitchAtmosStreamVariants:(char)a0;
- (void)setAutomaticallyPreservesTimeOffsetFromLive:(char)a0;
- (void)setBlendsVideoFrames:(char)a0;
...
- (void)setContinuesPlayingDuringPrerollForRateChange:(char)a0;
- (void)setContinuesPlayingDuringPrerollForSeek:(char)a0;
- (void)setDecodesAllFramesDuringOrdinaryPlayback:(char)a0;
...
- (void)setNonForcedSubtitleDisplayEnabled:(char)a0;
...
- (void)setPrefersOfflinePlayableVariants:(char)a0;
- (void)setRTCReportingFlags:(unsigned int)a0;
- (void)setRefreshesClosedCaptionsImmediatelyAfterSeeking:(char)a0;
...
- (void)setRequiresAccessLog:(char)a0;
- (void)setRestrictsAutomaticMediaSelectionToAvailableOfflineOptions:(char)a0;
- (void)setReversePlaybackEndTime:(struct { long long x0; int x1; unsigned int x2; long long x3; })a0;
- (void)setReversesMoreVideoFramesInMemory:(char)a0;
- (void)setSavesDownloadedDataToDiskWhenDone:(char)a0;
- (void)setSeekingWaitsForVideoCompositionRendering:(char)a0;
- (void)setShouldOptimizeForExclusivePlayback:(char)a0;
- (void)setSnapTimeToPausePlayback:(unsigned char)a0;
- (void)setSpeedThresholdForIFrameOnlyPlayback:(float)a0;
- (void)setStartsOnFirstEligibleVariant:(char)a0;
...
- (void)setUsesIFrameOnlyPlaybackForHighRateScaledEdits:(char)a0;
- (void)setUsesMinimalLatencyForVideoCompositionRendering:(char)a0;
...
- (void)setWillNeverSeekBackwardsHint:(char)a0;
