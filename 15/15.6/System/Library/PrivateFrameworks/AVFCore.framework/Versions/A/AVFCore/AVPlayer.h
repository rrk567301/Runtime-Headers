...
@property (readonly, nonatomic, getter=_isPIPModePossible) char PIPModePossible;
@property (readonly, nonatomic, getter=isAudioPlaybackEnabledAtAllRates) char audioPlaybackEnabledAtAllRates;
@property float minRateForAudioPlayback;
@property float maxRateForAudioPlayback;
@property char allowsPixelBufferPoolSharing;
@property char disallowsAMRAudio;
@property char appliesMediaSelectionCriteriaAutomatically;
@property (copy) NSString *audioOutputDeviceUniqueID;
@property char allowsOutOfBandTextTrackRendering;
...
@property (nonatomic, getter=_disallowsAutoPauseOnRouteRemovalIfNoAudio, setter=_setDisallowsAutoPauseOnRouteRemovalIfNoAudio:) char disallowsAutoPauseOnRouteRemovalIfNoAudio;
@property (retain, nonatomic, getter=_STSLabel, setter=_setSTSLabel:) NSString *STSLabel;
@property (nonatomic) char prefersPlayingSilentlyWhenConflictingWithOtherPlayback;
@property (readonly, nonatomic) char isSilencedDueToConflictWithOtherPlayback;
@property (nonatomic, getter=_silencesOtherPlaybackDuringPIP, setter=_setSilencesOtherPlaybackDuringPIP:) char silencesOtherPlaybackDuringPIP;
@property (readonly, nonatomic) NSNumber *mxSessionID;
@property (copy, nonatomic) NSString *backgroundPIPAuthorizationToken;
@property (nonatomic) char shouldWaitForVideoTarget;
...
@property (nonatomic) char allowsLegibleFallbackForAllAudibleMediaSelections;
@property (retain, nonatomic) AVPlayerMediaSelectionCriteria *legibleFallbackMediaSelectionCriteria;
@property (nonatomic, getter=_supportsAdvanceTimeForOverlappedPlayback, setter=_setSupportsAdvanceTimeForOverlappedPlayback:) char _supportsAdvanceTimeForOverlappedPlayback;
@property (nonatomic, getter=supportsSpeedRamps, setter=setSupportsSpeedRamps:) char supportsSpeedRamps;
...
- (void)_setRateDidChangeNotificationIncludesExtendedDiagnosticPayload:(char)a0;
- (void)addVideoTarget:(struct OpaqueFigVideoTarget { } *)a0;
- (void)setClosedCaptionDisplayEnabled:(char)a0;
- (void)setDefaultRate:(float)a0;
- (void)_changeStatusToFailedWithError:(id)a0;
- (void)_setParticipatesInAudioSession:(char)a0;
- (id)_subtitleLayers;
...
- (char)_clientRequestedPlaybackCoordinator;
...
- (struct OpaqueFigPlayerInterstitialCoordinator { } *)_copyInterstitialEventCoordinatorEnsuringItIsRemote:(char)a0;
...
- (char)_getDisplayVideoRange:(id *)a0 displaySize:(struct CGSize { double x0; double x1; } *)a1 refreshRate:(id *)a2;
- (char)_handleSetRate:(float)a0 withVolumeRampDuration:(struct { long long x0; int x1; unsigned int x2; long long x3; })a1 playImmediately:(char)a2 rateChangeReason:(int)a3 affectsCoordinatedPlayback:(char)a4;
- (char)_hasCurrentInterstitialEvent;
...
- (char)_isBufferedAirPlayActive;
- (char)_isDisplayingClosedCaptions;
- (char)_isInterstitialPlayer;
- (char)_isMuted;
...
- (char)_participatesInAudioSession;
...
- (char)_rateDidChangeNotificationIncludesExtendedDiagnosticPayload;
- (id)_rateDidChangeNotificationPayloadForAVFRateChangeReason:(id)a0 reasonIsExtendedDiagnostic:(char)a1 rateChangeIdentifier:(id)a2 rateChangeOriginator:(id)a3;
...
- (char)_runOnIvarAccessQueueOperationThatMayChangeCurrentItemWithPreflightBlock:(id /* block */)a0 modificationBlock:(id /* block */)a1 error:(id *)a2;
- (void)_setActionAtItemEnd:(long long)a0 allowingAdvance:(char)a1;
- (void)_setAncillaryPerformanceInformationForDisplay:(id)a0;
- (void)_setCurrentItem:(id)a0;
- (void)_setDisallowsVideoLayerDisplayCompositing:(char)a0;
...
- (void)_setIsInterstitialPlayer:(char)a0;
- (void)_setOptions:(id)a0 onClosedCaptionLayer:(id)a1;
- (void)_setPendingFigPlayerProperty:(id)a0 forKey:(id)a1;
- (void)_setPreservesAudioSessionSampleRate:(char)a0;
- (void)_setPrimaryPlayerNameForInterstitial:(id)a0;
- (char)_setRate:(float)a0 rateChangeReason:(int)a1 figPlayerSetRateHandler:(id /* block */)a2;
- (void)_setRate:(float)a0 withVolumeRampDuration:(struct { long long x0; int x1; unsigned int x2; long long x3; })a1 playImmediately:(char)a2 rateChangeReason:(int)a3 affectsCoordinatedPlayback:(char)a4;
- (void)_setSeparated:(char)a0 onClosedCaptionLayer:(id)a1;
- (void)_setStartupSyncIgnoresAudioDeviceLatency:(char)a0;
- (void)_setSuppressesAudioRendering:(char)a0;
- (void)_setUsesLegacyAutomaticWaitingBehavior:(char)a0;
...
- (char)_startupSyncIgnoresAudioDeviceLatency;
...
- (void)_updateVideoTargetOnFigPlayer:(id)a0 synchronously:(char)a1;
...
- (void)prerollOperationDidComplete:(char)a0 notificationPayload:(struct __CFDictionary { } *)a1;
...
- (void)removeVideoTarget:(struct OpaqueFigVideoTarget { } *)a0 synchronously:(char)a1;
...
- (void)setAllowsAirPlayVideo:(char)a0;
- (void)setAllowsExternalPlayback:(char)a0;
- (void)setAudioSession:(id)a0;
- (void)setAudiovisualBackgroundPlaybackPolicy:(long long)a0;
- (void)setAutoSwitchStreamVariants:(char)a0;
- (void)setAutomaticallyWaitsToMinimizeStalling:(char)a0;
- (void)setDisallowsHardwareAcceleratedVideoDecoder:(char)a0;
...
- (void)setPlayerRole:(id)a0 synchronously:(char)a1;
- (void)setPreparesItemsForPlaybackAsynchronously:(char)a0;
...
- (void)setShouldReduceResourceUsage:(char)a0;
- (void)setUsesAirPlayVideoWhileAirPlayScreenIsActive:(char)a0;
- (void)setUsesExternalPlaybackWhileExternalScreenIsActive:(char)a0;
