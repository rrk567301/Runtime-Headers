...
@interface WBSTranslationContext : NSObject <WBSTranslationErrorControllerDelegate, WBSFluidProgressStateSource, WBSTranslationContentExtractionDelegate, WBSTranslationContentFillingDelegate, WBSTranslationContextTranslating> {
    ...
    char _hasReportedInitialVisibleParagraphsFinishedFillingForAnalytics;
...
@property (readonly, nonatomic) char shouldOfferTranslationUI;
@property (readonly, nonatomic) unsigned long long detectionNotificationLevel;
@property (readonly, nonatomic) char hasStartedTranslating;
...
- (void)owningWebViewWillNavigateToBackForwardListItem:(id)a0 inPageCache:(char)a1;
- (id)progressState;
...
- (void)updateMaxVisibleHeightPercentageIfNeeded:(double)a0 userDriven:(char)a1;
- (char)validateTargetLocale:(id)a0;
- (id)_currentSnapshot;
- (void)_updateCachedAPISupportedValues;
- (id)_currentSession;
- (id)_availableTargetLocaleIdentifiersConsideringCurrentWebpageLocale:(char)a0;
- (char)_canLogSourcePageURL;
- (void)_cancelProgressForNavigation:(char)a0;
...
- (char)_nextURLIsEligibleForContinuedTranslation:(id)a0;
- (void)_notifyDelegateToReportProgressInUnifiedField:(char)a0;
...
- (char)_restoreContextSnapshotIfNeeded:(id)a0 forURL:(id)a1;
- (void)_setState:(unsigned long long)a0 usingLock:(char)a1;
- (void)_setState:(unsigned long long)a0 usingLock:(char)a1 validatingTransitionIsAllowed:(char)a2;
...
- (char)_validateTargetLocaleSynchronously:(id)a0;
...
- (void)owningWebViewWillNavigateAndSaveCurrentSnapshotToStore:(id)a0 isEligibleForContinuedTranslation:(char)a1;
- (void)owningWebViewWillNavigateToBackForwardListItemWithSnapshotStore:(id)a0 inPageCache:(char)a1;
- (void)requestSendFeedbackWithCompletionHandler:(id /* block */)a0;
- (void)setConsentedToFirstTimeAlert:(char)a0;
