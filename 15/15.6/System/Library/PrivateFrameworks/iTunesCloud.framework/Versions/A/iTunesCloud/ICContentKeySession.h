...
@interface ICContentKeySession : NSObject <AVContentKeySessionDelegate> {
    ...
    char _automaticKeyRenewalPaused;
    char _active;
...
@property (nonatomic) char bypassCache;
@property (nonatomic) char requestOfflineKeys;
@property (nonatomic) char allowFallbackToStreamingKeys;
@property (nonatomic) char isStoreKeyServer;
...
- (char)_isPrefetchKey:(id)a0;
- (void)_finishProcessingKeyWithIdentifier:(id)a0;
- (void)_invalidateKeyWithIdentifier:(id)a0 forAdamID:(id)a1 offline:(char)a2 usingAccountDSID:(id)a3 keyData:(id)a4 withCompletion:(id /* block */)a5;
- (void)_performKeyDeliveryRequestForContentKeyRequest:(id)a0 persistResponse:(char)a1 isRenewal:(char)a2;
- (void)_processKeyWithIdentifier:(id)a0;
- (void)_renewExpiredKeys;
- (void)_scheduleKeyRefreshTimer;
- (void)addAsset:(id)a0 shouldPreloadKeys:(char)a1 waitForKeyIdentifiers:(char)a2;
...
- (void)stopSessionInvalidatingKeys:(char)a0 withCompletion:(id /* block */)a1;
