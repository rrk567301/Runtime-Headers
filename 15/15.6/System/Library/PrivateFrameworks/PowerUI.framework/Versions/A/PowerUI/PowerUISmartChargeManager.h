...
@property (nonatomic) char signalsIgnored;
...
@property (nonatomic) char overrideAllSignals;
@property (nonatomic) char encounteredTLCDuringTopOff;
@property (nonatomic) char isDesktopDevice;
...
@property char remoteOBCEngaged;
...
@property (nonatomic) char OBCReengagementEvaluated;
...
@property (nonatomic) char manualChargeLimitWasEverEnabled;
...
@property (nonatomic) char becameOBCEligible;
@property (nonatomic) char reachedTargetSoC;
...
- (void)resetState;
- (void)cacheCurrentDEoCBehaviorForced:(char)a0;
- (void)handleCallback:(char)a0;
...
- (id)adjustedFullChargeDeadlineWithSignals:(id)a0 withDesktopMode:(char)a1 withFullChargeDeadline:(id)a2 withResult:(id)a3;
- (id)chargePrediction:(unsigned long long)a0 fullyCharged:(char)a1 previousCheckpoint:(unsigned long long)a2 predictor:(unsigned long long)a3;
...
- (double)durationToFullChargeFromBatteryLevel:(unsigned long long)a0 includeTLCDelay:(char)a1;
...
- (void)evaluateChargeLimitRecommendationForced:(char)a0;
- (void)evaluateIfDEoCDevice;
- (void)evaluateIfDesktopDevice;
- (void)evaluateIfDesktopDeviceForced:(char)a0;
...
- (void)handleNewBatteryLevel:(int)a0 whileExternalConnected:(char)a1 fullyCharged:(char)a2;
- (void)handleNewBatteryLevelForMCL:(int)a0 whileExternalConnected:(char)a1;
...
- (void)postOBCNotificationWithTopOff:(char)a0;
...
- (void)setCheckpoint:(unsigned long long)a0 withSelector:(SEL)a1 forceWrite:(char)a2;
...
- (void)setTemporarilyDisabled:(char)a0 until:(id)a1;
- (id)shouldDisableChargingAsOfDate:(id)a0 atBatteryLevel:(unsigned long long)a1 overrideAllSignals:(char)a2 withPredictor:(id)a3 bypassSaved:(char)a4;
- (id)shouldDisableChargingAtBatteryLevel:(unsigned long long)a0 withPredictor:(id)a1;
- (char)shouldDisableChargingOverrideModel:(unsigned long long)a0;
- (char)shouldOBCRoutineReengage;
...
- (void)updateNotificationSettings:(char)a0;
- (void)updateSleepWakeNotificationsState;
- (void)updateTimeLineForCurrentBatteryLevel:(int)a0 withIsExternalConnected:(char)a1 forDate:(id)a2;
