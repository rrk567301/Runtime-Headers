...
@property (nonatomic) char wantsPseudonymUpdates;
...
@property (nonatomic, getter=isPretendingToBeFull) char pretendingToBeFull;
@property (nonatomic) char manuallyAckMessages;
@property (nonatomic) char needsLaunchOnNearbyDevicesChanged;
...
+ (void)serviceWithIdentifier:(id)a0 commands:(id)a1 manuallyAckMessages:(char)a2 delegateContext:(id)a3 completion:(id /* block */)a4;
...
- (char)_canAccount:(id)a0 sendWithFromID:(id)a1;
...
- (char)canSendMessageWithAccount:(id)a0 toDestination:(id)a1;
- (char)cancelIdentifier:(id)a0 error:(id *)a1;
- (char)cancelMessageWithOptions:(id)a0 destination:(id)a1 identifier:(id *)a2 error:(id *)a3;
- (char)cancelMessageWithOptions:(id)a0 destinations:(id)a1 identifier:(id *)a2 error:(id *)a3;
- (char)cancelMessageWithOptions:(id)a0 identifier:(id *)a1 error:(id *)a2;
- (char)cancelOpportunisticDataWithIdentifier:(id)a0 error:(id *)a1;
...
- (void)connection:(id)a0 didCancelMessageWithSuccess:(char)a1 error:(id)a2 identifier:(id)a3;
...
- (void)connection:(id)a0 identifier:(id)a1 didSendWithSuccess:(char)a2 error:(id)a3 context:(id)a4;
...
- (void)connection:(id)a0 isActiveChanged:(char)a1;
- (void)connection:(id)a0 messageIdentifier:(id)a1 alternateCallbackID:(id)a2 updatedWithResponseCode:(long long)a3 error:(id)a4 lastCall:(char)a5 messageContext:(id)a6;
...
- (void)didSwitchActivePairedDevice:(id)a0 forService:(id)a1 wasHandled:(char *)a2;
- (void)finishedProvisioningPseudonym:(id)a0 success:(char)a1 error:(id)a2 forRequestUUID:(id)a3;
- (void)finishedRenewingPseudonym:(id)a0 success:(char)a1 error:(id)a2 requestUUID:(id)a3;
- (void)finishedReportingRequestUUID:(id)a0 withError:(id)a1;
- (void)finishedRevokingPseudonymWithSuccess:(char)a0 error:(id)a1 requestUUID:(id)a2;
- (void)finishedSigningForRequest:(id)a0 signedData:(id)a1 error:(id)a2;
- (void)finishedVerifyingSignedDataForRequest:(id)a0 success:(char)a1 error:(id)a2;
- (char)getProgressUpdateForIdentifier:(id)a0 error:(id *)a1;
- (id)groupContextController:(id)a0 accountsForAlises:(id)a1;
- (void)groupContextController:(id)a0 didCreateGroup:(id)a1;
- (id)initWithService:(id)a0 commands:(id)a1 manuallyAckMessages:(char)a2 delegateContext:(id)a3;
- (id)linkedDeviceForFromID:(id)a0 withRelationship:(long long)a1;
- (id)linkedDevicesWithRelationship:(long long)a0;
- (char)noteMetricOfType:(long long)a0 context:(id)a1;
- (void)performGroupTask:(id /* block */)a0;
- (SEL)protobufActionForType:(unsigned short)a0 isResponse:(char)a1;
...
- (char)sendAccessoryData:(id)a0 toAccessoryID:(id)a1 accessToken:(id)a2 options:(id)a3 identifier:(id *)a4 error:(id *)a5;
- (void)sendAckForMessageWithContext:(id)a0;
- (char)sendAheadGroup:(id)a0 priority:(long long)a1 options:(id)a2 identifier:(id *)a3 completion:(id /* block */)a4;
- (char)sendCertifiedDeliveryReceipt:(id)a0;
- (char)sendData:(id)a0 fromAccount:(id)a1 toDestinations:(id)a2 priority:(long long)a3 options:(id)a4 identifier:(id *)a5 error:(id *)a6;
- (char)sendData:(id)a0 priority:(long long)a1 options:(id)a2 identifier:(id *)a3 error:(id *)a4;
- (void)sendEncryptedOffGridMessage:(id)a0 options:(id)a1 completion:(id /* block */)a2;
- (char)sendInvitation:(id)a0 fromAccount:(id)a1 toDestination:(id)a2 options:(id)a3 identifier:(id *)a4 error:(id *)a5;
- (char)sendInvitationUpdate:(id)a0 fromAccount:(id)a1 toDestination:(id)a2 options:(id)a3 identifier:(id *)a4 error:(id *)a5;
- (char)sendMessage:(id)a0 fromAccount:(id)a1 toDestinations:(id)a2 priority:(long long)a3 options:(id)a4 identifier:(id *)a5 error:(id *)a6;
- (char)sendOpportunisticData:(id)a0 options:(id)a1 identifier:(id)a2 error:(id *)a3;
- (char)sendProtobuf:(id)a0 fromAccount:(id)a1 toDestinations:(id)a2 priority:(long long)a3 options:(id)a4 identifier:(id *)a5 error:(id *)a6;
- (char)sendResourceAtURL:(id)a0 metadata:(id)a1 fromAccount:(id)a2 toDestinations:(id)a3 priority:(long long)a4 options:(id)a5 identifier:(id *)a6 error:(id *)a7;
- (char)sendServerMessage:(id)a0 command:(id)a1 fromAccount:(id)a2;
- (void)sendServiceUpdateMessage:(id)a0 options:(id)a1 completion:(id /* block */)a2;
- (void)service:(id)a0 linkedDevicesUpdated:(id)a1;
- (void)service:(id)a0 restrictionReasonChanged:(unsigned long long)a1;
...
- (void)setPreferInfraWiFi:(char)a0;
- (void)setProtobufAction:(SEL)a0 forProtobufType:(unsigned short)a1 isResponse:(char)a2;
- (char)setWakingPushPriority:(long long)a0 error:(id *)a1;
...
- (char)updateSubServices:(id)a0 forDevice:(id)a1;
