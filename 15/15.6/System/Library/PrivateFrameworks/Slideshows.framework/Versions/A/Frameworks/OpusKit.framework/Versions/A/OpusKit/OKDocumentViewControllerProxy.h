@class NSMutableDictionary, OKJavaScriptContext, NSObject, OKProducer, NSMutableArray, OKDocumentView, OKDocumentCouchStep, NSString, NSRecursiveLock, OFNSOperationQueue, OFNSView, OKNavigatorViewController, OKDocument;
@protocol NSObject, OKDocumentViewControllerDelegate, OS_dispatch_queue, OKDocumentViewControllerEditingDelegate;

@interface OKDocumentViewControllerProxy : OFNSViewController <OKCollectionProxyDataSource, OKActionResponderPrivate, OKDocumentViewControllerProxyExport, OKActionResponder> {
    NSString *_mainNavigatorName;
    char _hasStartedCouchBefore;
    NSMutableArray *_producerPlugins;
    NSMutableDictionary *_navigatorViewControllers;
    OKJavaScriptContext *_jsContext;
    NSRecursiveLock *_jsEvaluationInitRecursiveLock;
    NSObject<OS_dispatch_queue> *_jsEvaluationScriptQueue;
    char _jsEvaluationInit;
    long long _couchCancellationCount;
    char _hasPreparedCouch;
    NSMutableArray *_audioDuckingStack;
    NSMutableDictionary *_controllerAudioTrackIDs;
    char _isPaused;
    id<OKDocumentViewControllerDelegate> _delegate;
    id<OKDocumentViewControllerEditingDelegate> _editingDelegate;
    id<NSObject> _activityLatency;
}

@property (retain, nonatomic) OKDocumentCouchStep *lastCouchStep;
@property (nonatomic) unsigned long long couchStepMode;
@property (nonatomic) double currentCouchStepDuration;
@property (readonly, retain, nonatomic) OFNSOperationQueue *highSpeedOperationQueue;
@property (readonly, retain, nonatomic) OFNSOperationQueue *consumingOperationQueue;
@property (nonatomic) char isExporting;
@property (nonatomic) unsigned long long playbackMode;
@property (retain, nonatomic) OKNavigatorViewController *mainNavigatorViewController;
@property (readonly, retain, nonatomic) OKJavaScriptContext *jsContext;
@property (nonatomic) char preloadAllLowThumbnails;
@property (readonly, retain, nonatomic) OKDocumentView *documentView;
@property (readonly, retain, nonatomic) OKDocument *document;
@property (readonly, retain, nonatomic) OKProducer *producer;
@property (readonly, retain, nonatomic) NSString *resolution;
@property (readonly, nonatomic) struct CGSize { double width; double height; } resolutionSize;
@property (readonly, nonatomic) struct CGSize { double width; double height; } displaySize;
@property (readonly, nonatomic) char keepAspectRatio;
@property (nonatomic) char allowsResizing;
@property (nonatomic) char interactivityEnabled;
@property (nonatomic) char stretchableWithoutReauthoring;
@property (nonatomic) char couchPotatoModeEnabled;
@property (nonatomic) char couchPotatoModeLoops;
@property (nonatomic) char couchPotatoRewindAudioOnLoop;
@property (nonatomic) float couchPotatoAudioFadeOutDuration;
@property (nonatomic) double audioVolume;
@property (nonatomic) id<OKDocumentViewControllerDelegate> delegate;
@property (nonatomic) id<OKDocumentViewControllerEditingDelegate> editingDelegate;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
@property (readonly) OFNSView *actionView;

+ (Class)viewClass;
+ (void)setupJavascriptContext:(id)a0;

- (void)dealloc;
- (id)valueForUndefinedKey:(id)a0;
- (char)isPaused;
- (void)willMoveToParentViewController:(id)a0;
- (void)commonInit;
- (void)didMoveToParentViewController:(id)a0;
- (char)isEditable;
- (void)viewDidLoad;
- (void)viewWillAppear:(char)a0;
- (void)viewDidAppear:(char)a0;
- (void)viewDidDisappear:(char)a0;
- (void)viewDidLayoutSubviews;
- (void)viewWillDisappear:(char)a0;
- (void)viewWillLayoutSubviews;
- (id)initWithDocument:(id)a0;
- (id)evaluateScript:(id)a0;
- (void)didReceiveMemoryWarning;
- (void)audioFinishedPlayingForTrackID:(id)a0;
- (char)prepareWithProgressBlock:(id /* block */)a0 error:(id *)a1;
- (void)instantPause;
- (void)instantResume;
- (void)navigateToItemAtKeyPath:(id)a0 animated:(char)a1 completionHandler:(id /* block */)a2;
- (char)sendAction:(id)a0 toTarget:(id)a1;
- (void)fadeAllAudioTracksWithDuration:(double)a0;
- (char)performActionScript:(id)a0 withAction:(id)a1;
- (void)_addSubWidgetViewsFromSubWidgets:(id)a0 toWidgetView:(id)a1;
- (id)_nextCouchStep:(id)a0;
- (struct CGPoint { double x0; double x1; })convertLocationInWindow:(struct CGPoint { double x0; double x1; })a0 toPage:(id)a1;
- (void)_couchPotatoPlaybackFinished;
- (void)_delayedPrepareForRefresh;
- (struct CGSize { double x0; double x1; })_fitDisplaySizeForRenderSize:(struct CGSize { double x0; double x1; })a0;
- (void)_notifyCouchPotatoPlaybackFinished;
- (void)_playLoopedCouchPotatoStep;
- (char)_registerProducerPlugin:(id)a0;
- (id)_registeredProducerPluginWithIdentifier:(id)a0;
- (id)actionBindingForAction:(id)a0 isTouchCountAgnostic:(char)a1;
- (void)addActionBinding:(id)a0 scope:(unsigned long long)a1;
- (id)allActionBindings;
- (id)allKeysForDictionaryProxy:(id)a0;
- (void)audioStartedPlayingForTrackID:(id)a0 withAVAsset:(id)a1 andVolume:(float)a2;
- (void)audioStartedPlayingForTrackID:(id)a0 withMediaURL:(id)a1 andVolume:(float)a2;
- (id)beginAudioForController:(id)a0 withAVAsset:(id)a1 andVolume:(float)a2;
- (id)beginAudioForController:(id)a0 withURL:(id)a1 andVolume:(float)a2;
- (id)cachedNavigatorPageViewControllers;
- (char)canEditTextForWidget:(id)a0;
- (char)canPanMediaForWidget:(id)a0;
- (char)canStartCouchPotatoPlayback;
- (void)cancelCouchPotatoPlayback;
- (void)cancelCouchPotatoWithAction:(id)a0;
- (void)cleanCouchLabelHUDView;
- (struct CGPoint { double x0; double x1; })convertLocationInWindow:(struct CGPoint { double x0; double x1; })a0 toWidget:(id)a1;
- (struct CGPoint { double x0; double x1; })convertPointToBackingLayout:(struct CGPoint { double x0; double x1; })a0 fromPage:(id)a1;
- (struct CGPoint { double x0; double x1; })convertPointToBackingLayout:(struct CGPoint { double x0; double x1; })a0 fromWidget:(id)a1;
- (unsigned long long)countOfDictionaryProxy:(id)a0;
- (void)decreaseCouchPotatoCancellationCounter;
- (id)deepestDisplayedPageViewController;
- (id)dictionaryProxy:(id)a0 objectForKey:(id)a1;
- (void)didChangeTextForWidget:(id)a0 toSettings:(id)a1;
- (id)displayedPageViewControllers;
- (double)documentViewScale;
- (id)dynamicsPushBehaviors;
- (void)endAudioForController:(id)a0 andTrackID:(id)a1;
- (id)evaluateScript:(id)a0 withCompletionBlock:(id /* block */)a1;
- (id)evaluateScript:(id)a0 withInfoDictionary:(id)a1 andCompletionBlock:(id /* block */)a2 forNavigatorViewController:(id)a3 pageViewController:(id)a4 andWidgetView:(id)a5;
- (void)fadeMainNavigatorTracksWithDuration:(double)a0;
- (void)increaseCouchPotatoCancellationCounter;
- (id)initWithDocument:(id)a0 andMainNavigatorName:(id)a1;
- (struct CGSize { double x0; double x1; })layoutFactor;
- (id)navigatorViewControllerForNavigatorWithName:(id)a0;
- (void)playNextCouchPotatoStep;
- (void)prepareCouchPotatoPlayback;
- (void)prepareForRefreshWithDelay:(double)a0;
- (void)removeActionBinding:(id)a0;
- (void)removeAllActionBindings;
- (void)restartCouchPotatoPlayback;
- (void)setupCouchLabelHUDView:(id /* block */)a0;
- (char)shouldChangeTextForWidget:(id)a0 toSettings:(id)a1;
- (char)shouldPanMediaForWidget:(id)a0 toOffset:(struct CGPoint { double x0; double x1; })a1;
- (void)startCouchPotatoPlayback;
- (void)startCouchPotatoPlaybackWithName:(id)a0;
- (void)updateDisplayResolution;
- (void)updateMotion;
- (id)widgetViewFromWidget:(id)a0;
- (void)willChangeFocusForTextWidget:(id)a0 toFocusState:(char)a1;

@end
