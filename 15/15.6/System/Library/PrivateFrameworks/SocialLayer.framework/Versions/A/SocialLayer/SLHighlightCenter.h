...
+ (char)isEnabled;
+ (id)rapportClient;
+ (char)isAutomaticSharingEnabled;
+ (id)SLVariantDefault;
+ (id)SLVariantNewsToday;
+ (char)_bundleIDExistsInPreferences:(id)a0;
+ (void)_checkAndInitializeBundleIDToAppPreferences:(id)a0;
+ (char)_isShareWithYouEnabled;
+ (char)_isShareWithYouOnboarded;
+ (id)highlightCenterQueue;
+ (char)shouldShowOnboardingShieldView;
