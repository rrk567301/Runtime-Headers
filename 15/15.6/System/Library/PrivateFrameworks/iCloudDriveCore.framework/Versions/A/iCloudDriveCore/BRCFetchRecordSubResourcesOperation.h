...
@interface BRCFetchRecordSubResourcesOperation : _BRCOperation <BRCOperationSubclass> {
    ...
    char _isDeltaSync;
    char _canSaveDirectly;
    char _canSaveDirectlyWithDeletes;
    char _contentRecordsFetchedInline;
...
- (id)createActivity;
- (char)_willFetchRecordForDeserialize:(id)a0;
...
- (char)_hasRecordsPendingSubResources;
- (char)_markSubResourceRecordAsFetchedIfNecessary:(id)a0;
...
- (id)initAndScheduleWithServerZone:(id)a0 parentOperation:(id)a1 pendingChangesStream:(id)a2 contentRecordsFetchedInline:(char)a3 fetchResourcesHandler:(id)a4 sessionContext:(id)a5;
- (id)movedZoneNames;
- (void)notifyWhenCaughtUp:(char)a0 whenRecordsAreFetchedAndFinish:(id /* block */)a1;
- (void)notifyWhenRecordsAreFetchedAndFinish:(id /* block */)a0;
- (void)recordIDWasDeleted:(id)a0;
- (char)saveRecordsWithQueryCursor:(id)a0;
- (char)saveRecordsWithServerChangeToken:(id)a0 clientChangeToken:(long long)a1 syncStatus:(long long)a2;
