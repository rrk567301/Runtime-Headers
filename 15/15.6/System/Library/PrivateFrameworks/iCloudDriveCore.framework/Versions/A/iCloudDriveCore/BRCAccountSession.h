...
@property (readonly, nonatomic) char hasOptimizeStorageEnabled;
...
+ (char)_checkIntegrity:(id)a0 serverTruth:(char)a1 session:(id)a2 skipControlFiles:(char)a3 dbCreationDate:(id *)a4 deviceIDChanged:(char *)a5 error:(id *)a6;
+ (char)_openConnection:(id)a0 databaseName:(id)a1 baseURL:(id)a2 readonly:(char)a3 error:(id *)a4;
+ (char)_openConnection:(id)a0 serverTruth:(char)a1 databaseName:(id)a2 baseURL:(id)a3 initialVersion:(unsigned int *)a4 lastCurrentVersion:(unsigned int *)a5 error:(id *)a6;
+ (void)_registerLastBootIfNeeded:(id)a0 table:(id)a1 skipControlFiles:(char)a2 dbCreationDate:(id *)a3 deviceIDChanged:(char *)a4;
+ (char)_registerStaticDBFunctions:(id)a0 error:(id *)a1;
+ (char)_validateDatabase:(id)a0 baseURL:(id)a1 session:(id)a2 serverTruth:(char)a3 initialVersion:(unsigned int)a4 lastCurrentVersion:(unsigned int)a5 error:(id *)a6;
+ (char)openAndValidateDatabase:(id)a0 serverTruth:(char)a1 session:(id)a2 baseURL:(id)a3 skipControlFiles:(char)a4 initialVersion:(unsigned int *)a5 lastCurrentVersion:(unsigned int *)a6 dbCreationDate:(id *)a7 deviceIDChanged:(char *)a8 error:(id *)a9;
...
+ (char)upgradeOfflineDB:(id)a0 serverTruth:(char)a1 session:(id)a2 error:(id *)a3;
...
- (char)_addOrImportDomainIfNeededWithError:(id *)a0 domainAdded:(char *)a1 domainImported:(char *)a2;
- (void)_mutexZonesLock;
- (void)_prepareGroupContainerLocations;
- (void)_setupThrottles;
- (id)appLibraryByID:(id)a0;
- (char)fpfsMigrationStatusTTRTriggered;
- (id)__getOrCreateAppLibrary:(id)a0 rowID:(id)a1 alreadyExists:(char *)a2 withClientZone:(id)a3 createCZMMoved:(char)a4;
- (id)__getOrCreateClientZone:(id)a0 withServerZone:(id)a1 alreadyExists:(char *)a2;
- (id)__getOrCreateServerZone:(id)a0;
- (void)__registerServerZone:(id)a0 clientZone:(id)a1 appLibrary:(id)a2 isShared:(char)a3;
...
- (id)_computePathOfItemWithName:(id)a0 appLibrary:(id)a1 parentClientZoneRowID:(id)a2 parentID:(id)a3 type:(char)a4 needsLocalName:(char)a5 db:(id)a6;
- (void)_createAccountSupportPathIfNeeded:(id)a0 protectParent:(char)a1;
- (char)_createAppLibrary:(id)a0;
- (id)_createAppTelemetryFPFSMigrationInvestigation:(unsigned int)a0;
- (char)_createClientZone:(id)a0;
- (void)_createGroupContainerStageOfType:(short)a0;
- (char)_createPrivateAppLibrary:(id)a0;
- (id)_createRecoveryExecutedErrorForImportError:(id)a0;
- (char)_decorateFPFSDomainUserInfoWithError:(id *)a0;
- (char)_decorateUserInfoForFPFSDomain:(id)a0 panicOnDomainIDMismatch:(char)a1 withError:(id *)a2;
- (char)_deleteClientZone:(id)a0;
- (char)_dumpClientZone:(id)a0 toContext:(id)a1 includeAllItems:(char)a2 error:(id *)a3;
- (void)_dumpClientZoneBlockedState:(id)a0 toContext:(id)a1;
- (char)_dumpClientZoneStatus:(id)a0 toContext:(id)a1 error:(id *)a2;
- (char)_enableiCloudDesktopWithError:(id *)a0;
- (void)_enumerateAppLibraryRowIDs:(id)a0 usingBlock:(id /* block */)a1;
- (char)_finishClientTruthConnectionSetupWithError:(id *)a0;
- (char)_finishServerTruthConnectionSetupWithError:(id *)a0;
...
- (char)_openConnection:(id)a0 databaseName:(id)a1 readonly:(char)a2 error:(id *)a3;
...
- (void)_recoverContentPolicyIfNecessaryForItemID:(id)a0 appLibrary:(id)a1 isAppLibraryRoot:(char)a2 isDocumentsFolder:(char)a3 completion:(id /* block */)a4;
- (char)_recursivelyPrepareFolderForLogOutAtURL:(id)a0 pruneEmptyFolders:(char)a1 pruneEmptyTopLevelFolder:(char)a2 maxDepth:(unsigned long long)a3;
- (void)_refreshRootsCache;
- (void)_registerBackgroundXPCActivities;
- (char)_registerDynamicDBFunctions:(id)a0 error:(id *)a1;
...
- (void)_setDatabaseProfilingForDB:(id)a0 enabled:(char)a1 verbose:(char)a2;
- (void)_setICloudDesktopEnabled:(char)a0 forAccount:(id)a1;
- (char)_setupBackupDetector:(struct backup_detector { unsigned long long x0; unsigned long long x1; unsigned long long x2; } *)a0 error:(id *)a1;
- (char)_setupConnection:(id)a0 readonly:(char)a1 forChecker:(char)a2 error:(id *)a3;
...
- (char)_shouldPrivateAppLibraryBeCZMMoved:(id)a0;
- (char)_shouldTriggerTapToRadar:(long long)a0 daysSinceLastMigrationProgress:(long long)a1;
- (void)_startWatcher;
- (char)_stepBackupDetector:(struct backup_detector { unsigned long long x0; unsigned long long x1; unsigned long long x2; })a0 newState:(struct backup_detector { unsigned long long x0; unsigned long long x1; unsigned long long x2; } *)a1 error:(id *)a2;
...
- (void)_updateFPFSMigrationItemTypes:(char)a0 withStateKey:(id)a1 isPackage:(char)a2;
...
- (char)backupDatabaseToURL:(id)a0 error:(id *)a1;
- (char)backupDatabaseToURL:(id)a0 includeServer:(char)a1 doNotObfuscate:(char)a2 error:(id *)a3;
...
- (char)createServerZone:(id)a0;
- (void)dbBecameCorruptedWithDescription:(id)a0 error:(id)a1;
- (char)decorateUserInfoForFPFSDomain:(id)a0 withError:(id *)a1;
- (char)deleteServerZone:(id)a0;
...
- (char)dumpDatabaseToFileHandle:(id)a0 zoneName:(id)a1 includeAllItems:(char)a2 verbose:(char)a3 tracker:(id)a4 error:(id *)a5;
- (char)dumpFPFSMigrationStatusToFileHandle:(id)a0 tracker:(id)a1 includeNonMigratedItems:(char)a2 error:(id *)a3;
- (char)dumpFileCoordinationInfoToFileHandle:(id)a0 error:(id *)a1;
- (char)dumpItemsToHandle:(id)a0 itemType:(id)a1 tracker:(id)a2 error:(id *)a3;
- (char)dumpItemsURLsToHandle:(id)a0 itemRowIDs:(id)a1 error:(id *)a2;
- (void)dumpMiscOperationsToContext:(id)a0;
- (char)dumpStatusToFileHandle:(id)a0 zoneName:(id)a1 tracker:(id)a2 error:(id *)a3;
...
- (char)enumerateItemsWithType:(id)a0 itemRowIDs:(id)a1 db:(id)a2 withBlock:(id /* block */)a3;
...
- (char)enumerateSharedToMeItemsWithBlock:(id /* block */)a0 error:(id *)a1;
- (id)fallbackAppLibraryForClientZone:(id)a0 extension:(id)a1;
- (id)fetchAccountWaitOperationWithAccountReady:(char *)a0;
...
- (id)getOrCreateAppLibraryAndPrivateZonesIfNecessary:(id)a0 appLibraryExists:(char *)a1;
...
- (char)globalID:(id)a0 isStrictChildOfGlobalID:(id)a1;
...
- (char)initializeOfflineDatabaseWhileUpgrading:(char)a0 error:(id *)a1;
- (char)initializeOfflineDatabaseWhileUpgrading:(char)a0 loadClientState:(char)a1 error:(id *)a2;
- (char)initializeOfflineDatabaseWhileUpgrading:(char)a0 loadClientState:(char)a1 forDBDump:(char)a2 error:(id *)a3;
- (void)invalidateExtensionXPCClient;
- (char)isConsolidatedAccount;
- (char)isMigratingToFPFS;
- (char)isOptimizeStorage;
...
- (id)newConnectionWithLabel:(id)a0 readonly:(char)a1 error:(id *)a2;
...
- (char)openAndValidateDatabase:(id)a0 serverTruth:(char)a1 initialVersion:(unsigned int *)a2 lastCurrentVersion:(unsigned int *)a3 deviceIDChanged:(char *)a4 error:(id *)a5;
- (char)openDBForNewDomain:(char)a0 deviceIDChanged:(char *)a1 withError:(id *)a2;
- (void)openForDumpingWithAccountDSID:(id)a0;
- (void)openForSendingTelemetry;
- (char)openWithError:(id *)a0 pushWorkloop:(id)a1;
...
- (char)reimportFPFSDomainWithError:(id *)a0;
- (void)resumeIfNecessary;
- (char)runDatabaseFixup:(unsigned long long)a0 error:(id *)a1;
- (char)runDatabaseFixups;
- (char)runDatabaseFixupsForDB:(id)a0 serverTruth:(char)a1;
- (char)saveClientZoneToDB:(id)a0;
...
- (void)setOptimizeStorageEnabled:(char)a0 forKey:(id)a1 synchronous:(char)a2;
- (void)setOptimizeStorageEnabled:(char)a0 synchronous:(char)a1;
