@class NSExtension, NSURL, NSDictionary, NSObject, AUParameterTree, NSMutableArray, NSString, AUAudioUnitBusArray, NSViewController, NSArray, AUv3HostIdentifier, NSXPCConnection, AUAudioUnitPreset, NSUUID;
@protocol OS_dispatch_queue, OS_dispatch_source;

@interface AUAudioUnit : NSObject {
    struct weak_ptr<APComponent> { struct APComponent *__ptr_; struct __shared_weak_count *__cntrl_; } _component;
...
@property char requestViewControllerSynchronously;
...
@property (readonly, copy) NSString *audioUnitShortName;
@property (readonly) unsigned int componentVersion;
@property (readonly) char renderResourcesAllocated;
@property (readonly) AUAudioUnitBusArray *inputBusses;
@property (readonly) AUAudioUnitBusArray *outputBusses;
@property (readonly) id /* block */ renderBlock;
@property (readonly) id /* block */ scheduleParameterBlock;
@property unsigned int maximumFramesToRender;
@property (retain) AUParameterTree *parameterTree;
@property (readonly) char allParameterValues;
@property (readonly, getter=isMusicDeviceOrEffect) char musicDeviceOrEffect;
@property (readonly) long long virtualMIDICableCount;
@property (readonly) id /* block */ scheduleMIDIEventBlock;
@property (readonly) id /* block */ scheduleMIDIEventListBlock;
@property (readonly, copy) NSArray *MIDIOutputNames;
@property (readonly) char providesUserInterface;
@property (copy) id /* block */ MIDIOutputEventBlock;
@property (copy) id /* block */ MIDIOutputEventListBlock;
@property (readonly) int AudioUnitMIDIProtocol;
@property int hostMIDIProtocol;
@property (copy) NSDictionary *fullState;
@property (copy) NSDictionary *fullStateForDocument;
@property (readonly, copy) NSArray *factoryPresets;
@property (readonly, copy) NSArray *userPresets;
@property (readonly) char supportsUserPresets;
@property (readonly) char isLoadedInProcess;
@property (retain) AUAudioUnitPreset *currentPreset;
@property (readonly) double latency;
@property (readonly) double tailTime;
@property long long renderQuality;
@property char shouldBypassEffect;
@property (readonly) char canProcessInPlace;
@property (getter=isRenderingOffline) char renderingOffline;
@property (readonly, copy) NSArray *channelCapabilities;
@property (copy) id /* block */ musicalContextBlock;
@property (copy) id /* block */ transportStateBlock;
@property (copy) NSString *contextName;
@property (readonly) NSArray *migrateFromPlugin;
@property (readonly) char supportsMPE;
@property (copy) NSArray *channelMap;
...
+ (char)_deleteUserPreset:(id)a0 error:(id *)a1;
...
+ (char)_saveUserPreset:(id)a0 state:(id)a1 error:(id *)a2;
+ (id)auAudioUnitForAudioUnit:(struct OpaqueAudioComponentInstance { } *)a0;
+ (id)audioComponentBundle:(id)a0;
...
+ (void)instantiateWithComponentDescription:(struct AudioComponentDescription { unsigned int x0; unsigned int x1; unsigned int x2; unsigned int x3; unsigned int x4; })a0 options:(unsigned int)a1 connectionProvider:(struct function<NSXPCConnection *(NSUUID *)> { struct __value_func<NSXPCConnection *(NSUUID *)> { struct type { unsigned char x0[32]; } x0; void *x1; } x0; })a2 completionHandler:(id /* block */)a3;
...
- (char)deleteUserPreset:(id)a0 error:(id *)a1;
...
- (char)disableProfile:(id)a0 cable:(unsigned char)a1 onChannel:(unsigned char)a2 error:(id *)a3;
- (char)enableProfile:(id)a0 cable:(unsigned char)a1 onChannel:(unsigned char)a2 error:(id *)a3;
- (void *)eventSchedule;
...
- (char)saveUserPreset:(id)a0 error:(id *)a1;
...
- (void)setLocalCachingDisabled:(char)a0;
- (void)setRenderResourcesAllocated:(char)a0;
...
- (char)shouldChangeToFormat:(id)a0 forBus:(id)a1;
