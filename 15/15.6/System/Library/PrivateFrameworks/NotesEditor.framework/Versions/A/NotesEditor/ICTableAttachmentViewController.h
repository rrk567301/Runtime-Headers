@class ICTableUndoTarget, ICTableAutoScroller, NSMutableOrderedSet, NSDictionary, ICTableColumnTextView, NSMutableSet, NSMutableArray, ICTableContentView, ICTableAccessibilityController, NSIndexSet, ICTableClipView, NSMapTable, ICTableAttachmentSelection, NSScrollView, ICTableAttachmentProvider, ICTable, ICTableAttachmentView, ICTTTextStorage, ICDimensionSumCache, ICTableSelectionView, ICTableTextViewManager, ICLayoutManager, NSArray, NSMutableDictionary, ICAppearanceInfo, NSView, NSString, NSObject, ICTableScrollView, ICMacBaseTextView, ICTableSelectionKnob, ICTableLayoutManager, ICTK2TextLayoutManager, NSUUID, NSUndoManager, ICTableColumnRowButton;
...
@property (nonatomic) char didRecentlyAutoAddRow;
...
@property (nonatomic) char isPerformingInitialLayout;
...
@property (nonatomic) char performedInitialLayout;
@property (nonatomic) char makingCellFirstResponder;
...
@property (nonatomic) char shouldBeginInitialEditing;
...
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
@property (nonatomic) char preventScrolling;
@property (readonly, nonatomic) char isNoteEditable;
@property (readonly, nonatomic) ICMacBaseTextView *noteTextView;
@property (readonly, nonatomic) double availableWidth;
@property (readonly, nonatomic) char canStyleText;
...
@property (nonatomic) char lockSelection;
...
- (void)setSelectionBIUSStyle:(unsigned long long)a0 toggleOn:(char)a1;
- (void)toggleEmphasis:(id)a0;
- (void)updateHeightCacheForColumn:(id)a0 row:(id)a1;
- (id)RTFDataForSelection;
- (void)disableItalics:(id)a0;
- (void)enableBoldface:(id)a0;
...
- (char)becomeFirstResponderForAuxStyling;
...
- (char)canIndentByamount:(long long)a0;
- (char)canOutdent;
...
- (char)cursorPrefersWordBoundary;
...
- (void)disableBoldface:(id)a0;
- (void)disableStrikethrough:(id)a0;
- (void)disableUnderline:(id)a0;
- (void)dragBeganOnColumnOrRow:(char)a0 atLocation:(struct CGPoint { double x0; double x1; })a1;
- (void)dragEndedOnColumnOrRow:(char)a0 atLocation:(struct CGPoint { double x0; double x1; })a1;
- (void)dragMovedOnColumnOrRow:(char)a0 atLocation:(struct CGPoint { double x0; double x1; })a1;
- (id)dragSnapshotFromRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 afterScreenUpdates:(char)a1;
- (void)enableItalics:(id)a0;
- (void)enableStrikethrough:(id)a0;
- (void)enableUnderline:(id)a0;
- (void)endCellEditingSessionWithTextView:(id)a0;
- (void)extendCellRangeSelectionInDirection:(unsigned long long)a0 toEnd:(char)a1;
...
- (char)makeSpaceToPasteSourceTable:(id)a0;
- (void)moveDownCell;
...
- (void)pasteTable:(id)a0 atColumnIndex:(unsigned long long)a1 rowIndex:(unsigned long long)a2 shouldSetSelectionToPastedRange:(char)a3;
...
- (void)redrawAndSave;
- (char)resignFirstResponderForAuxStyling;
- (void)reverseTableColumnDirection:(id)a0;
- (void)saveAttachmentChanges;
- (void)saveAttachmentChangesInBackground:(char)a0;
- (void)saveOnMainThread;
- (void)scrollToCaretIfNeededForTextView:(id)a0;
- (void)scrollToRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 animated:(char)a1 completion:(id /* block */)a2;
- (void)scrollToSelectionInTextView:(id)a0 animated:(char)a1 completion:(id /* block */)a2;
...
- (void)setHidden:(char)a0 forColumn:(id)a1;
- (void)setHidden:(char)a0 forRow:(id)a1;
- (void)setNeedsSave;
- (void)setSelectionAlignment:(long long)a0;
- (void)setSelectionBIUSStyle:(unsigned long long)a0 toggleOn:(char)a1 withColumns:(id)a2 rows:(id)a3;
...
- (void)speakAccessibilityExitedTableAnnouncementAndDidRemoveRow:(char)a0;
- (void)tableAttachmentDidChange;
- (void)tableAttachmentProviderDidMergeTable:(id)a0;
- (void)tableAttachmentProviderWillMergeTable:(id)a0;
