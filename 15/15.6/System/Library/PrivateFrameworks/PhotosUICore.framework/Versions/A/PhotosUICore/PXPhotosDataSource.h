@class NSDictionary, NSHashTable, PXLibraryFilterState, PXLIFOQueue, NSMutableArray, NSString, NSMutableSet, NSArray, NSMutableDictionary, PHAsset, PHObjectChangeDetails, PHFetchResult, NSPredicate, PXPhotosDataSourceSectionCache, NSObject, NSSet, PHPhotoLibrary, PXPhotosAssetsFetcher, PHAssetCollection, PXSharedLibraryStatusProvider, PHSocialGroup, PHCollection, NSMutableOrderedSet, NSNumber, PXBackgroundFetchToken;
...
@interface PXPhotosDataSource : NSObject <PXPhotoLibraryUIChangeObserver, PXChangeObserver> {
    ...
    char _inaccurateAssetCollectionsNeedsUpdate;
    ...
    char _backgroundFetchOriginSectionChanged;
    char _needToStartBackgroundFetch;
    char _didFinishBackgroundFetch;
    PXBackgroundFetchToken *_backgroundFetchToken;
    char _interruptBackgroundFetch;
    char _pauseBackgroundFetchResultsDelivery;
    ...
    char _processAndPublishScheduledOnRunloop;
...
@property (class, readonly) long long limitedInitialFetchCountLimit;
@property (class, readonly) NSObject<OS_dispatch_queue> *sharedPrefetchQueue;

@property (nonatomic, setter=_setPreviousCollectionsCount:) unsigned long long _previousCollectionsCount;
@property (nonatomic) char isBackgroundFetching;
...
@property (nonatomic) char allowNextChangeDeliveryOnAllRunLoopModes;
@property (nonatomic) char allowSlowFetchesOnClientQueue;
@property (readonly, nonatomic) PHFetchResult *emptyAssetsFetchResult;
@property (readonly, nonatomic) char wantsCurationForFirstAssetCollection;
...
@property (readonly, nonatomic) char isImmutable;
@property (readonly, nonatomic) char isEmpty;
@property (readonly, nonatomic) char containsMultipleAssets;
...
- (char)_addResultTuple:(id)a0 forAssetCollection:(id)a1 preparedResultRecord:(id)a2 toMutableResultRecord:(id)a3;
- (void)_adoptExistingAssetCollectionFetchResults:(id)a0;
- (char)_allSectionsConsideredAccurate;
- (id)_allowedUUIDsForAssetCollection:(id)a0;
- (char)_areFiltersDisabledForAssetCollection:(id)a0;
- (char)_areSecondaryFetchesEnabled;
- (unsigned long long)_assetCollectionFetchStatus:(id)a0;
- (id)_assetContainerForAssetCollection:(id)a0;
...
- (id)_createDataSourceWithAssetsCollections:(id)a0 existingAssetCollectionFetchResults:(id)a1;
...
- (id)_fetchTupleForAssetCollection:(id)a0 calledOnClientQueue:(char)a1 isLimitedInitialFetch:(char)a2;
- (id)_fetcherConfigurationForAssetCollection:(id)a0 useCase:(long long)a1 isLimitedInitialFetch:(char)a2;
- (id)_filterPredicateForAssetCollection:(id)a0;
- (id)_finalFilterPredicateForAssetCollection:(id)a0;
- (char)_forceAccurateSection:(long long)a0 andNumberOfSurroundingSectionsWithContent:(long long)a1;
- (void)_getFetchLimit:(unsigned long long *)a0 fetchWithReverseSortOrder:(char *)a1 forAssetCollection:(id)a2 isLimitedInitialFetch:(char)a3;
...
- (char)_isFiltering;
- (char)_isResultTupleApplicable:(id)a0 forAssetCollection:(id)a1;
...
- (void)_prefetchIndexesByFetchResult:(id)a0 onlyThumbnails:(char)a1;
...
- (char)_reverseSortOrderForAssetCollection:(id)a0;
...
- (id)createDataSourceWithAssetsInSectionOfAsset:(id)a0 usingNewTransientAssetCollection:(char)a1;
- (id)curatedAssetsForAssetCollection:(id)a0;
- (id)curatedAssetsInSection:(long long)a0;
- (void)enumerateStartingAtIndexPath:(id)a0 reverseDirection:(char)a1 usingBlock:(id /* block */)a2;
...
- (char)forceAccurateAssetCollectionsIfNeeded:(id)a0 reloadChanges:(char)a1;
- (char)forceAccurateIndexPath:(id)a0 andAssetsBeforeAndAfter:(long long)a1;
- (char)forceAccurateSection:(long long)a0 andSectionsBeforeAndAfter:(long long)a1;
- (char)forceAccurateSectionsIfNeeded:(id)a0;
- (char)forceAccurateSectionsIfNeeded:(id)a0 reloadChanges:(char)a1;
...
- (char)isAssetAtIndexPathPartOfCuratedSet:(struct PXSimpleIndexPath { long long x0; long long x1; long long x2; long long x3; })a0;
- (char)isCuratedAssetsEmptyForAssetCollection:(id)a0;
- (char)isCuratedAssetsFutilelyForAssetCollection:(id)a0;
...
- (void)prefetchApproximateAssetsAtIndexPaths:(id)a0 reverseOrder:(char)a1;
- (void)prefetchAssetsAtIndexPaths:(id)a0 onlyThumbnailAssets:(char)a1;
...
- (void)setDisableFilters:(char)a0 forAssetCollection:(id)a1;
...
- (void)setWantsCuration:(char)a0 forAssetCollection:(id)a1;
- (void)setWantsCurationForAllCollections:(char)a0 collectionsToDiff:(id)a1;
- (void)startBackgroundFetchIfNeeded;
- (void)stopExcludingAssets:(id)a0;
- (void)stopForceIncludingAllAssets;
- (char)supportsDynamicBackgroundFetching;
- (id)uncuratedAssetsForAssetCollection:(id)a0;
- (id)uncuratedAssetsInSection:(long long)a0;
- (char)wantsCurationForAssetCollection:(id)a0;
