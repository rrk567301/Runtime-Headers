...
@interface PXZoomablePhotosLayout : PXGSplitLayout <PXAssetsDataSourceManagerObserver, PXChangeObserver, PXGDisplayAssetSource, PXZoomableInlineHeadersLayoutGeometrySource, PXGItemsLayoutDelegate, PXZoomablePhotosViewModelGeometryDelegate, PXCuratedLibraryBodyLayout, PXPhotosSectionedLayoutBody> {
    struct { unsigned long long needsUpdate; unsigned long long updated; char isPerformingUpdate; char willPerformUpdate; } _preUpdateFlags;
    ...
    char _scrolledWhileAnimatingZoom;
    char _performedInitialVisualShift;
    long long _visuallyStableAnchorItem;
    long long _visuallyStableAnchorItemDesiredColumn;
    char _visuallyStableAnchorAllowedCloseToTop;
    char _visuallyStableAnchorLastRowFilled;
    ...
    struct NSEdgeInsets { double top; double left; double bottom; double right; } _surroundingScrollableContent;
...
@property (readonly, nonatomic) char shouldAnimateTowardsAnchor;
@property (readonly, nonatomic) char wasTracking;
...
@property (nonatomic) struct NSEdgeInsets { double top; double left; double bottom; double right; } effectiveOverlayInsets;
@property (nonatomic) char wantsOverBackgroundFloatingHeaderAppearance;
@property (readonly, nonatomic) PXGBurstStackEffect *burstStackEffect;
@property (copy, nonatomic) id /* block */ effectProvider;
@property (nonatomic) char enableAddContentAccessoryItem;
@property (nonatomic) char miniModeIsAnchoringToBottom;
...
@property (nonatomic) struct NSEdgeInsets { double top; double left; double bottom; double right; } overlayInsets;
@property (nonatomic) struct NSEdgeInsets { double top; double left; double bottom; double right; } inlineHeaderOverlayInsets;
@property (nonatomic) char preventFillingTopAreaInsets;
...
+ (void)getBestAnchorColumns:(long long *)a0 forItemLayouts:(id)a1 startingAtBaseLayout:(id)a2 anchorItem:(long long)a3 normalizedScaleCenterInAnchor:(struct CGPoint { double x0; double x1; })a4 biasTowardsCenter:(char)a5;
...
- (char)_isAllowedToShiftItem:(long long)a0 inLayout:(id)a1;
...
- (struct NSEdgeInsets { double x0; double x1; double x2; double x3; })floatingHeadersMarginsForLevel:(unsigned long long)a0;
- (void)_callAddContentActionHandler;
- (void)_configureLayout:(id)a0;
- (void)_configureMiniModeSpriteModifierForLayout:(id)a0;
...
- (char)_getMiniModeInfo:(out struct { struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x0; struct CGPoint { double x0; double x1; } x1; double x2; struct _NSRange { unsigned long long x0; unsigned long long x1; } x3; } *)a0 forLayout:(id)a1;
- (char)_hasLastRowFilledStrategy;
...
- (char)_layoutAllowsCaptionsInSquare:(id)a0;
- (char)_layoutHasIndividualItems:(id)a0;
- (char)_layoutIsAspectFit:(id)a0;
- (char)_layoutIsQuiteTransparent:(id)a0;
- (char)_layoutWantsSingleDateTitle;
- (void)_modifyMiniModeSpritesForLayout:(id)a0 spriteIndexRange:(struct _PXGSpriteIndexRange { unsigned int x0; unsigned int x1; })a1 sprites:(struct { unsigned int x0; struct *x1; struct *x2; struct *x3; struct *x4; })a2;
- (void)_performAnchorAlignmentVisualShiftWithBaseLayout:(id)a0;
- (char)_performVisualShiftWithLayout:(id)a0 movingItem:(long long)a1 toColumn:(long long)a2;
...
- (char)_wantsDecorationForNumberOfColumns:(long long)a0;
- (char)_wantsMiniDecorationForNumberOfColumns:(long long)a0;
...
- (void)getLastItem:(long long *)a0 lastColumn:(long long *)a1 forBodyLayout:(id)a2;
...
- (void)willUpdate;
- (char)zoomablePhotosViewModelIsResetToInitialState:(id)a0;
- (void)zoomablePhotosViewModelResetToInitialState:(id)a0;
