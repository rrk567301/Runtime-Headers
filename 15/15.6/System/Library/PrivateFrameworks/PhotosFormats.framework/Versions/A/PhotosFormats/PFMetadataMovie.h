@class AVAsset, NSMutableDictionary, PFMetadataStateHandler;

@interface PFMetadataMovie : PFMetadata {
    AVAsset *_asset;
    NSMutableDictionary *_cached_values;
    PFMetadataStateHandler *_stateHandler;
}

+ (char)parseISO6709String:(id)a0 outLatitude:(double *)a1 outLongitude:(double *)a2;
+ (char)_channelLayoutUsesChannelDescriptions:(const struct AudioChannelLayout { unsigned int x0; unsigned int x1; unsigned int x2; struct AudioChannelDescription { unsigned int x0; unsigned int x1; float x2[3]; } x3[1]; } *)a0;
+ (char)_channelLayoutUsesHOA:(const struct AudioChannelLayout { unsigned int x0; unsigned int x1; unsigned int x2; struct AudioChannelDescription { unsigned int x0; unsigned int x1; float x2[3]; } x3[1]; } *)a0;
+ (void)_enumerateTracksOfType:(id)a0 onAsset:(id)a1 withBlock:(id /* block */)a2;
+ (struct AudioStreamBasicDescription { double x0; unsigned int x1; unsigned int x2; unsigned int x3; unsigned int x4; unsigned int x5; unsigned int x6; unsigned int x7; unsigned int x8; })audioStreamBasicDescriptionForAsset:(id)a0;
+ (id)exifTimezoneOffsetFromDateString:(id)a0 offsetInSeconds:(long long *)a1;
+ (void)firstAudioTrackForAsset:(id)a0 resultBlock:(id /* block */)a1;
+ (void)firstVideoTrackForAsset:(id)a0 resultBlock:(id /* block */)a1;

- (id)copyWithZone:(struct _NSZone { } *)a0;
- (char)isEqual:(id)a0;
- (void).cxx_destruct;
- (struct { long long x0; int x1; unsigned int x2; long long x3; })duration;
- (id)keywords;
- (id)title;
- (long long)orientation;
- (id)speed;
- (id)altitude;
- (id)transferFunction;
- (id)author;
- (id)audioSampleRate;
- (id)imageDirection;
- (char)isCinematicVideo;
- (id)lensModel;
- (id)nominalFrameRate;
- (id)software;
- (void)loadMetadataWithCompletionHandler:(id /* block */)a0;
- (id)colorPrimaries;
- (char)isDecodable;
- (char)isPlayable;
- (char)isProRes;
- (char)isMovie;
- (char)isSpatialMedia;
- (id)cameraModel;
- (char)isSloMo;
- (id)captionAbstract;
- (char)hasSpatialAudio;
- (id)hevcProfileInfo;
- (id)apacAudioTrackBedChannelCount;
- (id)apacAudioTrackChannelCount;
- (id)apacAudioTrackCodecProfileLevelDescription;
- (id)apacAudioTrackHoaChannelCount;
- (id)artworkContentDescription;
- (id)audioDataRate;
- (id)audioTrackFormat;
- (id)cameraMake;
- (id)contentTypeFromFastModernizeVideoMedia;
- (id)firstVideoTrackFormatDebugDescription;
- (id)focalLengthIn35mm;
- (id)initWithAVAsset:(id)a0 options:(unsigned short)a1 timeZoneLookup:(id)a2;
- (id)initWithAVURL:(id)a0 options:(unsigned short)a1 timeZoneLookup:(id)a2;
- (char)isHDR;
- (char)isSpatialOverCapture;
- (char)isTimelapse;
- (id)livePhotoPairingIdentifier;
- (id)originatingAssetIdentifier;
- (id)outOfBandHintsBase64String;
- (id)playbackVariation;
- (id)spatialOverCaptureIdentifier;
- (id)videoCodecName;
- (unsigned int)firstVideoTrackCodec;
- (id)firstVideoTrackCodecString;
- (id)initWithAVProxyData:(id)a0 timeZoneLookup:(id)a1;
- (char)isActionCam;
- (id)montageString;
- (short)smartStyleVideoCastValue;
- (struct { long long x0; int x1; unsigned int x2; long long x3; })stillImageDisplayTime;
- (id)outOfBandHints;
- (struct opaqueCMFormatDescription { } *)videoTrackFormatDescription;
- (id)stillImageDisplayTimeDictionary;
- (id)_contextForKeys:(id)a0 collectionKeys:(id)a1;
- (void)_verifyAllValuesLoadedForAsset:(id)a0;
- (char)isAutoLivePhoto;
- (id)_apacProfileLevelDescriptionForFormatDescription:(struct opaqueCMFormatDescription { } *)a0 audioStreamBasicDescription:(struct AudioStreamBasicDescription { double x0; unsigned int x1; unsigned int x2; unsigned int x3; unsigned int x4; unsigned int x5; unsigned int x6; unsigned int x7; unsigned int x8; })a1;
- (struct AudioStreamBasicDescription { double x0; unsigned int x1; unsigned int x2; unsigned int x3; unsigned int x4; unsigned int x5; unsigned int x6; unsigned int x7; unsigned int x8; })_audioStreamBasicDescription;
- (long long)_audioVideoProxyPropertyListForAsset:(id)a0 propertyList:(id *)a1 error:(id *)a2;
- (void)_cacheAllTrackProperties;
- (void)_cacheCommonMetadataProperties;
- (void)_cacheComputedTrackMetadataProperties;
- (void)_cacheIndirectTrackMetadataProperties;
- (id)_cachedValueForKey:(id)a0 usingBlock:(id /* block */)a1;
- (void)_checkHFRMetadataKeyShouldPlayAtFullFrameRate:(char *)a0 containsHFRPlaybackKey:(char *)a1;
- (void)_computeAllProperties;
- (id)_computeApacAudioTrackInfo;
- (void)_computeChannelCountsForApacAudioTrackInfo:(id)a0 maxChannelCount:(unsigned long long)a1 audioChannelLayout:(const struct AudioChannelLayout { unsigned int x0; unsigned int x1; unsigned int x2; struct AudioChannelDescription { unsigned int x0; unsigned int x1; float x2[3]; } x3[1]; } *)a2;
- (struct opaqueCMFormatDescription { } *)_computeFirstFormatDescriptionOfType:(id)a0;
- (id)_computeFirstTrackOfType:(id)a0;
- (void)_computeFirstVideoTrackCodecAndName;
- (id)_computeHEVCProfileInfo;
- (char)_computeIsProRes;
- (id)_computeStillImageDisplayTimeDictionary;
- (char)_configureWithAVAsset:(id)a0;
- (char)_configureWithAVProxyData:(id)a0;
- (char)_configureWithAVURL:(id)a0;
- (char)_convertToAVAssetProxyIfPossible;
- (id)_exifTimezoneOffsetFromDateString:(id)a0 offsetInSeconds:(long long *)a1;
- (id)_firstAudioTrack;
- (id)_firstVideoTrack;
- (struct opaqueCMFormatDescription { } *)_firstVideoTrackFormatDescription;
- (void)_loadMetadataForAsset:(id)a0 withKeys:(id)a1 collectionKeys:(id)a2;
- (void)_loadMetadataForAsset:(id)a0 withKeys:(id)a1 collectionKeys:(id)a2 completionHandler:(id /* block */)a3;
- (id)_makeDateTimeProperties;
- (id)_makeDateTimePropertiesForAsset:(id)a0;
- (id)_makeGPSProperties;
- (id)_makeGeometryProperties;
- (long long)_spatialVideoRecommendationForImmersiveModeForAssetVideoTrack:(id)a0;
- (void)_verifyTrackValuesLoadedForAsset:(id)a0;
- (id)audioBitsPerChannel;
- (id)audioBytesPerFrame;
- (id)audioBytesPerPacket;
- (id)audioChannelsPerFrame;
- (id)audioFramesPerPacket;
- (id)audioTrackFormatFlags;
- (id)captureMode;
- (id)collectionKeysToLoad;
- (char)configureWithMetadataPlist:(id)a0;
- (id)gpsHPositioningError;
- (char)hasVitality;
- (id)initWithAVAsset:(id)a0 options:(unsigned short)a1 timeZoneLookup:(id)a2 preloadValues:(char)a3;
- (id)keysToLoad;
- (id)livePhotoMinimumClientVersion;
- (char)livePhotoVitalityLimitingAllowed;
- (id)livePhotoVitalityScore;
- (id)livePhotoVitalityTransitionScore;
- (id)plistForEncoding;
- (id)ptpMediaMetadata;
- (id)renderOriginatingAssetIdentifier;
- (id)spatialMediaTrack;
- (long long)spatialVideoRecommendationForImmersiveMode;
- (char)verifyShouldLoadTrackMetadata:(const char *)a0;
- (id)videoDataRate;
- (id)videoDynamicRange;

@end
