@interface RBDisplayListTransform : NSObject <NSCopying> {
    struct Transform { struct vector<RB::DisplayList::Transform::Term, 1UL, unsigned int> { unsigned char _p[48]; struct Term *_p; unsigned int _size; unsigned int _capacity; } _terms; } _transform;
}
