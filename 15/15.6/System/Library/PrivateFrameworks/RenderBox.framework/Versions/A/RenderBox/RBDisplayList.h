...
@interface RBDisplayList : NSObject <_RBDisplayListContents, RBDisplayListContents> {
    struct Builder { unsigned int _crop_seed; unsigned int _layer_id; struct refcounted_ptr<RB::DisplayList::Contents> { struct Contents *_p; } _contents; struct Pool<RB::DisplayList::State, const RB::Malloc> { struct Item *_freelist; } _state_pool; struct Pool<RB::DisplayList::Builder::LayerState, RB::Heap> { struct Item *_freelist; } _layer_state_pool; struct LayerState *_active_layer; struct State { struct State *_next; void *_context; struct AffineTransform { void /* unknown type, empty encoding */ ab; void /* unknown type, empty encoding */ cd; void /* unknown type, empty encoding */ txy; } _ctm; struct Identity { unsigned int identifier; struct UUID { unsigned char bytes[16]; } ns; } _identity; struct refcounted_ptr<const RB::Transition> { struct Transition *_p; } _transition; struct ClipNode *_clip; struct Style *_style; struct AffineTransform *_copied_ctm; struct Metadata *_copied_metadata; struct Rect { void /* unknown type, empty encoding */ origin; void /* unknown type, empty encoding */ size; } _crop; unsigned int _layer_id; unsigned int _crop_seed; struct optional_float { float _value_or_nan; } _numeric_value; struct optional<RB::ColorSpace> { union { char __null_state_; unsigned char __val_; } ; BOOL __engaged_; } _default_color_space; struct TextIdentity *_text_identity; struct AnimationLayer { unsigned int layer_index; unsigned int layer_count; } _animation_layer; } _root_state; struct LayerState { struct Layer *_layer; struct LayerState *_next_layer; struct Rect { void /* unknown type, empty encoding */ origin; void /* unknown type, empty encoding */ size; } _crop; unsigned int _crop_seed; } _root_layer_state; struct State *_state; BOOL _optimized; } _list;
...
@property (nonatomic) char linearColors;
...
- (void)addBlurFilterWithRadius:(double)a0 opaque:(char)a1;
