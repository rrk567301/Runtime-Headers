@class NSString, AXLTTranscriber, NSDate, AVAudioFormat;
...
- (void)_calcHistogramForBuffer:(struct AudioQueueBuffer { unsigned int x0; void *x1; unsigned int x2; void *x3; unsigned int x4; struct AudioStreamPacketDescription *x5; unsigned int x6; } *)a0 packetCount:(unsigned int)a1 isSilence:(char)a2;
- (long long)_findSilencePacketOffset:(struct AudioQueueBuffer { unsigned int x0; void *x1; unsigned int x2; void *x3; unsigned int x4; struct AudioStreamPacketDescription *x5; unsigned int x6; } *)a0 packetCount:(unsigned int)a1 silentSamplesMin:(long long)a2 zeroOffsetOnly:(char)a3;
- (char)_isSilenceOnlyInBuffer:(struct AudioQueueBuffer { unsigned int x0; void *x1; unsigned int x2; void *x3; unsigned int x4; struct AudioStreamPacketDescription *x5; unsigned int x6; } *)a0 packetCount:(unsigned int)a1;
- (void)_pauseSpeechRecognition;
- (char)createAudioBuffersWithBufferByteSize:(unsigned int)a0 error:(id *)a1;
