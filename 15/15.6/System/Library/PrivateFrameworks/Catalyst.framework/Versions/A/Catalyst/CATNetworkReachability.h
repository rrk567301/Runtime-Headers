...
@property (readonly, nonatomic, getter=isReachable) char reachable;

+ (id)reachabilityForInternetConnection;
+ (id)reachabilityForLocalWiFi;
+ (id)reachabilityWithAddress:(id)a0;
+ (char)isReachableForNetworkWithFlags:(unsigned int)a0;
+ (char)isReachableForLocalWiFiWithFlags:(unsigned int)a0;
