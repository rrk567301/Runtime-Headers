...
@interface BookmarksController : NSObject <BookmarkGroupDelegate, FileChangeObserverClient, FileLockerClient, FileLockerLogDelegate, WBTabGroupManagerObserver, BookmarksUndoControllerDataStore, Command0Through9Receiver> {
    ...
    char _isReloadingScopedBookmarks;
}

@property (nonatomic) struct SHA256Hash { unsigned char data[32]; } builtInBookmarksStateHash;
@property (nonatomic) char didFillWithBuiltInBookmarks;
@property (nonatomic, getter=isReloadingBookmarks) char reloadingBookmarks;
@property (nonatomic, getter=isSavePending) char savePending;
...
@property (nonatomic) char shouldResetCloudKitMigrationStateOnNextLoad;
...
+ (void)_requestSyncClientTriggerSyncForBookmarkGroup:(id)a0 skipRequestIfNoChanges:(char)a1;
...
+ (void)requestSyncClientTriggerSyncForBookmarkGroup:(id)a0 skipRequestIfNoChanges:(char)a1;
...
- (char)_areScopedBookmarkLists:(id)a0 equivalentToSafariBookmarkLists:(id)a1;
- (void)_beginObservingNetworkChangeNotifications;
- (id)_bookmarkSourceCreateIfNeededWithPossibleTitles:(id)a0 displayTitle:(id)a1 indexInTopBookmark:(unsigned long long)a2 fileNeedsUpdate:(char *)a3;
- (id)_bookmarkSourceWithPossibleTitles:(id)a0 displayTitle:(id)a1 bookmarkIndex:(unsigned long long *)a2 isFirstTitle:(char *)a3;
...
- (char)_favoritesContainsBookmark:(id)a0;
...
- (void)_flushPendingSaveCompletionHandlersWithSuccess:(char)a0;
...
- (id)_insertNewBookmarkList:(id)a0 indexInTopBookmark:(unsigned long long)a1 excludedFromPlist:(char)a2;
- (void)_invalidatePendingSaveTimer;
- (void)_loadTabGroupScopedBookmarks;
- (void)_lockFileLockerWithCompletionHandler:(id /* block */)a0;
- (char)_menuContainsBookmark:(id)a0;
...
- (char)_reloadBookmarksCheckingIfFileChanged:(char)a0;
...
- (void)_savePendingChangesSoonForPossiblyInteractiveBookmarkChange:(char)a0 completionHandler:(id /* block */)a1;
...
- (void)bookmarkGroup:(id)a0 bookmarkDidChange:(id)a1 changeWasInteractive:(char)a2;
...
- (void)exportBookmarksToPath:(id)a0 shouldHideExtension:(char)a1;
