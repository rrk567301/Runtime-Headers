...
@interface MFMessageRouter : NSObject <NSCopying, MCActivityTarget, EFLoggable> {
    ...
    char _registeredForMailboxNotifications;
    char _junkMailTrustHeaders;
    char _junkMailEvaluationAfterRules;
...
@property (readonly, nonatomic) char colorsUpdateDynamically;
...
@property (nonatomic) char junkMailTrustHeaders;
@property (nonatomic) char junkMailEvaluationAfterRules;
...
- (char)_colorRuleExistsAfterRule:(id)a0;
- (id)_defaultSortRules;
- (void)_diagnosticsNotificationReceived:(id)a0;
- (char)_hasColorRules;
- (id)_junkMailRuleForBehavior:(long long)a0 abCriterion:(char)a1 ahCriterion:(char)a2 fnCriterion:(char)a3;
...
- (void)_putRulesThatWantToHandleMessage:(id)a0 intoArray:(id)a1 colorRulesOnly:(char)a2 fetchingBody:(char)a3 needsBody:(char *)a4;
...
- (id)_sortRuleDictionaryRepresentationsIncludingPII:(char)a0;
...
- (id)colorForMessage:(id)a0 highlightTextUsingColor:(char *)a1;
...
- (id)routeMessages:(id)a0 fromStores:(id)a1 fetchingBodies:(char)a2 messagesNeedingBodies:(id)a3;
- (void)ruleAffectingColorsDidChange:(id)a0;
- (void)ruleDidChange:(id)a0;
- (void)scheduleWriteRules;
- (id)test_routeMessages:(id)a0 fromStores:(id)a1 fetchingBodies:(char)a2 messagesNeedingBodies:(id)a3;
