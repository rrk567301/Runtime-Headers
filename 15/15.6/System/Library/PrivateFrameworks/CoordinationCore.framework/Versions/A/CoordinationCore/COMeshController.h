@class NSObject, NSDictionary, COMeshNodeStateTrackerSet, COMeshLocalNode, CODiscoveryManager, NSOrderedSet, COBallot, NSString, COIDSBrowser, CONodeManager, NSUserDefaults, NSArray, COConstituent, CORapportBrowser, COBallotBrowser, COMeshNode;
@protocol OS_dispatch_source, COCompanionLinkClientFactoryProtocol, OS_dispatch_queue;
...
- (void)_finalizeCompletionOfNode:(id)a0 memberOfMesh:(char)a1 eventProvider:(id /* block */)a2;
...
- (char)_isCurrentElectionVariant:(long long)a0;
- (char)_isElectionComplete;
- (void)_logElectionSummary;
- (id)_newCompanionLinkClient;
- (id)_nodeDetails:(id)a0;
- (void)_performElectionGeneration:(unsigned long long)a0 source:(id)a1 allowingPostTransition:(char)a2;
...
- (void)broadcastRequest:(id)a0 includingSelf:(char)a1 recipientsCallback:(id /* block */)a2 completionHandler:(id /* block */)a3;
...
- (char)removeAddOn:(id)a0;
- (void)sendCommand:(id)a0 toPeer:(id)a1;
