@class CNCancelationToken, NSArray, CNAutocompleteUserSession, CNAutocompleteFetchRequest, NSString, CNAutocompleteDelegateWrapper;
...
+ (char)searchTypeSupportsSuppressionOfAlreadyChosenAddresses:(unsigned long long)a0;
+ (char)shouldPerformQueryForRequest:(id)a0;
+ (char)shouldSortResultsForPolicy:(id)a0;
+ (char)shouldSuppressAddressesAlreadyChosenForRequest:(id)a0;
