@class NSData, NSString, NSArray, NSURL, AFPowerContextPolicy, NSUUID, NSDictionary, CESRSpeechParameters, CLLocation, AFASRSharedUserInfo;

@interface _CESRSpeechParametersMutation : NSObject <CESRSpeechParametersMutating> {
    CESRSpeechParameters *_baseModel;
    NSString *_language;
    NSString *_requestIdentifier;
    NSString *_dictationUIInteractionIdentifier;
    NSString *_task;
    NSArray *_loggingContext;
    NSString *_applicationName;
    NSData *_profile;
    NSDictionary *_overrides;
    NSURL *_modelOverrideURL;
    NSURL *_originalAudioFileURL;
    NSString *_codec;
    char _narrowband;
    char _detectUtterances;
    char _censorSpeech;
    char _farField;
    char _secureOfflineOnly;
    char _shouldStoreAudioOnDevice;
    char _continuousListening;
    char _shouldHandleCapitalization;
    char _isSpeechAPIRequest;
    double _maximumRecognitionDuration;
    double _endpointStart;
    NSString *_inputOrigin;
    CLLocation *_location;
    NSArray *_jitGrammar;
    char _deliverEagerPackage;
    char _disableDeliveringAsrFeatures;
    char _enableEmojiRecognition;
    char _enableAutoPunctuation;
    char _enableVoiceCommands;
    NSArray *_sharedUserInfos;
    NSString *_prefixText;
    NSString *_postfixText;
    NSString *_selectedText;
    AFPowerContextPolicy *_powerContext;
    double _recognitionStart;
    char _shouldGenerateVoiceCommandCandidates;
    NSUUID *_asrId;
    AFASRSharedUserInfo *_activeUserInfo;
    struct _mutationFlags { unsigned char isDirty : 1; unsigned char hasLanguage : 1; unsigned char hasRequestIdentifier : 1; unsigned char hasDictationUIInteractionIdentifier : 1; unsigned char hasTask : 1; unsigned char hasLoggingContext : 1; unsigned char hasApplicationName : 1; unsigned char hasProfile : 1; unsigned char hasOverrides : 1; unsigned char hasModelOverrideURL : 1; unsigned char hasOriginalAudioFileURL : 1; unsigned char hasCodec : 1; unsigned char hasNarrowband : 1; unsigned char hasDetectUtterances : 1; unsigned char hasCensorSpeech : 1; unsigned char hasFarField : 1; unsigned char hasSecureOfflineOnly : 1; unsigned char hasShouldStoreAudioOnDevice : 1; unsigned char hasContinuousListening : 1; unsigned char hasShouldHandleCapitalization : 1; unsigned char hasIsSpeechAPIRequest : 1; unsigned char hasMaximumRecognitionDuration : 1; unsigned char hasEndpointStart : 1; unsigned char hasInputOrigin : 1; unsigned char hasLocation : 1; unsigned char hasJitGrammar : 1; unsigned char hasDeliverEagerPackage : 1; unsigned char hasDisableDeliveringAsrFeatures : 1; unsigned char hasEnableEmojiRecognition : 1; unsigned char hasEnableAutoPunctuation : 1; unsigned char hasEnableVoiceCommands : 1; unsigned char hasSharedUserInfos : 1; unsigned char hasPrefixText : 1; unsigned char hasPostfixText : 1; unsigned char hasSelectedText : 1; unsigned char hasPowerContext : 1; unsigned char hasRecognitionStart : 1; unsigned char hasShouldGenerateVoiceCommandCandidates : 1; unsigned char hasAsrId : 1; unsigned char hasActiveUserInfo : 1; } _mutationFlags;
}

@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (id)init;
- (void).cxx_destruct;
- (void)setLanguage:(id)a0;
- (void)setLocation:(id)a0;
- (void)setSelectedText:(id)a0;
- (void)setOverrides:(id)a0;
- (void)setTask:(id)a0;
- (void)setProfile:(id)a0;
- (void)setRequestIdentifier:(id)a0;
- (void)setApplicationName:(id)a0;
- (void)setCodec:(id)a0;
- (void)setInputOrigin:(id)a0;
- (void)setPostfixText:(id)a0;
- (void)setCensorSpeech:(char)a0;
- (void)setPrefixText:(id)a0;
- (void)setDetectUtterances:(char)a0;
- (void)setEnableVoiceCommands:(char)a0;
- (void)setEndpointStart:(double)a0;
- (void)setFarField:(char)a0;
- (void)setMaximumRecognitionDuration:(double)a0;
- (void)setShouldGenerateVoiceCommandCandidates:(char)a0;
- (id)generate;
- (id)initWithBaseModel:(id)a0;
- (void)setContinuousListening:(char)a0;
- (void)setJitGrammar:(id)a0;
- (void)setLoggingContext:(id)a0;
- (void)setModelOverrideURL:(id)a0;
- (void)setNarrowband:(char)a0;
- (void)setOriginalAudioFileURL:(id)a0;
- (void)setSecureOfflineOnly:(char)a0;
- (void)setShouldHandleCapitalization:(char)a0;
- (void)setShouldStoreAudioOnDevice:(char)a0;
- (void)setAsrId:(id)a0;
- (void)setEnableAutoPunctuation:(char)a0;
- (void)setActiveUserInfo:(id)a0;
- (void)setDeliverEagerPackage:(char)a0;
- (void)setDictationUIInteractionIdentifier:(id)a0;
- (void)setDisableDeliveringAsrFeatures:(char)a0;
- (void)setEnableEmojiRecognition:(char)a0;
- (void)setIsSpeechAPIRequest:(char)a0;
- (void)setPowerContext:(id)a0;
- (void)setRecognitionStart:(double)a0;
- (void)setSharedUserInfos:(id)a0;

@end
