@class SiriTTSDaemonSession, SVXModule, SVXSpeechRequestConfigurer, SVXAFSpeakableUtteranceParserProvider, SVXSynthesisVoiceConfigurer, SVXSpeechSynthesisUtils, NSMutableArray, SVXSpeechSynthesisAnnouncer, NSString, SVXSpeechSynthesisConfigurer, SVXSessionManager, NSMutableSet, SVXSessionUtils, SVXDefaultNotificationCenterRegistrar, SVXLocalizationUtils, SVXSpeechSynthesisConfigurationStateManager, SVXSpeechSynthesisMetricsContextFactory, SVXAudioStreamingAnnouncer, SVXVoiceGenderConverter, SVXSpeechInstrumentationUtilities, SVXSpeechSynthesisContext;

@interface SVXSpeechSynthesizer : NSObject <SVXModuleInstance> {
    SVXSessionManager *_sessionManager;
    char _needsPrewarm;
    unsigned long long _ttsSignpostInterval;
    SVXModule *_module;
    SVXAudioStreamingAnnouncer *_audioStreamingAnnouncer;
    SVXSpeechSynthesisAnnouncer *_announcer;
    SVXSpeechSynthesisConfigurationStateManager *_configurationManager;
    SVXSpeechSynthesisContext *_currentSpeakingContext;
    NSMutableArray *_pendingContexts;
    NSMutableSet *_synthesizingContexts;
    SVXSpeechSynthesisUtils *_speechSynthesisUtils;
    SVXSessionUtils *_sessionUtils;
    SVXVoiceGenderConverter *_voiceGenderConverter;
    SVXSpeechSynthesisConfigurer *_speechSynthesisConfigurer;
    SVXSpeechRequestConfigurer *_speechRequestConfigurer;
    SVXSynthesisVoiceConfigurer *_voiceConfigurer;
    SVXDefaultNotificationCenterRegistrar *_notificationCenterRegistrar;
    SVXAFSpeakableUtteranceParserProvider *_speakableUtteranceParserProvider;
    SVXLocalizationUtils *_localizationUtils;
    SVXSpeechInstrumentationUtilities *_speechInstrumentationUtilities;
    SVXSpeechSynthesisMetricsContextFactory *_metricsContextFactory;
}

@property (readonly, nonatomic) SiriTTSDaemonSession *ttsSession;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (void)dealloc;
- (void).cxx_destruct;
- (void)prewarm;
- (void)removeListener:(id)a0;
- (char)_isInternalInstall;
- (void)addListener:(id)a0;
- (void)cancelRequest:(id)a0;
- (void)updateWithConfiguration:(id)a0;
- (void)_prewarm;
- (void)cancelPendingRequests;
- (id)initWithModule:(id)a0;
- (void)duckTTSVolumeTo:(float)a0 rampTime:(double)a1 completion:(id /* block */)a2;
- (void)_duckTTSVolumeTo:(float)a0 rampTime:(double)a1 completion:(id /* block */)a2;
- (void)didFinishSpeakingRequest:(id)a0 successfully:(char)a1 phonemesSpoken:(id)a2 withError:(id)a3;
- (void)stopWithModuleInstanceProvider:(id)a0;
- (void)prewarmRequest:(id)a0;
- (void)_cancelPendingContextsAtIndexes:(id)a0;
- (void)_cancelPendingContextsWithOperationType:(long long)a0;
- (void)_cancelPendingSpeakingContextWithRequest:(id)a0;
- (void)_cancelPendingSpeakingContextsWithPriorityBelow:(long long)a0;
- (void)_configureWithConfiguration:(id)a0;
- (char)_continueContext:(id)a0 error:(id *)a1;
- (void)_enqueueContext:(id)a0;
- (void)_finalizeContext:(id)a0 withResultType:(long long)a1 utterance:(id)a2 error:(id)a3;
- (void)_handleDidFinishPresynthesizedAudioRequest:(id)a0 instrumentMetrics:(id)a1 error:(id)a2;
- (void)_handleDidFinishSpeakingWithSpeechRequest:(id)a0 instrumentMetrics:(id)a1;
- (void)_handleDidFinishSpeakingWithSpeechRequest:(id)a0 success:(char)a1 error:(id)a2;
- (void)_handleDidFinishSynthesizingForSpeechRequest:(id)a0 instrumentMetrics:(id)a1 error:(id)a2;
- (void)_handleDidGenerateAudioChunkData:(id)a0 forSpeechRequest:(id)a1;
- (void)_handleDidReceiveSpeechWordTimingInfoArray:(id)a0 forSpeechRequest:(id)a1;
- (void)_handleDidStartPresynthesizedAudioRequest:(id)a0 timestamp:(unsigned long long)a1;
- (void)_handleDidStartSpeakingWithSpeechRequest:(id)a0 timestamp:(unsigned long long)a1;
- (void)_handleDidStopPresynthesizedAudioRequest:(id)a0 success:(char)a1 error:(id)a2;
- (char)_isTTSPlaybackDisabled;
- (unsigned long long)_numberOfContexts;
- (void)_postcool;
- (void)_prewarmWithContext:(id)a0;
- (void)_processPendingContexts;
- (void)_startContext:(id)a0;
- (char)_startPresynthesizedAudioRequestForContext:(id)a0 error:(id *)a1;
- (char)_startSpeechRequestForContext:(id)a0 error:(id *)a1;
- (void)_stopCurrentSpeakingContextWithInterruptionBehavior:(long long)a0;
- (void)_stopCurrentSpeakingContextWithRequest:(id)a0 withInterruptionBehavior:(long long)a1;
- (char)_useStreamingAudio;
- (void)didFinishPresynthesizedAudioRequest:(id)a0 withInstrumentMetrics:(id)a1 error:(id)a2;
- (void)didFinishSpeakingRequest:(id)a0 withInstrumentMetrics:(id)a1;
- (void)didFinishSynthesisRequest:(id)a0 withInstrumentMetrics:(id)a1 error:(id)a2;
- (void)didStartPresynthesizedAudioRequest:(id)a0;
- (void)didStartSpeakingRequest:(id)a0;
- (void)didStopPresynthesizedAudioRequest:(id)a0 atEnd:(char)a1 error:(id)a2;
- (void)enqueueRequest:(id)a0 languageCode:(id)a1 voiceName:(id)a2 gender:(long long)a3 preparation:(id /* block */)a4 finalization:(id /* block */)a5 taskTracker:(id)a6 analyticsContext:(id)a7;
- (id)initWithModule:(id)a0 speechSynthesisUtils:(id)a1 sessionUtils:(id)a2 voiceGenderConverter:(id)a3 announcer:(id)a4 configurationManager:(id)a5 audioStreamingAnnouncer:(id)a6 ttsSession:(id)a7 speechSynthesisConfigurer:(id)a8 speechRequestConfigurer:(id)a9 voiceConfigurer:(id)a10 notificationCenterRegistrar:(id)a11 speakableUtteranceParserProvider:(id)a12 localizationUtils:(id)a13 speechInstrumentationUtilities:(id)a14 metricsContextFactory:(id)a15;
- (void)languageCodeChanged:(id)a0;
- (void)outputVoiceChanged:(id)a0;
- (void)postcool;
- (void)startWithModuleInstanceProvider:(id)a0 platformDependencies:(id)a1;
- (void)stopCurrentRequestWithInterruptionBehavior:(long long)a0;
- (void)stopRequest:(id)a0 withInterruptionBehavior:(long long)a1;
- (void)synthesizeRequest:(id)a0 audioChunkHandler:(id /* block */)a1 taskTracker:(id)a2 analyticsContext:(id)a3 completion:(id /* block */)a4;
- (void)withRequest:(id)a0 didGenerateAudioChunk:(id)a1;
- (void)withRequest:(id)a0 didReceiveTimingInfo:(id)a1;

@end
