...
@property (getter=isDisabledInternal) char disabledInternal;
@property (readonly, nonatomic) NSDictionary *cloudObjectClassesByRecordType;
@property (nonatomic) char needsToProcessAllObjects;
...
@property (nonatomic, getter=isFetchingEnabled) char fetchingEnabled;
@property (nonatomic) char syncDisabledByServer;
...
@property (nonatomic) char didEnqueueLongLivedOperations;
@property (nonatomic, getter=isEnqueueingLongLivedOperations) char enqueueingLongLivedOperations;
...
@property (readonly, nonatomic) char fetchOperationsPending;
@property char needsToUpdateSubscriptions;
@property (nonatomic) char shouldResumeSyncOnForeground;
@property (nonatomic) long long qualityOfService;
@property (nonatomic) unsigned long long discretionaryNetworkBehavior;
@property (nonatomic) char enableLongLivedOperations;
@property (nonatomic) char disableAutomaticallyRetryNetworkFailures;
@property (nonatomic) char disableRetryTimer;
@property (nonatomic) char syncOnlyIfReachable;
@property (getter=isDisabled) char disabled;
@property (readonly, nonatomic) char hasPendingOperations;
...
+ (char)haveZoneIDsInAccountZoneIDs:(id)a0;
+ (char)isZoneConfigurations:(id)a0 subsetOfZoneConfigurations:(id)a1;
+ (id)newNotesContainer;
+ (id)newNotesContainerForAccountID:(id)a0;
+ (id)objectsByAccount:(id)a0;
+ (id)objectsByDatabaseScope:(id)a0;
+ (void)registerStateHandler;
+ (char)shouldIgnoreErrorForBackoffTimer:(id)a0;
...
- (char)partialError:(id)a0 containsErrorCode:(long long)a1;
- (void)clearContainers;
- (void)modifyRecordsOperation:(id)a0 recordWasSavedWithRecordID:(id)a1 record:(id)a2 error:(id)a3;
- (void)_addModifyRecordsOperationsWithCloudObjectsToSave:(id)a0 delete:(id)a1 accountID:(id)a2 operationGroupName:(id)a3 waitForDependencies:(char)a4 completionHandler:(id /* block */)a5;
...
- (void)_ingestCloudKitRecord:(id)a0 forAccountID:(id)a1 forceMerge:(char)a2 context:(id)a3;
...
- (void)_syncWithReason:(id)a0 uploadUnsyncedChanges:(char)a1 completionHandler:(id /* block */)a2;
...
- (void)addModifyRecordsOperationsWithCloudObjectsToSave:(id)a0 delete:(id)a1 accountID:(id)a2 operationGroupName:(id)a3 waitForDependencies:(char)a4 completionHandler:(id /* block */)a5;
...
- (void)deleteRecordZonesWithZoneIDs:(id)a0 accountID:(id)a1 markZonesAsUserPurged:(char)a2 completionHandler:(id /* block */)a3;
...
- (id)existingCloudObjectForUserSpecificRecordID:(id)a0 createPlaceholderIfNecessary:(char)a1 accountID:(id)a2 context:(id)a3;
...
- (char)hasNoteContextOptions:(unsigned long long)a0;
...
- (void)ingestCloudKitRecord:(id)a0 forAccountID:(id)a1 forceMerge:(char)a2 context:(id)a3;
...
- (id)operationsToModifyRecordsForCloudObjectsToSave:(id)a0 delete:(id)a1 deleteShares:(id)a2 saveUserSpecificRecords:(id)a3 operationGroupName:(id)a4 addDependencies:(char)a5 accountID:(id)a6;
...
- (void)syncWithReason:(id)a0 uploadUnsyncedChanges:(char)a1 completionHandler:(id /* block */)a2;
