...
@property (readonly) char isRelayRunning;
...
- (void)lock;
- (void)unlock;
- (char)createAudioConvertersWithError:(id *)a0;
- (void)releasePTPClock;
- (void)stopRelayThread;
- (char)canSetPropertyWithError:(id *)a0;
...
- (char)setClientIO:(id)a0 withError:(id *)a1;
- (char)setIOBufferDuration:(double)a0 withError:(id *)a1;
- (char)setNetworkClockID:(unsigned long long)a0 withError:(id *)a1;
- (char)setRemoteIO:(id)a0 withError:(id *)a1;
- (void)sleepForTime:(struct timespec { long long x0; long long x1; } *)a0;
- (char)startClientIO;
- (void)startPeriodicHealthPrint;
- (char)startRelayIO:(struct tagVCAudioRelayIOInfo { id x0; struct _opaque_pthread_mutex_t { long long x0; char x1[56]; } x1; id x2; id x3; } *)a0 otherRelayIO:(struct tagVCAudioRelayIOInfo { id x0; struct _opaque_pthread_mutex_t { long long x0; char x1[56]; } x1; id x2; id x3; } *)a1;
- (char)startRelayThreadWithError:(id *)a0;
- (char)startRemoteIO;
- (void)stopClientIO;
