...
@property (nonatomic) char initialSecondaryRelaySetupDone;
...
@property char allowsRelay;
@property char isCaller;
...
- (struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; })IPPortForPrimaryConnectionOnLocalInterface:(char)a0;
...
- (id)connectionData:(char *)a0 connectionDataSizeInBytes:(int)a1 shouldUseRelay:(char)a2;
- (id)connectionDataUsingRelay:(char)a0 isInitialRelay:(char)a1 nonCellularCandidateTimeout:(double)a2 error:(id *)a3;
- (void)connectionResultCallback:(struct tagCONNRESULT { unsigned int x0; int x1; int x2; unsigned int x3; int x4; int x5; unsigned short x6; unsigned short x7; struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } x8; struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } x9; struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } x10; struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } x11; struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } x12; struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } x13; unsigned int x14; int x15; int x16; int x17; int x18; unsigned short x19; int x20; int x21; int x22; struct tagCONNRESULT *x23; int x24; } *)a0 didReceiveICEPacket:(char)a1 didUseRelay:(char)a2 secretKey:(id)a3 skeResult:(int)a4;
...
- (void)createSecondaryRelayDispatchTimer:(float)a0 callID:(unsigned int)a1 callerRequired:(char)a2;
- (char)createTCPTunnelForParticipantID:(id)a0 relayDictionary:(id)a1 didOriginateRequest:(char)a2 relayType:(unsigned char)a3 error:(id *)a4;
...
- (void)getConnectionDataUsingRelay:(char)a0 isInitialRelay:(char)a1 nonCellularCandidateTimeout:(double)a2 queue:(id)a3 completionHandler:(id /* block */)a4;
- (char)getConnectionSetupData:(id *)a0 withOptions:(id)a1 error:(id *)a2;
- (void)handleConnectionSetupDataChangeMessageDelivered;
- (char)handleExchangedKey:(id)a0 result:(int)a1;
...
- (double)iceTimeoutInSeconds:(char)a0;
...
- (void)processRelayRequestResponse:(id)a0 didOriginateRequest:(char)a1;
- (void)processRelayUpdate:(id)a0 didOriginateRequest:(char)a1;
...
- (void)resetLoopback;
- (void)setForceRelay;
- (void)setupInitialSecondaryRelayWithCallbackRelayFlag:(char)a0 callID:(unsigned int)a1 connectionId:(int)a2;
- (void)setupLoopbackWithConnectionType:(unsigned int)a0;
- (void)setupPendingSecondaryRelayWithNewPrimary:(id)a0;
- (void)setupSecondaryRelayForCall:(unsigned int)a0 callerRequired:(char)a1;
- (void)setupTransport;
- (char)startConnectionWithBlob:(id)a0 useRelay:(char)a1 isInitialRelay:(char)a2 error:(id *)a3;
- (void)stopLoopback;
