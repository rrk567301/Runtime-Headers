@class NSMutableDictionary, VCMediaRecorder, VCAudioIO, NSDictionary, VCCallInfoBlob, AVCBasebandCongestionDetector, NSObject, NSMutableArray, VCSecurityKeyManager, VCNetworkFeedbackController, NSString, VCAudioRuleCollection, VCAudioCaptionsCoordinator, AVCStatisticsCollector, VCSessionMediaNegotiator, VCRedundancyControllerVideo, NSArray, NSData, VCSessionParticipantOneToOneConfig;
...
@property (nonatomic, getter=isLocalOnWiFi) char localOnWiFi;
...
@property (readonly, nonatomic) char isParticipantIDBasedKeyLookupEnabled;
...
+ (id)participantDataWithParticipantData:(id)a0 isReinit:(char)a1;
...
- (char)configureOneToOneAudioStreamsWithDeviceRole:(int)a0 streamInfoArray:(id)a1;
- (void)configureOneToOneVideoSendingConfigs:(id)a0 streamGroupID:(unsigned int)a1;
- (char)configureOneToOneVideoStreamsWithDeviceRole:(int)a0;
- (char)configureOneToOneVideoStreamsWithDeviceRole:(int)a0 streamInfoArray:(id)a1;
- (char)configureOneToOneWithConfig:(id)a0;
- (char)configureStreamInfosForMultiway:(id)a0 streamGroupID:(unsigned int)a1;
- (char)configureWithDeviceRole:(int)a0 negotiatedVideoEnabled:(char)a1 negotiatedScreenEnabled:(char)a2 operatingMode:(int)a3;
...
- (char)didNegotiateStreamGroupWithID:(unsigned int)a0;
...
- (char)dispatchedConfigureWithDeviceRole:(int)a0 negotiatedVideoEnabled:(char)a1 negotiatedScreenEnabled:(char)a2 operatingMode:(int)a3;
- (char)dispatchedIsAnyCameraMediaTypeEnabled;
- (char)dispatchedIsPersonaCameraEnabled;
- (void)dispatchedSetAudioEnabled:(char)a0;
- (void)dispatchedSetAudioPaused:(char)a0;
- (void)dispatchedSetMediaState:(unsigned int)a0 forMediaType:(unsigned int)a1;
- (void)dispatchedSetScreenEnabled:(char)a0;
- (void)dispatchedSetVideoEnabled:(char)a0;
- (void)dispatchedSetVideoPaused:(char)a0;
...
- (char)isAnyCameraMediaTypeEnabled;
- (char)isAudioStream:(id)a0;
- (char)isEnabledMediaType:(unsigned int)a0;
- (char)isOneToOneTemporalSupportedForSettings:(id)a0 streamDirection:(long long)a1;
- (char)isPersonaCameraEnabled;
- (char)isStreamGroupActive:(id)a0;
- (char)isVideoStream:(id)a0;
- (void)logAllStreamTokens;
- (unsigned char)mediaControlVersionForStreamGroup:(unsigned int)a0;
- (long long)negotiatedCipherSuiteForStreamGroupID:(unsigned int)a0;
- (id)newAudioRateControlConfigWithMediaControlInfoVersion:(unsigned char)a0 enableFeedbackController:(char)a1 isOneToOne:(char)a2;
...
- (id)newVideoRateControlConfigWithMediaControlInfoVersion:(unsigned char)a0 enableMediaControlInfoGenerator:(char)a1 enableFeedbackController:(char)a2 isOneToOne:(char)a3;
...
- (id)pauseAudioStreams:(char)a0;
- (id)pauseVideo:(char)a0;
...
- (char)rtcpPSFBForFeedbackStoreBagConfig;
- (id)screenGroup;
- (int)securityKeyHolderUsageForStreamConfigDirection:(long long)a0;
- (void)sendAudioPowerSpectrumSourceRegistration:(char)a0;
- (void)serverDidDie;
- (char)setAudioPosition:(float)a0;
- (id)setAudioStreamGroupsPaused:(char)a0;
- (id)setCameraStreamGroupsPaused:(char)a0;
- (void)setFECConfigurationOnStreamConfig:(id)a0 withStreamGroupID:(unsigned int)a1;
...
- (id)setPaused:(char)a0 onStreamGroups:(id)a1;
- (void)setStreamGroup:(unsigned int)a0 syncSourceStreamGroupID:(unsigned int)a1;
- (char)setupAudioStreamConfiguration:(id)a0 audioRules:(id)a1;
- (char)setupMediaSyncForStreamGroup:(id)a0;
- (void)setupMediaTypeSettingsWithConfig:(id)a0;
- (void)setupNetworkAddressesForMediaConfig:(id)a0;
- (void)setupPreferredIOSettings;
- (char)setupSecurityKeyHolderForStreamConfig:(id)a0;
- (char)setupStreamGroupMediaSync;
- (char)shouldSetPause:(char)a0 onStream:(id)a1;
- (void)startAudioDump;
- (id)startAudioIO;
...
- (void)streamGroup:(id)a0 didReceiveFlushRequestWithPayloads:(id)a1;
- (void)streamGroup:(id)a0 didRemoteEnableChange:(char)a1;
- (void)streamGroup:(id)a0 didRemotePauseChange:(char)a1;
...
- (void)streamGroup:(id)a0 remoteMediaStalled:(char)a1 duration:(double)a2;
...
- (char)updateConfigurationWithDeviceRole:(int)a0;
- (id)updateMediaState:(unsigned int)a0 forStreamGroups:(id)a1;
- (char)updateMediaStatesWithConfig:(id)a0;
