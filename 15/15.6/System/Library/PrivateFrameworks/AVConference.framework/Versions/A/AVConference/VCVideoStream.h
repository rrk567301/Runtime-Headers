...
@interface VCVideoStream : VCMediaStream <VCVideoReceiverDelegate, VCMediaStreamSyncDestination, VCVideoSink, VCVideoCaptureClient, VCMediaStreamRateAdaptationDelegate, VCRedundancyControllerDelegate> {
    ...
    char _shouldCacheRemoteVideoFrame;
    ...
    char _sendContinuousStallMessages;
    ...
    char _sensitiveContentAnalysisEnabled;
}
...
@property (readonly, nonatomic) char isCompoundStreamIDsIncreased;
@property (nonatomic) char shouldEnableFaceZoom;
@property (nonatomic) char didReceiveFirstFrame;
...
@property (readonly, nonatomic) char isTemporalScalingEnabled;
...
+ (id)capabilities;
+ (char)isFeatureListStringExpectedForStreamType:(long long)a0;
+ (char)isLowLatencyStreamType:(long long)a0;
+ (id)newFeatureListStringsWithConfiguration:(id)a0;
+ (char)shouldEnableRateControlForStreamType:(long long)a0;
+ (char)shouldRegisterReportingForStreamType:(long long)a0;
+ (char)shouldUseRandomRTPTimestampWithType:(long long)a0;
...
- (char)isScreenConfig:(id)a0;
- (char)isTransportIPv6;
- (void)onCallIDChanged;
- (int)remoteCameraReportingClientType;
- (void)setUplinkRetransmissionEnabled:(char)a0;
...
- (char)configureStreamInputCaptureSourceWithStreamInputID:(long long)a0 frameRate:(unsigned int)a1;
...
- (char)enableRateControlFeebackInConfig:(id)a0;
...
- (char)isClientSpecificUserInfoSupported:(int)a0;
- (char)isLowLatencyStreamType;
...
- (char)registerForCameraCaptureWithConfig:(id)a0;
...
- (char)setRTPPayloads:(int *)a0 numPayloads:(int)a1 withError:(id *)a2;
...
- (char)useUEPForVideoConfig:(int)a0;
- (char)validateStreamConfiguration:(id)a0 error:(id *)a1;
- (char)validateVideoStreamConfigurations:(id)a0;
...
- (struct tagVCVideoStreamReceiverConfig { struct tagHANDLE *x0; id x1; struct tagVCVideoReceiverDelegateRealtimeInstanceVTable { void /* function */ *x0; void /* function */ *x1; } x2; struct opaqueRTCReporting *x3; unsigned int x4; int x5; id x6; char x7; void *x8; void /* function */ *x9; id x10; })videoStreamReceiverConfigWithDumpID:(unsigned int)a0 reportingAgent:(struct opaqueRTCReporting { } *)a1;
