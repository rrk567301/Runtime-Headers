...
- (id)init;
- (void)cleanup;
- (char)ipPortStillExists:(struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } *)a0 requiresWifi:(char)a1;
...
- (int)getConnectionDataForCallID:(unsigned int)a0 version:(unsigned char)a1 useCompressedData:(char)a2 pConnectionData:(void **)a3 connectDataSizeInBytes:(int *)a4 relayDictionary:(struct __CFDictionary { } *)a5 interfaceUpdate:(char)a6 nonCellularCandidateTimeout:(double)a7;
...
- (void)iceRemoveIPPort:(struct tagIPPORT { int x0; char x1[16]; union { unsigned int x0; unsigned char x1[16]; } x2; unsigned short x3; } *)a0 forCallID:(unsigned int)a1 isLocalInterface:(char)a2;
...
- (void)receivedDataPacket:(char *)a0 length:(int)a1 forCallID:(unsigned int)a2 encrypted:(char)a3 OFTType:(int)a4;
...
- (int)sendData:(id)a0 localCallID:(unsigned int)a1 remoteCallID:(unsigned int)a2 encrypted:(char)a3 OFTType:(int)a4;
...
- (int)updateInterfaceList:(unsigned int)a0 shouldFilterCellInterface:(char)a1 isUpdateNeeded:(int *)a2;
- (int)updateQoS:(char)a0;
