...
@interface VCConnectionManagerIDS : VCConnectionManager {
    ...
    char _enableCoreMotionDetection;
    char _optIntoExistingSubscribedStreams;
}
...
- (char)addConnectionToConnectionArray:(id)a0;
- (void)addDuplicationConnectionUpdateTelemetryWithSuggestedLinkTypeCombo:(id)a0 payload:(id)a1;
- (void)applyConstrainWithThreshold:(id)a0 inBandwidth:(unsigned int *)a1 thredshold:(unsigned int)a2;
- (char)canOptOutAllStreamsForConnection:(id)a0;
- (id)connectionForDuplication;
- (void)didLinkProbingLockdownEnd;
- (void)didReceiveStatsResponse:(char)a0;
...
- (char)isEndToEndLinkAvailable;
- (char)isEndToEndLinkWithCellAvailable:(char)a0;
- (id)lastPrimaryConnectionInUse;
- (unsigned int)oneToOneBitrateCapForConnectionWithType:(unsigned int)a0;
- (char)optIntoExistingSubscribedStreams;
...
- (void)setOptIntoExistingSubscribedStreams:(char)a0;
...
- (char)shouldPreferWiredConnections;
- (char)shouldReplaceConnection:(id)a0 withConnection:(id)a1;
- (char)shouldUpdateServerBasedConnection:(id)a0;
...
- (void)updateDuplicationStateWithConnectionOperation:(int)a0 isLocalOnWiFi:(char)a1 isRemoteOnWiFi:(char)a2;
