...
@property (nonatomic) char remoteScreenEnabled;
...
@property (nonatomic) char configurationInProgress;
@property (nonatomic) char hasPendingChanges;
...
+ (unsigned int)defaultStateForMediaType:(unsigned int)a0 isLocal:(char)a1;
+ (char)isMediaStateAPISupportedForMediaType:(unsigned int)a0;
...
- (void)completeAudioEnabled:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)completeAudioPaused:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)completeScreenEnabled:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)completeSetMediaState:(unsigned int)a0 forMediaType:(unsigned int)a1 didSucceed:(char)a2 error:(id)a3;
- (void)completeVideoEnabled:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)completeVideoPaused:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)deregisterFromNotifications;
- (void)dispatchedCompleteAudioEnabled:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)dispatchedCompleteAudioPaused:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)dispatchedCompleteScreenEnabled:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)dispatchedCompleteVideoEnabled:(char)a0 didSucceed:(char)a1 error:(id)a2;
- (void)dispatchedCompleteVideoPaused:(char)a0 didSucceed:(char)a1 error:(id)a2;
...
- (char)generateMediaStateEntryForMediaType:(unsigned int)a0;
- (void)handleLegacyTransitionsForMediaType:(unsigned int)a0 mediaState:(unsigned int)a1 previousState:(unsigned int)a2 didSucceed:(char)a3 error:(id)a4;
...
- (char)isPausedMediaType:(unsigned int)a0;
...
- (void)setMediaType:(unsigned int)a0 enabled:(char)a1 mediaString:(const char *)a2 xpcMessageKey:(char *)a3 xpcOperationKey:(id)a4 completionBlock:(id /* block */)a5;
- (void)setMediaType:(unsigned int)a0 paused:(char)a1 mediaString:(const char *)a2 xpcMessageKey:(char *)a3 xpcOperationKey:(id)a4 completionBlock:(id /* block */)a5;
- (void)setMediaTypeToSpatialSourceIDMap:(id)a0;
- (void)setStreamTokens:(id)a0;
- (char)setupMediaStates;
- (void)setupNotificationQueue:(id)a0;
- (unsigned long long)spatialAudioSourceIDForMediaType:(unsigned int)a0;
- (void)stateTransitionForMediaType:(unsigned int)a0 mediaState:(unsigned int)a1 previousState:(unsigned int)a2 didSucceed:(char)a3 error:(id)a4;
