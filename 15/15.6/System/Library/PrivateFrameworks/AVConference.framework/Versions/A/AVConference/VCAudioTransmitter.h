@class VCAudioTier, NSDictionary, VCRedundancyControllerAudio, VCAudioRedBuilder, VCTransportSession, NSMutableArray, VCPacketBundler, NSString, VCAudioTierPicker, AVCStatisticsCollector, NSArray, VCAudioPayload, VCAudioPayloadConfig;
...
@interface VCAudioTransmitter : NSObject <VCAudioIOSink, VCConnectionChangedHandler> {
    ...
    char _sendSilenceSamplesEnabled;
    ...
    char _supportsCodecBandwidthUpdate;
    ...
    char _isSID;
    unsigned int _totalEncodedBytesPerBundle;
    ...
    char _targetBitrateIsAudioOnly;
...
@property (readonly, nonatomic) char isRedEnabled;
@property (readonly, nonatomic) char ignoreSilence;
...
@property (nonatomic) char isAudioStalled;
...
@property char periodicReportingEnabled;
...
- (void)handleCodecModeChangeEvent:(struct _VCAudioCodecModeChangeEvent { int x0; int x1; int x2; unsigned int x3; struct EVSRFParams { unsigned int x0; unsigned int x1; } x4; })a0 didUpdateBandwidth:(char *)a1 didUpdateBitrate:(char *)a2;
- (void)initAudioValues;
- (char)allocateLastInputSampleBuffer:(unsigned int)a0;
...
- (char)isCodecModeChangeRequestValid:(struct _VCAudioCodecModeChangeEvent { int x0; int x1; int x2; unsigned int x3; struct EVSRFParams { unsigned int x0; unsigned int x1; } x4; })a0;
- (char)isStandaloneStreamMode;
...
- (unsigned int)redundancyControllerModeForOperatingMode:(int)a0 isRedEnabled:(char)a1 isHigherAudioREDCutoverU1Enabled:(char)a2;
- (void)registerReportingTask;
- (void)removeUnusedAudioPayloads;
- (void)reportRTCPPacket;
- (void)reportRedundancyConfigChange;
- (char)sendCodecModeChangeEvent:(struct _VCAudioCodecModeChangeEvent { int x0; int x1; int x2; unsigned int x3; struct EVSRFParams { unsigned int x0; unsigned int x1; } x4; })a0;
- (unsigned int)sentAudioBytesShort;
- (void)setCellTech:(int)a0 remoteCellular:(int)a1 isIPV6:(int)a2 audioCap:(unsigned int)a3;
- (void)setRedNumPayloads:(int)a0 withMaxDelay:(int)a1 shouldResetHistory:(char)a2 shouldEnableShortRED:(char)a3;
- (void)setStreamIDs:(id)a0;
- (char)setupAudio:(id *)a0;
- (char)setupAudioEncoders;
- (void)setupAudioHeaderSize;
- (char)setupAudioTierPicker;
- (char)setupEncoderBuffer;
- (char)shouldUpdateCodecBandwidth:(struct _VCAudioCodecModeChangeEvent { int x0; int x1; int x2; unsigned int x3; struct EVSRFParams { unsigned int x0; unsigned int x1; } x4; })a0;
- (char)shouldUpdateCodecBitrate:(struct _VCAudioCodecModeChangeEvent { int x0; int x1; int x2; unsigned int x3; struct EVSRFParams { unsigned int x0; unsigned int x1; } x4; })a0;
- (void)updateAudioTxBitrate;
- (void)updateAudioTxRate;
- (void)useAudioPayload:(id)a0 withBitrate:(unsigned int)a1 redNumPayloads:(unsigned int)a2;
