...
@interface VCAudioStreamSendGroup : VCMediaStreamSendGroup <VCRedundancyControllerDelegate, VCAudioStreamGroup> {
    char _lastEnqueuedStreamUpdateEventWasEmpty;
    ...
    char _shouldScheduleMediaQueue;
    float _averageInputPower;
    char _audioControllerSupportsVoiceActivityDetection;
}

@property (readonly) VCAudioStreamGroupCommon *common;
@property (nonatomic, setter=setVADFilteringEnabled:) char isVADFilteringEnabled;
@property (nonatomic, setter=setCurrentDTXEnabled:) char isCurrentDTXEnabled;
...
- (char)configureAudioStreams:(id)a0 deviceRole:(int)a1 operatingMode:(int)a2;
- (char)configureStreams;
- (char)createIOEventQueues;
...
- (char)startCaptureIfNeeded:(char)a0;
- (void)startDynamicDucker;
- (void)startVoiceActivityDetection;
- (char)stopCaptureForEndToEndStreamIfNeeded;
- (void)stopDynamicDucker;
- (void)stopVoiceActivityDetection;
- (void)updateActiveVoiceOnly:(char)a0;
