...
@interface AVCStreamInput : NSObject {
    ...
    char _didInitializeSuccessfully;
    char _serverDiedWhileStarted;
    unsigned long long _memoryPoolSize;
    char _streamInputIDString[20];
    char _printSampleBufferDetailsEnabled;
    char _requireTileIndexAttachment;
...
- (char)pushSampleBuffer:(struct opaqueCMSampleBuffer { } *)a0 error:(id *)a1;
- (void)registerBlocksForNotification;
- (void)registerDidServerDieBlock:(id)a0;
- (void)resetStats;
- (int)resetXPCConnection;
- (char)sendSampleBufferOverXPC:(struct opaqueCMSampleBuffer { } *)a0 error:(id *)a1;
- (void)senderQueueUnlock;
- (int)setupDelegate:(id)a0 delegateQueue:(id)a1;
- (int)setupXPCConnection;
- (int)setupXPCQueue;
- (void)tearDownConnectionWithTerminateMessage:(char)a0;
