@class VCMediaStreamConfig, VCRateSharingGroup, VCNetworkFeedbackController, VCCallInfoBlob, AVCBasebandCongestionDetector, NSObject, NSMutableArray, VCTransportSession, TimingCollection, AVCRateControlFeedbackController, NSString, VCWeakObjectHolder, VCMediaStreamRateAdaptation, AVCStatisticsCollector, VCConnection, VCDatagramChannelIDS, VCMediaStreamTransport, NSArray, NSError, VCMediaKeyIndex;
...
@interface VCMediaStream : VCObject <VCMediaStreamProtocol, RTCPReportProvider, VCSecurityEventHandler, VCConnectionChangedHandler, VCMediaTimeSync> {
    ...
    char _rtcpHeartbeatCancelled;
    char _rtcpDidTimeout;
    ...
    char _mediaControlInfoCallbacksRegistered;
    void /* function */ *_statisticsHandler;
    char _isWRMinitialized;
    char _isRTTBasedFIRThrottlingEnabled;
    char _isServerPacketRetransmissionEnabled;
    char _isNWMonitorSignalEnabled;
...
@property (readonly, nonatomic) char isSendingMedia;
...
@property (nonatomic, setter=setUplinkRetransmissionEnabled:) char isUplinkRetransmissionEnabled;
...
- (void)setRtcpEnabled:(char)a0;
- (void)setRtcpSendInterval:(double)a0;
- (void)setRtcpTimeOutEnabled:(char)a0;
- (void)setRtcpTimeOutInterval:(double)a0;
- (void)setRtpTimeOutEnabled:(char)a0;
...
- (void)setPause:(char)a0 withCompletionHandler:(id /* block */)a1;
...
- (id)getInvalidParamErrorForSetPause:(char)a0 didSucceed:(char)a1;
...
- (void)handleStartDidSucceed:(char)a0 withError:(id)a1;
...
- (char)initializeTransportSessionWithIDSDestination:(id)a0 error:(id *)a1;
- (char)initializeTransportSessionWithRTPNWConnectionID:(id)a0 rtcpNWConnectionID:(id)a1 error:(id *)a2 isConnectionStartDelayed:(char)a3;
- (char)initializeTransportSessionWithSocketDictionary:(id)a0 error:(id *)a1;
- (char)initializeTransportSetupInfoWithIDSDestination:(id)a0 error:(id *)a1;
...
- (char)initializeTransportSetupInfoWithTransportSession:(id *)a0;
...
- (void)setDecryptionTimeOutEnabled:(char)a0;
...
- (char)setupNWConnectionWithID:(id)a0;
- (id)setupRTPForIDS;
...
- (char)shouldStopReportingTimer;
- (char)shouldUseNWConnectionBackingSocket;
- (char)startMediaTransportsWithError:(id *)a0;
...
- (char)updateRemoteAddressWithConfig:(id)a0 error:(id *)a1;
