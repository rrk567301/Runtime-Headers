...
@interface VCAudioManager : VCObject <VCAudioIOControllerControl, VCAudioSessionDelegate> {
    ...
    char _spatialAudioDisabled;
    void **_audioSession;
    int _playbackMode;
    char _useOptimizedHandoversForTelephony;
...
+ (unsigned char)audioIOTypeWithAudioUnitProperties:(id)a0 shouldEnableAudioInput:(char)a1;
...
- (void)activateStartingClient:(id)a0 applyControllerFormat:(char)a1;
...
- (char)audioCallbacksRunning;
- (void)cleanupInterruptThread;
- (void)completeStartForAllStartingClients;
- (char)computeAllowAudioRecordingWithPreferredClient:(id)a0;
...
- (void)disableSpatialStreamParametersForClient:(id)a0;
- (void)enableSpatialStreamParametersForClient:(id)a0;
...
- (id)newAudioSessionMediaPropertiesForSystemAudioWithPreferredClient:(id)a0 audioUnitProperties:(id)a1 isInput:(char)a2;
- (id)newAudioSessionMediaPropertiesWithPreferredClient:(id)a0 audioUnitProperties:(id)a1;
- (id)newAudioUnitPropertiesForSystemAudioWithPreferredClient:(id)a0 isInput:(char)a1;
...
- (char)resetAUIOWithProperties:(id)a0;
- (char)resetAudioLimiterWithProperties:(id)a0;
- (char)resetAudioSessionWithProperties:(id)a0 interruptSuccessful:(char *)a1;
- (void)resetAudioTimestamps;
- (void)resetChannelMask;
- (void)serverDidDie;
- (void)setCurrentInputDeviceInternal:(id)a0;
- (void)setInputMetering;
- (void)setMute:(char)a0 forClient:(id)a1;
...
- (void)setupSpatialContextWithAudioUnitProperties:(id)a0 forceUpdate:(char)a1;
- (char)setupVAD;
- (char)shouldResetAudioSessionWithProperties:(id)a0;
- (char)shouldResetAudioUnitWithProperties:(id)a0;
- (char)startAUIOWithProperties:(id)a0;
- (char)startAudioSessionWithProperties:(id)a0;
- (void)startClient:(id)a0;
- (void)startInterruptThread;
- (void)startVADWithAudioUnitProperties:(id)a0;
- (char)stateIdleWithAudioUnitProperties:(id)a0 sessionProperties:(id)a1 client:(id)a2 newState:(unsigned int *)a3;
- (char)stateInterruptedShouldGoToRunning:(id)a0;
- (char)stateInterruptedShouldGoToStarted:(id)a0;
- (char)stateInterruptedWithAudioUnitProperties:(id)a0 sessionProperties:(id)a1 client:(id)a2 newState:(unsigned int *)a3;
- (char)stateRunningShouldTransitionToInterrupted:(id)a0;
- (char)stateRunningShouldTransitionToStarted:(id)a0;
- (void)stateRunningToSessionStarted;
- (char)stateRunningWithAudioUnitProperties:(id)a0 sessionProperties:(id)a1 client:(id)a2 newState:(unsigned int *)a3;
- (char)stateSessionStartedWithAudioUnitProperties:(id)a0 sessionProperties:(id)a1 client:(id)a2 newState:(unsigned int *)a3;
...
- (void)updateDirectionWithClient:(id)a0 settings:(const struct tagVCAudioIOControllerClientSettings { unsigned char x0; char x1; char x2; char x3; } *)a1 isClientRegistered:(char)a2;
- (void)updateSpatialAudioWithClient:(id)a0 settings:(const struct tagVCAudioIOControllerClientSettings { unsigned char x0; char x1; char x2; char x3; } *)a1 isClientRegistered:(char)a2;
- (char)updateStateWithAudioIOClient:(id)a0;
- (void)updateVoiceProcessingWithClient:(id)a0 settings:(const struct tagVCAudioIOControllerClientSettings { unsigned char x0; char x1; char x2; char x3; } *)a1 isClientRegistered:(char)a2;
