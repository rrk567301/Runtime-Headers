...
@interface VCAVFoundationCapture : VCVideoCapture <VCVideoSource, AVCaptureVideoDataOutputSampleBufferDelegate, VCEffectsManagerDelegate, AVCaptureMetadataOutputObjectsDelegate, AVCaptureDataOutputSynchronizerDelegate> {
    ...
    char _viewPointCorrectionEnabled;
    char _needCenterStageMetadataForViewpointCorrection;
    char _cframingEnabled;
    char _isCaptureSIFRPreferred;
    int _captureSIFRPreferredOverride;
    char _captureFormatNonBinOnlyOverride;
    char _captureFormatPrefer16By9ForSquare;
    char _deviceSupportCinematicFraming;
    char _deviceSupportVideoEffects;
    char _isBackgroundBlurActive;
    char _isReactionActive;
    int _orientation;
    char _supportPortraitResolution;
    char _isCaptureDeviceInternal;
    char _beginCommitConfigForFormatSwitch;
    ...
    char _isPreviewing;
    char _isCapturing;
    char _forceMirrorCapture;
    char _forceDynamicEffectsFramerate;
    char _forcePearlCamera;
    char _isServerProcess;
    char _avCaptureDeviceInputLocked;
    int _frameCount;
    ...
    char _isViewpointSupported;
    char _useAVConferenced;
    char _enableBufferStatsDump;
    ...
    char _shouldRemoveExtraRotation;
    unsigned int _capturePixelFormat;
    char _shouldUseCompressedPixelFormat;
...
@property char faceMeshTrackingEnabled;
@property char effectsApplied;

+ (char)isServerProcess;
+ (char)deviceInputSupportsCenterStage:(id)a0;
...
+ (char)captureDeviceformatSupportsVideoEffects:(id)a0;
+ (char)deviceInputSupportsVideoEffects:(id)a0;
...
- (char)cameraContainsCaptureFormatWithWidth:(int)a0 height:(int)a1;
...
- (char)cameraSupportsNoQueueFormatWidth:(int)a0 height:(int)a1;
...
- (char)featureStatusForFeatureSupported:(char)a0 enabled:(char)a1;
...
- (char)isBackCamera;
- (char)isCurrentCaptureDeviceAppleProduct;
- (char)isFormatMaxFrameRateSupported:(id)a0 frameRate:(int)a1;
- (char)isFrameRateActive:(int)a0 captureFormat:(id)a1;
- (char)isFrontCamera;
- (char)isInternalCamera;
- (char)isPortraitModeSupported;
- (char)isPortraitResolutionCaptureActive;
- (char)isReactionSupported;
- (char)isVideoCaptureOutputAspectRatioOverrideOptimizedSupported;
...
- (void)reportStatus:(struct __CFDictionary { } *)a0 forKey:(struct __CFString { } *)a1 supported:(char)a2 enabled:(char)a3;
...
- (char)setCaptureDeviceMinFrameRateIfNeeded:(int)a0;
- (int)setFrameRateInternal:(int)a0 forceSourceFrameRateDidChange:(char)a1;
- (void)setMetaDataObjectsDelegate;
- (void)setReactionActive:(char)a0;
- (void)setUpBackgroundBlurDelegate:(char)a0;
- (void)setUpCaptureDeviceObserver:(char)a0 keyPath:(id)a1;
- (void)setUpCaptureDevicePropertyObservers:(char)a0;
...
- (void)setVideoStabilization:(char)a0;
- (void)setViewPointCorrectionEnabled:(char)a0;
- (int)setWidth:(int)a0 height:(int)a1 frameRate:(int)a2;
- (char)shouldAddDepthData;
- (char)shouldAddTrackedFacesData;
- (char)shouldAdjustCaptureWithOrientation:(int)a0;
- (char)shouldResizeWithCaptureSize:(struct CGSize { double x0; double x1; })a0 requestSize:(struct CGSize { double x0; double x1; })a1;
- (char)shouldUseCameraVideoStabilization;
- (int)startCaptureWithWidth:(int)a0 height:(int)a1 frameRate:(int)a2;
- (char)supportsDepthDataFormats;
...
- (void)updateCenterStageRegionOfInterestWithLock:(char)a0;
- (void)updateDepthFrameRate;
- (void)updateIsCaptureDeviceInternal;
- (void)updateRenderProcessFrameRate:(id)a0;
- (char)updateVideoDataOutputVideoSettingResolution:(int)a0 requestHeight:(int)a1 captureFormat:(id)a2;
