@class NSMutableDictionary, VCStatsRecorder, NSObject, VCConnectionSelector, VCWRMHandler, VCLinkProbingHandler, NSMutableArray, NSString, AVCStatisticsCollector, VCConnectionHealthMonitor, VCWifiAssistManager, VCConnectionStatisticsCollector, VCDuplicationHandler, VCCoreMotionManager;
...
@interface VCConnectionManager : NSObject <VCConnectionHealthMonitorDelegate, VCWifiAssistManagerDelegate, VCLinkProbingHandlerDelegate, VCCoreMotionManagerDelegate> {
    ...
    char _isRTXSupported;
    char _disallowAlternateConnectionForRTXSupportWhenVideoDegraded;
    VCCoreMotionManager *_coreMotionManager;
    char _multipleInterfacesAvailable;
    unsigned int _persistentSentPacketCount[256];
    unsigned int _persistentRecvPacketCount[256];
    char _isFirstPacketSentCount;
    char _isFirstPacketReceivedCount;
    ...
    char _isNWConnectionMonitorEnabled;
    char _isNWConnectionMonitorCellularEnabled;
    char _ignoreNWConnectionMonitorNotification;
    NSString *_nwMonitorWiFiInterfaceName;
    NSString *_nwMonitorCellularInterfaceName;
    struct { unsigned char version; unsigned char direction; unsigned char interfaceType; unsigned char notificationType; unsigned long long timestamp; unsigned long long maxThroughputBps; unsigned long long totalByteCount; unsigned int flushableQueueSize; unsigned int nonFlushableQueueSize; unsigned int averageDelayMillisecond; unsigned long long averageThroughputBps; int rateTrendSuggestion; unsigned int packetLossPerFrame; unsigned short sequenceNumber; union { struct { unsigned char frequencyBand; unsigned char intermittentState; unsigned short estimatedIntermittentPeriod; unsigned short singleOutagePeriod; unsigned char btCoex; unsigned char radioCoex; unsigned char qualityScoreDelayRx; unsigned char qualityScoreDelayTx; unsigned char qualityScoreLossRx; unsigned char qualityScoreLossTx; unsigned char qualityScoreChannel; float offChannelTimeRatio; BOOL detectedFrequentOffChannelActivity; unsigned short wlanDutyCycle; unsigned int wifiObservedTxBitrate[6]; unsigned char maxRadioCoex; long long accumulatedOffChannelTime; unsigned short maxSingleOutagePeriod; float qualityScoreDelayRxAverage; float qualityScoreDelayTxAverage; float qualityScoreLossRxAverage; float qualityScoreLossTxAverage; float qualityScoreChannelAverage; } wifi; struct { unsigned char radioAccessTechnology; short referenceSignalLevel; short signalLevel; char signalQuality; unsigned char uplinkBLER; unsigned char downlinkBLER; unsigned char bandwidthLimitationIndication; unsigned char cdrxState; unsigned short cdrxCycle; unsigned short estimatedOutagePeriod; unsigned char outageState; } baseband; } ; } _lastWiFiNWStatistics;
...
@property (readonly) char isPreWarmStateEnabled;
@property (nonatomic) char remotePreWarmStateEnabled;
@property (nonatomic) char shouldLimitMultiwayBandwidthWhenConstrained;
...
@property char supportDuplication;
@property (getter=isGftTLEEnabled) char gftTLEEnabled;
...
@property (nonatomic, setter=setPreferRelayOverP2PEnabled:) char preferRelayOverP2PEnabled;
@property char fastMediaDuplicationEnabled;
@property char cellPrimaryInterfaceChangeEnabled;
@property char duplicateImportantPktsEnabled;
@property char lowNetworkModeEnabled;
@property char duplicationEnhancementEnabled;
@property (nonatomic, setter=setiRATDuplicationEnabled:) char iRATDuplicationEnabled;
...
@property char optIntoExistingSubscribedStreams;
@property (nonatomic) char isDuplicationAllowed;
@property (nonatomic) char isOneToOneModeEnabled;
@property (nonatomic) char useMediaDrivenDuplication;
@property (nonatomic) char startConnectionHealthMonitoring;
@property (nonatomic) char isRemoteMediaQualityDegraded;
...
@property (nonatomic) char isOneToOneScreenEnabled;
@property (nonatomic) char isRemoteNetworkQualityDegraded;
@property (readonly) char enableNetworkConditionMonitoring;
@property (readonly) char enableMotionBasedDuplication;
@property (nonatomic) char isUplinkRetransmissionSupported;
...
- (void)addDistinctConnectionInterfaceToSet:(id)a0 withInterfaceType:(int)a1;
- (void)addMediaHealthStatsHistoryEntryForParticipantID:(id)a0;
- (void)aggregateStallTimeDuration:(struct { double x0; char x1; double x2; double x3; } *)a0 idsParticipantID:(id)a1;
- (void)applyLinkFlags:(unsigned short)a0 isCellular:(char)a1;
- (void)applyLinkRecommendation;
- (void)applyRemoteLinkFlags:(unsigned short)a0 isCellular:(char)a1;
...
- (void)connectionHealthDidUpdate:(int)a0 isLocalConnection:(char)a1;
- (void)copyPersistenPacketCounts:(unsigned int *)a0 isOutgoing:(char)a1 size:(int)a2;
...
- (void)didChangeWifiAssistAvailable:(char)a0 reason:(unsigned char)a1;
- (void)disableRemotePreferredInterfaceInference:(char)a0;
- (void)duplicationStateUpdateWithEvent:(struct { unsigned char x0; char x1; char x2; char x3; char x4; char x5; char x6; char x7; int x8; char x9; char x10; })a0;
...
- (int)getCellularTechForActiveConnectionWithQuality:(int)a0 forLocalInterface:(char)a1;
...
- (int)getConnectionTypeForActiveConnectionWithQuality:(int)a0 forLocalInterface:(char)a1;
...
- (void)handleReportingSymptom:(unsigned int)a0;
- (char)hasSameReportingConfig:(id)a0 oldPrimaryConnection:(id)a1;
- (void)invalidateNetworkConditionMonitor;
- (char)isBetterConnection:(id)a0 asPrimary:(char)a1;
- (char)isBetterPrimaryConnectionAvailable;
- (char)isConnectedOnIPv6ForActiveConnectionWithQuality:(int)a0;
- (char)isConnectedOnRelayForActiveConnectionWithQuality:(int)a0;
- (char)isDuplicationAllowedForParticipantID:(id)a0 bucketsToSum:(int)a1 threshold:(double)a2;
- (char)isDuplicationEnabled;
- (char)isOptimalConnection:(id)a0 asPrimary:(char)a1 interfaceMask:(int)a2;
- (char)localOrRemoteHasMultipleInterfaceAvailable:(id)a0 remoteInterface:(id)a1;
...
- (int)processRemoteWRMSuggestion:(int)a0 isRemoteDuplicating:(char)a1;
...
- (void)renewNWMonitor;
- (void)reportActiveConnectionOneToOne:(id)a0 isAudioOnly:(char)a1;
...
- (void)reportFirstActiveConnectionOneToOne:(id)a0 remoteOSVersion:(id)a1 redState:(char)a2 isAudioOnly:(char)a3;
...
- (void)setConnectionPause:(char)a0 isLocalConnection:(char)a1;
...
- (void)setPreferRelayOverP2P:(char)a0 reason:(unsigned char)a1;
...
- (char)shouldDropCurrentPrimaryConnectionWithConnectionStats:(struct { double x0; double x1; double x2; double x3; double x4; BOOL x5; int x6; } *)a0;
...
- (void)updateConnectionSelectionPolicyWithPreferE2E:(char)a0;
...
- (void)updateEncryptionConfig:(id)a0;
...
- (void)updateMediaHealthStats:(struct { double x0; char x1; double x2; double x3; } *)a0 idsParticipantID:(id)a1;
...
- (void)updateWithRemoteMediaHealthDuplication:(char)a0;
