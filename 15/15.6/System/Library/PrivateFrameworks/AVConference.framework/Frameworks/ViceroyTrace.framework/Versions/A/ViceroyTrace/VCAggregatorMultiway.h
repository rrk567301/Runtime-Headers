...
@interface VCAggregatorMultiway : VCAggregator <SegmentStatsDelegate> {
    ...
    char _isFullsizeUI;
    char _isGridView;
    char _isUplinkScreenEnabled;
    char _reportScreenUpgrade;
    char _isFullScreenCapture;
    ...
    char _sessionAggregatedScreenLocalParticipantEnabled;
    char _isSessionFromOneToOneMode;
    char _isSessionGoingToOneToOneMode;
    ...
    char _isExpanseEnabled;
    VCDurationHistogram *_pipThermalDurations;
    char _isCenterStageEnabled;
    char _isPortraitBlurEnabled;
    ...
    char _reportRateControlExperimentRemote;
    ...
    char _reportNetworkCapabilities;
    char _isUplinkRetransmissionEnabled;
    ...
    char _lastUsableLinkReceivedAfterReconnect;
    unsigned long long _reconnectCount;
    char _isDuplicationPossible;
    ...
    char _hasReportedSliceStatusFailedSymptom;
    char _hasReportedSliceStatusNotReceivedSymptom;
}
...
@property char currentNoRemoteState;
@property double noRemoteStartTime;
@property char currentNoRemoteState_Alternate;
...
- (char)checkSegmentCanBeUpdated:(id)a0;
- (char)checkSliceStatus:(id)a0 hasValuesOnlyForStatus:(int)a1;
- (char)currentUISize;
...
- (char)isAllowlistedEvent:(unsigned short)a0;
- (char)isDuplicationChangedWithType:(unsigned short)a0;
- (char)isParticipantLive:(id)a0;
...
- (void)processSliceStatusABCSymptoms:(id)a0 currentSegmentName:(id)a1;
...
- (void)processUISizeEventForParticipant:(id)a0 isFullSize:(char)a1;
...
- (void)processVideoDegraded:(char)a0 payload:(id)a1;
...
- (char)shouldConnectionEstablishmentStartNewSegment:(id)a0;
- (char)shouldRestBoundedAlgoScore;
...
- (double)timeWeightedNumberOfParticipantsWebOnly:(char)a0;
...
- (void)updateCurrentSliceStatusWithPayload:(id)a0 shouldUpdateOneToOneRemoteStatus:(char)a1;
- (void)updateDeviceOutputLatency:(id)a0;
- (void)updateDuplicationPossibility:(char)a0;
...
- (void)updateNoRemoteState:(id)a0 state:(char)a1;
- (void)updateNoRemoteState_Alternate:(id)a0 state:(char)a1;
...
- (void)updateRoleModeTransport:(unsigned short)a0 deviceRole:(unsigned short)a1 transportType:(unsigned short)a2 isInitiator:(char)a3;
