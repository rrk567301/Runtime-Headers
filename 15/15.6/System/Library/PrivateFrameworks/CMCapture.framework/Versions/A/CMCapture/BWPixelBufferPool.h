...
@interface BWPixelBufferPool : NSObject <BWPixelBufferProvider> {
    ...
    char _reportSlowBackPressureAllocations;
    NSObject<OS_dispatch_semaphore> *_backPressureSemaphore;
    _Atomic unsigned long long _backpressureSemaphoreWaitID;
}

@property (readonly, nonatomic, getter=isPixelBufferAvailable) char pixelBufferAvailable;
...
@property (readonly, nonatomic) char usesMemoryPool;
...
- (id)initWithVideoFormat:(id)a0 capacity:(unsigned long long)a1 name:(id)a2 clientProvidesPool:(char)a3 memoryPool:(id)a4 providesBackPressure:(char)a5 reportSlowBackPressureAllocations:(char)a6;
- (id)initWithVideoFormat:(id)a0 capacity:(unsigned long long)a1 name:(id)a2 memoryPool:(id)a3;
- (id)initWithVideoFormat:(id)a0 capacity:(unsigned long long)a1 name:(id)a2 memoryPool:(id)a3 additionalPixelBufferAttributes:(id)a4 providesBackPressure:(char)a5 reportSlowBackPressureAllocations:(char)a6;
- (int)preallocate;
- (void)preallocateWithCompletionHandler:(id /* block */)a0;
- (char)prefetch:(int)a0 lastEmittedSurfaceID:(unsigned int)a1;
...
- (char)waitForAvailablePixelBuffer;
