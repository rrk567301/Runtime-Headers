...
@interface BWSmartStyleLearningNode : BWNode <BWInferenceFormatProvider, BWInferencePixelBufferPoolProvider, BWSemanticStyleSceneObserver> {
    ...
    char _propagateMasks;
    char _preLTMThumbnailFound;
    struct { long long value; int timescale; unsigned int flags; long long epoch; } _previousPTS;
    char _generateCoefficients;
    char _generateImageStats;
    char _runVMRefiner;
    char _runVMRefinerFPRejection;
    char _gdcAttachmentsEnabled;
    ...
    char _bypassStyleEngineForStandardStyle;
}

@property (retain, nonatomic) FigCaptureSmartStyle *smartStyle;
@property (nonatomic) char disableWaitForCoefficientsOnFirstFrame;
@property (nonatomic) char subjectRelightingEnabled;
...
- (void)_asynchronouslyLearnWithContainer:(struct opaqueCMSampleBuffer { } *)a0 inputUnstyledSampleBuffer:(struct opaqueCMSampleBuffer { } *)a1 withUnrefinedMasks:(id)a2 withStats:(id)a3 withStatsExtended:(id)a4 styleToLearn:(id)a5 shouldBypass:(char)a6;
...
- (id)_undistortMasks:(id)a0 unstyledSampleBuffer:(struct opaqueCMSampleBuffer { } *)a1 currentPTS:(struct { long long x0; int x1; unsigned int x2; long long x3; })a2 useIntermediatePools:(char)a3;
...
- (id)initWithOutputs:(unsigned long long)a0 masksRefinerEnabled:(char)a1 propagateMasks:(char)a2;
...
- (int)processVideoSampleBuffer:(struct opaqueCMSampleBuffer { } *)a0 frameEmitted:(char *)a1;
