...
@interface BWPreviewGyroStabilization : NSObject {
    struct PreviewGyroStabilizationTransformContext { int width; int height; struct CGPoint { double x; double y; } imageCenter; float pixelsPerMicron; struct { struct { int x; int y; } origin; struct { int width; int height; } size; } finalCropRect; float limitFactor; char prevTransformLimited; char forceIdentity; } _transformContext;
...
- (float)_computeSmoothParameterForNonStationaryCaseWithPanningStatus:(char)a0 frameIsSphereStabilized:(char)a1 previousLowPassParameter:(float)a2 rampDownRate:(float)a3 rampUpRate:(float)a4 targetSmoothParameter:(float)a5;
- (float)_computeSmoothParameterForPhysicalTripod:(char)a0 lowpassParameter:(float)a1 previousLowPassParameter:(float)a2 frameRateNormalization:(float)a3;
- (float)_computeSmoothParameterForStationaryCaseWithPanningStatus:(char)a0 translation:(float)a1 previousLowPassParameter:(float)a2 rampDownRate:(float)a3 rampUpRate:(float)a4 targetSmoothParameter:(float)a5;
...
- (void)_setDefaultParametersForStillImagePreview:(char)a0 isFrontCamera:(char)a1;
- (struct PreviewStabilizationFrameRateCompensatedParameter { float x0; float x1; float x2; float x3; float x4; float x5; })_updateTripodSmoothParametersWithSphereVideoEnabled:(char)a0 frameRateNormalization:(double)a1;
...
- (id)initForStillImagePreview:(char)a0;
