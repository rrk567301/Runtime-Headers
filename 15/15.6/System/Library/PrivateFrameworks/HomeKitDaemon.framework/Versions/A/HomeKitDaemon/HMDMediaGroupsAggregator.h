...
@protocol HMFTimerProvider, HMDMediaGroupsAggregatorDelegate, HMELastEventStoreReadHandle, HMDMediaGroupsAggregatorDataSource, HMDFeaturesDataSource, HMESubscriptionProviding, HMEEventForwarder;

@interface HMDMediaGroupsAggregator : HMFObject <HMDMediaSystemsAggregateDataGeneratorDataSource, HMFTimerProvider, HMFLogging, HMFTimerDelegate, HMEEventConsumer, HMDMediaGroupsAggregatorMessengerDelegate> {
...
- (id)destinationManagerWithIdentifier:(id)a0;
...
- (void)fixupGroupsForRemovedParticipantAccessoryUUID:(id)a0;
...
- (char)isRunningAsPrimary;
- (id)localGroupWithIdentifier:(id)a0;
- (void)notifyOfStoppedState;
- (void)notifyOfUpdatedMediaGroupsAggregateData:(id)a0;
...
- (char)removeDestinationControllerDataWithParentIdentifier:(id)a0;
- (char)removeDestinationWithParentIdentifier:(id)a0;
...
- (char)setState:(unsigned long long)a0 previousState:(unsigned long long *)a1;
- (char)shouldWarmUp;
- (void)startWarmUpTimer;
- (void)unregisterForAllEvents;
- (void)unregisterForEventsForAccessory:(id)a0;
- (char)updateAssociatedGroupIdentifier:(id)a0 forGroupIdentifier:(id)a1 error:(id *)a2;
- (void)updateDestinationWithIdentifier:(id)a0 audioGroupIdentifier:(id)a1;
