...
@property (nonatomic, readonly) char isDemoModeV2Active;
@property (nonatomic, readonly) char isDemoModeV2WithoutCKEnabled;
@property (nonatomic, readonly) id<HMDDemoModeDataSourceProtocol> demoModeDataSource;
...
@property (readonly, nonatomic) char isCloudKitRequiredForHH2;
@property (retain, nonatomic) HMDDeviceSetupConfiguringController *configuringStateController;
@property (readonly, nonatomic) char isCurrentDeviceResident;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
...
@property (readonly, nonatomic, getter=isResidentEnabled) char residentEnabled;
@property char pcsEnabled;
@property (nonatomic) long long residentEnabledState;
@property (getter=isAccessAllowedWhenLocked) char accessAllowedWhenLocked;
@property (nonatomic) char cacheUseAllowed;
...
@property (nonatomic) char speakersAreConfigured;
@property (nonatomic) char speakersAreConfiguredInOwnedHomes;
...
@property char missingHomeDataRecord;
...
@property char hasAccessoryBrowsingBeenRequested;
@property (readonly) HMDWidgetTimelineRefresher *widgetTimelineRefresher;
@property char hasLoadedData;
@property char didCreateAddACWGKeysOperation;
...
@property char initialPCSStatusFetchCompleted;
...
@property (nonatomic, getter=isHomeManagerZoneFirstFetch) char homeManagerZoneFirstFetch;
...
@property (nonatomic) char forceFetchHomeManagerZone;
...
@property (retain, nonatomic) id<HMDFMFHandlerProtocol> fmfHandler;
...
@property (readonly, nonatomic) char zoneFetchFailed;
...
@property (nonatomic, getter=isHH2MigrationInProgress) char hh2MigrationInProgress;
...
@property (nonatomic) char haveQueuedUpdateAccessories;
...
@property (nonatomic) char assistantIdentifierChanged;
@property (readonly) HMFPromise *startupPromise;
@property (readonly) char startupHasCompleted;
@property (readonly) char deviceAccountHasSettled;
@property (readonly) char firstCloudKitImportComplete;
...
@property char isOwnerPingPending;
...
@property char postSyncDataUpdatedNotification;
@property (retain, nonatomic) id<HAPKeyStore> hapKeyStore;
@property (retain, nonatomic) id<HMDDatabase> defaultDatabase;
@property (retain, nonatomic) id<HMMLogEventSubmitting> logEventSubmitter;
@property (retain, nonatomic) HMDAppleAccountManager *appleAccountManager;
...
@property (readonly, getter=isDataSyncInProgress) char dataSyncInProgress;
@property char hasFinishedStartingUp;
@property char hasBeenOneMinuteSinceFinishedStartingUp;
...
@property (readonly, getter=isNetworkConnectionAvailable) char networkConnectionAvailable;
...
@property (readonly) char isHH1EOLEnabled;
@property (readonly) id<HMDCompositeSettingsSiriEndpointProfileBridge> siriEndpointBridge;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
@property (readonly) char legacyZoneHasRecordsAvailable;
...
+ (id)createIdentifierSalt:(id)a0 deviceSpecific:(char)a1;
+ (id)deriveIntentGroupIdentifierFromBaseUUID:(id)a0;
+ (char)doesSaveReasonAffectHomeManager:(id)a0;
+ (char)doesSaveReasonAffectOnlyLocalData:(id)a0;
+ (char)doesSaveReasonNotAffectLocalData:(id)a0;
+ (char)doesSaveReasonRequireForceSyncToWatch:(id)a0;
...
+ (char)isPrefSetForHH2WithoutCloudKit;
...
+ (char)shouldIgnoreExpectedConfigurationVersionUpdateForReason:(id)a0;
+ (char)shouldIncrementGenerationCounterForReason:(id)a0;
...
- (char)shouldPostHH2UpgradeRequired;
...
- (char)userWithMergeIdIsMemberOfAHome:(id)a0;
...
- (char)restoreMediaSystemHintsFromDisk;
- (void)setFirstCloudKitImportComplete:(char)a0;
...
- (void)_acceptHomeInviteFromAccount:(id)a0 message:(id)a1 trackInvite:(char)a2;
- (void)_accessoriesAreLocallyReachableOnTransientDevice:(char)a0 forHome:(id)a1;
...
- (char)_associateAccessories:(id)a0 withHomes:(id)a1;
...
- (char)_capabilitiesAreSupported:(id)a0;
...
- (void)_checkForRemotePeersAndRegisterForRemoteNotifications:(char)a0;
...
- (char)_configureHomes:(id)a0 uncommittedTransactions:(id)a1;
- (char)_configureHomesImpl:(id)a0 uncommittedTransactions:(id)a1;
- (id)_currentHome;
- (id)_dataForPersistentStoreIncrementingGeneration:(char)a0 reason:(id)a1;
...
- (void)_eraseConfiguration:(char)a0 completionHandler:(id /* block */)a1;
- (void)_eraseLocalHomeConfiguration;
- (void)_eraseLocalHomeConfigurationAfterSignOut;
- (void)_eraseLocalHomeConfigurationAndDeleteMetadata:(char)a0 completionQueue:(id)a1 completion:(id /* block */)a2;
- (void)_eraseLocalHomeConfigurationAndDeleteMetadata:(char)a0 reason:(unsigned long long)a1 completionQueue:(id)a2 completion:(id /* block */)a3;
...
- (char)_findAnyAccessoryWithIdentities:(id)a0 inAccessoryServers:(id)a1;
...
- (void)_generateAssistantSyncDataAndIncrementVersion:(char)a0 requestSync:(char)a1 urgent:(char)a2 completion:(id /* block */)a3;
- (id)_getAssistantHashingData;
- (id)_getListOfUsersToPushMetadataChangesTo;
- (id)_getRequestedState:(id)a0 activity:(id)a1;
- (void)_getRuntimeStateUpdateForHomeManager:(char)a0 includeMediaAccessorySessionState:(char)a1 options:(unsigned long long)a2 includeResidentDeviceState:(char)a3 completion:(id /* block */)a4;
...
- (char)_handleAccessoryDiagnosticStateQueryWithResponse:(id)a0 hasAdditionalRequest:(char)a1 error:(id)a2 completion:(id /* block */)a3;
...
- (char)_isAutoAcceptMetadataPresentForHome:(id)a0 owner:(id)a1;
- (char)_isMetadataExpiredForHomeAwaitingAutoAccept:(id)a0;
- (id)_legacyContainer;
...
- (void)_loadMessageDispatcher:(id)a0 accessoryBrowser:(id)a1 messageFilterChain:(id)a2 homeData:(id)a3 localDataDecryptionFailed:(char)a4 identityRegistry:(id)a5 accountRegistry:(id)a6 uncommittedTransactions:(id)a7 backingStoreFactory:(id /* block */)a8 reloadData:(char)a9;
...
- (void)_notifyCurrentHomeUpdated:(id)a0 isLocalUpdate:(char)a1;
- (void)_notifyMetadata:(id)a0 limitToSPIClients:(char)a1 message:(id)a2 invokeResponseHandler:(char)a3;
...
- (id)_prepareDataForDevicesOnSameAccountForHome:(id)a0 remoteGateway:(char)a1 isAtLeastV4:(char)a2;
- (id)_prepareDataForDevicesOnSameAccountForHome:(id)a0 remoteGateway:(char)a1 isAtLeastV4:(char)a2 migrateToHH2:(char)a3;
- (id)_prepareHomesVersionDict;
- (void)_processAnyPendingRequestsForRemoteAccess:(char)a0;
...
- (char)_processTestModeCurrentHomeOverride:(id)a0 error:(id *)a1;
...
- (char)_processTestModeOverridesPayload:(id)a0 error:(id *)a1;
...
- (void)_reachabilityMonitorChanged:(char)a0;
- (void)_readAccessAllowedWhenLockedSettingFromLocalStore;
- (char)_redirectAppDataRequestToResidentWithMessage:(id)a0;
...
- (char)_removeAndAddKeyPair:(id)a0 userName:(id)a1 eraseReason:(unsigned long long)a2;
- (void)_removeCloudZone:(id)a0 updateHomeManager:(char)a1;
...
- (void)_removeHome:(id)a0 withMessage:(id)a1 saveToStore:(char)a2 notifyUsers:(char)a3 shouldRemovePairings:(char)a4;
- (void)_removeIncomingInvitation:(id)a0;
- (char)_removeLegacyHomeArchive:(id *)a0;
- (void)_removePendingDataSyncAcksForUser:(id)a0 forHome:(id)a1;
- (void)_resetSiriSyncNotification;
- (id)_runtimeState;
- (void)_saveAccessAllowedWhenLockedToLocalStore:(char)a0 message:(id)a1;
- (void)_saveAssistantHashingData:(id)a0;
- (void)_saveToPersistentStore:(char)a0 reason:(id)a1;
...
- (void)_sendHomeDataToAllWatchesMigrateToHH2:(char)a0 completionHandler:(id /* block */)a1;
- (void)_sendHomeDataToAllWatchesWithCompletion:(id /* block */)a0;
- (void)_sendHomeDataToWatch:(id)a0 completionHandler:(id /* block */)a1;
- (void)_sendHomeDataToWatch:(id)a0 migrateToHH2:(char)a1 completionHandler:(id /* block */)a2;
- (void)_sendOwnerPingMessageToAccountHandle:(id)a0;
- (void)_sendUserAdded:(id)a0 destination:(id)a1 toHome:(id)a2;
- (void)_sendUserRemoved:(id)a0 fromHome:(id)a1 pairingUsername:(id)a2 pushToCloud:(char)a3 completionHandler:(id /* block */)a4;
...
- (char)_setPrimaryHome:(id)a0 idsDataSync:(char)a1;
- (void)_setUniqueDeviceIdSalt:(id)a0;
- (char)_shouldDecodeMessage:(id)a0 error:(id *)a1;
- (char)_shouldHandleHomeDataSync:(id)a0 remoteHome:(id)a1 sourceDeviceVersion:(id)a2;
- (char)_shouldNotifyClientsAboutGenerationCounterUpdate;
...
- (char)_submitSpamReportToIDS:(id)a0;
- (void)_teardownRemoteAccessForHome:(id)a0;
- (void)_teardownRemoteAccessForHomeCommon:(id)a0 isCompanion:(char)a1;
...
- (char)_updateAccessAllowedWhenLockedToLocalStore:(char)a0 managedObjectContext:(id)a1;
- (char)_updateAccessoriesConfigured;
- (void)_updateCurrentHomeIfNecessary;
- (void)_updateGenerationCounterWithReason:(id)a0 sourceUUID:(id)a1 shouldNotifyClients:(char)a2;
- (void)_updateHome:(id)a0 configurationVersion:(long long)a1;
- (void)_updateHomesAboutNetworkConnectivity:(char)a0;
...
- (char)_updatePreferencesForConfiguredHomes;
- (char)_updatePreferencesForCurrentHome;
- (void)_updateResidentEnabledOnThisDevice:(char)a0 forceNotify:(char)a1 message:(id)a2;
...
- (void)accessoriesAreLocallyReachableOnTransientDevice:(char)a0 forHome:(id)a1;
...
- (char)addHH2KeyInResponsePayload:(id)a0 invitation:(id)a1;
- (void)addHome:(id)a0;
...
- (char)areThereAnyTTSUSessionsOngoing;
...
- (char)checkConflictInHomeNamespaceWithName:(id)a0 options:(unsigned long long)a1 namespaceUUID:(id)a2 error:(id *)a3;
- (void)checkForRemotePeers;
- (void)cleanupLocalConfiguration;
- (void)clearMediaSystemHints:(char)a0;
...
- (char)daemonHasLoadedSceneGraph;
- (void)dataSyncInProgressUpdatedNotification:(id)a0;
- (char)dataSyncInProgressWithState:(unsigned long long *)a0 withMessage:(id)a1;
...
- (void)determineDataSyncSateForHH2:(char *)a0 homeManagerDataSyncState:(unsigned long long *)a1 homeManagerStatus:(unsigned long long *)a2;
- (char)deviceAddressBelongsToResidentOfCurrentHome:(id)a0;
- (void)dismissBulletinOnAllMyTransientDevicesWithContext:(id)a0;
- (void)dismissDialogOnAllMyTransientDevicesWithContext:(id)a0 selection:(char)a1;
...
- (char)expectingInvitationResponseForIdentifier:(id)a0;
- (void)fetchAndRemoveAllZonesFromDatabase:(id)a0;
- (char)fetchAndRemoveNextBatchOfZonesFromDatabase:(id)a0;
...
- (id)filterHomes:(id)a0 isSPIEntitled:(char)a1;
...
- (id)generatePayloadFromHome:(id)a0 forAdmin:(char)a1 user:(id)a2 supportedFeatures:(id)a3;
- (char)getOrCreateControllerPublicKey:(id *)a0 controllerUsername:(id *)a1 error:(id *)a2;
...
- (char)hasClientRequestedMediaAccessoryControl:(id)a0;
- (char)hasHAPAccessoryInAnyHome;
- (char)hasManatee;
- (char)hasMediaSystemHints;
...
- (id)initWithMessageDispatcher:(id)a0 accessoryBrowser:(id)a1 messageFilterChain:(id)a2 homeData:(id)a3 localDataDecryptionFailed:(char)a4 identityRegistry:(id)a5 accountRegistry:(id)a6 metricsManager:(id)a7 configuringStateController:(id)a8 diagnosticInfoController:(id)a9 currentAccessorySetupMetricDispatcher:(id)a10 uncommittedTransactions:(id)a11 featuresDataSource:(id)a12;
- (id)initWithMessageDispatcher:(id)a0 accessoryBrowser:(id)a1 messageFilterChain:(id)a2 homeData:(id)a3 localDataDecryptionFailed:(char)a4 identityRegistry:(id)a5 accountRegistry:(id)a6 metricsManager:(id)a7 darwinNotificationProvider:(id)a8 notificationCenter:(id)a9 backingStoreFactory:(id /* block */)a10 wifiManagerFactory:(id /* block */)a11 appleAccountManager:(id)a12 remoteAccountManager:(id)a13 userDefaults:(id)a14 hmdUserDefaults:(id)a15 biomeEventManager:(id)a16 logEventSubmitter:(id)a17 widgetConfigurationReader:(id)a18 configuringStateController:(id)a19 diagnosticInfoController:(id)a20 currentAccessorySetupMetricDispatcher:(id)a21 uncommittedTransactions:(id)a22 featuresDataSource:(id)a23;
- (void)initalizeMediaGroupParticipantDataLocalStorage;
- (void)initializeHH2FrameworkSwitch;
- (char)isCurrentResidentDeviceRunningThreadNetwork:(id)a0;
- (char)isDataSyncInProgressWithMessage:(id)a0;
- (char)isFirstCloudImportComplete;
- (char)isHH1EOLForResidentDeviceRunningSoftwareVersion:(id)a0;
- (char)isInitialReplay:(id)a0;
- (char)isPairedWithWatch;
- (char)isStartThreadNetworkInProgress;
...
- (char)needHH2UpgradeRecommendation;
...
- (void)notifyClientsResidentCapable:(char)a0;
- (void)notifySPIClientsAboutAccessAllowedWhenLocked:(char)a0;
...
- (char)pendingInviteExistsForSenderOfMessage:(id)a0;
- (void)performCleanupOnHH2Container;
- (void)performInitialGraphLoad:(id)a0;
- (void)pingDevice:(id)a0 secure:(char)a1 restrictToLocalNetwork:(char)a2 completionHandler:(id /* block */)a3;
...
- (void)resetHomeConfigForHH2:(char)a0 message:(id)a1;
- (void)resetTTSUHH2SettingsMigrationKey;
- (void)runTransactionForAddHomeMessage:(id)a0 withInitialHomeObjects:(id)a1 homeManagerModel:(id)a2 homeManagerHomeModel:(id)a3 homeBackingStore:(id)a4 homeUUID:(id)a5 homeName:(id)a6 makeNewHomePrimaryHome:(char)a7;
...
- (void)saveMediaSystemHintsWithHomeUUIDString:(id)a0 mediaSystemUUIDString:(id)a1 peerAccessoryUUIDString:(id)a2 peerAccessoryRoleString:(id)a3 persistToDisk:(char)a4;
...
- (void)sendPairingIdentity:(id)a0 includePrivateKey:(char)a1 requestMessage:(id)a2;
...
- (void)sendUserRemoved:(id)a0 fromHome:(id)a1 pairingUsername:(id)a2 pushToCloud:(char)a3 completionHandler:(id /* block */)a4;
- (void)setAccessoryBrowsingRequested;
- (void)setAppDataWithMessage:(id)a0;
- (void)setDeviceAccountHasSettled:(char)a0;
- (void)setHomePodsPresent:(char)a0 inOwnedHomes:(char)a1;
- (char)setLocalPairingIdentity:(id)a0 error:(id *)a1;
- (void)setStartupHasCompleted:(char)a0;
...
- (char)shouldAcceptInvitationPayload:(id)a0 error:(id *)a1;
- (char)shouldAcceptInvitationWithHomeName:(id)a0;
- (char)shouldClearDeviceSetupFollowUp;
- (char)shouldDropNotification;
...
- (id)writeMigrationRecordWithShouldSkipKeyRollOperations:(char)a0 forceMigrationFailureForTesting:(char)a1 migrateFromTestDirectory:(char)a2 dryRun:(char)a3 isAutoMigration:(char)a4;
