@class NSUUID, NSString, NSMapTable, NSMutableDictionary, HMFMessageDispatcher, HMDRemoteEventRouterServerMessageReceiver, NSMutableArray, NSObject, HMEMessageDatagramServer, HMDEventRouterTimerProvider, NSNotificationCenter;
...
@interface HMDRemoteEventRouterServer : NSObject <HMFLogging, HMFMessageReceiver, HMEMessageDatagramServerDataSource, HMEThreadSafePendingEventsCollectionDelegate, HMEMessageDatagramServerDelegate> {
    ...
    char _isResidentEventProviding;
...
- (void)_handleFetchEventsRequest:(id)a0 originalMessage:(id)a1 connectionMode:(long long)a2;
...
- (void)configureWithHome:(id)a0 isPrimaryResident:(char)a1 isResidentEventProviding:(char)a2 additionalPolicies:(id)a3;
- (id)dumpStateDescription;
- (void)handleHomeUserRemovedNotification:(id)a0;
- (void)handlePrimaryResidentIsCurrentDeviceChangeNotification:(id)a0;
- (id)initWithPrimaryModeUUID:(id)a0 residentModeUUID:(id)a1 queue:(id)a2 messageDispatcher:(id)a3 dataSource:(id)a4 notificationCenter:(id)a5 requestMessageName:(id)a6 updateMessageName:(id)a7 multiHopFetchResponseMessageName:(id)a8 logCategory:(const char *)a9 logEventSubmitter:(id)a10 connectionServerFactory:(id /* block */)a11;
- (id)initWithPrimaryModeUUID:(id)a0 residentModeUUID:(id)a1 queue:(id)a2 messageDispatcher:(id)a3 dataSource:(id)a4 notificationCenter:(id)a5 requestMessageName:(id)a6 updateMessageName:(id)a7 multiHopFetchResponseMessageName:(id)a8 subscriptionProvider:(id)a9 registrationEventRouter:(id)a10 storeReadHandle:(id)a11;
- (char)isErrorHMENotAcceptingConnections:(id)a0;
...
- (void)pendingEventCollectionDidComplete:(id)a0;
- (void)pendingEventsCollectionDidAddItems:(id)a0;
- (void)refreshConnections:(id)a0;
- (void)respondToMessage:(id)a0 underlyingResponseData:(id)a1 connection:(id)a2 label:(id)a3;
- (void)respondToMessage:(id)a0 underlyingResponseData:(id)a1 supportsFragmentMessage:(char)a2 label:(id)a3;
...
- (char)server:(id)a0 isConnectionTerminatingError:(id)a1;
- (void)server:(id)a0 logMetricsForEventTopic:(id)a1 eventSize:(unsigned long long)a2 isCached:(char)a3;
- (void)server:(id)a0 logMetricsForMessageDatagramFragmentation:(char)a1 isFragmented:(char)a2;
- (void)server:(id)a0 sendEvents:(id)a1 cachedEvents:(id)a2 connection:(id)a3 completion:(id /* block */)a4;
- (char)server:(id)a0 shouldProcessEventsForConnection:(id)a1 shouldLogState:(char)a2;
...
- (char)serverSupportFragmentCachedEvents:(id)a0;
- (id)serverTimerProvider:(id)a0;
- (char)shouldAllowEvent:(id)a0 topic:(id)a1 connection:(id)a2;
