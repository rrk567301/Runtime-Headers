...
@protocol HMMLogEventSubmitting, HMDUserNotificationCenterSettingsProviding, HMDDoorbellBulletinUtilities, OS_dispatch_queue, HMDFileManager;
...
@property (retain) id<HMDDoorbellBulletinUtilities> doorbellBulletinUtilities;
...
+ (id)logCategory;
+ (char)isCriticalNonSecureServiceType:(id)a0;
...
+ (char)isBulletinSupportedForCharacteristicType:(id)a0 serviceType:(id)a1;
+ (char)isBulletinSupportedForMatterPath:(id)a0 accessory:(id)a1 endpointID:(id)a2;
+ (char)isBulletinSupportedForNonSecureCharacteristicType:(id)a0 serviceType:(id)a1;
+ (void)messageAndTitleForLockUserChangeEvent:(id *)a0 title:(id *)a1 accessory:(id)a2 lockDataType:(unsigned char)a3 dataOperationType:(unsigned char)a4 ecosystemName:(id)a5 isAffectedUser:(char)a6;
...
+ (char)presentationValueOfCharacteristic:(id)a0 equalTo:(id)a1;
...
- (char)_hasDuplicateBulletinForCharacteristic:(id)a0;
- (char)_hasDuplicateBulletinForSnapshotCharacteristic:(id)a0;
...
- (char)_shouldPostBulletinOnCurrentValueChangeForCharacteristic:(id)a0 includeChangeFromNil:(char)a1;
- (char)_shouldSkipBulletinForChangedCharacteristic:(id)a0;
...
- (void)_updateCharacteristicTupleFor:(id)a0 withCurrentType:(id)a1 changedByThisDevice:(char)a2;
- (char)_updateDoorbellPressNotificationsWithSignificantEventBulletin:(id)a0;
...
- (id)initWithNotificationCenter:(id)a0 fileManager:(id)a1 workQueue:(id)a2 doorbellBulletinUtilities:(id)a3 logEventSubmitter:(id)a4;
...
- (void)insertBulletinForLockUserChange:(id)a0 lockDataType:(unsigned char)a1 dataOperationType:(unsigned char)a2 ecosystemName:(id)a3 isAffectedUser:(char)a4 flow:(id)a5;
- (void)insertBulletinForMatterAttributeWithAccessory:(id)a0 attributePath:(id)a1 value:(id)a2 fields:(id)a3;
- (void)insertBulletinForMatterEventWithAccessory:(id)a0 eventPath:(id)a1 eventFields:(id)a2;
- (void)insertBulletinsForChangedCharacteristics:(id)a0 modifiedCharacteristics:(id)a1 changedByThisDevice:(char)a2 changeNotificationFromPrimary:(char)a3 completion:(id /* block */)a4;
...
- (void)insertHomeHubReachabilityBulletinForHome:(id)a0 reachable:(char)a1 hasMultipleResidents:(char)a2;
...
- (void)insertReachabilityEventBulletinForAccessory:(id)a0 reachable:(char)a1 date:(id)a2;
- (void)insertUserPreferredHomeHubReachabilityBulletinForHome:(id)a0 reachable:(char)a1 electedPrimary:(id)a2 previousPrimary:(id)a3;
- (void)insertWalletKeyExpressModeSetUpBulletinForHome:(id)a0;
- (void)insertWalletKeySupportAddedBulletinForAccessory:(id)a0;
- (id)matterEventBulletinMessageFor:(id)a0 eventPath:(id)a1 eventFields:(id)a2;
...
- (void)removeNotificationRequestsWithIdentifiers:(id)a0 shouldDeleteAttachments:(char)a1;
...
- (id)threadIdentifierForNotificationGroupWithHomeContextID:(id)a0;
