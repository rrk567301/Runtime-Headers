...
@property (nonatomic, readonly) char allowedToContributeActivityStates;
...
@property char needsiTunesMultiUserRepair;
...
@property char ownerCompletedSwitchingHomesOnboardingUI;
@property char ownerDismissed2024EnergyOnboarding;
@property char ownerDismissedAccessCodeOnboarding;
@property char ownerDismissedAccessoryFirmwareUpdateOnboarding;
@property char ownerDismissedAnnounceOnboarding;
@property char ownerDismissedCameraRecordingOnboarding;
@property char ownerDismissedCameraRecordingSetupBanner;
@property char ownerDismissedCameraUpgradeOfferBanner;
@property char ownerDismissedHomeTheaterOnboarding;
@property char ownerDismissedIdentifyVoiceOnboarding;
@property char ownerDismissedIdentifyVoiceSetupBanner;
@property char ownerDismissedNaturalLightingOnboarding;
@property char ownerDismissedTVViewingProfileOnboarding;
@property char ownerDismissedTVViewingProfileSetupBanner;
@property char ownerDismissedUserSplitMediaAccountWarning;
@property char ownerDismissedWalletKeyExpressModeOnboarding;
@property char ownerDismissedWelcomeUI;
@property char ownerHasUserSeenRMVNewLanguageNotification;
@property char allowExplicitContent;
@property char allowiTunesAccount;
@property char dolbyAtmosEnabled;
@property char losslessMusicEnabled;
@property char playbackInfluencesEnabled;
@property char siriIdentifyVoiceEnabled;
...
@property (readonly, getter=isRemoteGateway) char remoteGateway;
...
@property (readonly) char isAccessCurrentlyAllowedBySchedule;
...
+ (id)av_authorizedPeerForPairingIdentity:(id)a0 isOwner:(char)a1;
+ (id)currentUserWithPrivilege:(unsigned long long)a0 forHome:(id)a1;
+ (id)currentUserWithPrivilege:(unsigned long long)a0 forHome:(id)a1 forceHH1Key:(char)a2;
...
+ (char)isAllowedToAddOrRemoveHAPPairingsOnAccessoryFor:(id)a0;
...
+ (id)userDictionaryWithUserID:(id)a0 privilege:(unsigned long long)a1 remoteAccessAllowed:(char)a2 cameraAccessLevel:(unsigned long long)a3 announceAccessAllowed:(id)a4 announceAccessLevel:(unsigned long long)a5 restrictedGuestAccessSettings:(id)a6;
...
- (id)privateZoneName;
- (char)shouldWeOverrideRemoteAccessAllowedDespiteNoRemoteAccessAllowed;
...
- (char)_isExplicitContentValueEnabledForAppleMediaAccessory:(id)a0;
...
- (void)_sendAccountMessage:(id)a0 payload:(id)a1 deviceCapabilities:(id)a2 multicast:(char)a3 preferPrimary:(char)a4 completion:(id /* block */)a5;
- (void)_sendSecureShareClientPayloadToMostEligibleDevice:(id)a0 completion:(id /* block */)a1;
- (char)_setRemoteAccessAllowed:(char)a0;
...
- (char)areAccessoriesInAllowedList:(id)a0;
...
- (char)cloudShareTrustManager:(id)a0 shouldShareTrustWithUser:(id)a1;
...
- (char)hasDeviceWithHomeKitVersionGreaterThanVersion:(id)a0;
...
- (char)isAccessPolicySet;
- (char)isAccountEqualWithUser:(id)a0;
- (char)isAllowedToAccessActivityHistory;
- (char)isAllowedToAddOrRemoveHAPPairingsOnAccessory;
- (char)isAllowedToContributeActivityStates;
- (char)isAllowedToHaveCATId;
- (char)isAllowedToSharePhotosPersonManagerData;
- (char)isCurrentUserAndOwner;
- (char)isLocalAccessoryCommunicationAllowed;
- (char)isOwnerCapableForTrustManager:(id)a0;
- (char)isSharedUsersMeDeviceForProfileMetadata;
- (char)matchingPairingIdentityIdentifier:(id)a0;
...
- (id)setHasUserSeenRMVNewLanguageNotification:(char)a0;
...
- (char)updateAdministrator:(char)a0;
...
- (char)userDataController:(id)a0 isAccessoryUserListeningHistoryUpdateCapable:(id)a1;
- (char)userDataController:(id)a0 isAppleMediaAccessory:(id)a1;
- (char)userDataController:(id)a0 isHAPAccessory:(id)a1;
- (char)userDataController:(id)a0 isMediaContentProfileCapableAccessoryID:(id)a1;
- (char)userDataController:(id)a0 isPersonalRequestCapableAccessoryID:(id)a1;
