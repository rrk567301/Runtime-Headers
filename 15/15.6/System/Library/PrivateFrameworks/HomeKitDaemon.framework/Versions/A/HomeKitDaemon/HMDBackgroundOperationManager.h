...
- (char)_scheduleKeyRollForAccessory:(id)a0;
- (void)addDefaultDataSources;
- (char)addOperationDependency:(id)a0 dependsOn:(id)a1;
- (void)auditKeyChainEntriesAndScheduleKeyRollOperationsIfNecessaryForHome:(id)a0 isRunningOnResident:(char)a1;
- (void)auditKeyChainEntryForAccessory:(id)a0 isRunningOnResident:(char)a1;
- (void)auditOwnerPairingForAirPlayAccessory:(id)a0 isRunningOnResident:(char)a1;
...
- (char)scheduleHH2KeyRollForAccessory:(id)a0 previousIdentity:(id)a1 newIdentity:(id)a2;
- (void)scheduleHH2KeyRollForAirPlayAccessory:(id)a0;
- (char)scheduleHH2KeyRollForAirPlayAccessory:(id)a0 previousIdentity:(id)a1 newIdentity:(id)a2;
- (void)scheduleHH2KeyRollIfNecessary;
- (void)scheduleHH2KeyRollIfNecessaryDueToNotification:(id)a0;
- (void)scheduleHH2KeyRollOperationForHome:(id)a0 isRunningOnResident:(char)a1;
- (char)scheduleOperationsWithDependenciesOnEachOtherFromArray:(id)a0;
- (void)scheduleTimerToCheckAndRemoveExpiredOperation;
- (void)scheduleTimerToRetryDeferredOperations;
- (char)shouldWeScheduleKeyRollOperationsOnThisDevice;
- (void)startSchedulingKeyRollOperations:(char)a0 forAccessory:(id)a1;
