@class NSNotificationCenter, NSDate, NSDictionary, HMFMessage, HMDActionSetEvent, HMFMessageDispatcher, NSObject, NSMutableArray, HMDApplicationData, NSString, NSArray, HMDHome, HMFTimer, NSUUID;
@protocol OS_dispatch_queue;

@interface HMDActionSet : HMFObject <HMFLogging, HMFTimerDelegate, HMDHomeMessageReceiver, NSSecureCoding, HMFDumpState, HMDBackingStoreObjectProtocol, HMDApplicationDataContainer> {
...
@property (readonly, nonatomic) char containsMediaPlaybackActions;
@property (readonly, nonatomic) char containsShortcutActions;
...
- (id)_executeCommandActions:(id)a0 message:(id)a1;
- (void)_handleAddLightProfileNaturalLightingActionTransaction:(id)a0 message:(id)a1;
- (id)actionWithUUID:(id)a0;
- (void)executeWithTriggerSource:(id)a0 captureCurrentState:(char)a1 completionHandler:(id /* block */)a2;
...
- (void)_execute:(id)a0 captureCurrentState:(char)a1 writeRequestTuples:(id)a2;
...
- (void)_executeCommandActions:(id)a0 source:(unsigned long long)a1 clientName:(id)a2 completionHandler:(id /* block */)a3;
...
- (id)_executeNaturalLightingActionsForMatter:(id)a0;
...
- (void)_handleAddCommandActionTransaction:(id)a0 message:(id)a1;
...
- (char)_handleRenameActionSetTransaction:(id)a0 message:(id)a1;
...
- (char)_isValidCharacteristicWriteAction:(id)a0;
...
- (char)configure:(id)a0 messageDispatcher:(id)a1 queue:(id)a2;
- (char)containsSecureCharacteristic;
- (char)containsUnsecuringAction;
- (void)handleExecutionCompletionWithResponsePayload:(id)a0 error:(id)a1;
...
- (id)initWithName:(id)a0 uuid:(id)a1 type:(id)a2 home:(id)a3 queue:(id)a4 notificationCenter:(id)a5;
