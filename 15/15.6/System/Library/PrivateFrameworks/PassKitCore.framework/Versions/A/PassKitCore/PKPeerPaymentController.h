...
@protocol OS_dispatch_queue, OS_dispatch_group, PKPeerPaymentControllerDelegate;
...
@interface PKPeerPaymentController : NSObject <PKPaymentAuthorizationCoordinatorDelegate, PKPaymentAuthorizationCoordinatorPrivateDelegate> {
    ...
    char _quoteAuthorizationSuccess;
    NSError *_quoteAuthorizationError;
    char _performOnAuthorization;
...
+ (id)_displayNameForRecipientAddress:(id)a0 contactResolver:(id)a1 foundInContacts:(char *)a2;
+ (id)_displayableErrorOverrideForUnderlyingError:(id)a0;
+ (id)_paymentNotCompletedErrorWithError:(id)a0;
+ (id)_peerPaymentPassURL;
+ (void)_sendAuthorizedQuoteAnalyticsWithSuccess:(char)a0 mode:(unsigned long long)a1 authorizedQuote:(id)a2 status:(long long)a3;
+ (id)displayNameForAddress:(id)a0 contactResolver:(id)a1;
+ (id)displayableErrorForError:(id)a0;
+ (char)errorIsIdentityVerificationRequiredError:(id)a0;
+ (char)errorIsTermsAcceptanceRequiredError:(id)a0;
...
- (char)_contactInfoIsValidForAuthorizedQuote:(id)a0 errors:(id *)a1;
- (id)_contactResolver;
- (id)_defaultAlternateFundingSourceForMode:(unsigned long long)a0;
- (char)_ensureState:(unsigned long long)a0;
- (id)_featureDescriptorForMode:(unsigned long long)a0;
- (void)_handleAccountChanged:(id)a0;
- (char)_modeIsSendType:(unsigned long long)a0;
- (char)_modeSupportsExternalFundingSource:(unsigned long long)a0;
- (unsigned long long)_peerPaymentRequestTypeFromControllerMode:(unsigned long long)a0;
- (void)_performAuthorizedQuote:(id)a0 completion:(id /* block */)a1;
...
- (void)_setPerformQuoteSuccess:(char)a0;
- (void)_setState:(unsigned long long)a0 notify:(char)a1;
- (void)_updateLastUsedAlternateFundingSource;
- (id)analyticsSessionSubjectForPaymentAuthorizationCoordinator:(id)a0;
- (id)analyticsSessionTokenForPaymentAuthorizationCoordinator:(id)a0;
- (void)authorizeQuote:(id)a0 performOnAuthorization:(char)a1 completion:(id /* block */)a2;
- (id)displayNameForRecipientAddress:(id)a0 foundInContacts:(char *)a1;
- (id)displayableErrorForError:(id)a0;
...
- (char)restoreStateWithExternalizedControllerState:(id)a0;
