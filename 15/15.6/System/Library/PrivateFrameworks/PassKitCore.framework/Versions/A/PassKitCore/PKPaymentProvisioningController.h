@class NSHashTable, PKPaymentCredential, PKAccountService, NSMutableArray, NSString, NSMutableSet, PKPaymentSetupProductModel, PKPaymentPass, PKPaymentProvisioningControllerCredentialQueue, PKPaymentService, NSTimer, NSArray, PKAddPaymentPassRequestConfiguration, PKProvisioningUtility, PKSecureElementProvisioningState, PKPaymentCloudStoreZoneCreationManager, NSObject, NSSet, NSData, PKPaymentSetupProduct, PKPaymentOffersController, CLLocationManager, CIDVUIProofingFlowManager, CLLocation, PKPaymentWebService, PKPaymentProvisioningResponse, PKProvisioningSEStorageSnapshot, NSMutableOrderedSet, PKPaymentRequirementsResponse, PKPaymentEligibilityResponse, NSExtension;
@protocol PKCloudStoreCoordinatorDelegate, OS_dispatch_source, OS_dispatch_queue, PKPaymentDataProvider;
...
@interface PKPaymentProvisioningController : NSObject <CLLocationManagerDelegate, PKPaymentServiceDelegate, PKAccountServiceObserver, PKPaymentWebServiceDelegate> {
    ...
    char _isPreflighting;
    unsigned long long _isTargetUserUnder13;
    char _preflightedAccounts;
    char _preflightedApplications;
    NSArray *_accounts;
    char _hasPayLaterPass;
    char _queriedHasPayLaterPass;
...
@property (readonly, nonatomic) char suppressDefaultCardholderNameField;
...
@property (readonly, nonatomic) char hasExternalizedAuth;
@property (copy, nonatomic) NSString *referrerIdentifier;
@property (nonatomic) char isProvisioningForAltAccount;
...
@property (readonly, nonatomic, getter=isPasscodeUpgradeRequired) char passcodeUpgradeRequired;
...
@property (readonly, nonatomic, getter=isExpressModeSetupOptional) char expressModeSetupOptional;
@property (readonly, nonatomic) NSSet *automaticExpressModes;
@property (readonly, nonatomic, getter=isAMPEnrollmentAvailable) char AMPEnrollmentAvailable;
...
@property (nonatomic) char preventsFeatureApplications;
@property (nonatomic) char fetchProductsFromStaticDataSource;
@property (nonatomic) char allowProductsInUnsupportedRegion;
...
- (char)_credentialIsValidForSetupConfiguration:(id)a0;
...
- (char)_featureApplicationInValidStateToPresent:(id)a0;
...
- (char)_isExtensionSupported:(id)a0 usingUnsupportedProvisioningExtensions:(id)a1;
- (char)_isProvisioningPaymentAccount;
- (char)_isValidLocation:(id)a0;
...
- (void)_setState:(long long)a0 notify:(char)a1;
...
- (char)isWatchSupportedForProduct:(id)a0;
- (id)localCredentials;
- (void)noteProvisioningUserInterfaceDidAppear;
- (void)noteProvisioningUserInterfaceDidDisappear;
- (void)passcodeUpgradeCompleted:(char)a0;
...
- (void)resetForNewProvisioningForce:(char)a0;
...
- (char)setState:(long long)a0 forCredential:(id)a1;
- (void)setupAccountCredentialForProvisioning:(id)a0 completion:(id /* block */)a1;
- (void)setupProductForProvisioning:(id)a0 includePurchases:(char)a1 withCompletionHandler:(id /* block */)a2;
