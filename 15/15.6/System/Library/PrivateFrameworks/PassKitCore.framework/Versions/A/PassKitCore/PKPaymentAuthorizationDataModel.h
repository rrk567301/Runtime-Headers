@class PKPeerPaymentService, PKSelectedPaymentOffer, PKRemoteDevice, NSMutableArray, PKPaymentRequest, NSDecimalNumber, PKPaymentOrderDetails, PKContactFormatValidator, PKAccountEnhancedMerchant, PKCurrencyAmount, PKPaymentInstructions, NSMapTable, PKPayLaterFinancingController, PKPaymentPass, PKPeerPaymentQuote, CNContact, PKPaymentSummaryItem, PKPaymentOptionsDefaults, PKRemotePaymentInstrument, PKPayLaterProductAssessmentCollection, PKPassLibrary, PKPaymentOptionsRecents, NSArray, PKPaymentOffersController, PKPayLaterFinancingOption, PKBankAccountInformation, NSMutableDictionary, PKShippingMethod, PKPaymentWebService, NSString, PKPaymentApplication, NSSet, PKPeerPaymentAccount, PKPayment;
...
@property (readonly, nonatomic) char isInlineSummaryItemsEligible;
...
@property (nonatomic) char isHideMyEmailLoading;
...
@property (nonatomic) char isNestedPresentation;
...
@property (readonly, nonatomic) char pinRequired;
@property (readonly, nonatomic) char praguePolicyRequired;
...
@property (nonatomic) char supportsPreservePeerPaymentBalance;
...
@property (nonatomic) char canAddPasses;
@property (nonatomic) char supportsEmptyPass;
@property (readonly, nonatomic) char wantsInstructions;
...
@property (nonatomic) char hasAnyPayLaterOptions;
@property (nonatomic) char hasAnyPayWithRewardsOffers;
...
- (id)_additionalEligiblePaymentPassesForPaymentRequest:(id)a0;
...
- (id)_filterAndProcessPaymentApplicationsUsingConfiguration:(id)a0 withPrimaryPaymentApplication:(id)a1 passHasAssociatedPeerPaymentAccount:(char)a2;
- (id)_filterAndProcessPaymentPassesUsingConfiguration:(id)a0 additionalPaymentPasses:(id)a1;
...
- (char)canSupportDisbursementsOnRemoteDevice:(id)a0;
- (char)canSupportMerchantCategoryCodeAccordingToWebService:(id)a0 forPass:(id)a1;
- (char)canSupportMerchantCategoryCodeAccordingToWebService:(id)a0 forPaymentInstrument:(id)a1;
...
- (void)refreshPaymentMethodsAndNotifyFinancingController:(char)a0;
...
- (void)setShippingEmailError:(id)a0;
- (void)setStatus:(long long)a0 forItemWithType:(long long)a1;
- (void)setStatus:(long long)a0 forItemWithType:(long long)a1 notify:(char)a2;
- (char)shouldShowPeerPaymentBalanceToggle;
- (char)shouldUpdateContactDataItem;
- (void)showPeerPaymentCardDataItem:(char)a0 withCardDataItem:(char)a1;
...
- (void)updatePeerPaymentPromotionForPeerPaymentQuote:(char)a0;
- (void)updateRemoteDevices:(id)a0;
- (void)updateRemoteDevices:(id)a0 ignoreProximity:(char)a1;
