@class PKAccountService, PKPayLaterFinancingController, NSMutableArray, NSString, PKPaymentService, PKContinuityPaymentService, NSArray, PKPaymentAuthorizationDataModel, PKPaymentAuthorizationClientCallbackStateParam, PKPeerPaymentService, NSObject, PKPaymentSheetExperiment, PKInAppPaymentSession, PKAccountEnhancedMerchantsFetcher, PKPaymentOffersController, PKContinuityPaymentCoordinator, PKAccount, PKPaymentWebService, NSUUID, PKSecureElement, PKPeerPaymentSession, PKBluetoothMonitorer;
@protocol OS_dispatch_group, PKPaymentAuthorizationStateMachineDelegate, OS_dispatch_source, PKAggregateDictionaryProtocol;

@interface PKPaymentAuthorizationStateMachine : NSObject <PKContinuityPaymentCoordinatorDelegate, PKPayLaterFinancingControllerObserver, PKPaymentOffersControllerObserver, PKBluetoothMonitorerDelegate>
...
@property (nonatomic) char hasReceivedRemoteDeviceUpdate;
@property (nonatomic) char awaitingClientCallbackReply;
...
@property (nonatomic) char awaitingWebServiceResponse;
...
@property (readonly, nonatomic) char useSecureElement;
@property (nonatomic) char detectedBluetoothOn;
...
@property (nonatomic) char odiAssessmentInFlight;
...
@property (nonatomic) char showingExternalPasswordEntry;
...
- (char)isErrorAboutPostalAddressField:(id)a0;
...
- (void)_advanceToNextStateFromResolveError:(char)a0;
...
- (char)_canUpdateWithAutomaticReloadPaymentRequest:(id)a0 error:(id *)a1;
- (char)_canUpdateWithBillingAgreement:(id)a0 oldAgreement:(id)a1 error:(id *)a2;
- (char)_canUpdateWithDeferredPaymentRequest:(id)a0 error:(id *)a1;
- (char)_canUpdateWithMultiTokenContexts:(id)a0 error:(id *)a1;
- (char)_canUpdateWithRecurringPaymentRequest:(id)a0 error:(id *)a1;
...
- (void)_emitSignpostEvent:(unsigned long long)a0;
...
- (void)_enqueueDidAuthorizePaymentWithByPassPayment:(id)a0 rewrapResponse:(id)a1;
- (void)_enqueueDidAuthorizePaymentWithInstallmentAuthorizationToken:(id)a0 rewrapResponse:(id)a1;
- (void)_enqueueDidAuthorizePaymentWithPayment:(id)a0 rewrapResponse:(id)a1;
- (void)_enqueueDidAuthorizePaymentWithRemotePayment:(id)a0 rewrapResponse:(id)a1;
- (void)_enqueueDidAuthorizePaymentWithToken:(id)a0 rewrapResponse:(id)a1;
...
- (void)_enrichPaymentRewardsRedemptionWithPaymentTransaction:(id)a0 usingPass:(id)a1;
...
- (void)_insertPendingOrderDetails:(char *)a0;
- (void)_insertPendingPaymentTransactionsWithParam:(id)a0;
- (void)_insertPendingTransactionRegistration;
- (char)_isDeviceTransactionAssessmentEnabled;
- (char)_isMerchantTokensSupported;
- (char)_isMultiTokenContextsSupported;
- (char)_isRequestUsedForTransactionAssessment;
- (char)_isServiceProviderAssessmentEnabled;
...
- (void)_performPassRewardsUpdateForCooldownLevel:(unsigned long long)a0 ignoreErrorBackoff:(char)a1 merchantIdentifier:(id)a2;
...
- (void)_processPaymentRewardsRedemption:(id)a0;
- (void)_promptTapToRadar;
- (void)_purchaseMerchantIdentifier:(id *)a0 originURL:(id *)a1 webMerchantIdentifier:(id *)a2 webMerchantName:(id *)a3 adamIdentifier:(id *)a4 isMerchantTokenRequest:(char *)a5 isMultiTokenRequest:(char *)a6;
- (char)_purchaseSupportsFinancingOptions;
- (char)_purchaseSupportsPaymentOffers;
...
- (char)_shouldSignPurpleTrustData;
...
- (void)didBecomeActive:(char)a0;
- (void)didBecomeArmable:(char)a0;
- (void)didBecomeOccluded:(char)a0;
- (void)didChangeCouponCode:(id)a0;
- (void)didChangeFundingMode:(long long)a0;
- (void)didChangePeerPaymentUsage:(char)a0;
...
- (void)didSelectPaymentOffer:(id)a0;
- (void)didSelectPaymentOffer:(id)a0 updateReason:(unsigned long long)a1;
...
- (void)paymentOfferActionsChanged:(id)a0;
- (void)paymentOfferCatalogChanged:(id)a0;
- (void)paymentOfferDynamicContentChanged:(id)a0;
- (void)paymentOffersChanged:(id)a0 forPassUniqueIdentifier:(id)a1;
- (id)sanitizeClientErrors:(id)a0;
- (void)selectedPaymentOfferChanged:(id)a0 forPassUniqueIdentifier:(id)a1;
