@class NSData, WFWorkflowCreationOptions, WFWorkflowQuarantine, WFDebouncer, WFVariableAvailability, WFWorkflowIcon, WFInputAction, NSString, WFWorkflowReference, WFDatabase, NSArray, WFIcon, WFActionGroupingCache, WFWorkflowRecord;
...
@property (readonly, nonatomic) char hasStorageProvider;
@property (nonatomic) char ignoreModifications;
...
+ (char)checkClientVersion:(id)a0 currentVersion:(id)a1 error:(id *)a2;
+ (id)localizedSubtitleWithActionCount:(unsigned long long)a0;
+ (id)supportedInputClassNames;
+ (id)supportedInputClasses;
+ (id)workflowWithReference:(id)a0 database:(id)a1 migrateIfNecessary:(char)a2 environment:(long long)a3 error:(id *)a4;
...
- (void)setHiddenFromLibraryAndSync:(char)a0;
- (void)insertActions:(id)a0 atIndexes:(id)a1;
- (void)saveWithCompletionBlock:(id /* block */)a0;
- (char)createReferenceIfNecessaryWithName:(id)a0 nameCollisionBehavior:(unsigned long long)a1 error:(id *)a2;
...
- (id)inferOutputTypesWithHasOutputFallback:(char *)a0;
- (id)initWithActionDonation:(id)a0 error:(id *)a1;
- (id)initWithCreationOptions:(id)a0 database:(id)a1 error:(id *)a2;
- (id)initWithRecord:(id)a0 reference:(id)a1 storageProvider:(id)a2 migrateIfNecessary:(char)a3 environment:(long long)a4 error:(id *)a5;
...
- (char)queue_reloadReferenceFromStorage;
- (char)record:(id)a0 isEquivalentToRecord:(id)a1;
- (void)reloadFromRecord;
- (char)reloadReferenceFromStorage;
- (char)renameWorkflowTo:(id)a0 error:(id *)a1;
- (void)saveFromDebouncer:(id)a0;
- (char)shouldShowInputAction;
