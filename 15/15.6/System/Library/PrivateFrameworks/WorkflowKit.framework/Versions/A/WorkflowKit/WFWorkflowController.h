@class NSMutableDictionary, WFAction, NSMapTable, NSDate, WFContentCollection, WFWorkflowControllerState, NSDictionary, WFContentItemCache, WFOutOfProcessWorkflowController, NSObject, WFSmartPromptsExfiltrationLogger, WFWorkflowRunningContext, WFControlFlowAttributionTracker, NSString, WFWorkflowRunViewSource, FBSDisplayLayoutMonitor, RBSAssertion, WFWorkflow, NSProgress;
...
@property (nonatomic, getter=isStepping) char stepping;
...
@property (nonatomic) char actionDidRunRemotely;
...
@property (nonatomic) char shouldDisablePrivacyPrompts;
...
@property (nonatomic) char acquiresAssertionWhileRunning;
...
- (char)setContent:(id)a0 forVariableWithName:(id)a1;
- (id)currentRunningContextForAction:(id)a0;
- (void)action:(id)a0 didDecideRunningProgressIsAllowed:(char)a1;
...
- (void)didFinishRunningWithError:(id)a0 cancelled:(char)a1;
- (void)didRunAction:(id)a0 error:(id)a1 withCompletion:(id /* block */)a2;
- (id)environmentValueForKey:(id)a0;
- (id)errorByAddingActionIndex:(long long)a0 toError:(id)a1;
- (void)handleControlFlowActionCompletion:(id)a0 actionGroupSkipped:(char)a1;
- (void)handleError:(id)a0 fromAction:(id)a1 completionHandler:(id /* block */)a2;
- (void)invalidateAssertionIfNeeded;
- (void)logFinishActionEventWithAction:(id)a0 completed:(char)a1 error:(id)a2;
- (void)logFinishRunEvent:(char)a0;
...
- (char)runningAsStepWiseExecutor;
- (void)setEnvironmentValue:(id)a0 forKey:(id)a1;
- (void)setFinishedRunningWithSuccess:(char)a0;
- (void)setUpRunState;
- (id)stateWithActionIndex:(unsigned long long)a0 input:(id)a1 processedParameters:(id)a2;
- (void)trackRunActionEventWithKey:(id)a0 action:(id)a1 completed:(char)a2 error:(id)a3;
- (void)trackRunShortcutEventWithKey:(id)a0 completed:(char)a1;
- (id)workflowInput;
