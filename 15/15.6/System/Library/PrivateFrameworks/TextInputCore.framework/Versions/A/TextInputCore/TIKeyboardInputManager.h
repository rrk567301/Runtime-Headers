...
@interface TIKeyboardInputManager : TIKeyboardInputManagerBase <TIRevisionHistoryDelegate, TILanguageSelectionControllerDelegate> {
    ...
    char _skipStickerSuggestionGating;
...
@property (nonatomic, getter=isWordLearningEnabled) char wordLearningEnabled;
@property (nonatomic) char isEditingWordPrefix;
...
- (void)setAutocapitalizationEnabled:(char)a0;
- (void)setInputIndex:(unsigned int)a0;
- (void)setKeyboardEventsLagging:(char)a0;
...
- (id)sortingMethods;
- (char)canGenerateCompletionCandidates;
...
- (char)shouldGenerateInlineCompletions;
- (char)usesMLTapTyping;
- (int)TIInlineCompletionAcceptanceRejectionTypefromTIRegisterLearningMode:(id)a0;
- (char)_canStartSentenceAfterString:(id)a0 maxRecursionDepth:(unsigned long long)a1;
- (char)_hasSupplementalPrefix;
...
- (char)acceptsRange:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a0 inString:(id)a1;
...
- (char)allowsAutocorrectionForCurrentInputString;
...
- (id)autocorrectionCandidateForInput:(id)a0 rawInput:(id)a1 withCandidate:(const void *)a2 insertingSpace:(char)a3 sharedPrefixLength:(unsigned long long)a4;
...
- (char)autocorrectionWasRecentlyRejected:(id)a0;
- (char)autocorrectionWasRecentlyRejectedForCurrentInput;
- (BOOL)blockedByNegativeLearning:(id)a0 correction:(id)a1 ignoreSoft:(BOOL)a2;
- (id)cachedAutocorrectionListResponseFor:(id)a0;
- (char)canComputeSentenceContextForInputStem:(id)a0;
- (char)canRetrocorrectInputAtIndex:(unsigned int)a0;
- (char)canStartSentenceAfterString:(id)a0;
- (char)canTrimInputAtIndex:(unsigned int)a0;
...
- (char)closeCandidateGenerationContextWithResults:(id)a0 forced:(char)a1;
- (id)closingQuotes;
...
- (char)dictionaryContainsWord:(id)a0;
...
- (id)externalStringToInternal:(id)a0 ignoreCompositionDisabled:(char)a1;
...
- (struct TITokenID { unsigned int x0; unsigned int x1; })findTokenIDForWord:(id)a0 contextTokens:(id)a1 tokenLookupMode:(unsigned int)a2 surfaceFormPtr:(id *)a3 hasCaseInsensitiveStaticVariant:(char *)a4;
...
- (id)finishGeneratingInlineCompletions:(id)a0 completion:(id /* block */)a1 syncResponse:(char)a2;
...
- (void)generateAndRenderProactiveSuggestionsWithTriggers:(id)a0 withAdditionalPredictions:(id)a1 withInput:(id)a2 async:(char)a3 completionHandler:(id /* block */)a4;
...
- (char)generateAutofillFormWithKeyboardState:(id)a0 completionHandler:(id /* block */)a1;
...
- (void)incrementUsageTrackingKeyForAppWithIsSentence:(char)a0;
...
- (id)internalStringToExternal:(id)a0 ignoreCompositionDisabled:(char)a1;
- (char)isContinuousPathCandidate:(id)a0 replacementForOriginalConversion:(id)a1;
...
- (char)isStickerSuggestionsEnabled;
- (char)isStringBasedModel;
...
- (char)nextInputWouldStartSentenceAfterInput:(id)a0;
...
- (id)phraseCandidateCompletedByWord:(const struct String { unsigned short x0; unsigned short x1; unsigned short x2; unsigned char x3; char *x4; char x5[16]; } *)a0 allowNoSuggest:(char)a1 forAutocorrection:(char)a2 shiftContext:(int)a3;
...
- (unsigned long long)prefixLengthOfInput:(id)a0 sharedWithCandidate:(const void *)a1 useCandidateLength:(char)a2;
...
- (void)recordAcceptedAutocorrection:(id)a0 fromPredictiveInputBar:(char)a1;
- (void)recordRejectedAutocorrectionForAcceptedText:(id)a0 fromPredictiveInputBar:(char)a1;
...
- (char)removeSuffixOfInputContext:(id)a0;
...
- (void)setInputStringFromDocumentState:(id)a0 isDeleteEvent:(char)a1;
- (void)setInputStringFromDocumentState:(id)a0 isDeleteEvent:(char)a1 didDeleteLastAcceptedPath:(char)a2;
...
- (id)shortcutConversionForInput:(id)a0 andExistingString:(id)a1 existingStringStartsInMiddleOfWord:(char)a2;
- (id)shortcutConversionForString:(id)a0 stringStartsInMiddleOfWord:(char)a1;
- (struct _NSRange { unsigned long long x0; unsigned long long x1; })shortcutSearchRangeForString:(id)a0;
- (char)shouldAllowContextWord:(id)a0;
- (char)shouldAllowCorrectionOfAcceptedCandidate:(id)a0;
...
- (char)shouldInsertSpaceBeforeInput:(id)a0;
- (char)shouldInsertSpaceBeforePredictions;
- (char)shouldLearnWord:(id)a0;
- (char)shouldMonitorTypingSession;
- (char)shouldOfferStickers;
- (char)shouldOmitEmojiCandidates;
- (char)shouldPromoteAutocorrectionsToInlineCompletions:(id)a0;
- (char)shouldPromoteInlineCompletionsToAutocorrections:(id)a0 inlineCompletions:(id)a1;
- (char)shouldSkipShortcutConversionForDocumentState:(id)a0;
...
- (unsigned int)sourceMaskForWordString:(struct String { unsigned short x0; unsigned short x1; unsigned short x2; unsigned char x3; char *x4; char x5[16]; })a0 caseSensitive:(char)a1;
- (char)spaceAndNextInputWouldStartSentence;
- (id)spaceDeletingCharacters;
- (id)staticDictionaryPathForInputMode:(id)a0;
- (void)storeLanguageModelDynamicDataIncludingCache;
- (char)stringEndsWithClosingQuote:(id)a0;
...
- (char)updateAdaptationContextToKeyboardState:(id)a0 from:(id)a1;
...
- (void)willChangeToKeyboardState:(id)a0 afterContextChange:(char)a1;
