...
@property char headphonesConnected;
...
@property char isTLCNotificationSurfaced;
@property char smartChargingEngaged;
...
@property char shouldLogEABatteryConfig;
...
@property char forceSampleNextEntry;
...
+ (char)hasChargingInfoLogging;
+ (char)hasExternalAccessory;
+ (char)hasLightning;
+ (char)hasWirelessCharger;
+ (char)shouldLogBTM;
+ (char)shouldLogCPMS;
+ (char)shouldLogPPMDebugDetail;
+ (char)supportsAccConnectionLogging;
+ (char)supportsSmartCharging;
+ (char)supportsxFlags;
...
- (void)handleSBC;
- (void)handleTLCNotification:(id)a0 externalConnected:(char)a1;
- (char)hasAppleSmartBattery;
- (void)initSmartChargingLogging;
- (char)isEABatteryConnectedNow;
- (char)isEAPencilConnectedNow;
- (id)isOriginalBattery;
- (char)isUPOEntry:(id)a0;
...
- (void)logEABatteryConnectedEventToAnalyticsIgnoreLastLogTimestamp:(char)a0;
- (void)logEABatteryConnectedTimeToAnalyticsWithEAConnectionState:(char)a0;
...
- (char)modifiedExternalChargerEntry:(id)a0;
...
- (void)refreshEABatteryStatusTriggeredByConnectionEvent:(char)a0;
...
- (void)setupEALoggingTriggeredByConnectionEvent:(char)a0;
- (char)shouldWaitForLifetimeDataWithRawData:(id)a0;
- (void)showOrHideTLCNotification:(char)a0 timeInTLC:(double)a1;
