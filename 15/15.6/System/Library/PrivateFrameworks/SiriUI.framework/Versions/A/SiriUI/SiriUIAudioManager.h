...
@property char isSiriUIVisible;
@property char isAcousticIDRecording;

+ (id)sharedManager;
+ (unsigned int)currentBluetoothOutputDevice;
+ (char)isAudioDucked;
+ (char)isDoAPSupportedForBluetoothAudioDevice:(unsigned int)a0;
...
- (char)setBluetoothAudioFormat:(unsigned int)a0 forDevice:(unsigned int)a1;
