@class NSURL, INInteraction, NSDate, NSDictionary, CHHandle, NSString, NSSet, CNContact, NSDateInterval, NSArray, CHPhoneBookIOSManager, NSNumber, NSUUID;
...
@property (nonatomic) char wasAutoAnswered;
...
@property (nonatomic) char callerIdIsBlocked;
...
@property (nonatomic) char multiCall;
...
+ (id)predicateForCallsWithStatusRead:(char)a0;
+ (id)unarchivedObjectClasses;
+ (id)predicateForCallsWithGroupUUID:(id)a0;
+ (id)callCategoryAsString:(unsigned int)a0;
+ (id)callDirectoryIdentificationTypeAsString:(long long)a0;
...
+ (id)predicateForCallsContainingServiceProvider:(id)a0;
...
+ (id)predicateForCallsWithConversationID:(id)a0;
...
+ (id)predicateForCallsWithRemoteParticipantHandles:(id)a0;
+ (id)predicateForCallsWithServiceProvider:(id)a0;
+ (id)predicateForCallsWithStatus:(unsigned int)a0;
+ (id)predicateForCallsWithStatusAnswered:(char)a0;
+ (id)predicateForCallsWithStatusJunk:(char)a0;
+ (id)predicateForCallsWithStatusOriginated:(char)a0;
...
- (char)canCoalesceRemoteParticipantHandlesFromCall:(id)a0;
- (char)canCoalesceSyncWithRecentsStrategyWithCall:(id)a0;
- (char)canCoalesceWithCall:(id)a0 withStrategy:(id)a1;
- (char)coalesceWithCall:(id)a0 withStrategy:(id)a1;
...
- (char)isEqualToRecentCall:(id)a0;
