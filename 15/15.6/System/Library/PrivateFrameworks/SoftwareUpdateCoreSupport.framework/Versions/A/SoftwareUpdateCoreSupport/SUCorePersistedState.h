@class NSString, NSMutableDictionary, NSObject;
@protocol OS_dispatch_queue;

@interface SUCorePersistedState : NSObject

@property (nonatomic) char defaultLevelLogging;
@property (retain, nonatomic) NSMutableDictionary *persistedState;
@property (retain, nonatomic) NSString *persistencePath;
@property (retain, nonatomic) NSString *versionPolicyLayer;
@property (retain, nonatomic) NSString *versionSUCore;
@property (readonly, nonatomic) NSObject<OS_dispatch_queue> *persistedStateQueue;

- (id)description;
- (void).cxx_destruct;
- (id)dictionaryForKey:(id)a0;
- (id)objectForKey:(id)a0 ofClass:(Class)a1;
- (id)stringForKey:(id)a0;
- (id)dataForKey:(id)a0;
- (id)dateForKey:(id)a0;
- (id)summary;
- (id)initWithDispatchQueue:(id)a0 withPersistencePath:(id)a1 forPolicyVersion:(id)a2;
- (id)initWithDispatchQueue:(id)a0 withPersistencePath:(id)a1 forPolicyVersion:(id)a2 issuingDefaultLevelLogging:(char)a3;
- (char)loadPersistedState;
- (id)secureCodedObjectForKey:(id)a0 ofClass:(Class)a1;
- (void)persistSecureCodedObject:(id)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)persistState;
- (void)persistString:(id)a0 forKey:(id)a1;
- (void)persistString:(id)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)persistULL:(unsigned long long)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)removePersistedState;
- (unsigned long long)ullForKey:(id)a0;
- (id)_createEmptyPersistedState;
- (id)_keyNameForPersistedStateType:(int)a0;
- (void)_writePersistedState;
- (char)booleanForKey:(id)a0;
- (char)booleanForKey:(id)a0 forType:(int)a1;
- (id)dataForKey:(id)a0 forType:(int)a1;
- (id)dateForKey:(id)a0 forType:(int)a1;
- (id)dictionaryForKey:(id)a0 forType:(int)a1;
- (char)isPersistedStateLoaded;
- (id)objectForKey:(id)a0 ofClass:(Class)a1 forType:(int)a2;
- (void)persistBoolean:(char)a0 forKey:(id)a1;
- (void)persistBoolean:(char)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (void)persistBoolean:(char)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)persistData:(id)a0 forKey:(id)a1;
- (void)persistData:(id)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (void)persistData:(id)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)persistDate:(id)a0 forKey:(id)a1;
- (void)persistDate:(id)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (void)persistDate:(id)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)persistDictionary:(id)a0 forKey:(id)a1;
- (void)persistDictionary:(id)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (void)persistDictionary:(id)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)persistObject:(id)a0 forKey:(id)a1;
- (void)persistObject:(id)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (void)persistObject:(id)a0 forKey:(id)a1 shouldPersist:(char)a2;
- (void)persistSecureCodedObject:(id)a0 forKey:(id)a1;
- (void)persistSecureCodedObject:(id)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (void)persistString:(id)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (void)persistULL:(unsigned long long)a0 forKey:(id)a1;
- (void)persistULL:(unsigned long long)a0 forKey:(id)a1 forType:(int)a2 shouldPersist:(char)a3;
- (id)persistedContentsType;
- (id)persistedCoreVersion;
- (id)persistedPolicyVersion;
- (id)secureCodedObjectForKey:(id)a0 ofClass:(Class)a1 encodeClasses:(id)a2;
- (id)secureCodedObjectForKey:(id)a0 ofClass:(Class)a1 encodeClasses:(id)a2 forType:(int)a3;
- (id)secureCodedObjectForKey:(id)a0 ofClass:(Class)a1 forType:(int)a2;
- (id)stringForKey:(id)a0 forType:(int)a1;
- (unsigned long long)ullForKey:(id)a0 forType:(int)a1;

@end
