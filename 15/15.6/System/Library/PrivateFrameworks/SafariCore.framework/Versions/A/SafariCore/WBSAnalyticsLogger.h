...
@interface WBSAnalyticsLogger : NSObject {
    ...
    char _safariReportedNonOtherBlockingReason;
    long long _safariQuitReason;
}

@property (readonly, nonatomic) char safariIsTerminating;
...
- (void)didSubmitFormOfType:(unsigned long long)a0 withFieldType:(long long)a1 elementType:(unsigned long long)a2 isAutoFilled:(char)a3 isManuallyFilledByUser:(char)a4 modificationType:(unsigned long long)a5 autoFillOfferedType:(unsigned long long)a6 webpageLocale:(id)a7;
...
- (void)didDetermineReaderAvailability:(char)a0;
...
- (void)didActivateLinkWithZoomScale:(float)a0 recentlyZoomedIn:(char)a1;
- (void)didActivateReaderWithTrigger:(long long)a0;
- (void)didActivateVoiceSearchAccidentally:(char)a0;
...
- (void)didAddWebAppToDock:(char)a0 hasManifest:(char)a1 withMethod:(long long)a2 withLength:(unsigned long long)a3 withIconKind:(long long)a4 withDisplayMode:(id)a5 withCustomization:(long long)a6;
- (void)didAddWebClip;
- (void)didBanWebsiteWithMetadata:(id)a0;
- (void)didCancelScribble;
- (void)didChangeStartPageSectionVisibility:(id)a0 visible:(char)a1;
...
- (void)didChooseTopHit:(char)a0 matchLength:(unsigned long long)a1 matchScore:(float)a2;
...
- (void)didCreateTabGroupFromLocation:(long long)a0 prepopulatedWithTabs:(char)a1;
- (void)didDeleteBookmarkInMemory;
- (void)didDeleteNumberOfBookmarksInMemory:(unsigned long long)a0;
- (void)didDetectLanguageOfWebpage:(id)a0 primaryLocale:(id)a1 webpageCanBeTranslatedToPrimaryLocale:(char)a2 canOfferTranslation:(char)a3 firstNonPrimaryLocaleThatCanBeTranslatedTo:(id)a4 numberOfPreferredLocales:(unsigned long long)a5 notificationLevel:(long long)a6;
- (void)didDetectPossibleUserTrackingInHostApp:(id)a0 urlHasIDFA:(char)a1 urlHasQueryString:(char)a2 userInteracted:(char)a3 viewControllerIsHidden:(char)a4;
- (void)didDismissCompletionListWithItemAccepted:(char)a0 goKeyTapped:(char)a1 unifiedFieldContentType:(long long)a2;
...
- (void)didManuallyMarkReadingListItemAsRead:(char)a0;
...
- (void)didOpenNewTabWithURLWithTrigger:(long long)a0 isExternalLink:(char)a1 tabCollectionViewType:(long long)a2;
...
- (void)didOpenReadingListItemWithReachableNetwork:(char)a0 isContinuousTransition:(char)a1;
...
- (void)didRequestPageShowingSideBar:(char)a0;
- (void)didRequestSearchSuggestions;
- (void)didRequestTranslatingToLocale:(id)a0 webpageLocale:(id)a1 requestType:(long long)a2 isTargetLocalePrimaryLocale:(char)a3;
...
- (void)didSetDownloadFolderToProviderWithIdentifier:(id)a0 isDefaultFolder:(char)a1;
- (void)didSetPrivateBrowsingEnabled:(char)a0;
- (void)didSetShowNavigationControlsInWebApp:(char)a0;
- (void)didSetShowTitleBarColorInWebApp:(char)a0;
- (void)didShowEntityCardSBA;
- (void)didShowPasswordGenerationPromptWithPasswordAccepted:(char)a0;
...
- (void)didSuggestUsernameInPromptAndUserKeptSuggestion:(char)a0;
...
- (void)didToggleShowMoreButtonForSection:(long long)a0 isShowingMore:(char)a1;
...
- (void)didVisitWebpageSBAWithUserOptedIn:(char)a0;
...
- (void)reportCustomizationSyncEnablement:(char)a0;
- (void)reportExperimentalFeaturesOnLaunchForWebXREnabled:(char)a0 webXRGamepadsModuleEnabled:(char)a1 webXRHandInputModuleEnabled:(char)a2 modelElementEnabled:(char)a3;
...
- (void)reportOverlayStatusBarIsEnabled:(char)a0;
...
- (void)reportSafariRecoveredWindowStateWithSuccess:(char)a0;
- (void)reportSafariTabStateMismatchWithError:(long long)a0 systemTALEnabled:(char)a1 safariRestoreOnLaunchEnabled:(char)a2 excludePrivateWindows:(char)a3 systemSaysPreserveState:(char)a4 stringRepresentation:(id)a5;
- (void)reportSafariVersion:(id)a0 safariAndWebKitVersion:(id)a1;
- (void)reportShowFullURLInSmartSearchFieldPreference:(char)a0;
- (void)reportShowStandaloneTabBarPreference:(char)a0;
- (void)reportStartPageBackgroundImageVisible:(char)a0;
- (void)reportStartPageBackgroundName:(id)a0;
- (void)reportStartPageOverrideStatistics:(id)a0;
- (void)reportStartPageSectionVisibility:(long long)a0 visible:(char)a1;
...
- (void)reportUnifiedTabBarStatusWithLayout:(long long)a0 numberOfTabsInCurrentTabBar:(unsigned long long)a1 tabBarIsCurrentlyScrollable:(char)a2 tabsShowOnlyIcons:(char)a3 backgroundColorInTabBarEnabled:(char)a4;
- (void)reportUserConsentStateSBA:(id)a0;
- (void)reportUserPreferencesOnLaunchForJavaScriptEnabled:(char)a0 safeBrowsingEnabled:(char)a1 siteSpecificSearchEnabled:(char)a2 trackingPolicy:(unsigned long long)a3;
