...
@interface WBSSavedAccountStore : NSObject <WBSSavedAccountStorePasskeyExtras, WBSSavedAccountStoreWebsiteNameConsumer, WBSOngoingSharingGroupProviderSubscriber, WBSAuthenticationServicesAgentDelegate> {
    ...
    char _shouldShowServiceNamesForPasswordAndPasskeyItems;
    char _didInitializeShouldShowServiceNamesSetting;
}

@property (class, readonly, nonatomic) WBSSavedAccountStore *sharedStore;
@property (class, nonatomic) char shouldShowServiceNamesForPasswordAndPasskeyItems;
...
@property (readonly, nonatomic) char isForTesting;
...
@property (nonatomic) char shouldWarmDataStoreAfterReset;
...
@property (readonly, nonatomic) char hasPasswordsEligibleForAutoFill;
@property (readonly, nonatomic) char hasPasskeysEligibleForAutoFill;
...
@property (nonatomic) char shouldShowServiceNamesForPasswordAndPasskeyItems;
...
+ (char)getProtectionSpaceAndHighLevelDomainForUserTypedSite:(id)a0 protectionSpace:(id *)a1 highLevelDomain:(id *)a2;
+ (void)removeCredentialTypes:(long long)a0 forSavedAccount:(id)a1 forGroup:(id)a2;
+ (char)userTypedTitleShouldBeTreatedAsAWebsiteWhenAddingNewAccount:(id)a0;
...
- (id)_allPersonalSidecarsForGroupID:(id)a0 fromRecentlyDeleted:(char)a1;
- (id)_allPersonalSidecarsFromPersonalKeychainFromRecentlyDeleted:(char)a0;
- (id)_allSharedSidecarsForGroupID:(id)a0 fromRecentlyDeleted:(char)a1;
- (id)_allSidecarsFromPersonalKeychain;
- (char)_applyServiceNamesToSavedAccounts:(id)a0;
- (char)_canChangeSavedAccount:(id)a0 toUser:(id)a1 password:(id)a2 forGroupID:(id)a3;
- (char)_canChangeSavedAccountInPersonalKeychain:(id)a0 toUser:(id)a1 password:(id)a2;
- (char)_canMoveSavedAccount:(id)a0 toGroupWithID:(id)a1;
- (char)_canMoveSavedAccountWithPasskey:(id)a0 toGroup:(id)a1;
- (char)_canPerformMaintenanceTasks;
- (void)_cleanUpRedundantCredentialsWithoutUsernames;
- (void)_cleanUpSharedSavedAccountsWithUnknownOriginalContributorParticipantIDsIfNecessaryFromRecentlyDeleted:(char)a0;
- (void)_clearExpiredRecentlyDeletedNotificationDates;
- (id)_convertSignInWithAppleAccountsToSavedAccounts:(id)a0;
- (char)_copySavedAccount:(id)a0 toGroup:(id)a1;
- (char)_deleteSavedAccountsWithUnknownOriginalContributorParticipantIDsFromSavedAccounts:(id)a0 inGroup:(id)a1;
...
- (char)_hasPersonWithParticipantID:(id)a0 contributedSavedAccountsToGroupWithID:(id)a1;
- (char)_hasSavedAccountWithSameUserNameOfSavedAccount:(id)a0 forProtectionSpace:(id)a1;
- (void)_identifySharedSavedAccountsWithSameUsernameAndHighLevelDomainPairs;
- (char)_isMigratingKeychainItemsWithInvalidAuthenticationTypes;
...
- (id)_loadSavedAccountsWithPasskeysFromPasskeyData:(id)a0 forGroupID:(id)a1 fromRecentlyDeleted:(char)a2 withDictionaryForSavedAccountsWithPasskeys:(id)a3;
- (id)_loadSavedAccountsWithPasswordsFromKeychainData:(id)a0 forGroupID:(id)a1 withDictionaryForSavedAccountsWithPasswords:(id)a2 fromRecentlyDeleted:(char)a3;
...
- (char)_moveSavedAccountsOriginallyContributedByCurrentUserToPersonalKeychainFromGroupID:(id)a0 isForAlreadyExitedGroup:(char)a1;
- (char)_moveSavedAccountsOriginallyContributedByCurrentUserToPersonalKeychainFromGroupIDOnInternalQueue:(id)a0 isForAlreadyExitedGroup:(char)a1;
- (char)_moveSignInWithAppleSavedAccount:(id)a0 toGroupID:(id)a1;
...
- (void)_prewarm;
- (void)_pruneSavedAccountTreeMatchesOnInternalQueue:(id)a0 basedOnUsernameAndPasswordCriteria:(id)a1 autoFillPasskeyIdentifiers:(id)a2 keepAllPasskeyMatches:(char)a3;
- (char)_reapplyServiceNamesFromProvider;
...
- (char)_shareSignInWithAppleAccountWithContext:(id)a0 withGroupInfo:(id)a1 savedAccount:(id)a2;
- (char)_shouldLoadCredentialRecordWithHostIntoAccountStore:(id)a0;
- (char)_shouldUpdateLastUsedDateForSavedAccount:(id)a0 forProtectionSpace:(id)a1 inContext:(id)a2;
- (void)_signInWithApplePushNotificationReceived:(id)a0;
- (char)_unshareSignInWithAppleAccountWithContext:(id)a0;
...
- (char)canChangeSavedAccount:(id)a0 toUser:(id)a1 password:(id)a2;
- (char)canChangeSavedAccountWithRequest:(id)a0;
- (char)canSaveUser:(id)a0 password:(id)a1 forProtectionSpace:(id)a2 highLevelDomain:(id)a3 notes:(id)a4 customTitle:(id)a5 groupID:(id)a6 error:(id *)a7;
- (char)canSaveUser:(id)a0 password:(id)a1 forUserTypedSite:(id)a2 notes:(id)a3 customTitle:(id)a4 groupID:(id)a5 error:(id *)a6;
...
- (id)initAndPrewarmSavedAccounts:(char)a0 forTesting:(char)a1;
- (id)initPartialStoreForDomains:(id)a0 forTesting:(char)a1;
- (id)initWithAuthenticationServicesAgentProxy:(id)a0 shouldPrewarmSavedAccounts:(char)a1 limitedToDomains:(id)a2 forTesting:(char)a3;
...
- (void)test_copyPasskeyWithCredentialID:(id)a0 toGroup:(id)a1 inRecentlyDeleted:(char)a2 completionHandler:(id /* block */)a3;
...
- (void)test_createPasskeyWithUserName:(id)a0 displayName:(id)a1 relyingPartyIdentifier:(id)a2 userHandle:(id)a3 groupID:(id)a4 inRecentlyDeleted:(char)a5 completionHandler:(id /* block */)a6;
- (void)test_createPasskeyWithUserName:(id)a0 displayName:(id)a1 relyingPartyIdentifier:(id)a2 userHandle:(id)a3 groupID:(id)a4 inRecentlyDeleted:(char)a5 unsafeAllowRealKeychainAccess:(char)a6 completionHandler:(id /* block */)a7;
- (void)test_reloadAllSavedAccounts;
- (void)test_setSharedAccountsGroups:(id)a0;
- (char)unshareSignInWithAppleSavedAccount:(id)a0;
