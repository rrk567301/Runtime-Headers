...
+ (id)supportedUTIs;
+ (char)_canConvertPNGToJPEG:(struct CGImage { } *)a0;
+ (char)_imageContainsTranslucentPixels:(struct CGImage { } *)a0;
...
- (void)_imMetricsCollectorForLQMQualityEstimatorModeImageTypeHEIC:(unsigned long long)a0 shouldUseQualityEstimatorModel:(char)a1 inputImageFeatures:(id)a2 metricsDict:(id)a3;
- (id)_imMetricsCollectorForLQMQualityEstimatorModel:(struct __CFString { } *)a0 suggestedMaxLength:(unsigned long long)a1 shouldUseQualityEstimatorModel:(char)a2 inputImageFeatures:(id)a3;
- (char)_isHEIFImageFormat:(struct __CFString { } *)a0;
- (char)_isWebPImageFormat:(struct __CFString { } *)a0;
- (char)_isWideGamutImage:(struct CGImageSource { } *)a0;
- (id)_newGeneratedImage:(struct CGImageSource { } *)a0 sourceWidth:(id)a1 sourceHeight:(id)a2 sourceLength:(unsigned long long)a3 sourceProps:(id)a4 target:(long long)a5 hardwareEncoder:(id)a6 inFormat:(struct __CFString { } *)a7 fromFormat:(struct __CFString { } *)a8 withMaxLength:(unsigned long long)a9 withMaxCount:(unsigned long long)a10 withCompressionQuality:(double)a11 enforceMaxes:(char)a12 subsampling:(int)a13 shouldSkipAuxillaryData:(char)a14;
...
- (id)_writeHEIFImage:(struct CGImageSource { } *)a0 sourceURL:(id)a1 inFormat:(struct __CFString { } *)a2 withMaxByteSize:(unsigned long long)a3 maxDimension:(unsigned long long)a4 downgradingMultiFrameImageToSingleFrame:(char)a5 actualSize:(unsigned long long *)a6 startingLengthIndex:(int)a7 usedLengthIndex:(int *)a8 telemetry:(id)a9;
- (id)_writeImage:(struct CGImageSource { } *)a0 sourceURL:(id)a1 target:(long long)a2 hardwareEncoder:(id)a3 inFormat:(struct __CFString { } *)a4 fromFormat:(struct __CFString { } *)a5 withMaxByteSize:(unsigned long long)a6 maxDimension:(unsigned long long)a7 actualSize:(unsigned long long *)a8 startingLengthIndex:(int)a9 usedLengthIndex:(int *)a10 estimator:(id)a11 isLQMEnabled:(char)a12 telemetry:(id)a13;
- (id)_writeImageData:(id)a0 inFormat:(struct __CFString { } *)a1 sourceURL:(id)a2;
- (id)_writeRepresentationsForImage:(struct CGImageSource { } *)a0 target:(long long)a1 sourceURL:(id)a2 sizes:(id)a3 maxDimension:(unsigned long long)a4 srcUTI:(struct __CFString { } *)a5 inFormat:(struct __CFString { } *)a6 downgradingMultiFrameImageToSingleFrame:(char)a7 estimator:(id)a8 isLQMEnabled:(char)a9 telemetry:(id)a10;
