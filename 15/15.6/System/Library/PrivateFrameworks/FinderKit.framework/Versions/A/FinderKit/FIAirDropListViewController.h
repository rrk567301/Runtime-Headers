...
@interface FIAirDropListViewController : FI_TTableViewController <TMarkTornDown> {
    struct function<void (const TFENode &)> { struct __value_func<void (const TFENode &)> { struct type { unsigned char __lx[32]; } __buf_; void *__f_; } __f_; } _clickHandler;
...
- (struct function<void (const TFENode &)> { struct __value_func<void (const TFENode &)> { struct type { unsigned char x0[32]; } x0; void *x1; } x0; })clickHandler;
