...
@interface FI_TColumnViewController : FI_TBrowserViewController <TableView_Common_Delegate, FIAliasResolution, TColumnViewDelegate, TDuplicateTaskDelegateProtocol, TListNameFieldDelegate> {
    ...
    struct TNotificationCenterObserver { NSObject *fObservedObject; struct TString { struct TRef<const __CFString *, TRetainReleasePolicy<CFStringRef>> { struct __CFString *fRef; } fString; } fNotificationName; FI_TNotificationCenterObserverGlue *fFunctorGlue; BOOL fIsDistributedObserving; struct function<NSNotificationCenter *()> { struct __value_func<NSNotificationCenter *()> { struct type { unsigned char __lx[32]; } __buf_; void *__f_; } __f_; } fCenterProvider; } _userDidResizeColumnsObserver;
...
- (char)browser:(id)a0 isLeafItem:(id)a1;
...
- (char)browser:(id)a0 shouldAutoExpandItem:(id)a1;
- (char)browser:(id)a0 shouldEditItem:(id)a1;
- (double)browser:(id)a0 shouldSizeColumn:(long long)a1 forUserResize:(char)a2 toWidth:(double)a3;
- (char)browser:(id)a0 shouldTypeSelectForEvent:(id)a1 withCurrentSearchString:(id)a2;
