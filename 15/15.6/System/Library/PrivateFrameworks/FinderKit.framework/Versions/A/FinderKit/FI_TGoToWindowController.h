...
@interface FI_TGoToWindowController : FI_TScriptableWindowController <NSTouchBarDelegate, TTouchBarGoToViewDelegate, NSWindowDelegate, NSTextFieldDelegate, NSMenuDelegate> {
    ...
    struct function<void (FI_TGoToWindowController *)> { struct __value_func<void (FI_TGoToWindowController *)> { struct type { unsigned char __lx[32]; } __buf_; void *__f_; } __f_; } _initialItemsDidLoadHandler;
    struct function<void (FI_TGoToWindowController *, const TFENode &, const TFENode &, const TString &)> { struct __value_func<void (FI_TGoToWindowController *, const TFENode &, const TFENode &, const TString &)> { struct type { unsigned char __lx[32]; } __buf_; void *__f_; } __f_; } _completionHandler;
