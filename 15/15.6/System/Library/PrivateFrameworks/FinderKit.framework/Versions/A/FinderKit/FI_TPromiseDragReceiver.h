...
@interface FI_TPromiseDragReceiver : NSObject <TCoalescingNodeObserverProtocol> {
    struct function<void (const TFENode &)> { struct __value_func<void (const TFENode &)> { struct type { unsigned char __lx[32]; } __buf_; void *__f_; } __f_; } _promiseDragNodeAddedCallback;
    struct function<void (const TFENodeVector &, const TError &)> { struct __value_func<void (const TFENodeVector &, const TError &)> { struct type { unsigned char __lx[32]; } __buf_; void *__f_; } __f_; } _promiseDragCompletionCallback;
