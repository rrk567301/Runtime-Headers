...
@interface FBWorkspaceScene : NSObject <FBSSceneHandle, FBSSceneAgentProxy, BSDescriptionProviding> {
    ...
    char _lock_sentSceneCreate;
    char _reconnect;
    char _lock_invalidated;
    FBSSerialQueue *_workspaceQueue;
}

@property (readonly, nonatomic) char usesOutgoingConnection;
...
- (void)_workspaceQueue_decrementInFlightUpdatesForAction:(unsigned char)a0 allowThrottling:(char)a1 externallyManaged:(char)a2;
- (void)_workspaceQueue_incrementInFlightUpdatesForAction:(unsigned char)a0 allowThrottling:(char)a1 externallyManaged:(char)a2;
...
- (id)initWithConnection:(id)a0 host:(id)a1 settings:(id)a2 clientSettings:(id)a3 fromRemnant:(char)a4;
