...
@interface FBProcess : NSObject <FBProcessBootstrapping, FBSProcessInternal, RBSProcessMatching, FBSProcess> {
    ...
    char _lock_pendingExit;
    char _lock_invalidated;
    char _lock_attemptedBootstrap;
    char _lock_waitForDebugger;
    char _lock_updatingState;
    char _lock_didExit;
    char _lock_launchFinalized;
    char _lock_launchSuccess;
    char _lock_attemptedGracefulTermination;
    long long _lock_executableLivesOnSystemPartition;
    long long _lock_platformBinary;
    char _calloutQueue_sentWillExit;
    char _calloutQueue_sentDidExit;
    char _calloutQueue_sentAppWillLaunch;
    char _calloutQueue_sentAppDidLaunch;
}

@property (class, readonly, retain, nonatomic) NSObject<OS_dispatch_queue> *calloutQueue;

@property (readonly, nonatomic) char executableLivesOnSystemPartition;
...
@property (readonly, nonatomic, getter=isBeingDebugged) char beingDebugged;
@property (readonly, nonatomic, getter=isCurrentProcess) char currentProcess;
@property (readonly, nonatomic, getter=isFinishedLaunching) char finishedLaunching;
@property (readonly, nonatomic, getter=isPendingExit) char pendingExit;
...
- (void)_killForReason:(long long)a0 andReport:(char)a1 withDescription:(id)a2 completion:(id /* block */)a3;
- (void)_launchDidComplete:(char)a0 finalizeBlock:(id /* block */)a1;
...
- (char)_shouldWatchdogWithDeclineReason:(id *)a0;
- (char)_startWatchdogTimerForContext:(id)a0;
...
- (char)bootstrapWithDelegate:(id)a0;
