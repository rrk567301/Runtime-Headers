@class NSError, NSString, DMCEnrollmentFlowController, NSMutableDictionary, NSDictionary, CPProfile, NSObject, NSWindow, CPUI_ProgressWindowController;
@protocol CPUI_ProgressHandler;

@interface CPUI_EnrollmentController : NSObject <DMCEnrollmentFlowPresenter, DMCEnrollmentFlowMCBridge>

@property unsigned long long lastEnrollmentType;
@property unsigned long long completedPhases;
@property (retain) DMCEnrollmentFlowController *enrollmentFlowController;
@property long long enrollmentFlow;
@property (retain) NSWindow *callerHostWindow;
@property BOOL honorSilentReAuth;
@property BOOL suppressUI;
@property (readonly) NSWindow *hostWindow;
@property (retain) NSObject<CPUI_ProgressHandler> *hostProgress;
@property (retain) CPUI_ProgressWindowController *progressController;
@property (retain) NSString *progressLogPrefix;
@property (retain) NSString *progressTitle;
@property (retain) NSString *managedAppleID;
@property (retain) NSMutableDictionary *appleAccountInfo;
@property (retain) CPProfile *enrollmentProfile;
@property (retain) NSDictionary *akAuthResults;
@property (retain) NSDictionary *orgInfo;
@property (retain) NSDictionary *cloudConfig;
@property (retain) NSError *enrollmentFailure;
@property (retain) NSDictionary *extAuth;
@property (retain) NSString *installedMDMProfileID;
@property (retain) NSDictionary *installedMDMProfileReference;
@property (retain) NSDictionary *setupInfoForMacBuddy;
@property BOOL forceEphemeralMAIDAuth;
@property (retain) NSString *DEPSource;
@property BOOL isDEPUpdate;
@property (retain) NSString *serviceURL;
@property BOOL isDoingOrgoPhase2;
@property (readonly) BOOL authAsAdmin;
@property (readonly) NSString *enrollmentTypeDesc;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (void)dealloc;
- (id)init;
- (void).cxx_destruct;
- (void)buildAuthKitPromptTitle:(id *)a0 reason:(id *)a1;
- (void)setAppleAccountInfoKey:(id)a0 toValue:(id)a1;
- (id)assignedManagedAppleIDFromMDMProfile:(id)a0 error:(id *)a1;
- (id)bestOrgName;
- (BOOL)debug_SimulateFailMAIDSignInWithError:(id *)a0;
- (BOOL)debug_SimulatingDeferUserAccountCreationTillLogin;
- (BOOL)deferUserAccountCreationTillLogin;
- (id)detailsFromMDMProfile:(id)a0 error:(id *)a1;
- (BOOL)expectICloudInPrimarySlot;
- (BOOL)expectITunesActive;
- (unsigned long long)expectedServerType;
- (id)friendlyNameForMDMPayloadInProfile:(id)a0 error:(id *)a1;
- (id)getDeferAccountCreationInfoForBearerToken:(id)a0;
- (id)getMachineInfoForEnrollmentType:(unsigned long long)a0 enrollmentMethod:(unsigned long long)a1;
- (void)handleEnrollmentCompletionSuccess:(BOOL)a0 cancelled:(BOOL)a1 error:(id)a2 completion:(id /* block */)a3;
- (BOOL)inMacBuddy;
- (void)installEnrollmentProfile:(id)a0 devicePasscode:(id)a1 personaID:(id)a2 rmAccountIdentifier:(id)a3 managedProfileIdentifiers:(id)a4 installationSource:(id)a5 completionHandler:(id /* block */)a6;
- (BOOL)isADDEFlow;
- (char)isDevicePasscodeSet;
- (BOOL)mdmCanRequireSUVia403;
- (id)organizationFromMDMProfile:(id)a0 error:(id *)a1;
- (id)profileIdentifierFromMDMProfile:(id)a0 error:(id *)a1;
- (BOOL)promptForMDMInstallAuthRightDuringMAIDSignIn;
- (id)readEnrollmentDirtyState;
- (void)removeProfileWithIdentifier:(id)a0 async:(char)a1;
- (void)removeProgress;
- (void)requestDevicePasscodeWithCompletionHandler:(id /* block */)a0;
- (void)requestMAIDAuthenticationWithManagedAppleID:(id)a0 personaID:(id)a1 ephemeral:(char)a2 requireAppleMAID:(char)a3 completionHandler:(id /* block */)a4;
- (void)requestMAIDSignInWithAuthenticationResults:(id)a0 personaID:(id)a1 makeiTunesAccountActive:(char)a2 completionHandler:(id /* block */)a3;
- (void)requestSilentMAIDAuthenticationWithAuthenticationResults:(id)a0 personaID:(id)a1 requireAppleMAID:(char)a2 completionHandler:(id /* block */)a3;
- (void)requestUsernameWithCompletionHandler:(id /* block */)a0;
- (void)requestWebAuthenticationWithWebAuthURL:(id)a0 authenticator:(id)a1 authParams:(id)a2 completionHandler:(id /* block */)a3;
- (void)setProgress:(id)a0 info:(id)a1;
- (BOOL)shouldDeferRequestMAIDSignIn:(id)a0;
- (void)showEnrollmentCompletionScene;
- (void)showEnrollmentFailure:(id)a0;
- (void)tagAndSaveAccountAsManagedByProfileID:(id)a0;
- (BOOL)usesDataSeparation;

@end
