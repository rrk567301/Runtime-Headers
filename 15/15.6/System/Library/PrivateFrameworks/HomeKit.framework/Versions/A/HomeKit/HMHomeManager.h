...
@interface HMHomeManager : NSObject <HMCoreAnalyticsMetricEventDispatcherFactory, HMAccessorySettingsDataSourceFactory, HMAccessorySettingsMetricsDispatcherFactory, HMAccessorySettingsMessengerFactory, HMAccessorySettingsDataSourceDataSource, HMAccessoryInfoDataProviderFactory, HMAccessoryInfoDataProviderDataSource, HMAccessorySettingsControllerDataSource, HMAccessorySettingsControllerFactory, HMFTimerDelegate, HMUserActionPredictionProviderDataSource, HMUserCloudShareManagerDataSource, HMFLogging, HMFMessageReceiver, HMMutableApplicationData, HMApplicationData> {
    ...
    char _assistantIdentifierChanged;
    HMELastEventStore *_lastEventStore;
    char _eventStoreStarted;
}
...
@property (nonatomic, getter=isInitialMergeComplete) char initialMergeComplete;
@property (nonatomic) char shouldConnect;
...
@property (nonatomic, getter=isViewServiceActive) char viewServiceActive;
...
@property (nonatomic, getter=isDaemonRunningWithROARFramework) char daemonRunningWithROARFramework;
...
@property (readonly, nonatomic, getter=isDataSyncInProgress) char dataSyncInProgress;
@property (readonly, nonatomic) unsigned long long dataSyncState;
@property (readonly) unsigned long long status;
@property (readonly, getter=isThisDeviceResidentCapable) char thisDeviceResidentCapable;
@property (readonly, getter=isResidentEnabledForThisDevice) char residentEnabledForThisDevice;
@property (readonly, getter=isAccessAllowedWhenLocked) char accessAllowedWhenLocked;
...
@property (readonly, nonatomic, getter=hasOptedToHH2) char optedToHH2;
@property (readonly, nonatomic, getter=isHH2MigrationAvailable) char hh2MigrationAvailable;
@property (readonly, getter=isHH2MigrationInProgress) char hh2MigrationInProgress;
@property (readonly) NSError *hh2MigrationFailedError;
@property (readonly) char homeSafetySecurityEnabled;
@property (readonly) HMWidgetManager *widgetManager;
@property (readonly) long long multiUserStatus;
@property (readonly, nonatomic) char shouldPostHH2UpgradeRequired;
...
+ (id)sharedEventStore;
+ (id)instanceTracker;
+ (char)dataSyncInProgressFromDataSyncState:(unsigned long long)a0;
...
- (void)_writeCaches:(id)a0 homeData:(char)a1 metadata:(char)a2;
- (void)__start;
...
- (void)__preconfigure;
- (void)__processSyncResponse:(id)a0 refreshRequested:(char)a1 completionHandler:(id /* block */)a2;
...
- (void)_addKeysToRepairListIfUnique:(id)a0 includePrivateKeys:(char)a1 repairList:(id)a2;
...
- (void)_fetchHomeConfigurationWithRefreshRequested:(char)a0;
...
- (void)_handleHH2UpgradeRecommendationRequired:(id)a0;
...
- (char)_isValidCachedHomeConfiguration:(id)a0;
- (id)_loadHH2MigrationInfoFromHMCache:(id)a0;
- (void)_mergeCurrentHomesWithNewHomes:(id)a0 newPrimaryHome:(id)a1 newCurrentHome:(id)a2 newInvitations:(id)a3 newAppData:(id)a4 refreshRequested:(char)a5;
- (void)_mergeHomeInvitations:(id)a0;
- (void)_mergeNewAppData:(id)a0;
- (void)_notifyAccessAllowedWhenLockedUpdated:(char)a0;
- (void)_notifyDelegateOfAppDataUpdate;
- (void)_pingDeviceWithUUID:(id)a0 monitor:(char)a1 secure:(char)a2 restrictToLocalNetwork:(char)a3 completionHandler:(id /* block */)a4;
- (void)_processHomeAddedPayload:(id)a0 completionHandler:(id /* block */)a1;
- (void)_processHomeConfigurationResponse:(id)a0 refreshRequested:(char)a1;
...
- (void)_saveMigrationProgressToHMCache:(char)a0 error:(id)a1;
- (void)_setInitialHomes:(id)a0;
- (void)_setShouldConnectToDaemon;
- (char)_shouldWeMergeLatestHomeGraphFromDaemonResponse:(id)a0;
...
- (void)_updatePrimaryHome:(id)a0 notifyDelegate:(char)a1;
- (void)_updateStatusWithPayload:(id)a0;
- (void)_updateStatusWithPayload:(id)a0 sourceIsFetch:(char)a1;
...
- (char)accessoryInfoDataProvider:(id)a0 transformHomeUUID:(id)a1 accessoryUUID:(id)a2 toClientHomeIdentifier:(id *)a3 clientAccessoryIdentifier:(id *)a4;
...
- (char)accessorySettingsDataSource:(id)a0 transformHomeUUID:(id)a1 accessoryUUID:(id)a2 toClientHomeIdentifier:(id *)a3 clientAccessoryIdentifier:(id *)a4;
- (char)accessorySettingsDataSource:(id)a0 transformHomeUUID:(id)a1 mediaSystemUUID:(id)a2 toClientHomeIdentifier:(id *)a3 clientMediaSystemIdentifier:(id *)a4;
...
- (char)deviceUnlockedSinceBootForUserActionPredictionProvider:(id)a0;
- (void)dumpCloudDatabase:(char)a0 localDatabase:(char)a1 workingDatabase:(char)a2 fakeCloudModels:(char)a3 completion:(id /* block */)a4;
- (void)dumpState:(id)a0 payload:(id)a1 completion:(id /* block */)a2;
- (void)enableUARPPacketCapture:(char)a0 capturePath:(id)a1 completionHandler:(id /* block */)a2;
- (void)endBatchNotificationsForDataProvider:(id)a0 reason:(id)a1;
- (void)endBatchNotificationsForDataSource:(id)a0 reason:(id)a1;
- (void)eraseHomeDataAndDeleteMetadata:(char)a0 completionHandler:(id /* block */)a1;
...
- (void)fetchAppleMediaAccesoryDiagnosticInfo:(id)a0 options:(unsigned long long)a1 filteringKeyPaths:(id)a2 completion:(id /* block */)a3;
...
- (id)fetchPairingIdentitiesInRankOrder:(id)a0 error:(id *)a1;
...
- (char)isHomeRemovedPermanently:(id)a0;
- (char)isiPhoneOnlyPairingSupportedForMatterAccessories;
- (void)listEphemeralContainersWithCompletion:(id /* block */)a0;
- (char)needsOSUpdateToRunHH2;
- (void)notifyDelegateOfBatchNotificationsEndWithReason:(id)a0;
- (void)notifyDelegateOfBatchNotificationsStartWithReason:(id)a0;
- (void)notifyResidentCapableUpdated:(char)a0;
- (void)notifyResidentEnabledUpdated:(char)a0;
...
- (char)removeAllHomeKitPairingIdentities;
- (void)removeHome:(id)a0 completionHandler:(id /* block */)a1;
- (void)requestLogEventDailySchedulerStatusWithCompletion:(id /* block */)a0;
- (void)resetConfiguration:(char)a0 withoutPopup:(char)a1 completionHandler:(id /* block */)a2;
...
- (void)setAccessAllowedWhenLocked:(char)a0;
- (void)setDataSyncState:(unsigned long long)a0;
- (void)setHasOptedToHH2:(char)a0;
- (void)setHh2MigrationFailedError:(id)a0;
- (void)setHh2MigrationInProgress:(char)a0;
- (void)setHomeSafetySecurityEnabled:(char)a0;
- (void)setIsHH2MigrationAvailable:(char)a0;
- (void)setMetadata:(id)a0 completionHandler:(id /* block */)a1;
- (void)setMultiUserStatus:(long long)a0;
- (void)setResidentEnabledForThisDevice:(char)a0;
- (void)setResidentProvisioningStatus:(unsigned long long)a0;
- (void)setSetupMode:(unsigned long long)a0 completion:(id /* block */)a1;
- (void)setShouldPostHH2UpgradeRequired:(char)a0;
- (void)setThisDeviceResidentCapable:(char)a0;
- (id)shareManager:(id)a0 homeWithUUID:(id)a1;
- (void)startBatchNotificationsForDataProvider:(id)a0 reason:(id)a1;
- (void)startBatchNotificationsForDataSource:(id)a0 reason:(id)a1;
...
- (void)updateAccessAllowedWhenLocked:(char)a0 completionHandler:(id /* block */)a1;
- (void)updateApplicationData:(id)a0 completionHandler:(id /* block */)a1;
- (void)updateMobileAssetsAndForce:(char)a0 completion:(id /* block */)a1;
- (void)updatePrimaryHome:(id)a0 completionHandler:(id /* block */)a1;
- (void)updateResidentEnabledForThisDevice:(char)a0 completionHandler:(id /* block */)a1;
- (void)updateiCloudSwitchState:(char)a0 completionHandler:(id /* block */)a1;
