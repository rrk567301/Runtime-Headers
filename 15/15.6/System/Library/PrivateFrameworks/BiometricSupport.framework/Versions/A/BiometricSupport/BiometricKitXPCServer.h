...
@interface BiometricKitXPCServer : NSObject <NSXPCListenerDelegate> {
    ...
    struct { unsigned long long timeStamp; unsigned char result; struct { unsigned int userID; unsigned char uuid[16]; } matchedIdentity; } _lastMatchEvent;
...
@property (nonatomic) char ariadneSignposts;
...
@property (nonatomic) char templatesAtBoot;
@property (nonatomic) char catacombFileAccessed;
@property (nonatomic) char catacombRestored;
@property (readonly, nonatomic) char lastDisplayState;
...
@property (readonly, nonatomic) struct { unsigned long long x0; unsigned char x1; struct { unsigned int x0; unsigned char x1[16]; } x2; } *lastMatchEvent;
...
- (void)notifyAppIsInactive:(char)a0 withClient:(id)a1;
...
- (int)diagnostics:(int)a0 withOptions:(id)a1 passed:(char *)a2 withDetails:(id *)a3 withClient:(id)a4;
- (void)disconnectingClient:(id)a0;
- (int)displayStateChanged:(char)a0;
- (int)dropUnlockTokenWithClient:(id)a0;
- (int)enableBackgroundFdet:(char)a0 withClient:(id)a1;
- (int)enableMatchAutoRetry:(char)a0 withClient:(id)a1;
...
- (char)isCatacombAccessible;
- (char)isClassCFileAccessible;
- (char)isFingerOnWithClient:(id)a0;
- (char)isFingerprintModificationRestricted;
- (char)isGibraltarDevice;
- (char)isM8Device;
- (int)isPeriocularEnrollmentSupported:(char *)a0 withClient:(id)a1;
- (int)isValidUser:(unsigned int)a0;
- (char)isWakingUpFromS5;
- (char)isXARTAvailableWithClient:(id)a0;
...
- (int)pauseFaceDetectTimer:(char)a0 withClient:(id)a1;
...
- (int)performDisplayStatusChangedCommand:(char)a0;
...
- (int)performGetTemplatesValidityCommand:(unsigned int)a0 isValid:(char *)a1;
- (int)performIsXARTAvailableCommand:(char *)a0;
...
- (id)pullDebugImageData:(char)a0 rotated:(char)a1 hasWidth:(unsigned int *)a2 hasHeight:(unsigned int *)a3 withClient:(id)a4;
- (int)queryIdentityMigrationFailureForUser:(unsigned int)a0 failed:(char *)a1 clear:(char)a2 withClient:(id)a3;
...
- (void)registerDelegate:(char)a0 withClient:(id)a1;
...
- (void)serviceStatus:(unsigned int)a0 withData:(id)a1 referenceTimestamp:(unsigned long long)a2 continuousTimeDelta:(long long)a3;
- (int)setAppleMesaSEPLoggingLevel;
- (int)setDebugImages:(char)a0 withClient:(id)a1;
- (char)setPreferencesValue:(id)a0 forKey:(id)a1 withClient:(id)a2;
...
- (char)supportsRemovableAccessories;
- (void)suspendAllConnections:(char)a0;
- (int)suspendEnrollment:(char)a0 withClient:(id)a1;
- (void)switchToNextBioOperation:(char)a0;
...
- (void)updateEnrollmentChangedNotification:(char)a0;
