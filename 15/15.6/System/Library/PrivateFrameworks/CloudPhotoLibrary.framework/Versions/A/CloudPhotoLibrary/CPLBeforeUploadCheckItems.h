@class NSString, NSArray, CPLRecordTargetMapping, CPLChangeBatch, NSDictionary, NSMutableDictionary, NSMutableSet, CPLFingerprintContext;
...
@property (nonatomic) char shouldCheckOverQuotaChangesWithServer;
...
@property (readonly, nonatomic) char shouldResetExceedingQuotaOnSuccessfulUpload;
...
@property (readonly, nonatomic) char willUploadSomeResources;
@property (readonly, nonatomic) char hasDroppedSomeResources;

- (void).cxx_destruct;
- (void)addAdditionalRecordWithScopedIdentifierToUploadBatch:(id)a0;
- (char)checkBatchWithFoundRecords:(id)a0 error:(id *)a1;
- (id)initWithBatch:(id)a0 targetMapping:(id)a1 ruleGroups:(long long)a2 pushRepositoryPriority:(unsigned long long)a3 fingerprintContext:(id)a4 provider:(id)a5;
- (void)itemShouldBeReinjectedInPushRepository:(id)a0;
- (void)itemWillDropResourceChange:(id)a0;
- (id)itemWithScopedIdentifier:(id)a0;
- (char)prepareBatchBeforeUploadWithError:(id *)a0;
- (id)recordFromTransportWithScopedIdentifier:(id)a0;
- (char)requestFetchOfRecordWithScopedIdentifier:(id)a0 forItem:(id)a1 rule:(id)a2 error:(id *)a3;
- (char)transportHasRecordWithScopedIdentifier:(id)a0;
