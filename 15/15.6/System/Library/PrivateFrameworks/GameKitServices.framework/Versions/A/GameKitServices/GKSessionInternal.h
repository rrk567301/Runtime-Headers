@class GKList, NSString, NSArray, GKAutoPeerIDTable, GKConnection, GKVoiceChatSessionListener, GKTable, GKSession;
...
- (char)sendData:(id)a0 toPeers:(id)a1 withDataMode:(unsigned int)a2 error:(id *)a3;
- (char)sendDataToAllPeers:(id)a0 withDataMode:(unsigned int)a1 error:(id *)a2;
...
- (void)didFindService:(const char *)a0 fromIF:(const char *)a1 withError:(int)a2 moreComing:(char)a3;
- (void)didLookupHostname:(struct _DNSServiceRef_t { } *)a0 forPeer:(id)a1 hostName:(const char *)a2 address:(const struct sockaddr_in { unsigned char x0; unsigned char x1; unsigned short x2; struct in_addr { unsigned int x0; } x3; char x4[8]; } *)a3 interface:(unsigned int)a4 withError:(int)a5 moreComing:(char)a6;
- (void)didPublishWithError:(int)a0;
- (void)didRemoveService:(const char *)a0 fromIF:(const char *)a1 withError:(int)a2 moreComing:(char)a3;
- (void)didResolveService:(struct _DNSServiceRef_t { } *)a0 forPeer:(id)a1 hostName:(const char *)a2 port:(unsigned short)a3 interface:(unsigned int)a4 txtLen:(unsigned short)a5 txtRecord:(const void *)a6 withError:(int)a7 moreComing:(char)a8;
- (void)didUpdateTXTRecordForPeer:(id)a0 fromIF:(const char *)a1 txtLen:(unsigned short)a2 txtRecord:(const void *)a3 withError:(int)a4 moreComing:(char)a5;
- (char)filterService:(const char *)a0 withPID:(unsigned int)a1;
- (void)handleNewGKOOBAudioMessage:(id)a0 messageData:(id)a1 remotePID:(unsigned int)a2 callbackData:(id *)a3 maxCallbackCount:(int)a4 checkDelegateCallbackSelector:(char *)a5 tellDelegateSelector:(SEL *)a6 callbackCount:(int *)a7;
...
- (char)parseServiceName:(const char *)a0 intoDisplayName:(id *)a1 pid:(unsigned int *)a2 state:(id *)a3;
- (char)passesSendDataSanityCheck:(id)a0 toPeers:(id)a1 withDataMode:(unsigned int)a2 error:(id *)a3;
...
- (char)sendAudioData:(id)a0 toPeers:(id)a1 withDataMode:(unsigned int)a2 error:(id *)a3;
- (void)sendCallbacksToDelegate:(struct { int x0; char *x1; int x2; unsigned int x3; } *)a0 remotePeer:(unsigned int)a1;
- (char)sendData:(id)a0 toPeers:(id)a1 withDataMode:(unsigned int)a2 enableOOB:(char)a3 error:(id *)a4;
- (char)sendVoiceChatData:(id)a0 toPeers:(id)a1 error:(id *)a2;
...
- (void)tellDelegate_didReceiveDataFromPeerWithContext:(id)a0;
