@class NSString, GFX9AMD_MPSPlugin;
@protocol MTLDevice;

@interface GFX9AMD_MPSPluginMatrixInstance : MPSExternalPluginBase <MPSExternalMatrixMultiplication> {
    struct AMD_MPSPluginMatrixInstanceMembersRec { GFX9AMD_MPSPlugin *mpsPlugin; struct { char transposeLeft; char transposeRight; } MPSMatrixMulAttrs; id<MTLDevice> matrixDevice; struct vector<Tensile::KernelInvocation, std::allocator<Tensile::KernelInvocation>> { struct KernelInvocation *__begin_; struct KernelInvocation *__end_; struct __compressed_pair<Tensile::KernelInvocation *, std::allocator<Tensile::KernelInvocation>> { struct KernelInvocation *__value_; } __end_cap_; } kernelDispatcher; unsigned int pluginUsage; } m_members;
}
