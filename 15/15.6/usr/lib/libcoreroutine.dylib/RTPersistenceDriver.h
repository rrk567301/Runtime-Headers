...
@property char requiresDirtyTransaction;
@property char requiresSetupTransaction;
...
@property (nonatomic) char shouldResetCloudStore;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
...
+ (id)persistCurrentDeviceWithIdentifier:(id)a0 inStore:(id)a1 context:(id)a2 platform:(id)a3 notificationHelper:(id)a4 error:(id *)a5;
...
- (char)prepareStore:(id)a0 withContext:(id)a1 error:(id *)a2;
- (void)onCloudSyncAuthorizationChange:(id)a0;
- (int)_acquireBackgroundAssertionForFileDescriptor:(int)a0;
- (char)_acquireBackgroundProcessingPermissionForStoreURL:(id)a0 cacheFileExtension:(id)a1 isRetry:(char)a2;
- (char)_acquireBackgroundProcessingPermissionsForStoreType:(unsigned long long)a0 isRetry:(char)a1;
- (void)_cleanUpFileDescriptoersForPersistenceStore;
- (char)_didPersistenceStoreBackgroundProcessingAssertionsSucceedWithRetry:(char)a0;
- (char)_evaluatePersistentHistoryForDeletedDeviceDuringResetSync:(char *)a0 resetSyncContext:(id)a1 managedObjectContext:(id)a2 error:(id *)a3;
- (void)_evaluteDirtyTransactionAfterConfigurationChange:(long long)a0;
- (int)_getFileDescriptorForPersistenceStoreFile:(id)a0;
- (char)_isBackgroundProcessingAssertionActive;
...
- (char)_purgeLocalEntityRowsUsingModel:(id)a0 managedObjectContext:(id)a1 persistenceManager:(id)a2 error:(id *)a3;
- (int)_releaseBackgroundAssertionForFileDescriptor:(int)a0;
- (char)_releaseBackgroundProcessingPermissionForStoreURL:(id)a0 cacheFileExtension:(id)a1;
- (char)_releaseBackgroundProcessingPermissionsForStoreType:(unsigned long long)a0;
- (char)_releasePersistenceStoreBackgroundProcessingAssertions;
...
- (char)_shouldAttemptSetupAccordingToDeviceConfiguration:(long long)a0;
...
- (char)backupPersistenceStore:(id)a0 error:(id *)a1;
...
- (char)copyPersistenceStore:(id)a0 outputURL:(id)a1 error:(id *)a2;
...
- (char)importSourceStore:(id)a0 sourceCoordinator:(id)a1 destinationStore:(id)a2 destinationCoordinator:(id)a3 managedObjectModel:(id)a4 configuration:(id)a5 error:(id *)a6;
...
- (char)persistCurrentDeviceRecordInStore:(id)a0 context:(id)a1 error:(id *)a2;
...
- (void)persistenceManager:(id)a0 didFinishSetup:(char)a1;
...
- (char)persistenceMirroringManagerDidFinishZonePurge:(id)a0 store:(id)a1 context:(id)a2 error:(id *)a3;
...
- (char)store:(id)a0 validateAppleIDs:(id)a1;
