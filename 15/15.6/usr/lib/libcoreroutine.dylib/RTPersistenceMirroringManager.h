...
@property (nonatomic) char exportingAvailable;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
...
- (char)_calculateStoreShouldResetFromChangeHistoryCount:(unsigned long long)a0;
- (char)_calculateStoreShouldResetFromTransactionHistory:(id)a0;
- (char)_dataAvailableToMirror;
- (void)_dequeueNextMirroringRequest;
- (void)_enqueueMirroringRequest:(id)a0 context:(id)a1;
- (char)_evaluatePersistentHistoryLengthWithManagedObjectContext:(id)a0 shouldResetSync:(char *)a1 error:(id *)a2;
- (char)_fetchDatabaseChangesForDatabase:(id)a0 outputURL:(id)a1 error:(id *)a2;
- (char)_fetchZoneChangesForDatabase:(id)a0 outputURL:(id)a1 error:(id *)a2;
...
- (char)_shouldResetForChangeCountForManagedObjectContext:(id)a0 currentExporterToken:(id)a1 changeRequestError:(id *)a2;
- (void)_shutdownWithHandler:(id /* block */)a0;
- (char)_transactionHistorySizeError:(id *)a0;
...
- (char)disableMirroringForPerProcessMemoryLimitViolation:(id)a0 platform:(id)a1;
- (char)exceedingMemoryThreshold;
- (char)exportMirroringIsAvailableWithExpirationEnforcer:(id)a0 expirationDate:(id)a1 context:(id)a2 error:(id *)a3;
...
- (char)mirroringRequest:(id)a0 didFailWithError:(id)a1;
...
- (void)setMirroringAttemptMapValue:(char)a0 buildVersion:(id)a1;
