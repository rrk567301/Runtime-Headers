...
@property (nonatomic) char receiverContactStored;
@property (nonatomic) char receiverContactStoreInProgress;
@property (nonatomic) char receiverContactUpdatePending;
@property (nonatomic) char acceptShareInvitationInProgress;
@property (nonatomic) char safetyCacheFetchPending;
...
- (void)computeResponseModality:(id)a0 completionHandler:(id /* block */)a1;
- (void)copyKeyReleaseMessageParameters:(id)a0;
- (void)copySessionStartMessageParameters:(id)a0;
- (void)decryptAndStoreSafetyCacheDataWithPhoneCacheData:(id)a0 watchCacheData:(id)a1 userInitiatedDownload:(char)a2 metricsDict:(id *)a3;
...
- (long long)normalizedSMErrorForCacheDownloadError:(id)a0;
- (void)onCacheDownloadResult:(char)a0 userInitiatedDownload:(char)a1 withError:(id)a2;
...
- (void)onLowPowerModeWarningStateUpdateMessageReceived:(id)a0;
...
- (void)onShareInvitationAcceptenceResult:(char)a0 withError:(id)a1;
- (void)onUserCacheDownloadRequest;
- (void)onViewingSessionDetails;
- (void)requestMissingKeyReleaseInfo;
- (char)shouldRetryCacheDownloadForError:(id)a0;
- (char)shouldRetryZoneShareAcceptanceForError:(id)a0;
...
- (void)transitionFromCacheDownloadFailedToCachePersistenceDueToEvent:(long long)a0;
- (void)transitionFromCacheDownloadFailedToCacheReleasedDueToEvent:(long long)a0;
- (void)transitionFromCacheDownloadFailedToCleanupDueToEvent:(long long)a0;
- (void)transitionFromCachePersistenceToCleanupDueToEvent:(long long)a0;
- (void)transitionFromCacheReleasedToCacheDownloadFailedDueToEvent:(long long)a0;
...
- (void)transitionFromInitializingToCacheDownloadFailedDueToEvent:(long long)a0;
