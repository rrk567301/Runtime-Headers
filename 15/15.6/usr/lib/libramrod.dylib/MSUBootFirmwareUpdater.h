...
@property (readonly) char supportsSlotIDs;
...
+ (id)_updaterClasses;
+ (char)requiresSEPInFW;
+ (char)isSingleStageBoot;
+ (char)supportsAFUH;
+ (char)supportsDualSPINAND;
+ (char)supportsDualiBoot;
+ (char)supportsPCIeNANDBoot;
+ (id)updaterForDevice;
