...
@interface MLTreeEnsembleClassifier : MLClassifier <MLModelSpecificationLoader, MLCompiledModelLoader, MLSpecificationCompiler> {
    struct vector<unsigned char, std::allocator<unsigned char>> { char *__begin_; char *__end_; char *__cap_; } _cached_model;
    NSData *_model_data;
    unsigned long long num_dimensions;
    struct vector<std::string, std::allocator<std::string>> { void *__begin_; void *__end_; void *__cap_; } _classes_by_string;
    struct vector<long long, std::allocator<long long>> { long long *__begin_; long long *__end_; long long *__cap_; } _classes_by_int64_t;
