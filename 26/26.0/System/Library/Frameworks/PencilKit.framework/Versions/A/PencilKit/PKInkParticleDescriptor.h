...
@interface PKInkParticleDescriptor : NSObject {
    double _dynamicStep;
    struct unique_ptr<PKFunction, std::default_delete<PKFunction>> { struct PKFunction *__ptr_; } _particleSpacingFunction;
}
...
- (void)setParticleSpacingFunction:(struct unique_ptr<PKFunction, std::default_delete<PKFunction>> { struct PKFunction *x0; })a0;
