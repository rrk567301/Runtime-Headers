...
@interface HDQuantitySampleSeriesDataEnumerator : NSObject {
    ...
    struct deque<std::tuple<double, double, float>, std::allocator<std::tuple<double, double, float>>> { struct __split_buffer<std::tuple<double, double, float> *, std::allocator<std::tuple<double, double, float> *>> { void **__first_; void **__begin_; void **__end_; void **__cap_; } __map_; unsigned long long __start_; unsigned long long __size_; } _data;
...
- (id)initWithTransaction:(id)a0 persistentID:(long long)a1 startTime:(double)a2 endTime:(double)a3 HFDKey:(long long)a4;
