@class NSCharacterSet, NSMutableDictionary, SCRHIDProperties, SCRCTargetSelectorTimer, SCRGestureFactory, NSString, NSSet, SCRCIndexMap, SCRKeyboardKey, SCREventCapsLockDetector, AXSSEventManager, SCRBezelServicesManager, SCREvent, SCRCUserDefaults, AXSSActionManager, NSMutableSet;
@protocol SCREventFactoryDelegate;
...
@property (readonly, weak, nonatomic) id<SCREventFactoryDelegate> _workspaceDelegate;
...
@property (nonatomic) BOOL brailleKeyboardInputEnabled;
@property (nonatomic) BOOL brailleKeyboardInputForBrailleAccessEnabled;
...
+ (void)initialize;
+ (BOOL)_disableAllInputEvents;
+ (BOOL)isSecurityApprovedEvent:(struct __CGEvent { } *)a0;
+ (void)setDisableAllInputEvents;
...
- (void)setBrailleKeyboardInputEnabled:(BOOL)a0;
...
- (BOOL)_handleBrailleKeyboardEvent:(struct __CGEvent { } *)a0;
...
- (BOOL)anyTimersPendingForTesting;
- (BOOL)arrowKeyQuickNavEnabled;
- (BOOL)arrowKeyQuickNavTemporarilyDisabled;
- (BOOL)brailleKeyboardInputEnabled;
- (BOOL)brailleKeyboardInputForBrailleAccessEnabled;
...
- (id)initWithWorkspaceDelegate:(id)a0 userDefaults:(id)a1;
...
- (void)processForwardedEvent:(id)a0;
...
- (void)setBrailleKeyboardInputForBrailleAccessEnabled:(BOOL)a0;
