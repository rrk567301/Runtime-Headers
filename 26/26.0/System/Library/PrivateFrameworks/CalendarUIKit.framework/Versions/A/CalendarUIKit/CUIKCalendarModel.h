@class _EKNotificationMonitor, NSMutableDictionary, EKEventStore, NSObject, CUIKPasteboardManager, NSDate, EKCalendarDate, CUIKEventLoader, NSMutableArray, CUIKIPendingReminderTracker, NSCalendar, EKEvent, NSSet, NSString, EKSource, CUIKUndoManager, NSArray, CUIKCalendarModelSceneState, NSLock;
@protocol OS_dispatch_queue, CUIKOccurrenceCacheDataSourceProtocol;
...
+ (id)eventStoreAppropriateForDataPath:(id)a0;
...
- (id)_calendarsExcludingBlockedShared:(id)a0;
...
- (void)handleEventLoaderOccurrencesDidUpdateBetweenStart:(double)a0 end:(double)a1 generation:(int)a2 wasEmptyLoad:(BOOL)a3;
...
- (id)monitorPredicate:(id)a0 options:(unsigned long long)a1 resultsBlock:(id /* block */)a2;
...
- (id)predicateForStart:(id)a0 end:(id)a1 calendars:(id)a2;
...
- (id)selectedOccurrencesForSelectionSource:(unsigned long long)a0;
...
- (void)setSelectedOccurrences:(id)a0 source:(unsigned long long)a1 forceChange:(BOOL)a2;
