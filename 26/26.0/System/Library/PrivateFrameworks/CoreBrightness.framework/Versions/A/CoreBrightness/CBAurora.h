...
@interface CBAurora : NSObject {
    ...
    struct { float targetMargin; BOOL rampInProgress; float targetScaler; float tripMaxBrightness; } _rtplc;
    struct { BOOL firstEvaluation; BOOL auroraStateSatisfied; BOOL displayStateSatisfied; BOOL luxSatisfied; BOOL autoBrightnessSatisfied; BOOL lowPowerModeSatisfied; BOOL dominoModeSatisfied; BOOL autoDimSatisfied; BOOL gracePeriodSatisfied; BOOL cltmSatisfied; BOOL upoSatisfied; } _entryConditions;
    ...
    BOOL _supportCLTMAwareAurora;
    struct { struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } sdrLUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } rampUpMinAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } rampUpMaxAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } rampDownMinAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } rampDownMaxAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } maxGain; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } energyConsumptionTarget; } _powerAwareAurora;
}
...
- (void)setCLTMActivationThreshold:(float)a0;
