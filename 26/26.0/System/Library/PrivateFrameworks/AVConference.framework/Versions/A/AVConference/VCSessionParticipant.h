@class NSMutableDictionary, VCMediaRecorder, VCAudioMachineLearningCoordinator, VCAudioIO, NSDictionary, VCCallInfoBlob, AVCBasebandCongestionDetector, NSObject, NSMutableArray, VCSecurityKeyManager, VCNetworkFeedbackController, NSString, VCAudioRuleCollection, VCExperimentManager, AVCStatisticsCollector, VCSessionMediaNegotiator, VCRedundancyControllerVideo, NSArray, NSData, VCSessionParticipantOneToOneConfig;
...
@interface VCSessionParticipant : VCObject <VCMediaStreamDelegate, VCSecurityEventHandler, VCAudioIOSink, VCAudioIOSource, VCAudioIODelegate, VCConnectionChangedHandler, VCRedundancyControllerDelegate, VCAudioPowerSpectrumSourceDelegate, VCMediaStreamGroupDelegate> {
    ...
    BOOL _shouldConfigureMicrophoneForThirdPartyCall;
    BOOL _shouldSynchronizeSystemAudioStartWithAVCScreenCapture;
...
@property (readonly, nonatomic) BOOL isParticipantIDBasedKeyLookupEnabled;
...
- (void)configureFECFeedbackVersionAndGenerator:(id)a0 withStreamGroupID:(unsigned int)a1;
- (void)configureFramebasedFecForOneToOneVideoStreamConfig:(id)a0 withStreamGroupID:(unsigned int)a1;
- (void)configureFullScreenCameraPropertiesOnVideoStreamConfig:(id)a0 videoSettings:(id)a1 streamDirection:(long long)a2;
...
- (void)configureParticipantForLowPowerModeEnabled:(BOOL)a0;
...
- (void)dispatchedStop;
- (id)generateEncryptionKeyForCipherSuite:(long long)a0;
- (id)getAudioDumpName;
...
- (void)mkiCollisionDetectedForStreamGroup:(id)a0;
...
- (BOOL)remoteSupportsFullScreenReceive:(id)a0;
...
- (void)syncLegacyWithEnabled:(BOOL *)a0 paused:(BOOL *)a1 fromState:(id)a2;
- (void)syncMicMediaStateFromLegacyState;
- (void)syncScreenMediaStateFromLegacyState;
- (void)syncState:(id)a0 fromLegacyWithEnabled:(BOOL)a1 paused:(BOOL)a2;
- (id)systemAudioGroup;
