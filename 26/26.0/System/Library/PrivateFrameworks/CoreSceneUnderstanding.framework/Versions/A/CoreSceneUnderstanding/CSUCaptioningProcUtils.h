...
@property (nonatomic) struct BeamSearchOptions { unsigned long long maxSeqLen; BOOL terminateAtEOS; struct optional<int> { union { char __null_state_; int __val_; } ; BOOL __engaged_; } maxSteps; struct optional<std::vector<std::string>> { union { char __null_state_; struct vector<std::string, std::allocator<std::string>> { void *__begin_; void *__end_; void *__cap_; } __val_; } ; BOOL __engaged_; } excludeTokens; NSArray *denyListRules; float lengthNormalizationAlpha; float lengthNormFactor; unsigned long long scorerType; int beamWidth; int topKPerStep; float lengthNormalizationAlpha; } beamSearchOptions;
@property (nonatomic) struct EspressoTensor { void /* function */ **_vptr$Tensor; int type_; struct TensorShape { struct vector<unsigned long, std::allocator<unsigned long>> { unsigned long long *__begin_; unsigned long long *__end_; unsigned long long *__cap_; } sizes_; } shape_; struct shared_ptr<ik::TensorStorage> { struct TensorStorage *__ptr_; struct __shared_weak_count *__cntrl_; } storage_; } encodedFeaturesBuffer;
