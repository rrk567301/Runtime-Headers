...
@interface _EARFormatter : NSObject {
    struct unique_ptr<SpeechITN, std::default_delete<SpeechITN>> { struct SpeechITN *__ptr_; } _itn;
...
+ (void)initialize;
+ (struct vector<quasar::Token, std::allocator<quasar::Token>> { struct Token *x0; struct Token *x1; struct Token *x2; })convertStringsToQuasarTokens:(id)a0;
+ (struct vector<quasar::Token, std::allocator<quasar::Token>> { struct Token *x0; struct Token *x1; struct Token *x2; })convertStringsToQuasarTokens:(id)a0 offset:(unsigned int)a1;
+ (BOOL)supportedByQuasarConfig:(id)a0;
+ (BOOL)supportedByQuasarSystemConfig:(const void *)a0;
+ (BOOL)useEmojiHammerWhenRecognizeEmoji:(BOOL)a0;

- (id).cxx_construct;
- (void).cxx_destruct;
- (struct vector<quasar::Token, std::allocator<quasar::Token>> { struct Token *x0; struct Token *x1; struct Token *x2; })formatWords:(const void *)a0 unrepairedWordsOut:(void *)a1;
- (struct basic_string<char, std::char_traits<char>, std::allocator<char>> { union __rep { struct __short { char x0[23]; unsigned char x1 : 7; unsigned char x2 : 1; } x0; struct __long { char *x0; unsigned long long x1; unsigned long x2 : 63; unsigned char x3 : 1; } x1; } x0; })getOrthography:(const void *)a0;
...
- (id)_recognizeEmojiInString:(id)a0 alternativesOut:(id)a1 persistEmoji:(BOOL)a2;
...
- (struct vector<std::string, std::allocator<std::string>> { void *x0; void *x1; void *x2; })emojiAlternativesForEmojis:(id)a0;
...
- (struct unordered_set<std::string, std::hash<std::string>, std::equal_to<std::string>, std::allocator<std::string>> { struct __hash_table<std::string, std::hash<std::string>, std::equal_to<std::string>, std::allocator<std::string>> { struct unique_ptr<std::__hash_node_base<std::__hash_node<std::string, void *> *> *[], std::__bucket_list_deallocator<std::allocator<std::__hash_node_base<std::__hash_node<std::string, void *> *> *>>> { void **x0; struct __bucket_list_deallocator<std::allocator<std::__hash_node_base<std::__hash_node<std::string, void *> *> *>> { unsigned long long x0; } x1; } x0; struct __hash_node_base<std::__hash_node<std::string, void *> *> { void *x0; } x1; unsigned long long x2; float x3; } x0; })endsOfSentencePunctuationsForLanguage:(const void *)a0;
- (id)formatWords:(id)a0 task:(id)a1 autoPunctuate:(BOOL)a2 recognizeEmoji:(BOOL)a3;
- (struct vector<quasar::Token, std::allocator<quasar::Token>> { struct Token *x0; struct Token *x1; struct Token *x2; })formatWords:(const void *)a0 unrepairedWordsOut:(void *)a1 task:(id)a2;
- (struct vector<quasar::Token, std::allocator<quasar::Token>> { struct Token *x0; struct Token *x1; struct Token *x2; })formatWords:(const void *)a0 unrepairedWordsOut:(void *)a1 task:(id)a2 language:(id)a3 preItnLeftContext:(const void *)a4 separateAutoEndPunctuation:(BOOL)a5 partialResults:(void *)a6 timestampOffset:(unsigned int)a7 zeroTimestamp:(BOOL)a8 continuousListeningConfig:(struct shared_ptr<quasar::ContinuousListeningConfig> { struct ContinuousListeningConfig *x0; struct __shared_weak_count *x1; })a9 postItnLeftContext:(const void *)a10 itnResult:(void *)a11 itnOverrides:(const void *)a12 itnEnablingFlags:(unsigned short)a13 recognizeEmoji:(BOOL)a14 leftContextProvidedByClient:(BOOL)a15 preItnRightContext:(const void *)a16 postItnLeftContextStr:(id)a17 postItnRightContextStr:(id)a18 stripLeadingAppendedAutoPunctuation:(BOOL)a19 emojiTokenIndices:(void *)a20 persistEmoji:(BOOL)a21 shouldHideTrailingPunctuation:(BOOL)a22 isTrailingPunctuationHidden:(struct shared_ptr<bool> { BOOL *x0; struct __shared_weak_count *x1; })a23 isFinal:(BOOL)a24 choiceIdx:(int)a25 itnCompletion:(id /* block */)a26;
...
- (struct vector<quasar::Token, std::allocator<quasar::Token>> { struct Token *x0; struct Token *x1; struct Token *x2; })formattedTokensWithoutEmojiModifier:(const void *)a0 emojiTokenIndices:(const void *)a1 recognizeEmoji:(BOOL)a2;
...
- (void)recognizeEmojiForTokens:(void *)a0 emojiTokenIndices:(void *)a1 persistEmoji:(BOOL)a2 choiceIdx:(int)a3;
