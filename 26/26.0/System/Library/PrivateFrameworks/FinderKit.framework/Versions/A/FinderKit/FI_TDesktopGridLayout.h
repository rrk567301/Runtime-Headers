@interface FI_TDesktopGridLayout : NSObject {
    ...
    struct TIconLayoutBitmap { long long fRowStride; struct vector<unsigned char, std::allocator<unsigned char>> { char *__begin_; char *__end_; char *__cap_; } fBitmap; long long fWidth; long long fHeight; struct Location { long long x; long long y; } fLastOpenLocation; int fStyle; BOOL fExpands; } _bitmap;
}
...
- (struct vector<CGPoint, std::allocator<CGPoint>> { struct CGPoint *x0; struct CGPoint *x1; struct CGPoint *x2; })occupyGridSpots:(unsigned long long)a0;
