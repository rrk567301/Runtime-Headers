@class NSObject, NSArray, FIFinderViewGutsController, NSURL, NSStackView, FI_TNotificationCenterObserverGlue;
@protocol FIFinderViewDelegate;

@interface FIFinderView : NSView {
    id<FIFinderViewDelegate> _delegate;
    struct TNotificationCenterObserver { NSObject *fObservedObject; struct TString { struct TRef<const __CFString *, TRetainReleasePolicy<CFStringRef>> { struct __CFString *fRef; } fString; } fNotificationName; FI_TNotificationCenterObserverGlue *fFunctorGlue; BOOL fIsDistributedObserving; struct function<NSNotificationCenter *()> { struct __value_func<NSNotificationCenter *()> { union type { unsigned char __data[24]; } __buf_; void *__f_; } __f_; } fCenterProvider; } _windowDidBecomeKeyObserver;
}
