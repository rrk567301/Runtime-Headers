@class NSUUID, NSXPCConnection, __cap_, __end_, AUAudioUnit, NSMutableArray, AUParameterTree, NSObject;
...
@interface AURemoteHost : NSObject <AUAudioUnitXPCProtocol> {
    ...
    struct KVOAggregator { struct vector<KVOAggregator::Record, std::allocator<KVOAggregator::Record>> { struct Record *__begin_; struct Record *__end_; struct Record *__cap_; } mRecords; } _kvoAggregator;
    struct vector<AURemoteMessageChannel *, std::allocator<AURemoteMessageChannel *>> { __end_ **__begin_; __cap_ **x0; id *x1; } _remoteMessageChannels;
    struct unique_ptr<XOSTransactor, std::default_delete<XOSTransactor>> { struct XOSTransactor *__ptr_; } _transactorIO;
}
