...
@interface CMMediaSession : NSObject <CMDistributedSensingDelegate> {
    ...
    BOOL _enableTempestJ314P1Workaround;
...
- (struct AudioAccessorySample { struct { struct CMOQuaternion { float x0[4]; } x0; struct CMVector<float, 3UL> { float x0[3]; } x1; struct CMVector<float, 3UL> { float x0[3]; } x2; struct CMVector<float, 3UL> { float x0[3]; } x3; struct Status { unsigned short x0; } x4; } x0; unsigned long long x1; double x2; double x3; struct CMVector<float, 3UL> { float x0[3]; } x4; float x5; int x6; unsigned int x7; int x8; int x9; unsigned long long x10; double x11; float x12[3]; BOOL x13; unsigned char x14; BOOL x15; })_getLastAudioAccessorySample;
...
- (void)feedPoseAnchorWithAttitude:(struct { double x0; double x1; double x2; double x3; })a0 position:(struct CMVector<float, 3UL> { float x0[3]; })a1 timestampUs:(unsigned long long)a2;
