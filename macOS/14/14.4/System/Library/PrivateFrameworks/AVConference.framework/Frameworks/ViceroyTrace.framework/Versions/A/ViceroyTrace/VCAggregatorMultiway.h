@class NSString, VCHistogram, NSMutableDictionary, VCReportingDistribution, DownlinkSegment, NSMutableArray, UplinkSegment, NSNumber;
...
@interface VCAggregatorMultiway : VCAggregator <SegmentStatsDelegate> {
    ...
    unsigned long long _expensiveConnectionDuration;
    unsigned long long _constrainedConnectionDuration;
    BOOL _countExpensiveConnectionDuration;
    BOOL _countConstrainedConnectionDuration;
...
- (void)addStreamGroupTelemetryForCall:(id)a0 callReport:(id)a1 maxMediaQualityKeysForParticipant:(struct tagMaxMediaQualityKeysForParticipant { int x0; int x1; int x2; int x3; int x4; } *)a2;
...
- (void)calculateAverageReceivingBitrateForStreamGroup:(id)a0 audioStreamsCount:(unsigned int *)a1 avgAudioRecvBitrate:(unsigned int *)a2 vedioStreamsCount:(unsigned int *)a3 avgVedioRecvBitrate:(unsigned int *)a4;
...
- (void)processDataMode:(unsigned char)a0;
...
- (void)transferSegmentStateToNewUplinkSegment:(id)a0 fromPreviousUplinkSegment:(id)a1;
...
- (void)updateModeDurationOnSegment:(id)a0 countExpensive:(BOOL)a1 OrCountConstrained:(BOOL)a2;
...
- (void)updateRTStatsOnCallAndSession:(id)a0 audioPacketsSent:(unsigned int)a1;
