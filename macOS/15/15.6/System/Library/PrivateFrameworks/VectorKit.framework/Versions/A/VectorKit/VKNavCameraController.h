...
@interface VKNavCameraController : VKCameraController <VKNavigationCameraController, VKGesturingCameraController, VKNavContextObserver> {
    ...
    struct basic_string<char, std::char_traits<char>, std::allocator<char>> { struct __compressed_pair<std::basic_string<char>::__rep, std::allocator<char>> { union __rep { struct __short { char __data_[23]; unsigned char __padding_[0]; unsigned char __size_ : 7; unsigned char __is_long_ : 1; } __s; struct __long { char *__data_; unsigned long long __size_; unsigned long __cap_ : 63; unsigned char __is_long_ : 1; } __l; } __value_; } __r_; } _currentStyleName;
    ...
    struct Monitorable<md::ConfigValue<GEOConfigKeyUInteger, unsigned int>> { struct { struct { unsigned int identifier; void *metadata; } key; } _key; unsigned int _value; _valid *_listener; struct shared_ptr<std::atomic<bool>> { void *__ptr_; struct __shared_weak_count *__cntrl_; } x0; struct function<void (unsigned int)> { struct __value_func<void (unsigned int)> { struct type { unsigned char __lx[24]; } __buf_; void *__f_; } __f_; } _delegate; } _debugPage;
