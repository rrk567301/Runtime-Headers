@class NSMutableDictionary, NSMapTable, NSPredicate, NSDictionary, EFLazyCache, NSObject, EMMailboxScope, NSString, EFFuture, NSSet, EMCollectionItemIDStateCapturer, EMThreadScope, EMObjectID, EMMessageRepository;
@protocol OS_os_log, EFScheduler, OS_dispatch_queue;

@interface EMMessageList : EMQueryingCollection <EFContentProtectionObserver, EFLoggable, EMCollectionChangeObserver, EMCollectionItemIDStateCapturerDelegate, EMMessageListQueryResultsObserver, EMMessageList> {
...
+ (id)simpleMessageListForMailboxes:(id)a0 withRepository:(id)a1 additionalQueryOptions:(unsigned long long)a2 countOfItemsToPrecache:(unsigned long long)a3 shouldUpdateDisplayDate:(BOOL)a4 sortDescriptors:(id)a5 transformPredicate:(id /* block */)a6 ignoreMessageAdditionsPredicate:(id)a7;
...
+ (id)threadedMessageListForMailboxes:(id)a0 withRepository:(id)a1 additionalQueryOptions:(unsigned long long)a2 countOfItemsToPrecache:(unsigned long long)a3 shouldUpdateDisplayDate:(BOOL)a4 sortDescriptors:(id)a5 transformPredicate:(id /* block */)a6 ignoreMessageAdditionsPredicate:(id)a7;
...
- (id)_itemIDsForChangeObserver:(id)a0 extraInfo:(id)a1 outObserverSection:(long long *)a2 outObjectIDsBySection:(id *)a3;
- (id)_nextThreadItemIDAfterThreadItemID:(id)a0;
- (id)_sectionFromExtraInfo:(id)a0;
- (id)_sectionItemIDsForChangeObserver:(id)a0 extraInfo:(id)a1 outUndeletedItemID:(id *)a2 isAfter:(BOOL)a3;
...
- (void)finishRecovery;
- (id)initWithMailboxes:(id)a0 repository:(id)a1 sortDescriptors:(id)a2 transformPredicate:(id /* block */)a3 ignoreMessageAdditionsPredicate:(id)a4 targetClass:(Class)a5 additionalQueryOptions:(unsigned long long)a6 countOfItemsToPrecache:(unsigned long long)a7 shouldUpdateDisplayDate:(BOOL)a8 labelPrefix:(id)a9;
