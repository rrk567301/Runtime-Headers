...
@interface MPSGraphExecutable : MPSGraphObject {
    ...
    void *_builder;
    struct LazyLoadableModuleRef { struct shared_ptr<mlir::MLIRContext> { struct MLIRContext *__ptr_; struct __shared_weak_count *__cntrl_; } _ctx; struct OwningOpRef<mlir::ModuleOp> { struct ModuleOp { struct Operation *state; } op; } _originalModule; NSURL *_moduleURL; struct shared_ptr<ModuleResourcesLoader> { struct ModuleResourcesLoader *__ptr_; struct __shared_weak_count *__cntrl_; } _resourceLoader; BOOL _needsPlistRestored; struct basic_string<char, std::char_traits<char>, std::allocator<char>> { struct __compressed_pair<std::basic_string<char>::__rep, std::allocator<char>> { struct __rep { union { struct __short { char __data_[23]; unsigned char __padding_[0]; unsigned char __size_ : 7; unsigned char __is_long_ : 1; } __s; struct __long { char *__data_; unsigned long long __size_; unsigned long __cap_ : 63; unsigned char __is_long_ : 1; } __l; struct __raw { unsigned long long __words[3]; } __r; } ; } __value_; } __r_; } _modelFileArchivePath; BOOL _keepANECUnitNameAttrs; } _originalModule;
    void *_symbolTable;
