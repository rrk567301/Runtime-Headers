@interface Recount.CountingMode : _TtCs12_SwiftObject {
    void /* unknown type, empty encoding */ id;
    void /* unknown type, empty encoding */ name;
    void /* unknown type, empty encoding */ displayName;
    void /* unknown type, empty encoding */ synopsis;
    void /* unknown type, empty encoding */ documentation;
    void /* unknown type, empty encoding */ metrics;
    void /* unknown type, empty encoding */ triggers;
    void /* unknown type, empty encoding */ settings;
    void /* unknown type, empty encoding */ displays;
    void /* unknown type, empty encoding */ expressions;
    void /* unknown type, empty encoding */ thresholds;
    void /* unknown type, empty encoding */ constants;
    void /* unknown type, empty encoding */ evaluator;
    void /* unknown type, empty encoding */ counterTriggers;
    void /* unknown type, empty encoding */ spec;
    void /* unknown type, empty encoding */ platform;
}

@end
