@class NSString, NSArray, NSOrderedSet, CHPowerLogger;

@interface CHCTCRecognitionModel : CHRecurrentNeuralNetworkCoreML {
    struct unique_ptr<CoreHandwriting::PreprocessingManager, std::default_delete<CoreHandwriting::PreprocessingManager>> { struct { struct PreprocessingManager *__ptr_; } ; } _preprocessor;
    struct unique_ptr<CoreHandwriting::FeatureExtractionProcessor, std::default_delete<CoreHandwriting::FeatureExtractionProcessor>> { struct { struct FeatureExtractionProcessor *__ptr_; } ; } _featureExtractor;
    CHPowerLogger *_powerLogger;
    NSOrderedSet *__stringCodeMap;
}

@property (readonly, nonatomic) long long sequenceCompression;
@property (nonatomic) BOOL padLastPoint;
...
- (id)_extractFeaturesFromDrawing:(id)a0 inputName:(id)a1 interpointDistance:(double)a2 paddingPerStroke:(void *)a3 error:(id *)a4;
- (void)enumerateActivationsInResultArray:(id)a0 indexArray:(id)a1 block:(id /* block */)a2;
- (id)featureProviderForDrawing:(id)a0 initialVectorAnchorPoint:(struct CGPoint { double x0; double x1; })a1 normalizeFeatures:(BOOL)a2 padFeatures:(BOOL)a3 outStrokeEndings:(void *)a4 outInputSequenceLength:(long long *)a5 outOutputSequenceLength:(long long *)a6 outPaddingPerStroke:(void *)a7 locales:(id)a8;
...
- (id)initWithModelName:(id)a0 modelPrecision:(long long)a1 decodingPruningPolicy:(struct CVNLPTextDecodingPruningPolicy { long long x0; BOOL x1; float x2; unsigned int x3; })a2;
- (BOOL)reachesActivationThreshold:(double)a0 inCharacterSet:(id)a1 resultArray:(id)a2 indexArray:(id)a3;
- (id)recognizeDrawing:(id)a0 minimumDrawingSize:(struct CGSize { double x0; double x1; })a1 initialVectorAnchorPoint:(struct CGPoint { double x0; double x1; })a2 activeCharacterSet:(id)a3 outStrokeIndexMapping:(void *)a4 outStrokeEndings:(void *)a5 outFallbackSymbol:(id *)a6 outFallbackSymbolScore:(double *)a7 outPrincipalLinePoints:(id *)a8 locales:(id)a9;
