...
@interface CHTransformedTextSegment : NSObject <CHTransformedText> {
    ...
    struct vector<CGPoint, std::allocator<CGPoint>> { struct CGPoint *__begin_; struct CGPoint *__end_; struct { struct CGPoint *__cap_; } ; } _points;
    struct vector<std::vector<CGPoint>, std::allocator<std::vector<CGPoint>>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _strokePoints;
}
