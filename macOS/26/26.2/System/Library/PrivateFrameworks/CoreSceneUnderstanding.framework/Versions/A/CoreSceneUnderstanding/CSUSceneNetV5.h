...
@interface CSUSceneNetV5 : NSObject {
    ...
    struct unique_ptr<ImageDescriptorProcessorHyperplaneLSH, std::default_delete<ImageDescriptorProcessorHyperplaneLSH>> { struct { struct ImageDescriptorProcessorHyperplaneLSH *__ptr_; } ; } _fingerprinter;
}
...
- (struct vector<float, std::allocator<float>> { float *x0; float *x1; struct { float *x0; } x2; })computeAllSceneClassificationLikelihoods:(id)a0;
