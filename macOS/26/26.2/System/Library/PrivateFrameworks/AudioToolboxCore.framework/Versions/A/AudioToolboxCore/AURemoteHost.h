@class NSUUID, NSXPCConnection, , __end_, AUAudioUnit, NSMutableArray, AUParameterTree, NSObject;
...
@interface AURemoteHost : NSObject <AUAudioUnitXPCProtocol> {
    ...
    struct reply_watchdog_factory { BOOL mDebugging; int mDefaultTimeoutMS; struct function<void ()> { struct __value_func<void ()> { union type { unsigned char __data[24]; } __buf_; void *__f_; } __f_; } mTimeoutHandler; } _replyWatchdogFactory;
    ...
    struct KVOAggregator { struct vector<KVOAggregator::Record, std::allocator<KVOAggregator::Record>> { struct Record *__begin_; struct Record *__end_; struct { struct Record *__cap_; } ; } mRecords; } _kvoAggregator;
    struct vector<AURemoteMessageChannel *, std::allocator<AURemoteMessageChannel *>> { __end_ **__begin_;  **x0; struct { id *__cap_; } x1; } _remoteMessageChannels;
    struct unique_ptr<XOSTransactor, std::default_delete<XOSTransactor>> { struct { struct XOSTransactor *__ptr_; } ; } _transactorIO;
}
...
- (id)initWithConnection:(id)a0 config:(id)a1 timeOutHandler:(struct function<void ()> { struct __value_func<void ()> { union type { unsigned char x0[24]; } x0; void *x1; } x0; })a2;
