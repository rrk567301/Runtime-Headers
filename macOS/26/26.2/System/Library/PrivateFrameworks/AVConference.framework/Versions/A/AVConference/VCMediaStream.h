@class VCExperimentManager, AVCBasebandCongestionDetector, NSMutableArray, NSString, NSError, NSArray, VCMediaKeyIndex, VCNetworkFeedbackController, VCTransportSession, VCWeakObjectHolder, NSObject, TimingCollection, VCMediaStreamRateAdaptation, VCMediaStreamConfig, VCConnection, VCMediaStreamTransport, VCWiFiChannelSequenceMonitor, VCRateSharingGroup, AVCRateControlFeedbackController, AVCStatisticsCollector, VCCallInfoBlob, VCDatagramChannelIDS;
...
@interface VCMediaStream : VCObject <VCMediaStreamProtocol, RTCPReportProvider, VCSecurityEventHandler, VCConnectionChangedHandler, VCMediaTimeSync> {
    ...
    struct _VCMediaStreamTransportSetupInfo { unsigned char setupType; union { struct { int rtpSocket; int rtcpSocket; } socketInfo; struct { struct tagIPPORT { int iFlags; char szIfName[16]; union { unsigned int dwIPv4; unsigned char abIPv6[16]; } IP; unsigned short wPort; } srcIPPORT; struct tagIPPORT { int iFlags; char szIfName[16]; union { unsigned int dwIPv4; unsigned char abIPv6[16]; } IP; unsigned short wPort; } srcRTPIPPort; } ipInfo; struct { void *context; void /* function */ *creationCallback; BOOL isReceiveExternallyScheduled; BOOL requiresLargeReceiveBuffer; BOOL isRTCPReceiveFromMultipleSSRCsEnabled; int transportStreamIndex; } transportStreamInfo; NSObject<OS_nw_connection> *nwConnection; } ; unsigned int sourceRate; unsigned int datagramChannelToken; BOOL isSessionIDValid; void *defaultMediaKeyIndex; } _transportSetupInfo;
    ...
    BOOL _isMediaStreamTargetBitrateRemainUnchanged;
    ...
    BOOL _isNWConnectionStartDelayed;
...
- (void)handleStartTransportSessionCompletionBlockDidSucceed:(BOOL)a0 withError:(id)a1;
...
- (void)applyClientSessionID:(int)a0 clientUserInfo:(id)a1;
...
- (double)computeNextTimoutWithEnabledTime:(double)a0 timeoutInterval:(double)a1 lastReceivedPacketTime:(double)a2 currentTime:(double)a3 lastTimeoutReportTime:(double)a4 timeoutReportInterval:(double)a5;
- (void)configureDefaultMediaKeyIndexForStreamConfig:(id)a0;
- (id)copyDelegate;
...
- (id)initWithTransportSessionID:(unsigned int)a0 localSSRC:(unsigned int)a1 streamToken:(long long)a2;
- (id)initWithTransportSessionID:(unsigned int)a0 localSSRC:(unsigned int)a1 streamToken:(long long)a2 logPrefix:(id)a3;
- (BOOL)initializeTransportSessionWithIDSDestination:(id)a0 error:(id *)a1;
- (BOOL)initializeTransportSessionWithLocalNWEnpoint:(id)a0 clientAuditToken:(id)a1 error:(id *)a2;
- (BOOL)initializeTransportSessionWithRTPNWConnectionID:(id)a0 rtcpNWConnectionID:(id)a1 error:(id *)a2 isConnectionStartDelayed:(BOOL)a3;
...
- (BOOL)isEndpointManagementAllowed:(id *)a0;
- (BOOL)isRCCMCipherSuiteWithStreamConfig:(long long)a0;
- (BOOL)isRTCPSendEnabled;
- (BOOL)isRTCPTimeoutEnabled;
- (BOOL)isRTPTimeoutEnabled;
- (void)manageRateAdaptationRemoteEndpointForEndpointWithSSRC:(unsigned int)a0 actionType:(unsigned char)a1;
...
- (id)setUpRTPWithLocalNWEndpoint:(id)a0 error:(id *)a1;
...
- (void)startTransportSessionWithCompletion:(id /* block */)a0;
...
- (BOOL)updateNWConnectionClientIDWithConfig:(id)a0 error:(id *)a1;
