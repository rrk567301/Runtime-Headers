...
@interface SCMLTextSanitizer : NSObject {
    ...
    struct unique_ptr<scml::MultiwordGazetteer, std::default_delete<scml::MultiwordGazetteer>> { struct { struct MultiwordGazetteer *__ptr_; } ; } _personsGazetteer;
    struct unique_ptr<scml::MultiwordGazetteer, std::default_delete<scml::MultiwordGazetteer>> { struct { struct MultiwordGazetteer *__ptr_; } ; } _visualGenerationIgnoreGazetteer;
    struct unique_ptr<scml::(anonymous namespace)::RuleBasedTextSanitizer, std::default_delete<scml::(anonymous namespace)::RuleBasedTextSanitizer>> { struct { struct RuleBasedTextSanitizer *__ptr_; } ; } _rulesBasedSanitizer;
