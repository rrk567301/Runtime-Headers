@interface FI_TSegmentedControl : NSSegmentedControl {
    struct map<long, FI_TSegmentedControlHelper *, std::less<long>, std::allocator<std::pair<const long, FI_TSegmentedControlHelper *>>> { struct __tree<std::__value_type<long, FI_TSegmentedControlHelper *>, std::__map_value_compare<long, std::__value_type<long, FI_TSegmentedControlHelper *>, std::less<long>>, std::allocator<std::__value_type<long, FI_TSegmentedControlHelper *>>> { void *__begin_node_; struct { struct __tree_end_node<std::__tree_node_base<void *> *> { void *__left_; } __end_node_; } ; struct { unsigned long long __size_; } ; } __tree_; } _segmentToHelperMap;
}
