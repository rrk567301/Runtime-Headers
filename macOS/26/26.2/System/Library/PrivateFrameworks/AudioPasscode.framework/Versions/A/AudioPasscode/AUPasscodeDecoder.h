...
@interface AUPasscodeDecoder : AUAudioUnit {
    struct unique_ptr<caulk::concurrent::messenger, std::default_delete<caulk::concurrent::messenger>> { struct { struct messenger *__ptr_; } ; } _rtMessenger;
    struct unique_ptr<APCDecoderBase, std::default_delete<APCDecoderBase>> { struct { struct APCDecoderBase *__ptr_; } ; } _kernel;
    ...
    struct vector<unsigned char, std::allocator<unsigned char>> { char *__begin_; char *__end_; struct { char *__cap_; } ; } _rxDataBuffer;
    struct array<std::unique_ptr<DecodedDataMessage>, 10UL> { struct unique_ptr<DecodedDataMessage, std::default_delete<DecodedDataMessage>> { struct { struct DecodedDataMessage *__ptr_; } ; } __elems_[10]; } _messagePool;
    struct map<unsigned int, std::any, std::less<unsigned int>, std::allocator<std::pair<const unsigned int, std::any>>> { struct __tree<std::__value_type<unsigned int, std::any>, std::__map_value_compare<unsigned int, std::__value_type<unsigned int, std::any>, std::less<unsigned int>>, std::allocator<std::__value_type<unsigned int, std::any>>> { void *__begin_node_; struct { struct __tree_end_node<std::__tree_node_base<void *> *> { void *__left_; } __end_node_; } ; struct { unsigned long long __size_; } ; } __tree_; } _apcDecoderConfig;
