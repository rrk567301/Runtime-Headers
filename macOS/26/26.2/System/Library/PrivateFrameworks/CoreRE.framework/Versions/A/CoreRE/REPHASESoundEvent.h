@class , PHASEMixerParameters, __end_, NSObject, PHASEEngine, PHASEExternalOutputStreamController, NSString, PHASESoundEvent, PHASEListener, PHASESoundEventNodeAsset, AVAudioSession, PHASESource, AUAudioUnit, PHASEExternalOutputStream;
@protocol OS_dispatch_queue, REPHASESoundEventObserver;

@interface REPHASESoundEvent : NSObject <PHASEExternalStreamDelegate> {
    struct queue<REPHASESoundEventCommandState, std::deque<REPHASESoundEventCommandState>> { struct deque<REPHASESoundEventCommandState, std::allocator<REPHASESoundEventCommandState>> { struct __split_buffer<REPHASESoundEventCommandState *, std::allocator<REPHASESoundEventCommandState *>> { struct REPHASESoundEventCommandState **__first_; struct REPHASESoundEventCommandState **__begin_; struct REPHASESoundEventCommandState **__end_; struct { struct REPHASESoundEventCommandState **__cap_; } ; } __map_; unsigned long long __start_; struct { unsigned long long __size_; } ; } c; } _commandQueue;
    ...
    PHASEEngine *_engine;
    PHASESource *_source;
    struct vector<PHASESource *, std::allocator<PHASESource *>> { __end_ **__begin_;  **x0; struct { id *__cap_; } x1; } _sources;
...
- (id)initSyncGroupWithEngine:(id)a0 rootNode:(id)a1 assetIdentifier:(id)a2 listener:(id)a3 mixerParameters:(id)a4 audioSession:(id)a5 phaseQueue:(id)a6 sources:(struct vector<PHASESource *, std::allocator<PHASESource *>> { id *x0; id *x1; struct { id *x0; } x2; })a7 outError:(id *)a8;
...
- (id)initWithEngine:(id)a0 sources:(struct vector<PHASESource *, std::allocator<PHASESource *>> { id *x0; id *x1; struct { id *x0; } x2; })a1 listener:(id)a2 audioSession:(id)a3 phaseQueue:(id)a4;
...
- (struct vector<simd_float4x4, std::allocator<simd_float4x4>> { struct *x0; struct *x1; struct { struct *x0; } x2; })sourcePoses;
- (void)stopAndDestroy;
