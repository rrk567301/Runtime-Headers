...
@interface CBFrameStats : NSObject {
    ...
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } _rtplcFrameDelayedNitsTable;
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } _rtplcFrameDelayedAPCETable;
...
- (void)processFrameInfo:(struct { unsigned long long x0; unsigned int x1; BOOL x2; BOOL x3; float x4; float x5; float x6; float x7; })a0;
- (BOOL)shouldPop;
