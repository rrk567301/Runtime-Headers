...
@interface MTRServerEndpoint : NSObject {
    ...
    struct unique_ptr<EmberAfCluster[], std::default_delete<EmberAfCluster[]>> { struct { struct EmberAfCluster *__ptr_; } ; } _matterClusterMetadata;
    struct { struct EmberAfCluster *cluster; unsigned char clusterCount; unsigned short endpointSize; } _matterEndpointMetadata;
    struct unique_ptr<chip::app::DataModel::DeviceTypeEntry[], std::default_delete<chip::app::DataModel::DeviceTypeEntry[]>> { struct { struct DeviceTypeEntry *__ptr_; } ; } _matterDeviceTypes;
    struct unique_ptr<unsigned int[], std::default_delete<unsigned int[]>> { struct { unsigned int *__ptr_; } ; } _matterDataVersions;
