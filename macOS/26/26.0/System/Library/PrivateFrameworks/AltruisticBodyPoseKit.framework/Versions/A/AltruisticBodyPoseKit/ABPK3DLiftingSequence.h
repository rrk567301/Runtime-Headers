...
@interface ABPK3DLiftingSequence : NSObject <ABPK3DLiftingAlgorithm> {
    struct RingBuffer<std::vector<std::array<float, 3>>, 243UL> { unsigned long long _insertionIndex; struct array<std::vector<std::array<float, 3>>, 243UL> { struct vector<std::array<float, 3>, std::allocator<std::array<float, 3>>> { void *__begin_; void *__end_; void *__cap_; } __elems_[243]; } _storage; BOOL _filled; } _3DLiftingInputBuffer;
    double _imageWidth;
    double _imageHeight;
    int _frameCount;
    ...
    struct vector<simd_float4x4, std::allocator<simd_float4x4>> { struct *__begin_; struct *__end_; struct *__cap_; } _liftingResultAsModelPoses;
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } _gaussianSmoothingFilter;
    struct RingBuffer<std::vector<float>, 9UL> { unsigned long long _insertionIndex; struct array<std::vector<float>, 9UL> { struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } __elems_[9]; } _storage; BOOL _filled; } _smoothingFilterRingBuffer;
...
- (struct vector<float __attribute__((ext_vector_type(3))), std::allocator<float __attribute__((ext_vector_type(3)))>> { void ***x0; })getGaussianSmoothedOutput:(const void *)a0;
