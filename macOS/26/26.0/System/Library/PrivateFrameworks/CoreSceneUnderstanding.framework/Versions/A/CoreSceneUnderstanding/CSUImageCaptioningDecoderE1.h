...
@interface CSUImageCaptioningDecoderE1 : NSObject {
    struct unique_ptr<ik::EspressoNet, std::default_delete<ik::EspressoNet>> { struct EspressoNet *__ptr_; } _decoderNet;
    struct unique_ptr<ik::EspressoNet, std::default_delete<ik::EspressoNet>> { struct EspressoNet *__ptr_; } _bridgeNet;
...
- (struct vector<float, std::allocator<float>> { float *x0; float *x1; float *x2; })nextTokensForInputs:(const void *)a0 AndforMaskPosition:(unsigned int)a1;
