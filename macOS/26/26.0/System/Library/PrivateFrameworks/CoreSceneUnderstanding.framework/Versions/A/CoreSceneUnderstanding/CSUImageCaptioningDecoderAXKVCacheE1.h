...
- (struct ModelOutput { struct vector<float, std::allocator<float>> { float *x0; float *x1; float *x2; } x0; struct shared_ptr<std::unordered_map<std::string, std::vector<float>>> { void *x0; struct __shared_weak_count *x1; } x1; BOOL x2; })nextTokensForInputs:(const void *)a0 KVCache:(const void *)a1 AndforMaskPosition:(unsigned int)a2;
