@class __end_, CKXORCReaderOptions, __cap_, CKDSReadableStorage;
@protocol CKXORCHelpers;

@interface CKXORCReader : CKXReaderBase <CKXReaderProtocol> {
    struct unique_ptr<orc::Reader, std::default_delete<orc::Reader>> { struct Reader *__ptr_; } _reader;
    struct vector<std::pair<std::unique_ptr<orc::RowReader>, std::unique_ptr<orc::ColumnVectorBatch>>, std::allocator<std::pair<std::unique_ptr<orc::RowReader>, std::unique_ptr<orc::ColumnVectorBatch>>>> { void *__begin_; void *__end_; void *__cap_; } _structToReaderBatchPair;
    struct vector<NSIndexSet *, std::allocator<NSIndexSet *>> { __end_ **__begin_; __cap_ **x0; id *x1; } _rowsForTopLevelContainerStructs;
}
