...
@interface ASDTIOA2LegacyDevice : ASDTAudioDevice {
    struct unique_ptr<ASDT::IOA2UCDevice, std::default_delete<ASDT::IOA2UCDevice>> { struct IOA2UCDevice *__ptr_; } _ucDevice;
    ...
    struct unique_ptr<ASDT::MachPort, std::default_delete<ASDT::MachPort>> { struct MachPort *__ptr_; } _ucDeviceNotificationPort;
    struct mutex { struct _opaque_pthread_mutex_t { long long __sig; char __opaque[56]; } __m_; } _ucIOLock;
    struct unique_ptr<CADeprecated::CADispatchQueue, std::default_delete<CADeprecated::CADispatchQueue>> { struct CADispatchQueue *__ptr_; } _machDispatchQueue;
...
- (struct array<std::vector<unsigned int>, 2UL> { struct vector<unsigned int, std::allocator<unsigned int>> { unsigned int *x0; unsigned int *x1; unsigned int *x2; } x0[2]; })_getStreamListUCIDLists;
- (void)_mapIOBuffer:(char **)a0 ofSize:(unsigned int *)a1 forStream:(unsigned int)a2;
- (void)_mapIOBuffers;
- (id)_markOrCreateControlsWithDictionaries:(id)a0;
- (id)_markOrCreateStreamsWithIDs:(struct vector<unsigned int, std::allocator<unsigned int>> { unsigned int *x0; unsigned int *x1; unsigned int *x2; })a0 direction:(unsigned int)a1;
