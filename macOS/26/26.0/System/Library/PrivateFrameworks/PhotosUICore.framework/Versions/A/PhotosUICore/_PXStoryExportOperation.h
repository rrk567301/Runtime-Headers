@class PXStoryModel, AVAssetWriterInputGroup, PXStoryExportVideoOptions, NSString, NSError, AVAssetWriter, PXGEngine, PXStorySongController, PXMediaProvider, NSObject, PXGPixelBufferMetalRenderDestination, AVAssetReaderAudioMixOutput, PXStoryExportConfiguration, PXObservable, PXStoryRelatedController, PXStoryPacingController, AVAssetWriterInputPixelBufferAdaptor, AVAssetReader, NSURL, NSProgress, AVAssetWriterInput;
@protocol PXStoryStyle, OS_os_log, PXStorySongResource, OS_dispatch_semaphore, OS_dispatch_queue, PXAudioCueSource;
...
+ (id)_metadataItemWithKey:(id)a0 value:(id)a1;
...
- (void)_audioWriteQueue_exportAudioWithInput:(id)a0 audioMixOutput:(id)a1 completionHandler:(id /* block */)a2;
- (void)_audioWriteQueue_setupAudioReaderWithAudioExportComposition:(id)a0 audioMix:(id)a1 audioMixWithoutMusic:(id)a2;
- (void)_checkForDriftFromCurrentTime:(struct { long long x0; int x1; unsigned int x2; long long x3; })a0;
- (void)_exportDefaultAudioTrackWithCompletionHandler:(id /* block */)a0;
- (void)_exportVideo;
- (void)_exportWithoutMusicAudioTrackWithCompletionHandler:(id /* block */)a0;
- (void)_finishWriting;
...
- (id)initWithURL:(id)a0 configuration:(id)a1 qualityOfService:(long long)a2 initialSongResource:(id)a3 progressHandler:(id /* block */)a4;
