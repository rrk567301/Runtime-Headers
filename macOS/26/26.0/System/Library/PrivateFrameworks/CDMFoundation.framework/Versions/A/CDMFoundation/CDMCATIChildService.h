...
@interface CDMCATIChildService : NSObject {
    ...
    struct unique_ptr<float[], std::default_delete<float[]>> { float *__ptr_; } _weightPtr;
    struct unique_ptr<float[], std::default_delete<float[]>> { float *__ptr_; } _multiturnWeightPtr;
...
- (struct vector<float, std::allocator<float>> { float *x0; float *x1; float *x2; })getCATIEmbeddingTensor:(id)a0;
- (BOOL)getFlatbuffersBFAt:(id)a0;
- (id)getInvocationInferenceResults:(struct vector<float, std::allocator<float>> { float *x0; float *x1; float *x2; })a0;
- (BOOL)getInvocationOverridesEnabled;
- (id)getManifestFromPath:(id)a0;
- (id)getMultiturnEnabledExactMatchIntents;
- (id)getMultiturnInferenceResults:(struct vector<float, std::allocator<float>> { float *x0; float *x1; float *x2; })a0;
