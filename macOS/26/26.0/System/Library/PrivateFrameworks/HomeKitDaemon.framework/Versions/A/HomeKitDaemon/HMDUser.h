@class NSDictionary, HMDHome, HAPPairingIdentity, HMDAccountIdentifier, HMPhotosPersonManagerSettings, HMDCloudShareTrustManager, HMDUserDataController, NSString, HMDCloudShareMessenger, HMDAssistantAccessControl, HMDUserPhotosPersonDataManager, HMDAccountHandle, HMBShareUserID, HMFMessageDispatcher, HMDUserPresenceRegion, NSObject, NSSet, NSData, HMDRestrictedGuestHomeAccessSettings, HMDDevice, AVOutputDeviceAuthorizedPeer, NAFuture, HMDPersonSettingsManager, HMDSiriMultiUserNewLanguageNotificationManager, NSUUID, HMDPhotosPersonManager, HMUserPresenceAuthorization, HMDUserSettingsBackingStoreController, NSNumber, NSDate;
@protocol HMDUserDataSource, HMDUserAccessorySettingsDataController, HMDUserSwiftExtensions, HMDAccountMetadata, OS_dispatch_queue, HAPKeyStore;
...
+ (id)fetchSharedUserDataRootForHomeUUID:(id)a0 moc:(id)a1 coreData:(id)a2;
+ (id)fetchSharedUserDataRootForHomeUUID:(id)a0 moc:(id)a1 coreData:(id)a2 ttrManager:(id)a3;
...
- (id)privateZoneName;
- (void)sendDisclosureNotificationWithLocationSharingDevice:(HMDDevice *)a0 completion:(void (^)(void))a1;
...
- (void)_didHandleFMFDeviceChangedNotificationWithStatus:(unsigned long long)a0;
...
- (void)_pushPhotoMetadataToStoreForHome:(id)a0;
...
- (void)_pushTextMetadataToStoreForHome:(id)a0;
...
- (void)handleCompleteDisclosureOfAdaptiveTemperatureAutomationsMessage:(id)a0;
- (void)handleCompleteDisclosureOfAdaptiveTemperatureAutomationsRemoteMessage:(id)a0;
- (void)handleMediaContentProfileAccessControlUpdate:(id)a0;
- (void)handlePrimaryResidentUpdateNotification:(id)a0;
- (void)handleRequestDisclosureNotificationAccountMessage:(id)a0;
...
- (void)setIsDisclosedOnAdaptiveTemperatureAutomations:(long long)a0;
...
- (id)sharedZoneName;
- (BOOL)updateAdaptiveTemperatureAutomationsDisclosureStatus:(long long)a0;
