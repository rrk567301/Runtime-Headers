@class NSMutableArray, NSSet, NSData, __cap_, __end_;
...
@interface MTLGPUDebugDevice : MTLToolsDevice {
    struct MetalBufferHeap { struct mutex { struct _opaque_pthread_mutex_t { long long __sig; char __opaque[56]; } __m_; } _mutex; struct vector<id<MTLBuffer>, std::allocator<id<MTLBuffer>>> { __end_ **__begin_; __cap_ **x0; id *x1; } _buffers; struct vector<unsigned int, std::allocator<unsigned int>> { unsigned int *__begin_; unsigned int *__end_; unsigned int *__cap_; } _freeList; int _currentFreeIndex; unsigned long long _totalMemoryAllocated; unsigned long long _totalMemoryInUse; unsigned long long _bufferLength; MTLGPUDebugDevice *_device; } bufferHeap;
