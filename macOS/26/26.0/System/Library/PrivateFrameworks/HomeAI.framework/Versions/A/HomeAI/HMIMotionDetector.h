...
- (id)calculateMotionDetection:(struct vector<unsigned char, std::allocator<unsigned char>> { char *x0; char *x1; char *x2; })a0 score:(struct vector<float, std::allocator<float>> { float *x0; float *x1; float *x2; })a1 srcFeatureCVPoints:(struct vector<cv::Point_<float>, std::allocator<cv::Point_<float>>> { void *x0; void *x1; void *x2; })a2 dstFeatreCVPoints:(struct vector<cv::Point_<float>, std::allocator<cv::Point_<float>>> { void *x0; void *x1; void *x2; })a3 activityZones:(id)a4 operationMode:(unsigned long long)a5 srcPyramid:(struct vector<cv::Mat, std::allocator<cv::Mat>> { struct Mat *x0; struct Mat *x1; struct Mat *x2; })a6 frameSize:(struct CGSize { double x0; double x1; })a7 brightnessChange:(float)a8;
