...
@interface ADDispartiyToDepthFitEstimator : NSObject {
    struct unique_ptr<DisparityToDepth::DisparityToDepthFitEstimator, std::default_delete<DisparityToDepth::DisparityToDepthFitEstimator>> { struct DisparityToDepthFitEstimator *__ptr_; } _impl;
    unsigned int _updateWorldPointsCounter;
    unsigned int _estimateWithDisparityCounter;
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } _x;
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } _y;
    struct vector<ADDisparityToDepthFitWorldPoint, std::allocator<ADDisparityToDepthFitWorldPoint>> { struct ADDisparityToDepthFitWorldPoint *__begin_; struct ADDisparityToDepthFitWorldPoint *__end_; struct ADDisparityToDepthFitWorldPoint *__cap_; } _worldPoints;
}
