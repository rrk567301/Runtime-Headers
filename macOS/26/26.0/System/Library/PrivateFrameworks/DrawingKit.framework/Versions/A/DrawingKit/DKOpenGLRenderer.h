...
@interface DKOpenGLRenderer : NSObject {
    ...
    struct vector<Vertex, std::allocator<Vertex>> { struct Vertex *__begin_; struct Vertex *__end_; struct Vertex *__cap_; } mContiguousVertexStorage;
    struct vector<VertexGroup, std::allocator<VertexGroup>> { struct VertexGroup *__begin_; struct VertexGroup *__end_; struct VertexGroup *__cap_; } mVertexHistory;
    struct vector<std::pair<float __attribute__((ext_vector_type(2))), float __attribute__((ext_vector_type(2)))>, std::allocator<std::pair<float __attribute__((ext_vector_type(2))), float __attribute__((ext_vector_type(2)))>>> { void *__begin_; void *__end_; void *__cap_; } mNonCollisionSegments;
    void /* unknown type, empty encoding */ U_COLOR;
    struct vector<Page, std::allocator<Page>> { struct Page *__begin_; struct Page *__end_; struct Page *__cap_; } mPages;
...
- (struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })regionWithVertices:(struct vector<Vertex, std::allocator<Vertex>> { struct Vertex *x0; struct Vertex *x1; struct Vertex *x2; })a0 withInflationAmount:(float)a1;
...
- (void)updateVertexControllerWithData:(struct vector<Vertex, std::allocator<Vertex>> { struct Vertex *x0; struct Vertex *x1; struct Vertex *x2; })a0 range:(struct _NSRange { unsigned long long x0; unsigned long long x1; })a1 allowWet:(BOOL)a2;
