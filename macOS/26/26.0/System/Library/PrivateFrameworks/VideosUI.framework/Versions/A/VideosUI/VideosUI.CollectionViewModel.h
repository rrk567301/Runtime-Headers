@interface VideosUI.CollectionViewModel : VideosUI.ViewModel {
    void /* unknown type, empty encoding */ headerViewModel;
    void /* unknown type, empty encoding */ footerViewModel;
    void /* unknown type, empty encoding */ prototypeItemViewModels;
    void /* unknown type, empty encoding */ groups;
    void /* unknown type, empty encoding */ allowsCellExpansion;
    void /* unknown type, empty encoding */ sizeClassCollectionTypes;
    void /* unknown type, empty encoding */ eventMonitorDescriptors;
    void /* unknown type, empty encoding */ controllerRef;
    void /* unknown type, empty encoding */ contextDataRef;
    void /* unknown type, empty encoding */ appendContextDataRef;
    void /* unknown type, empty encoding */ updateRequestor;
    void /* unknown type, empty encoding */ delegate;
    void /* unknown type, empty encoding */ fragmentServiceRequest;
    void /* unknown type, empty encoding */ fragmentServiceRequestCancellable;
    void /* unknown type, empty encoding */ eventMonitorController;
    void /* unknown type, empty encoding */ updatingLockupViewModels;
    void /* unknown type, empty encoding */ $__lazy_storage_$__highlightIndex;
}

@end
