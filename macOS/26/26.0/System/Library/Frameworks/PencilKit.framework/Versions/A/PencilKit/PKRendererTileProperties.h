@interface PKRendererTileProperties : NSObject <NSCopying>

@property (readonly, nonatomic) long long level;
@property (readonly, nonatomic) struct CGPoint { double x; double y; } offset;
@property (readonly, nonatomic) BOOL sixChannelMode;
@property (readonly, nonatomic) BOOL extendedDynamicRange;
@property (readonly, nonatomic) BOOL transparentBlending;
@property (readonly, nonatomic) struct CGAffineTransform { double a; double b; double c; double d; double tx; double ty; } drawingTransform;

- (id)copyWithZone:(struct _NSZone { } *)a0;
- (unsigned long long)hash;
- (BOOL)isEqual:(id)a0;
- (id)initWithLevel:(long long)a0 offset:(struct CGPoint { double x0; double x1; })a1 drawingTransform:(struct CGAffineTransform { double x0; double x1; double x2; double x3; double x4; double x5; })a2 sixChannelMode:(BOOL)a3 transparentBlending:(BOOL)a4 extendedDynamicRange:(BOOL)a5;

@end
