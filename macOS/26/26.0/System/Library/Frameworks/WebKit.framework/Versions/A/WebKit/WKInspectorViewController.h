@class WKInspectorWKWebView, NSString, _WKInspectorConfiguration, WKWebView;
...
@interface WKInspectorViewController : NSObject <WKUIDelegate, WKNavigationDelegate, WKInspectorWKWebViewDelegate> {
    ...
    struct RetainPtr<WKInspectorWKWebView> { WKInspectorWKWebView *m_ptr; } _webView;
    struct WeakObjCPtr<id<WKInspectorViewControllerDelegate>> { id m_weakReference; } _delegate;
    struct RetainPtr<_WKInspectorConfiguration> { _WKInspectorConfiguration *m_ptr; } _configuration;
}
...
- (id)_horizontallyAttachedInspectedWebView;
