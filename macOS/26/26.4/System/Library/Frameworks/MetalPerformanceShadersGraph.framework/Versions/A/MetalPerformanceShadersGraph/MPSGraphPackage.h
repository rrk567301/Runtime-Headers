...
@interface MPSGraphPackage : MPSGraphObject {
    ...
    struct unique_ptr<llvm::LockFileManager, std::default_delete<llvm::LockFileManager>> { struct { struct LockFileManager *__ptr_; } ; } _temporaryPackageFileLock;
    struct unique_ptr<InProcessPackageWritingGuard::LockGuard, std::default_delete<InProcessPackageWritingGuard::LockGuard>> { struct { struct LockGuard *__ptr_; } ; } _writingLockGuard;
}
...
- (id)addBinaryResourceFile:(id)a0 withFileType:(unsigned long long)a1 toBinaryResourceFileDict:(id)a2;
