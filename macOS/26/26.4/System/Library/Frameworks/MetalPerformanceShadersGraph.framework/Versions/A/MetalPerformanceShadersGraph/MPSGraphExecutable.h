...
@interface MPSGraphExecutable : MPSGraphObject {
    ...
    void *_builder;
    struct OriginalModuleRef { struct unique_ptr<BaseModuleRef, std::default_delete<BaseModuleRef>> { struct { struct BaseModuleRef *__ptr_; } ; } _module; } _originalModule;
    void *_symbolTable;
    struct unique_ptr<GPU::detail::ReadDataFromFileCache, std::default_delete<GPU::detail::ReadDataFromFileCache>> { struct { struct ReadDataFromFileCache *__ptr_; } ; } _readDataFromFileCache;
    ...
    BOOL _enable1x1ConvMatMulFusion;
    BOOL _toolsOptimizedExecutableSkipPasses;
    long long _forcePlacementOnDevice;
    struct optional<std::unordered_set<std::string>> { union { char __null_state_; struct unordered_set<std::string, std::hash<std::string>, std::equal_to<std::string>, std::allocator<std::string>> { struct __hash_table<std::string, std::hash<std::string>, std::equal_to<std::string>, std::allocator<std::string>> { struct unique_ptr<std::__hash_node_base<std::__hash_node<std::string, void *> *> *[], std::__bucket_list_deallocator<std::allocator<std::__hash_node_base<std::__hash_node<std::string, void *> *> *>>> { struct { void **__ptr_; struct __bucket_list_deallocator<std::allocator<std::__hash_node_base<std::__hash_node<std::string, void *> *> *>> { struct { unsigned long long __size_; } ; } __deleter_; } ; } __bucket_list_; struct { struct __hash_node_base<std::__hash_node<std::string, void *> *> { void *__next_; } __first_node_; } ; struct { unsigned long long __size_; } ; struct { float __max_load_factor_; } ; } __table_; } __val_; } ; BOOL __engaged_; } _funcNamesInOriginalModule;
    NSMutableSet *_snapshotLocModuleSet;
    struct unique_ptr<mpsgraph::ShapeFunctionRegistry, std::default_delete<mpsgraph::ShapeFunctionRegistry>> { struct { struct ShapeFunctionRegistry *__ptr_; } ; } _shapeFunctionRegistry;
...
+ (void)prepareExecDescriptorAndRuntimeSpecialization:(id)a0 device:(id)a1 executableExecutionDescriptor:(id)a2;
...
- (void)commonPostInit:(struct unique_ptr<InMemoryModuleRef, std::default_delete<InMemoryModuleRef>> { struct { struct InMemoryModuleRef *x0; } x0; })a0;
...
- (id)bytecodeDictionaryFromCacheEntries:(struct SmallVector<MPSGraphExecutableCacheValue, 1U> { void *x0; unsigned int x1; unsigned int x2; char x3[40]; })a0;
- (void)checkCallablesForModule:(void *)a0;
- (BOOL)checkSpecializationValidForSingleEntry;
- (struct OwningOpRef<mlir::ModuleOp> { struct ModuleOp { struct Operation *x0; } x0; })cloneForFeeds:(id)a0 targetTensors:(id)a1 targetOperations:(id)a2 compilationDescriptor:(id)a3;
- (void)commonPreInitWithDescriptor:(id)a0;
- (struct vector<mlir::Type, std::allocator<mlir::Type>> { struct Type *x0; struct Type *x1; struct { struct Type *x0; } x2; })convertMPSGraphShapesToMLIRTypes:(id)a0;
- (struct vector<mlir::Type, std::allocator<mlir::Type>> { struct Type *x0; struct Type *x1; struct { struct Type *x0; } x2; })convertMPSGraphShapesToMLIRTypes:(id)a0 funcOp:(struct FuncOp { struct Operation *x0; })a1 compilationDescriptor:(id)a2;
- (struct Attribute { struct AttributeStorage *x0; })convertShapeRangesToMLIR:(id)a0 feedTensors:(id)a1 context:(void *)a2;
...
- (struct vector<mlir::NamedAttribute, std::allocator<mlir::NamedAttribute>> { struct NamedAttribute *x0; struct NamedAttribute *x1; struct { struct NamedAttribute *x0; } x2; })getAttributesFromDescriptors:(id)a0 context:(void *)a1 deviceDescriptor:(id)a2;
...
- (void *)getRuntimeSpecializationAndEntryFunction:(id)a0 shapedEntryPoint:(id)a1 perEntryPointToSymbolAndFileNameMap:(id)a2 cacheSpecializedModuleLookups:(BOOL)a3 entryFuncOp:(void *)a4 runtime:(struct BaseRuntime **)a5;
...
- (id)initWithMLIRCommon:(struct unique_ptr<llvm::MemoryBuffer, std::default_delete<llvm::MemoryBuffer>> { struct { struct MemoryBuffer *x0; } x0; })a0 executableDescriptor:(id)a1 error:(id *)a2;
...
- (id)optimizedBytecode:(id)a0 entryPoint:(id)a1 compilationDescriptor:(id)a2;
...
- (void)serializeOptimizedBytecode:(struct ModuleOp { struct Operation *x0; })a0 signature:(id)a1 bytecodeDictionary:(id)a2;
...
- (struct MPSGraphExecutableCacheValue { struct MPSGraphExecutableSpecializedModule *x0; struct BaseModuleRef *x1; struct basic_string<char, std::char_traits<char>, std::allocator<char>> { struct { union __rep { struct __short { char x0[23]; unsigned char x1 : 7; unsigned char x2 : 1; } x0; struct __long { char *x0; unsigned long long x1; unsigned long x2 : 63; unsigned char x3 : 1; } x1; } x0; } x0; } x2; })specializeWithDevice:(id)a0 shapedEntryPoint:(id)a1 compilationDescriptor:(id)a2;
