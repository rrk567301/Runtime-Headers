...
@interface VNContoursObservation : VNObservation {
    ...
    struct vector<unsigned int, std::allocator<unsigned int>> { unsigned int *__begin_; unsigned int *__end_; struct { unsigned int *__cap_; } ; } _topLevelContoursIndices;
    struct vector<std::vector<unsigned int>, std::allocator<std::vector<unsigned int>>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _contourChildrenIndices;
}
