@class _VZFramebuffer, NSString, _VZMultiTouchDevice, VZGraphicsDisplay, VZVirtualMachine, _VZVNCSecurityConfiguration, NSObject;
...
@interface _VZVNCServer : NSObject <_VZFramebufferObserver> {
    ...
    struct DispatchQueue { struct OpaqueId *_object; } _serverQueue;
    struct unique_ptr<Vnc::Server, std::default_delete<Vnc::Server>> { struct { struct Server *__ptr_; } ; } _server;
    struct unique_ptr<(anonymous namespace)::ServerDelegate, std::default_delete<(anonymous namespace)::ServerDelegate>> { struct { struct ServerDelegate *__ptr_; } ; } _serverDelegate;
    struct DispatchQueue { struct OpaqueId *_object; } _delegateQueue;
    ...
    BOOL _hdrEnabled;
    unsigned int _bytesPerRowAlignmentRequirement;
}
...
@property (retain) VZGraphicsDisplay *graphicsDisplay;
