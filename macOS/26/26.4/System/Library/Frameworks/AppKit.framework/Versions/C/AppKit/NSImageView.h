@class NSString, NSImage, _NSImageBezelView, NSImageSymbolConfiguration, NSFont, NSColor;

@interface NSImageView : NSControl <_NSAsynchronousPreparationDelegate, NSAccessibilityImage, NSMenuItemValidation> {
    struct { unsigned char _hasImageSubview : 1; unsigned char _usesSubview : 1; unsigned char _hasCachedUsesSubview : 1; unsigned char _isObservingIconAppearance : 1; unsigned int _unused : 21; unsigned char _rejectsMultiFileDrops : 1; unsigned char _compatibleScalingAndAlignment : 1; unsigned char _reserved : 1; unsigned char _overridesDrawing : 1; unsigned char _allowsCutCopyPaste : 1; unsigned char _editable : 1; unsigned char _placeholderPropertyPrecedence : 1; } _ivFlags;
    id _target;
    SEL _action;
}

@property (class) long long defaultPreferredImageDynamicRange;

@property (nonatomic, readonly) Class _intelligenceBaseClass;
@property (nonatomic) _NSImageBezelView *_bezelView;
@property (copy) NSColor *templateTintColor;
@property (nonatomic, setter=_setUsesCachedImage:) BOOL _usesCachedImage;
@property (retain) NSImage *placeholderImage;
@property (copy) NSFont *symbolFont;
@property long long symbolScale;
@property (retain) NSImage *image;
@property (getter=isEditable) BOOL editable;
@property unsigned long long imageAlignment;
@property unsigned long long imageScaling;
@property unsigned long long imageFrameStyle;
@property (copy) NSImageSymbolConfiguration *symbolConfiguration;
@property (copy) NSColor *contentTintColor;
@property BOOL animates;
@property BOOL allowsCutCopyPaste;
@property long long preferredImageDynamicRange;
@property (readonly) long long imageDynamicRange;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

+ (void)initialize;
+ (id)imageViewWithImage:(id)a0;
+ (id)imageWellWithImage:(id)a0 target:(id)a1 action:(SEL)a2;

- (long long)backgroundStyle;
- (BOOL)_contentHuggingDefault_isUsuallyFixedHeight;
- (void)setNeedsDisplayInRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0;
- (void)cut:(id)a0;
- (void)setAction:(SEL)a0;
- (id)menuForEvent:(id)a0;
- (SEL)action;
- (BOOL)respondsToSelector:(SEL)a0;
- (void)_commonInit;
- (void)updateLayer;
- (BOOL)isOpaque;
- (void)removeAllSymbolEffectsWithOptions:(id)a0 animated:(BOOL)a1;
- (void)setLayer:(id)a0;
- (void)paste:(id)a0;
- (void)_intelligenceCollectContentIn:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 collector:(id)a1;
- (void)didChangeValueForKey:(id)a0;
- (void)setObjectValue:(id)a0;
- (struct NSEdgeInsets { double x0; double x1; double x2; double x3; })alignmentRectInsets;
- (void)layout;
- (void)addSymbolEffect:(id)a0 options:(id)a1 animated:(BOOL)a2;
- (void)drawRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0;
- (void)setTarget:(id)a0;
- (void)copy:(id)a0;
- (BOOL)_contentHuggingDefault_isUsuallyFixedWidth;
- (void)keyDown:(id)a0;
- (BOOL)validateMenuItem:(id)a0;
- (void)encodeWithCoder:(id)a0;
- (void)removeSymbolEffectOfType:(id)a0 options:(id)a1 animated:(BOOL)a2;
- (void)setEnabled:(BOOL)a0;
- (id)accessibilityLabel;
- (struct CGSize { double x0; double x1; })intrinsicContentSize;
- (id)init;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0;
- (double)firstBaselineOffsetFromTop;
- (id)initWithCoder:(id)a0;
- (BOOL)allowsVibrancy;
- (void)dealloc;
- (id)target;
- (double)lastBaselineOffsetFromBottom;
- (void)delete:(id)a0;
- (void)draggingExited:(id)a0;
- (id)_vibrancyFilter;
- (void)_addSymbolEffect:(id)a0 options:(id)a1 animated:(BOOL)a2;
- (BOOL)_copyImageToPasteboard;
- (struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })_drawingRectForImage;
- (void)_iconAppearanceConfigurationDidChange:(id)a0;
- (id)_imageSubview;
- (id)_imageSubviewForTesting;
- (void)_invalidateImageViewHierarchy:(BOOL)a0;
- (id)_menuForEvent:(id)a0;
- (BOOL)_rejectsMultiFileDrops;
- (void)_removeAllSymbolEffectsWithOptions:(id)a0 animated:(BOOL)a1;
- (void)_removeSymbolEffectOfType:(id)a0 options:(id)a1 animated:(BOOL)a2;
- (void)_setBackgroundStyleForSubtree:(long long)a0;
- (void)_setImageAndNotifyTarget:(id)a0;
- (void)_setImageSubview:(id)a0;
- (void)_setImageViewHierarchyNeedsDisplay:(BOOL)a0;
- (void)_setImageViewHierarchyNeedsLayout:(BOOL)a0;
- (void)_setPreferredImageDynamicRange:(long long)a0 animated:(BOOL)a1;
- (void)_setRejectsMultiFileDrops:(BOOL)a0;
- (void)_setSidebarTintConfiguration:(id)a0;
- (void)_setSymbolImage:(id)a0 withContentTransition:(id)a1 options:(id)a2;
- (BOOL)_shouldTrackMouseWithEvent:(id)a0;
- (id)_sidebarTintConfiguration;
- (void)_updateBezelView;
- (void)_updateImageSubview;
- (void)_updateImageSubviewIfNecessary;
- (void)_updateUsesSubview;
- (BOOL)_usesSubview;
- (BOOL)_validateMenuItem:(id)a0;
- (void)addSymbolEffect:(id)a0;
- (void)addSymbolEffect:(id)a0 options:(id)a1;
- (id)asynchronousPreparation:(id)a0 prepareResultUsingParameters:(id)a1;
- (void)asynchronousPreparationDidChangePreparedResult:(id)a0;
- (void)awakeFromNib;
- (void)concludeDragOperation:(id)a0;
- (unsigned long long)draggingEntered:(id)a0;
- (BOOL)gestureRecognizer:(id)a0 shouldAttemptToRecognizeWithEvent:(id)a1;
- (id)ns_widgetType;
- (BOOL)performDragOperation:(id)a0;
- (void)registerForDrags;
- (void)removeAllSymbolEffects;
- (void)removeAllSymbolEffectsWithOptions:(id)a0;
- (void)removeSymbolEffectOfType:(id)a0;
- (void)removeSymbolEffectOfType:(id)a0 options:(id)a1;
- (void)setCell:(id)a0;
- (void)setFrameSize:(struct CGSize { double x0; double x1; })a0;
- (void)setSymbolImage:(id)a0 withContentTransition:(id)a1;
- (void)setSymbolImage:(id)a0 withContentTransition:(id)a1 options:(id)a2;
- (BOOL)supportsFauxSolariumControlMetrics;
- (void)toggleAllAnimations:(id)a0;
- (void)toggleAnimation:(id)a0;
- (void)updateCell:(id)a0;
- (BOOL)wantsUpdateLayer;

@end
