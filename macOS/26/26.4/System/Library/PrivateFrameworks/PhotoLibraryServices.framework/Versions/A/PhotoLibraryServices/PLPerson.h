...
@interface PLPerson : PLManagedObject <PLSyncablePerson, PLSearchableManagedObject, PLCloudDeletable, PLFileSystemMetadataPersistence, PLUserFeedbackSupporting, PLGraphNodeActor> {
    ...
    BOOL _insertedPersonSyncWorkItem;
}

@property (class, readonly, copy) NSString *cloudUUIDKeyForDeletion;

@property (readonly, nonatomic) BOOL shouldIndexOnAssetsForSearch;
@property (readonly, nonatomic) BOOL isNotYetEligibleForSearch;
...
+ (void)batchFetchPersonUUIDsByAssetUUIDWithAssetUUIDs:(id)a0 predicate:(id)a1 includedDetectionTypes:(id)a2 includeTemporalDetectedFaces:(BOOL)a3 inManagedObjectContext:(id)a4 completion:(id /* block */)a5;
...
+ (id)_batchFetchPersonUUIDsByAssetUUIDWithAssetUUIDs:(id)a0 predicate:(id)a1 includedDetectionTypes:(id)a2 includeTemporalDetectedFaces:(BOOL)a3 inManagedObjectContext:(id)a4 error:(id *)a5;
...
+ (id)fetchPersonsWithUUIDs:(id)a0 managedObjectContext:(id)a1;
...
- (void)_signalBackgroundPersonNeededForPersonSyncWithJobFlags:(long long)a0;
