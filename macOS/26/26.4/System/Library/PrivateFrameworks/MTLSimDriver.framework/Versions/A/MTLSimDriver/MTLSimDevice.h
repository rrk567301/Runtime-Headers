@class MTLArchitecture, NSString, MTLDeviceFeatureQueries, MTLResourceListPool, NSArray, NSDictionary, NSObject, MTLSerializer, MTLGPUBVHBuilder, MTLTargetDeviceArchitecture;
@protocol OS_xpc_object, OS_dispatch_queue, MTLDeviceSPI;

@interface MTLSimDevice : _MTLDevice <MTLDeviceSPI> {
    struct MTLSimConnection { NSObject<OS_xpc_object> *_connection; NSObject<OS_dispatch_queue> *_serialQueue; } _mainConnection;
    struct MTLSimConnection { NSObject<OS_xpc_object> *_connection; NSObject<OS_dispatch_queue> *_serialQueue; } _resourceConnection;
    struct MTLSimConnection { NSObject<OS_xpc_object> *_connection; NSObject<OS_dispatch_queue> *_serialQueue; } _compilerConnection;
    unsigned long long _processRef;
    unsigned int _deserializerVersion;
    NSObject<OS_dispatch_queue> *_commandBufferQueue;
    struct unordered_map<unsigned int, MTLSimCommandBuffer *, std::hash<unsigned int>, std::equal_to<unsigned int>, std::allocator<std::pair<const unsigned int, MTLSimCommandBuffer *>>> { struct __hash_table<std::__hash_value_type<unsigned int, MTLSimCommandBuffer *>, std::__unordered_map_hasher<unsigned int, std::pair<const unsigned int, MTLSimCommandBuffer *>, std::hash<unsigned int>, std::equal_to<unsigned int>>, std::__unordered_map_equal<unsigned int, std::pair<const unsigned int, MTLSimCommandBuffer *>, std::equal_to<unsigned int>, std::hash<unsigned int>>, std::allocator<std::pair<const unsigned int, MTLSimCommandBuffer *>>> { struct unique_ptr<std::__hash_node_base<std::__hash_node<std::__hash_value_type<unsigned int, MTLSimCommandBuffer *>, void *> *> *[], std::__bucket_list_deallocator<std::allocator<std::__hash_node_base<std::__hash_node<std::__hash_value_type<unsigned int, MTLSimCommandBuffer *>, void *> *> *>>> { struct { void **__ptr_; struct __bucket_list_deallocator<std::allocator<std::__hash_node_base<std::__hash_node<std::__hash_value_type<unsigned int, MTLSimCommandBuffer *>, void *> *> *>> { struct { unsigned long long __size_; } ; } __deleter_; } ; } __bucket_list_; struct { struct __hash_node_base<std::__hash_node<std::__hash_value_type<unsigned int, MTLSimCommandBuffer *>, void *> *> { void *__next_; } __first_node_; } ; struct { unsigned long long __size_; } ; struct { float __max_load_factor_; } ; } __table_; } _commandBuffers;
...
@property (readonly) BOOL deserializerSupportsAsyncWithReply;
...
- (unsigned int)registerCommandBuffer:(id)a0;
- (id)_fixUpSwizzleForTexture:(id)a0 key:(unsigned int)a1;
- (id)commandBufferForReference:(unsigned int)a0;
- (void)dispatchNotificationWithReference:(unsigned int)a0;
- (unsigned int)getNewResourceID;
- (id)newBufferWithLength:(unsigned long long)a0 options:(unsigned long long)a1 pointer:(void *)a2 copyBytes:(BOOL)a3 deallocator:(id /* block */)a4;
- (id)newFunctionVariantWithCompilerOutput:(id)a0 functionType:(unsigned long long)a1;
- (void)registerEventNotification:(unsigned int)a0 listener:(id)a1 event:(id)a2 value:(unsigned long long)a3 block:(id /* block */)a4;
- (void)registerViewTexture:(id)a0 baseTextureView:(id)a1;
...
- (id)wrapComputeState:(id)a0 descriptor:(id)a1;
- (id)wrapRenderState:(id)a0 descriptor:(id)a1;
