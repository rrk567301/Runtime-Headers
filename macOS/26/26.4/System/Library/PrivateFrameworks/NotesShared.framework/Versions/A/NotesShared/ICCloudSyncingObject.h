@class NSURL, NSMutableDictionary, ICAppContext, NSDate, Protocol, NSDictionary, ICTTOrderedSetVersionedDocument, ICInvitation, NSString, NSSet, CKRecord, ICCloudState, CKRecordID, NSManagedObjectID, NSArray, NSData, ICMergeableDictionary, CKShare, NSUUID;
@protocol ICCloudSyncingObjectCryptoStrategy;

@interface ICCloudSyncingObject : NSManagedObject <ICCloudObject, ICLoggable>

@property (class, readonly, nonatomic) NSURL *temporaryAssetDirectoryURL;
...
@property (nonatomic) BOOL needsToFetchUserSpecificRecordAssets;
@property (nonatomic) BOOL needsToUpdateUserSpecificRecordReferenceActions;
@property (nonatomic) BOOL isRecoveringFromTrash;
...
- (void)findAndResaveUserSpecificRecordThrowingReferenceViolationForDeletionWithError:(id)a0;
- (void)fixBrokenReferencesWithError:(id)a0;
- (BOOL)hasAllMandatoryFields;
- (BOOL)hasAssetSignaturesForUserSpecific:(BOOL)a0;
- (BOOL)hasCommonAssetSignatures;
...
- (unsigned long long)numberOfAssetsInTemporaryRecord:(id)a0;
...
- (id)outOfDateAssetSignaturesForUserSpecific:(BOOL)a0;
...
- (id)updateFetchFlagsAndReturnRecordIDsNeedingFetchWithContext:(id)a0 shouldFetchObject:(id /* block */)a1;
