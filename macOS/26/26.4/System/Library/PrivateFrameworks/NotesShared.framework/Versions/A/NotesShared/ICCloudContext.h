@class NSTimer, NSManagedObjectContext, ICCloudOperationObserver, NSString, NSOperationQueue, NSMutableDictionary, NSDictionary, NSMutableSet, CKRecordZoneID, NSObject, ICSelectorDelayer;
@protocol ICCloudContextDelegate, ICCloudAnalyticsDelegate, ICCloudSessionDelegate, OS_dispatch_queue;

@interface ICCloudContext : NSObject <ICStateHandlerProvider>

@property (class, readonly, nonatomic) ICCloudContext *sharedContext;
@property (class, readonly, nonatomic) CKRecordZoneID *notesZoneID;
@property (class, readonly, nonatomic) CKRecordZoneID *metadataZoneID;

@property (retain, nonatomic) NSOperationQueue *operationQueue;
@property (retain, nonatomic) NSObject<OS_dispatch_queue> *processingQueue;
@property (retain, nonatomic) NSManagedObjectContext *processingQueueBackgroundContext;
@property (retain, nonatomic) NSObject<OS_dispatch_queue> *containersCreationQueue;
@property (retain, nonatomic) NSManagedObjectContext *containersCreationQueueBackgroundContext;
@property (retain, nonatomic) NSManagedObjectContext *genericBackgroundContext;
@property (retain, nonatomic) NSMutableSet *objectIDsToRetry;
@property (retain) NSTimer *retryTimer;
@property (retain, nonatomic) NSMutableDictionary *retryCountsByOperationType;
@property (nonatomic) long long accountStatus;
@property (getter=isDisabledInternal) BOOL disabledInternal;
@property (readonly, nonatomic) NSDictionary *cloudObjectClassesByRecordType;
@property (nonatomic) BOOL needsToProcessAllObjects;
@property (retain, nonatomic) NSMutableSet *objectIDsToProcess;
@property (retain) ICSelectorDelayer *processingSelectorDelayer;
@property (nonatomic) BOOL didAddObservers;
@property (nonatomic, getter=isFetchingEnabled) BOOL fetchingEnabled;
@property (nonatomic) BOOL syncDisabledByServer;
@property (retain, nonatomic) NSDictionary *containersByAccountID;
@property (retain, nonatomic) NSMutableDictionary *accountZoneIDsNeedingFetchChanges;
@property (retain, nonatomic) NSMutableDictionary *accountZoneIDsFetchingChanges;
@property (retain, nonatomic) NSMutableDictionary *accountZoneIDsNeedingToBeSaved;
@property (retain) NSMutableSet *subscribedSubscriptionIDs;
@property (nonatomic) BOOL didEnqueueLongLivedOperations;
@property (nonatomic, getter=isEnqueueingLongLivedOperations) BOOL enqueueingLongLivedOperations;
@property (retain, nonatomic) NSMutableSet *operationIDsBeforeEnqueuingLongLivedOperations;
@property (retain, nonatomic) ICCloudOperationObserver *operationObserver;
@property (retain, nonatomic) NSMutableSet *recordIdsToShowQuotaExceededErrorOnTrashRecoveryAlert;
@property (nonatomic) BOOL isDaemonProcess;
@property (weak, nonatomic) id<ICCloudContextDelegate> cloudContextDelegate;
@property (weak, nonatomic) id<ICCloudAnalyticsDelegate> cloudAnalyticsDelegate;
@property (weak, nonatomic) id<ICCloudSessionDelegate> cloudSessionDelegate;
@property (readonly, nonatomic) BOOL fetchOperationsPending;
@property BOOL needsToUpdateSubscriptions;
@property (nonatomic) BOOL shouldResumeSyncOnForeground;
@property (readonly, nonatomic, getter=hasCompletedInitialSync) BOOL completedInitialSync;
@property (nonatomic) long long qualityOfService;
@property (nonatomic) unsigned long long discretionaryNetworkBehavior;
@property (nonatomic) BOOL enableLongLivedOperations;
@property (nonatomic) BOOL disableAutomaticallyRetryNetworkFailures;
@property (nonatomic) BOOL disableRetryTimer;
@property (nonatomic) BOOL syncOnlyIfReachable;
@property (getter=isDisabled) BOOL disabled;
@property (readonly, nonatomic) BOOL hasPendingOperations;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

+ (id)zoneIDsFromZoneInfos:(id)a0;
+ (id)errorCodesToIgnoreForBackoffTimer;
+ (id)zoneInfosFromZoneIDs:(id)a0;
+ (id)allZoneIDsInAccountZoneIDs:(id)a0;
+ (void)batchRecordsToSave:(id)a0 delete:(id)a1 maxRecordCountPerBatch:(unsigned long long)a2 maxRecordSizePerBatch:(unsigned long long)a3 withBlock:(id /* block */)a4;
+ (BOOL)isZoneConfigurations:(id)a0 subsetOfZoneConfigurations:(id)a1;
+ (id)newNotesContainer;
+ (id)errorsFromError:(id)a0;
+ (id)errorForDisabledCloudSyncing;
+ (id)sortedRecords:(id)a0;
+ (id)objectsByDatabaseScope:(id)a0;
+ (id)errorFromErrors:(id)a0;
+ (id)errorFromOperations:(id)a0;
+ (BOOL)haveZoneIDsInAccountZoneIDs:(id)a0;
+ (id)objectsByAccount:(id)a0;
+ (id)errorForWaitingForRetryTimer;
+ (id)userRecordNameForContainer:(id)a0;
+ (BOOL)shouldIgnoreErrorForBackoffTimer:(id)a0;
+ (id)newNotesContainerForAccountID:(id)a0;
+ (id)deduplicatedRecordsForCloudObjects:(id)a0;
+ (void)registerStateHandler;

- (void)updateConfiguration:(id)a0;
- (id)allCloudObjectIDsInContext:(id)a0;
- (void)fetchRecordZoneChangesOperation:(id)a0 zoneID:(id)a1 accountID:(id)a2 changeTokenUpdated:(id)a3 context:(id)a4 dispatchGroup:(id)a5;
- (void)pauseCloudSyncWhileSynchronouslyPerformingBlock:(id /* block */)a0;
- (id)allCloudObjectIDsInContext:(id)a0 predicate:(id)a1;
- (void)addDependenciesForModifyRecordsOperation:(id)a0;
- (void)fetchDatabaseChangesOperation:(id)a0 recordZoneWithIDChanged:(id)a1 accountID:(id)a2 session:(id)a3;
- (id)_objectIDsToProcessForFullSyncInContext:(id)a0;
- (id)serverChangeTokenForRecordZoneID:(id)a0 databaseScope:(long long)a1 accountID:(id)a2 context:(id)a3;
- (void)enqueueLongLivedOperationsIfNeededWithCompletionHandler:(id /* block */)a0;
- (id)operationsToModifyRecordsToSave:(id)a0 delete:(id)a1 forSession:(id)a2 rootRecordIDsByShareID:(id)a3 database:(id)a4;
- (void)fetchDatabaseChangesOperation:(id)a0 changeTokenUpdated:(id)a1 accountID:(id)a2 context:(id)a3;
- (id)initWithDelegate:(id)a0;
- (void)clearRetryCountForOperationType:(id)a0;
- (id)operationToSaveZonesForZoneIDs:(id)a0 accountID:(id)a1;
- (void)processPendingCloudObjectsInSession:(id)a0 withCompletionHandler:(id /* block */)a1;
- (void)receivedZoneNotFound:(id)a0 operation:(id)a1 context:(id)a2;
- (void)addModifyRecordsOperationsWithCloudObjectsToSave:(id)a0 delete:(id)a1 accountID:(id)a2 forSession:(id)a3 operationGroupName:(id)a4 waitForDependencies:(BOOL)a5 completionHandler:(id /* block */)a6;
- (void)incrementOrClearRetryCountForOperationType:(id)a0 error:(id)a1;
- (void)deleteRecordZonesWithZoneIDs:(id)a0 accountID:(id)a1 markZonesAsUserPurged:(BOOL)a2 completionHandler:(id /* block */)a3;
- (void)handleCloudKitNotification:(id)a0 completionHandler:(id /* block */)a1;
- (void)fetchDatabaseChangesOperation:(id)a0 finishedWithServerChangeToken:(id)a1 accountID:(id)a2 session:(id)a3 error:(id)a4 completionHandler:(id /* block */)a5 context:(id)a6;
- (void)deleteServerChangeTokenForRecordZoneID:(id)a0 databaseScope:(long long)a1 accountID:(id)a2 context:(id)a3;
- (id)backgroundContext;
- (id)operationsToFetchRecordIDs:(id)a0 forSession:(id)a1 shouldDownloadAssets:(BOOL)a2 database:(id)a3 qualityOfService:(long long)a4;
- (void)addFetchOperationsForRecordIDs:(id)a0 forSession:(id)a1 accountID:(id)a2 qualityOfService:(long long)a3 operationGroupName:(id)a4 completionHandler:(id /* block */)a5;
- (void)saveSubscriptionsForDatabase:(id)a0 completionHandler:(id /* block */)a1;
- (void)enqueueLongLivedOperationsWithIDsIfNeeded:(id)a0 container:(id)a1 completionHandler:(id /* block */)a2;
- (void)addOperations:(id)a0;
- (id)readinessLoggingDescription;
- (void)_filterCloudSyncingObjects:(id)a0 accountID:(id)a1 objectsToSave:(id *)a2 objectsToDelete:(id *)a3 objectsToDeleteShares:(id *)a4 objectsToSaveUserSpecificRecord:(id *)a5;
- (id)operationsToFetchRecordIDs:(id)a0 forSession:(id)a1 shouldDownloadAssets:(BOOL)a2 qualityOfService:(long long)a3 operationGroupName:(id)a4 accountID:(id)a5;
- (void)addOperationsToFetchRecordZoneChangesForAccountZoneIDs:(id)a0 session:(id)a1 dispatchGroup:(id)a2 completionHandler:(id /* block */)a3;
- (void)fetchRecordZoneChangesOperation:(id)a0 completedFetchForZoneID:(id)a1 serverChangeToken:(id)a2 error:(id)a3 context:(id)a4 dispatchGroup:(id)a5;
- (id)persistentStoreCoordinator;
- (void)contextDidSaveOrMerge:(id)a0;
- (id)operationToFetchRecordZoneChangesForZoneIDs:(id)a0 database:(id)a1 session:(id)a2 dispatchGroup:(id)a3;
- (void)observeValueForKeyPath:(id)a0 ofObject:(id)a1 change:(id)a2 context:(void *)a3;
- (void)updateCloudContextState;
- (void)_processCloudObjects:(id)a0 inSession:(id)a1 completionHandler:(id /* block */)a2;
- (void)syncWithReason:(id)a0 completionHandler:(id /* block */)a1;
- (id)viewContext;
- (void)fetchRecordZoneChangesForZoneIDs:(id)a0 accountID:(id)a1 reason:(id)a2 completionHandler:(id /* block */)a3;
- (void)deleteServerChangeTokenForChangedZonesInDatabase:(id)a0 accountID:(id)a1 context:(id)a2;
- (void)validateAccountZoneIDsNeedingFetchChanges;
- (void)fetchRecordZoneChangesOperationDidComplete:(id)a0 session:(id)a1 error:(id)a2 dispatchGroup:(id)a3;
- (id)existingCloudObjectForRecordID:(id)a0 recordType:(id)a1 accountID:(id)a2 context:(id)a3 excludingRecordTypes:(id)a4;
- (void)cancelEverythingWithCompletionHandler:(id /* block */)a0;
- (id)operationToFetchRecordIDs:(id)a0 forSession:(id)a1 shouldDownloadAssets:(BOOL)a2 database:(id)a3 qualityOfService:(long long)a4;
- (void)modifyRecordsOperation:(id)a0 recordWasSavedWithRecordID:(id)a1 record:(id)a2 session:(id)a3 error:(id)a4;
- (void)fetchSubscriptionsForDatabase:(id)a0 completionHandler:(id /* block */)a1;
- (BOOL)isReadyToSync;
- (void)fetchRecordZoneChangesForAccountZoneIDs:(id)a0 session:(id)a1 dispatchGroup:(id)a2 completionHandler:(id /* block */)a3;
- (id)subscriptionForDatabase:(id)a0;
- (void)enumerateAllCloudObjectsInContext:(id)a0 batchSize:(unsigned long long)a1 saveAfterBatch:(BOOL)a2 usingBlock:(id /* block */)a3;
- (void)deleteSharesForObjects:(id)a0 completionHandler:(id /* block */)a1;
- (void)modifyRecordsOperation:(id)a0 recordWasSavedWithRecordID:(id)a1 record:(id)a2 session:(id)a3 error:(id)a4 context:(id)a5;
- (void)printOperationQueue;
- (id)newPlaceholderObjectForRecordID:(id)a0 recordType:(id)a1 accountID:(id)a2 context:(id)a3;
- (void)modifyRecordsOperation:(id)a0 forSession:(id)a1 didCompleteWithError:(id)a2;
- (double)timeIntervalToRetryAfterFromError:(id)a0;
- (id)accountIDForContainer:(id)a0;
- (id)operationToFetchDatabaseChangesForDatabase:(id)a0 session:(id)a1 completionHandler:(id /* block */)a2;
- (void)informCloudAnalyticsDelegateForOperationDidEnd:(id)a0 recordsByRecordID:(id)a1 operationError:(id)a2;
- (void)pushCloudObjects:(id)a0 operationGroupName:(id)a1 completionHandler:(id /* block */)a2;
- (void)cloudKitAccountChanged:(id)a0;
- (id)newSessionForReason:(id)a0;
- (BOOL)isInForeground;
- (void)fetchCloudObjects:(id)a0 accountID:(id)a1 completionHandler:(id /* block */)a2;
- (void)startRetryTimerIfNecessaryWithError:(id)a0;
- (void)_ingestCloudKitRecord:(id)a0 forAccountID:(id)a1 forceMerge:(BOOL)a2 session:(id)a3 context:(id)a4;
- (void)setAssetDownloadStateOnObject:(id)a0 withRecord:(id)a1 managedObjectContext:(id)a2;
- (void)clearSubscribedSubscriptionIDs;
- (void)addFetchOperationsForRecordIDs:(id)a0 forSession:(id)a1 accountID:(id)a2 qualityOfService:(long long)a3 operationGroupName:(id)a4 shouldDownloadAssets:(BOOL)a5 completionHandler:(id /* block */)a6;
- (BOOL)isCloudKitAccountAvailable;
- (id)existingCloudObjectForRecordID:(id)a0 recordType:(id)a1 accountID:(id)a2 context:(id)a3;
- (void)_processPendingCloudObjectsInSession:(id)a0 withCompletionHandler:(id /* block */)a1;
- (void)saveServerChangeToken:(id)a0 forChangedZonesInDatabase:(id)a1 accountID:(id)a2 context:(id)a3;
- (BOOL)useDispatchGroupForDaemonProcessWithGroup:(id)a0;
- (void)fetchAssetsOnDemandIfEnabledForNoteObjectID:(id)a0;
- (id)existingCloudObjectForUserSpecificRecordID:(id)a0 createPlaceholderIfNecessary:(BOOL)a1 accountID:(id)a2 context:(id)a3;
- (BOOL)hasNoteContextOptions:(unsigned long long)a0;
- (void).cxx_destruct;
- (void)finishOperationsForRecordID:(id)a0 qualityOfService:(long long)a1 completionHandler:(id /* block */)a2;
- (void)updateAccountStatusWithCompletionHandler:(id /* block */)a0;
- (void)setCompletedInitialSync:(BOOL)a0;
- (void)fetchUserRecordWithAccountID:(id)a0 completionHandler:(id /* block */)a1;
- (void)fetchOperation:(id)a0 progressChangedWithRecordID:(id)a1 progress:(double)a2;
- (BOOL)shouldAlertOnQuotaExceededErrorFromTrashRecoveryWithObject:(id)a0 error:(id)a1;
- (void)saveServerChangeToken:(id)a0 forRecordZoneID:(id)a1 databaseScope:(long long)a2 accountID:(id)a3 context:(id)a4;
- (void)processAllCloudObjectsInSession:(id)a0 withCompletionHandler:(id /* block */)a1;
- (void)ingestCloudKitRecord:(id)a0 forAccountID:(id)a1 forceMerge:(BOOL)a2 context:(id)a3;
- (void)incrementRetryCountForOperationType:(id)a0;
- (void)fetchRecordZoneChangesForZoneIDs:(id)a0 accountID:(id)a1 session:(id)a2 completionHandler:(id /* block */)a3;
- (void)fetchOperation:(id)a0 didCompleteWithRecordsByRecordID:(id)a1 session:(id)a2 error:(id)a3;
- (void)syncWithReason:(id)a0 uploadUnsyncedChanges:(BOOL)a1 completionHandler:(id /* block */)a2;
- (void)deleteAllServerChangeTokens;
- (void)enterDispatchGroupIfNeededWithGroup:(id)a0;
- (void)syncForSession:(id)a0 completedWithErrors:(id)a1;
- (void)modifyRecordsOperation:(id)a0 recordWasDeletedWithRecordID:(id)a1 rootRecordIDsByShareID:(id)a2 session:(id)a3 error:(id)a4 context:(id)a5;
- (void)clearZoneFetchState;
- (BOOL)supportsDeferredAssetDownload;
- (void)didFetchShare:(id)a0 accountID:(id)a1 context:(id)a2;
- (void)fetchDatabaseChangesForDatabases:(id)a0 session:(id)a1 completionHandler:(id /* block */)a2;
- (void)fetchOperation:(id)a0 recordWasFetchedWithRecordID:(id)a1 record:(id)a2 session:(id)a3 error:(id)a4;
- (id)operationsToFetchRecordZoneChangesForZoneIDs:(id)a0 accountID:(id)a1 session:(id)a2 dispatchGroup:(id)a3;
- (id /* block */)completionEndingProcessingSession:(id)a0 wrappingCompletion:(id /* block */)a1;
- (void)finishOperationsForRecordID:(id)a0 completionHandler:(id /* block */)a1;
- (void)fetchRecordIDs:(id)a0 forSession:(id)a1 accountID:(id)a2 operationGroupName:(id)a3 completionHandler:(id /* block */)a4;
- (void)retryOperationsIfNecessary;
- (BOOL)shouldUpdateServerRecordForObject:(id)a0 withRecord:(id)a1;
- (id)accountIDForDatabase:(id)a0;
- (void)processAllCloudObjectsWithCompletionHandler:(id /* block */)a0;
- (void)deleteSharesForObjects:(id)a0 forSession:(id)a1 accountID:(id)a2 completionHandler:(id /* block */)a3;
- (void)_addModifyRecordsOperationsWithCloudObjectsToSave:(id)a0 delete:(id)a1 accountID:(id)a2 forSession:(id)a3 operationGroupName:(id)a4 waitForDependencies:(BOOL)a5 completionHandler:(id /* block */)a6;
- (void)processObjectIDs:(id)a0 inSession:(id)a1 completionHandler:(id /* block */)a2;
- (void)processPendingCloudObjectsWithCompletionHandler:(id /* block */)a0;
- (void)fetchRecordZoneChangesOperation:(id)a0 recordWasChangedWithRecordID:(id)a1 record:(id)a2 error:(id)a3 session:(id)a4 context:(id)a5 dispatchGroup:(id)a6;
- (void)modifyRecordsOperation:(id)a0 recordWasDeletedWithRecordID:(id)a1 rootRecordIDsByShareID:(id)a2 session:(id)a3 error:(id)a4;
- (void)fetchDatabaseChangesOperation:(id)a0 recordZoneWithIDWasDeleted:(id)a1 accountID:(id)a2 session:(id)a3;
- (BOOL)isFetchingAllRecordZones;
- (id)invernessClientForAccountID:(id)a0;
- (id)containerForAccountID:(id)a0;
- (void)clearPendingActivity;
- (void)reachabilityChanged:(id)a0;
- (void)addCallbackBlocksToModifyRecordsOperation:(id)a0 rootRecordIDsByShareID:(id)a1 session:(id)a2;
- (void)fetchUserRecordWithContainer:(id)a0 completionHandler:(id /* block */)a1;
- (void)fetchCloudObjects:(id)a0 forSession:(id)a1 accountID:(id)a2 completionHandler:(id /* block */)a3;
- (id)operationsToModifyRecordsForCloudObjectsToSave:(id)a0 delete:(id)a1 deleteShares:(id)a2 saveUserSpecificRecords:(id)a3 forSession:(id)a4 operationGroupName:(id)a5 addDependencies:(BOOL)a6 accountID:(id)a7;
- (void)_syncWithReason:(id)a0 uploadUnsyncedChanges:(BOOL)a1 completionHandler:(id /* block */)a2;
- (void)fetchRecordZoneChangesOperation:(id)a0 recordWasDeletedWithRecordID:(id)a1 recordType:(id)a2 session:(id)a3 context:(id)a4 dispatchGroup:(id)a5;
- (id)newCloudObjectForRecord:(id)a0 accountID:(id)a1 context:(id)a2;
- (BOOL)shouldSimulateQuotaExceededOnTrashRecovery;
- (void)clearContainers;
- (void)handleGenericPartialFailuresForError:(id)a0 operation:(id)a1;
- (void)updateSubscriptionsWithCompletionHandler:(id /* block */)a0;
- (void)processPendingCloudObjects;
- (void)fetchRecordZoneChangesWithReason:(id)a0 completionHandler:(id /* block */)a1;
- (id)serverChangeTokenForChangedZonesInDatabase:(id)a0 accountID:(id)a1 context:(id)a2;
- (id)allZoneIDs;
- (id)newOperationToFetchRecordZoneChangesWithZoneConfigurations:(id)a0 database:(id)a1 session:(id)a2 dispatchGroup:(id)a3;
- (void)fetchRecordIDs:(id)a0 accountID:(id)a1 operationGroupName:(id)a2 completionHandler:(id /* block */)a3;
- (void)addOperationToProcessObjectsInSession:(id)a0 withCompletionHandler:(id /* block */)a1;
- (void)updateSelectorDelayers:(id)a0;
- (void)disableCloudSyncingIfCurrentVersionNotSupported:(id)a0;
- (id)operationToModifyRecordsToSave:(id)a0 delete:(id)a1 forSession:(id)a2 rootRecordIDsByShareID:(id)a3 database:(id)a4;
- (void)fetchDatabaseChangesForSession:(id)a0 completionHandler:(id /* block */)a1;
- (void)loadZoneFetchState;
- (void)fetchRecordZoneChangesForSession:(id)a0 dispatchGroup:(id)a1 completionHandler:(id /* block */)a2;
- (BOOL)isInternetReachable;
- (void)handleRemoteNotificationWithUserInfo:(id)a0 completion:(id /* block */)a1;
- (void)dealloc;
- (BOOL)partialError:(id)a0 containsErrorCode:(long long)a1;
- (void)saveZoneFetchState;
- (void)resetZoneForCloudAccount:(id)a0 withReason:(id)a1;
- (void)leaveDispatchGroupIfNeededWithGroup:(id)a0;
- (id)existingCloudObjectForRecord:(id)a0 accountID:(id)a1 context:(id)a2;
- (void)updateCloudContextStateWithCompletion:(id /* block */)a0;
- (id)overrideAccountID;
- (id)operationToSaveZonesIfNecessaryForAccountID:(id)a0;

@end
