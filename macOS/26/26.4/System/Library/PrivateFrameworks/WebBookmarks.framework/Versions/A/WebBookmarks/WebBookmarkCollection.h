@class NSURL, SafariFetcherServerProxy, NSMutableArray, WebBookmark, WebBookmarkList, WebBookmarkChangeSet, NSString, WBDatabaseLockAcquisitor, WebBookmarkTitleWordTokenizer, NSArray, NSData, NSNumber, WBCollectionConfiguration, WBBookmarkDatabaseSyncData;
...
@interface WebBookmarkCollection : NSObject <WBDatabaseLockAcquisitorDelegate, WBBookmarkProvider> {
    ...
    struct unique_ptr<WebBookmarks::BookmarkSQLWriteTransaction, std::default_delete<WebBookmarks::BookmarkSQLWriteTransaction>> { struct { struct BookmarkSQLWriteTransaction *__ptr_; } ; } _currentTransaction;
...
@property (readonly, nonatomic) BOOL userHasSharedTabGroups;
...
+ (id)deviceIdentifierManagerForCloudKitWithCollectionType:(long long)a0 generateIfNeeded:(BOOL)a1;
...
+ (id)_deviceIdentifierManagerForCloudKitWithCollectionType:(long long)a0;
+ (BOOL)_isLockedSync;
+ (BOOL)_lockSync;
...
- (BOOL)_setDouble:(double)a0 forDatabaseProperty:(id)a1;
...
- (unsigned long long)_migrateReadingListMarkedFromBookmarksFolder:(id)a0 excludingFolderID:(int)a1;
- (id)_rootFolderHiddenChildrenClause;
- (BOOL)_setString:(id)a0 forDatabaseProperty:(id)a1;
- (BOOL)deleteBookmark:(id)a0;
- (id)folderContents:(int)a0;
...
- (id)_archivedDataWithRootObject:(id)a0;
...
- (BOOL)_boolForDatabaseProperty:(id)a0;
...
- (void)_clearAndCreateSessionStateTable;
...
- (id)_dataForDatabaseProperty:(id)a0;
- (struct BookmarkSQLStatement { void /* function */ **x0; struct sqlite3_stmt *x1; BOOL x2; BOOL x3; int x4; })_databasePropertySelectStatementForKey:(id)a0;
- (struct BookmarkSQLStatement { void /* function */ **x0; struct sqlite3_stmt *x1; BOOL x2; BOOL x3; int x4; })_databasePropertyUpdateStatementForKey:(id)a0;
...
- (BOOL)_deleteBookmarks:(id)a0 leaveTombstone:(BOOL)a1 cleanBookmarks:(BOOL)a2;
...
- (void)_deviceIdentifierDidChange:(id)a0;
- (double)_doubleForDatabaseProperty:(id)a0;
...
- (void)_enumerateContentsOfBookmarkFolder:(int)a0 includingSubfolders:(BOOL)a1 includeHidden:(BOOL)a2 usingBlock:(id /* block */)a3;
...
- (int)_favoritesFolderIdentifier;
- (id)_fetchBookmarkIDsInFolder:(int)a0 onlyFolders:(BOOL)a1 includingHiddenBookmarks:(BOOL)a2 sortedByOrderIndex:(BOOL)a3;
...
- (void)_insertMigratedRowToSessionStateTableForDataKey:(id)a0 localAttributes:(id)a1 uuidColumnName:(id)a2 bookmarkID:(int)a3;
- (BOOL)_insertTombsonesRecursivelyForRootFolder:(id)a0 insertTombstoneBlock:(id /* block */)a1;
- (BOOL)_insertTombstoneWithServerID:(id)a0 itemType:(long long)a1 subtype:(long long)a2 orderIndex:(long long)a3 syncData:(id)a4;
- (int)_intFromExecutingSQL:(id)a0;
- (long long)_integerForDatabaseProperty:(id)a0;
- (BOOL)_isBookmarkMarkedAsReadingListItem:(id)a0;
- (BOOL)_isDatabaseWriteAllowed;
- (BOOL)_isSafariVersionAffected;
- (BOOL)_isTombstoneWithServerID:(id)a0;
- (BOOL)_isVersionInTargetRange:(id)a0;
...
- (BOOL)_removeValuesForKeyPrefix:(id)a0;
...
- (BOOL)_setBool:(BOOL)a0 forDatabaseProperty:(id)a1;
- (BOOL)_setData:(id)a0 forDatabaseProperty:(id)a1;
- (BOOL)_setInteger:(long long)a0 forDatabaseProperty:(id)a1;
...
- (id)_stringForDatabaseProperty:(id)a0;
...
- (BOOL)_updateSessionStateDataUUID:(id)a0 forBookmarks:(id)a1 dataType:(long long)a2;
- (BOOL)_updateSessionStateDataUUID:(id)a0 forBookmarks:(id)a1 startIndex:(unsigned long long)a2 batchSize:(unsigned long long)a3 dataType:(long long)a4;
- (void)_updateSyncNotificationType;
- (BOOL)_usesCloudKit;
- (BOOL)_verifyAllTablesExist:(int *)a0;
- (id)_whereClauseForArchiveMode:(long long)a0;
- (id)_whereClauseForArchiveMode:(long long)a0 automaticArchivingEnabled:(BOOL)a1;
...
- (void)enumerateDescendantsOfBookmarkID:(int)a0 includeHidden:(BOOL)a1 usingBlock:(id /* block */)a2;
- (void)enumerateDescendantsOfBookmarkID:(int)a0 usingBlock:(id /* block */)a1;
- (unsigned int)expectedOrderIndexOfSpecialID:(int)a0;
...
- (void)performDeduplicationWithMode:(long long)a0;
