...
@interface VCSessionParticipantRemote : VCSessionParticipant <VCSessionDownlinkBandwidthAllocatorClient> {
    ...
    BOOL _isLowTierFRCEnabled;
    BOOL _shouldDisableU14xScaler;
    BOOL _useCameraAsConnectionTimingSource;
    BOOL _forceAudioAsConnectionTimingSource;
    BOOL _isConnectionTimingSourceDetected;
    BOOL _isConnectionTimingReported;
    int _remoteDeviceOrientation;
    BOOL _isJBLatencySensitiveModeEnabled;
    BOOL _isPlaybackSynchronizationGroupMember;
}
...
@property (nonatomic) struct _VCSessionParticipantCapabilities { BOOL isPeace; unsigned int participantType; int controlChannelVersion; BOOL supportsGFTSwitchToOneToOne; BOOL supportsParticipantIDBasedKeyLookup; BOOL isDawnburstOrLater; BOOL isLuckierOrLater; } capabilities;
...
+ (unsigned int)maxBdatQualityIndexForVideoQuality:(unsigned char)a0 isLocalOnWiFi:(BOOL)a1;
+ (unsigned int)maxCameraQualityIndexForVideoQuality:(unsigned char)a0 isLocalOnWiFi:(BOOL)a1 isRedundancyRequested:(BOOL)a2 isSharingEnabled:(BOOL)a3 enableNewTier:(BOOL)a4;
+ (unsigned int)maxCameraQualityIndexForVideoQuality:(unsigned char)a0 useWiFiTable:(BOOL)a1 isSharingEnabled:(BOOL)a2 enableNewTier:(BOOL)a3;
+ (unsigned int)maxCameraQualityIndexWithRedundancyForVideoQuality:(unsigned char)a0 useWiFiTable:(BOOL)a1 enableNewTier:(BOOL)a2;
+ (unsigned int)maxFtxtQualityIndexForVideoQuality:(unsigned char)a0 isLocalOnWiFi:(BOOL)a1 isRedundancyRequested:(BOOL)a2 isSharingEnabled:(BOOL)a3;
+ (unsigned int)maxQualityIndexForStreamGroupID:(unsigned int)a0 videoQuality:(unsigned char)a1 isLocalOnWiFi:(BOOL)a2 isRedundancyRequested:(BOOL)a3 isSharingEnabled:(BOOL)a4 enableNewTier:(BOOL)a5;
+ (unsigned int)maxWebRTCCameraQualityIndexForVideoQuality:(unsigned char)a0 isLocalOnWiFi:(BOOL)a1 isSharingEnabled:(BOOL)a2 enableNewTier:(BOOL)a3;
...
- (void)updateAndHandleVideoMediaStall:(BOOL)a0;
...
- (void)applyPositionalInfoPendingChangesWithShouldReapply:(BOOL)a0;
...
- (void)dispatchedReportConnectionTimingWithStreamGroup:(id)a0;
- (void)dispatchedSetPlaybackSynchronizationGroupMember:(BOOL)a0;
...
- (BOOL)isMediaTypeExpected:(unsigned int)a0;
...
- (void)reconfigureJitterBufferMode:(int)a0;
...
- (void)setForceAudioAsConnectionTimingSource:(BOOL)a0;
- (void)setLocalCameraCaptureActive:(BOOL)a0;
...
- (BOOL)shouldDisableMLByDeviceType:(unsigned int)a0;
- (BOOL)shouldDisableMLByUIState:(unsigned int)a0;
- (BOOL)shouldDisableMLEnhanceCommonFactors;
- (BOOL)shouldEnableMLEnhance:(unsigned int)a0;
- (BOOL)shouldEnableMLEnhanceLowTier;
- (BOOL)shouldEnableMLEnhanceMultiway:(unsigned int)a0;
- (BOOL)shouldEnablePacketRetransmissionForStreamGroupID:(unsigned int)a0;
- (BOOL)shouldReportConnectionTimingWithStreamGroup:(id)a0;
- (BOOL)shouldSetupStreamGroupWithID:(unsigned int)a0;
- (BOOL)shouldUseCameraAsConnectionTimingSource;
- (int)spatialChannelIndex:(unsigned int *)a0 forMediaType:(unsigned int)a1;
...
- (void)updatePositionalInfoWithData:(id)a0 shouldReapply:(BOOL)a1;
- (void)updateRemoteDeviceOrientation:(int)a0;
