@class VCRateSharingGroup, NSObject, AVCBasebandCongestionDetector, VCRateControlMediaController, VCRateControlServerBag, VCRateControlProbingSequenceController, AVCRateControlFeedbackController, NSString, VCExperimentManager, AVCStatisticsCollector, VCConnection, VCNWConnectionCongestionDetector, VCRateControlTrafficMetricCollector;
...
@interface AVCRateController : NSObject <VCRateSharingClient> {
    ...
    BOOL _lowBandwidthOptimizationEnabled;
    BOOL _isUnconditionalNetworkStatisticsReportingEnabled;
}
...
+ (unsigned char)rateControlTrafficTypeForScreen:(BOOL)a0 isPersona:(BOOL)a1;

- (BOOL)stop;
- (void)configure:(struct AVCRateControlConfig { unsigned int x0; unsigned int x1; unsigned int x2; unsigned int x3; unsigned int x4; unsigned int x5; BOOL x6; BOOL x7; struct tagVCRateControlAlgorithmConfig *x8; unsigned int x9; })a0;
- (BOOL)start;
- (void)dealloc;
- (id)initWithDelegate:(id)a0 dumpID:(id)a1 isUplink:(BOOL)a2 isForSimulation:(BOOL)a3 useExternalThread:(BOOL)a4 rateSharingGroupToken:(unsigned int)a5;
...
- (void)configureTrafficType:(unsigned char)a0 inRateControlConfig:(struct AVCRateControlConfig { unsigned int x0; unsigned int x1; unsigned int x2; unsigned int x3; unsigned int x4; unsigned int x5; BOOL x6; BOOL x7; struct tagVCRateControlAlgorithmConfig *x8; unsigned int x9; } *)a1;
- (void)configureWithOperatingMode:(int)a0 isLocalCellular:(BOOL)a1 localCellTech:(int)a2 isRemoteCellular:(BOOL)a3 remoteCellTech:(int)a4 bitrateCapKbps:(unsigned int)a5 trafficType:(unsigned char)a6 featureFlags:(unsigned int)a7;
...
- (id)initWithDelegate:(id)a0 params:(const struct tagVCRateControlParams { id x0; BOOL x1; BOOL x2; id x3; id x4; BOOL x5; id x6; BOOL x7; id x8; unsigned int x9; id x10; id x11; } *)a1;
...
- (id)sharingGroupFromToken:(unsigned int)a0;
