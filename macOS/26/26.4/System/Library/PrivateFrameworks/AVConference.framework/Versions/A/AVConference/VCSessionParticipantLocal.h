@class NSString, NSArray, VCSystemAudioCaptureSession, VCSessionMediaStreamPresenceConfigurationProvider, NSMutableDictionary, VCVideoRule, VCRedundancyControllerAudio, NSMutableSet, VCAudioPowerSpectrumSource, VCRedundancyControllerVideo, VCSwitchManager, NSMutableArray;
...
@interface VCSessionParticipantLocal : VCSessionParticipant <VCVideoSink, VCVideoCaptureClient, VCIDSStreamIDGenerator, VCSessionUplinkBandwidthAllocatorClient, VCDeviceStateProtocol> {
    ...
    BOOL _isLowBandwidthSinglePacketDuplicationEnabled;
    BOOL _isLateKeyFrameDetectionEnabled;
...
@property (nonatomic) BOOL alwaysHDCaptureScreenEnabled;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

+ (BOOL)isCameraEnabledWithMediaStates:(id)a0;
...
- (BOOL)addFeatureListStringsForVideoConfiguration:(id)a0 mediaType:(unsigned char)a1;
- (BOOL)addMediaConfigurationsToLocalConfiguration:(id)a0;
- (BOOL)addVideoConfigurationsToLocalConfiguration:(id)a0;
...
- (BOOL)completeStreamSetup:(id)a0 rtpCipherSuite:(long long)a1;
...
- (void)deregisterForVideoCaptureAndStopProcessingAllMediaRecorderRequests:(BOOL)a0;
...
- (void)dispatchedSetPeerSubscribedStreams:(id)a0;
...
- (struct tagVCNetworkStreamIdentifiers { unsigned int x0; unsigned short x1; unsigned short x2; unsigned short x3; })generateSSRC:(BOOL)a0 streamID:(BOOL)a1 repairStreamID:(BOOL)a2 v2StreamID:(BOOL)a3;
- (id)getAudioDumpName;
...
- (void)localDeviceOrientationDidChange:(int)a0;
...
- (id)newStreamInfoWithAudioStreamWithConfiguration:(id)a0 idsDestination:(id)a1 isMultiway:(BOOL)a2 streamGroupID:(unsigned int)a3 streamToken:(long long)a4;
...
- (BOOL)prepareVideoFrame:(struct opaqueCMSampleBuffer { } *)a0;
- (id)processCachedParticipantData:(id)a0;
- (id)qualityTierTableForStreamToken:(long long)a0 withRedundancyEnabled:(BOOL)a1;
- (void)reactionDidStart:(id)a0;
- (BOOL)reconfigureAudioIOForMode:(int)a0 error:(id *)a1;
