@class VCAudioMachineLearningCoordinator, VCMediaRecorder, VCAudioTransmitter, VCTelephonyInterface, VCAudioIO, NSObject, VCAudioHALPluginRemoteDeviceInfo, NSMutableArray, VCAudioPowerSpectrumSource, NSString, DTMFEventHandler, AVCStatisticsCollector, NSNumber, WRMClient, VCAudioPayload;
...
@interface VCAudioStream : VCMediaStream <VCTelephonyInterfaceDelegate, VCMediaStreamRateAdaptationDelegate, VCPowerManagerThermalDelegate, VCMediaStreamSyncSource, VCAudioIOSource, VCAudioIOSink, VCAudioIODelegate> {
    ...
    BOOL _isACC24ForU1Enabled;
    BOOL _isACC24ForGFTEnabled;
}
...
- (id)getCodecConfigForPayload:(int)a0 streamConfig:(id)a1;
...
- (id)initWithClientPid:(int)a0 ssrc:(unsigned int)a1 transportSessionID:(unsigned int)a2 streamToken:(long long)a3 experimentManager:(id)a4 logPrefix:(id)a5;
...
- (unsigned int)packetExpirationTimeForStreamConfig:(id)a0;
...
- (void)setupAudioMachineLearningCoordinatorsWithFormat:(struct tagVCAudioFrameFormat { struct AudioStreamBasicDescription { double x0; unsigned int x1; unsigned int x2; unsigned int x3; unsigned int x4; unsigned int x5; unsigned int x6; unsigned int x7; unsigned int x8; } x0; unsigned int x1; } *)a0 direction:(long long)a1;
...
- (BOOL)shouldEnableAACELDInactiveFrames:(BOOL)a0 streamConfig:(id)a1;
...
- (BOOL)shouldSetUpMediaRecorderWithParticipantInfoDictionary:(id)a0;
- (BOOL)shouldStartTransportSessionBeforeStream;
