@class NSUUID, RBAnimationSequencer, RBTransition, NSString;

@interface RBDisplayList : NSObject <_RBDisplayListContents, RBDisplayListContents> {
    struct Builder { unsigned int _crop_seed; unsigned int _layer_id; struct refcounted_ptr<RB::DisplayList::Contents> { struct Contents *_p; } _contents; struct State *_free_states; struct LayerState *_free_layer_states; struct LayerState *_active_layer; struct State { struct State *_next; void *_context; struct AffineTransform { void /* unknown type, empty encoding */ ab; void /* unknown type, empty encoding */ cd; void /* unknown type, empty encoding */ txy; } _ctm; struct Identity { unsigned int identifier; struct UUID { unsigned char bytes[16]; } ns; } _identity; struct refcounted_ptr<const RB::Transition> { struct Transition *_p; } _transition; struct ClipNode *_clip; struct Style *_style; struct AffineTransform *_copied_ctm; struct Metadata *_copied_metadata; struct Rect { void /* unknown type, empty encoding */ origin; void /* unknown type, empty encoding */ size; } _crop; unsigned int _layer_id; unsigned int _crop_seed; struct optional_float { float _value_or_nan; } _numeric_value; struct optional<RB::ColorSpace> { union { char __null_state_; struct ColorSpace { unsigned char primaries : 4; unsigned char transfer_fn : 4; } __val_; } ; BOOL __engaged_; } _default_color_space; struct TextIdentity *_text_identity; struct AnimationLayer { unsigned int layer_index; unsigned int layer_count; } _animation_layer; void /* unknown type, blank encoding */ _target_headroom; unsigned char _draw_flags; } _root_state; struct LayerState { struct Layer *_layer; struct LayerState *_next_layer; struct Rect { void /* unknown type, empty encoding */ origin; void /* unknown type, empty encoding */ size; } _crop; unsigned int _crop_seed; } _root_layer_state; struct State *_state; unsigned char _profile; } _list;
...
- (void)addAnimationStyle:(id)a0 id:(struct { unsigned char x0[16]; })a1 flags:(unsigned int)a2;
...
- (void)drawLayerByReference:(id)a0 alpha:(float)a1 blendMode:(int)a2 flags:(unsigned int)a3;
...
- (void)addDistanceFilterWithMaxDistance:(double)a0 scale:(double)a1 flags:(unsigned int)a2;
- (void)addPredicateStyle:(id)a0;
- (void)drawPlaceholder:(id)a0;
...
- (void)addDistanceFilterWithZeroDistance:(double)a0 oneDistance:(double)a1 scale:(double)a2 flags:(unsigned int)a3;
...
- (void).cxx_destruct;
- (void)addShadowStyleWithRadius:(double)a0 midpoint:(float)a1 offset:(struct CGSize { double x0; double x1; })a2 color:(struct { float x0; float x1; float x2; float x3; })a3 colorSpace:(int)a4 blendMode:(int)a5 flags:(unsigned int)a6;
...
- (void)addContentHeadroom:(float)a0;
- (void)addStyle:(int)a0 data:(const void *)a1;
...
- (void)addPlaceholderStyle:(id)a0;
...
- (void)addFilterLayerWithShader:(id)a0 border:(struct CGSize { double x0; double x1; })a1 layerBorder:(struct CGSize { double x0; double x1; })a2 bounds:(const struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } *)a3 flags:(unsigned int)a4;
