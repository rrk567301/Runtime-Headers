...
@property (readonly, nonatomic) struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } domainCacheLock;
@property (retain) NSMutableDictionary *volumesToCaches;
@property (readonly, nonatomic) struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } volumeCacheLock;
@property (retain) NSMutableDictionary *fsidsToCaches;
@property (readonly, nonatomic) struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } fsidCacheLock;
...
- (void)_afterGenerationForGeneratorRequest:(id)a0 withGenerator:(id)a1 bitmapImages:(id)a2 contentRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a3 shouldUpdateGenstore:(BOOL)a4 completionHandler:(id /* block */)a5;
