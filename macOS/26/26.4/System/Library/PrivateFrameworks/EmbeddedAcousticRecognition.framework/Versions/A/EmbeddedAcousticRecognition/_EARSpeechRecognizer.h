@class NSObject, NSDictionary, _EARRelevantTextContext, _EARPersonalNamedEntityRetrievalOptions, _EARSpeakerCodeInfo, EARVoiceCommandActiveSet, _EARRecognitionMetrics, _EARSpeechRecognitionAudioBuffer, NSString, _EARFormatter, NSSet, _EARSpeechModelInfo, NSArray, NSData, NSNumber, _EARTokenizer;
...
@interface _EARSpeechRecognizer : NSObject {
    ...
    struct vector<std::string, std::allocator<std::string>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _rightContextTokens;
...
@property (readonly, nonatomic) BOOL continuousListeningFormatAcrossUtterances;
@property (readonly, nonatomic) NSString *configPath;
@property (readonly, nonatomic) unsigned short itnEnablingFlags;
@property (readonly, nonatomic) BOOL hasUtteranceDetection;
...
@property (readonly) BOOL enableDRTOptimization;
...
@property (nonatomic) BOOL allowUtteranceCompletionDelay;
...
@property (nonatomic) BOOL enableFullPayloadCorrection;
@property (copy, nonatomic) NSString *clientId;
@property (nonatomic) BOOL disableEagerLimit;
...
+ (void)initialize;
+ (BOOL)compileRecognizerModels2WithConfiguration:(id)a0 error:(id *)a1;
...
- (void)_testGetMuxIdMask:(id *)a0 muxIdReverseMask:(id *)a1 maskedMuxIds:(id *)a2;
- (id)_tokenizer;
...
- (id)initWithConfig:(id)a0 overrides:(id)a1 overrideConfigFiles:(id)a2 language:(id)a3 activeConfiguration:(id)a4 modelLoadingOptions:(id)a5 enableSpeakerCodeTraining:(BOOL)a6 supportEmojiRecognition:(BOOL)a7 voiceCommandActiveSet:(id)a8 modelContextDelegate:(id)a9 enableItn:(BOOL)a10 error:(id *)a11;
...
- (BOOL)isVadGatingSupported:(unsigned long long)a0 task:(id)a1 device:(id)a2 farField:(BOOL)a3 atypicalSpeech:(BOOL)a4 aneContext:(id)a5 cpuContext:(id)a6 gpuContext:(id)a7;
- (void)loadParser;
...
- (void)setUserProfiles:(id)a0 multiUserIds:(id)a1;
- (struct vector<std::string, std::allocator<std::string>> { void *x0; void *x1; struct { void *x0; } x2; })splitWithTokenizer:(id)a0 isLeftContext:(BOOL)a1 shouldTruncate:(BOOL)a2 outTokensInVocab:(id *)a3;
- (struct vector<std::string, std::allocator<std::string>> { void *x0; void *x1; struct { void *x0; } x2; })splitWithTokenizer:(id)a0 outTokensInVocab:(id *)a1 isLeftContext:(BOOL)a2;
- (id)testFormattingWithOneBestResults:(id)a0 uttMillis:(id)a1;
- (id)tokenizeText:(id)a0 fromEnd:(BOOL)a1 withLimit:(unsigned long long)a2 outTokensInVocab:(id *)a3;
