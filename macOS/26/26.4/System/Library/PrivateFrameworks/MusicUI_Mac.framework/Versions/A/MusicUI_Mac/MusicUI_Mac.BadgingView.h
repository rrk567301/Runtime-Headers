@interface MusicUI_Mac.BadgingView : NSView {
    void /* unknown type, empty encoding */ isHovered;
    void /* unknown type, empty encoding */ wantsContextActionDisplayedOnHover;
    void /* unknown type, empty encoding */ isSelected;
    void /* unknown type, empty encoding */ badges;
    void /* unknown type, empty encoding */ layoutStyle;
    void /* unknown type, empty encoding */ $__lazy_storage_$_addButton;
    void /* unknown type, empty encoding */ textLabel;
    void /* unknown type, empty encoding */ $__lazy_storage_$_previewLabel;
    void /* unknown type, empty encoding */ contextualActionButton;
}

@property (nonatomic) long long userInterfaceLayoutDirection;

- (void)layout;
- (void).cxx_destruct;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0;
- (id)initWithCoder:(id)a0;

@end
