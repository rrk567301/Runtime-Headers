...
@interface TIKeyboardInputManager : TIKeyboardInputManagerBase <TIRevisionHistoryDelegate, TILanguageSelectionControllerDelegate> {
    ...
    struct unique_ptr<TI::RejectionsDatabase, std::default_delete<TI::RejectionsDatabase>> { struct { struct RejectionsDatabase *__ptr_; } ; } m_rejectionsDatabase;
    ...
    struct vector<RecentMessage, std::allocator<RecentMessage>> { struct RecentMessage *__begin_; struct RecentMessage *__end_; struct { struct RecentMessage *__cap_; } ; } conversation_history;
    ...
    BOOL _wasDeviceLockedLastSync;
    BOOL _isDeviceLockFlagValid;
}
...
- (void)attachToDynamicResourceDirectory:(id)a0;
...
- (void)insertSpaceBeforeInputWithContext:(id)a0;
...
- (void)updateDynamicDirectoryForScreenLockState:(BOOL)a0;
