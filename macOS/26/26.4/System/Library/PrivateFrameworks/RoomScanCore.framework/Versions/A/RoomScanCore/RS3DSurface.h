...
@interface RS3DSurface : NSObject <NSCopying, NSSecureCoding> {
    struct vector<float __attribute__((ext_vector_type(3))), std::allocator<float __attribute__((ext_vector_type(3)))>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _polygon;
    struct vector<RSEdgeStateType, std::allocator<RSEdgeStateType>> { char *__begin_; char *__end_; struct { char *__cap_; } ; } _polygonEdgeStates;
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } _polygonEdgeConfidence;
    struct vector<RSEdgeSideType, std::allocator<RSEdgeSideType>> { char *__begin_; char *__end_; struct { char *__cap_; } ; } _polygonEdgeSides;
    unsigned char type;
