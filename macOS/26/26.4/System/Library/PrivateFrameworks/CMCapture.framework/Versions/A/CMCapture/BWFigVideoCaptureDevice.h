@class BWActionCameraFlickerAvoidanceMonitor, BWMovingWindowStats, NSDictionary, BWSmartFramingSceneMonitor, BWStats, NSMutableSet, NSMutableArray, BWFigCaptureDevice, BWHistogramStats, BWAmbientLightSensor, BWStereoVideoCaptureSceneMonitor, BWInferenceResult, BWZoomCommandHandler, BWMemoryPool, BWFigVideoCaptureSynchronizedStreamsGroup, BWImageControlModeTransitionMonitor, NSNumber, BWFigCaptureDeviceVendor, BWStillImageCaptureSettings, BWFigVideoCaptureStream, BWDeviceMotionActivityDetector, BWMetadataTimeMachine, BWBravoStreamSelector, BWVariableFrameRateSelector, NSArray, BWFrameStatisticsByPortType, BWRamp, NSMutableDictionary, FigWeakReference, BWZoomRange, NSString, NSObject, FigStateMachine, BWStreamStartStopSynchronizer, NSSet;
@protocol BWStillImageProcessingSupportProvider, BWFigVideoCaptureDeviceSubjectSelectionDelegate, BWFigVideoCaptureDeviceCenterStageDelegate, BWSupplementalPointCloudCaptureDeviceDelegate, BWPreviewTimeMachineProcessor, BWFigVideoCaptureDeviceManualCinematicFramingDelegate, BWAttachedMediaTimeMachineDelegate, OS_dispatch_queue, BWPortraitSceneMonitor, BWFigVideoCaptureDeviceZoomFactorChangedDelegate, BWAdaptiveBracketingController, BWFigVideoCaptureDeviceStreamingFilterDelegate;
...
@interface BWFigVideoCaptureDevice : NSObject <BWZoomCompletionDelegate, BWSupplementalPointCloudCaptureDeviceDelegate> {
    ...
    BOOL _awbStatisticsAveragingSupported;
    ...
    BOOL _smartFramingIsMonitoringScene;
    ...
    BOOL _smartFramingFlashScene;
    BOOL _smartFramingAvoidsResolvingSuggestedFieldOfViewForFlashScene;
    ...
    BOOL _afWindowParamsByPortTypeSupported;
    ...
    BOOL _faceFusionMeteringExposureModeSupported;
    ...
    BOOL _stillImageCaptureWithNondisruptiveSwitchingEnabled;
    BOOL _stillImageCaptureWithNondisruptiveSwitchingInProgress;
    ...
    BOOL _ultraHighResolutionZeroShutterLagCannotDeferMitigationEnabled;
    ...
    BOOL _configuresStreamingImageIntentForActionCamera;
    ...
    BOOL _secureMetadataEnabled;
    BOOL _hitPortraitInPhotoPreviewThermalPressure;
...
@property (nonatomic) BOOL smartCropSupported;
...
@property (nonatomic) BOOL stillImageEnhancedResByMaintainingSensorResolutionSupported;
...
@property (readonly, nonatomic, getter=isFaceFusionMeteringExposureModeSupported) BOOL faceFusionMeteringExposureModeSupported;
...
@property (nonatomic) BOOL teleAutoVideoFrameRateAllows24FPS;
...
@property (nonatomic) BOOL configuresStreamingImageIntentForActionCamera;
...
@property (nonatomic) BOOL zoomPIPSingleStreamModeEnabled;
...
@property BOOL outputAspectRatioSupported;
@property (readonly) BWFigVideoCaptureSynchronizedStreamsGroup *synchronizedStreamsGroup;
@property (readonly) BOOL midFrameSynchronizationEnabled;
...
@property BOOL wideAsStatisticsPrimaryPreferred;
@property BOOL awbStatisticsAveragingEnabled;
@property (readonly) struct { struct { float x0; float x1; } x0[3]; struct { float x0; float x1; } x1[3]; struct { float x0; float x1; } x2[3]; struct { float x0; float x1; } x3[3]; BOOL x4; struct { unsigned int x0; struct { float x0; float x1; } x1[3]; float x2[3]; float x3[3]; float x4[3]; int x5[3]; int x6; float x7[7]; float x8[7][12]; int x9; int x10; struct { float x0; float x1; } x11; struct { float x0; float x1; } x12[3]; float x13[3]; float x14[3]; float x15[3]; BOOL x16; float x17; float x18; } x5; double x6; } sphereShiftState;
...
@property (nonatomic) BOOL smartFramingEnabled;
@property (nonatomic) BOOL smartFramingRequiresSceneMonitoring;
@property (nonatomic) BOOL smartFramingIsMonitoringScene;
...
@property (nonatomic) BOOL dockedTrackingActive;
...
@property (nonatomic) BOOL ringLightSupported;
@property (nonatomic) BOOL ringLightEnabled;
@property (nonatomic) float ringLightColor;
@property (nonatomic) BOOL ringLightAutoOverridden;
...
- (int)setDeskViewAutoExposureAreaOfInterest:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0;
- (BOOL)setAutoFlashEnabled:(BOOL)a0 error:(id *)a1;
- (void)setMaximumFrameRate:(float)a0 attachedSessionID:(id)a1;
- (BOOL)isFlashScene;
...
- (int)setSmartFramingEnabledFieldsOfView:(id)a0;
- (void)setCinematicVideoFocus:(id)a0;
- (int)setExposureModeLocked;
- (void)setSensorRegionOfInterestForCameraControls:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 forSensorDimensions:(struct { int x0; int x1; })a1;
...
- (struct OpaqueFigSampleBufferProcessor { } *)_autofocusProcessor;
- (id)synchronizationPrimaryForMidFrameSynchronizationEnabled:(BOOL)a0;
...
- (id)initWithCaptureDevice:(id)a0 attributes:(id)a1 synchronizedStreamsAttributes:(id)a2 unsynchronizedStreamsAttributes:(id)a3 multiCamEnabled:(BOOL)a4 midFrameSynchronizationEnabled:(BOOL)a5 applicationID:(id)a6 clientAuditToken:(struct { unsigned int x0[8]; })a7 tccIdentity:(id)a8 mediaEnvironment:(id)a9 deviceClientPriority:(int)a10 error:(int *)a11;
- (void)_resetStillImageCaptureRequestState;
- (void).cxx_destruct;
- (struct { long long x0; int x1; unsigned int x2; long long x3; })exposureDuration;
- (void)updateSmartFramingCamGazeProbabilities:(id)a0;
...
- (id)cameraInfoByPortTypeIncludingLSCGains:(BOOL)a0 includeCameraInfoForSensorRawProcessing:(BOOL)a1;
...
- (id)stillImageCaptureSettingsWithSettings:(id)a0 speedOverQualityCaptureEnabled:(BOOL)a1;
- (void)captureStream:(id)a0 willEmitStillImageSampleBuffer:(struct opaqueCMSampleBuffer { } *)a1 fromTimeMachine:(BOOL)a2;
- (void)liveReconfigureForOutputAspectRatioFirstBufferReceived:(struct { long long x0; int x1; unsigned int x2; long long x3; })a0 forConfigurationID:(id)a1;
...
- (void)updateLensSmudgeDetectionInfo:(id)a0;
...
- (id)_initWithCaptureDevice:(id)a0 attributes:(id)a1 synchronizedStreamsAttributes:(id)a2 unsynchronizedStreamsAttributes:(id)a3 multiCamEnabled:(BOOL)a4 midFrameSynchronizationEnabled:(BOOL)a5 applicationID:(id)a6 clientAuditToken:(struct { unsigned int x0[8]; })a7 tccIdentity:(id)a8 mediaEnvironment:(id)a9 deviceVendor:(id)a10 createAutofocusSampleBufferProcessorFunction:(void /* function */ *)a11 cameraParameters:(id)a12 deviceClientPriority:(int)a13 error:(int *)a14;
...
- (int)setExposureModeAutoWithRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 isDefaultRect:(BOOL)a1;
...
- (void)liveReconfigureForOutputAspectRatioCompleted:(long long)a0 forConfigurationID:(long long)a1;
