@class CWFAutoJoinManager, CWFBrokenBackhaulMonitor, NSMutableSet, CWFNetworkProfile, NSMutableDictionary, CWFKernelEventMonitor, CWFIO80211, CWFPrivateMACManager, NSObject, CWFSCNetworkConfiguration, CWFXPCConnection, NSSet, NSOperationQueue, CWFNetworkOfInterestManager, CWFSCNetworkReachability, CLLocation, CWFHotspotClientManager, CWFWiFiNetworkSharingManager, CWFScanResult, NSMutableOrderedSet, CWFConfigurationProfileManager;
@protocol CWFXPCRequestProxyDelegate, OS_dispatch_source, OS_dispatch_queue, CWFAssetMgmtProtocol;

@interface CWFXPCRequestProxy : NSObject {
    NSObject<OS_dispatch_queue> *_internalQueue;
    NSObject<OS_dispatch_queue> *_mutexQueue;
    NSObject<OS_dispatch_queue> *_eventQueue;
    NSObject<OS_dispatch_queue> *_systemConfigInterfaceQueue;
    NSObject<OS_dispatch_queue> *_apple80211InterfaceQueue;
    NSObject<OS_dispatch_queue> *_cellularOutrankQueue;
    NSMutableSet *_nonApple80211InterfaceList;
    NSMutableDictionary *_apple80211Map;
    NSMutableSet *_apple80211EligibleRemovalList;
    NSMutableDictionary *_SCNetworkServiceMap;
    NSMutableDictionary *_SCNetworkInterfaceMap;
    NSMutableDictionary *_EAP8021XMap;
    CWFSCNetworkConfiguration *_SCNetworkConfiguration;
    CWFSCNetworkReachability *_SCNetworkReachability;
    CWFIO80211 *_IO80211;
    CWFKernelEventMonitor *_keventMonitor;
    NSSet *_mutableEventIDs;
    BOOL _isMonitoringEvents;
    NSMutableDictionary *_mutableLinkDownStatusMap;
    NSMutableDictionary *_mutableRoamStatusMap;
    NSMutableDictionary *_mutableJoinStatusMap;
    NSMutableDictionary *_mutableAutoJoinTriggerMap;
    NSMutableDictionary *_mutableAutoJoinStatusMap;
    NSMutableDictionary *_mutableLinkQualityMap;
    double _lastLinkQualityMetricLoggedAt;
    NSMutableDictionary *_mutableQueueMap;
    NSOperationQueue *_defaultQueue;
    NSMutableDictionary *_mutableLinkDownNetworkMap;
    CWFNetworkProfile *_cachedKnownNetwork;
    NSObject<OS_dispatch_source> *_privateMACConnectivityTimer;
    long long _nextPrivateMACConnectivityCheckInterval;
    CWFPrivateMACManager *_privateMACManager;
    CWFAutoJoinManager *_autoJoinManager;
    CWFHotspotClientManager *_hotspotClientManager;
    CWFXPCConnection *_internalRequestXPCConnection;
    struct NETRBClient { } *_netRBClient;
    unsigned int _netRBAssertionID;
    id /* block */ _netRBStartServiceReply;
    id /* block */ _netRBStopServiceReply;
    id<CWFAssetMgmtProtocol> _mobileAssetManager;
    CWFConfigurationProfileManager *_configProfileManager;
    CWFNetworkOfInterestManager *_noiManager;
    NSMutableOrderedSet *_mutableRecommendedKnownNetworks;
    NSObject<OS_dispatch_source> *_recommendedKnownNetworksExpirationTimer;
    CWFBrokenBackhaulMonitor *_bbhMonitor;
    NSMutableDictionary *_mutableUIStateFlags;
    NSMutableDictionary *_mutableUIScanManagers;
}

@property (retain) id<CWFXPCRequestProxyDelegate> delegate;
@property (copy) CLLocation *location;
@property (copy) CWFScanResult *associatedNetwork;
@property (readonly) CWFPrivateMACManager *privateMACManager;
@property (readonly) CWFAutoJoinManager *autoJoinManager;
@property (readonly) CWFAutoJoinManager *autoJoinManagerCached;
@property (readonly) CWFNetworkOfInterestManager *noiManager;
@property (readonly) CWFWiFiNetworkSharingManager *wifiNetworkSharingManager;
@property (copy) id /* block */ wifiUserAgentConnection;
@property (copy) id /* block */ wifiNetworkSharingUIServiceXPCConnection;
@property (copy) id /* block */ wifiNetworkSharingAppexXPCConnections;
@property (copy) id /* block */ wifiNetworkSharingAppXPCConnection;
@property (nonatomic) int cellularOutrankWiFiNotificationToken;
@property (nonatomic) unsigned long long cellularOutrankWiFiSymtpomsState;

- (void)setDurationForJoinPMAssertion:(double)a0;
- (void)__startMonitoringDefaultEventIDsForInterfaceName:(id)a0;
- (id)SCNetworkServiceWithInterfaceName:(id)a0;
- (id)__hashedSSID:(id)a0 XPCConnection:(id)a1 error:(id *)a2;
- (id)__autojoinStatusWithInterfaceName:(id)a0;
- (void)__getGlobalIPv6Addresses:(id)a0;
- (void)__getGlobalIPv4Addresses:(id)a0;
- (id)__queueForXPCRequest:(id)a0;
- (id)__noiManager;
- (void)__updateSystemConfigurationInterfacesWithReason:(id)a0 reply:(id /* block */)a1;
- (void)__getQuickProbeRequired:(id)a0;
- (void)__sendXPCEvent:(id)a0;
- (void)__setCachedKnownNetwork:(id)a0;
- (void)__setChannel:(id)a0;
- (void)__getEAP8021XControlState:(id)a0;
- (unsigned long long)__WiFiUIStateFlagsForInterfaceName:(id)a0;
- (void)__getBSSID:(id)a0 XPCConnection:(id)a1;
- (void)__getDHCPv6ServerID:(id)a0;
- (void)__getIO80211ControllerInfo:(id)a0;
- (void)__handleKnownNetworkProfileChangedEvent:(id)a0;
- (id)apple80211WithVirtualInterfaceRole:(int)a0 parentInterfaceName:(id)a1;
- (void)updateJoinStatusWithIPv6:(id)a0;
- (void)__getCloudNetworks:(id)a0;
- (void)resume;
- (unsigned long long)_updateCellularRankingState;
- (id)apple80211WithInterfaceName:(id)a0;
- (void)__handleUIScanFinished:(id)a0 errorInfo:(id)a1;
- (id)__interfaceNames;
- (void)__getGlobalIPv6InterfaceName:(id)a0;
- (void)__updateJoinStatusWithAuth:(id)a0;
- (void)__getNetworkManagedByMDM:(id)a0;
- (void)__performAutoJoinWithParameters:(id)a0 interfaceName:(id)a1;
- (void)__handleAutoJoinStateChangedInternalEvent:(id)a0;
- (void)__setJoinStatus:(id)a0 interfaceName:(id)a1;
- (void)__setRoamStatus:(id)a0 interfaceName:(id)a1;
- (void)activate;
- (void)__getLinkQualityMetric:(id)a0;
- (void)__stopNetworkRelayBridgeForInterfaceName:(id)a0 reply:(id /* block */)a1;
- (void)__getDHCPLeaseExpirationTime:(id)a0;
- (void)__handleNearbyRecommendedNetworksChangedEvent:(id)a0;
- (void)XPCManager:(id)a0 XPCConnection:(id)a1 canceledXPCRequestsWithUUID:(id)a2;
- (void)__getIPv4NetworkSignature:(id)a0;
- (void)__updateRecommendedKnownNetworks;
- (void)__getDeviceSupervised:(id)a0;
- (id)__hiddenKnownNetworkNames;
- (void)__addJoinStatusWithScanResult:(id)a0 knownNetworkProfile:(id)a1 isAutoJoin:(BOOL)a2 interfaceName:(id)a3;
- (void)__privateMACEvaluationCompletedWithState:(long long)a0 knownNetwork:(id)a1 interfaceName:(id)a2;
- (void)__getGlobalIPv4Router:(id)a0;
- (void)__updateEventRegistration:(id)a0 XPCConnection:(id)a1 XPCManager:(id)a2;
- (id)__apple80211WithInterfaceName:(id)a0;
- (void)__getWiFiUIStateFlags:(id)a0;
- (void)__updateWiFiNetworkServices;
- (void)__askToShareWiFiNetworkSharingNetworkFromAppex:(id)a0;
- (id)__uiScanManagerForInterface:(id)a0;
- (void)XPCManager:(id)a0 XPCConnection:(id)a1 receivedXPCRequest:(id)a2;
- (BOOL)isMonitoringEvents;
- (void)__setLinkQualityMetric:(id)a0 interfaceName:(id)a1;
- (id)interfaceNames;
- (void)__confirmBrokenBackhaul:(id)a0;
- (id)__SCNetworkServiceWithInterfaceName:(id)a0;
- (void)__handleWillAssocInternalEvent:(id)a0;
- (void)__stopMonitoringDefaultEventIDs;
- (void)__getGlobalIPv4InterfaceName:(id)a0;
- (void)__updateWiFiInterfacesWithReason:(id)a0 reply:(id /* block */)a1;
- (int)__defaultInterfaceRoleForRequestType:(long long)a0;
- (void)__setWiFiNetworkSharingAskToShareStatus:(id)a0;
- (void)__stopUIScan:(id)a0;
- (void)__startEventMonitoring;
- (void)__getWiFiNetworkSharingAskToShareStatus:(id)a0;
- (id)__linkDownNetworkWithInterfaceName:(id)a0;
- (void)__getAutoJoinStatistics:(id)a0;
- (void)__getGlobalIPv6NetServiceName:(id)a0;
- (void)__getRoamStatus:(id)a0;
- (void)__resetWiFiNetworkSharing:(id)a0;
- (void)__allowBrokenBackhaulPersonalHotspotFallbackForAutoJoinWithNetwork:(id)a0 reply:(id /* block */)a1;
- (id)SCNetworkInterfaceWithInterfaceName:(id)a0;
- (void)__getNetworkOfInterestHomeState:(id)a0;
- (long long)__shareableEAPNetworkHandler:(id)a0 withShareableEAPConfiguration:(id *)a1;
- (void)__cancelRequestsWithNamePrefix:(id)a0;
- (void)__disassocWithReason:(long long)a0 interfaceName:(id)a1;
- (void)__getHardwareMACAddress:(id)a0;
- (id)__joinStatusWithInterfaceName:(id)a0;
- (void)__getKnownNetworkMatchingScanResult:(id)a0;
- (void)__getCaptivePortalCredentials:(id)a0;
- (void)__autoJoinStartedWithTrigger:(long long)a0 interfaceName:(id)a1;
- (void)__getEAP8021XClientStatus:(id)a0;
- (void)__getGlobalIPv4NetServiceName:(id)a0;
- (void)suspend;
- (void)__getWiFiNetworkSharingConnectedAPList:(id)a0;
- (id)__hotspotClientManager;
- (void)__removeUnusedKnownNetworks;
- (void)__getBSSIDForVendor:(id)a0 XPCConnection:(id)a1;
- (void)privateMACEvaluationCheckConnectivityWithInterfaceName:(id)a0;
- (void)__getJoinStatus:(id)a0;
- (void)__getIPv4Router:(id)a0;
- (void)__removeNetworkProfile:(id)a0 reason:(long long)a1 interfaceName:(id)a2 reply:(id /* block */)a3;
- (id)defaultInfraInterfaceName;
- (void)__detectBrokenBackhaulForAutoJoinWithNetwork:(id)a0 reply:(id /* block */)a1;
- (id)__deviceUUID;
- (id)__currentScanResultWithInterfaceName:(id)a0 forceNoCache:(BOOL)a1;
- (id)__privateMACManager;
- (void)__stopMonitoringDefaultEventIDsForInterfaceName:(id)a0;
- (void)__updateApple80211InterfacesWithReason:(id)a0 reply:(id /* block */)a1;
- (void)__updateEventMonitoring:(id)a0;
- (void)XPCManager:(id)a0 XPCConnection:(id)a1 updatedProcessState:(id)a2;
- (void)__forwardXPCEvent:(id)a0 reply:(id /* block */)a1;
- (void)__getCurrentScanResult:(id)a0 XPCConnection:(id)a1 reply:(id /* block */)a2;
- (void)__updateJoinStatusWithError:(id)a0 interaceName:(id)a1;
- (id)__roamStatusWithInterfaceName:(id)a0;
- (void)__setPrivateMACAddressUserJoinFailureUIState:(id)a0;
- (void)__getNetServiceName:(id)a0;
- (void)__setLQMConfig:(id)a0;
- (void)__startUIScan:(id)a0;
- (void)__updateMonitoring:(BOOL)a0 eventIDs:(id)a1;
- (void)XPCManager:(id)a0 sendXPCEvent:(id)a1 reply:(id /* block */)a2;
- (id)__EAP8021XWithInterfaceName:(id)a0;
- (void)__getDHCPLeaseStartTime:(id)a0;
- (void)__getPrivateMACAddressMode:(id)a0;
- (void)__updateNetworkProfile:(id)a0 updateProperties:(id)a1 interfaceName:(id)a2 reply:(id /* block */)a3;
- (void).cxx_destruct;
- (void)__acknowledgeWiFiNetworkSharingNetworkListUpdate:(id)a0;
- (void)__getIPv4SubnetMasks:(id)a0;
- (void)__getSystemEventIDs:(id)a0 XPCManager:(id)a1;
- (void)invalidate;
- (void)__getDNSServerAddresses:(id)a0;
- (id)__networkStackMACAddressWithInterfaceName:(id)a0;
- (void)__privateMACSettingChangedForNetworkProfile:(id)a0 interfaceName:(id)a1;
- (void)__updateAutoJoinState:(long long)a0 interfaceName:(id)a1;
- (void)__acknowledgeWiFiNetworkSharingConnectedAPUpdate:(id)a0;
- (BOOL)__allowXPCRequestWithoutAvailableInterface:(id)a0;
- (void)__getKnownNetworkInfoForLocalNetworkPrompt:(id)a0 XPCConnection:(id)a1;
- (void)__performUIScanWithParameters:(id)a0 scanType:(long long)a1 interfaceName:(id)a2 completion:(id /* block */)a3;
- (void)__setUCMExtProfile:(id)a0;
- (id)__cachedKnownNetwork;
- (void)__updateJoinStatusWithDurationForJoinPMAssertion:(id)a0 knownNetworkProfile:(id)a1 durationForJoinPMAssertion:(double)a2 interfaceName:(id)a3;
- (BOOL)isNon80211InterfaceName:(id)a0;
- (void)__logLqmSummary:(id)a0;
- (void)__scheduleNextRecommendedKnownNetworksExpirationTimer;
- (void)__stopHostAPMode:(id)a0 XPCConnection:(id)a1;
- (void)__updateJoinStatusWithLinkUp:(id)a0;
- (void)__updateWiFiUIStateFlagsWithInterfaceNameIfNeeded:(id)a0;
- (void)__updateJoinStatusWithAssoc:(id)a0;
- (id)__interfaceSpecificEventIDs:(id)a0;
- (void)__getSecurity:(id)a0;
- (void)__handleBrokenBackhaulState:(long long)a0 timestamp:(id)a1;
- (void)__getAutoJoinStatus:(id)a0;
- (void)__getNetworkReachability:(id)a0;
- (id)virtualInterfaceNames;
- (void)__getGlobalDNSServerAddresses:(id)a0;
- (void)__getSSID:(id)a0 XPCConnection:(id)a1;
- (BOOL)__cachedWiFiUIStateFlags:(unsigned long long *)a0 forInterfaceName:(id)a1;
- (void)__getPrivateMACAddress:(id)a0;
- (void)__getDHCPServerID:(id)a0;
- (id)__queryMobileAssetA11:(id *)a0;
- (void)__configureScanManagerChannels:(id)a0 forInterface:(id)a1;
- (void)__getGlobalIPv4NetServiceID:(id)a0;
- (id)init;
- (id)__knownNetworkProfiles;
- (void)__currentScanResultWithInterfaceName:(id)a0 forceNoCache:(BOOL)a1 reply:(id /* block */)a2;
- (id)wifiInterfaceNames;
- (void)__addKnownNetwork:(id)a0 XPCConnection:(id)a1;
- (id)__apple80211WithVirtualInterfaceRole:(int)a0 parentInterfaceName:(id)a1;
- (void)__askToShareWiFiNetworkSharingNetworkFromApp:(id)a0;
- (id)EAP8021XWithInterfaceName:(id)a0;
- (void)__getGlobalIPv6NetServiceID:(id)a0;
- (void)__getIO80211InterfaceInfo:(id)a0;
- (void)__getNetServiceID:(id)a0;
- (void)__stopEventMonitoring;
- (void)__getSystemActivities:(id)a0 XPCManager:(id)a1;
- (BOOL)__didFindMatching80211InterfaceForXPCRequest:(id)a0;
- (void)__getPrivateMACAddressModeSystemSetting:(id)a0;
- (void)__handleAssocDoneInternalEvent:(id)a0;
- (void)__performScan:(id)a0 XPCConnection:(id)a1;
- (void)__updateJoinStatusWithKnownNetwork:(id)a0;
- (void)__getCurrentScanResult:(id)a0 XPCConnection:(id)a1;
- (void)__getLinkDownStatus:(id)a0;
- (void)__getRecommendedKnownNetworks:(id)a0;
- (void)__getSSIDForVendor:(id)a0 XPCConnection:(id)a1;
- (void)__setPrivateMACAddressMode:(id)a0;
- (id)__vendorSpecificIdentifierForXPCConnection:(id)a0;
- (id)__virtualInterfaceNames;
- (id)__linkDownStatusWithInterfaceName:(id)a0;
- (void)__updateCurrentKnownBSSWithIPConfigurationForInterfaceName:(id)a0;
- (void)__getAutoJoinMetric:(id)a0;
- (void)__getWAPISubtype:(id)a0;
- (void)__getIPv6Router:(id)a0;
- (id)__stringForUIScanType:(long long)a0;
- (void)__autoJoinEndedWithResult:(BOOL)a0 interfaceName:(id)a1;
- (void)__getEAP8021XSupplicantState:(id)a0;
- (void)updateJoinStatusWithIPv4:(id)a0;
- (void)XPCManager:(id)a0 invalidatedXPCConnection:(id)a1;
- (void)__startNetworkRelayBridgeWithHostAPConfiguration:(id)a0 interfaceName:(id)a1 reply:(id /* block */)a2;
- (void)__getEAP8021XControlMode:(id)a0;
- (void)__reportQuickProbeResult:(id)a0;
- (void)__startMonitoringDefaultEventIDs;
- (void)__getActivities:(id)a0 XPCConnection:(id)a1;
- (void)updateJoinStatusWithEAP8021X:(id)a0;
- (void)__startHostAPMode:(id)a0 XPCConnection:(id)a1;
- (void)__dumpLogs:(id)a0;
- (void)__getEventIDs:(id)a0 XPCConnection:(id)a1;
- (void)__getBrokenBackhaulStateUpdatedAt:(id)a0;
- (void)__getWiFiNetworkSharingAskToShareNetworkList:(id)a0;
- (void)__setupEventHandlersWithInterfaceName:(id)a0;
- (id)__currentNetworkProfileWithInterfaceName:(id)a0;
- (void)__requestWiFiNetworkSharingAuthorization:(id)a0;
- (void)__sendUIScanResultsEvent:(id)a0 scanType:(long long)a1 interfaceName:(id)a2;
- (void)__setLinkDownStatus:(id)a0 interfaceName:(id)a1;
- (void)__setPrivateMACAddressModeSystemSetting:(id)a0;
- (id)__SCNetworkInterfaceWithInterfaceName:(id)a0;
- (id)__wifiInterfaceNames;
- (id)__hashedBSSID:(id)a0 XPCConnection:(id)a1 error:(id *)a2;
- (void)__getBrokenBackhaulState:(id)a0;
- (void)__getNetworkOfInterestWorkState:(id)a0;
- (void)__setCaptivePortalCredentials:(id)a0;
- (void)__getNetworkWarningFlags:(id)a0 XPCConnection:(id)a1;
- (void)__getGlobalIPv6Router:(id)a0;
- (void)__resetAutoJoinStatistics:(id)a0;
- (void)__setupEventHandlers;
- (void)__getIPv4ARPResolvedIPAddress:(id)a0;
- (void)__getNearbyRecommendedNetworks:(id)a0;
- (id)__linkQualityMetricWithInterfaceName:(id)a0;
- (void)__updateMultipleKnownNetworks:(id)a0 updateProperties:(id)a1 interfaceName:(id)a2 reply:(id /* block */)a3;
- (void)__checkin:(id)a0 XPCConnection:(id)a1;
- (void)__getApple80211:(id)a0;
- (void)__getWEPSubtype:(id)a0;
- (void)__setAutoJoinStatus:(id)a0 interfaceName:(id)a1;
- (void)XPCManager:(id)a0 XPCConnection:(id)a1 updatedRegisteredEventIDs:(id)a2;
- (void)__updateAutoJoinStatusWithJoin:(id)a0;
- (void)__getMACAddress:(id)a0;
- (void)__getNetServiceEnabled:(id)a0;
- (void)__getIPv6NetworkSignature:(id)a0;
- (id)__userAutoJoinDisabled;
- (void)XPCManager:(id)a0 XPCConnection:(id)a1 completedXPCRequest:(id)a2;
- (void)__getKnownNetworkMatchingNetworkProfile:(id)a0;
- (void)__getWiFiNetworkSharingNetworkList:(id)a0;
- (void)setTargetQueue:(id)a0 requestType:(long long)a1 interfaceName:(id)a2;
- (void)__getIPv4Addresses:(id)a0;
- (void)__getWiFiNetworkSharingAuthorizationLevel:(id)a0;
- (void)__getPlatformCapabilities:(id)a0;
- (void)__getIPv6Addresses:(id)a0;
- (void)__getIPv4ARPResolvedHardwareAddress:(id)a0;

@end
