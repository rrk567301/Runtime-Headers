@interface FI_IPropertySizeExtractor : FI_IPropertyValueExtractor {
    ...
    struct map<TFENode, NSMutableDictionary *, std::less<TFENode>, std::allocator<std::pair<const TFENode, NSMutableDictionary *>>> { struct __tree<std::__value_type<TFENode, NSMutableDictionary *>, std::__map_value_compare<TFENode, std::pair<const TFENode, NSMutableDictionary *>, std::less<TFENode>>, std::allocator<std::pair<const TFENode, NSMutableDictionary *>>> { void *__begin_node_; struct { struct __tree_end_node<std::__tree_node_base<void *> *> { void *__left_; } __end_node_; } ; struct { unsigned long long __size_; } ; } __tree_; } _nodesAndSizeMap;
}
