...
@interface FI_TGoToWindowController : FI_TScriptableWindowController <NSTouchBarDelegate, TTouchBarGoToViewDelegate, NSWindowDelegate, NSTextFieldDelegate, NSMenuDelegate> {
    ...
    struct function<void (FI_TGoToWindowController *)> { struct __value_func<void (FI_TGoToWindowController *)> { union type { unsigned char __data[24]; } __buf_; void *__f_; } __f_; } _initialItemsDidLoadHandler;
    struct function<void (FI_TGoToWindowController *, const TFENode &, const TFENode &, const TString &)> { struct __value_func<void (FI_TGoToWindowController *, const TFENode &, const TFENode &, const TString &)> { union type { unsigned char __data[24]; } __buf_; void *__f_; } __f_; } _completionHandler;
    ...
    struct vector<TNotificationCenterObserver, std::allocator<TNotificationCenterObserver>> { struct TNotificationCenterObserver *__begin_; struct TNotificationCenterObserver *__end_; struct { struct TNotificationCenterObserver *__cap_; } ; } _notificationCenterObservers;
    struct vector<TKeyValueObserver, std::allocator<TKeyValueObserver>> { struct TKeyValueObserver *__begin_; struct TKeyValueObserver *__end_; struct { struct TKeyValueObserver *__cap_; } ; } _keyValueObservers;
