...
@interface FIAirDropListViewController : FI_TTableViewController <TMarkTornDown> {
    struct function<void (const TFENode &)> { struct __value_func<void (const TFENode &)> { union type { unsigned char __data[24]; } __buf_; void *__f_; } __f_; } _clickHandler;
    struct vector<TNotificationCenterObserver, std::allocator<TNotificationCenterObserver>> { struct TNotificationCenterObserver *__begin_; struct TNotificationCenterObserver *__end_; struct { struct TNotificationCenterObserver *__cap_; } ; } _notificationObservers;
...
- (struct function<void (const TFENode &)> { struct __value_func<void (const TFENode &)> { union type { unsigned char x0[24]; } x0; void *x1; } x0; })clickHandler;
