@interface _TtCV10PegasusAPI39Apple_Parsec_Search_PegasusQueryContextP33_EE810B9D0971AA851E285CF9B9F1905813_StorageClass : _TtCs12_SwiftObject {
    void /* unknown type, empty encoding */ _secretKey;
    void /* unknown type, empty encoding */ _countryCode;
    void /* unknown type, empty encoding */ _locale;
    void /* unknown type, empty encoding */ _effectiveSystemLanguage;
    void /* unknown type, empty encoding */ _preferredLanguages;
    void /* unknown type, empty encoding */ _storeFront;
    void /* unknown type, empty encoding */ _userGuid;
    void /* unknown type, empty encoding */ _timeZone;
    void /* unknown type, empty encoding */ _skuRegion;
    void /* unknown type, empty encoding */ _calendar;
    void /* unknown type, empty encoding */ _temperatureUnit;
    void /* unknown type, empty encoding */ _measurementSystem;
    void /* unknown type, empty encoding */ _hourFormat;
    void /* unknown type, empty encoding */ _location;
    void /* unknown type, empty encoding */ _mapsSession;
    void /* unknown type, empty encoding */ _subscriptions;
    void /* unknown type, empty encoding */ _transportationMode;
    void /* unknown type, empty encoding */ _installedAppsSignature;
    void /* unknown type, empty encoding */ _uiScale;
    void /* unknown type, empty encoding */ _internalBuild;
    void /* unknown type, empty encoding */ _seedBuild;
    void /* unknown type, empty encoding */ _hsEnabled;
    void /* unknown type, empty encoding */ _managedDevice;
    void /* unknown type, empty encoding */ _abTestSeed;
    void /* unknown type, empty encoding */ _siriLocale;
    void /* unknown type, empty encoding */ _region;
    void /* unknown type, empty encoding */ _sil;
    void /* unknown type, empty encoding */ _trialIdentifiers;
    void /* unknown type, empty encoding */ _ampBaselineExperimentMetadata;
    void /* unknown type, empty encoding */ _siriResponseLanguageVariant;
    void /* unknown type, empty encoding */ _marketplaceInfo;
    void /* unknown type, empty encoding */ _deviceModel;
    void /* unknown type, empty encoding */ _isGenerativeModelDevice;
    void /* unknown type, empty encoding */ _companionDevice;
    void /* unknown type, empty encoding */ _deviceMetadata;
    void /* unknown type, empty encoding */ _searchOptOut;
    void /* unknown type, empty encoding */ _longSecretKey;
    void /* unknown type, empty encoding */ _keyboardIme;
    void /* unknown type, empty encoding */ _weatherUnits;
    void /* unknown type, empty encoding */ _encryptedAuthToken;
    void /* unknown type, empty encoding */ _appleIntelligenceStatus;
    void /* unknown type, empty encoding */ _webContentRestriction;
}

@end
