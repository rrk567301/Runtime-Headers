@class SiriTTSDaemonSession, NSString, CUVoiceRequest, NSObject, NSMutableArray;
@protocol OS_dispatch_queue;

@interface CUVoiceSession : NSObject {
    CUVoiceRequest *_currentRequest;
    BOOL _invalidateCalled;
    BOOL _invalidateDone;
    unsigned int _invalidateFlags;
    NSMutableArray *_requests;
    SiriTTSDaemonSession *_speechSynthesizer;
    struct LogCategory { int x0; int x1; char *x2; unsigned int x3; char *x4; char *x5; int x6; struct LogCategory *x7; struct LogOutput *x8; struct LogOutput *x9; unsigned long long x10; unsigned long long x11; unsigned int x12; unsigned int x13; char *x14; struct LogCategoryPrivate *x15; } *_ucat;
    NSMutableArray *_voiceRequests;
}

@property (retain, nonatomic) NSObject<OS_dispatch_queue> *dispatchQueue;
@property (copy, nonatomic) id /* block */ invalidationHandler;
@property (copy, nonatomic) NSString *label;

- (void)_invalidate;
- (void)_invalidated;
- (void).cxx_destruct;
- (void)invalidate;
- (id)init;
- (void)dealloc;
- (void)stopSpeaking;
- (void)_completeAllRequestsWithError:(id)a0;
- (void)_completeRequest:(id)a0 error:(id)a1;
- (void)_processQueuedRequests;
- (void)_speakText:(id)a0 flags:(unsigned int)a1 languageCode:(id)a2 volume:(double)a3 completion:(id /* block */)a4;
- (void)invalidateWithFlags:(unsigned int)a0;
- (void)speakText:(id)a0 flags:(unsigned int)a1 completion:(id /* block */)a2;
- (void)speakText:(id)a0 flags:(unsigned int)a1 languageCode:(id)a2 volume:(double)a3 completionHandler:(id /* block */)a4;
- (void)speakText:(id)a0 flags:(unsigned int)a1 volume:(double)a2 completionHandler:(id /* block */)a3;

@end
