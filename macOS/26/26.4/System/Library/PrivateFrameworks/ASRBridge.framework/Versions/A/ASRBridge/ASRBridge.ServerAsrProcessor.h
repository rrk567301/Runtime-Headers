@interface ASRBridge.ServerAsrProcessor : _TtCs12_SwiftObject {
    void /* unknown type, empty encoding */ requestType;
    void /* unknown type, empty encoding */ assistantId;
    void /* unknown type, empty encoding */ sessionId;
    void /* unknown type, empty encoding */ requestId;
    void /* unknown type, empty encoding */ isIdentityScoreConsumptionEnabled;
    void /* unknown type, empty encoding */ serverEndPointReceived;
    void /* unknown type, empty encoding */ serverEndPointSentToSpeech;
    void /* unknown type, empty encoding */ serverEndpointIdentifiedMessage;
    void /* unknown type, empty encoding */ userId;
    void /* unknown type, empty encoding */ serviceHelper;
    void /* unknown type, empty encoding */ scdaDecisionMonitor;
    void /* unknown type, empty encoding */ bufferedResultCandidateHandler;
    void /* unknown type, empty encoding */ instrumentationUtil;
    void /* unknown type, empty encoding */ isMedocEnabled;
    void /* unknown type, empty encoding */ muxRequestContext;
}

@end
