@interface SRDRequestDispatcher : NSObject <AFRequestDispatcher> {
    void /* unknown type, empty encoding */ requestDispatcherServiceHelper;
    void /* unknown type, empty encoding */ requestDispatcher;
    void /* unknown type, empty encoding */ featureChecker;
    void /* unknown type, empty encoding */ reportingProvider;
    void /* unknown type, empty encoding */ intelligenceFlowRequestDispatcher;
    void /* unknown type, empty encoding */ doesSessionAllowIFRequestDispatcher;
    void /* unknown type, empty encoding */ localRequestIDs;
    void /* unknown type, empty encoding */ uiConnectionProxy;
    void /* unknown type, empty encoding */ deviceSelectionProvider;
}
...
- (void)emitAIREventForSiriAvailabiltyWithLocale:(id)a0 countryCode:(id)a1 isAvailable:(BOOL)a2 orchestrationMode:(unsigned long long)a3 unavailabilityReasons:(unsigned long long)a4;
- (void)emitAIREventsForSiriRequestWithRequestId:(id)a0 missingAssets:(unsigned long long)a1;
...
- (id)initWithUnderlyingRequestDispatcher:(id)a0 requestDispatcherServiceHelper:(id)a1;
...
- (void)startListenerSelectionWithActivation:(id)a0 resultCallback:(id /* block */)a1;
- (void)startLocalRequestWithAssistantId:(id)a0 requestId:(id)a1 inputOrigin:(id)a2 requestContextData:(id)a3;
- (void)startMultiUserTestRequestWithAssistantId:(id)a0 requestId:(id)a1 inputOrigin:(id)a2 utterance:(id)a3 expectedSelectedSharedUserId:(id)a4 voiceIdConfidenceScores:(id)a5 requestContextData:(id)a6;
...
- (void)updateConversationContextForRemoteResponseWithAssistantId:(id)a0 requestId:(id)a1 fullSpeak:(id)a2 redactedFullSpeak:(id)a3 fullPrint:(id)a4 redactedFullPrint:(id)a5 listenAfterSpeaking:(BOOL)a6;
