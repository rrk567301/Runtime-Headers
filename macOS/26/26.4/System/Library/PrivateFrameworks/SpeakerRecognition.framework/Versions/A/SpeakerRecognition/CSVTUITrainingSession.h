@class NSUUID, NSString, NSTimer, SFSpeechAudioBufferRecognitionRequest, CSAudioZeroCounter, NSDictionary, _TtC18SpeakerRecognition24SSRSpeechTranscriberIntf, SFSpeechRecognitionTask, SFSpeechRecognizer, NSMutableArray, NSObject, CSVTUITrainingSessionHelper;
@protocol CSVTUIAudioSession, CSVTUIKeywordDetectorProtocol, OS_dispatch_queue, CSVTUITrainingSessionDelegate;

@interface CSVTUITrainingSession : NSObject <SFSpeechRecognitionTaskDelegate, SpeakerRecognition.SSRSpeechTranscriberIntfDelegate, CSVTUIAudioSessionDelegate, CSVTUIEndPointDelegate> {
    long long _status;
    ...
    BOOL _speechStartDetected;
    BOOL _didFirstPassTrigger;
    BOOL _snrCheckEnabled;
    ...
    BOOL _shouldUseEnhancedSpeechTranscriber;
}
...
- (id)initWithUtteranceId:(long long)a0 sessionNumber:(long long)a1 Locale:(id)a2 audioSession:(id)a3 keywordDetector:(id)a4 speechRecognizer:(id)a5 speechRecognitionRequest:(id)a6 sessionDelegate:(id)a7 sessionDispatchQueue:(id)a8 zeroCounter:(id)a9 snrCheckEnabled:(BOOL)a10 completion:(id /* block */)a11;
- (id)initWithUtteranceId:(long long)a0 sessionNumber:(long long)a1 Locale:(id)a2 vtAssetConfigVersion:(id)a3 audioSession:(id)a4 keywordDetector:(id)a5 speechRecognizer:(id)a6 speechRecognitionRequest:(id)a7 sessionDelegate:(id)a8 sessionDispatchQueue:(id)a9 mhUUID:(id)a10 zeroCounter:(id)a11 snrCheckEnabled:(BOOL)a12 completionWithResult:(id /* block */)a13;
...
- (void)speechTranscriber:(id)a0 didFinishWithError:(id)a1;
