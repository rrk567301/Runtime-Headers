@class NSMutableDictionary, LiveFSVolume, NSMutableOrderedSet, NSObject, NSMutableSet;
@protocol OS_dispatch_queue;

@interface LiveFSAppleDoubleManager : NSObject {
    NSMutableSet *_ADActiveForBaseHandleSet;
    struct _opaque_pthread_rwlock_t { long long __sig; char __opaque[192]; } _ADActiveForBaseHandleSetRWLock;
    struct _opaque_pthread_mutex_t { long long __sig; char __opaque[56]; } _ADActiveForBaseHandleSetMutex;
    struct _opaque_pthread_cond_t { long long __sig; char __opaque[40]; } _ADActiveForBaseHandleSetCond;
    unsigned long long _ADAcctiveForBaseHandleSetWaiters;
}

@property unsigned long long maxCacheSize;
@property (retain) NSMutableDictionary *appleDoubleCache;
@property (retain) NSMutableOrderedSet *appleDoubleCacheLRUOrder;
@property (retain) LiveFSVolume *mount;
@property (retain) NSObject<OS_dispatch_queue> *queue;
@property unsigned long long clientID;

+ (id)AppleDoubleManagerForMount:(id)a0;
+ (BOOL)isDotUnderBarFileName:(id)a0;

- (void)clearCache;
- (void).cxx_destruct;
- (void)dealloc;
- (void)removeFromCache:(id)a0;
- (id)AppleDoubleForPurpose:(int)a0 withBaseFile:(id)a1 named:(id)a2 inDirectory:(id)a3;
- (id)AppleDoubleForWritingWithBaseFile:(id)a0 named:(id)a1 inDirectory:(id)a2;
- (id)AppleDoubleNameForFileNamed:(id)a0;
- (void)updateLRUOrder:(id)a0;
- (id)AppleDoubleForNamespaceWithBaseFile:(id)a0 named:(id)a1 inDirectory:(id)a2;
- (id)AppleDoubleForReadingWithBaseFile:(id)a0 named:(id)a1 inDirectory:(id)a2;
- (void)acquireForBaseFile:(id)a0;
- (void)addToCache:(id)a0 forKey:(id)a1;
- (id)cacheKeyForBaseFile:(id)a0 named:(id)a1 inDirectory:(id)a2;
- (BOOL)enumerateDirectory:(id)a0 intoArray:(id)a1;
- (void)evictLRUIfNeeded;
- (id)getAppleDoubleFromCacheWithBaseFile:(id)a0 named:(id)a1 inDirectory:(id)a2;
- (id)getFromCache:(id)a0;
- (id)initWithMount:(id)a0;
- (void)releaseForBaseFile:(id)a0;
- (BOOL)scrubDirectoryNamed:(id)a0 inDirectory:(id)a1;

@end
