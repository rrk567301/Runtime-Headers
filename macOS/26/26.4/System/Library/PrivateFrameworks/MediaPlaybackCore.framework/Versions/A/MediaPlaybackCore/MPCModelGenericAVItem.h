...
@protocol OS_dispatch_queue, MPCModelPlaybackAssetCacheProviding;
...
@interface MPCModelGenericAVItem : MPAVItem <AVAssetResourceLoaderDelegate, AVPlayerItemMetadataOutputPushDelegate, ICEnvironmentMonitorObserver, ICContentKeySessionDelegateProtocol, MPCPlaybackEngineInfoProvider, MPCGaplessInfoProvider, MPCTrackInfoDataProvider, MPCEventStreamSettable> {
    ...
    BOOL _hasCheckedPlayerItemForMissingEditList;
    BOOL _playerItemIsMissingEditList;
...
+ (BOOL)_isCorruptedAssetErrorCode:(long long)a0;
...
- (long long)type;
- (BOOL)hasDownloadedCatalogLyrics;
...
- (void)contentKeySession:(id)a0 didStartProcessingKey:(id)a1 isPrefetchKey:(BOOL)a2 isPersistable:(BOOL)a3 isRenew:(BOOL)a4;
...
- (id)_formattedDateStringForDateComponents:(id)a0;
...
- (id)cachableAssetWithURL:(id)a0 isHLS:(BOOL)a1 options:(id)a2;
- (id)chapterAt:(double)a0;
- (void)configureAVPlayerItemWithAudioFormat:(id)a0 forceSpatial:(BOOL)a1;
- (void)downloadToAssetCacheWithPreferredAssetType:(long long)a0;
...
- (void)playerItemDidFailToPlayWithError:(id)a0;
