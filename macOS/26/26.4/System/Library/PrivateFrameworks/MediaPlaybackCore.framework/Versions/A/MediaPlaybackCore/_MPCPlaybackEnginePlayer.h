@class MPCItemBookmarker, NSDictionary, MPCAVItemTrace, MPCLeaseController, MPCPlaybackErrorController, NSMutableArray, MPAVItem, AVQueuePlayer, NSString, MPCPlaybackEngine, MPCExternalPlaybackController, NSNumber, MPCPlayerItemConfigurator, AVPlayerViewController;
@protocol MFQueueManagement, MFQueueControlling, MFPlaybackStackController_IPI, MFAssetLoading, MFPlaybackStackController;
...
@property (nonatomic, getter=isAirPlayLocalPlaybackEnabled) BOOL airPlayLocalPlaybackEnabled;
...
- (void).cxx_destruct;
- (void)setVideoEnabled:(BOOL)a0 reason:(long long)a1;
- (void)dealloc;
...
- (void)_configurePrefersOfflinePlayableVariantsForItem:(id)a0;
...
- (void)_playbackDidStopForItem:(id)a0 source:(id)a1 reason:(id)a2 time:(double)a3 timeStamp:(id)a4;
- (BOOL)_prefersAudioOnlyForExternalPlayback;
...
- (void)becomeActiveWithOptions:(unsigned long long)a0 completion:(id /* block */)a1;
- (void)collectLateArrivingAVMetricsForTimeoutError:(id)a0 commandType:(unsigned int)a1;
...
- (void)donateMetricsForTransitionWillStartFrom:(id)a0 to:(id)a1 transitionProvided:(long long)a2;
...
- (void)extendLifetimeUntilFirstAudioFrame:(id)a0 isSiriSetQueue:(BOOL)a1;
...
- (void)interstitialDidBeginFor:(id)a0 segment:(id)a1 timeStamp:(id)a2;
- (void)interstitialDidEndFor:(id)a0 segment:(id)a1 timeStamp:(id)a2;
- (void)interstitialsDidChangeFor:(id)a0 segments:(id)a1 timeStamp:(id)a2;
- (BOOL)isInPlaybackStartCriticalSection;
- (void)isMutedDidChangeTo:(BOOL)a0 timeStamp:(id)a1;
- (BOOL)isUserSelectedItem:(id)a0;
...
- (void)overlappingTransitionDidReachPivotPointFrom:(id)a0 to:(id)a1 transitionTime:(id)a2 incomingItemAveragePostPivotTransitionRate:(double)a3 transitionType:(long long)a4 timeStamp:(id)a5;
...
- (void)performWhenMediaServicesAreAvailable:(id /* block */)a0 identifier:(id)a1;
...
- (void)playbackDidStartForItem:(id)a0 rate:(float)a1 expectedStartTime:(double)a2 fromStalling:(BOOL)a3 timeStamp:(id)a4;
...
- (void)smartTransitionDidEndFrom:(id)a0 to:(id)a1 transitionTime:(id)a2 timeStamp:(id)a3;
- (void)smartTransitionWillBeginFrom:(id)a0 to:(id)a1 transitionTime:(id)a2 outgoingItemAveragePrePivotTransitionRate:(double)a3 timeStamp:(id)a4 parameters:(id)a5;
...
- (double)updateDurationSnapshotForItem:(id)a0 time:(double)a1;
...
- (void)videoRenderingModeDidChange:(long long)a0 reason:(long long)a1;
