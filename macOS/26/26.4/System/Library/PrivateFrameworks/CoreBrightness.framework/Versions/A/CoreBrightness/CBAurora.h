...
@interface CBAurora : NSObject {
    ...
    struct { struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } sdrLUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } rampUpMinAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } rampUpMaxAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } rampDownMinAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } rampDownMaxAPCELUT; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } maxGain; struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } energyConsumptionTarget; } _powerAwareAurora;
}
...
- (id)initWithQueue:(id)a0 andDisplayModule:(id)a1 andBrtCapabilities:(id)a2 andFrameStats:(id)a3 andDisplayID:(unsigned long long)a4;
