@class NSMutableDictionary, CBThermalBrightnessCap, NSObject, CBRingLight, CBGammaContrastPreservation, CBRampManager, CBIndicatorBrightnessModule, NSMutableArray, CBLuminanceNotification, CBDynamicSlider, NSString, CBEDR, CBAurora, CBNVRAM, CBAppliedCompensations, AABCHistograms;
@protocol CBPowerManagementClient, OS_os_log, OS_dispatch_source, CBBrightnessProxy, CBSliderMapping;
...
@interface CBDisplayModuleSKL : CBDisplayModule <CBHIDServiceProtocol> {
    ...
    BOOL _autoBrightnessDisabledByPreset;
...
- (float)getPreDynamicSliderNits:(float)a0;
...
- (BOOL)handleBrightnessFloorCapOverride:(id)a0;
...
- (id)initWithBrightnessControl:(id)a0 andQueue:(id)a1 andCBPM:(id)a2 andRingLight:(id)a3;
- (void)initialiseDisplayConfiguration;
- (void)initialiseIndicatorBrightnessModule;
- (float)luminanceToPerceptual:(float)a0;
- (id)newDisplayInfo;
- (id)newGCP;
- (id /* block */)newRampProgressCallbackForNitsCapWithDescription:(const char *)a0;
- (id)newStatusInfo;
...
- (BOOL)rampNitsCap:(float)a0 length:(float)a1;
- (BOOL)rampNitsFloorCap:(float)a0 length:(float)a1;
- (BOOL)rampPCC:(float)a0 withLength:(float)a1 error:(id *)a2;
...
- (void)requestIndicatorBrightnessRamp;
- (void)requestIndicatorBrightnessRampStop;
...
- (void)updateChromaCorrectionRamp:(id)a0 andCCModule:(id)a1 state:(int)a2;
...
- (void)updateNitsCapLuminanceNotification;
