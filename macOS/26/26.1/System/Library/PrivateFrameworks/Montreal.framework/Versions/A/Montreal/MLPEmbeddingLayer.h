...
@interface MLPEmbeddingLayer : MLPLayer <MLPLayerOptimizerProtocol> {
    struct vector<std::map<unsigned int, std::vector<unsigned int>>, std::allocator<std::map<unsigned int, std::vector<unsigned int>>>> { void *__begin_; void *__end_; void *__cap_; } wordIDRepetitions;
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; float *__cap_; } weightsInitial;
}
