@class NSLock, NSString, CBLEAudioSessionInfo, AXDispatchTimer, NSMutableDictionary, CBCentralManager, NSObject, NSMutableArray;
...
@interface AXHearingAidDeviceController : HUDeviceController <CBPairingAgentDelegate, CBCentralManagerDelegate, AXHADeviceControllerProtocol> {
    ...
    CBLEAudioSessionInfo *_leAudioSessionInfo;
    NSLock *_centralRequestsLock;
    BOOL _isScanning;
    BOOL _stoppedScanningForLEAudio;
    BOOL _centralIsOn;
...
- (BOOL)isLEAudioServiceInServiceUUIDs:(id)a0;
- (void)addAvailableDevice:(id)a0;
- (void)addConnectedDevice:(id)a0;
- (void)addFakeHearingAidsWithType:(id)a0;
- (void)addLoadedDevice:(id)a0;
- (id)addPeripheral:(id)a0 toDevice:(id)a1;
...
- (id)hearingAidForPeripheralID:(id)a0;
...
- (void)processConnectedIdentifiers:(id)a0 andLocations:(id)a1;
