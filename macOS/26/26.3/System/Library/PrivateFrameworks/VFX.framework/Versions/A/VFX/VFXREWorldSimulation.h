...
@interface VFXREWorldSimulation : NSObject {
    ...
    double _systemTime;
    struct unique_ptr<VFXMTLRenderCommandEncoder, std::default_delete<VFXMTLRenderCommandEncoder>> { struct { struct VFXMTLRenderCommandEncoder *__ptr_; } ; } _commandEncoder;
    struct unique_ptr<CFX::BufferAllocatorPerFrame, std::default_delete<CFX::BufferAllocatorPerFrame>> { struct { struct BufferAllocatorPerFrame *__ptr_; } ; } _frameBufferAllocator;
