@interface _TtCO24IntelligencePlatformCore25HKEmergencyContactStructs6Person : _TtCs12_SwiftObject {
    void /* unknown type, empty encoding */ entityIdentifier;
    void /* unknown type, empty encoding */ predicate;
    void /* unknown type, empty encoding */ entityClass;
    void /* unknown type, empty encoding */ source;
    void /* unknown type, empty encoding */ isCurrentUser_predicate;
    void /* unknown type, empty encoding */ isCurrentUser;
    void /* unknown type, empty encoding */ name_predicate;
    void /* unknown type, empty encoding */ name;
    void /* unknown type, empty encoding */ entityAliasRelationship;
    void /* unknown type, empty encoding */ hasAssociation;
    void /* unknown type, empty encoding */ hasContactInformation;
}

@end
