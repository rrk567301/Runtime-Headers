@class VCAudioTier, VCRedundancyControllerAudio, NSDictionary, VCAudioRedBuilder, NSMutableArray, VCPacketBundler, VCTransportSession, NSString, VCAudioTierPicker, VCExperimentManager, AVCStatisticsCollector, NSArray, VCAudioPayload, VCAudioPayloadConfig;
...
@interface VCAudioTransmitter : NSObject <VCAudioIOSink, VCConnectionChangedHandler> {
    ...
    struct tagVCMemoryPool { struct { void *x0; long long x1; } x0; unsigned long long x1; unsigned long long x2; unsigned long long x3; BOOL x4; } *_audioBundlePool;
...
- (int)redundancyControllerModeForOperatingMode:(int)a0 isRedEnabled:(BOOL)a1 isHigherAudioREDCutoverU1Enabled:(BOOL)a2;
...
- (void)setRedNumPayloads:(int)a0 withMaxDelay:(int)a1 shouldResetHistory:(BOOL)a2 shouldEnableShortRED:(BOOL)a3;
