...
@interface ADJasperColorInFieldCalibrationPipeline : NSObject {
    ...
    struct unique_ptr<PixelBufferUtilsSession, std::default_delete<PixelBufferUtilsSession>> { struct { struct PixelBufferUtilsSession *__ptr_; } ; } _colorInputProcessingSession;
    ...
    struct unique_ptr<std::vector<std::vector<float>>[], std::default_delete<std::vector<std::vector<float>>[]>> { struct { void *__ptr_; } ; } _sdfHistoryFrames;
