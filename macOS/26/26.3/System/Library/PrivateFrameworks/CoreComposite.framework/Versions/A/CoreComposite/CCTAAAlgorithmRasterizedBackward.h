...
@interface CCTAAAlgorithmRasterizedBackward : CCAlgorithm {
    ...
    struct vector<float, std::allocator<float>> { float *__begin_; float *__end_; struct { float *__cap_; } ; } _accumulationFactorLUT;
    struct vector<PerTileData, std::allocator<PerTileData>> { struct PerTileData *__begin_; struct PerTileData *__end_; struct { struct PerTileData *__cap_; } ; } _innerPeripheryTiles;
    struct vector<unsigned char, std::allocator<unsigned char>> { char *__begin_; char *__end_; struct { char *__cap_; } ; } _tileOccupancy;
