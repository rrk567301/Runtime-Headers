...
@interface VKNavCameraController : VKCameraController <VKNavigationCameraController, VKGesturingCameraController, VKNavContextObserver> {
    ...
    struct basic_string<char, std::char_traits<char>, std::allocator<char>> { struct { union __rep { struct __short { char __data_[23]; unsigned char __size_ : 7; unsigned char __is_long_ : 1; } __s; struct __long { char *__data_; unsigned long long __size_; unsigned long __cap_ : 63; unsigned char __is_long_ : 1; } __l; } __rep_; } ; } _currentStyleName;
    struct vector<geo::Coordinate3D<geo::Radians, double>, std::allocator<geo::Coordinate3D<geo::Radians, double>>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _coordinatesToFrame;
    ...
    struct vector<NavCameraSpeedFrameRate, std::allocator<NavCameraSpeedFrameRate>> { struct NavCameraSpeedFrameRate *__begin_; struct NavCameraSpeedFrameRate *__end_; struct { struct NavCameraSpeedFrameRate *__cap_; } ; } _speedFrameRate;
    ...
    struct vector<gm::Matrix<float, 2, 1>, std::allocator<gm::Matrix<float, 2, 1>>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _movementSamplePoints;
    struct vector<gm::Matrix<double, 2, 1>, std::allocator<gm::Matrix<double, 2, 1>>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _movementSamples;
    ...
    struct CameraFramingDebug { struct vector<TargetConstraintDebug, std::allocator<TargetConstraintDebug>> { struct TargetConstraintDebug *__begin_; struct TargetConstraintDebug *__end_; struct { struct TargetConstraintDebug *__cap_; } ; } _targetConstraints; BOOL isFullyFramed; struct Coordinate3D<geo::Radians, double> { struct Unit<geo::RadianUnitDescription, double> { double _value; } latitude; struct Unit<geo::RadianUnitDescription, double> { double _value; } longitude; struct Unit<geo::MeterUnitDescription, double> { double _value; } altitude; } peripheralPoints[4]; } _framingDebug;
}
