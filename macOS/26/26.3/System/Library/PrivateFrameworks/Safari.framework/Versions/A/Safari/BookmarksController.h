@class NSURL, NSMutableDictionary, SHA256Hash, SpotlightBookmarksWriter, WebBookmarkGroup, SafariWebBookmark, NSMutableArray, WBSSiteMetadataManager, WBSManagedBookmarksController, NSString, SafariWebBookmarkList, NSTimer, NSArray, FileChangeObserver, CloudBookmarksMigrationCoordinationConsul;
...
@interface BookmarksController : NSObject <BookmarkGroupDelegate, FileChangeObserverClient, FileLockerClient, FileLockerLogDelegate, WBTabGroupManagerObserver, BookmarksUndoControllerDataStore, Command0Through9Receiver> {
    ...
    struct unique_ptr<Safari::FileLocker, std::default_delete<Safari::FileLocker>> { struct { struct FileLocker *__ptr_; } ; } _fileLocker;
    ...
    BOOL _isReloadingManagedBookmarks;
}

@property (copy, nonatomic) SHA256Hash *builtInBookmarksStateHash;
...
+ (long long)bookmarkSortedByDate:(id)a0 with:(id)a1;
...
- (void)_addManagedBookmarkList:(id)a0 toFolder:(id)a1;
- (void)_addManagedBookmarksIfNeeded;
...
- (BOOL)bookmarkIsEligibleForRecentlySaved:(id)a0;
...
- (void)fetchRecentlySavedBookmarksSortedByDate:(id /* block */)a0;
...
- (id)parentFolderOfAllManagedBookmarks;
...
- (BOOL)shouldDisplayBookmarkInUI:(id)a0;
