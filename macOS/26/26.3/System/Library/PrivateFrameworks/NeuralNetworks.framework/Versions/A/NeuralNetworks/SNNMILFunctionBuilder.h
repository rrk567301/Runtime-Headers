...
@interface SNNMILFunctionBuilder : NSObject {
    struct map<std::string, const MIL::IRValueType *, std::less<std::string>, std::allocator<std::pair<const std::string, const MIL::IRValueType *>>> { struct __tree<std::__value_type<std::string, const MIL::IRValueType *>, std::__map_value_compare<std::string, std::__value_type<std::string, const MIL::IRValueType *>, std::less<std::string>>, std::allocator<std::__value_type<std::string, const MIL::IRValueType *>>> { void *__begin_node_; struct { struct __tree_end_node<std::__tree_node_base<void *> *> { void *__left_; } __end_node_; } ; struct { unsigned long long __size_; } ; } __tree_; } _inputs;
    struct vector<std::string, std::allocator<std::string>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _outputs;
    struct vector<std::shared_ptr<MIL::IROperation>, std::allocator<std::shared_ptr<MIL::IROperation>>> { void *__begin_; void *__end_; struct { void *__cap_; } ; } _operations;
    unsigned long long _symbolCounter;
}

@property (readonly, nonatomic) struct basic_string<char, std::char_traits<char>, std::allocator<char>> { struct { union __rep { struct __short { char x0[23]; unsigned char x1 : 7; unsigned char x2 : 1; } x0; struct __long { char *x0; unsigned long long x1; unsigned long x2 : 63; unsigned char x3 : 1; } x1; } x0; } x0; } milName;
...
- (void).cxx_destruct;
- (id).cxx_construct;
- (id)constantOperationWithValue:(struct unique_ptr<const MIL::IRValue, std::default_delete<const MIL::IRValue>> { struct { struct IRValue *x0; } x0; })a0 name:(id)a1 location:(id)a2;
...
- (struct unique_ptr<MIL::IRArgument, std::default_delete<MIL::IRArgument>> { struct { struct IRArgument *x0; } x0; })milArgumentForSNNMILValue:(id)a0;
- (id)nextSymbol;
