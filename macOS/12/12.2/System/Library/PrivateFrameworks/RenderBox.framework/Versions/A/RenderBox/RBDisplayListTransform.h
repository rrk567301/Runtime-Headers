@interface RBDisplayListTransform : NSObject <NSCopying> {
    struct DisplayListTransform { struct vector<RB::DisplayListTransform::Term, 1, unsigned int> { unsigned char _p[24]; struct Term *_p; unsigned int _size; unsigned int _capacity; } _terms; } _transform;
}
