@class NSVB_ViewServiceBehaviorProxy, NSDictionary, NSObject, NSServiceViewController, NSMutableArray, NSResponder, NSLayoutConstraint, NSString, NSCFRunLoopObserver, NSViewBridge, NSWindow, NSArray, NSAccessibilityRemoteUIElement, NSXPCInterface, NSXPCConnection;
...
@interface NSViewServiceMarshal : NSView <NSXPCListenerDelegate, AuxiliaryCallsService, NSViewServiceMarshal, NSVBXPCConnectionClient> {
    ...
    unsigned char _hostWindowHasSheet : 1;
    ...
    unsigned char _remoteAccessoryViewCanBecomeKeyView : 1;
    ...
    unsigned char _setStyleMaskInProgress : 1;
    unsigned char _setWindowFrameInProgress : 1;
    ...
    unsigned char _transactionBegun : 1;
    ...
    unsigned char _marshalIgnoredMouseEventsDuringBootstrap : 1;
}

@property (readonly) BOOL hostWindowHasSheet;
...
@property (retain) NSDictionary *accessoryAccessibilityChildren;
...
+ (void)whenWindow:(id)a0 isAssociatedWithRemoteViewPerform:(id /* block */)a1;
...
- (void)requestFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })a0 transaction:(id)a1 animate:(BOOL)a2 completion:(id /* block */)a3;
...
- (void)bootstrap:(struct { id x0; id x1; id x2; id x3; id x4; double x5; struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x6; unsigned long long x7; unsigned int x8; BOOL x9; BOOL x10; BOOL x11; id x12; BOOL x13; })a0 withReply:(id /* block */)a1;
...
- (void)setServiceObject:(id)a0 forKey:(id)a1 withReply:(id /* block */)a2;
...
- (void)remoteViewBecameFirstResponder:(unsigned long long)a0 forWindowWithKey:(BOOL)a1 inActiveApp:(BOOL)a2 inVisibleWindow:(BOOL)a3 reply:(id /* block */)a4;
- (void)remoteViewResignedFirstResponderInActiveApp:(BOOL)a0 inFavorOf:(unsigned char)a1;
- (void)remoteViewWillStartLiveResize;
- (void)remoteViewDidEndLiveResize:(id /* block */)a0;
- (void)hostWindowBecameKeyInActiveApp:(BOOL)a0 withSheet:(BOOL)a1 firstResponderState:(unsigned char)a2 isContentView:(BOOL)a3;
- (void)hostWindowResignedKeyFocus:(BOOL)a0 inActiveApp:(BOOL)a1 isContentView:(BOOL)a2;
...
- (void)didBecomeAssociatedWithHostWindow:(unsigned int)a0 withKeyness:(BOOL)a1 isFirstResponder:(BOOL)a2 withSheet:(BOOL)a3 inActiveApp:(BOOL)a4;
- (void)setAccessoryViewSize:(struct CGSize { double x0; double x1; })a0 alignmentRectInsets:(struct NSEdgeInsets { double x0; double x1; double x2; double x3; })a1;
- (void)setRemoteAccessoryViewCanBecomeKeyView:(BOOL)a0;
- (void)accessibilityTokenForFocusedUIElement:(id /* block */)a0;
- (void)allowAppNap:(BOOL)a0;
- (void)remoteViewIsContentView:(BOOL)a0;
- (void)invokeBlockOnCurrentFenceIfAny:(id /* block */)a0;
- (void)refreshAccessoryViewBitmap:(id)a0;
...
- (void)forwardPotentialCommandEquivalentToAuxService:(id)a0 eventDeliveredByAppKit:(BOOL)a1 reply:(id /* block */)a2;
- (void)serviceWindowWouldActivate;
- (void)beginHostAppModalSession:(id)a0 parameters:(const struct { struct { unsigned char x0; struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x1; unsigned int x2; unsigned long long x3; } x0; } *)a1 title:(id)a2 style:(unsigned long long)a3 size:(struct CGSize { double x0; double x1; })a4 withReply:(id /* block */)a5;
...
- (void)setRemoteObject:(id)a0 forKey:(id)a1;
...
- (void)orderWindow:(id)a0 mode:(long long)a1 relativeTo:(long long)a2 findKey:(BOOL)a3 forCounter:(BOOL)a4 force:(BOOL)a5 isModal:(BOOL)a6;
...
- (int)hostWindowBelongsToTopAppModalSession:(id)a0;
...
- (void)satisfyPerformersAwaitingAssociation;
...
- (void)completeWithError:(id)a0 andRequestedSize:(struct CGSize { double x0; double x1; })a1 withRemoteViewSize:(struct CGSize { double x0; double x1; })a2 andWindow:(id)a3;
- (id)viewBridgeAuxiliaryProxyWithErrorHandler:(id /* block */)a0;
- (void)addChildWindow:(id)a0;
- (BOOL)windowIsOrderingIn;
- (void)hostAppIsActive:(BOOL)a0;
- (unsigned char)remoteViewBecameFirstResponder:(unsigned long long)a0 forWindowWithKey:(BOOL)a1 inVisibleWindow:(BOOL)a2;
- (void)remoteViewChangedFirstResponder:(id /* block */)a0;
- (void)remoteViewResignedFirstResponderInFavorOfAccessoryView:(BOOL)a0;
- (void)informHostOfChangeToFirstResponder:(id)a0 forWindow:(id)a1 inDirection:(unsigned long long)a2;
...
- (void)observeValueForAccessoryView:(id)a0;
...
- (void)orderingWindow:(id /* block */)a0;
- (void)becomeKeyBecause:(const char *)a0;
- (void)hostWindowChangedKeyness:(id /* block */)a0;
- (void)acquireKeyAppearanceBecauseHostWindow:(const char *)a0;
...
- (void)remoteViewGeometryDidChangeDueToTransaction:(id)a0 serviceWindowSize:(struct CGSize { double x0; double x1; } *)a1 oldRemoteViewFrameSize:(struct CGSize { double x0; double x1; })a2;
- (void)_remoteViewGeometryDidChange:(id)a0 serviceWindowSize:(struct CGSize { double x0; double x1; } *)a1 oldRemoteViewFrameSize:(struct CGSize { double x0; double x1; })a2;
- (void)afterResignKey;
- (void)hostWindowResignedKey:(BOOL)a0;
- (BOOL)responder:(id)a0 isAccessoryViewOf:(id)a1;
...
- (void)_bootstrap:(const struct { id x0; id x1; id x2; id x3; id x4; double x5; struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } x6; unsigned long long x7; unsigned int x8; BOOL x9; BOOL x10; BOOL x11; id x12; BOOL x13; } *)a0 replyData:(id)a1 completion:(id /* block */)a2;
...
- (id)updateAccessoryViewAccessibility:(id)a0 enhancedUserInterface:(id)a1;
- (void)serviceAccessoryViewBecameFirstResponder:(BOOL)a0;
