...
@interface PSEVideoProcessor : NSObject {
    ...
    struct ConstBuf { float fPixelCnt; BOOL bMitigate; BOOL bDebug; BOOL bCopyOnly; BOOL bVideoRange; BOOL bProtected; unsigned int uEOTF; unsigned int uColSpace; float maxNits; float edr; float displayEDR; float fps; void /* unknown type, empty encoding */ u2CbCrSubSampleRate; BOOL bManualCbCr; float fNIU2; } _bufferConstants;
    float _fps;
    float _nits;
    float _area;
    float _avl;
