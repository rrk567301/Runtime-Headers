@class NSCharacterSet, NSMutableDictionary, SCROutputManager, SCRWorkspace, SCRCTargetSelectorTimer, SCRGestureFactory, NSString, NSSet, SCRCIndexMap, SCRKeyboardKey, SCREventCapsLockDetector, AXSSEventManager, SCRBezelServicesManager, SCRCUserDefaults, SCREvent, AXSSActionManager, NSMutableSet;

@interface SCREventFactory : NSObject <SCRCursorAutoHideDelegate> {
    ...
    struct { long long lastState; BOOL isTrackpadCommanderEnabled; unsigned long long continuityHash; long long continuityZone; double continuityDistance; double continuityThreshold; unsigned long long repeatHash; unsigned long long pressRepeatHash; struct CGPoint { double x; double y; } trackingLocation; double lastEventTime; double lastCommandTime; double lastClickTime; double lastCursorUpdateTime; BOOL isCursorHidden; BOOL snarfingEvents; struct CGPoint { double x; double y; } cursorLocation; BOOL tapDragEnabled; double tapDragStartTime; unsigned long long mouseDownCount; BOOL mouseDownIsRight; unsigned long long fingerCount; unsigned long long tapCount; long long axisDirection; BOOL isControlled; BOOL isCommanded; long long rotorSoundZone; BOOL requireUp; BOOL onlySpeakWhenTouching; BOOL isInPassthruTouchMode; struct CGPoint { double x; double y; } passthruTouchStartPoint; } _gesture;
    SCRCTargetSelectorTimer *_mouseDownTimer;
    SCRCIndexMap *_uniCharToVirtualKeysMap;
    unsigned long long _repostedMacKeyCode;
    struct { unsigned char controlOnly : 1; unsigned char eventCaptureEnabled : 1; unsigned char fastSearchEnabled : 1; unsigned char invalid : 1; unsigned char kbCommanderEnabled : 1; unsigned char kbCommanderTracking : 1; unsigned char keyboardPassthruEnabled : 1; unsigned char modalSessionEnabled : 1; unsigned char nonArrowQuickNavEnabled : 1; unsigned char numPadCommanderEnabled : 1; unsigned char quickNavOverridesSelectionEvents : 1; unsigned char speakAfterTextInsertionEnabled : 1; unsigned char vocFollowsKeyWindow : 1; unsigned char itCommanderEnabled : 1; unsigned char itCommanderCaptureEnabled : 1; unsigned char itCommanderTemporarilyDisabled : 1; } _flags;
...
@property (retain, nonatomic) NSString *_lastITCommand;
...
@property (nonatomic) BOOL screenSaverEnabled;
...
- (void)setQuickNavTemporarilyDisabled:(BOOL)a0;
- (void)_delayedHandleEvent:(id)a0;
- (void)_delayedWorkspaceDidChange;
- (BOOL)_dispatchInvertedTCommand:(id)a0;
...
- (BOOL)_handleInvertedTArrowEvent:(struct __CGEvent { } *)a0;
- (void)_handleInvertedTCommanderPress;
- (void)_handleInvertedTDownArrowRepostPress:(id)a0;
- (BOOL)_handleInvertedTEvent:(struct __CGEvent { } *)a0;
- (BOOL)_handleInvertedTNonArrowEvent:(struct __CGEvent { } *)a0;
