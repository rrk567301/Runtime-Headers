@class NSMutableDictionary, VCSessionParticipantMediaStreamInfo, NSDictionary, NSObject, TimingCollection, NSString, VCAudioCaptionsCoordinator, VCAudioPowerSpectrumSource, VCPositionalInfo, NSArray, NSNumber, VCSessionBandwidthAllocationTable;
...
+ (unsigned int)maxCameraNetworkBitrateForVideoQuality:(unsigned char)a0 isLocalOnWiFi:(BOOL)a1 isRedundancyRequested:(BOOL)a2;
+ (unsigned int)maxFtxtNetworkBitrateForVideoQuality:(unsigned char)a0 isLocalOnWiFi:(BOOL)a1 isRedundancyRequested:(BOOL)a2;
+ (unsigned int)maxNetworkBitrateForStreamGroupID:(unsigned int)a0 videoQuality:(unsigned char)a1 isLocalOnWiFi:(BOOL)a2 isRedundancyRequested:(BOOL)a3;
