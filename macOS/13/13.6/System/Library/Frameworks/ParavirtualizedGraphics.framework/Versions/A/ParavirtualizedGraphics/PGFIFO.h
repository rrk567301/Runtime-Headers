@class NSThread, _PGDevice;
@protocol MTLEvent;

@interface PGFIFO : NSObject {
    _PGDevice *_device;
    BOOL _running;
    struct { unsigned int stampValue; unsigned int commandID; unsigned int signaled; } _pendingStamps[32];
    unsigned int _pendingStampWriteCount;
    unsigned int _pendingStampReadCount;
    unsigned int _lastPendingStamp;
    _Atomic unsigned int *_lastPending;
...
- (BOOL)stampAtHeadLocked:(unsigned int)a0 offset:(unsigned int)a1;
