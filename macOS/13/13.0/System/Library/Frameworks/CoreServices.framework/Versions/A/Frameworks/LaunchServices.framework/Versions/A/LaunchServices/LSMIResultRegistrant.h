@class NSUUID, NSDictionary;
@protocol LSMIResultRegistrantStrategy;

@interface LSMIResultRegistrant : NSObject {
    id<LSMIResultRegistrantStrategy> _strategy;
    NSUUID *_uuid;
    NSDictionary *_miDict;
}

- (void).cxx_destruct;
- (id)initWithContext:(id)a0 operationUUID:(id)a1 itemInfoDict:(id)a2;
- (void)_replyWithError:(id)a0 onQueue:(id)a1 block:(id /* block */)a2;
- (void)runPostProcessingForBundleID:(id)a0 success:(BOOL)a1 isSystemApp:(BOOL)a2 isPlaceholder:(BOOL)a3 notificationJournaller:(id)a4;
- (void)runWithCompletion:(id /* block */)a0;

@end
