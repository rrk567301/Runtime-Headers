@class FIBrowserOverlayStackViewController, NSStackView, FI_TBrowserContentViewController, NSURL, NSObject, FIContainerController, FI_NewFolderController, FIFinderView, FI_TNotificationCenterObserverGlue, NSString, NSArray, FI_TUpdateLayerView, NSValue;
...
@interface FIFinderViewGutsController : FI_TViewController <FILocationPopUpDelegate, FIBrowserOverlayStackViewDelegate, NSMenuDelegate, NSMenuItemValidation, BrowserContainerTargeting, BrowserContainerSearching, BrowserContainerDelegate> {
    ...
    struct TNSWeakPtr<FI_TRunAfterHelper> { NSValue *fWeakObject; } _pendingWindowOrderedOutToken;
    ...
    BOOL _hideExtension;
    struct TNSWeakPtr<FI_TRunAfterHelper> { NSValue *fWeakObject; } _handleInitialTargetingToken;
    struct TNSWeakPtr<FI_TRunAfterHelper> { NSValue *fWeakObject; } _historyBeginsNowToken;
...
@property (nonatomic) BOOL enableAppCentricFeatures;
...
- (BOOL)containerIsCompatibleWithSavePanel:(const struct TFENode { struct OpaqueNodeRef *x0; } *)a0;
...
- (struct TString { struct TRef<const __CFString *, TRetainReleasePolicy<CFStringRef>> { struct __CFString *x0; } x0; })extensionForPanel:(id)a0;
